...
fn is_workspace_crate_name(name: &str) -> bool {
    name.starts_with("sofia_") || matches!(name, "app_test_support" | "core_test_support")
}
...
fn workspace_crate_filter_accepts_first_party_names_only() {
    assert!(is_workspace_crate_name("sofia_core"));
    assert!(is_workspace_crate_name("sofia_tui"));
