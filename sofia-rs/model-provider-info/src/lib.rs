//! Registry of model providers supported by Sofia.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Sofia works out-of-the-box.
//!   2. User-defined entries inside `~/.sofia/config.toml` under the `model_providers`
...
pub const CHATGPT_CODEX_BASE_URL: &str = "https://chatgpt.com/backend-api/sofia";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "sofia";
...
impl ModelProviderInfo {
    ...
    /// `$SOFIA_HOME`, then `~/.sofia`, then `~/.config/sofia`, then `~/.sofia`.
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These SOFIA_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_codex_oss_base_url = format!(
        "http://localhost:{sofia_oss_port}/v1",
        sofia_oss_port = std::env::var("SOFIA_OSS_PORT")
            .ok()
    ...
    let sofia_oss_base_url = std::env::var("SOFIA_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_codex_oss_base_url);
    create_oss_provider_with_base_url(&sofia_oss_base_url, wire_api)
}
...
/// Read the API key for `env_key` from `sofia-auth.json`, if present.
/// Searches `SOFIA_HOME`, then legacy `~/.sofia` and `~/.config/sofia`.
