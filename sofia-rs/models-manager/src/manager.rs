...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use Sofia backend-only models.
...
impl OpenAiModelsManager {
    ...
    pub fn new(
        ...
        let cache_path = sofia_home.join(model_cache_file_name(
...
impl OpenAiModelsManager {
    ...
    async fn refresh_available_models(
        ...
        // (Sofia backend / command-auth). Custom `model_providers` entries (e.g.
    ...
    async fn try_load_cache(&self) -> bool {
        ...
        let _timer =
            sofia_otel::start_global_timer("sofia.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    // retry for namespaced slugs like `custom/gpt-5.3-sofia`.
    ...
    let model_info = if let Some(remote) = remote {
        ...
        if let Some(ref mut messages) = remote.model_messages {
            ...
            remote.model_messages = Some(sofia_protocol::openai_models::ModelMessages {
