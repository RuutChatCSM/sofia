...
use sofia_connectors::ConnectorMetadata;
use sofia_connectors::ConnectorMetadataStore;
use sofia_connectors::ConnectorToolSummary;
use sofia_connectors::DirectoryListResponse;
use sofia_connectors::merge::merge_connectors;
use sofia_connectors::merge::merge_plugin_connectors;
use sofia_core::config::Config;
pub use sofia_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use sofia_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use sofia_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use sofia_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use sofia_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use sofia_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "sofia";
...
async fn connector_auth(config: &Config) -> anyhow::Result<CodexAuth> {
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Sofia backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = sofia_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = sofia_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_codex_backend(),
        "ChatGPT backend requests require Sofia backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run sofia login"
    );
...
mod tests {
    ...
    use serde_json::json;
    use sofia_connectors::metadata::connector_install_url;
    use sofia_plugin::AppConnectorId;
