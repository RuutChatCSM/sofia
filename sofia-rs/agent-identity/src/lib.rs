...
use sofia_http_client::HttpError;
use sofia_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "sofia-app-server";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"sofia-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/backend-api/sofia"
            | "https://chat.openai.com"
            | "https://chat.openai.com/backend-api"
            | "https://chat.openai.com/sofia"
            | "https://chat.openai.com/backend-api/sofia" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/sofia"
            | "https://chatgpt-staging.com/backend-api/sofia" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single Sofia run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "sofia-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "sofia-cli".to_string(),
        },
...
mod tests {
    ...
    use sofia_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://chatgpt.com/backend-api/sofia")?,
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/sofia"),
            "http://localhost:8080/api/sofia/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/sofia/"),
            "http://localhost:8080/api/sofia/agent-identities/jwks"
