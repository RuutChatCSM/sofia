...
use serde_json::json;
use sofia_backend_client::ConfigBundleResponse;
use sofia_backend_client::DeliveredTomlFragment;
use sofia_config::AbsolutePathBuf;
use sofia_config::CloudConfigFragment;
use sofia_config::CloudConfigTomlBundle;
use sofia_config::CloudRequirementsFragment;
use sofia_config::CloudRequirementsTomlBundle;
...
use sofia_login::auth::ExternalAuth;
use sofia_login::auth::ExternalAuthRefreshContext;
...
fn write_auth_json(sofia_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(sofia_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(sofia_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(sofia_home, "/"))
}
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let sofia_home = tempdir().expect("tempdir");
    let cache = create_test_cache(sofia_home.path());
    cache
        .save(
...
async fn get_bundle_without_cache_fails_closed_on_request_failure() {
    let sofia_home = tempdir().expect("tempdir");
    create_test_cache(sofia_home.path())
        .save(
            Some("user-12345".to_string()),
            Some("account-12345".to_string()),
            test_bundle(),
...
async fn production_loader_recovers_startup_timeouts_in_background() {
    ...
    assert_eq!(
        ...
        sofia_protocol::protocol::AskForApproval::Never
    );
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(sofia_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            sofia_backend_client::DeliveredRequirementsToml {
