...
use sofia_config::CloudConfigBundle;
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let sofia_home = AbsolutePathBuf::resolve_path_against_base(sofia_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(sofia_home.clone()),
            cache_enabled: true,
            sofia_home,
            timeout,
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            sofia_otel::start_global_timer("sofia.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.sofia_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.sofia_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
