...
use sofia_login::AuthEnvTelemetry;
...
/// Filename used for the redacted `sofia doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "sofia-doctor-report.json";
/// Filename used for the raw Sofia Apps MCP tools cache feedback attachment.
pub const SOFIA_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "sofia-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const SOFIA_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "sofia-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_codex_api_key_present = tracing::field::debug(auth_env.sofia_api_key_env_present),
        auth_env_codex_api_key_enabled = tracing::field::debug(auth_env.sofia_api_key_env_enabled),
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("sofia_http_client::transport", LevelFilter::DEBUG)
                    .with_target("sofia_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("sofia_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("sofia_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `sofia-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn log_attachment(&self, logs_override: Option<Vec<u8>>) -> FeedbackAttachment {
        FeedbackAttachment {
            filename: "sofia-logs.log".to_string(),
...
mod tests {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "sofia_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "sofia_http_client::transport", "transport-trace");
        tracing::trace!(target: "sofia_api::sse", "sse-trace");
        tracing::trace!(target: "sofia_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "sofia_http_client::transport", "transport-debug");
        tracing::debug!(target: "sofia_api::sse::responses", "sse-debug");
        tracing::trace!(target: "sofia_feedback_test", "unrelated-trace");
        log::trace!(target: "sofia_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<sofia_http_client::RouteAwareRequestError>()
                .is_some_and(sofia_http_client::RouteAwareRequestError::is_connect)
        );
    }

    #[test]
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("sofia-feedback-extra-{}.jsonl", ThreadId::new());
    ...
    fn path_backed_attachments_use_binary_content_types() {
        let suffix = ThreadId::new();
        let gzip_filename = format!("sofia-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("sofia-feedback-extra-{suffix}.binunknown");
