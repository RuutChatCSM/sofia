...
use sofia_exec_server_protocol::DISCOVERABLE_PLUGIN_MANIFEST_PATHS;
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".sofia-plugin/plugin.json");
    ...
    fn rejects_nonregular_legacy_plugin_manifest_before_lower_precedence_manifest() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let sofia_path = plugin_root.join(".sofia-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(&sofia_path).expect("nonregular Sofia manifest");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_before_lower_precedence_manifest() {
        ...
        let sofia_path = plugin_root.join(".sofia-plugin/plugin.json");
        let claude_path = plugin_root.join(ALTERNATE_PLUGIN_CLA_MANIFEST_RELATIVE_PATH);
        fs::create_dir_all(sofia_path.parent().expect("Sofia manifest parent"))
            .expect("Sofia manifest parent");
        fs::create_dir_all(claude_path.parent().expect("Claude manifest parent"))
            .expect("Claude manifest parent");
        fs::write(plugin_root.join("benign.json"), r#"{"name":"sample"}"#)
            .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink("../benign.json", &sofia_path).expect("Sofia manifest symlink");
    ...
    fn rejects_symlinked_legacy_plugin_manifest_directory_before_lower_precedence_manifest() {
        ...
        .expect("benign manifest");
        fs::write(&claude_path, r#"{"name":"sample"}"#).expect("Claude manifest");
        std::os::unix::fs::symlink(&manifest_directory, plugin_root.join(".sofia-plugin"))
            .expect("Sofia manifest directory symlink");
    ...
    fn preserves_codex_claude_cursor_legacy_precedence() {
        ...
        for path in [&sofia_path, &claude_path, &cursor_path] {
        ...
        assert_eq!(
            ...
            Some(sofia_path.clone())
        );
        fs::remove_file(sofia_path).expect("remove Sofia manifest");
