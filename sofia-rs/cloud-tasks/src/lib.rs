...
use sofia_cloud_tasks_client::TaskStatus;
use sofia_git_utils::current_branch_name;
use sofia_git_utils::default_branch_name;
...
use sofia_login::default_client::get_codex_user_agent;
...
struct ApplyJob {
    task_id: sofia_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn sofia_cloud_tasks_client::CloudBackend>,
    base_url: String,
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let use_mock = matches!(
        std::env::var("SOFIA_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    let base_url = std::env::var("SOFIA_CLOUD_TASKS_BASE_URL")
    ...
    if use_mock {
        ...
        return Ok(BackendContext {
            backend: Arc::new(sofia_cloud_tasks_mock_client::MockClient),
            base_url,
    ...
    let mut http = sofia_cloud_tasks_client::HttpClient::new(base_url.clone(), http_client_factory)
        .with_user_agent(ua);
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'sofia login' to sign in with ChatGPT, then re-run 'sofia cloud'."
            );
            std::process::exit(1);
        }
    };
    ...
    let auth_provider = sofia_model_provider::auth_provider_from_auth(&auth);
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("sofia_cloud_tasks_exec").await?;
    ...
    let created = sofia_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `sofia cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `sofia cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn parse_task_id(raw: &str) -> anyhow::Result<sofia_cloud_tasks_client::TaskId> {
    ...
    Ok(sofia_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn sofia_cloud_tasks_client::CloudBackend,
    task_id: &sofia_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        sofia_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        sofia_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = sofia_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &sofia_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &sofia_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[sofia_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("sofia_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        sofia_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    let ctx = init_backend("sofia_cloud_tasks_list").await?;
    let env_filter = if let Some(env) = args.environment {
        Some(resolve_environment_id(&ctx, &env).await?)
    } else {
        None
    };
    let page = sofia_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("sofia cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("sofia_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("sofia_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = sofia_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        sofia_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: sofia_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        sofia_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        sofia_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        sofia_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = sofia_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = sofia_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `sofia cloud` subcommand.
pub async fn run_main(cli: Cli, _codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext {
        ...
        base_url,
        environment_http,
    } = init_backend("sofia_cloud_tasks_tui").await?;
    ...
    let force_internal = matches!(
        std::env::var("SOFIA_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx
                            .send(Instant::now() + sofia_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match sofia_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, sofia_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let sofia_tui::ComposerAction::Submitted(text) =
                                        page.composer.input(key)
                                    {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = sofia_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, /*qa_mode*/ false, best_of_n).await;
                                    ...
                                    if page.composer.is_in_paste_burst() {
                                        let _ = frame_tx.send(
                                            Instant::now()
                                                + sofia_tui::ComposerInput::recommended_flush_delay(),
                                        );
                                    }
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match sofia_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match sofia_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = sofia_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match sofia_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
mod tests {
    ...
    use sofia_cloud_tasks_client::DiffSummary;
    use sofia_cloud_tasks_client::TaskId;
    use sofia_cloud_tasks_client::TaskStatus;
    use sofia_cloud_tasks_client::TaskSummary;
    use sofia_cloud_tasks_mock_client::MockClient;
