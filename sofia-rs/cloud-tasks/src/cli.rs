...
pub enum Command {
    /// Submit a new Sofia Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Sofia Cloud task.
    Status(StatusCommand),
    /// List Sofia Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Sofia Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Sofia Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Sofia Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `sofia cloud` to browse).
    ...
    /// Git branch to run in Sofia Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Sofia Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Sofia Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Sofia Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
