...
impl RuntimeDbSpec {
    fn path(self, sofia_home: &Path) -> PathBuf {
        sofia_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Sofia SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Sofia SQLite database, creating it if necessary.
    ...
    /// Open an existing Sofia SQLite database without creating or modifying it.
