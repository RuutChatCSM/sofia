use super::persistence::SOFIA_APPS_TOOLS_CACHE_MAX_BYTES;
use super::persistence::SOFIA_APPS_TOOLS_CACHE_SCHEMA_VERSION;
...
const SOFIA_APPS_MCP_SERVER_NAME: &str = "sofia_apps";
...
#[test]
fn sofia_apps_tools_cache_is_overwritten_by_last_write() {
    ...
    let tools_gateway_1 = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "two")];
...
#[test]
fn sofia_apps_tools_cache_is_scoped_per_user() {
    let sofia_home = tempdir().expect("tempdir");
    ...
    let tools_user_1 = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "two")];
...
#[test]
fn sofia_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let sofia_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn sofia_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let sofia_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": SOFIA_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
...
#[test]
fn sofia_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let sofia_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
        Some("account-one"),
...
fn startup_cached_codex_apps_tools_loads_from_disk_cache() {
    ...
    assert_eq!(startup_tools[0].server_name, SOFIA_APPS_MCP_SERVER_NAME);
...
fn startup_cached_codex_apps_tools_loads_without_server_info_cache() {
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": SOFIA_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
    let cache_context = create_codex_apps_tools_cache_context(
...
#[test]
fn sofia_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let sofia_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn sofia_apps_tools_cache_context_does_not_reread_disk_after_creation() {
    let sofia_home = tempdir().expect("tempdir");
    let writer_cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "cached")];
    ...
    let updated_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "updated")];
...
#[test]
fn sofia_apps_tools_cache_publishes_newest_shared_snapshot() {
    let sofia_home = tempdir().expect("tempdir");
    ...
    let server_info = create_test_server_info("Sofia Apps");
    let newer_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "newer")];
    let older_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "older")];
...
#[test]
fn sofia_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let sofia_home = tempdir().expect("tempdir");
    let sofia_home_file = sofia_home.path().join("not-a-directory");
    std::fs::write(&sofia_home_file, b"occupied").expect("create sofia home file");
    ...
    let tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "live")];
...
fn connector_runtime_without_cache_ignores_disk_state() {
    ...
    let tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "cached")];
    let server_info = create_test_server_info("Sofia Apps");
...
fn connector_runtime_without_cache_publishes_without_writing() {
    ...
    assert!(!sofia_home.exists());
}

#[cfg(unix)]
#[test]
fn sofia_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    let sofia_home = PathBuf::from(std::ffi::OsString::from_vec(
        b"/tmp/sofia-home-\xff".to_vec(),
    ));
    let cache = ConnectorRuntimeManager::<TestTool>::default();
...
fn contexts_for_different_identities_keep_isolated_snapshots() {
    ...
    let tools_a = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "tool-a")];
    ...
    let tools_b = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "tool-b")];
    let snapshot_b = context_b.publish_runtime_if_newest_accepted(
        context_b.begin_fetch(ConnectorRuntimeFetchSource::HardRefresh),
        &create_test_server_info("Sofia Apps"),
        tools_b.clone(),
    );
    let newer_tools_a = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "newer-a")];
    ...
    let stale_snapshot_a = context_a.publish_runtime_if_newest_accepted(
        older_ticket_a,
        &create_test_server_info("Sofia Apps"),
        vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "stale-a")],
    );
...
fn oversized_tools_cache_is_ignored_during_initial_load() {
    ...
    file.set_len(SOFIA_APPS_TOOLS_CACHE_MAX_BYTES + 1)
...
fn accepted_generations_finish_persistence_in_order() {
    ...
    let older_publish = std::thread::spawn(move || {
        older_context.publish_runtime_if_newest_accepted_with(
            older_ticket,
            &create_test_server_info("Sofia Apps"),
            vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "older")],
            move |_, _, _| {
                older_persisting_tx
                    .send(())
    ...
    let newer_publish = std::thread::spawn(move || {
        newer_context.publish_runtime_if_newest_accepted_with(
            newer_ticket,
            &create_test_server_info("Sofia Apps"),
            vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "newer")],
            move |_, _, _| {
                newer_persisting_tx
                    .send(())
...
fn personal_and_workspace_contexts_are_distinct_even_with_matching_ids() {
    ...
    let personal_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "personal")];
    ...
    let workspace_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "workspace")];
...
fn live_publish_sets_timestamp_and_stale_publish_preserves_it() {
    ...
    let current = context.publish_runtime_if_newest_accepted(
        current_ticket,
        &create_test_server_info("Sofia Apps"),
        vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "current")],
    );
    ...
    let stale = context.publish_runtime_if_newest_accepted(
        stale_ticket,
        &create_test_server_info("Sofia Apps"),
        vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "stale")],
    );
