...
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "sofia.mcp.tools.cache_write.duration_ms";
const SOFIA_APPS_TOOLS_CACHE_DIR: &str = "cache/sofia_apps_tools";
pub(crate) const SOFIA_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 4;
const SOFIA_APPS_SERVER_INFO_CACHE_DIR: &str = "cache/sofia_apps_server_info";
const SOFIA_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION: u8 = 1;
pub(crate) const SOFIA_APPS_TOOLS_CACHE_MAX_BYTES: u64 = 32 * 1024 * 1024;

pub(crate) fn tools_cache_path(identity: &ConnectorRuntimeIdentity) -> PathBuf {
    cache_path_in(identity, SOFIA_APPS_TOOLS_CACHE_DIR)
}

pub(crate) fn server_info_cache_path(identity: &ConnectorRuntimeIdentity) -> PathBuf {
    cache_path_in(identity, SOFIA_APPS_SERVER_INFO_CACHE_DIR)
}

fn cache_path_in(identity: &ConnectorRuntimeIdentity, cache_dir: &str) -> PathBuf {
    // `sofia_home` is already the parent directory. Keep it out of the
...
pub(crate) fn load_cached_connector_runtime_for_identity<T: ConnectorRuntimePayload>(
    ...
    (cache.schema_version == SOFIA_APPS_TOOLS_CACHE_SCHEMA_VERSION).then_some(
...
pub(crate) fn load_cached_codex_apps_server_info<T: ConnectorRuntimePayload>(
    ...
    (cache.schema_version == SOFIA_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION)
...
fn write_cached_codex_apps_server_info<T: ConnectorRuntimePayload>(
    ...
    let bytes = serde_json::to_vec_pretty(&CodexAppsServerInfoDiskCache {
        schema_version: SOFIA_APPS_SERVER_INFO_CACHE_SCHEMA_VERSION,
        server_info: server_info.clone(),
    })
    .context("failed to serialize Sofia Apps server info cache")?;
    ...
    T: ConnectorRuntimePayload,
{
    ...
    if let Err(err) = &server_info_result {
        tracing::warn!("failed to write Sofia Apps server info cache: {err:#}");
    }
...
fn read_bounded_cache_file(cache_path: &Path) -> anyhow::Result<(Vec<u8>, SystemTime)> {
    ...
    if metadata.len() > SOFIA_APPS_TOOLS_CACHE_MAX_BYTES {
        return Err(anyhow!(
            "cache `{}` is {} bytes, exceeding the {} byte limit",
            cache_path.display(),
            metadata.len(),
            SOFIA_APPS_TOOLS_CACHE_MAX_BYTES
        ));
    }
    let mut bytes = Vec::with_capacity(metadata.len() as usize);
    std::io::Read::by_ref(&mut file)
        .take(SOFIA_APPS_TOOLS_CACHE_MAX_BYTES + 1)
        .read_to_end(&mut bytes)
        .with_context(|| format!("failed to read cache `{}`", cache_path.display()))?;
    if bytes.len() as u64 > SOFIA_APPS_TOOLS_CACHE_MAX_BYTES {
        return Err(anyhow!(
...
fn write_codex_apps_cache_file(
    ...
    let parent = cache_path.parent().ok_or_else(|| {
        anyhow!(
            "Sofia Apps {cache_name} cache path `{}` has no parent",
            cache_path.display()
        )
    })?;
    std::fs::create_dir_all(parent).with_context(|| {
        format!(
            "failed to create Sofia Apps {cache_name} cache directory `{}`",
            parent.display()
        )
    })?;
    let mut temporary = NamedTempFile::new_in(parent).with_context(|| {
        format!(
            "failed to create temporary Sofia Apps {cache_name} cache in `{}`",
            parent.display()
        )
    })?;
    temporary.write_all(&bytes).with_context(|| {
        format!(
            "failed to write temporary Sofia Apps {cache_name} cache for `{}`",
            cache_path.display()
        )
    })?;
    temporary.persist(cache_path).map_err(|error| {
        anyhow!(
            "failed to atomically replace Sofia Apps {cache_name} cache `{}`: {}",
            cache_path.display(),
            error.error
        )
    })?;
