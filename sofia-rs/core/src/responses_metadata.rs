...
/// Caller-owned snapshot of Sofia metadata sent to ResponsesAPI.
///
/// The full Sofia turn metadata blob is transported canonically as
/// `client_metadata["x-sofia-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
...
impl CodexResponsesMetadata {
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        // Direct x-sofia-turn-metadata is compatibility output. Keep the unbounded tool inventory
    ...
    fn turn_metadata_payload(&self) -> CodexTurnMetadataPayload<'_> {
        ...
        CodexTurnMetadataPayload {
            ...
            compaction,
            // Extra metadata enriches the Sofia turn metadata blob, not literal top-level
            // Responses client_metadata. Product metadata is validated while loading config;
            // app-server metadata has reserved Sofia-owned keys filtered when it enters turn state.
            extra: &self.extra,
        }
    }
}
