...
//!
//! `ModelClient` is intended to live for the lifetime of a Sofia session and holds the stable
...
//! per-turn state such as the `x-sofia-turn-state` token used for sticky routing.
//!
...
use sofia_api::MemoriesClient as ApiMemoriesClient;
use sofia_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use sofia_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use sofia_api::Provider as ApiProvider;
use sofia_api::RawMemory as ApiRawMemory;
use sofia_api::RealtimeCallClient as ApiRealtimeCallClient;
use sofia_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use sofia_api::Reasoning;
use sofia_api::ReasoningContext;
use sofia_api::RequestTelemetry;
use sofia_api::ReqwestTransport;
use sofia_api::ResponseCreateWsRequest;
use sofia_api::ResponsesApiRequest;
use sofia_api::ResponsesClient as ApiResponsesClient;
use sofia_api::ResponsesEndpoint;
use sofia_api::ResponsesOptions as ApiResponsesOptions;
use sofia_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use sofia_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use sofia_api::ResponsesWsRequest;
use sofia_api::SharedAuthProvider;
use sofia_api::SseTelemetry;
use sofia_api::StreamOptions;
use sofia_api::TransportError;
use sofia_api::WebsocketTelemetry;
use sofia_api::auth_header_telemetry;
use sofia_api::build_session_headers;
use sofia_api::create_text_param_for_request;
use sofia_api::response_create_client_metadata;
...
use sofia_login::RefreshTokenError;
use sofia_login::UnauthorizedRecovery;
use sofia_login::default_client::add_originator_header;
use sofia_login::default_client::create_client_for_route;
...
// ResponseItemId already imported above
...
use sofia_protocol::protocol::Event as ProtocolEvent;
use sofia_protocol::protocol::EventMsg;
use sofia_protocol::protocol::InternalSessionSource;
use sofia_protocol::protocol::SessionSource;
use sofia_protocol::protocol::W3cTraceContext;
use sofia_rollout_trace::CompactionTraceContext;
use sofia_rollout_trace::InferenceTraceAttempt;
use sofia_rollout_trace::InferenceTraceContext;
use sofia_tools::create_tools_json_for_responses_api;
use sofia_tools::create_tools_json_for_responses_lite;
use sofia_tools::create_tools_raw_json_for_responses_api;
...
use sofia_model_provider::AgentIdentitySessionFallback;
use sofia_model_provider::ProviderAuthScope;
use sofia_model_provider::ProviderUnauthorizedRecovery;
use sofia_model_provider::SharedModelProvider;
use sofia_model_provider::create_model_provider;
#[cfg(test)]
use sofia_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use sofia_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_CODEX_INSTALLATION_ID_HEADER: &str = "x-sofia-installation-id";
pub const X_CODEX_ROUTING_HINT_HEADER: &str = "x-sofia-routing-hint";
pub const X_CODEX_TURN_STATE_HEADER: &str = "x-sofia-turn-state";
pub const X_CODEX_TURN_METADATA_HEADER: &str = "x-sofia-turn-metadata";
pub const X_CODEX_PARENT_THREAD_ID_HEADER: &str = "x-sofia-parent-thread-id";
pub const X_CODEX_WINDOW_ID_HEADER: &str = "x-sofia-window-id";
...
const X_CODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-sofia-ws-stream-request-start-ms";
...
///
/// This holds configuration and state that should be shared across turns within a Sofia session
...
/// - The `x-sofia-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Sofia turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-sofia-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-sofia-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Sofia session. Per-turn values
    ...
    pub fn new(
        ...
        let sofia_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.sofia_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), sofia_api_key_env_enabled);
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "sofia.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: sofia_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: sofia_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &CodexResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth)
                ...
                .connect(
                    &self.http_client_factory,
                    headers,
                    sofia_login::default_client::default_headers(),
                    /*turn_state*/ None,
...
///
/// These headers implement Sofia-specific conventions:
///
/// - `x-sofia-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-sofia-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-sofia-beta-features", header_value);
    }
...
fn map_response_stream(
    api_stream: sofia_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let sofia_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = sofia_api::ResponseStream {
        rx_event,
...
/// Whether chat-completions requests for this model must echo
/// `reasoning_content` back on assistant messages that carry tool calls.
///
/// Thinking-mode providers (DeepSeek, MiMo, Kimi, Qwen, ...) reject a request
/// whose assistant tool-call message omits the field with "The
/// `reasoning_content` in the thinking mode must be passed back to the API."
///
/// This is keyed off the model's declared reasoning capability rather than the
/// items present in the outgoing history, so a compacted, resumed, or aborted
/// turn cannot leave behind a tool-call message that silently omits the field.
/// An explicit `reasoning_effort = "none"` opt-out is honored, because the
/// provider is then not in thinking mode and does not expect the field.
fn echoes_reasoning_content(
    model_info: &ModelInfo,
    effort: &Option<ReasoningEffortConfig>,
) -> bool {
...
fn repair_chat_completions_reasoning_content(request_body: &mut serde_json::Value) -> bool {
...
/// Whether an HTTP error status should include the outbound request body in the
/// logs. Restricted to request-shape failures: rate limits and timeouts are
/// noisy and carry no extra diagnostic value.
fn log_chat_completions_request_body_for(status: http::StatusCode) -> bool {
    matches!(
        status,
...
/// Log the outbound request body next to a provider error so that transport
/// incompatibilities (unknown roles, missing thinking fields) are diagnosable.
fn log_chat_completions_request_body(
    status: http::StatusCode,
    response_body: &str,
    request_body: &serde_json::Value,
) {
    let request_body = serde_json::to_string(request_body).unwrap_or_default();
    warn!(
        status = %status,
        req_body_len = request_body.len(),
        resp_body = %response_body,
        "stream_chat_completions_api: request body logged after HTTP error"
    );
    if request_body.len() > 2000 {
        let preview = request_body.chars().take(2000).collect::<String>();
        warn!(
            req_body_preview = %preview,
            "stream_chat_completions_api: request body preview (truncated)"
        );
    } else {
        warn!(
            req_body = %request_body,
            "stream_chat_completions_api: request body"
        );
    }
}
...
fn build_chat_completions_body(
    ...
    //
    // Whether the field is echoed at all is decided by the model's declared
    // reasoning capability, not by the items that happen to be in this request
    // (mirrors mimocode's `capabilities.interleaved` provider transform).
    // Gating on the observed history drops the field whenever a turn left an
    // assistant tool-call message behind without a `Reasoning` item — compacted
    // or resumed sessions, aborted turns, or a tool call the model made without
    // thinking — and the provider then rejects the whole request.
    let mut pending_reasoning_content = String::new();
    let mut saw_reasoning = false;
    // Thinking-capable models always echo the field, even when the captured
    // reasoning is empty: DeepSeek returns empty reasoning_content that still
    // must be sent back.
    let force_reasoning_content = echoes_reasoning_content(model_info, effort);
    let echo_reasoning_content = |saw: bool| saw || force_reasoning_content;
