...
use sofia_execpolicy::AmendError;
use sofia_execpolicy::Decision;
use sofia_execpolicy::Error as ExecPolicyRuleError;
...
use sofia_execpolicy::RequirementsExecPolicy;
use sofia_execpolicy::RuleMatch;
use sofia_execpolicy::blocking_append_allow_prefix_rule;
use sofia_execpolicy::blocking_append_network_rule;
...
use sofia_shell_command::is_dangerous_command::DangerousCommandMatch;
use sofia_shell_command::is_dangerous_command::DangerousCommandPlatform;
use sofia_shell_command::is_dangerous_command::dangerous_command_match_for_platform;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(sofia_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(sofia_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &sofia_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            sofia_shell_command::is_dangerous_command::dangerous_powershell_words_match(
                command,
                command_platform,
            )
        }
...
pub(crate) fn default_policy_path(sofia_home: &Path) -> PathBuf {
    sofia_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy_for_platform(
    ...
    if command_platform == DangerousCommandPlatform::Windows
        && let Some(commands) =
            sofia_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
        && !commands.is_empty()
    {
        return ExecPolicyCommands {
            commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and sofia prompts the user to run outside the sandbox
