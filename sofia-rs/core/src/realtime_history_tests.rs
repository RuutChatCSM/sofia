...
use sofia_protocol::items::DynamicToolCallStatus;
use sofia_protocol::items::ImageGenerationItem;
...
fn promotes_backing_agent_artifacts_once_without_a_client_request() {
    let mut state = started_state();
    let first_delta = assistant_delta("message-1", "[analysis] ::sofia-realtime-inline{}");
    ...
    let completed = completed_item(TurnItem::AgentMessage(AgentMessageItem {
        ...
        content: vec![AgentMessageContent::Text {
            text: "[analysis] ::sofia-realtime-inline{}\nVisible explanation".to_string(),
        }],
        phase: None,
    ...
    let next_message = assistant_delta("message-2", "::sofia-realtime-inline{}\nNext message");
    assert_eq!(
...
fn promotes_distinct_visualizations_once_and_ignores_markdown_fences() {
    let mut state = started_state();
    let text = "```\n::sofia-inline-vis{file=hidden}\n```\n~~~\nvisualize{file=also-hidden}\n~~~\n::sofia-inline-vis{file=first}\nvisualize{file=second}";
...
fn promotes_successful_codex_app_mcp_calls_and_splits_active_transcripts() {
    ...
    for (server, status) in [
        ...
        ("sofia_app", McpToolCallStatus::InProgress),
        ("sofia_app", McpToolCallStatus::Failed),
    ] {
        assert!(
            state
