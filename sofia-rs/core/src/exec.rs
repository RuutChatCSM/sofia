...
use sofia_protocol::protocol::ExecCommandOutputDeltaEvent;
use sofia_protocol::protocol::ExecOutputStream;
...
use sofia_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use sofia_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use sofia_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use sofia_sandboxing::record_filesystem_sandbox_violation;
#[cfg(test)]
use sofia_sandboxing::resolve_windows_elevated_filesystem_overrides;
#[cfg(test)]
use sofia_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use sofia_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
#[cfg(any(test, target_os = "windows"))]
use sofia_sandboxing::windows_sandbox_uses_elevated_backend;
...
use sofia_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: sofia_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: sofia_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
    ...
    sofia_linux_sandbox_exe: &Option<PathBuf>,
...
pub fn build_exec_request(
    ...
    let request = manager
        .transform(SandboxTransformRequest {
            ...
            sofia_linux_sandbox_exe: sofia_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        sofia_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = sofia_otel::global() {
        let _ = metrics.counter(
            "sofia.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use sofia_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use sofia_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            sofia_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let sofia_home = find_codex_home().map_err(|err| {
        CodexErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve sofia_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                sofia_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    sofia_home: sofia_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        sofia_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                sofia_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
