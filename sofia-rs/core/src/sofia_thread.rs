...
use sofia_diagnostics::Gauge;
use sofia_diagnostics::GaugeGuard;
use sofia_exec_server::SelectedCapabilityRootsStatus;
...
use sofia_protocol::turn_input::RecoverTurnRequest;
use sofia_protocol::turn_input::StartIfIdleSubmission;
use sofia_protocol::turn_input::SteerSubmission;
use sofia_protocol::turn_input::SuspendTurnOutcome;
...
use sofia_thread_store::ThreadStoreResult;
...
pub use sofia_guardian_context::GuardianRootMessage;
...
/// (formerly called a conversation) in Sofia.
impl CodexThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub fn thread_extension_data(&self) -> &sofia_extension_api::ExtensionData {
    ...
    pub(crate) async fn emit_thread_ready_lifecycle(&self) {
        ...
        for contributor in contributors {
            contributor
                .on_thread_ready(sofia_extension_api::ThreadReadyInput {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(sofia_extension_api::ThreadResumeInput {
                    session_store: &self.session.services.session_extension_data,
                    thread_store: &self.session.services.thread_extension_data,
                })
                .await;
    ...
    ) -> std::collections::HashMap<String, sofia_protocol::mcp::McpServerConnectionStatus> {
        self.session
            .services
            .mcp_runtime
    ...
    ) -> (sofia_mcp::McpConfig, sofia_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<sofia_mcp::McpConfig>, sofia_mcp::McpRuntimeContext) {
    ...
    pub async fn start_mcp_event_stream(
    ...
    ) -> anyhow::Result<sofia_mcp::McpEventStream> {
        ...
        self.session.refresh_mcp_if_dirty().await;
        sofia_mcp::McpResourceClient::new(Arc::clone(&self.session.services.mcp_runtime))
