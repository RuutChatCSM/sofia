...
use sofia_config::types::AppConfig;
use sofia_config::types::AppToolConfig;
use sofia_config::types::AppToolsConfig;
use sofia_config::types::ApprovalsReviewer;
use sofia_config::types::AppsConfigToml;
...
use sofia_protocol::protocol::EnvironmentConfig;
...
fn approval_config(turn_context: &TurnContext) -> sofia_mcp::McpConfig {
...
fn mcp_tool_metadata_resolves_advertised_account_selector(
    ...
    let tool_info = serde_json::from_value(serde_json::json!({
        "server_name": SOFIA_APPS_MCP_SERVER_NAME,
...
fn write_sample_plugin_mcp(sofia_home: &std::path::Path) {
    let plugin_root = sofia_home.join("plugins/cache/test/sample/local");
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.sofia_home)
        .expect("create sofia home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.sofia_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = sofia_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.sofia_home,
        hook_list.hooks,
    );
...
fn openai_file_params_are_only_honored_for_codex_apps() {
    ...
    assert_eq!(
        openai_file_input_optional_fields_for_server(SOFIA_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        McpToolCallSpanFields {
            ...
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/sofia/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("sofia.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("sofia.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("sofia.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("sofia.mcp.target.id=")
            && !invalid_logs.contains("sofia.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("sofia.mcp.target.id=")
            && !missing_logs.contains("sofia.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("sofia.mcp.target.id=")
            && !no_meta_logs.contains("sofia.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("sofia.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn sofia_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        SOFIA_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn sofia_apps_tool_question_without_elicitation_omits_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        SOFIA_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn sofia_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: SOFIA_APPS_MCP_SERVER_NAME.to_string(),
...
fn mcp_tool_call_item_metadata_only_trusts_codex_apps_identity() {
    ...
    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(SOFIA_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn sofia_apps_tool_call_request_meta_includes_turn_metadata_and_codex_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let turn_context = Arc::new(turn_context);
    let step_context = StepContext::for_test(Arc::clone(&turn_context));
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
#[tokio::test]
async fn sofia_apps_tool_call_request_meta_includes_call_id_without_existing_codex_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let turn_context = Arc::new(turn_context);
    let step_context = StepContext::for_test(Arc::clone(&turn_context));
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
fn sofia_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn sofia_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn sofia_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = sofia_apps_auth_failure_result();
    let metadata = sofia_apps_auth_failure_metadata();
...
#[tokio::test]
async fn sofia_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn sofia_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn sofia_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(request.server_name, SOFIA_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        sofia_protocol::mcp::RequestId::String("sofia_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        sofia_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            SOFIA_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("sofia_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(sofia_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .sofia_home(sofia_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents = std::fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_codex_dir = project_dir.path().join(".sofia");
    std::fs::create_dir_all(&project_codex_dir).expect("create project .sofia dir");
    std::fs::write(
