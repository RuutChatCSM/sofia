...
/// Start an interactive sub-Sofia thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_codex_thread_interactive(
    ...
    if config.permissions.approval_policy.value() != AskForApproval::Never {
        return Err(CodexErr::InvalidRequest(
            "Sofia delegates require approval policy `never`".to_string(),
        ));
    }
    ...
    let mut thread_extension_init = sofia_extension_api::ExtensionDataInit::default();
