...
use sofia_analytics::GuardianApprovalRequestSource;
...
use sofia_guardian_context::ConversationTranscriptEntry;
use sofia_guardian_context::ConversationTranscriptEntryKind;
use sofia_history::RolloutItem;
use sofia_model_provider::create_model_provider;
use sofia_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
use sofia_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
use sofia_model_provider_info::ModelProviderInfo;
use sofia_model_provider_info::OPENAI_PROVIDER_ID;
use sofia_models_manager::manager::StaticModelsManager;
use sofia_network_proxy::NetworkProxyConfig;
use sofia_protocol::ThreadId;
use sofia_protocol::approvals::GuardianAssessmentAction;
...
use sofia_protocol::models::SandboxPermissions;
...
// The memories extension depends on sofia-core, so this probe verifies the nested Guardian config
...
impl sofia_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: sofia_extension_api::ThreadStartInput<'a, Config>,
...
impl sofia_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a sofia_extension_api::ExtensionData,
        thread_store: &'a sofia_extension_api::ExtensionData,
    ) -> sofia_extension_api::ExtensionFuture<'a, Vec<sofia_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![sofia_extension_api::PromptFragment::developer_policy(
                    GUARDIAN_MEMORY_CONTEXT_PROBE,
                    sofia_extension_api::ContentItemKind(
...
fn guardian_exec_command_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::ExecCommand {
        ...
        cwd: test_path_buf("/repo/sofia-rs/core").abs().into(),
        guardian_cwd: native_guardian_cwd("/repo/sofia-rs/core"),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    name: None,
                    namespace: None,
                    output: sofia_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/sofia-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[sofia_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            sofia_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Sofia agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = sofia_utils_output_truncation::truncate_text(
        &approval_reason,
        sofia_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            sofia_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: sofia_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: sofia_protocol::permissions::FileSystemSpecialPath::project_roots(Some(
                        "private".to_string(),
                    )),
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Sofia agent has requested the following next action:\n"));
...
fn collect_guardian_transcript_entries(
    history: &dyn sofia_guardian_context::SectionHistory,
    ...
    .find_map(|section| match section {
        sofia_guardian_context::ContextSection::ConversationTranscript { items } => Some(items),
        _ => None,
    })
    .unwrap_or_default()
}
...
fn collect_guardian_transcript_entries_includes_recent_tool_calls_and_output() {
    ...
    if let ResponseItem::FunctionCallOutput { output, .. } = &mut items[2] {
        *output =
            sofia_protocol::models::FunctionCallOutputPayload::from_text(oversized_result.clone());
    }
    for token_cap in [
...
fn collect_guardian_transcript_entries_preserves_named_unpaired_tool_sources() {
    ...
    items.extend(
        ...
        .map(|(call_id, text)| ResponseItem::FunctionCallOutput {
            ...
            name: None,
            namespace: None,
            output: sofia_protocol::models::FunctionCallOutputPayload::from_text(text.to_string()),
            internal_chat_message_metadata_passthrough: None,
        }),
    );
    ...
    if let ResponseItem::FunctionCallOutput { output, .. } = &mut items[0] {
        *output = sofia_protocol::models::FunctionCallOutputPayload::from_content_items(vec![
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "sofia_core__guardian__tests__network_access_guardian_prompt_layout",
...
#[test_case::test_case(sofia_utils_path_uri::PathConvention::Windows, r"C:\workspace"; "windows")]
#[test_case::test_case(sofia_utils_path_uri::PathConvention::Posix, "/C:/workspace"; "posix")]
fn guardian_exec_command_uses_executor_cwd_convention(
    cwd_convention: sofia_utils_path_uri::PathConvention,
    expected_cwd: &str,
) -> serde_json::Result<()> {
    let cwd = PathUri::parse("file:///C:/workspace").expect("valid executor cwd");
    let action = crate::tools::sandboxing::ApprovalAction::ExecCommand {
        id: "command-1".to_string(),
        environment_id: sofia_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
    ...
    assert_eq!(
        ...
        GuardianAssessmentAction::Command {
            source: sofia_protocol::approvals::GuardianCommandSource::UnifiedExec,
...
fn guardian_timeout_message_distinguishes_timeout_from_policy_denial() {
    let mut model = sofia_models_manager::model_info::model_info_from_slug("acting-model");
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    sofia_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = sofia_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    update_turn_settings_for_test(&mut turn, |settings| {
        Arc::make_mut(&mut settings.model_info).auto_review_model_override =
            Some("sofia-auto-review".to_string());
    });
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    seed_guardian_parent_history(&session, &turn).await;
    ...
    assert!(matches!(
        ...
        Some(sofia_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    assert!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "sofia_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            sofia_protocol::user_input::UserInput::Text { text, .. } => text,
            sofia_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
        ...
        .replace_history(
            vec![
                ResponseItem::Compaction {
                    id: Some(sofia_protocol::ResponseItemId::from_server(
    ...
    assert!(matches!(
        ...
        Some(sofia_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "sofia_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn escalated_retry_bypasses_extension_approval_and_runs_guardian() -> anyhow::Result<()> {
    ...
    impl sofia_extension_api::ApprovalReviewContributor for AutoApprovingReviewContributor {
        fn decide<'a>(
            &'a self,
            _input: &'a sofia_extension_api::ApprovalDecisionInput<'_>,
        ) -> sofia_extension_api::ExtensionFuture<'a, Option<sofia_extension_api::ApprovalDecision>>
        {
            Box::pin(async { Some(sofia_extension_api::ApprovalDecision::Allow) })
        }
    }
...
async fn guardian_ephemeral_retry_preserves_parallel_trunk_and_fork_history() -> anyhow::Result<()>
{
    ...
    let handle = std::thread::Builder::new()
        ...
        let feedback = sofia_feedback::guardian_review_failures(&[session.thread_id()])
            .attachment
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(sofia_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
...
async fn guardian_review_session_config_clears_context_overrides_for_distinct_effective_model() {
    ...
    let (session, mut turn) = guardian_test_session_and_turn(&server).await;
    let mut config = (*turn.config).clone();
    config.model = Some("sofia-auto-review".to_string());
