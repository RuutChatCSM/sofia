...
use sofia_extension_api::ApprovalDecisionInput;
use sofia_extension_api::SynchronousApprovalReviewer;
use sofia_protocol::approvals::GuardianReviewReason;
...
pub(crate) fn spawn_approval_decision(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("sofia-approval-review".to_string())
...
pub(crate) async fn decide_approval(
    ...
    let require_guardian = options.require_guardian
        ...
        || requirements
            .approvals_reviewer
            .can_set(&sofia_protocol::config_types::ApprovalsReviewer::User)
            .is_err();
    let require_fresh_review = options.require_synchronous_review
        || model_requires_review
            && !turn
                .config
                .features
                .enabled(sofia_features::Feature::GuardianV2)
        || options
            .external_cancel
            .as_ref()
        ...
        Some(ApprovalDecision::Allow) if !require_fresh_review => {
                ...
                .is_some()
            {
                session
                    ...
                    .track_guardian_v2_event(sofia_analytics::GuardianV2Event {
                        ...
                        kind: sofia_analytics::GuardianV2EventKind::FastDecision {
                            decision: "approved",
                        },
                    });
            }
