...
use sofia_analytics::AppInvocation;
use sofia_analytics::InvocationType;
use sofia_analytics::build_track_events_context;
use sofia_api::HostedFileUploadContext;
use sofia_config::ConfigLayerSource;
use sofia_config::types::AppToolApproval;
use sofia_connectors::AppToolPolicy;
use sofia_connectors::AppToolPolicyEvaluator;
use sofia_connectors::AppToolPolicyInput;
use sofia_extension_api::McpToolContext;
use sofia_features::Feature;
use sofia_mcp::MCP_TOOL_CODEX_APPS_META_KEY;
use sofia_mcp::McpPermissionPromptAutoApproveContext;
use sofia_mcp::PreparedMcpCall;
use sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME;
use sofia_mcp::SandboxState;
use sofia_mcp::ToolInfo;
use sofia_mcp::auth_elicitation_completed_result;
use sofia_mcp::build_auth_elicitation_plan;
use sofia_mcp::mcp_permission_prompt_is_auto_approved;
use sofia_protocol::ResponseItemId;
use sofia_protocol::approvals::ElicitationRequest;
use sofia_protocol::items::McpToolCallError;
use sofia_protocol::items::McpToolCallItem;
use sofia_protocol::items::McpToolCallStatus;
use sofia_protocol::items::TurnItem;
use sofia_protocol::mcp::CONFIRMATION_POLICIES_META_KEY;
use sofia_protocol::mcp::CallToolResult;
use sofia_protocol::mcp::is_node_repl_backed_server;
use sofia_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use sofia_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use sofia_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use sofia_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use sofia_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use sofia_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use sofia_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use sofia_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use sofia_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use sofia_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use sofia_protocol::request_user_input::RequestUserInputAnswer;
use sofia_protocol::request_user_input::RequestUserInputArgs;
use sofia_protocol::request_user_input::RequestUserInputQuestion;
use sofia_protocol::request_user_input::RequestUserInputQuestionOption;
...
use sofia_utils_output_truncation::TruncationPolicy;
use sofia_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "sofia/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "sofia.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "sofia.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == SOFIA_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == SOFIA_APPS_MCP_SERVER_NAME {
    ...
    if server == SOFIA_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == SOFIA_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.sofia_apps_meta.as_ref())
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        sofia.mcp.target.id = Empty,
        sofia.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        sofia.mcp.error.code = Empty,
    );
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    if server != SOFIA_APPS_MCP_SERVER_NAME {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        sofia_connectors::metadata::connector_install_url(
...
async fn refresh_codex_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Sofia Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        ...
        sofia_linux_sandbox_exe: prepared_call.config().sofia_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                sofia_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    sofia_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
impl Session {
    async fn register_mcp_tool_approval_metadata(
        ...
        turn_state.lock().await.insert_mcp_tool_approval_metadata(
            call_id.to_string(),
            (invocation.server == SOFIA_APPS_MCP_SERVER_NAME
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, sofia_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == SOFIA_APPS_MCP_SERVER_NAME {
        let mut sofia_apps_meta = metadata
            .and_then(|metadata| metadata.sofia_apps_meta.clone())
            .unwrap_or_default();
        sofia_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_CODEX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(sofia_apps_meta),
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    let action = ApprovalAction::McpToolCall {
        ...
        connected_account_email: (invocation.server == SOFIA_APPS_MCP_SERVER_NAME)
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::Sofia(_)) => ReviewDecision::Abort,
        },
    )
}
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == SOFIA_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
fn mcp_tool_metadata(
    ...
    let connector_description = (server == SOFIA_APPS_MCP_SERVER_NAME)
        .then(|| tool_info.namespace_description.clone())
        .flatten();

    let sofia_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        ...
        .cloned();
    let link_id = if server == SOFIA_APPS_MCP_SERVER_NAME {
        account::resolve_account(&tool_info, arguments)?
    } else {
        None
    };
    let connected_account_email = if server == SOFIA_APPS_MCP_SERVER_NAME {
        sofia_apps_meta
            .as_ref()
...
fn openai_file_input_optional_fields_for_server(
    ...
    (server == SOFIA_APPS_MCP_SERVER_NAME)
    ...
    if let Some(metadata) = metadata {
                ...
                || metadata.connector_description.is_some())
        {
            ...
            if let Some(link_id) = metadata.link_id.as_ref() {
                ...
                // Match the Sofia Apps service's reserved implicit-link ID prefix.
                meta.insert(
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == SOFIA_APPS_MCP_SERVER_NAME {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, sofia_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
