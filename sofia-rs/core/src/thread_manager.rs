...
use crate::sofia_thread::CodexThread;
use crate::tasks::InterruptedTurnHistoryMarker;
use crate::tasks::interrupted_turn_history_marker;
use futures::StreamExt;
use futures::stream::FuturesUnordered;
use sofia_agent_graph_store::AgentGraphStore;
use sofia_agent_graph_store::LocalAgentGraphStore;
...
use sofia_attachment_store::AttachmentStore;
use sofia_attachment_store::InlineAttachmentStore;
use sofia_code_mode::CodeModeSessionProvider;
use sofia_code_mode::DisabledCodeModeSessionProvider;
use sofia_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use sofia_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use sofia_thread_store::InMemoryThreadStore;
use sofia_thread_store::LoadThreadHistoryParams;
use sofia_thread_store::LocalThreadStore;
use sofia_thread_store::LocalThreadStoreConfig;
use sofia_thread_store::MoveThreadToSectionParams;
use sofia_thread_store::PreparedFork;
use sofia_thread_store::ReadThreadByRolloutPathParams;
use sofia_thread_store::ReadThreadParams;
use sofia_thread_store::StoredModelContext;
use sofia_thread_store::StoredThread;
use sofia_thread_store::ThreadMetadataPatch;
use sofia_thread_store::ThreadStore;
use sofia_thread_store::ThreadStoreError;
use sofia_thread_store::UpdateThreadMetadataParams;
...
/// Represents a newly created Sofia thread (formerly called a conversation), including the first event
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<sofia_protocol::dynamic_tools::DynamicToolSpec>,
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            if has_state_db && background_migration_enabled {
                ...
                let sofia_home = config.sofia_home.to_path_buf();
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        sofia_rollout::spawn_rollout_compression_worker(sofia_home);
                    }
                });
            } else if compression_enabled {
                sofia_rollout::spawn_rollout_compression_worker(config.sofia_home.to_path_buf());
            }
            store
        }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        sofia_apps_tools_cache: CodexAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let sofia_home = std::env::temp_dir().join(format!(
            "sofia-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&sofia_home)
            .unwrap_or_else(|err| panic!("temp sofia home dir create failed: {err}"));
        ...
        manager._test_codex_home_guard = Some(TempCodexHomeGuard { path: sofia_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided CodexAuth and sofia home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_codex_home = match AbsolutePathBuf::from_absolute_path_checked(&sofia_home) {
            Ok(sofia_home) => sofia_home,
            Err(err) => panic!("test sofia_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: sofia_state::SqliteConfig::new_for_testing(absolute_codex_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(sofia_home, /*config_model_catalog*/ None),
                git_root_discovery: Arc::default(),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
    ...
    pub async fn list_models(
        ...
        http_client_factory: sofia_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
...
impl ThreadManagerState {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> sofia_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return sofia_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(sofia_rollout_trace::ThreadTraceContext::disabled)
    }
}
