...
use sofia_analytics::TurnProfileFact;
use sofia_analytics::TurnTokenUsageFact;
use sofia_context_fragments::RenderedFragment;
use sofia_otel::SessionTelemetry;
use sofia_otel::TURN_E2E_DURATION_METRIC;
use sofia_otel::TURN_MEMORY_METRIC;
use sofia_otel::TURN_NETWORK_PROXY_METRIC;
use sofia_otel::TURN_TOKEN_USAGE_METRIC;
use sofia_otel::TURN_TOOL_CALL_METRIC;
use sofia_otel::TURN_UNIFIED_EXEC_RUNNING_PROCESSES_METRIC;
...
const TASK_COMPACT_METRIC: &str = "sofia.task.compact";
...
///
/// Implementations encapsulate a specific Sofia workflow (regular chat,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            sofia.turn.reasoning_effort = %reasoning_effort,
            sofia.turn.token_usage.input_tokens = field::Empty,
            sofia.turn.token_usage.cached_input_tokens = field::Empty,
            sofia.turn.token_usage.cache_write_input_tokens = field::Empty,
            sofia.turn.token_usage.non_cached_input_tokens = field::Empty,
            sofia.turn.token_usage.output_tokens = field::Empty,
            sofia.turn.token_usage.reasoning_output_tokens = field::Empty,
            sofia.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Sofia will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<sofia_extension_items::sleep::SleepItem>()
            .is_some()
    }
    ...
    pub async fn on_task_finished(
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "sofia.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "sofia.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "sofia.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "sofia.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "sofia.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "sofia.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "sofia.turn.token_usage.total_tokens",
