...
use sofia_api::RealtimeAudioFrame;
use sofia_api::RealtimeContextAppendChannel;
use sofia_api::RealtimeEvent;
use sofia_api::RealtimeEventParser;
use sofia_api::RealtimeSessionConfig;
use sofia_api::RealtimeSessionMode;
use sofia_api::RealtimeWebsocketClient;
use sofia_api::RealtimeWebsocketConnection;
use sofia_api::RealtimeWebsocketEvents;
use sofia_api::RealtimeWebsocketWriter;
...
use sofia_login::read_openai_api_key_from_env;
...
use sofia_protocol::protocol::CodexResponseHandoffMode;
...
use sofia_protocol::protocol::RealtimeConversationClosedEvent;
use sofia_protocol::protocol::RealtimeConversationRealtimeEvent;
use sofia_protocol::protocol::RealtimeConversationSdpEvent;
use sofia_protocol::protocol::RealtimeConversationStartedEvent;
...
use sofia_utils_output_truncation::approx_bytes_for_tokens;
use sofia_utils_string::approx_token_count;
use sofia_utils_string::take_bytes_at_char_boundary;
use sofia_utils_string::to_ascii_json_string;
...
const STANDALONE_HANDOFF_ID: &str = "sofia";
const DEFAULT_REALTIME_MODEL: &str = "gpt-realtime-1.5";
const DEFAULT_FRAMELESS_REALTIME_MODEL: &str = "gpt-live-1-sofia";
...
struct RealtimeHandoffState {
    ...
    sofia_responses_as_items: bool,
    sofia_response_item_prefix: Option<String>,
    sofia_response_handoff_mode: CodexResponseHandoffMode,
    sofia_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.sofia_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.sofia_response_handoff_mode == CodexResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    sofia_response_item_prefix: Option<String>,
    sofia_response_handoff_mode: CodexResponseHandoffMode,
    sofia_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> CodexResult<RealtimeStartOutput> {
        let RealtimeStart {
            ...
            sofia_responses_as_items,
            sofia_response_item_prefix,
            sofia_response_handoff_mode,
            sofia_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            sofia_response_item_prefix,
            sofia_response_handoff_mode,
            sofia_response_handoff_channel_prefixes: Arc::new(
                sofia_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.sofia_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.sofia_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.sofia_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn handoff_complete(&self) -> CodexResult<()> {
        ...
        let output = if handoff.sofia_responses_as_items {
...
async fn prepare_realtime_start(
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        sofia_responses_as_items: params.sofia_responses_as_items,
        sofia_response_item_prefix: params.sofia_response_item_prefix,
        sofia_response_handoff_mode: params.sofia_response_handoff_mode,
        sofia_response_handoff_channel_prefixes: params.sofia_response_handoff_channel_prefixes,
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.sofia_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    sofia_error_info: CodexErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
