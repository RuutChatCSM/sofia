...
pub use sofia_connectors::AppBranding;
pub use sofia_connectors::AppInfo;
pub use sofia_connectors::AppMetadata;
use sofia_connectors::ConnectorDirectoryCacheContext;
use sofia_connectors::ConnectorDirectoryCacheKey;
use sofia_connectors::apps_config_from_layer_stack;
...
use sofia_mcp::McpStartupPolicy;
use sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME;
use sofia_mcp::ToolInfo;
use sofia_mcp::ToolPluginProvenance;
use sofia_mcp::effective_mcp_servers;
use sofia_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = sofia_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        sofia_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == SOFIA_APPS_MCP_SERVER_NAME);
    ...
    let sofia_apps_auth_manager =
        sofia_mcp::host_owned_codex_apps_enabled(&mcp_config, auth.as_ref())
    ...
    let mcp_runtime = McpRuntime::new(McpRuntimeInput {
        ...
        runtime_context,
        sofia_apps_tools_cache: mcp_manager.sofia_apps_tools_cache(),
        tool_catalog_cache: mcp_manager.tool_catalog_cache(),
        sofia_apps_tools_cache_key: connector_runtime_context_key(auth.as_ref()),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: auth.clone(),
        auth_manager: sofia_apps_auth_manager,
    ...
    let refreshed_tools = if force_refetch {
            ...
            .await
        {
            ...
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{SOFIA_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let sofia_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(SOFIA_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_runtime
            .latest_wait_for_server_ready(SOFIA_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_runtime
                .latest_wait_for_server_ready(SOFIA_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if sofia_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + sofia_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    sofia_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != SOFIA_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(sofia_connectors::accessible::AccessibleConnectorTool {
    ...
    sofia_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &sofia_config::ConfigLayerStack,
    ...
    let app_reviewer = if server_name == SOFIA_APPS_MCP_SERVER_NAME {
