//! Bridges Apps SDK-style `openai/fileParams` metadata into Sofia's MCP flow.
//!
//! Strategy:
...
//!
//! The model-facing local-path schema is owned by `sofia-mcp` alongside MCP tool inventory, so this
...
use sofia_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use sofia_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("ChatGPT auth is required to upload files for Sofia Apps tools".to_string());
    };
    ...
    let upload_auth = sofia_model_provider::auth_provider_from_auth(auth);
