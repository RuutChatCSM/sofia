...
use sofia_analytics::TurnAnalyticsMetadata;
...
use sofia_sandboxing::get_platform_sandbox;
...
async fn detached_memory_responses_metadata_starts_an_independent_root_turn() {
    ...
    let thread_manager = crate::ThreadManager::with_models_provider_for_tests(
        sofia_login::CodexAuth::from_api_key("test"),
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responses_api_metadata(BTreeMap::from([
        ("sofia_security_surface".to_string(), "sdk".to_string()),
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        (
            "sofia_security_surface".to_string(),
            "client-supplied".to_string(),
        ),
        ...
        ("sofia_version".to_string(), "client-supplied".to_string()),
        (
        ...
        (
            "x-sofia-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-sofia-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert_eq!(json["sofia_security_surface"].as_str(), Some("sdk"));
    ...
    assert!(json.get("x-sofia-installation-id").is_none());
    assert!(json.get("x-sofia-parent-thread-id").is_none());
    ...
    assert_eq!(
        model_request_json["sofia_security_surface"].as_str(),
        Some("sdk")
    );
    assert_eq!(
    ...
    let meta = state
        ...
        .expect("turn metadata should be present");
    assert_eq!(meta["model"].as_str(), Some("gpt-5.4"));
    assert_eq!(meta["sofia_version"], env!("CARGO_PKG_VERSION"));
    assert_eq!(meta["reasoning_effort"].as_str(), Some("high"));
    ...
    assert!(meta.get("sofia_security_surface").is_none());
    assert_eq!(state.workspace_kind().as_deref(), Some("projectless"));
    assert_eq!(
        (state.turn_trigger(), state.sofia_turn_source()),
        (
            Some("goal".to_string()),
    ...
    for (configured, client, expected) in [
        ...
        assert_eq!(state.sofia_turn_source(), expected);
    }
}
...
fn turn_metadata_state_overlays_compaction_only_on_compaction_requests() {
    ...
    assert_eq!(compact_json["sofia_security_surface"].as_str(), Some("sdk"));
    assert_eq!(
    ...
    assert_eq!(regular_json["sofia_security_surface"].as_str(), Some("sdk"));
