...
use sofia_config::types::OtelExporterKind as Kind;
use sofia_config::types::OtelHttpProtocol as Protocol;
...
use sofia_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = sofia_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = sofia_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
