...
use sofia_mcp::McpBinding;
use sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME;
use sofia_mcp::ToolInfo as McpToolInfo;
...
impl McpHandlerCache {
    pub(crate) fn append_mcp_tools(
        ...
        config: &Config,
        apps_enabled: bool,
        mcp_server_catalog: &sofia_mcp::ResolvedMcpCatalog,
...
fn filter_non_codex_apps_mcp_tools_only(
    ...
    mcp_tools.iter().filter(|tool| {
        tool.server_name != SOFIA_APPS_MCP_SERVER_NAME && tool_is_model_visible(tool)
    })
}
