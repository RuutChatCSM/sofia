...
use sofia_config::RequirementsExecPolicy;
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        sofia_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(sofia_config::Sourced::new(
            sofia_config::RequirementsExecPolicy::new(requirements_exec_policy),
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(sofia_config::CONFIG_TOML_FILE))
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let sofia_home = temp.path().join("home_ignore_user_config");
    let rules_dir = sofia_home.join(RULES_DIR_NAME);
...
async fn request_rule_falls_back_when_prefix_rule_does_not_approve_all_commands() {
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
        "cargo install cargo-insta && rm -rf /tmp/sofia".to_string(),
    ];
    let manager = ExecPolicyManager::default();
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let sofia_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(sofia_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(sofia_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(sofia_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let sofia_home = temp.path().join("home_execpolicy_nested");
    let project_root = temp.path().join("project_execpolicy_nested");
    let nested = project_root.join("nested");
    let root_rules = project_root.join(".sofia").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".sofia").join(RULES_DIR_NAME);

    fs::create_dir_all(&sofia_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".sofia").join(RULES_DIR_NAME);
    ...
    for (name, trust_entries, expected_decision) in cases {
        let sofia_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&sofia_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let sofia_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&sofia_home)?;
        write_project_trust_config(&sofia_home, &trust_entries).await?;
