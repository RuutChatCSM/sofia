...
use sofia_protocol::capabilities::CapabilityRootLocation;
use sofia_protocol::capabilities::SelectedCapabilityRoot;
use sofia_protocol::config_types::WindowsSandboxLevel;
use sofia_protocol::mcp::ClientMcpExtensions;
use sofia_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
use sofia_protocol::mcp::OPENAI_FORM_EXTENSION_ID;
...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    config.sofia_home = temp_dir.path().join("sofia-home").abs();
    config.cwd = config.sofia_home.abs();
    std::fs::create_dir_all(&config.sofia_home).expect("create sofia home");
    ...
    assert!(matches!(
        ...
        sofia_protocol::error::CodexErrorDetails::InvalidRequest(message)
...
impl sofia_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: sofia_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> sofia_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<sofia_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> sofia_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<sofia_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> sofia_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: sofia_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl sofia_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: sofia_extension_api::McpServerContributionContext<'a, Config>,
        ) -> sofia_extension_api::ExtensionFuture<'a, Vec<sofia_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
...
async fn spawn_internal_guardian_session_preserves_windows_sandbox_proxy_settings() {
    ...
    assert_eq!(
        ...
        (
            ...
            sofia_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
        )
    );
...
async fn spawn_internal_session_preserves_parent_lineage_without_forking_history() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<Config> for ParentLifecycleContributor {}

    impl sofia_extension_api::McpServerContributor<Config> for ParentLifecycleContributor {
        fn id(&self) -> &'static str {
        ...
        fn contribute<'a>(
            &'a self,
            context: sofia_extension_api::McpServerContributionContext<'a, Config>,
        ) -> sofia_extension_api::ExtensionFuture<'a, Vec<sofia_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    struct ParentInstructionsProvider(sofia_extension_api::Instructions);

    impl sofia_extension_api::UserInstructionsProvider for ParentInstructionsProvider {
        fn load_user_instructions(&self) -> sofia_extension_api::LoadUserInstructionsFuture<'_> {
            Box::pin(async move {
                sofia_extension_api::LoadedUserInstructions {
    ...
    let mut managed_exec_policy = sofia_execpolicy::Policy::empty();
    managed_exec_policy
        .add_prefix_rule(&["rm".to_string()], sofia_execpolicy::Decision::Forbidden)
        .expect("add managed execution restriction");
    let mut requirements = config.config_layer_stack.requirements().clone();
    requirements.exec_policy = Some(sofia_config::Sourced::new(
        sofia_execpolicy::RequirementsExecPolicy::new(managed_exec_policy),
        sofia_config::RequirementSource::Unknown,
    ));
    requirements.additional_developer_instructions = Some(sofia_config::Sourced::new(
    ...
    config.config_layer_stack = sofia_config::ConfigLayerStack::new(
        config
            .config_layer_stack
    ...
    let parent_instructions = sofia_extension_api::Instructions {
        text: "parent user instructions must not be inherited".to_string(),
        source: config.sofia_home.join("AGENTS.md"),
    };
    ...
    manager_state.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        manager_state.mcp_manager.sofia_apps_tools_cache(),
    ));
    manager_state.user_instructions_provider =
        Arc::new(ParentInstructionsProvider(parent_instructions.clone()));
    let parent = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("sofia_work_desktop".to_string()),
    ...
    reviewer_environment.config =
        EnvironmentConfigState::Ready(sofia_protocol::protocol::EnvironmentConfig {
            ...
            exec_policy: Some(sofia_execpolicy::RequirementsExecPolicy::new(
                sofia_execpolicy::Policy::empty(),
            )),
            mcp_policy: None,
            network_policy: None,
    ...
    assert_eq!(reviewer_config.originator, "sofia_work_desktop");
    assert_eq!(
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl sofia_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        ) -> sofia_extension_api::ExtensionFuture<'a, Vec<sofia_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = sofia_mcp::sofia_apps_mcp_server_config(
                ...
                vec![sofia_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = sofia_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &sofia_mcp::McpConfig| {
        sofia_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let sofia_apps_server = sofia_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME)
        .expect("Sofia Apps server should be configured");
    let sofia_apps_headers = match sofia_apps_server.transport {
        sofia_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        sofia_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Sofia Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        sofia_apps_headers
            .expect("Sofia Apps headers should be configured")
            .get("originator"),
        Some(&"sofia_work_desktop".to_string())
    );
}
...
async fn explicit_installation_id_skips_codex_home_file() {
    ...
    assert!(!config.sofia_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.sofia_home.join("models_cache.json").exists());
}
