...
use sofia_config::ConfigError;
use sofia_config::ConfigLayerEntry;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLoadError;
use sofia_config::ConfigLoadOptions;
use sofia_config::ConfigRequirements;
use sofia_config::ConfigRequirementsToml;
use sofia_config::ConfigRequirementsWithSources;
use sofia_config::FilesystemDenyReadPattern;
use sofia_config::LoaderOverrides;
use sofia_config::RequirementSource;
use sofia_config::RequirementsLayerEntry;
use sofia_config::SessionThreadConfig;
use sofia_config::StaticThreadConfigLoader;
use sofia_config::ThreadConfigSource;
use sofia_config::compose_requirements;
use sofia_config::config_error_from_ignored_toml_fields;
use sofia_config::config_error_from_toml;
use sofia_config::config_error_from_typed_toml;
...
use sofia_config::loader::load_requirements_toml;
...
use sofia_protocol::config_types::EnvironmentVariablePattern;
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when SOFIA_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    let tmp = tempdir().expect("tempdir");
    let sofia_home = tmp.path().join("home");
    let workspace = tmp.path().join("workspace");
    tokio::fs::create_dir_all(sofia_home.as_path()).await?;
    ...
    .await?;
    let project_config_dir = workspace.join(".sofia");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .sofia_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = sofia_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        sofia_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = sofia_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = sofia_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when SOFIA_HOME/config.toml does not exist");
...
async fn managed_preferences_requirements_resolve_paths_against_codex_home() -> anyhow::Result<()> {
    ...
    let tmp = tempdir()?;
    let sofia_home = tmp.path().join("sofia-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.sofia/logs", &sofia_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(sofia_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(sofia_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![sofia_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(sofia_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(sofia_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sofia_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sofia_home).await?;
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.sofia".to_string(),
...
"#,
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let config_toml = AbsolutePathBuf::from_absolute_path(sofia_home.join(CONFIG_TOML_FILE))?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(sofia_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", sofia_home.display()))
                .expect("bundle requirements path should resolve against sofia_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let sofia_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sofia_home)
        .await
        .expect("create sofia home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            sofia_config::SandboxModeRequirement::ReadOnly,
            sofia_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    sofia_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".sofia")).await?;
    tokio::fs::create_dir_all(project_root.join(".sofia")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".sofia").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".sofia").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".sofia").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".sofia").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".sofia")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".sofia")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".sofia"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".sofia"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".sofia"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".sofia"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".sofia")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".sofia").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".sofia")).await?;
    tokio::fs::create_dir_all(repo_root.join(".sofia")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".sofia").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".sofia").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".sofia"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
"#;
    tokio::fs::write(project_root.join(".sofia").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".sofia").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".sofia").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".sofia").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let sofia_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sofia_home).await?;
    tokio::fs::write(sofia_home.join(CONFIG_TOML_FILE), "").await?;
...
async fn project_layer_is_added_when_dot_codex_exists_without_config_toml() -> std::io::Result<()> {
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".sofia"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn sofia_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let sofia_home = home_dir.join(".sofia");
    tokio::fs::create_dir_all(&sofia_home).await?;
    tokio::fs::write(
        sofia_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn sofia_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".sofia");
...
"#,
    ...
    let sofia_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&sofia_home_untrusted).await?;
    make_config_for_test(
        &sofia_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = sofia_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let sofia_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&sofia_home_unknown).await?;
    tokio::fs::write(
        sofia_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    // `.sofia` folder. The malformed profile value below would fail typed path
...
responses_api_metadata = { sofia_security_surface = "attacker" }
...
"#,
    ...
    // still use the project-local `.sofia` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".sofia")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".sofia").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let sofia_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&sofia_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let sofia_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&sofia_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &sofia_home,
...
mod requirements_exec_policy_tests {
    ...
    use sofia_config::ConfigRequirementsWithSources;
    use sofia_config::RequirementSource;
    use sofia_config::RequirementsExecPolicyDecisionToml;
    use sofia_config::RequirementsExecPolicyParseError;
    use sofia_config::RequirementsExecPolicyPatternTokenToml;
    use sofia_config::RequirementsExecPolicyPrefixRuleToml;
    use sofia_config::RequirementsExecPolicyToml;
    use sofia_execpolicy::Decision;
    use sofia_execpolicy::Evaluation;
    use sofia_execpolicy::RuleMatch;
    use sofia_utils_absolute_path::AbsolutePathBuf;
    use std::path::Path;
    use tempfile::tempdir;
    use toml::Value as TomlValue;
