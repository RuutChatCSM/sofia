...
use sofia_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use sofia_network_proxy::managed_proxy_ports;
use sofia_network_proxy::normalize_host;
use sofia_network_proxy::validate_policy_against_constraints;
...
impl NetworkProxySpec {
    ...
    pub fn windows_sandbox_proxy_listeners(
        ...
        sofia_windows_sandbox::WindowsSandboxProvisioningSettings,
        sofia_windows_sandbox::WindowsSandboxProxyListeners,
    )> {
        if !self.config.enabled {
            return Ok((
                sofia_windows_sandbox::WindowsSandboxProvisioningSettings::default(),
                sofia_windows_sandbox::WindowsSandboxProxyListeners::default(),
            ));
        }
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    sofia_network_proxy::NetworkDomainPermission::Allow
                } else {
                    sofia_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
