...
use sofia_config::CONFIG_TOML_FILE;
use sofia_config::ConfigLayerEntry;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLayerStack;
use sofia_config::McpServerCommandMatcher;
use sofia_config::McpServerIdentity;
use sofia_config::McpServerRequirement;
use sofia_config::McpServerValueMatcher;
use sofia_config::ProfileV2Name;
use sofia_config::RequirementSource;
use sofia_config::Sourced;
use sofia_config::config_toml::AgentRoleToml;
use sofia_config::config_toml::AgentsToml;
use sofia_config::config_toml::AutoReviewToml;
use sofia_config::config_toml::ConfigToml;
use sofia_config::config_toml::ExperimentalRequestUserInput;
use sofia_config::config_toml::ProjectConfig;
use sofia_config::config_toml::RealtimeConfig;
use sofia_config::config_toml::RealtimeToml;
use sofia_config::config_toml::RealtimeTransport;
use sofia_config::config_toml::RealtimeWsMode;
use sofia_config::config_toml::RealtimeWsVersion;
use sofia_config::config_toml::ToolsToml;
...
use sofia_config::permissions_toml::NetworkDomainPermissionToml;
use sofia_config::permissions_toml::NetworkDomainPermissionsToml;
use sofia_config::permissions_toml::NetworkMitmActionToml;
use sofia_config::permissions_toml::NetworkMitmHookToml;
use sofia_config::permissions_toml::NetworkMitmToml;
...
use sofia_config::permissions_toml::WorkspaceRootsToml;
use sofia_config::types::AppToolApproval;
use sofia_config::types::ApprovalsReviewer;
use sofia_config::types::BundledSkillsConfig;
use sofia_config::types::FeedbackConfigToml;
use sofia_config::types::HistoryPersistence;
use sofia_config::types::McpServerEnvVar;
use sofia_config::types::McpServerOAuthConfig;
use sofia_config::types::McpServerToolConfig;
use sofia_config::types::McpServerTransportConfig;
use sofia_config::types::MemoriesConfig;
use sofia_config::types::MemoriesToml;
use sofia_config::types::ModelAvailabilityNuxConfig;
use sofia_config::types::Notice;
use sofia_config::types::NotificationCondition;
use sofia_config::types::NotificationMethod;
use sofia_config::types::Notifications;
use sofia_config::types::OtelConfigToml;
use sofia_config::types::OtelExporterKind;
use sofia_config::types::ResumeCwdMode;
use sofia_config::types::SandboxWorkspaceWrite;
use sofia_config::types::SessionPickerViewMode;
use sofia_config::types::SkillsConfig;
use sofia_config::types::ToolSuggestDisabledTool;
use sofia_config::types::ToolSuggestDiscoverableType;
use sofia_config::types::Tui;
use sofia_config::types::TuiKeymap;
use sofia_config::types::TuiNotificationSettings;
use sofia_config::types::TuiPetAnchor;
use sofia_config::types::WindowsSandboxModeToml;
use sofia_config::types::WindowsToml;
use sofia_exec_server::LOCAL_FS;
use sofia_features::Feature;
use sofia_features::FeaturesToml;
use sofia_login::default_client::RESIDENCY_HEADER_NAME;
use sofia_login::test_support::auth_manager_from_optional_auth;
use sofia_model_provider::ProviderCapabilities;
use sofia_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use sofia_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use sofia_model_provider_info::WireApi;
use sofia_models_manager::bundled_models_response;
use sofia_network_proxy::NetworkMode;
use sofia_protocol::config_types::ModelProviderAuthInfo;
use sofia_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use sofia_protocol::config_types::ServiceTier;
use sofia_protocol::models::ActivePermissionProfile;
use sofia_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use sofia_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use sofia_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use sofia_protocol::models::ManagedFileSystemPermissions;
use sofia_protocol::models::PermissionProfile;
use sofia_protocol::models::SandboxEnforcement;
use sofia_protocol::openai_models::MultiAgentRoleMessages;
...
use sofia_protocol::permissions::NetworkSandboxPolicy;
use sofia_protocol::protocol::MultiAgentVersion;
use sofia_protocol::protocol::NetworkAccess;
use sofia_protocol::protocol::RealtimeVoice;
use sofia_protocol::protocol::SandboxPolicy;
use sofia_utils_path_uri::LegacyAppPathString;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
excluded_tool_namespaces = ["mcp__sofia_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__sofia_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
name = "OpenAI"
http_headers = { "X-OpenAI-Internal-Sofia-Residency" = "request-override", "x-provider-header" = "preserved" }
env_http_headers = { "x-openai-internal-sofia-residency" = "SOFIA_TEST_UNSET_RESIDENCY_HEADER", "x-provider-env-header" = "SOFIA_TEST_UNSET_PROVIDER_HEADER" }
"#,
        )
        ...
        assert_eq!(
            static_headers.get("X-OpenAI-Internal-Sofia-Residency"),
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("SOFIA_TEST_UNSET_RESIDENCY_HEADER")
        );
        assert_eq!(
        ...
        assert_eq!(
            environment_headers
                ...
                .map(String::as_str),
            Some("SOFIA_TEST_UNSET_PROVIDER_HEADER")
        );
...
async fn tui_auto_recap_defaults_and_cli_overrides() -> anyhow::Result<()> {
    for (toml, override_value, expected) in [
        ...
        std::fs::write(sofia_home.path().join(CONFIG_TOML_FILE), toml)?;
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        sofia_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        sofia_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        sofia_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = sofia_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let sofia_home = temp_dir.path().join("sofia-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".sofia"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".sofia".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&root.join(".sofia"), cwd.as_path()),
            "expected .sofia carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_local_path_with_cwd(&cwd.path().join(".sofia"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".sofia"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Sofia and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Sofia and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Sofia."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    sofia_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            sofia_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sofia_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.sofia", sofia_home.path());
    ...
    let requirements_toml = sofia_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = sofia_config::ConfigRequirements {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(sofia_home.path().to_path_buf()),
            ..Default::default()
        },
        sofia_home.abs(),
...
-> anyhow::Result<()> {
    let sofia_home = TempDir::new()?;
    let plugin_root = sofia_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".sofia-plugin"))?;
    std::fs::write(
        plugin_root.join(".sofia-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            sofia_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let default_zsh_path = sofia_home.path().join("packaged-zsh");
...
async fn sqlite_home_defaults_to_codex_home_for_workspace_write() -> std::io::Result<()> {
    ...
    assert_eq!(config.sqlite.home(), sofia_home.path());
...
-> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let memories_root = sofia_home.path().join("memories");
    let writable_root = sofia_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn config_applies_managed_auth_store_and_chatgpt_base_url() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        requirements,
        sofia_config::ConfigRequirementsToml::default(),
    )?;
...
"#,
    ...
    let project_config_dir = workspace.path().join(".sofia");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let sofia_home = TempDir::new()?;
    let managed_path = sofia_home.path().join("managed_config.toml");
    let config_path = sofia_home.path().join(CONFIG_TOML_FILE);
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &Vec::new(),
        overrides,
        &sofia_config::NoopThreadConfigLoader,
    )
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), sofia_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(sofia_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let sofia_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(sofia_home.path());
    ...
    let loaded = load_global_mcp_servers(sofia_home.path()).await?;
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, sofia_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        sofia_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(sofia_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/sofia-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/sofia-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-sofia-123".to_string()),
                callback_url: None,
                callback_port: None,
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-sofia-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-sofia-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(sofia_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(sofia_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let sofia_home = TempDir::new()?;
    let base_config = sofia_home.path().join(CONFIG_TOML_FILE);
    let selected_config = sofia_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(sofia_home.path(), &profile_name),
        sofia_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn sofia_home(&self) -> AbsolutePathBuf {
        self.sofia_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let workspace = sofia_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, sofia_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        sofia_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let missing_path = sofia_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let role_config_path = sofia_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = sofia_config::ConfigLayerStack::new(
        vec![sofia_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: sofia_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".sofia").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let researcher_path = sofia_home.path().join("agents").join("researcher.toml");
    let reviewer_path = sofia_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = sofia_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let catalog_path = sofia_home.path().join("catalog.json");
...
"#;
    ...
    let sofia_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        sofia_home: sofia_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        ...
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: sofia_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let sofia_home = fixture.sofia_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = sofia_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![sofia_config::WebSearchModeRequirement::Cached]),
        application: None,
    ...
    let requirement_source = sofia_config::RequirementSource::Unknown;
    ...
    let requirements = sofia_config::ConfigRequirements {
        web_search_mode: sofia_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        sofia_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let sofia_home = temp_dir.path();
    let config_path = sofia_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(sofia_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(sofia_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(sofia_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(sofia_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
-> std::io::Result<()> {
    ...
    let instructions = "x".repeat(sofia_utils_string::approx_bytes_for_tokens(
        /*tokens*/ 10_000,
    ));
...
"#,
    ...
    assert_eq!(
        ...
        Some(sofia_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Sofia would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let cwd = sofia_home.path().join("workspace");
    let outside_root = sofia_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let selected_config = sofia_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let catalog_path = sofia_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = sofia_home.path().join("required-state");
    let required_log_dir = sofia_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&sofia_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let config = load_with_enterprise_requirement(&sofia_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let sofia_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(sofia_home.path().join("required-state"))?;
    let environment = sofia_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$SOFIA_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );
