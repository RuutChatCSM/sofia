...
use sofia_config::permissions_toml::NetworkUnixSocketPermissionToml;
use sofia_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use sofia_features::NetworkProxyDomainPermissionToml;
use sofia_features::NetworkProxyModeToml;
use sofia_features::NetworkProxyUnixSocketPermissionToml;
use sofia_network_proxy::NetworkMode;
use sofia_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use sofia_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
/// for Sofia to function. These should always be added to the
...
pub(crate) fn get_readable_roots_required_for_codex_runtime(
    ...
    let arg0_root = AbsolutePathBuf::from_absolute_path(sofia_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Sofia versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Sofia. Filesystem access will remain restricted. Upgrade Sofia if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Sofia and will be ignored. Upgrade Sofia if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Sofia and will be ignored. Upgrade Sofia if this path is required."
            ),
        },
    );
}
