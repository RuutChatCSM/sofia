...
use serde::Serialize;
use sofia_agent_roles::load_agent_roles;
use sofia_config::CloudConfigBundleLoader;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLayerStack;
use sofia_config::ConfigRequirements;
use sofia_config::ConfigRequirementsToml;
use sofia_config::ConstrainedWithSource;
use sofia_config::FeatureRequirementsToml;
use sofia_config::ManagedAuthPolicy;
use sofia_config::McpServerRequirement;
use sofia_config::PluginRequirementsToml;
use sofia_config::ProfileV2Name;
use sofia_config::ResidencyRequirement;
use sofia_config::SandboxModeRequirement;
use sofia_config::Sourced;
use sofia_config::ThreadConfigLoader;
use sofia_config::config_toml::ConfigToml;
use sofia_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use sofia_config::config_toml::ProjectConfig;
use sofia_config::config_toml::RealtimeAudioConfig;
use sofia_config::config_toml::RealtimeConfig;
use sofia_config::config_toml::ThreadStoreToml;
use sofia_config::config_toml::validate_model_providers;
use sofia_config::loader::load_config_layers_state;
...
use sofia_config::sandbox_mode_requirement_for_permission_profile;
use sofia_config::types::ApprovalsReviewer;
use sofia_config::types::AuthCredentialsStoreMode;
use sofia_config::types::AuthKeyringBackendKind;
use sofia_config::types::History;
use sofia_config::types::McpServerConfig;
use sofia_config::types::McpServerDisabledReason;
use sofia_config::types::MemoriesConfig;
use sofia_config::types::ModelAvailabilityNuxConfig;
use sofia_config::types::Notice;
use sofia_config::types::OAuthCredentialsStoreMode;
use sofia_config::types::ResumeCwdMode;
use sofia_config::types::SessionPickerViewMode;
use sofia_config::types::ToolSuggestConfig;
use sofia_config::types::ToolSuggestDisabledTool;
use sofia_config::types::ToolSuggestDiscoverable;
use sofia_config::types::TuiKeymap;
use sofia_config::types::TuiNotificationSettings;
use sofia_config::types::TuiPetAnchor;
use sofia_config::types::UriBasedFileOpener;
use sofia_config::types::WindowsSandboxModeToml;
use sofia_core_plugins::PluginLoadOutcome;
use sofia_core_plugins::PluginsConfigInput;
use sofia_exec_server::ExecutorFileSystem;
use sofia_exec_server::LOCAL_FS;
use sofia_exec_server::ReadFileOptions;
use sofia_features::CodeModeConfigToml;
use sofia_features::CurrentTimeReminderConfigToml;
use sofia_features::CurrentTimeReminderDeliveryMode;
use sofia_features::CurrentTimeSource;
use sofia_features::Feature;
use sofia_features::FeatureConfigSource;
use sofia_features::FeatureOverrides;
use sofia_features::FeatureToml;
use sofia_features::Features;
use sofia_features::FeaturesToml;
use sofia_features::MultiAgentV2ConfigToml;
use sofia_features::NetworkProxyConfigToml;
use sofia_features::SleepToolMode;
use sofia_features::TokenBudgetConfigToml;
...
use sofia_install_context::InstallContext;
use sofia_login::AuthManagerConfig;
use sofia_login::AuthRouteConfig;
use sofia_mcp::DEFAULT_OPTIONAL_MCP_STARTUP_GRACE;
use sofia_mcp::McpConfig;
use sofia_mcp::McpPluginAttribution;
use sofia_mcp::McpProtocolMode;
use sofia_mcp::McpServerRegistration;
use sofia_mcp::ResolvedMcpCatalog;
use sofia_memories_read::memory_root;
use sofia_model_provider::ProviderCapabilities;
use sofia_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use sofia_model_provider_info::ModelProviderInfo;
use sofia_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use sofia_model_provider_info::built_in_model_providers;
use sofia_model_provider_info::merge_configured_model_providers;
use sofia_models_manager::ModelsManagerConfig;
use sofia_protocol::config_types::AltScreenMode;
use sofia_protocol::config_types::AutoCompactTokenLimitScope;
...
use sofia_protocol::config_types::SandboxMode;
use sofia_protocol::config_types::ServiceTier;
use sofia_protocol::config_types::ShellEnvironmentPolicy;
use sofia_protocol::config_types::TrustLevel;
use sofia_protocol::config_types::Verbosity;
use sofia_protocol::config_types::WebSearchConfig;
...
use sofia_protocol::models::BaseInstructionsProvenance;
...
use sofia_protocol::openai_models::ModelMessages;
...
use sofia_protocol::permissions::ReadDenyMatcher;
...
use sofia_rmcp_client::McpOAuthRefreshMode;
pub use sofia_thread_store::ExtraConfig;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_absolute_path::AbsolutePathBufGuard;
...
pub use sofia_agent_roles::AgentRoleConfig;
pub use sofia_config::ConfigLoadOptions;
pub use sofia_config::Constrained;
pub use sofia_config::ConstraintError;
pub use sofia_config::ConstraintResult;
pub use sofia_config::LoaderOverrides;
pub use sofia_network_proxy::NetworkProxyAuditMetadata;
use sofia_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use sofia_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(sofia_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(sofia_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Sofia-only writable roots are intentionally excluded.
...
// when Sofia is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<sofia_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Sofia will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Sofia
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.sofia/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Sofia"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Sofia '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Sofia home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Sofia can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Sofia unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/sofia-rs/rmcp-client/src/oauth.rs#L2
    /// file: SOFIA_HOME/.credentials.json
    ///       This file will be readable to Sofia and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Sofia will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Sofia state (defaults to `~/.sofia` but can be
    /// overridden by the `SOFIA_HOME` environment variable).
    pub sofia_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Sofia SQLite databases.
    pub sqlite: sofia_state::SqliteConfig,

    /// Directory where Sofia writes log files (defaults to `$SOFIA_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.sofia/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Sofia executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub sofia_self_exe: Option<PathBuf>,

    /// Path to the `sofia-linux-sandbox` executable. This must be set if
    /// [`sofia_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `sofia-linux-sandbox`.
    pub sofia_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `sofia-execve-wrapper` executable used for shell
    ...
    /// Whether Sofia-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Sofia updates on startup and surfaces update prompts.
    /// Set to `false` only if your Sofia updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Sofia product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Sofia product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: sofia_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn sofia_home(&self) -> PathBuf {
        self.sofia_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn sofia_home(mut self, sofia_home: PathBuf) -> Self {
        self.sofia_home = Some(sofia_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let sofia_home = match sofia_home {
            Some(sofia_home) => AbsolutePathBuf::from_absolute_path(sofia_home)?,
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&sofia_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = sofia_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    sofia_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(sofia_config::io_error_from_config_error(
...
impl Config {
    pub fn sqlite_config(&self) -> &sofia_state::SqliteConfig {
        &self.sqlite
    }

    /// Whether Guardian may use the unmetered Sofia inference endpoints.
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &sofia_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            sofia_home: self.sofia_home.to_path_buf(),
            ...
            sofia_linux_sandbox_exe: self.sofia_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                sofia_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.sofia_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_codex_home(
            sofia_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific Sofia home without reading
    ...
    pub async fn load_default_with_cli_overrides_for_codex_home(
        ...
        let cli_layer = sofia_config::build_cli_overrides_layer(&cli_overrides);
        sofia_config::merge_toml_values(&mut merged, &cli_layer);
        let sofia_home = AbsolutePathBuf::from_absolute_path_checked(sofia_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &sofia_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `sofia exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `sofia_self_exe`, `sofia_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(sofia_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(sofia_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, sofia_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .sofia/ folders.
...
/// Patch `SOFIA_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(sofia_home: &Path, provider: &str) -> std::io::Result<()> {
    sofia_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &sofia_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            sofia_protocol::permissions::FileSystemSandboxEntry {
                path: sofia_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: sofia_protocol::permissions::FileSystemAccessMode::Deny,
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&sofia_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&sofia_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(sofia_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| sofia_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| sofia_home.clone());
        ...
        let config = Self {
            ...
            sofia_home,
            sqlite: sofia_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
    ...
    pub fn bundled_skills_enabled(&self) -> bool {
        sofia_config::bundled_skills_enabled_from_stack(&self.config_layer_stack)
    }
...
/// specified by the `SOFIA_HOME` environment variable. If not set, defaults to
/// `~/.sofia`.
///
/// - If `SOFIA_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `SOFIA_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    sofia_utils_home_dir::find_codex_home()
}

/// Returns the path to the folder where Sofia logs are stored. Does not verify
