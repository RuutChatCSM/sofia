...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = sofia_home.join("config-link.toml");
    let config_path = sofia_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let sofia_home = tmp.path();
    let link_a = sofia_home.join("a.toml");
    let link_b = sofia_home.join("b.toml");
    let config_path = sofia_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(sofia_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#,
    ...
    apply_blocking(
        sofia_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-sofia-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-sofia-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-sofia-123\"
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides(output_token_limit: usize) {
    ...
    let result = apply_blocking(sofia_home, &[ConfigEdit::ReplaceMcpServers(servers)]);
...
"#,
    ...
    apply_blocking(sofia_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !sofia_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).expect("read config");
