...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let sofia_home = temp_dir.path().join(".sofia");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = sofia_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("sofia-arg0-session");
    let sibling_arg0_dir = arg0_root.join("sofia-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("sofia-execve-wrapper");
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        config.unix_sockets,
        Some(sofia_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        config.domains,
        Some(sofia_network_proxy::NetworkDomainPermissions {
            entries: vec![sofia_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: sofia_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
