...
use sofia_analytics::ImagePreparationFact;
use sofia_analytics::ImagePreparationMetadata;
use sofia_analytics::SubAgentThreadStartedInput;
use sofia_analytics::TurnCodexErrorFact;
use sofia_async_utils::OrCancelExt;
use sofia_connectors::connector_runtime_context_key;
use sofia_context_fragments::RenderedFragment;
use sofia_exec_server::Environment;
use sofia_exec_server::EnvironmentManager;
use sofia_execpolicy::prefix_rule_migration;
use sofia_extension_api::ConversationHistorySnapshot;
use sofia_extension_api::ExtensionDataInit;
use sofia_extension_api::LoadedUserInstructions;
use sofia_extension_api::PromptSlot;
use sofia_extension_api::TurnContextContributionInput;
use sofia_features::FEATURES;
use sofia_features::Feature;
use sofia_features::unstable_features_warning_event;
use sofia_history::RolloutItem;
use sofia_hooks::Hooks;
use sofia_hooks::HooksConfig;
use sofia_login::AuthManager;
use sofia_login::CodexAuth;
use sofia_login::auth_env_telemetry::collect_auth_env_telemetry;
use sofia_mcp::McpResourceClient;
use sofia_mcp::McpRuntime;
use sofia_mcp::McpRuntimeContext;
use sofia_mcp::McpRuntimeInput;
use sofia_models_manager::manager::RefreshStrategy;
use sofia_models_manager::manager::SharedModelsManager;
use sofia_network_proxy::NetworkProxy;
use sofia_network_proxy::NetworkProxyAuditMetadata;
use sofia_network_proxy::normalize_host;
use sofia_otel::current_span_trace_id;
use sofia_otel::current_span_w3c_trace_context;
use sofia_otel::set_parent_from_w3c_trace_context;
use sofia_protocol::ResponseUsageMetadata;
use sofia_protocol::SessionId;
use sofia_protocol::ThreadId;
use sofia_protocol::approvals::ElicitationRequest;
use sofia_protocol::approvals::ElicitationRequestEvent;
use sofia_protocol::approvals::ExecPolicyAmendment;
use sofia_protocol::approvals::NetworkPolicyAmendment;
use sofia_protocol::approvals::NetworkPolicyRuleAction;
...
use sofia_protocol::dynamic_tools::DynamicToolResponse;
use sofia_protocol::dynamic_tools::DynamicToolSpec;
use sofia_protocol::items::EnteredReviewModeItem;
use sofia_protocol::items::SubAgentActivityItem;
use sofia_protocol::items::TurnItem;
use sofia_protocol::items::UserMessageItem;
use sofia_protocol::models::ActivePermissionProfile;
use sofia_protocol::models::AdditionalPermissionProfile;
...
use sofia_protocol::openai_models::ModelPreset;
...
use sofia_protocol::protocol::FileChange;
use sofia_protocol::protocol::HasLegacyEvent;
...
use sofia_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use sofia_protocol::protocol::MultiAgentVersion;
use sofia_protocol::protocol::RawResponseItemEvent;
...
use sofia_protocol::protocol::ThreadHistoryMode;
...
use sofia_protocol::protocol::TurnAbortReason;
use sofia_protocol::protocol::TurnContextItem;
use sofia_protocol::protocol::TurnContextNetworkItem;
use sofia_protocol::protocol::TurnEnvironmentSelection;
use sofia_protocol::protocol::W3cTraceContext;
use sofia_protocol::protocol::WorldStateItem;
...
use sofia_protocol::request_permissions::RequestPermissionsArgs;
use sofia_protocol::request_permissions::RequestPermissionsEvent;
use sofia_protocol::request_permissions::RequestPermissionsResponse;
use sofia_protocol::request_user_input::RequestUserInputArgs;
use sofia_protocol::request_user_input::RequestUserInputResponse;
use sofia_rmcp_client::ElicitationResponse;
use sofia_rollout::state_db;
use sofia_rollout_trace::AgentResultTracePayload;
use sofia_rollout_trace::ThreadStartedTraceMetadata;
use sofia_rollout_trace::ThreadTraceContext;
use sofia_sandboxing::policy_transforms::intersect_permission_profiles_with_context;
use sofia_shell_command::parse_command::parse_command;
use sofia_terminal_detection::user_agent;
use sofia_thread_store::CreateThreadParams;
use sofia_thread_store::LiveThread;
use sofia_thread_store::LiveThreadInitGuard;
use sofia_thread_store::LocalThreadStore;
use sofia_thread_store::PersistContext;
use sofia_thread_store::ReadThreadParams;
use sofia_thread_store::ResumeThreadParams;
use sofia_thread_store::ThreadPersistenceMetadata;
use sofia_thread_store::ThreadStore;
use sofia_utils_audio::prepare_response_items as prepare_audio_response_items;
use sofia_utils_git_discovery::GitRootDiscovery;
...
use crate::sofia_thread::CodexThreadSettingsOverrides;
use crate::sofia_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use sofia_protocol::exec_output::StreamOutput;
...
use sofia_core_plugins::PluginCommandAttribution;
use sofia_core_plugins::PluginsManager;
use sofia_core_plugins::RecommendedPluginCandidatesInput;
use sofia_git_utils::get_git_repo_root;
...
use sofia_mcp::effective_mcp_servers;
use sofia_otel::SessionTelemetry;
use sofia_otel::THREAD_STARTED_METRIC;
use sofia_otel::TelemetryAuthMode;
use sofia_protocol::ResponseItemId;
use sofia_protocol::approvals::ExecApprovalKind;
...
use sofia_protocol::mcp::ClientMcpExtensions;
use sofia_protocol::models::LocalImagePreparation;
use sofia_protocol::models::ResponseInputItem;
use sofia_protocol::models::ResponseItem;
use sofia_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use sofia_protocol::protocol::ApplyPatchApprovalRequestEvent;
use sofia_protocol::protocol::AskForApproval;
use sofia_protocol::protocol::CodexErrorInfo;
use sofia_protocol::protocol::DeprecationNoticeEvent;
...
use sofia_protocol::protocol::ModelRerouteEvent;
use sofia_protocol::protocol::ModelRerouteReason;
use sofia_protocol::protocol::ModelVerification;
use sofia_protocol::protocol::ModelVerificationEvent;
use sofia_protocol::protocol::NetworkApprovalContext;
use sofia_protocol::protocol::Op;
use sofia_protocol::protocol::RateLimitSnapshot;
use sofia_protocol::protocol::RawResponseCompletedEvent;
use sofia_protocol::protocol::RequestUserInputEvent;
...
use sofia_protocol::protocol::SessionNetworkProxyRuntime;
use sofia_protocol::protocol::StreamErrorEvent;
use sofia_protocol::protocol::Submission;
use sofia_protocol::protocol::ThreadMemoryMode;
...
use sofia_protocol::protocol::TurnModerationMetadataEvent;
use sofia_protocol::protocol::WarningEvent;
...
use sofia_protocol::turn_input::TurnStartOptions;
use sofia_protocol::user_input::UserInput;
use sofia_skills_extension::HostSkillsService;
use sofia_tools::ToolName;
use sofia_tools::UnifiedExecShellMode;
use sofia_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use sofia_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn sofia_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<sofia_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        sofia_sandboxing::WindowsSandboxProxySettingsMode,
}
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/sofia/concepts/cyber-safety";
    ...
    pub(crate) fn spawn(
        ...
        Box::pin(async move {
            let parent_trace = match args.parent_trace {
                Some(trace) => {
                    if sofia_otel::context_from_w3c_trace_context(&trace).is_some() {
                        Some(trace)
                    } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                ...
                .exec_policy
                .as_deref()
                .map_or_else(sofia_execpolicy::Policy::empty, |policy| {
                    policy.as_ref().clone()
                });
            ...
            {
                let sofia_home = config.sofia_home.clone();
                let policy_path = default_policy_path(sofia_home.as_path());
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            sofia_models_manager::manager::RefreshStrategy::Offline
        } else {
            sofia_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.sofia_home)
        })?;
...
impl Session {
    ...
    /// Builds the `x-sofia-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &sofia_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn sofia_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn sofia_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn sofia_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.sofia_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Sofia performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<sofia_extension_api::Instructions> {
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &sofia_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| sofia_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => sofia_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&sofia_home, amendment)
            .await?;
    ...
    async fn capture_step_context_inner(
        ...
        let executor_capability_discovery = self
            ...
            .await?;
        let extension_data = sofia_extension_api::ExtensionData::new(turn_context.sub_id.clone());
        ...
        let mut selected_plugins = self
            .services
            .thread_extension_data
            .get::<sofia_extension_api::SelectedPluginSnapshot>()
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-sofia, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(sofia_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(sofia_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = sofia_error.to_string();
        let sofia_error_info = CodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: sofia_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<sofia_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
