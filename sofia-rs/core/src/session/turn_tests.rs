...
impl TurnItemContributor for RewriteAgentMessageContributor {
    fn contribute<'a>(
        ...
        item: &'a mut TurnItem,
    ) -> sofia_extension_api::ExtensionFuture<'a, Result<(), String>> {
        Box::pin(async move {
    ...
    let subscriber = tracing_subscriber::registry()
        .with(
            tracing_subscriber::fmt::layer()
                .with_writer(writer)
                .with_filter(sofia_state::log_db::default_filter()),
        )
...
fn realtime_user_verification_notice_excludes_request_payload() {
    let event = EventMsg::ElicitationRequest(sofia_protocol::approvals::ElicitationRequestEvent {
        turn_id: None,
        server_name: "private-server-name".to_string(),
        id: sofia_protocol::mcp::RequestId::String("private-request-id".to_string()),
        request: sofia_protocol::approvals::ElicitationRequest::UserVerification {
