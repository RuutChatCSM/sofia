...
use sofia_analytics::TurnResolvedConfigFact;
...
use sofia_extension_api::TurnInputContext;
use sofia_extension_api::TurnInputEnvironment;
use sofia_features::Feature;
use sofia_file_system::FindUpErrorPolicy;
use sofia_file_system::find_nearest_ancestor_with_markers;
use sofia_login::CodexAuth;
use sofia_model_provider::RemoteCompactionSupport;
...
use sofia_protocol::items::PlanItem;
...
use sofia_protocol::protocol::AgentMessageContentDeltaEvent;
use sofia_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use sofia_protocol::protocol::PlanDeltaEvent;
use sofia_protocol::protocol::ReasoningContentDeltaEvent;
use sofia_protocol::protocol::ReasoningRawContentDeltaEvent;
use sofia_protocol::protocol::SafetyBufferingEvent;
...
use sofia_skills::ToolMentionKind;
use sofia_skills::app_id_from_path;
use sofia_skills::build_skill_name_counts;
use sofia_skills::collect_explicit_skill_mentions;
use sofia_skills::tool_kind_for_path;
use sofia_skills_extension::HostSkillPrompts;
use sofia_skills_extension::InjectedHostSkillPrompts;
use sofia_thread_store::PersistContext;
use sofia_tools::DiscoverableTool;
use sofia_tools::ToolName;
use sofia_tools::filter_request_plugin_install_discoverable_tools_for_client;
use sofia_utils_path_uri::PathUri;
use sofia_utils_stream_parser::AssistantTextChunk;
use sofia_utils_stream_parser::AssistantTextStreamParser;
use sofia_utils_stream_parser::ProposedPlanSegment;
use sofia_utils_stream_parser::extract_proposed_plan_text;
use sofia_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "sofia_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    // Although from the perspective of sofia.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        match sampling_request_result {
            ...
            {
                sess.track_turn_codex_error(turn_context.as_ref(), &sofia_error);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &sofia_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = sofia_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
    ...
    sess.services
        ...
        .track_turn_resolved_config(TurnResolvedConfigFact {
            ...
            guardian_v2_enabled: sess
                ...
                .get::<sofia_extension_api::GuardianV2Enabled>()
                .is_some(),
...
///
/// Returns `None` when the active authentication does not use the Sofia backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    let uses_codex_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(sofia_login::AuthManager::current_auth_uses_codex_backend);
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = sofia_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<sofia_extension_api::ExtensionData>,
    ...
    loop {
        ...
        {
            sofia_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
...
pub(crate) async fn built_tools(
    sess: &Session,
    turn_context: &TurnContext,
    model_info: &sofia_protocol::openai_models::ModelInfo,
    model_messages: Option<&ModelMessages>,
    environments: &TurnEnvironmentSnapshot,
    mcp: &Arc<sofia_mcp::McpBinding>,
...
pub(super) fn agent_message_text(item: &sofia_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            sofia_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: sofia_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(sofia_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &sofia_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            sofia.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            sofia.usage.reasoning_output_tokens = field::Empty,
            sofia.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(sofia_async_utils::CancelErr::Cancelled) => {
        ...
        match event {
            ResponseEvent::Created { response_id } => {
                if let Some(response_id) = response_id {
                    turn_context
                        .extension_data
                        .insert(sofia_api::ResponseId(response_id));
                }
            }
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(sofia_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![sofia_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        sofia_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
