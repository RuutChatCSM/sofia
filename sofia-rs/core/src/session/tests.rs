...
use sofia_analytics::CompactionPhase;
use sofia_analytics::CompactionReason;
use sofia_config::ConfigLayerStack;
use sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use sofia_config::LoaderOverrides;
use sofia_config::NetworkConstraints;
use sofia_config::NetworkDomainPermissionToml;
use sofia_config::NetworkDomainPermissionsToml;
use sofia_config::RequirementSource;
use sofia_config::Sourced;
use sofia_config::loader::project_trust_key;
use sofia_config::types::McpServerConfig;
use sofia_config::types::McpServerTransportConfig;
use sofia_config::types::ToolSuggestDisabledTool;
use sofia_config::types::WindowsSandboxModeToml;

use sofia_features::Feature;
use sofia_file_system::FileSystemSandboxContext;
use sofia_http_client::ClientRouteClass;
use sofia_http_client::HttpClientFactory;
use sofia_http_client::OutboundProxyPolicy;
use sofia_http_client::RouteAwareClientPool;
...
use sofia_model_provider_info::built_in_model_providers;
use sofia_models_manager::bundled_models_response;
use sofia_models_manager::model_info;
use sofia_models_manager::test_support::construct_model_info_offline_for_tests;
use sofia_models_manager::test_support::get_model_offline_for_tests;
use sofia_protocol::AgentPath;
use sofia_protocol::ResponseItemId;
use sofia_protocol::SessionId;
use sofia_protocol::ThreadId;
use sofia_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use sofia_protocol::config_types::ServiceTier;
use sofia_protocol::config_types::TrustLevel;
use sofia_protocol::exec_output::ExecToolCallOutput;
use sofia_protocol::models::AgentMessageInputContent;
use sofia_protocol::models::FileSystemPermissions;
use sofia_protocol::models::FunctionCallOutputBody;
...
use sofia_protocol::models::SandboxEnforcement;
use sofia_protocol::openai_models::ModelServiceTier;
use sofia_protocol::openai_models::ToolMode;
use sofia_protocol::permissions::FileSystemAccessMode;
use sofia_protocol::permissions::FileSystemPath;
use sofia_protocol::permissions::FileSystemSandboxEntry;
use sofia_protocol::permissions::FileSystemSandboxPolicy;
use sofia_protocol::permissions::FileSystemSandboxPolicyContext;
use sofia_protocol::permissions::FileSystemSpecialPath;
use sofia_protocol::protocol::EnvironmentConfigState;
use sofia_protocol::protocol::ErrorEvent;
use sofia_protocol::protocol::SandboxPolicy;
use sofia_protocol::protocol::TurnEnvironmentSelections;
use sofia_protocol::request_permissions::PermissionGrantScope;
use sofia_protocol::request_permissions::RequestPermissionProfile;
use sofia_protocol::turn_input::TurnInput as SubmittedTurnInput;
use sofia_protocol::turn_input::TurnInputMode;
use sofia_protocol::turn_input::TurnInputRequest;
use sofia_protocol::turn_input::TurnInputSubmission;
use sofia_tools::ToolSpec;
use sofia_utils_path_uri::PathUri;
...
use sofia_config::config_toml::ConfigToml;
use sofia_config::config_toml::ProjectConfig;
use sofia_config::permissions_toml::FilesystemPermissionToml;
use sofia_config::permissions_toml::FilesystemPermissionsToml;
use sofia_config::permissions_toml::NetworkToml;
use sofia_config::permissions_toml::PermissionProfileToml;
use sofia_config::permissions_toml::PermissionsToml;
use sofia_execpolicy::Decision;
use sofia_execpolicy::NetworkRuleProtocol;
use sofia_execpolicy::Policy;
use sofia_history::CodexHarnessMetadata;
use sofia_history::CompactedItem;
use sofia_history::InitialHistory;
use sofia_history::ResponseItemEnvelope;
use sofia_history::ResumedHistory;
use sofia_history::RolloutItem;
use sofia_network_proxy::NetworkProxyConfig;
use sofia_otel::MetricsClient;
use sofia_otel::MetricsConfig;
use sofia_otel::TelemetryAuthMode;
use sofia_protocol::config_types::CollaborationMode;
use sofia_protocol::config_types::ModeKind;
use sofia_protocol::config_types::Settings;
use sofia_protocol::items::HookPromptFragment;
use sofia_protocol::items::build_hook_prompt_message;
use sofia_protocol::models::BaseInstructions;
use sofia_protocol::models::ContentItem;
use sofia_protocol::models::ContentItemKind;
use sofia_protocol::models::InternalChatMessageMetadataPassthrough;
...
use sofia_protocol::protocol::CreditsSnapshot;
use sofia_protocol::protocol::GranularApprovalConfig;
use sofia_protocol::protocol::HistoryPosition;
use sofia_protocol::protocol::InterAgentCommunication;
use sofia_protocol::protocol::MultiAgentVersion;
use sofia_protocol::protocol::NetworkApprovalProtocol;
use sofia_protocol::protocol::RateLimitSnapshot;
use sofia_protocol::protocol::RateLimitWindow;
use sofia_protocol::protocol::RealtimeAudioFrame;
use sofia_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use sofia_protocol::protocol::RealtimeVoice;
use sofia_protocol::protocol::RealtimeVoicesList;
use sofia_protocol::protocol::SessionMeta;
use sofia_protocol::protocol::SessionMetaLine;
use sofia_protocol::protocol::Submission;
use sofia_protocol::protocol::ThreadRolledBackEvent;
use sofia_protocol::protocol::ThreadSettingsOverrides;
use sofia_protocol::protocol::TokenCountEvent;
use sofia_protocol::protocol::TokenUsage;
use sofia_protocol::protocol::TokenUsageInfo;
use sofia_protocol::protocol::TokenUsageRecord;
use sofia_protocol::protocol::TurnAbortedEvent;
use sofia_protocol::protocol::TurnCompleteEvent;
use sofia_protocol::protocol::TurnStartedEvent;
use sofia_protocol::protocol::UserMessageEvent;
use sofia_protocol::protocol::W3cTraceContext;
use sofia_rmcp_client::ElicitationAction;
use std::path::Path;
...
use serde_json::json;
use sofia_protocol::mcp::CallToolResult as McpCallToolResult;
use std::path::PathBuf;
use std::sync::Arc;
use std::sync::OnceLock;
use std::time::Duration as StdDuration;

pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<sofia_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &sofia_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(sofia_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    extension_metrics.counter(
        "sofia.test.extension.counter",
        /*inc*/ 2,
        &[("component", "skills"), ("model", "extension-model")],
    );

    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "sofia.test.extension");
    let counter = find_metric(&snapshot, "sofia.test.extension.counter");
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl sofia_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: sofia_extension_api::WorldStateContributionInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<
            'a,
            Vec<sofia_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("sofia.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder = sofia_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "sofia.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("sofia-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        sofia_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        sofia_home.join(sofia_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<sofia_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&sofia_hooks::SessionStartRequest {
            session_id: thread_id,
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: sofia_hooks::StartHookTarget::SessionStart {
                source: sofia_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn sofia_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { sofia_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::ExecCommand {
                ...
                environment_id: sofia_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> sofia_sandboxing::SandboxablePreference {
            sofia_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(sofia_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: sofia_tools::ToolName::plain("probe"),
    };
...
async fn get_base_instructions_no_user_content() {
    let prompt_path = sofia_utils_cargo_bin::find_resource!(
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let sofia_home = session.sofia_home().await;
    std::fs::create_dir_all(&sofia_home).expect("create sofia home");
    let config_toml_path = sofia_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = sofia_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let sofia_home = session.sofia_home().await;
    std::fs::create_dir_all(&sofia_home).expect("create sofia home");
    let base_config_path = sofia_home.join(CONFIG_TOML_FILE);
    let profile_config_path = sofia_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(sofia_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Sofia hooks");
    })
    .await?;
    let sofia_home = session.sofia_home().await;
    std::fs::create_dir_all(&sofia_home)?;
    let config_toml_path = sofia_home.join(CONFIG_TOML_FILE);
    let user_config: sofia_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = sofia_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = sofia_hooks::list_hooks(sofia_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..sofia_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        sofia_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: sofia_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: sofia_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: sofia_config::MatcherGroup {
                matcher: None,
                hooks: vec![sofia_config::HookHandlerConfig::Command {
        ...
        let identity = sofia_config::TomlValue::try_from(identity)?;
        sofia_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&sofia_home).expect("create sofia home");
    std::fs::write(
        sofia_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = sofia_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = sofia_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: sofia_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_initial_history_seeds_token_info_from_rollout() {
    ...
    let info1 = TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            total_tokens: 30,
            sofia_rollout_budget_units: None,
        },
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl sofia_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a sofia_extension_api::ExtensionData,
            thread_store: &'a sofia_extension_api::ExtensionData,
            turn_store: &'a sofia_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl sofia_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: sofia_extension_api::TurnStartInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl sofia_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: sofia_extension_api::TurnErrorInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl sofia_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &sofia_extension_api::ExtensionData,
            thread_store: &sofia_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: sofia_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        ...
        config.permissions.approval_policy =
            sofia_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.sofia.ensure_rollout_materialized().await;
    initial
        .sofia
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        sofia_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "sofia_core__codex_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: sofia_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            sofia_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            sofia_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            sofia_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    let sofia_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(sofia_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(sofia_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("sofia_other".to_string()),
        limit_name: Some("sofia_other".to_string()),
        normal_model_slug: None,
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(sofia_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.sofia_error_info == Some(CodexErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(sofia_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/sofia/analytics-events/events"))
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("sofia-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "sofia_thread_initialized")
                    }) {
...
async fn session_configuration_apply_client_metadata_preserves_permissions() {
    ...
    let updated = configuration
        .apply(
            &SessionSettingsUpdate {
                app_server_client_name: Some("sofia-tui".to_string()),
    ...
    assert_eq!(
        ...
        (Some("sofia-tui".to_string()), Some("1.0.0".to_string())),
    );
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = sofia_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        sofia_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let sofia_home = tempfile::tempdir().expect("create sofia home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        sofia_home.path().join(sofia_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let sofia_home = parent_config.sofia_home.clone();
    let skill_dir = sofia_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&sofia_exec_server::LOCAL_FS));
    ...
    let role_path = sofia_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let sofia_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(sofia_home.path()).await;
    config
        .features
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(sofia_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(sofia_extension_api::ExtensionRegistryBuilder::new().build()),
        sofia_extension_api::ExtensionDataInit::default(),
        ...
        Arc::new(sofia_thread_store::LocalThreadStore::new(
            sofia_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        sofia_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        sofia_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(sofia_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: sofia_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(sofia_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: sofia_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: sofia_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: sofia_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(sofia_thread_store::LocalThreadStore::new(
            sofia_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .sofia_home(config.sofia_home.to_path_buf())
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            sofia_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(sofia_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(sofia_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            sofia_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(sofia_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = sofia_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(sofia_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    ...
                    call_id,
                    sofia_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(sofia_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: sofia_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_tool_rejects_invalid_requests(
    ...
    Arc::make_mut(&mut turn_context.config)
        .permissions
        .approval_policy = sofia_config::Constrained::allow_any(AskForApproval::Never);
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(sofia_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = sofia_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        sofia_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = sofia_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(sofia_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn sofia_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<sofia_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        sofia_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: sofia_extension_api::ThreadStopInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: sofia_extension_api::ThreadStopInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl sofia_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: sofia_extension_api::TurnAbortInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(sofia_home).await;
    configure_config(&mut config);
    let state_db = None;
    let config = Arc::new(config);
    let thread_id = ThreadId::default();
    let auth_manager = AuthManager::from_auth_for_testing_with_home(auth, sofia_home.to_path_buf());
    let models_manager = models_manager_with_provider(
        config.sofia_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            sofia_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = sofia_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !sofia_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        sofia_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        sofia_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_active_reviewer_and_latest_runtime_policy(
    ...
    if live_update {
        assert_eq!(
            session
                .apply_turn_settings(
                    &old_turn.sub_id,
                    sofia_protocol::protocol::TurnSettingsUpdate {
                        approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                        ..Default::default()
                    },
                )
                .await,
            sofia_protocol::protocol::TurnSettingsUpdateOutcome::Applied
        );
    } else {
        session
            .update_settings(SessionSettingsUpdate {
    ...
    let request = sofia_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: sofia_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("sofia_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("sofia_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(sofia_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| sofia_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: sofia_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let sofia_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        turn_environments.push(TurnEnvironment::new(
            ...
            Arc::new(
                sofia_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(sofia_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(sofia_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
async fn build_initial_context_reuses_in_flight_recommendation_prewarm() {
    ...
    Mock::given(method("GET"))
        .and(path("/ps/plugins/suggested/sofia"))
...
impl sofia_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a sofia_extension_api::ExtensionData,
        thread_store: &'a sofia_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<sofia_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    sofia_extension_api::PromptFragment::developer_policy(
                        "prompt extension enabled",
                        sofia_extension_api::ContentItemKind("test.prompt_extension".to_string()),
                    )
                })
                .into_iter()
                .collect()
...
-> Arc<sofia_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = sofia_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl sofia_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: sofia_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<sofia_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            ...
            .then(|| {
                sofia_extension_api::PromptFragment::developer_policy(
                    ...
                    sofia_extension_api::ContentItemKind("test.turn_context".to_string()),
                )
            })
            .into_iter()
            .collect()
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<sofia_thread_store::InMemoryThreadStore> {
    let store = Arc::new(sofia_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn sofia_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &sofia_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: sofia_extension_api::ThreadIdleInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = sofia_protocol::user_input::TextElement::new(
        sofia_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl sofia_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: sofia_extension_api::ThreadIdleInput<'a>,
        ) -> sofia_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(sofia_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn active_turn_keeps_first_root_when_mail_coalesces(inherited_root: Option<&str>) {
    ...
    .enumerate()
    {
        sess.input_queue
            .enqueue_mailbox_communication(
                communication,
                sofia_protocol::turn_input::TurnStartOptions {
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(sofia_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: sofia_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let sofia_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_codex = project_root.join(".sofia");
    let nested_dot_codex = nested.join(".sofia");
    ...
    write_project_trust_config(&sofia_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .sofia_home(sofia_home)
    ...
    let expected_source_path = sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".sofia");
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let sofia_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&sofia_home)?;
        write_project_trust_config(&sofia_home, &trust_entries).await?;
