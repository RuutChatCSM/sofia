...
use sofia_protocol::protocol::HookCompletedEvent;
...
pub(crate) struct Session {
    ...
    pub(super) mcp_elicitation_reviewer_handle: OnceLock<sofia_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<sofia_mcp::ElicitationLifecycle>,
    ...
    /// Remaining goal-judge continuations for the current turn. Reset to a
    /// bounded ceiling at the start of each agent turn; decremented each time
    /// the goal judge reports that work remains and the turn is re-sampled.
    pub(crate) goal_judge_budget: AtomicU32,
}

/// Chooses the provider recorded for a thread snapshot. A `provider/model`
/// namespaced slug carries its provider, so follow it; otherwise keep the
/// session's configured provider. This keeps the persisted `(model, provider)`
/// pair consistent when the user switches to another provider's model.
fn resolve_model_provider_id(model: &str, fallback: &str) -> String {
...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all Sofia state for this session.
    pub(super) sofia_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn sofia_home(&self) -> &AbsolutePathBuf {
        &self.sofia_home
    }
    ...
    pub(super) fn sandbox_policy(
        ...
        sofia_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
    ...
    pub(super) fn thread_config_snapshot(
        ...
        ThreadConfigSnapshot {
            ...
            model_provider_id: resolve_model_provider_id(
            ...
            full_access: sofia_protocol::protocol::has_full_access(
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: sofia_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
async fn warm_plugins_and_skills_for_session_init(
    ...
    extensions: &sofia_extension_api::ExtensionRegistry<Config>,
) -> Vec<SkillError> {
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        environment_selections: &[TurnEnvironmentSelection],
        config: Arc<Config>,
        user_instructions: Option<sofia_extension_api::Instructions>,
        ...
        code_mode_session_provider: Arc<dyn sofia_code_mode::CodeModeSessionProvider>,
        extensions: Arc<sofia_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: sofia_sandboxing::WindowsSandboxProxySettingsMode,
            ...
            ForkPersistence::Copied => match &initial_history {
                InitialHistory::Resumed(resumed) => {
                    ...
                    resumed.history.first().and_then(|item| match item {
                        ...
                        {
                            sofia_rollout::forked_from_ordinal_exclusive(
                                &meta.meta,
        ...
        thread_extension_init.insert(sofia_extension_api::ThreadOriginator(
        ...
        let thread_extension_data = sofia_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let config_path = config.sofia_home.join(CONFIG_TOML_FILE);
            ...
            let auth_env_telemetry = collect_auth_env_telemetry(
                ...
                auth_manager.sofia_api_key_env_enabled(),
            );
            ...
            let is_worktree = session_configuration.cwd().canonicalize().ok().and_then(|cwd| {
                sofia_git_utils::repository_identity(&cwd).and_then(|_| {
            ...
            let mcp_server_names =
                sofia_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                sofia_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(sofia_extension_api::ThreadStartInput {
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => sofia_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    sofia_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => sofia_hooks::SessionStartSource::Clear,
            };
