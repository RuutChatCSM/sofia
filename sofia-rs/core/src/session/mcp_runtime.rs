...
//! inputs and the mutable [`sofia_mcp::McpRuntime`]. Background scheduling
//! belongs elsewhere.
...
impl Session {
        ...
        Box::pin(async move {
            ...
            for selected in environments.turn_environments() {
                ...
                for (name, mut server) in servers {
                    ...
                    self.services.session_telemetry.counter(
                        "sofia.mcp.executor_discovery.server",
                        /*inc*/ 1,
                        &[("server_name", name.as_str()), ("outcome", outcome)],
                    );
                    if name == SOFIA_APPS_MCP_SERVER_NAME
                    ...
                    catalog.build_with_environment_authority(|environment_id| {
                        let Some(selection) = selections
                            ...
                            return if environment_id
                                == sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID
                            {
    ...
    pub(super) fn build_mcp_runtime_input(
        ...
        config
            .environment_cwds
            .entry(sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string())
        ...
        McpRuntimeInput {
            ...
            runtime_context,
            sofia_apps_tools_cache: self.services.mcp_manager.sofia_apps_tools_cache(),
