...
use serde_json::Map;
use sofia_exec_server::ExecutorCapabilityDiscoveryCache;
use sofia_exec_server::ExecutorCapabilityDiscoverySnapshot;
use sofia_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use sofia_exec_server::ResolvedSelectedCapabilityRoot;
use sofia_mcp::ElicitationReviewRequest;
use sofia_mcp::ElicitationReviewer;
use sofia_mcp::ElicitationReviewerHandle;
...
use sofia_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use sofia_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use sofia_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use sofia_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use sofia_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use sofia_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use sofia_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use sofia_protocol::mcp_approval_meta::PERSIST_KEY as MCP_ELICITATION_PERSIST_KEY;
use sofia_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use sofia_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use sofia_protocol::mcp_approval_meta::SENSITIVE_ACTION_KEY as MCP_ELICITATION_SENSITIVE_ACTION_KEY;
use sofia_protocol::mcp_approval_meta::STRICT_AUTO_REVIEW_KEY as MCP_ELICITATION_STRICT_AUTO_REVIEW_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use sofia_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use sofia_protocol::openai_models::ModelInfo;
use sofia_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        sofia_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn refresh_codex_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<sofia_mcp::CodexAppsToolSnapshot> {
    ...
    pub(crate) async fn hard_refresh_latest_codex_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<sofia_mcp::ToolInfo>> {
        self.refresh_mcp_if_dirty().await;
        let _refresh = self
            .mcp_refresh
            .acquire()
        ...
        anyhow::ensure!(
            input.mcp_servers.contains_key(SOFIA_APPS_MCP_SERVER_NAME),
            "unknown MCP server '{SOFIA_APPS_MCP_SERVER_NAME}'"
        );
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        Arc::new(sofia_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> sofia_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                sofia_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: sofia_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                sofia_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                sofia_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    let user_cua_execution = step_settings.approvals_reviewer() == ApprovalsReviewer::User
        ...
        && request.elicitation.meta().is_some_and(|meta| {
            metadata_str(meta, MCP_ELICITATION_TOOL_NAME_KEY) == Some("js")
                ...
                && meta.get("sofia_requires_user_input") != Some(&Value::Bool(true))
        });
    ...
    let guardian_request = if strict_auto_review {
        ...
        let trusted_guardian_request = if request.server_name == SOFIA_APPS_MCP_SERVER_NAME {
            let Some(call_id) = request
                .elicitation
                .meta()
        ...
        match approval_policy {
            AskForApproval::Never => {
                ...
                if sofia_mcp::mcp_permission_prompt_is_auto_approved(
                    approval_policy,
                    permission_profile,
                    sofia_mcp::McpPermissionPromptAutoApproveContext::default(),
                ) && matches!(
                    &request.elicitation,
                    Elicitation::Mcp(
        ...
        match guardian_elicitation_review_request(&request, originating_call_id) {
            ...
            GuardianElicitationReview::Decline(reason) => {
                ...
                crate::guardian::ReviewAction {
                    ...
                    category: sofia_protocol::openai_models::GuardianScope::for_mcp_server(
                        &request.server_name,
                    ),
    ...
    let decision = crate::guardian::decide_approval(
        ...
        crate::guardian::GuardianReviewOptions {
            ...
            approval_request_source: sofia_analytics::GuardianApprovalRequestSource::MainTurn,
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let sofia_protocol::approvals::ElicitationRequest::Form {
