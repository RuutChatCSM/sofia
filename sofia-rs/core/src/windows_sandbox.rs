...
use sofia_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(sofia_home: &Path) -> bool {
    sofia_windows_sandbox::sandbox_setup_is_complete(sofia_home)
}
...
pub fn prepare_elevated_sandbox(
    ...
    if !sandbox_setup_is_complete(sofia_home) {
        let permissions =
            sofia_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                permission_profile,
                workspace_roots,
            )?;
        sofia_windows_sandbox::run_elevated_setup(sofia_windows_sandbox::SandboxSetupRequest {
    ...
    sofia_windows_sandbox::run_setup_refresh(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
) -> std::io::Result<sofia_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(sofia_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(sofia_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    sofia_windows_sandbox::run_elevated_provisioning_setup(
        sofia_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    sofia_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        sofia_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    sofia_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let sofia_home = request.sofia_home;
    let setup_codex_home = sofia_home.clone();
    ...
    ConfigEditsBuilder::new(sofia_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let _ = metrics.record_duration(
        "sofia.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "sofia.windows_sandbox.setup_success",
        /*inc*/ 1,
...
pub fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "sofia.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = sofia_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(sofia_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if sofia_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
                        sofia_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
                    )
                }) {
                    "sofia.windows_sandbox.elevated_setup_canceled"
                } else {
                    "sofia.windows_sandbox.elevated_setup_failure"
                };
            let _ = metrics.counter(metric_name, /*inc*/ 1, &failure_tags);
        }
    } else {
        let _ = metrics.counter(
            "sofia.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }
