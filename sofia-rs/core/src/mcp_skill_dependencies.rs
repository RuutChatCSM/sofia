...
use sofia_config::load_global_mcp_servers;
use sofia_login::default_client::is_first_party_originator;
...
use sofia_skills::SkillToolDependency;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(sofia_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&sofia_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&sofia_home)
