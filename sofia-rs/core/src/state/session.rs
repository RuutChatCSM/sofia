...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_sources: VecDeque<sofia_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    pub(crate) fn queue_pending_session_start_source(
        &mut self,
        value: sofia_hooks::SessionStartSource,
    ) {
...
// missing values as the default `"sofia"` bucket.
fn merge_rate_limit_fields(
    previous: Option<&RateLimitSnapshot>,
    mut snapshot: RateLimitSnapshot,
) -> RateLimitSnapshot {
    if snapshot.limit_id.is_none() {
        snapshot.limit_id = Some("sofia".to_string());
    }
