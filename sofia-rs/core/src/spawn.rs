...
///
/// 1. The process was spawned by Sofia as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    let inherited_fd = env
        .get(sofia_shell_escalation::ESCALATE_SOCKET_ENV_VAR)
    ...
    // If this Sofia process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                sofia_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            // macOS cannot receive the fd with close-on-exec set atomically.
            #[cfg(target_os = "macos")]
            sofia_utils_pty::pty::close_inherited_fds_except(inherited_fd.as_slice());
            Ok(())
        });
    }
