...
use sofia_protocol::intersect_effective_permission_profiles;
    ...
    pub(crate) async fn restore_v2_agent_metadata(
        ...
        let descendant_ids = match agent_graph_store
            .list_thread_spawn_descendants(
                root_thread_id,
                Some(sofia_agent_graph_store::ThreadSpawnEdgeStatus::Open),
            )
            .await
        {
            ...
            match item {
                ...
                RolloutItem::Compacted(compacted) => {
                    ...
                    compacted.retained_context = (context_mode == GuardianContextMode::ThreadOwned
                        && multi_agent_version == MultiAgentVersion::V2)
                        .then(sofia_history::RetainedContext::default);
