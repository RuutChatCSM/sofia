...
use anyhow::anyhow;
use serde::Serialize;
use sofia_agent_roles::parse_agent_role_file_contents;
use sofia_config::ConfigLayerEntry;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLayerStack;
use sofia_config::SkillsConfig;
use sofia_config::loader::resolve_relative_paths_in_config_toml;
use sofia_exec_server::read_sensitive_file_to_string;
...
async fn apply_role_to_config_inner(
    ...
    let role_config = deserialize_config_toml_with_base(role_layer_toml, &config.sofia_home)?;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.sofia_home.as_path())
    } else {
