...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.sofia_linux_sandbox_exe = Some(PathBuf::from("/tmp/sofia-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/sofia-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.sofia_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/sofia-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/sofia-execve-wrapper"))
    );
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&sofia_exec_server::LOCAL_FS)),
        )
        .await;
