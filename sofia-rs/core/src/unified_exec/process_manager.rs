...
use crate::exec_env::SOFIA_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::SOFIA_THREAD_ID_ENV_VAR;
use crate::exec_env::SOFIA_VERSION_ENV_VAR;
...
use crate::sofia_thread::BackgroundTerminalInfo;
...
use sofia_core_plugins::PLUGIN_METRICS_OUTPUT_ENV_VAR;
use sofia_core_plugins::PluginCommandAttribution;
use sofia_core_plugins::PluginMetricsSidecar;
use sofia_core_plugins::strip_output_env;
use sofia_network_proxy::NetworkPolicyDecider;
...
use sofia_protocol::protocol::TerminalInteractionEvent;
use sofia_protocol::shell_environment::is_non_inheritable_env_var;
use sofia_sandboxing::SandboxCommand;
use sofia_shell_command::is_dangerous_command::DangerousCommandPlatform;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Sofia sandbox network proxy.";
...
pub(super) fn exec_env_policy_from_shell_policy(
...
) -> sofia_exec_server::ExecEnvPolicy {
    ...
    exclude.extend([
        ...
        SOFIA_VERSION_ENV_VAR.to_string(),
        sofia_apply_patch::SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
        PLUGIN_METRICS_OUTPUT_ENV_VAR.to_string(),
    ]);
    let mut r#set = policy.r#set.clone();
    r#set.retain(|key, _| {
        ![
            SOFIA_PERMISSION_PROFILE_ENV_VAR,
            SOFIA_VERSION_ENV_VAR,
            sofia_apply_patch::SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn exec_server_env_for_request(
    ...
    Option<sofia_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    sofia_exec_server::ExecParams {
        ...
        metadata: tool_ctx.map(|ctx| sofia_exec_server::ExecMetadata {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Sofia(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn write_stdin(
        ...
        if let Some((approval, approval_reason)) = approval {
            ...
            context.step_context.session_telemetry.counter(
                "sofia.unified_exec.stdin_review.size_check",
                /*inc*/ 1,
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<sofia_network_proxy::RemoteNetworkProxyLaunchConfig>,
        environment_id: Option<&str>,
        exec_server_env_config: Option<ExecServerEnvConfig>,
        shell_snapshot: Option<sofia_exec_server::ShellSnapshotRequest>,
        windows_sandbox_proxy_settings_mode: sofia_sandboxing::WindowsSandboxProxySettingsMode,
        tty: bool,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Sofia(CodexErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == sofia_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == sofia_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(sofia_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = sofia_sandboxing::spawn_process(sofia_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            SOFIA_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Sofia(err) => match err.details() {
