...
use sofia_exec_server::ExecProcessEventReceiver;
use sofia_exec_server::ExecProcessFuture;
...
use sofia_exec_server::WriteResponse;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, sofia_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, sofia_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), sofia_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(sofia_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        "export SOFIA_INTERACTIVE_SHELL_VAR=sofia\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "echo $SOFIA_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        ...
        &turn,
        process_id,
        format!("export SOFIA_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        ...
        &turn,
        process_id,
        "sleep 5 && echo $SOFIA_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = sofia_exec_server::Environment::default_for_tests();
...
async fn stdin_approval_preserves_the_reviewed_terminal() -> anyhow::Result<()> {
    ...
    let (session, mut turn, events) = make_session_and_context_with_auth_and_config_and_rx(
        sofia_login::CodexAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {
