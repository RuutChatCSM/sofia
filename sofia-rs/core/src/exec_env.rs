pub use sofia_apply_patch::SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
pub use sofia_protocol::shell_environment::SOFIA_SESSION_ID_ENV_VAR;
pub use sofia_protocol::shell_environment::SOFIA_THREAD_ID_ENV_VAR;

pub(crate) const SOFIA_VERSION_ENV_VAR: &str = "SOFIA_VERSION";
...
pub const SOFIA_PERMISSION_PROFILE_ENV_VAR: &str = "SOFIA_PERMISSION_PROFILE";
...
///
/// `SOFIA_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_session_env(env: &mut HashMap<String, String>, session_id: SessionId) {
    env.insert(SOFIA_SESSION_ID_ENV_VAR.to_string(), session_id.to_string());
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(SOFIA_VERSION_ENV_VAR));
    }
    env.insert(
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(SOFIA_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(SOFIA_PERMISSION_PROFILE_ENV_VAR);
    }
...
pub fn inject_apply_patch_env(env: &mut HashMap<String, String>, features: &Features) {
    env.retain(|key, _| !key.eq_ignore_ascii_case(SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR));
