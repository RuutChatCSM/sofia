...
use sofia_analytics::CompactionTrigger;
use sofia_analytics::HookRunFact;
...
use sofia_core_plugins::executor_plugin_hook_sources;
use sofia_hooks::InterruptRequest;
use sofia_hooks::PermissionRequestDecision;
use sofia_hooks::PermissionRequestOutcome;
use sofia_hooks::PermissionRequestRequest;
use sofia_hooks::PostToolUseOutcome;
use sofia_hooks::PostToolUseRequest;
use sofia_hooks::PreToolUseOutcome;
use sofia_hooks::PreToolUseRequest;
use sofia_hooks::SessionStartOutcome;
use sofia_hooks::StartHookTarget;
use sofia_hooks::StopHookTarget;
use sofia_hooks::StopOutcome;
use sofia_hooks::SubagentHookContext;
use sofia_hooks::UserPromptSubmitOutcome;
use sofia_hooks::UserPromptSubmitRequest;
use sofia_hooks::hook_execution_mode_label;
use sofia_hooks::hook_handler_type_label;
use sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME;
use sofia_otel::HOOK_RUN_DURATION_METRIC;
use sofia_otel::HOOK_RUN_METRIC;
use sofia_plugin::ExecutorPluginHookSource;
use sofia_protocol::items::FunctionCallOutputItem;
...
use sofia_protocol::protocol::HookOutputEntryKind;
use sofia_protocol::protocol::HookRunStatus;
use sofia_protocol::protocol::HookRunSummary;
use sofia_protocol::protocol::HookSource;
use sofia_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = sofia_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = sofia_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = sofia_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = sofia_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(sofia_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: sofia_hooks::HookEvent::AfterAgent {
                event: sofia_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            sofia_hooks::HookResult::Success => continue,
            sofia_hooks::HookResult::FailedContinue(error) => (error, false),
            sofia_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(sofia_protocol::protocol::ErrorEvent {
        misalignment: None,
        message,
        sofia_error_info: Some(CodexErrorInfo::Other),
    });
...
fn hook_run_analytics_payload(
    ...
    turn_context: &TurnContext,
    completed: &HookCompletedEvent,
) -> (sofia_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use sofia_protocol::protocol::HookHandlerType;
    use sofia_protocol::protocol::HookRunStatus;
    use sofia_protocol::protocol::HookScope;
    use sofia_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    sofia_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_hide_builtin_and_async_runs_but_preserve_metrics() {
        ...
        assert!(matches!(
            started.msg,
            sofia_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            sofia_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
