...
use serde_json::json;
use sofia_guardian_context::RenderedVerifiedAnswers;
...
async fn multi_agent_v2_request_user_input_rejects_subagent_threads() {
    ...
    .handle(ToolInvocation {
        ...
        tool_name: sofia_tools::ToolName::plain(REQUEST_USER_INPUT_TOOL_NAME),
        ...
        *session.active_turn.lock().await = Some(ActiveTurn::default());
        ...
        if thread_context_enabled {
            let RenderedVerifiedAnswers {
                fragments,
                complete,
            } = sofia_guardian_context::render_verified_answers(
        ...
        match &answer {
            Some(("pick_one", answer)) if !thread_context_enabled && !answer.trim().is_empty() => {
                ...
                assert!(sofia_utils_output_truncation::approx_token_count(&fragment) <= 900);
                assert_eq!(
