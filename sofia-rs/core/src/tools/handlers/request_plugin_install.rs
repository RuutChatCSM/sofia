...
use sofia_analytics::PluginInstallRequestSource;
use sofia_analytics::PluginInstallRequested;
use sofia_analytics::PluginInstallRequestedPlugin;
...
use sofia_tools::DiscoverableToolAction;
use sofia_tools::DiscoverableToolType;
use sofia_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use sofia_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use sofia_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use sofia_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use sofia_tools::RequestPluginInstallArgs;
use sofia_tools::RequestPluginInstallResult;
use sofia_tools::ToolName;
use sofia_tools::ToolSpec;
use sofia_tools::all_requested_connectors_picked_up;
use sofia_tools::build_request_plugin_install_elicitation_request;
use sofia_tools::filter_request_plugin_install_discoverable_tools_for_client;
use sofia_tools::verified_connector_install_completed;
use tracing::warn;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("sofia-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in sofia-tui yet".to_string(),
            ));
        }
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match sofia_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.sofia_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    sofia_home: &sofia_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&sofia_login::CodexAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_codex_apps_tools().await {
        ...
        Err(err) => {
            warn!(
                "failed to refresh sofia apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
