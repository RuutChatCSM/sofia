...
use sofia_protocol::protocol::FileSystemAccessMode;
use sofia_protocol::protocol::FileSystemPath;
use sofia_protocol::protocol::FileSystemSandboxEntry;
use sofia_protocol::protocol::FileSystemSandboxPolicy;
use sofia_protocol::protocol::InterAgentCommunication;
use sofia_protocol::protocol::ItemCompletedEvent;
use sofia_protocol::protocol::NetworkSandboxPolicy;
...
fn invocation(
    ...
    ToolInvocation {
        ...
        tool_name: sofia_tools::ToolName::plain(tool_name),
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.sofia_home)
        .await
        .expect("sofia home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    sofia_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let mut turn = turn;
    turn.config = Arc::new(config);
    turn.multi_agent_version = sofia_protocol::protocol::MultiAgentVersion::V2;
...
async fn spawn_agent_role_service_tier_cannot_override_root_preference() {
    ...
    tokio::fs::create_dir_all(&turn.config.sofia_home)
        .await
        .expect("sofia home should be created");
    let role_config_path = turn.config.sofia_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn spawn_agent_reapplies_runtime_sandbox_after_role_config() {
    ...
    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        ...
        turn.config.sofia_home.to_path_buf(),
...
async fn build_agent_spawn_config_uses_turn_context_values(parent_enabled: bool) {
    ...
    config.sofia_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
