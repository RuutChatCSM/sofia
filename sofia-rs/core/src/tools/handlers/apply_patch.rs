...
use sofia_apply_patch::ApplyPatchAction;
use sofia_apply_patch::ApplyPatchFileChange;
use sofia_apply_patch::ApplyPatchFileUpdateMode;
use sofia_apply_patch::Hunk;
use sofia_apply_patch::StreamingPatchParser;
...
use sofia_protocol::protocol::PatchApplyUpdatedEvent;
use sofia_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use sofia_sandboxing::policy_transforms::merge_permission_profiles;
use sofia_sandboxing::policy_transforms::normalize_additional_permissions;
use sofia_sandboxing::policy_transforms::normalize_additional_permissions_with_context;
...
fn format_update_chunks_for_progress(chunks: &[sofia_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    file_paths: &[PathUri],
    file_system_sandbox_policy: &sofia_protocol::permissions::FileSystemSandboxPolicy,
...
async fn effective_patch_permissions(
    ...
    sofia_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match sofia_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match sofia_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            sofia_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            sofia_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            sofia_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            sofia_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    let sandbox = turn_environment.sandbox_context(/*additional_permissions*/ None);
    match sofia_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        sofia_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
