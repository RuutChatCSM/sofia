...
pub struct McpHandler {
    tool_info: ToolInfo,
    spec: Arc<ToolSpec>,
    code_mode_tool_definitions: OnceLock<Vec<sofia_code_mode::ToolDefinition>>,
}
...
impl McpHandler {
    async fn handle_call(
        ...
        let truncation_policy = prepared_mcp_call
            .as_ref()
            .and_then(sofia_mcp::PreparedMcpCall::output_token_limit)
...
impl CoreToolRuntime for McpHandler {
    ...
    fn cached_code_mode_definitions(&self) -> Option<&[sofia_code_mode::ToolDefinition]> {
        Some(
            self.code_mode_tool_definitions
                .get_or_init(|| {
                    let mut definitions = sofia_tools::collect_code_mode_tool_definitions(
    ...
    fn on_tool_result_accepted(&self, invocation: &ToolInvocation, result: &dyn ToolOutput) {
        ...
        let is_encrypted = |item: &Value| {
            item.get("_meta")
                .and_then(|meta| meta.get("sofia/encryptedContent"))
            ...
            .filter_map(|item| {
                ...
                match item.get("type").and_then(Value::as_str) {
                            ...
                            || image_capture_enabled =>
                    {
                        ...
                        let detail = item
                            .get("_meta")
                            .and_then(|meta| meta.get("sofia/imageDetail"))
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: sofia_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: sofia_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            ...
            truncation_policy: sofia_utils_output_truncation::TruncationPolicy::Bytes(1024),
        };
        let (session, turn) = make_session_and_context().await;
        let turn = Arc::new(turn);
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: sofia_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(
