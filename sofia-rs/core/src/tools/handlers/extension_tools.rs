...
use sofia_tools::ConversationHistory;
use sofia_tools::ExtensionTurnItem;
use sofia_tools::ResponsesApiNamespaceTool;
use sofia_tools::ToolCall as ExtensionToolCall;
use sofia_tools::ToolEnvironment;
use sofia_tools::ToolName;
use sofia_tools::ToolSearchInfo;
use sofia_tools::ToolSpec;
use sofia_tools::TurnItemEmissionFuture;
use sofia_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(
    Arc<dyn for<'call> sofia_tools::ToolExecutor<ExtensionToolCall<'call>>>,
);

impl ExtensionToolAdapter {
    pub(crate) fn new(
        executor: Arc<dyn for<'call> sofia_tools::ToolExecutor<ExtensionToolCall<'call>>>,
    ) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall<'_> {
    ...
    let sofia_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use sofia_extension_items::web_search::WebSearchItem;
    use sofia_protocol::items::TurnItem;
    ...
    use sofia_tools::ExtensionTurnItem;
    use sofia_tools::ToolCallSource as ExtensionToolCallSource;
    ...
    impl<'call> sofia_extension_api::ToolExecutor<sofia_tools::ToolCall<'call>>
        for StubExtensionExecutor
    {
        fn tool_name(&self) -> sofia_tools::ToolName {
            sofia_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> sofia_tools::ToolSpec {
            sofia_tools::ToolSpec::Function(sofia_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: sofia_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle<'a>(
            &'a self,
            _call: sofia_tools::ToolCall<'call>,
        ) -> sofia_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
            Box::pin(async {
                Ok(
                    Box::new(sofia_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn sofia_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<sofia_tools::ToolCall<'static>>>>,
        ...
        for CapturingExtensionExecutor
    {
        ...
        fn handle<'a>(
            &'a self,
            call: sofia_tools::ToolCall<'call>,
        ) -> sofia_tools::ToolExecutorFuture<'a>
        where
            'call: 'a,
        {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: sofia_tools::ToolCall<'_>,
        ) -> Result<Box<dyn sofia_tools::ToolOutput>, sofia_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: sofia_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = sofia_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        sofia_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        sofia_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
