...
use sofia_execpolicy::PolicyParser;
...
use sofia_sandboxing::policy_transforms::effective_permission_profile;
use sofia_shell_escalation::EscalationExecution;
use sofia_shell_escalation::EscalationPermissions;
use sofia_shell_escalation::ResolvedPermissionProfile;
...
async fn unsandboxed_intercepted_exec_strips_managed_network_env() -> anyhow::Result<()> {
    ...
    let executor = CoreShellCommandExecutor {
        sandbox_manager: sofia_sandboxing::SandboxManager::new(),
        command: Vec::new(),
        cwd: workdir.clone(),
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(sofia_execpolicy::Policy::empty())),
        ...
        stopwatch: sofia_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = sofia_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = sofia_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.sofia_home)
        .context("recreate sofia home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .sofia_home
        .join(sofia_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        sofia_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    ...
    session
        ...
        .await;
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(sofia_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        sofia_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        sofia_shell_escalation::EscalationDecision::Escalate(
            sofia_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, sofia_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        sofia_shell_escalation::EscalationDecision::Deny {
