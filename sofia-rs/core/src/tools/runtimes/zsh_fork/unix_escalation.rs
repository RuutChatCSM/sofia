...
use sofia_execpolicy::MatchOptions;
...
use sofia_protocol::protocol::NetworkPolicyRuleAction;
...
use sofia_shell_command::bash::parse_shell_lc_plain_commands;
use sofia_shell_escalation::EscalateServer;
use sofia_shell_escalation::EscalationDecision;
use sofia_shell_escalation::EscalationExecution;
use sofia_shell_escalation::EscalationPermissions;
use sofia_shell_escalation::EscalationPolicy;
use sofia_shell_escalation::EscalationPolicyFuture;
use sofia_shell_escalation::EscalationSession;
use sofia_shell_escalation::ExecResult;
use sofia_shell_escalation::PreparedExec;
use sofia_shell_escalation::ResolvedPermissionProfile;
use sofia_shell_escalation::ShellCommandExecutor;
use sofia_shell_escalation::ShellCommandExecutorFuture;
use sofia_shell_escalation::Stopwatch;
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sofia_linux_sandbox_exe: ctx.step_context.turn.config.sofia_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::Sofia(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<sofia_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["SOFIA_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            sofia_linux_sandbox_exe: self.sofia_linux_sandbox_exe.as_deref(),
