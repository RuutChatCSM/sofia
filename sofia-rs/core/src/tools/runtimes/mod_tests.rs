...
use sofia_network_proxy::CUSTOM_CA_ENV_KEYS;
use sofia_network_proxy::ConfigReloader;
use sofia_network_proxy::ConfigReloaderFuture;
use sofia_network_proxy::ConfigState;
use sofia_network_proxy::NetworkProxy;
use sofia_network_proxy::NetworkProxyConfig;
use sofia_network_proxy::NetworkProxyConstraints;
use sofia_network_proxy::NetworkProxyState;
use sofia_network_proxy::PROXY_ACTIVE_ENV_KEY;
use sofia_network_proxy::PROXY_ENV_KEYS;
#[cfg(target_os = "macos")]
use sofia_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
#[cfg(target_os = "macos")]
use sofia_network_proxy::SOFIA_PROXY_GIT_SSH_COMMAND_MARKER;
use sofia_protocol::config_types::WindowsSandboxLevel;
use sofia_protocol::models::PermissionProfile;
use sofia_sandboxing::SandboxManager;
use sofia_sandboxing::SandboxType;
...
async fn test_network_proxy_with_config(
    ...
    let state =
        sofia_network_proxy::build_config_state(config, NetworkProxyConstraints::default())?;
...
async fn explicit_escalation_prepares_exec_without_managed_network() -> anyhow::Result<()> {
    ...
    let command = sofia_sandboxing::SandboxCommand {
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/sofia-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/sofia-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/sofia-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/sofia-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/sofia-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/sofia-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/sofia-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/sofia-resources/zsh/bin:/bin:/package/sofia-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/sofia-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
fn maybe_wrap_shell_lc_with_snapshot_restores_codex_metadata_from_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_THREAD_ID='parent-thread'\nexport SOFIA_VERSION='old-version'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s|%s' \"$SOFIA_THREAD_ID\" \"$SOFIA_VERSION\"".to_string(),
    ];
    let env = HashMap::from([
        ("SOFIA_THREAD_ID".to_string(), "nested-thread".to_string()),
        (
...
fn maybe_wrap_shell_lc_with_snapshot_restores_permission_profile_from_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_PERMISSION_PROFILE='parent-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv SOFIA_PERMISSION_PROFILE".to_string(),
    ];
    let env = HashMap::from([(
        SOFIA_PERMISSION_PROFILE_ENV_VAR.to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(SOFIA_PERMISSION_PROFILE_ENV_VAR, "current-profile")
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_unsets_absent_permission_profile() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_PERMISSION_PROFILE='stale-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(SOFIA_PERMISSION_PROFILE_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_apply_patch_rollout_state() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS='stale'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS".to_string(),
    ];
    let env = HashMap::from([(
        SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR.to_string(),
        "1".to_string(),
    )]);
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR, "1")
        .output()
        .expect("run rewritten command");
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_reserved_metrics_output_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_PLUGIN_METRICS_OUTPUT='/stale/path'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"${SOFIA_PLUGIN_METRICS_OUTPUT-unset}\"".to_string(),
    ];
...
fn maybe_wrap_shell_lc_with_snapshot_refreshes_codex_proxy_git_ssh_command() {
    ...
    let stale_command = format!(
        "{SOFIA_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{SOFIA_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("sofia-path");
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport SOFIA_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${SOFIA_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$SOFIA_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "SOFIA_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("SOFIA_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");
