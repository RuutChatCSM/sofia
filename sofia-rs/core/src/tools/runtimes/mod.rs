...
use crate::exec_env::SOFIA_SESSION_ID_ENV_VAR;
use crate::exec_env::SOFIA_THREAD_ID_ENV_VAR;
use crate::exec_env::SOFIA_VERSION_ENV_VAR;
use crate::sandboxing::SandboxPermissions;
use crate::shell::Shell;
use crate::shell::ShellType;
use sofia_apply_patch::SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR;
...
pub(crate) use sofia_network_proxy::is_managed_proxy_env_var;
pub(crate) use sofia_network_proxy::strip_managed_proxy_env;
...
pub(crate) fn exec_env_for_sandbox_permissions(
    ...
    // defensive cleanup for a caller that passes an environment already prepared by Sofia.
...
/// PATH entries owned by Sofia runtime setup.
///
...
pub(crate) fn prepare_powershell_command_for_elevated_windows_sandbox(
    ...
    prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
        ...
        |path| {
            sofia_shell_command::shell_detect::fallback_powershell_shell_for_elevated_windows_sandbox(
                path,
            )
        },
    )
...
fn prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
    ...
    windows_sandbox_level: WindowsSandboxLevel,
    environment_is_remote: bool,
    find_fallback: impl FnOnce(&Path) -> Option<sofia_shell_command::shell_detect::DetectedShell>,
) -> Vec<String> {
...
/// runtime-only vars like `SOFIA_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Sofia-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        SOFIA_SESSION_ID_ENV_VAR,
        SOFIA_THREAD_ID_ENV_VAR,
        SOFIA_VERSION_ENV_VAR,
        SOFIA_PERMISSION_PROFILE_ENV_VAR,
        SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR,
...
fn build_proxy_env_exports(env: &HashMap<String, String>) -> (String, String) {
    let mut keys = PROXY_ENV_KEYS
        .iter()
        .copied()
        .chain(sofia_network_proxy::brokered_credential_env_keys(env))
...
fn build_codex_proxy_git_ssh_command_exports() -> (String, String) {
    ...
    let marker_pattern = format!("{}\\ *", SOFIA_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
...
mod prepare_powershell_command_tests {
    ...
    fn local_elevated_powershell_uses_discovered_fallback() {
        ...
        let rewritten = prepare_powershell_command_for_elevated_windows_sandbox_with_fallback(
            ...
            /*environment_is_remote*/ false,
            |_| {
                Some(sofia_shell_command::shell_detect::DetectedShell {
