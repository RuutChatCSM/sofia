...
fn approval_resolution_aborts_turn_when_approval_is_aborted() {
    ...
    assert!(matches!(
        resolution.into_tool_result(&model_info_from_slug("acting-model")),
        Err(ToolError::Sofia(error))
            if matches!(
                error.details(),
                sofia_protocol::error::CodexErrorDetails::TurnAborted
            )
    ));
}
...
#[cfg(unix)]
#[test_case::test_case(ApprovalsReviewer::User, sofia_extension_api::ApprovalDecision::AskUser; "manual prompt")]
#[test_case::test_case(ApprovalsReviewer::AutoReview, sofia_extension_api::ApprovalDecision::Allow; "cached allow")]
#[tokio::test]
async fn non_utf8_cwd_preserves_approval_routing(
    reviewer: ApprovalsReviewer,
    decision: sofia_extension_api::ApprovalDecision,
) -> anyhow::Result<()> {
    use anyhow::Context;
    use sofia_extension_api::ApprovalDecision;
    use std::os::unix::ffi::OsStringExt;

    struct Contributor {
        cwd: sofia_utils_path_uri::LegacyAppPathString,
        decision: ApprovalDecision,
    }

    impl sofia_extension_api::ApprovalReviewContributor for Contributor {
        fn decide<'a>(
            &'a self,
            input: &'a sofia_extension_api::ApprovalDecisionInput<'_>,
        ) -> sofia_extension_api::ExtensionFuture<'a, Option<ApprovalDecision>> {
            Box::pin(async move {
    ...
    extensions.approval_review_contributor(Arc::new(Contributor {
        cwd: sofia_utils_path_uri::LegacyAppPathString::from_path_uri(&cwd, PathConvention::Posix)?,
        decision,
    }));
    ...
    *session.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
    ...
    let expected = if reviewer == ApprovalsReviewer::User {
        tokio::select! {
            ...
            event = events.recv() => {
                let sofia_protocol::protocol::EventMsg::ExecApprovalRequest(request) =
                    event.context("receive user prompt")?.msg
                else {
                    panic!("expected a command approval prompt");
                };
                assert_eq!(request.cwd, sofia_utils_path_uri::LegacyAppPathString::from(cwd));
...
async fn explicit_mcp_reviewer_override_takes_precedence_over_action_context() {
    ...
    tokio::select! {
        ...
        event = events.recv() => {
            let sofia_protocol::protocol::EventMsg::ElicitationRequest(request) =
            ...
            assert_eq!(
                request.id,
                sofia_protocol::mcp::RequestId::String(
