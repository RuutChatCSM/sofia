...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(sofia_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(sofia_code_mode::WAIT_TOOL_NAME),
        ] {
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && sofia_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&sofia_code_mode::normalize_code_mode_identifier(
                &sofia_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        sofia_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && sofia_code_mode::is_code_mode_nested_tool(&sofia_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !sofia_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(sofia_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            if unified_image_budget_enabled(&turn_context.config.features, model_info) {
                sofia_code_mode::ImageDetailVisibility::Hidden
            } else {
                sofia_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, sofia_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| sofia_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &sofia_code_mode::ToolDefinition,
    right: &sofia_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, sofia_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &sofia_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, sofia_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
