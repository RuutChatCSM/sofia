...
use sofia_protocol::models::ToolResultSources;
...
fn executed_tool_call_recorder_bounds_pending_calls_and_preserves_overflow() {
    ...
    for index in 0..MAX_PENDING_EXECUTED_TOOL_CALLS + 2 {
        recorder.record_tool_call(
            &ToolCall {
                tool_name: sofia_tools::ToolName::plain("direct_tool"),
...
fn executed_tool_call_recorder_bounds_retained_history_and_reports_omissions() {
    ...
    for index in 0..512 {
        ...
        recorder.record_tool_call(
            &ToolCall {
                tool_name: sofia_tools::ToolName::plain(format!("retained_tool_{index}")),
        ...
        sofia_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt);
...
fn tool_call_completeness_survives_waits_without_changing_deltas() {
    let sources = ToolResultSources::new(vec![sofia_protocol::models::ToolResultSource {
    ...
    assert!(
        recorder.record_tool_result_sources(
            ...
            ToolResultSources::new(
                (0..32)
                    .map(|index| sofia_protocol::models::ToolResultSource {
                        r#type: "test_channel".to_string(),
