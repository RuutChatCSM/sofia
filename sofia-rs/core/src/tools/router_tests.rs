...
use sofia_extension_api::ResponsesApiTool;
use sofia_extension_api::ToolCall as ExtensionToolCall;
...
impl sofia_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl<'call> ToolExecutor<ExtensionToolCall<'call>> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: sofia_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    fn handle<'a>(&'a self, call: ExtensionToolCall<'call>) -> sofia_tools::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
...
impl ExtensionEchoExecutor {
    async fn handle_call(
        ...
        Ok(Box::new(sofia_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__sofia_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__sofia_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&visible_specs, "sofia_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        router.deferred_tool_namespaces(),
        BTreeMap::from([("sofia_app".to_string(), "Sofia app tools.".to_string())])
    );
    ...
    assert!(namespace_function_names(&updated_specs, "sofia_app").is_empty());
...
fn mcp_runtime(tool_info: sofia_mcp::ToolInfo) -> RegisteredTool {
...
async fn extension_tool_executors_are_model_visible_and_dispatchable() -> anyhow::Result<()> {
    ...
    let router = test_tool_router(
        ...
        extension_tool_executors(
            &session,
            &sofia_extension_api::ExtensionData::new(turn.sub_id.clone()),
        ),
        &turn.dynamic_tools,
    );
