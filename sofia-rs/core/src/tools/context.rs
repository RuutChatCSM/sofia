...
pub use sofia_tools::ToolOutput;
pub use sofia_tools::ToolPayload;
...
pub enum ToolCallSource {
    ...
    CodeMode {
        ...
        /// debugging the JS/runtime bridge, but it is not the Sofia tool call id
    ...
    pub(crate) async fn originating_item_id(&self) -> Option<ResponseItemId> {
        if let ToolCallSource::CodeMode { cell_id, .. } = &self.source {
            return self
                ...
                .services
                .code_mode_service
                .cell_originating_item_id(&sofia_code_mode::CellId::new(cell_id.clone()));
        }
