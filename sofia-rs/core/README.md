...
This crate implements the business logic for Sofia. It is designed to be used by the various Sofia UIs written in Rust.
...
Note that `sofia-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.sofia` read-only.
...
Expects the binary containing `sofia-core` to run the equivalent of `sofia sandbox` when `arg0` is `sofia-linux-sandbox`. See the `sofia-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `sofia-resources/bwrap`
binary shipped with Sofia and Sofia surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Sofia also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Sofia rejects sandboxed shell commands that would enter the
...
Expects the binary containing `sofia-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--sofia-run-as-apply-patch`. See the
`sofia-arg0` crate for details.
