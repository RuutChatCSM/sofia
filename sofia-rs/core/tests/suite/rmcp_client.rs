...
use sofia_config::types::McpServerAuth;
...
use sofia_protocol::mcp_policy::McpServerIdentity;
use sofia_protocol::mcp_policy::McpServerRequirement;
use sofia_protocol::mcp_policy::PluginMcpRequirements;
use sofia_protocol::models::ContentItem;
use sofia_protocol::models::PermissionProfile;
use sofia_protocol::models::PermissionProfileSnapshot;
use sofia_protocol::models::ResponseItem;
use sofia_protocol::openai_models::ConfigShellToolType;
use sofia_protocol::openai_models::ConfirmationPolicies;
use sofia_protocol::openai_models::InputModality;
use sofia_protocol::openai_models::ModelInfo;
use sofia_protocol::openai_models::ModelVisibility;
use sofia_protocol::openai_models::ModelsResponse;
use sofia_protocol::openai_models::ReasoningEffortPreset;
use sofia_protocol::openai_models::TruncationPolicyConfig;
...
use sofia_protocol::protocol::McpStartupStatus;
...
use sofia_utils_cargo_bin::cargo_bin;
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        sofia_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/sofia-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.sofia, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.sofia, |ev| {
    ...
    wait_for_event(&fixture.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn assert_openai_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.sofia, server_name).await?;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.sofia, "bounded").await?;

    fixture
        .sofia
        .start_or_steer_turn(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.sofia, |event| {
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    let sofia_home = Arc::new(tempdir()?);
    let test_env = test_env().await?;
    if from_plugin {
        let plugin_root =
            super::plugins::write_sample_plugin_manifest_and_config(sofia_home.as_ref());
    ...
    .await?;
    let (pending_config, _) = fixture.sofia.current_mcp_config_and_runtime_context().await;
    ...
    let (runtime_config, _) = fixture.sofia.current_mcp_config_and_runtime_context().await;
    ...
    fixture
        ...
        .await?;
    let (failed_config, _) = fixture.sofia.current_mcp_config_and_runtime_context().await;
...
async fn stdio_server_round_trip(server_name: &'static str, namespace: &str) -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.sofia, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_codex()
        .with_pre_build_hook(move |sofia_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.sofia, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.sofia.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before Sofia Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == SOFIA_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("Sofia Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.sofia.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.sofia.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.sofia, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    let namespace = format!("mcp__{server_name}");
    let mut models = sofia_models_manager::bundled_models_response()?;
    let model = models
        .models
        .iter_mut()
    ...
    fixture
        ...
        .list_models(
            ...
            sofia_core::test_support::default_http_client_factory(),
        )
        .await;
    ...
    let response_metadata: Value = serde_json::from_str(
        initial_request["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
    ...
    assert_eq!(
        output_json.pointer("/x-sofia-turn-metadata/node_repl_auto_review_required"),
        Some(&json!(node_repl_auto_review_required))
    );
    assert_eq!(
        output_json.pointer("/x-sofia-turn-metadata/node_repl_disabled"),
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            sofia_linux_sandbox_exe: fixture.config.sofia_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.sofia, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: sofia_protocol::openai_models::ReasoningEffort::Medium,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "SOFIA_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Sofia should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Sofia can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(mode: HeadersHelperMode) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Sofia will call the MCP echo tool
    ...
    // Phase 3: configure Sofia with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Sofia begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.sofia, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&configured_auth_fixture.sofia, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!("{}/api/sofia/ps/mcp", untrusted_apps.chatgpt_base_url);
    ...
    wait_for_mcp_server(&fixture.sofia, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/sofia/ps/mcp")
        .filter_map(|request| {
...
/// This test writes to a fallback credentials file in SOFIA_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire SOFIA_HOME through the code.
#[test]
#[serial(sofia_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Sofia will call the OAuth-backed
    ...
    // Phase 3: seed an isolated SOFIA_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", temp_home.path().as_os_str());
    let unset_authorization_env_var = format!(
        "SOFIA_TEST_UNSET_MCP_OAUTH_AUTHORIZATION_{}",
        std::process::id()
    );
    ...
    // Phase 4: configure Sofia with the OAuth-backed Streamable HTTP MCP
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `sofia mcp login {server_name}`.")
    } else {
    ...
    let startup = wait_for_event(&fixture.sofia, |event| {
    ...
    let (contended_turn, refreshed_failure, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.sofia.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.sofia.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.sofia, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(
        sofia_rmcp_client::delete_oauth_tokens(
            discovered_credential_name.as_ref(),
            http_server.url(),
            OAuthCredentialsStoreMode::File,
            sofia_config::types::AuthKeyringBackendKind::default(),
        )
        .await?
    );
    fixture.sofia.refresh_runtime_config(refreshed_config).await;
    ...
    // Phase 7: assert Sofia begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.sofia, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Sofia fixture.
...
async fn write_fallback_oauth_tokens(
    ...
    sofia_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        sofia_config::types::AuthKeyringBackendKind::default(),
