...
impl ApprovalReviewContributor for ApprovedReviewContributor {
    fn decide<'a>(
        ...
        Box::pin(async move {
            ...
            Some(sofia_extension_api::ApprovalDecision::Allow)
        })
    }
}
...
async fn remote_model_override_uses_catalog_model_for_strict_auto_review() -> Result<()> {
    ...
    let permissions_request = wait_for_event(&sofia, |event| {
        matches!(
            event,
            EventMsg::RequestPermissions(_) | EventMsg::TurnComplete(_)
        )
    ...
    timeout(Duration::from_secs(10), sofia.shutdown_and_wait()).await??;
