...
use sofia_core::exec::process_exec_tool_call;
...
fn sofia_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestCodexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep SOFIA_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let sofia_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&sofia_home)
            .with_context(|| format!("create stable test SOFIA_HOME {}", sofia_home.display()))?;
        return Ok(TestCodexHome::Persistent(sofia_home));
    }
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("sofia-resources");
    ...
    for helper_name in ["sofia-windows-sandbox-setup", "sofia-command-runner"] {
        let helper = sofia_utils_cargo_bin::cargo_bin(helper_name)?;
...
fn stage_windows_sandbox_cli(fixture_bin: &Path) -> anyhow::Result<(PathBuf, PathBuf)> {
    ...
    let resources_dir = fixture_bin.join("sofia-resources");
    std::fs::create_dir_all(&resources_dir)?;

    let sofia_source = sofia_utils_cargo_bin::cargo_bin("sofia")?;
    let sofia = fixture_bin.join("sofia.exe");
    std::fs::copy(&sofia_source, &sofia)
        .with_context(|| format!("copy {} to {}", sofia_source.display(), sofia.display()))?;
    for helper_name in ["sofia-windows-sandbox-setup", "sofia-command-runner"] {
        let helper = sofia_utils_cargo_bin::cargo_bin(helper_name)?;
    ...
    let probe_source = sofia_utils_cargo_bin::cargo_bin("sofia-windows-managed-deny-probe")?;
...
fn windows_sandbox_cli_preserves_managed_deny_reads_across_launches() -> anyhow::Result<()> {
    let sofia_home =
        sofia_home_for_windows_sandbox_test("windows-cli-managed-deny-read-sofia-home")?;
    ...
    let (sofia, probe) = stage_windows_sandbox_cli(&bin)?;
    ...
    for launch in 1..=2 {
        let output = Command::new(&sofia)
            ...
            .env("SOFIA_WINDOWS_ALLOWED_TEXT", &allowed_text)
            .env("SOFIA_WINDOWS_DENIED_TEXT", &denied_text)
            .env("SOFIA_WINDOWS_ALLOWED_MODULE", &allowed_module)
            .env("SOFIA_WINDOWS_DENIED_MODULE", &denied_module)
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let sofia_home =
        sofia_home_for_windows_sandbox_test("windows-restricted-token-deny-read-sofia-home")?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let sofia_home =
        sofia_home_for_windows_sandbox_test("windows-elevated-missing-metadata-sofia-home")?;
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", sofia_home.path().as_os_str());
    ...
    for name in sofia_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let sofia_home = sofia_home_for_windows_sandbox_test("windows-elevated-deny-read-sofia-home")?;
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", sofia_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let bundled_skill_dir = user_profile.path().join(".sofia/plugins/cache");
    std::fs::create_dir_all(&bundled_skill_dir)?;
    let bundled_skill = bundled_skill_dir.join("SKILL.md");
    let setup_marker = sofia_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(sofia_home.path()),
...
async fn windows_elevated_unified_exec_enforces_managed_deny_reads() -> anyhow::Result<()> {
    let sofia_home =
        sofia_home_for_windows_sandbox_test("windows-elevated-tool-runtime-deny-read-sofia-home")?;
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", sofia_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;

    let configured_codex_home = dunce::canonicalize(sofia_home.path())?.abs();
    let builder = test_codex()
        .with_windows_cmd_shell()
        .with_config(move |config| {
            config.sofia_home = configured_codex_home;
