...
fn configure_priority_role(config: &mut Config) {
    ...
    let role_path = config.sofia_home.join("priority-worker.toml");
...
async fn wait_for_turn_complete(thread: &sofia_core::CodexThread) {
...
async fn evicted_role_subagent_uses_root_service_tier_after_reload() -> Result<()> {
    ...
    assert_eq!(
        ...
        test.sofia.config_snapshot().await.service_tier,
