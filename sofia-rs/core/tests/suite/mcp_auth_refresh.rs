...
use sofia_mcp::CodexAppsToolsCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let mut hosted_plugin_runtime_config = sofia_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        sofia_apps_tools_cache: CodexAppsToolsCache::default(),
        tool_catalog_cache: McpToolCatalogCache::default(),
        sofia_apps_tools_cache_key: sofia_mcp::sofia_apps_tools_cache_key(Some(&expected_auth)),
    ...
    // so this tool call only sees the new token if the Sofia Apps provider
    ...
    let tool_call_request = requests
        .iter()
        .find(|request| {
            request.url.path() == "/api/sofia/ps/mcp"
        ...
        .expect("Sofia Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")
