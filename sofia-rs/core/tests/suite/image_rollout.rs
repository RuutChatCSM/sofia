...
fn find_user_message_with_image(text: &str) -> Option<ResponseItem> {
    for line in text.lines() {
        ...
        let rollout = match sofia_rollout::parse_rollout_line(trimmed) {
            Ok(rollout) => rollout,
            Err(_) => continue,
        };
    ...
    let expected = ResponseItem::Message {
        ...
        content: vec![
            ContentItem::InputText {
                text: sofia_protocol::models::local_image_open_tag_text_with_path(
...
async fn resumed_history_only_emits_resize_notices_for_new_images() -> anyhow::Result<()> {
    ...
    let persisted_developer_metadata = fs::read_to_string(&rollout_path)?
        ...
        .filter_map(|line| {
            let RolloutItem::ResponseItem(envelope) = sofia_rollout::parse_rollout_line(line)
