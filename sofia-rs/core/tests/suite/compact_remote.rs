...
use sofia_core::compact::SUMMARY_PREFIX;
...
use sofia_login::auth::AgentIdentityAuth;
use sofia_login::auth::AgentIdentityAuthRecord;
use sofia_login::auth::BedrockApiKeyAuth;
use sofia_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
...
use sofia_protocol::account::PlanType as AccountPlanType;
use sofia_protocol::config_types::ServiceTier;
use sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use sofia_protocol::dynamic_tools::DynamicToolFunctionSpec;
use sofia_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use sofia_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
const TURN_STATE_HEADER: &str = "x-sofia-turn-state";
...
async fn start_realtime_conversation(sofia: &sofia_core::CodexThread) -> Result<()> {
    sofia
    ...
    wait_for_event_match(sofia, |msg| match msg {
...
async fn close_realtime_conversation(sofia: &sofia_core::CodexThread) -> Result<()> {
...
async fn wait_for_turn_complete(sofia: &sofia_core::CodexThread) {
...
fn annotate_retained_user_in_rollout(path: &Path, retained_text: &str) -> Result<()> {
    let mut rollout = fs::read_to_string(path)?
        ...
        .map(sofia_rollout::parse_rollout_line)
...
async fn amazon_bedrock_manual_compaction_uses_v2_responses_endpoint() -> Result<()> {
    ...
    harness.test().sofia.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().sofia).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.sofia.shutdown_and_wait().await?;
    annotate_retained_user_in_rollout(&rollout_path, retained_text)?;

    let resumed = builder()
        .resume(&server, home, rollout_path.clone())
        .await?;
    resumed.sofia.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.sofia).await;
    resumed.sofia.shutdown_and_wait().await?;
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let sofia = &harness.test().sofia;
    let rollout_path = sofia.rollout_path().context("rollout path")?;
    sofia
    ...
    harness.test().submit_turn("before compact").await?;
    sofia.submit(Op::Compact).await?;
    wait_for_turn_complete(sofia).await;
    harness.test().submit_turn("after compact").await?;
    ...
    let replacement_history = fs::read_to_string(&rollout_path)?
        .lines()
        .filter_map(|line| sofia_rollout::parse_rollout_line(line).ok())
...
async fn remote_compact_v2_records_usage_before_output_validation() -> Result<()> {
    ...
    harness.test().submit_turn("before compact").await?;
    sofia.submit(Op::Compact).await?;
    wait_for_event(sofia, |event| matches!(event, EventMsg::Error(_))).await;
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    .await?;
    let sofia = harness.test().sofia.clone();
    ...
    assert_eq!(
        compact_request.header("x-sofia-installation-id").as_deref(),
    ...
    assert_eq!(
        ...
        compact_request.header("x-sofia-window-id").as_deref()
    );
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-sofia-window-id").as_deref()
    );
    assert_ne!(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let compact_turn_id = sofia.submit(Op::Compact).await?;
    wait_for_turn_complete(&sofia).await;
    ...
    let image_inputs = (1..=8)
        .map(|number| {
            ...
            Ok(UserInput::Image {
                ...
                detail: Some(sofia_protocol::models::ImageDetail::Original),
            })
        })
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-sofia-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "sofia_app".to_string(),
        description: "Sofia app tools.".to_string(),
        tools: vec![
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "sofia_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "sofia_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "sofia_app", tool_name, "{}"),
    ...
    test.sofia
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.sofia, |event| {
    ...
    test.sofia
        ...
        .await?;
    wait_for_turn_complete(&test.sofia).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&sofia, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&sofia, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    .await?;
    let baseline_codex = baseline_harness.test().sofia.clone();
    ...
    .await?;
    let override_codex = override_harness.test().sofia.clone();
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = sofia.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    sofia.submit(Op::Shutdown).await?;
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    ...
    for line in rollout_text
        ...
        .filter(|l| !l.is_empty())
    {
        let Ok(entry) = sofia_rollout::parse_rollout_line(line) else {
            continue;
        };
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.sofia.submit(Op::Compact).await?;
    wait_for_event(&initial.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    wait_for_event(&initial.sofia, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    wait_for_event(&test.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    start_realtime_conversation(test.sofia.as_ref()).await?;
    ...
    close_realtime_conversation(test.sofia.as_ref()).await?;
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    start_realtime_conversation(initial.sofia.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.sofia.as_ref()).await?;
    initial.sofia.submit(Op::Shutdown).await?;
    wait_for_event(&initial.sofia, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    start_realtime_conversation(resumed.sofia.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.sofia.as_ref()).await?;
