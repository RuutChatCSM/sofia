...
use serde_json::json;
use sofia_config::HookStateToml;
...
use sofia_core::config::ThreadStoreConfig;
...
use sofia_plugin::PluginHookSource;
use sofia_plugin::PluginId;
use sofia_protocol::items::parse_hook_prompt_fragment;
...
fn trust_plugin_hooks(config: &mut Config, plugin_hook_sources: Vec<PluginHookSource>) {
    ...
    let listed = sofia_hooks::list_hooks(sofia_hooks::HooksConfig {
...
fn rollout_hook_prompt_texts(text: &str) -> Result<Vec<String>> {
    ...
    for line in text.lines() {
        ...
        let rollout = sofia_rollout::parse_rollout_line(trimmed).context("parse rollout line")?;
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.sofia_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.sofia_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.sofia_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
    ...
    test.sofia
        .start_or_steer_turn(
            ...
            .with_thread_settings(
                sofia_protocol::protocol::ThreadSettingsOverrides {
...
async fn async_hook_finishing_while_idle_waits_for_the_next_turn(
    ...
    assert!(
        timeout(Duration::from_millis(150), test.sofia.next_event())
            .await
            .is_err(),
    ...
    test.sofia.start_turn_if_idle(next_turn).await?;
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.sofia_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
            .filter_map(|input| input.get("source").and_then(Value::as_str))
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.sofia_home_path())?;
...
async fn blocked_queued_prompt_does_not_strand_earlier_accepted_prompt(
    ...
    let history = test.sofia.conversation_history_snapshot().await;
...
async fn permission_request_hook_allow_bypasses_strict_auto_review() -> Result<()> {
    ...
    let request = wait_for_event(&test.sofia, |event| {
...
async fn permission_request_hook_allows_network_approval_without_prompt() -> Result<()> {
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://sofia-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
...
async fn permission_request_hook_denies_network_approval_with_custom_message() -> Result<()> {
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); opener.open('http://sofia-network-test.invalid', timeout=2).read()""#;
    run_network_permission_hook_test(
        "deny",
        denial,
...
"#,
    ...
    let test = test_codex()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = sofia_config::types::ApprovalsReviewer::AutoReview;
    ...
    if expected_denial.is_none() {
        ...
        assert!(
            timeout(
                ...
                wait_for_event(&test.sofia, |event| matches!(
                    event,
    ...
    assert_single_permission_request_hook_input(
        test.sofia_home_path(),
        command,
        Some("network-access http://sofia-network-test.invalid:80"),
    )?;
    let requests = responses.requests();
    assert_eq!(
        requests
...
async fn pre_tool_use_json_deny_blocks_exec_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.sofia_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.sofia_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_input"]["command"], command);
    assert_eq!(
...
async fn local_bundled_cleanup_hook_runs_without_saved_trust(
    ...
    fs::create_dir_all(plugin_root.join(".sofia-plugin"))?;
    ...
    let hook_key = sofia_hooks::hook_key(
    ...
    if !hooks_enabled {
        let regular_hooks = sofia_hooks::list_hooks(sofia_hooks::HooksConfig {
            feature_enabled: true,
    ...
    assert_eq!(
        sofia_hooks::hook_states_from_stack(Some(&test.config.config_layer_stack)),
        expected_hook_states
    );
    wait_for_mcp_server(&test.sofia, mcp_server_name).await?;
    test.sofia
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<sofia_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.sofia_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.sofia_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.sofia_home_path())?;
