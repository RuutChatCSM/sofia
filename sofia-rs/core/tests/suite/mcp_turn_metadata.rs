...
use sofia_protocol::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.sofia_home.join("config.toml").abs();
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.sofia, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request(
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.sofia, |event| {
    ...
    if !strict_auto_review {
        let EventMsg::ElicitationRequest(request) = wait_for_event(&test.sofia, |event| {
            matches!(
                event,
                EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
            )
    ...
    let mcp_turn_metadata = apps_tool_call
        .pointer("/params/_meta/x-sofia-turn-metadata")
        .expect("MCP tools/call turn metadata");
    assert_eq!(
        mcp_turn_metadata["sofia_version"],
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-sofia-turn-metadata/user_input_requested_during_turn"),
...
"#
                ...
                config.config_layer_stack = config
                    .config_layer_stack
                    .with_user_config(&config.sofia_home.join("config.toml").abs(), user_config)
                    .expect("apps user config should be valid");
            } else {
                set_default_app_approval_mode_and_reviewer(
                    config,
    ...
    let route_event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.sofia, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
