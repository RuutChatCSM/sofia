...
-> Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        ...
        config
            ...
            .insert("sofia_security_surface".to_string(), "sdk".to_string());
    });
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-sofia-window-id"))?;
    let child_window_id = child
        .header("x-sofia-window-id")
        .ok_or_else(|| anyhow!("child request missing x-sofia-window-id"))?;
    ...
    assert_eq!(
        child.header("x-sofia-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let parent_turn_metadata: serde_json::Value = serde_json::from_str(
        &parent
            .header("x-sofia-turn-metadata")
            .ok_or_else(|| anyhow!("parent request missing x-sofia-turn-metadata"))?,
    )?;
    assert_eq!(parent_turn_metadata["sofia_security_surface"], json!("sdk"));
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-sofia-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-sofia-turn-metadata"))?,
    )?;
    ...
    assert_eq!(child_turn_metadata["sofia_security_surface"], json!("sdk"));
