...
use sofia_core::SleepFuture;
use sofia_core::TimeFuture;
...
use sofia_protocol::openai_models::AutoReviewMessages;
...
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/sofia", false, "/backend-api/sofia/responses", true; "chatgpt_uses_responses_by_default")]
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/sofia", true, "/backend-api/sofia/guardian", true; "chatgpt_uses_guardian_when_enabled")]
...
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), "Custom", "/backend-api/sofia", true, "/backend-api/sofia/responses", true; "custom_provider_uses_responses")]
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), "OpenAI", "/backend-api/sofia", true, "/backend-api/sofia/guardian", false; "server_without_response_id")]
...
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), true, "sofia-auto-review"; "chatgpt_uses_codex_auto_review")]
#[test_case(CodexAuth::create_dummy_chatgpt_auth_for_testing(), false, "sofia-auto-review"; "chatgpt_without_free_guardian")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let bundled_models = sofia_models_manager::bundled_models_response()?.models;
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            ...
            let rules_dir = config.sofia_home.join("rules");
    ...
    for handshake in server.handshakes() {
        ...
        assert_eq!(
            ...
            if uses_guardian_endpoint {
                "/backend-api/sofia/guardian"
            } else if uses_codex_backend {
                "/backend-api/sofia/responses"
            } else {
                "/v1/responses"
            }
        );
        if uses_guardian_endpoint {
            assert_eq!(handshake.header("x-sofia-routing-hint"), None);
        }
    }
...
async fn guardian_session_is_reused_for_consecutive_tool_reviews_without_prewarm() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_workspace_setup(|cwd, fs| async move {
            fs.write_file(
                &sofia_utils_path_uri::PathUri::from_abs_path(&cwd.join("guardian-secret.txt")),
...
async fn guardian_denial_rejects_tool_call_with_rationale(
    ...
    let feedback = sofia_feedback::guardian_review_failures(&[test.session_configured.thread_id])
        .attachment
...
async fn guardian_timeout_rejects_tool_call_with_acting_model_instructions(
    ...
    impl sofia_extension_api::ApprovalReviewContributor for TimedOutReviewContributor {
        fn decide<'a>(
            &'a self,
            input: &'a sofia_extension_api::ApprovalDecisionInput<'_>,
        ) -> sofia_extension_api::ExtensionFuture<'a, Option<sofia_extension_api::ApprovalDecision>>
        {
            ...
            Box::pin(async {
                Some(sofia_extension_api::ApprovalDecision::Reviewed(
...
async fn cyber_model_guardian_denial_interrupts_turn_immediately() -> Result<()> {
    ...
    let warning = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
    ...
    let aborted = wait_for_event(&test.sofia, |event| {
        matches!(event, EventMsg::TurnAborted(_))
    })
    .await;
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the Sofia agent history whose request action you are assessing."
        ),
