...
async fn executor_stop_hook_runs_after_attachment() -> Result<()> {
    ...
    let call = &calls[0];
    let turn_metadata = &call["params"]["_meta"]["x-sofia-turn-metadata"];
...
async fn executor_interrupt_hook_runs_after_attachment() -> Result<()> {
    ...
    fixture.test.sofia.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.test.sofia, |event| {
    ...
    assert_eq!(call["_meta"]["x-sofia-turn-metadata"]["turn_id"], *turn_id);
    Ok(())
}
...
async fn executor_stop_hook_rejects_mismatched_environment() -> Result<()> {
    ...
    fixture
        ...
        .await;
    wait_for_mcp_server(&fixture.test.sofia, "node_repl").await?;
    assert_eq!(
        fixture
            .test
            .sofia
...
async fn executor_browser_and_computer_use_stop_hooks_use_separate_mcp_routes(
    ...
    if hook_event == "SubagentStop" {
        let child = fixture
            ...
            .start_thread(StartThreadOptions {
                ...
                environments: Some(fixture.test.sofia.environment_selections().await),
                ..StartThreadOptions::new(fixture.test.config.clone())
            })
            .await?;
        fixture.test.sofia = child.thread;
    ...
    let turn_metadata = &browser_params["_meta"]["x-sofia-turn-metadata"];
    assert_eq!(
        node_params["_meta"]["x-sofia-turn-metadata"],
        *turn_metadata
    );
...
async fn executor_plugin_hook_fixture(
    ...
    for (plugin_id, manifest) in plugins {
        ...
        let plugin_directory = plugin_root.join(".sofia-plugin")?;
...
impl ExecutorHookFixture {
    ...
    async fn interrupt_running_command(&self) -> Result<()> {
        wait_for_event(&self.test.sofia, |event| {
            matches!(event, EventMsg::ExecCommandBegin(_))
        })
        .await;
        self.test.sofia.submit(Op::Interrupt).await?;
        wait_for_event(&self.test.sofia, |event| {
            matches!(event, EventMsg::TurnAborted(_))
        })
        .await;
