...
use sofia_core::EnvironmentNetworkPolicy;
use sofia_core::StartThreadOptions;
use sofia_core::TurnInputRequest;
use sofia_core::WaitForEnvironmentToolConfig;
use sofia_core::compact::SUMMARIZATION_PROMPT;
use sofia_core::config::Config;
use sofia_core::config::Constrained;
use sofia_core::windows_sandbox::WindowsSandboxLevelExt;
use sofia_exec_server::CopyOptions;
use sofia_exec_server::CreateDirectoryOptions;
use sofia_exec_server::EnvironmentReadyInfo;
use sofia_exec_server::ExecServerError;
use sofia_exec_server::ExecServerRuntimePaths;
use sofia_exec_server::FileSystemSandboxContext;
use sofia_exec_server::LOCAL_ENVIRONMENT_ID;
use sofia_exec_server::NoiseChannelPublicKey;
use sofia_exec_server::NoiseRendezvousConnectBundle;
use sofia_exec_server::NoiseRendezvousConnectProvider;
use sofia_exec_server::REMOTE_ENVIRONMENT_ID;
use sofia_exec_server::RemoteEnvironmentConfig;
...
use sofia_extension_api::RenderedWorldStateFragment;
use sofia_extension_api::ThreadLifecycleContributor;
use sofia_extension_api::ThreadStartInput;
use sofia_extension_api::WorldStateContributionInput;
use sofia_extension_api::WorldStateSectionContribution;
...
use sofia_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = sofia_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
...
async fn approved_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/sofia-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\sofia-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn environment_permissions_follow_configuration_ownership() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    let persisted_settings = wait_for_event_match(&test.sofia, |event| match event {
        EventMsg::ThreadSettingsApplied(event) => Some(event.thread_settings.clone()),
        _ => None,
    })
    .await;
    let snapshot = test.sofia.config_snapshot().await;
    ...
    assert_eq!(
        persisted_settings,
        test.sofia.thread_settings_snapshot().await
    );
    assert_ne!(
    ...
    test.sofia
        .restore_thread_settings(test.sofia.restorable_thread_settings().await)
        .await?;
    let (mcp_config, _) = test.sofia.current_mcp_config_and_runtime_context().await;
    assert_eq!(
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let initial_environments = test.sofia.environment_selections().await;
    ...
    test.sofia
        ...
        .await?;
    let request = wait_for_event_match(&test.sofia, |event| match event {
    ...
    assert_eq!(
        test.sofia.environment_selections().await,
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    let mcp_url = format!("{}/api/sofia/ps/mcp", apps.chatgpt_base_url);
    let response_mock = mount_sse_sequence(
        &server,
        vec![
            sse(vec![
    ...
    core_test_support::wait_for_mcp_server(&test.sofia, "deferred").await?;
    test.sofia
        .submit(Op::UserInputAnswer {
            id: request.turn_id,
            response: RequestUserInputResponse {
    ...
    test.sofia.flush_rollout().await?;
    let rollout = fs::read_to_string(test.sofia.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    assert_eq!(
        test.sofia.inspect_selected_capability_roots().ready_roots,
        vec![root("first-root")]
    );
    assert_eq!(
        second
...
async fn owner_network_policy_rejects_unsupported_environment_authority() -> Result<()> {
    ...
    let selections = test.sofia.environment_selections().await;
    ...
    assert_eq!(test.sofia.environment_selections().await, selections);
    Ok(())
}
...
async fn ready_before_selection_resolves_resumed_thread_capability_root_after_wait(
    ...
    let remote_environment = tokio::spawn(sofia_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    test.sofia.submit(Op::Interrupt).await?;
    wait_for_event(&test.sofia, |event| {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    assert_eq!(test.sofia.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.sofia.ensure_rollout_materialized().await;
    test.sofia.flush_rollout().await?;
    let rollout_path = test.sofia.rollout_path().context("rollout path")?;
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sofia-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sofia-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sofia-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/sofia-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/sofia-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/sofia-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/sofia-remote-dotdot-{}", std::process::id()));
