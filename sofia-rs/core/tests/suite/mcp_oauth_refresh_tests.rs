...
async fn http_auth_challenge_reaches_agent_tool_call_events_without_replay() -> anyhow::Result<()> {
    ...
    let server_config: McpServerConfig = serde_json::from_value(json!({
        "url": format!("{}/api/sofia/ps/mcp", http_server.chatgpt_base_url),
    ...
    let fixture = test_codex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.sofia, "reauth").await?;
    fixture
        .sofia
...
async fn oauth_mode_refresh_replaces_the_live_connection(
    ...
    Mock::given(method("GET"))
        .and(path(
            "/.well-known/oauth-authorization-server/api/sofia/ps/mcp",
        ))
    ...
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", home.path().as_os_str());
    ...
    for (enabled, expected_initializations) in [(true, 2), (true, 2), (false, 3)] {
        ...
        match refresh_path {
            ...
            ConfigRefreshPath::Mcp => {
                fixture.sofia.refresh_mcp_config(refreshed_config).await;
            }
        }
