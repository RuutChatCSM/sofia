...
async fn root_reconciliation_reuses_pending_apps_startup() -> Result<()> {
    ...
    extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
        ...
        url: format!("{}/api/sofia/ps/mcp", gated_apps_server.chatgpt_base_url),
...
async fn timeout_refresh_replaces_pending_startup_and_reuses_ready_connection() -> Result<()> {
                        ...
                        .map(|(name, server)| {
                            (
                                ...
                                serde_json::from_value(json!({
                                    "url": format!("{}/api/sofia/ps/mcp", server.chatgpt_base_url),
    ...
    test.sofia.refresh_mcp_config(refresh_config).await;
...
async fn out_of_band_resource_read_reconciles_the_published_mcp_runtime() -> Result<()> {
    ...
    let mut refresh_config = test.config.clone();
    let user_config_path = refresh_config.sofia_home.join("config.toml");
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, sofia_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.sofia.refresh_runtime_config(refresh_config).await;
    test.sofia.submit(Op::RefreshMcpServers).await?;
...
async fn elevated_apps_catalog_limit_requires_host_owned_registration() -> Result<()> {
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        if let Some(id) = extension_id {
            ...
            extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
                id,
                url: format!("{}/api/sofia/ps/mcp", apps_server.chatgpt_base_url),
                root_resolved: None,
            }));
            builder = builder.with_extensions(Arc::new(extensions.build()));
        }
        let test = builder.build_with_auto_env(&server).await?;
        let startup = wait_for_mcp_server(&test.sofia, SOFIA_APPS_MCP_SERVER_NAME).await;
        ...
        if extension_id == Some("test-extension") {
            ...
            assert!(
                ...
                "an extension named sofia_apps must not inherit the trusted Apps limit: {error}"
            );
            continue;
        }
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__sofia_apps__calendar_create_event(args:")
        }),
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.sofia, SOFIA_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.sofia.ensure_rollout_materialized().await;
    initial.sofia.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__sofia_apps__calendar": "Plan events and manage your calendar."
        })
    );
    ...
    drop(initial);
    wait_for_mcp_server(&resumed.sofia, SOFIA_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.sofia, |event| {
    ...
    wait_for_event(&test.sofia, |event| {
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == SOFIA_APPS_MCP_SERVER_NAME
                    && matches!(update.status, sofia_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.sofia_home_path().join("cache/sofia_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.sofia.submit(Op::RefreshMcpServers).await?;
    test.sofia
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
