...
use sofia_core::config::RolloutBudgetConfig;
                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "sofia_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["sofia_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
    ...
    assert_eq!(
        error.message,
        "Fatal error: response.completed usage.sofia_rollout_budget_units must be finite and non-negative"
    );
    assert_eq!(error.sofia_error_info, Some(CodexErrorInfo::Other));
        ...
        wait_for_event(&test.sofia, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.sofia_error_info == Some(CodexErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.sofia, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["sofia_rollout_budget_units"] = json!(10.0);
    }
