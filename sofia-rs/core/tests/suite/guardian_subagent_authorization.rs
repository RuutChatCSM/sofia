...
use serde_json::json;
use sofia_core::GuardianRootMessage;
...
use sofia_prompts::render_review_exit_success;
...
fn is_worker_request(request: &wiremock::Request, root_thread_id: ThreadId) -> bool {
    request_body(request).is_some_and(|body| {
        body["client_metadata"]["x-sofia-parent-thread-id"] == json!(root_thread_id)
...
async fn guardian_subagent_review_preserves_late_root_user_authorization(
        ...
        root_history_items.extend(
            [
                format!(
                    ...
                    sofia_core::review_prompts::SUMMARY_PREFIX
                ),
    ...
    test.sofia.inject_response_items(root_history_items).await?;
    ...
    test.sofia
        ...
        .await?;
    let question = wait_for_event_match(&test.sofia, |event| match event {
    ...
    let legacy_answer = sofia_guardian_context::truncate_text(
        &format!(
            "{}{}",
    ...
    let expected_messages = match root_context {
        ...
        RootContext::RetainedAtMessageLimit => {
            ...
            messages.push(GuardianRootMessage::User(
                sofia_guardian_context::truncate_text(
    ...
    let feedback_thread_ids = test
        ...
        .await?;
    let failures = sofia_feedback::guardian_review_failures(&feedback_thread_ids);
