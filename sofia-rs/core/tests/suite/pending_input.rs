...
use sofia_core::StartIfIdleSubmission;
use sofia_core::TurnInput;
use sofia_core::TurnInputRequest;
use sofia_core::TurnInputSubmission;
use sofia_core::TurnStartOptions;
use sofia_core::config::CurrentTimeReminderConfig;
use sofia_extension_items::ExtensionItem;
use sofia_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    submit_queue_only_agent_mail(test.sofia.as_ref(), "pending mailbox input").await;
    let submission = test
        .sofia
    ...
    wait_for_turn_complete(test.sofia.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.sofia.as_ref(), |event| {
...
async fn build_codex(server: &StreamingSseServer) -> Arc<CodexThread> {
    test_codex()
        ...
        .await
        .expect("build streaming Sofia test session")
        .sofia
}

async fn submit_user_input(sofia: &CodexThread, text: &str) {
    sofia
...
async fn steer_user_input(sofia: &CodexThread, text: &str) {
...
async fn enqueue_queue_only_agent_mail(sofia: &CodexThread, text: &str) {
    sofia
...
async fn submit_queue_only_agent_mail(sofia: &CodexThread, text: &str) {
    enqueue_queue_only_agent_mail(sofia, text).await;
    sofia
        .submit(Op::RealtimeConversationListVoices)
        .await
        .expect("submit list-voices barrier");
    wait_for_event(sofia, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(sofia: &CodexThread) {
    wait_for_event(sofia, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(sofia: &CodexThread, text: &str) {
...
async fn wait_for_turn_complete(sofia: &CodexThread) {
    wait_for_event(sofia, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(sofia: &CodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(sofia, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(sofia: &CodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(sofia, |event| {
        matches!(
            event,
...
impl sofia_extension_api::ThreadLifecycleContributor<sofia_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: sofia_extension_api::ThreadStartInput<'a, sofia_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_with_previous_turn_context() {
    ...
    .await;
    let mut extensions =
        sofia_extension_api::ExtensionRegistryBuilder::<sofia_core::config::Config>::new();
    ...
    let sofia = test_codex()
        ...
        .await
        .expect("build Sofia test session")
        .sofia;
    ...
    enqueue_queue_only_agent_mail(&sofia, CHILD_MESSAGE).await;
    wait_for_turn_complete(&sofia).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    sofia_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            sofia_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&sofia, INITIAL_PROMPT).await;
    wait_for_event(&sofia, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&sofia, STEER_PROMPT).await;
    wait_for_turn_complete(&sofia).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&sofia, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&sofia, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&sofia, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&sofia, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&sofia, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&sofia, "new mailbox input").await;
    wait_for_sleep_item_completed(&sofia, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&sofia).await;
    ...
    sofia.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&sofia, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = sofia.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let sofia = build_codex(&server).await;

    submit_user_input(&sofia, "first prompt").await;

    wait_for_reasoning_item_started(&sofia).await;

    submit_queue_only_agent_mail(&sofia, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&sofia, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&sofia, "second prompt").await;
    wait_for_turn_complete(&sofia).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&sofia, "second prompt").await;
...
async fn terminal_compaction_error_does_not_retry_pending_input(
    ...
    let test = test_codex()
        ...
        .await?;
    let sofia = &test.sofia;

    if failure_point == CompactionFailurePoint::PreTurn {
        submit_user_input(sofia, "initial prompt").await;
        wait_for_turn_complete(sofia).await;
    }
    if pending_input == PendingInputAfterFailure::QueuedMail {
        submit_queue_only_agent_mail(sofia, PENDING_MESSAGE).await;
    }
    submit_user_input(sofia, "prompt that needs compaction").await;
    tokio::time::timeout(
    ...
    .await?;
    match pending_input {
        PendingInputAfterFailure::Steer => steer_user_input(sofia, PENDING_MESSAGE).await,
        ...
        PendingInputAfterFailure::TriggeringMail => {
            sofia
                ...
                .await?;
            sofia.submit(Op::RealtimeConversationListVoices).await?;
            wait_for_event(sofia, |event| {
                matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
            })
            .await;
    ...
    sofia.flush_rollout().await?;
    let history = sofia.load_history(/*include_archived*/ false).await?;
    ...
    submit_user_input(sofia, "retry after quota resets").await;
    wait_for_agent_message(sofia, "recovered answer").await;
    let completed = wait_for_event(sofia, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&sofia, "resumed old task").await;
    wait_for_turn_complete(&sofia).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&sofia, "processed steered prompt").await;
    wait_for_turn_complete(&sofia).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&sofia, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&sofia, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
