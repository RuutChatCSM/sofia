...
use serde_json::json;
use sofia_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use sofia_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use sofia_core::CodexResponsesMetadata;
use sofia_core::ModelClient;
use sofia_core::ModelClientSession;
...
use sofia_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use sofia_core::test_support::with_parent_turn;
...
use sofia_rollout_trace::ConversationPart;
...
async fn responses_websocket_preserves_credit_usage_metadata() {
    ...
    assert_eq!(
        usage_metadata,
        Some(sofia_protocol::ResponseUsageMetadata {
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(sofia_login::default_client::get_codex_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-sofia-installation-id"].as_str(),
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-sofia-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-sofia-turn-metadata"),
        None
    );
    assert_eq!(
        server
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-sofia-primary-used-percent": "100.0",
            "x-sofia-secondary-used-percent": "87.5",
            "x-sofia-primary-over-secondary-limit-percent": "95.0",
            "x-sofia-primary-window-minutes": "15",
            "x-sofia-secondary-window-minutes": "60",
            "x-sofia-credits-has-credits": "true",
            "x-sofia-credits-unlimited": "false",
            "x-sofia-credits-balance": "",
            "x-sofia-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.sofia, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.sofia, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = sofia_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(sofia_core::test_support::auth_manager_from_auth);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "sofia-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
