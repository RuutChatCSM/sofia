...
use sofia_models_manager::model_info::BASE_INSTRUCTIONS;
...
const REMOTE_MODEL_SLUG: &str = "sofia-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Sofia".to_string(),
    ...
    let manager = sofia_core::test_support::models_manager_with_provider(
        sofia_home.path().to_path_buf(),
        sofia_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-sofia-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-sofia-test");
    assert_eq!(
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&sofia, |event| {
        matches!(
            event,
...
async fn remote_models_long_model_slug_is_sent_with_supported_reasoning(
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-sofia-test";
    let prefix_model = "gpt-5.3-sofia";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.2-sofia";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&sofia, |_| true).await;
        match event {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "sofia-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "sofia-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "sofia-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    sofia_core::test_support::all_model_presets()
        .iter()
