...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.sofia);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.sofia, "after resume").await?;
    shutdown_thread(&resumed.sofia).await?;
...
async fn submit_user_turn(sofia: &Arc<CodexThread>, text: &str) -> Result<()> {
    sofia
...
async fn submit_compact_turn(sofia: &Arc<CodexThread>) -> Result<()> {
    sofia.submit(Op::Compact).await?;
    let warning_event = wait_for_event(sofia, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(sofia: &Arc<CodexThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-sofia-window-id header");
