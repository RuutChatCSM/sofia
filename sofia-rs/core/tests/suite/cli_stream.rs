...
fn repo_root() -> std::path::PathBuf {
    sofia_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = sofia_utils_cargo_bin::cargo_bin("sofia").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("openai_base_url=\"{}/api/sofia\"", server.uri()))
        .arg("-c")
        .arg(format!("chatgpt_base_url=\"{}/backend-api\"", server.uri()))
        .arg("-C")
        .arg(repo_root())
        .arg("hello?");
    cmd.env("SOFIA_HOME", home.path())
        .env(SOFIA_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("SOFIA_AUTHAPI_BASE_URL", server.uri())
        .env_remove(SOFIA_API_KEY_ENV_VAR)
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = sofia_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `sofia exec` subprocess tests in this file. These commands
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Sofia to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `sofia exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the sofia CLI and invoke `exec`, which is what records a session.
    let bin = sofia_utils_cargo_bin::cargo_bin("sofia").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        ...
        .arg(&prompt);
    cmd.env("SOFIA_HOME", home.path())
        .env(SOFIA_API_KEY_ENV_VAR, "dummy");
    ...
    let bin2 = sofia_utils_cargo_bin::cargo_bin("sofia").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("SOFIA_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume sofia-cli run failed");
