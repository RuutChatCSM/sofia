...
async fn guardian_revalidates_owning_session_before_allow(
    ...
    let test = test_codex()
        ...
        .with_code_mode_host_program(sofia_utils_cargo_bin::cargo_bin("sofia-code-mode-host")?)
    ...
    let before = test.sofia.guardian_authorization_version().await;
    ...
    assert_ne!(before, test.sofia.guardian_authorization_version().await);
    review_completion_tx
        .send(())
        .expect("release Guardian response");
    let status = wait_for_event_match(&test.sofia, |event| match event {
...
-> Result<()> {
    ...
    let mut expected = test.sofia.guardian_authorization_version().await;
    ...
    test.sofia
        ...
        .await?;
    assert_eq!(test.sofia.guardian_authorization_version().await, expected);
    ...
    .await;
    assert_ne!(test.sofia.guardian_authorization_version().await, expected);
    Ok(())
}
