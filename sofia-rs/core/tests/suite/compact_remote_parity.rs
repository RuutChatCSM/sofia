...
const FIXED_CWD: &str = "/tmp/sofia_remote_compaction_parity_workspace";
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&sofia, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().sofia_home_path();
...
async fn build_harness_inner(
    ...
    TestCodexHarness::with_builder(builder.with_config(move |config| {
        config.cwd = sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
async fn submit_user_input(sofia: &sofia_core::CodexThread, items: Vec<UserInput>) -> Result<()> {
    sofia
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<SOFIA_HOME>");
            search_start = start_index + "<SOFIA_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <SOFIA_HOME>/skills/.system/imagegen/SKILL.md and \
         <SOFIA_HOME>/skills/custom/SKILL.md"
    );
}
