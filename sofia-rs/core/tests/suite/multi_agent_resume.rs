...
fn configure_multi_agent_v2_with_role(
    config: &mut sofia_core::config::Config,
    ...
    let role_path = config.sofia_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let worker_thread_id = loop {
        if let Some(thread_id) = initial_child_request
            ...
            .find_map(|request| {
                ...
                if body["client_metadata"]["x-sofia-parent-thread-id"] != json!(root_thread_id) {
                    return None;
                }
                body["client_metadata"]["thread_id"]
                    .as_str()
                    .and_then(|thread_id| sofia_protocol::ThreadId::from_string(thread_id).ok())
            })
        {
            break thread_id;
        }
    ...
    assert_eq!(
        ...
        initial.sofia.config().await.model_provider,
    ...
    std::fs::write(
        resumed.config.sofia_home.join("durable-worker-role.toml"),
        format!(
            r#"model = "{ROLE_MODEL}"
model_reasoning_effort = "high"
developer_instructions = "{ROLE_DEVELOPER_INSTRUCTIONS}"
...
"#
    ...
    assert_eq!(
        ...
        resumed.sofia.config().await.model_provider,
    ...
    let body_for = |text: &str, thread: sofia_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {
    ...
    for (body, parent_thread, parent_turn) in [
        ...
        if let Some(parent_thread) = parent_thread {
            assert_eq!(
                body["client_metadata"]["x-sofia-parent-thread-id"],
                json!(parent_thread)
            );
        }
