...
-> anyhow::Result<()> {
    ...
    mount_response_once(
        ...
        .insert_header("x-sofia-safety-buffering-enabled", "true")
        .insert_header("x-sofia-safety-buffering-faster-model", FASTER_MODEL),
    )
    .await;
    ...
    let event = wait_for_event_match(&test.sofia, |event| match event {
