...
use sofia_core_plugins::store::PluginStore;
use sofia_extension_api::ExtensionRegistry;
...
use sofia_skills_extension::HostSkillsLoadInput;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__sofia_apps__google_calendar";
...
fn write_sample_plugin_manifest_and_config_at_root(
    ...
    std::fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context(
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local Sofia home cache"
    );

    let server = start_mock_server().await;
    let sofia_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(sofia_home.as_ref());
    std::fs::write(
        &script_path,
        r#"printf '%s' '{"version":1,"measurements":[{"name":"files_scanned","value":7}]}' > "$SOFIA_PLUGIN_METRICS_OUTPUT"
"#,
    )?;
    ...
    let sofia = Arc::clone(&test_codex.sofia);
    ...
    sofia
        .start_or_steer_turn(
            TurnInputRequest::user_input(vec![sofia_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&sofia, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&sofia, |event| match event {
    ...
    let measurement = wait_for_analytics_event(&server, "sofia_plugin_measurement_event").await;
    assert_eq!(
        serde_json::json!({
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_agent_plugin_skill_plugin(sofia_home.as_ref()))?;
    let mut builder = test_codex()
        .with_home(Arc::clone(&sofia_home))
    ...
    test_codex
        ...
        .await?;
    let warning = wait_for_event(&test_codex.sofia, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_codex.sofia, |ev| {
...
#[test_case("SOFIA", true, Some("native review skill"); "native skill wins over migrated command")]
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    let plugin_root = write_sample_plugin_manifest_and_config(sofia_home.as_ref());
    ...
    if include_migrated_command {
        let migrated_skill_dir =
            plugin_root.join(".sofia-plugin/migrated-command-skills/source-command-review");
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(sofia_home.as_ref());
...
"#,
    ...
    for (marketplace, skill) in [
        ...
        std::fs::create_dir_all(root.join(".sofia-plugin"))?;
        std::fs::write(
            root.join(".sofia-plugin/plugin.json"),
            r#"{"name":"sites"}"#,
        )?;
...
-> Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(sofia_home.as_ref());
    ...
    let mut builder = test_codex().with_home(Arc::clone(&sofia_home));
    let test_codex = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_codex.sofia, "agent").await?;
    let data_root = dunce::canonicalize(
        std::fs::read_dir(sofia_home.path().join("plugins/data/agent-plugins"))?
            .next()
    ...
    test_codex
        ...
        .await?;
    let end = wait_for_event(&test_codex.sofia, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    let overlay_end = wait_for_event(&test_codex.sofia, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_codex.sofia, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
"#,
        ...
        let expected_auth_mode = match fixture.target_auth {
            ...
            TargetAuth::ApiKey => {
                sofia_login::login_with_api_key(
                    sofia_home.path(),
                    "test-api-key",
                    sofia_login::AuthCredentialsStoreMode::File,
                    sofia_login::AuthKeyringBackendKind::default(),
                )?;
                test_codex.thread_manager.auth_manager().reload().await;
                Some(AuthMode::ApiKey)
            }
            TargetAuth::BedrockApiKey => {
                sofia_login::login_with_bedrock_api_key(
                    sofia_home.path(),
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    write_plugin_skill_plugin(sofia_home.as_ref());
    write_plugin_mcp_plugin(sofia_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(sofia_home.as_ref());
    ...
    let test_codex =
        build_apps_enabled_plugin_test_codex(&server, sofia_home, apps_server.chatgpt_base_url)
            .await?;
    let sofia = Arc::clone(&test_codex.sofia);
    wait_for_mcp_server(&sofia, SOFIA_APPS_MCP_SERVER_NAME).await?;

    sofia
        .start_or_steer_turn(TurnInputRequest::user_input(vec![
            sofia_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(sofia_home.as_ref());
    write_plugin_mcp_plugin(sofia_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(sofia_home.as_ref(), "sample_app");

    let test_codex =
        build_apps_enabled_plugin_test_codex(&server, sofia_home, apps_server.chatgpt_base_url)
            .await?;
    let sofia = Arc::clone(&test_codex.sofia);
    wait_for_mcp_server(&sofia, "sample").await?;
...
async fn system_marketplace_plugin_honors_layered_activation_and_mcp_policy(
    ...
    write_plugin_mcp_plugin(sofia_home.as_ref(), &stdio_server_bin()?);
    let user_config_path = sofia_home.path().join("config.toml");
    ...
    let system_config_path = sofia_home.path().join("system.toml");
    ...
    let requirements_path = sofia_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        format!(
    ...
    std::fs::create_dir_all(project.path().join(".sofia"))?;
    std::fs::write(
        project.path().join(".sofia/config.toml"),
        format!(
            "[plugins.\"{SAMPLE_PLUGIN_CONFIG_NAME}\"]\nenabled = {plugin_enabled}\n[plugins.\"{SAMPLE_PLUGIN_CONFIG_NAME}\".mcp_servers.sample]\nenabled = {project_enabled}\ndisabled_tools = [\"echo-tool\"]\n"
        ),
    )?;
    set_project_trust_level(sofia_home.path(), project.path(), trust_level)?;
    ...
    let startup = wait_for_event_match(&test.sofia, |event| match event {
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(write_plugin_skill_plugin(sofia_home.as_ref()))?;
    write_plugin_mcp_plugin(sofia_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(sofia_home.as_ref());
    let initialize_barrier = block_plugin_mcp_startup(sofia_home.as_ref(), &rmcp_test_server_bin);
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let sofia_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(sofia_home.as_ref());
    let test_codex = build_analytics_plugin_test_codex(&server, sofia_home).await?;
    let sofia = Arc::clone(&test_codex.sofia);
    ...
    let event = wait_for_analytics_event(&server, "sofia_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(sofia_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_remote_plugin_skill_plugin(sofia_home.as_ref()))?;
    persist_sample_remote_plugin_id(sofia_home.as_ref());
    let test_codex = build_analytics_plugin_test_codex(&server, sofia_home).await?;
    let sofia = Arc::clone(&test_codex.sofia);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id(
    ...
    let server = start_mock_server().await;
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(sofia_home.as_ref());
    persist_sample_remote_plugin_id(sofia_home.as_ref());
