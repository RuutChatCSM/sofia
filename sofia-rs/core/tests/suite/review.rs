...
use sofia_protocol::openai_models::ModelTokenBudgetConfig;
...
use sofia_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use sofia_protocol::protocol::EventMsg;
use sofia_protocol::protocol::ExitedReviewModeEvent;
use sofia_protocol::protocol::Op;
use sofia_protocol::protocol::ReviewCodeLocation;
use sofia_protocol::protocol::ReviewFinding;
use sofia_protocol::protocol::ReviewLineRange;
use sofia_protocol::protocol::ReviewOutputEvent;
...
use sofia_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Sofia sandbox network restrictions.
    skip_if_no_network!();
    ...
    let sofia = new_conversation_for_server(&server, sofia_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&sofia, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&sofia, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&sofia, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&sofia, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = sofia.rollout_path().expect("rollout path");
    let text = std::fs::read_to_string(&path).expect("read rollout file");
    let parent_thread_id = text
        .lines()
        .filter(|line| !line.trim().is_empty())
        .find_map(|line| {
            let rollout_line = sofia_rollout::parse_rollout_line(line).expect("rollout line");
    ...
    for line in text.lines() {
        ...
        let rl = sofia_rollout::decode_rollout_line(v).expect("rollout line");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match sofia.next_event().await.expect("review event").msg {
    ...
    sofia.submit(Op::Interrupt).await.unwrap();
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    let review = match closed {
...
async fn review_uses_updated_turn_permissions_and_approval_policy() {
    ...
    let sofia = Arc::clone(&test.sofia);
    ...
    let stored_settings = sofia.thread_settings_snapshot().await;
    sofia
        .submit(Op::Review {
            review_request: ReviewRequest {
                target: ReviewTarget::Custom {
    ...
    assert_eq!(sofia.thread_settings_snapshot().await, stored_settings);
    let request = request_log.single_request();
    ...
    let review_session_cwd = review_rollout
        .lines()
        .find_map(|line| {
            let rollout_line = sofia_rollout::parse_rollout_line(line)
...
async fn review_resolves_inherited_summary_preferences() -> anyhow::Result<()> {
    ...
    for summary in [None, Some(ReasoningSummary::Concise)] {
        ...
        let stored_settings = test.sofia.thread_settings_snapshot().await;
        test.sofia
            .submit(Op::Review {
                review_request: ReviewRequest {
                    target: ReviewTarget::Custom {
        ...
        .await;
        assert_eq!(test.sofia.thread_settings_snapshot().await, stored_settings);
    }
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let sofia = Arc::clone(&test.sofia);
    std::fs::remove_file(sofia_home.path().join("models_cache.json"))
    ...
    // Wait for completion
    let _entered = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&sofia, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = sofia_home.path().join("resume.jsonl");
    {
        ...
        let user = sofia_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = sofia_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let sofia =
        resume_conversation_for_server(&server, sofia_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let sofia = new_conversation_for_server(&server, sofia_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, sofia_home, resume_path)
        .await
