...
use sofia_core::context::NodeReplReviewEvidence;
use sofia_extension_api::ExtensionData;
use sofia_extension_api::ExtensionRegistryBuilder;
use sofia_extension_api::ToolCallOutcome;
use sofia_extension_api::ToolContributor;
use sofia_extension_api::ToolFinishInput;
use sofia_extension_api::ToolLifecycleContributor;
use sofia_extension_api::ToolLifecycleFuture;
use sofia_extension_api::ToolStartInput;
...
use sofia_tools::FreeformTool;
use sofia_tools::FreeformToolFormat;
use sofia_tools::FunctionCallError;
use sofia_tools::JsonToolOutput;
use sofia_tools::ResponsesApiNamespace;
use sofia_tools::ResponsesApiNamespaceTool;
use sofia_tools::ToolCall;
use sofia_tools::ToolExecutor;
use sofia_tools::ToolExecutorFuture;
use sofia_tools::ToolName;
use sofia_tools::ToolOutput;
use sofia_tools::ToolPayload;
use sofia_tools::ToolSpec;
use sofia_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "test-gpt-5.1-sofia", configure)
        .await
}
...
async fn run_unavailable_code_mode_turn(
    ...
    loop {
        match wait_for_event(&test.sofia, |_| true).await {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_model("test-gpt-5.1-sofia")
        .with_code_mode_host_program("sofia-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("sofia-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("sofia-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth_manager = sofia_core::test_support::auth_manager_from_auth(auth.clone());
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-sofia")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-sofia").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.sofia, "rmcp").await?;
...
async fn code_mode_exec_holds_captured_result_during_elicitation() -> Result<()> {
    ...
    assert_eq!(
        test.sofia.increment_out_of_band_elicitation_count().await?,
        1
    );
    assert_eq!(
        test.sofia.increment_out_of_band_elicitation_count().await?,
    ...
    let release_elicitation = async {
        ...
        assert_eq!(
            test.sofia.decrement_out_of_band_elicitation_count().await?,
            1
        );
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__sofia_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__sofia_apps__calendar_timezone_option_99",
...
async fn code_mode_wait_timeout_reconnects_on_next_exec() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    wait_for_event_match(&test.sofia, |event| match event {
...
struct ResponseIdObserver {
    ...
    originating_items: Mutex<Vec<(String, Option<sofia_protocol::ResponseItemId>)>>,
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let sofia_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    sofia_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
image({{
  ...
  data,
  mimeType: "image/png",
  _meta: {{ "sofia/imageDetail": "original" }}
}});
"#,
    ...
    assert!(!exec_description.contains("sofia/imageDetail"));
...
async fn code_mode_node_repl_screenshots_can_be_captured_without_guardian_transcript_flags(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    core_test_support::wait_for_mcp_server(&test.sofia, repl_server).await?;
    let evidence = test
        .sofia
...
async fn code_mode_node_repl_image_flag_without_enhanced_stays_disabled(
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.sofia, repl_server).await?;
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if reviewer_token_budget {
                    ...
                    reviewer.input_modalities =
                        vec![sofia_protocol::openai_models::InputModality::Text];
                } else {
...
"#,
        ...
        test.sofia
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(sofia_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        let turn_id = wait_for_event_match(&test.sofia, |event| match event {
        ...
        wait_for_event(&test.sofia, |event| match event {
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-sofia-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "sofia_app__hidden_dynamic_tool");
const out = await tools.sofia_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("sofia_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("sofia_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Sofia app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("sofia_app__hidden_dynamic_tool(args:")
            })
    );
