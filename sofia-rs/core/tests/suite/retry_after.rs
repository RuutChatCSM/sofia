...
use serde_json::json;
use sofia_client::RetryOn;
use sofia_client::RetryPolicy;
use sofia_client::run_with_retry;
use sofia_http_client::Request;
use sofia_http_client::TransportError;
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "sofia_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "sofia_otel.trace_safe" {
            return;
        }

        let mut visitor = RetryTelemetryVisitor::default();
        event.record(&mut visitor);
        if visitor.name.as_deref() != Some("sofia.retry") {
            return;
        }
...
async fn wait_for_turn_completion(test: &TestCodex) {
    let EventMsg::TurnComplete(completed) = wait_for_event(&test.sofia, |event| {
        ...
        loop {
            match wait_for_event(&test.sofia, |_| true).await {
                ...
                EventMsg::TurnComplete(event) => {
                    assert_eq!(
                        event.error.and_then(|error| error.sofia_error_info),
                        Some(CodexErrorInfo::ServerOverloaded)
                    );
                    break;
                }
