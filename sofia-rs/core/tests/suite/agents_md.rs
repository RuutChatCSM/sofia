...
async fn submit_thread_turn(thread: &Arc<sofia_core::CodexThread>, prompt: &str) -> Result<()> {
    thread
...
async fn runtime_trust_reload_refreshes_project_instructions() -> Result<()> {
    ...
    let mut untrusted_config = (*test.sofia.config().await).clone();
    untrusted_config.active_project.trust_level = Some(TrustLevel::Untrusted);
    test.sofia.refresh_runtime_config(untrusted_config).await;
    ...
    let mut trusted_config = (*test.sofia.config().await).clone();
    trusted_config.active_project.trust_level = Some(TrustLevel::Trusted);
    test.sofia.refresh_runtime_config(trusted_config).await;
...
-> Result<()> {
    ...
    let error = error
        .replace(&canonical_home_path, "$SOFIA_HOME")
        .replace(&home_path, "$SOFIA_HOME");
    insta::assert_snapshot!(error, @"
    Fatal error: Failed to initialize session: failed to load AGENTS.md instructions for environment `local`: failed to prepare fs sandbox: failed to prepare Seatbelt sandbox: writable root $SOFIA_HOME/visualizations contains symlink component $SOFIA_HOME/visualizations; symlinked writable roots are not supported.
    If this writable root is at or beneath SOFIA_HOME and you trust its symlink targets, set `allow_symlinked_codex_home = true` at the top level of `$SOFIA_HOME/config.toml` (normally `~/.sofia/config.toml`) on the execution host, then restart Sofia or its executor. This opt-out trusts targets outside SOFIA_HOME and targets changed between commands. It does not apply to other writable roots.
    ");
...
-> Result<()> {
    ...
    let EventMsg::Error(error) =
        wait_for_event(&test.sofia, |event| matches!(event, EventMsg::Error(_))).await
    else {
        unreachable!();
    };
    assert!(
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.sofia.instruction_sources().await, creation_sources);
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    assert_eq!(
        initial.sofia.instruction_sources().await,
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    assert_eq!(
        parent.sofia.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.sofia.ensure_rollout_materialized().await;
    parent.sofia.flush_rollout().await?;
    let rollout_path = parent.sofia.rollout_path().expect("rollout path");
    ...
    fork_config.sofia_self_exe = parent.config.sofia_self_exe.clone();
    fork_config
        .features
