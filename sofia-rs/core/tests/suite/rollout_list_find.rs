...
use sofia_core::RolloutRecorder;
use sofia_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(sofia_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = sofia_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(sofia_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(sofia_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = sofia_home.to_path_buf();
...
async fn find_handles_gitignore_covering_codex_home_directory() {
    ...
    let sofia_home = repo.path().join(".sofia");
    std::fs::create_dir_all(&sofia_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".sofia/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&sofia_home, id);

    let found =
        find_thread_path_by_id_str(&sofia_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
