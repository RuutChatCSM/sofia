...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-sofia-responses-lite";
...
fn configure_image_capable_model(model_info: &mut sofia_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_uses_input_items_for_instructions_and_tools() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        client_metadata["x-sofia-turn-metadata"]
            .as_str()
...
async fn responses_lite_includes_tool_namespaces_info_when_enabled() -> Result<()> {
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "direct": true,
            "deferred": false,
            "code_mode_name": "mcp__sofia_apps__calendar_create_event",
            "source": {
                "kind": "mcp",
                "server_name": "sofia_apps",
            },
        })
    );
