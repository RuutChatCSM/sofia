...
async fn sofia_apps_auth_failure_requests_elicitation_by_default() -> Result<()> {
...
"#,
    ...
    assert_eq!(
        request.id,
        sofia_protocol::mcp::RequestId::String(format!("sofia_apps_auth_{call_id}"))
    );
    assert_eq!(
        request.request,
        ElicitationRequest::Url {
            ...
            elicitation_id: format!("sofia_apps_auth_{call_id}"),
        }
    );
