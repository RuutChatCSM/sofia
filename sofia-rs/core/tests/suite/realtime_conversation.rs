...
use serde_json::json;
use sofia_app_server_protocol::ThreadRealtimeItemContent;
use sofia_app_server_protocol::ThreadRealtimeSessionOutcome;
use sofia_app_server_protocol::ThreadRealtimeTranscriptRole;
use sofia_app_server_protocol::ThreadTimelineEntry;
...
use sofia_core::test_support::auth_manager_from_auth;
use sofia_history::InitialHistory;
use sofia_history::RolloutItem;
use sofia_login::CodexAuth;
use sofia_login::OPENAI_API_KEY_ENV_VAR;
...
use sofia_protocol::protocol::ConversationTextRole;
...
use sofia_protocol::protocol::RealtimeConversationRealtimeEvent;
use sofia_protocol::protocol::RealtimeConversationVersion;
use sofia_protocol::protocol::RealtimeEvent;
use sofia_protocol::protocol::RealtimeHandoffRequested;
use sofia_protocol::protocol::RealtimeNoopRequested;
use sofia_protocol::protocol::RealtimeOutputModality;
use sofia_protocol::protocol::RealtimeTranscriptEntry;
...
use sofia_thread_store::ListTimelineParams;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Sofia.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = sofia_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "SOFIA_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in sofia_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.sofia.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = sofia_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    test.sofia
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            sofia_responses_as_items: false,
            sofia_response_item_prefix: None,
            sofia_response_handoff_mode:
                sofia_protocol::protocol::CodexResponseHandoffMode::Thinking,
            sofia_response_handoff_channel_prefixes: None,
            model: None,
    ...
    let started = wait_for_event_match(&test.sofia, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.sofia, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.sofia.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.sofia, |msg| match msg {
    ...
    test.sofia.ensure_rollout_materialized().await;
    let history = test.sofia.load_history(/*include_archived*/ false).await?;
    assert!(
        !history
            .items
            .iter()
...
async fn conversation_records_history_without_an_event_observer(
    ...
    test.sofia.submit(Op::Shutdown).await?;
...
async fn conversation_websocket_transports_send_codex_headers_without_creating_a_call(
    ...
    let conversation = test
        ...
        .start_thread(StartThreadOptions {
            ...
            environments: Some(test.sofia.config_snapshot().await.environments.environments),
            ..StartThreadOptions::new(test.config.clone())
        })
        .await?;
    let sofia = &conversation.thread;
    ...
    let started = wait_for_event_match(sofia, |msg| match msg {
        EventMsg::RealtimeConversationStarted(started) => Some(Ok(started.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
    ...
    let (expected_uri, expected_realtime_session_id) = match transport {
        ConversationStartTransport::Websocket => (
            "/v1/live?model=gpt-live-1-sofia",
    ...
    sofia.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(sofia, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
    sofia.shutdown_and_wait().await?;
    test.sofia.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}

#[test_case(None, "gpt-live-1-sofia", None, None; "default model without source")]
#[test_case(Some("session-override-model"), "session-override-model", None, None; "explicit model without source")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::User), Some("user".to_string()); "user source")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::Feature("voice_chat".to_string())), Some("voice_chat".to_string()); "voice chat source")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::Feature("realtime_voice".to_string())), Some("realtime_voice".to_string()); "realtime voice source")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::GuardianReview), Some("guardian_review".to_string()); "child shares parent session")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::Feature("x".repeat(/*n*/ 256))), Some("x".repeat(/*n*/ 256)); "source at byte limit")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::Feature("x".repeat(/*n*/ 257))), None; "oversized source omitted")]
#[test_case(None, "gpt-live-1-sofia", Some(ThreadSource::Feature("é\r\n\"".to_string())), Some("é\r\n\"".to_string()); "source escaped for headers")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_codex_headers_to_backend(
    ...
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/sofia/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let backend_base_url = format!("{}/backend-api/sofia", server.uri());
    let realtime_ws_base_url = realtime_server.uri().to_string();
    let mut builder = test_codex()
    ...
    let sofia = &conversation.thread;
    assert_eq!(sofia.config_snapshot().await.thread_source, thread_source);
    ...
    let created = wait_for_event_match(sofia, |msg| match msg {
    ...
    assert_eq!(
        ...
        (
            "/backend-api/sofia/realtime/calls",
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationSdp(created) => Some(Ok(created.clone())),
        EventMsg::Error(err) => Some(Err(err.clone())),
        _ => None,
    })
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=sofia-realtime-call-boundary")
    );
...
async fn conversation_webrtc_live_reconnects_sideband_after_unclean_disconnect(
    ...
    let handoff = timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.sofia.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.sofia, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        test.sofia.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.sofia.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.sofia_error_info, Some(CodexErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.sofia, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    let _ = wait_for_event_match(&test.sofia, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .await?;
    let sofia = resumed_thread.thread;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("sofia-rs/core"))?;
    ...
    assert!(startup_context.contains("sofia-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime(ephemeral: bool) -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.sofia, |event| match event {
        ...
        assert_eq!(
            timeline.items.iter().filter_map(|entry| match entry {
                ...
                ThreadTimelineEntry::Item { item, .. }
                    if matches!(item.as_ref(), sofia_app_server_protocol::ThreadItem::UserMessage { .. }) => Some("typed input"),
                _ => None,
            }).collect::<Vec<_>>(),
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.sofia
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            sofia_response_item_prefix: None,
            sofia_response_handoff_mode:
                sofia_protocol::protocol::CodexResponseHandoffMode::BemTags,
            sofia_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn_and_promotes_its_artifact() -> Result<()> {
    ...
    let response_mock = responses::mount_sse_once(
        ...
        responses::sse(vec![
            responses::ev_response_created("resp-1"),
            responses::ev_assistant_message("msg-1", "::sofia-realtime-inline{}\nShared artifact"),
            responses::ev_completed("resp-1"),
        ]),
    )
    .await;
    ...
    let turn_id = loop {
        let event = test.sofia.next_event().await?;
    ...
    assert_eq!(
        ...
        vec![ThreadRealtimeItemContent::BemItemPromoted {
            ...
            presentation:
                sofia_app_server_protocol::ThreadRealtimeBemItemPresentation::InlineMarkdown,
        }]
    );
