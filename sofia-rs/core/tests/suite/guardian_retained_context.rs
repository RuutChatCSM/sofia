...
use sofia_history::GuardianHistoryCheckpoint;
use sofia_history::InitialHistory;
use sofia_history::ResumedHistory;
use sofia_history::RetainedContext;
use sofia_history::RetainedContextEvent;
use sofia_history::RolloutItem;
use sofia_history::VerifiedAnswer;
use sofia_history::VerifiedQuestionAnswer;
...
use sofia_thread_store::ForkBoundary;
use sofia_thread_store::LoadThreadHistoryParams;
use sofia_thread_store::PrepareForkParams;
...
async fn retained_instructions_keep_identity_across_compaction_and_resume(
    ...
    let thread = resume(&test, &test.sofia).await?;
    ...
    let user_messages = [initial.as_str(), STEER]
        ...
        .map(|(index, text)| {
            ...
            json!({
                ...
                "text": sofia_guardian_context::truncate_text(text, /*max_tokens*/ 900),
...
async fn legacy_checkpoint_recovers_root_excerpt_before_discarding_backup(
    ...
    .await;
    let snapshot = test.sofia.conversation_history_snapshot().await;
    let source = snapshot
        .items()
        .find(|item| matches!(item, ResponseItem::Message { role, .. } if role == "user"))
        .cloned()
        .context("original instruction")?;
    let mut expected = compact_and_assert_answers(&test, &test.sofia, &[]).await?;
    let mut checkpoint = load_context(&test, &test.sofia)
        .await?
        .into_iter()
        .rev()
        .find_map(|item| match item {
    ...
    test.sofia
        ...
        .await?;
    let resumed = resume(&test, &test.sofia).await?;
    assert_eq!(
        resumed
            .conversation_history_snapshot()
            .await
...
async fn retained_answers_rollback_only_the_steered_instruction() -> Result<()> {
    ...
    test.sofia
        ...
        .await?;
    let mut thread = resume(&test, &test.sofia).await?;
...
async fn standalone_fork_retains_inherited_user_instructions(
    ...
    let answers = sofia_guardian_context::render_verified_answers(retained);
    assert_eq!(
        (answers.fragments, answers.complete),
        (
            vec![sofia_guardian_context::GuardianRootMessage::IncompleteVerifiedAnswers.render()],
            false
        ),
    );
    assert_eq!(
        retained
            ...
            .map(|(_, entry)| match entry {
                sofia_history::RetainedContextEntry::UserMessage(message) =>
                    (message.text.clone(), message.complete),
                sofia_history::RetainedContextEntry::VerifiedAnswer(_) =>
        ...
        vec![
            ...
            (
                sofia_guardian_context::truncate_text(&local_instruction, /*max_tokens*/ 900),
                !compacted,
            ),
        ],
    );
...
async fn forked_parent_instructions_do_not_become_local_authorization(
    ...
    assert!(
        root_before
            .messages
            .contains(&sofia_core::GuardianRootMessage::User(
    ...
    assert_eq!(
        expected.as_ref().map(|context| context
            .ordered_entries()
            .map(|(_, entry)| match entry {
                sofia_history::RetainedContextEntry::UserMessage(message) => message.text.as_str(),
                sofia_history::RetainedContextEntry::VerifiedAnswer(_) =>
