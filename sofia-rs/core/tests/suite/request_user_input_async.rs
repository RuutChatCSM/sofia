...
use sofia_protocol::items::AgentMessageDelivery;
use sofia_protocol::items::AgentMessageItem;
use sofia_protocol::items::AsyncUserInputQuestion;
...
async fn freeform_async_message_emits_an_item_without_ending_the_turn() -> Result<()> {
    ...
    let started = wait_for_event_match(test.sofia.as_ref(), |event| match event {
        EventMsg::ItemStarted(event) if event.item.id() == CALL_ID => Some(event.item.clone()),
        _ => None,
    })
    .await;
    let completed = wait_for_event_match(test.sofia.as_ref(), |event| match event {
    ...
    wait_for_event(test.sofia.as_ref(), |event| {
...
async fn request_user_input_async_emits_item_and_does_not_end_the_turn(
    ...
    let started = wait_for_event_match(test.sofia.as_ref(), |event| {
    ...
    let completed = wait_for_event_match(test.sofia.as_ref(), |event| {
