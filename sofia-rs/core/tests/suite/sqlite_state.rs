...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.sofia.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |sofia_home| {
            let rollout_path = sofia_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.sqlite.state_db_path();
    let rollout_path = test.config.sofia_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = sofia_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = sofia_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
