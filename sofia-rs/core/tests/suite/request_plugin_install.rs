...
use sofia_core_plugins::startup_sync::curated_plugins_repo_path;
...
use sofia_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__sofia_apps__calendar";
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if server
                ...
                .unwrap_or_default()
                .iter()
                .any(|request| request.url.path() == "/ps/plugins/suggested/sofia")
            {
                return;
            }
...
async fn start_gated_step_preparation(test: &TestCodex, server: &MockServer) -> Result<PathUri> {
    ...
    let prior_recommendation_count = server
        ...
        .unwrap_or_default()
        .into_iter()
        .filter(|request| request.url.path() == "/ps/plugins/suggested/sofia")
        .count();
...
async fn start_install_turn(test: &TestCodex, prompt: &str) -> Result<ElicitationRequestEvent> {
    ...
    Ok(wait_for_event_match(&test.sofia, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "sofia_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": sofia_login::default_client::originator().value,
            }
        })
    );
...
async fn run_remote_plugin_install_refresh_case(refreshed_tools: RefreshedAppsTools) -> Result<()> {
    ...
    drop(requests);
    test.sofia.refresh_runtime_config(test.config.clone()).await;
