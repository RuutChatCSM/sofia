...
use sofia_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
...
use sofia_protocol::protocol::CONTEXT_WINDOW_OPEN_TAG;
...
async fn assert_context_compaction_item_lifecycle(sofia: &std::sync::Arc<sofia_core::CodexThread>) {
...
async fn token_budget_guidance_precedes_standalone_context_window(
    ...
    let backend_url = format!("{}/backend-api/sofia", server.uri());
    let test = test_codex()
...
#[test_case("OpenAI", "/backend-api/sofia", None, true, true; "sofia_backend")]
#[test_case("OpenAI", "/backend-api/sofia", None, false, false; "unsupported_model")]
#[test_case("Custom", "/backend-api/sofia", None, true, false; "custom_provider")]
#[test_case("OpenAI", "/v1", None, true, false; "non_codex_endpoint")]
#[test_case("OpenAI", "/backend-api/sofia", Some("test-provider-token"), true, false; "provider_credentials")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_codex()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.sofia, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.sofia).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.sofia, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.sofia, |event| match event {
        EventMsg::HookCompleted(completed)
