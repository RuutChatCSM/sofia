...
use sofia_exec_server::RemoveOptions;
use sofia_extension_api::ExtensionDataInit;
use sofia_extension_api::ExtensionEventSink;
use sofia_extension_api::ExtensionFuture;
use sofia_extension_api::ExtensionRegistryBuilder;
use sofia_extension_api::ExtensionWarning;
use sofia_extension_api::SkillInvocationContributor;
use sofia_extension_api::SkillInvocationInput;
...
use sofia_skills_extension::ExecutorSkillProvider;
use sofia_skills_extension::HostSkillProvider;
use sofia_skills_extension::OrchestratorSkillProvider;
use sofia_skills_extension::SkillProvider;
use sofia_skills_extension::SkillProviderSource;
use sofia_skills_extension::SkillProviders;
use sofia_skills_extension::SkillsExtensionConfig;
use sofia_skills_extension::catalog::SkillAuthority;
use sofia_skills_extension::catalog::SkillCatalog;
use sofia_skills_extension::catalog::SkillCatalogEntry;
use sofia_skills_extension::catalog::SkillPackageId;
use sofia_skills_extension::catalog::SkillProviderError;
use sofia_skills_extension::catalog::SkillReadResult;
use sofia_skills_extension::catalog::SkillResourceId;
use sofia_skills_extension::catalog::SkillSearchResult;
use sofia_skills_extension::catalog::SkillSourceKind;
use sofia_skills_extension::install;
use sofia_skills_extension::install_with_providers;
use sofia_skills_extension::provider::SkillListQuery;
use sofia_skills_extension::provider::SkillProviderFuture;
use sofia_skills_extension::provider::SkillReadRequest;
use sofia_skills_extension::provider::SkillSearchRequest;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_path_uri::PathUri;
use sofia_utils_string::approx_token_count;
...
fn write_host_skills(sofia_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<sofia_extension_api::ExtensionRegistry<Config>>,
...
async fn wait_for_analytics_events(
    ...
    loop {
        let events = server
            ...
            .unwrap_or_default()
            .into_iter()
            .filter(|request| request.url.path() == "/sofia/analytics-events/events")
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.sofia_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    for _ in 0..turn_count {
        test.sofia
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.sofia, |_| true).await {
    ...
    let _codex_home_guard = sofia_home;
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    let sofia_home = Arc::new(TempDir::new()?);
    let host_skill_dir = sofia_home.path().join("skills/host-search");
    ...
    let plugin_root = sofia_home.path().join("plugins/cache/test/sample/local");
    ...
    core_test_support::wait_for_event(&test.sofia, |event| {
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_codex().with_home(sofia_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.sofia
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.sofia, |event| {
        matches!(
            event,
...
text({ names: result.skills.map(skill => skill.name), warnings: result.warnings, next_cursor: result.next_cursor });"#,
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/sofia/ps/mcp/?$"))
        .and(|request: &Request| {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.sofia, SOFIA_APPS_MCP_SERVER_NAME).await?;
...
async fn opted_in_executor_provider_skips_host_discovery_but_injects_discovered_skill() -> Result<()>
{
    ...
    let host_skill_path = sofia_home
        .path()
        .join("skills")
...
async fn executor_skill_invocation_is_environment_scoped_and_deduplicated() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_path = sofia_home.path().join("executor-skill/SKILL.md");
...
async fn production_turn_aliases_combined_skill_catalogs_under_shared_budget() -> Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(
        tempfile::Builder::new()
            .prefix("sofia-integration-shared-skill-catalog-roots-")
            .tempdir()?,
    );
    write_host_skills(
        sofia_home.path(),
    ...
    let developer_text = response
        ...
        .message_input_texts("developer")
        .join("\n");
    let host_root = dunce::canonicalize(sofia_home.path().join("skills"))?
        .to_string_lossy()
        .replace('\\', "/");
    for (alias, root) in [
        ("e0", EXECUTOR_ROOT),
...
async fn production_turn_uses_configured_skill_catalog_token_budget() -> Result<()> {
    ...
    write_host_skills(sofia_home.path(), &HOST_CATALOG)?;
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("sofia-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let sofia_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_uses_provider_host_catalog_and_core_snapshot_injection() -> Result<()> {
    ...
    write_host_skills(sofia_home.path(), &[(skill_name, snapshot_description)])?;
    let skill_path = sofia_home
        .path()
        .join("skills")
        .join(skill_name)
        .join("SKILL.md");
    ...
    let app_mentioned_events =
        wait_for_analytics_events(&server, "sofia_app_mentioned", /*expected_count*/ 1).await;
...
async fn production_turn_suppresses_only_the_superseded_host_skill_prompt() -> Result<()> {
    ...
    let first_skill_path = sofia_home.path().join("skills/first-host/SKILL.md");
    let second_skill_path = sofia_home.path().join("skills/second-host/SKILL.md");
...
async fn production_turn_warns_and_omits_unreadable_host_skill() -> Result<()> {
    ...
    let missing_skill_path =
        dunce::canonicalize(sofia_home.path().join("skills/missing-host/SKILL.md"))?;
    let available_skill_path =
        dunce::canonicalize(sofia_home.path().join("skills/available-host/SKILL.md"))?;
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    let skill_dir = sofia_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_keeps_orchestrator_world_state_incremental_across_turns() -> Result<()> {
    ...
    let skill_resource = "skill://sofia_apps/orchestrator-search/SKILL.md";
    let catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
                SkillPackageId("orchestrator/orchestrator-search".to_string()),
                SkillAuthority::new(SkillSourceKind::Orchestrator, SOFIA_APPS_MCP_SERVER_NAME),
                skill_name,
                skill_description,
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Sofia can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
