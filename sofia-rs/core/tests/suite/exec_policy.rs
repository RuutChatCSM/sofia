...
"#;
    ...
    let mut migrated_builder = test_codex().with_config(|config| {
        let policy_path = config.sofia_home.join("rules/default.rules");
    ...
    let migrated_policy_path = migrated.sofia_home_path().join("rules/default.rules");
    assert_eq!(fs::read_to_string(&migrated_policy_path)?, MIGRATED_POLICY);
    assert_eq!(
        fs::read_to_string(migrated.sofia_home_path().join(MIGRATION_MARKER_FILENAME))?,
        "v1\n"
    );
    ...
    let ignored_policy_path = ignored.sofia_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_codex().with_config(|config| {
        let policy_path = config.sofia_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.sofia_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),
...
async fn environment_command_restrictions_override_saved_prefix_approvals() -> Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config
            ...
            .enable(Feature::UnifiedExec)
            .expect("test config should allow feature update");
        let policy_path = config.sofia_home.join("rules").join("approved.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(
            policy_path,
...
async fn environment_command_policy_changes_invalidate_session_approvals() -> Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        ...
        config.approvals_reviewer = ApprovalsReviewer::User;
        let policy_path = config.sofia_home.join("rules").join("prompt.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(
            policy_path,
