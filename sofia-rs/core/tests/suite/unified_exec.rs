...
use sofia_protocol::config_types::ShellEnvironmentPolicyInherit;
...
use sofia_protocol::protocol::ExecCommandStatus;
use sofia_protocol::protocol::Op;
use sofia_protocol::protocol::ThreadSettingsOverrides;
use sofia_protocol::shell_environment::SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use sofia_protocol::user_input::UserInput;
use sofia_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.sofia, |event| match event {
...
async fn exec_command_does_not_expose_configured_noise_auth_token() -> Result<()> {
    ...
    let builder = test_codex().with_model("gpt-5.4").with_config(|config| {
        config.permissions.shell_environment_policy.r#set.insert(
            SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
            "configured-noise-token".to_string(),
        );
        config.permissions.shell_environment_policy.r#set.insert(
            SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_ascii_lowercase(),
    ...
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            "if [ -n \"${SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN:-}\" ] || [ -n \"${sofia_exec_server_noise_auth_token:-}\" ]; then echo leaked; else echo unset; fi"
        }
        core_test_support::TestTargetOs::Windows => {
            "if ($env:SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN) { Write-Output leaked } else { Write-Output unset }"
        }
    };
...
async fn exec_command_uses_installed_environment_shell_policy_with_explicit_overrides() -> Result<()>
{
    ...
    let command = match core_test_support::test_target_os() {
        core_test_support::TestTargetOs::Linux | core_test_support::TestTargetOs::MacOs => {
            r#"printf '%s:%s:%s:%s' "$KEEP" "${DROP:-missing}" "${OWNER_ONLY:-missing}" "$SOFIA_VERSION""#
        }
        core_test_support::TestTargetOs::Windows => {
            r#"if (Test-Path Env:DROP) { $drop = $env:DROP } else { $drop = 'missing' }; if (Test-Path Env:OWNER_ONLY) { $owner = $env:OWNER_ONLY } else { $owner = 'missing' }; Write-Output "${env:KEEP}:${drop}:${owner}:${env:SOFIA_VERSION}""#
        }
    };
    let arguments = json!({
        "cmd": command,
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    wait_for_event(&sofia, |event| match event {
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.sofia, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.sofia, |msg| match msg {
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.sofia, |_| true).await;
        match msg {
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://sofia-network-denied.invalid/ HTTP/1.1\\r\\nHost: sofia-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://sofia-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: sofia-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
async fn wait_for_unified_exec_end(
...
) -> (sofia_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = tokio::time::timeout(remaining, test.sofia.next_event())
            .await
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.sofia, |_| true).await;
        match event_msg {
...
async fn write_stdin_returns_exit_metadata_and_clears_session() -> Result<()> {
    ...
    let mut turn_completed = false;
    loop {
        let event = wait_for_event(&test.sofia, |_| true).await;
        match event {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&sofia, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&sofia, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    sofia.submit(Op::Interrupt).await?;
    wait_for_event(&sofia, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    sofia.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.sofia.next_event().await.expect("event");
...
"#
    ...
    let end_event = wait_for_event_match(&test.sofia, |event| match event {
