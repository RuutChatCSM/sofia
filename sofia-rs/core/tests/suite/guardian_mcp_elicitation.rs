...
use sofia_protocol::protocol::GuardianAssessmentAction;
        ...
        for index, meta in enumerate(approval_metas):
            ...
            pending_elicitations.append({
                ...
                "method": "elicitation/create", "params": {
                    ...
                    "_meta": {"sofia_request_type": "approval_request",
                              "sofia_approval_kind": "mcp_tool_call", "tool_name": "write_record",
...
async fn interrupt_aborts_server_initiated_mcp_guardian_review() -> Result<()> {
    ...
    wait_for_mcp_server(&test.sofia, "elicitation").await?;
...
async fn server_initiated_mcp_elicitation_can_require_synchronous_auto_review(
    ...
    if strict_auto_review {
        meta.insert("sofia_strict_auto_review".to_string(), json!(true));
    }
    if let Some(sensitive_action) = sensitive_action {
        meta.insert(
            "sofia_sensitive_action".to_string(),
...
async fn node_elicitations_attribute_independent_reviews_without_changing_actions(
    ...
    let meta = actions.map(|(tool_name, arguments)| {
        let mut meta = json!({
            "sofia_strict_auto_review": strict_auto_review,
            "sofia_sensitive_action": true,
            "tool_name": tool_name,
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.sofia, server_name).await?;
    if matches!(
        call_id_source,
        CallIdSource::WrongServer | CallIdSource::PreviousTurn
    ) {
    ...
    loop {
        let event = tokio::time::timeout(Duration::from_secs(30), test.sofia.next_event())
            .await
...
async fn user_review_preserves_unsupported_guardian_elicitations(mode: &str) -> Result<()> {
    ...
    let test = test_codex()
        .with_config(move |config| {
            ...
            config.permissions.approval_policy = Constrained::allow_any(AskForApproval::Granular(
                sofia_protocol::protocol::GranularApprovalConfig {
    ...
    match &request.request {
        sofia_protocol::approvals::ElicitationRequest::Form {
            requested_schema, ..
        } => {
            assert_eq!(requested_schema, &form_schema);
        }
        sofia_protocol::approvals::ElicitationRequest::Url { url, .. } => {
    ...
    test.sofia
        .submit(Op::ResolveElicitation {
            ...
            decision: sofia_protocol::approvals::ElicitationAction::Decline,
            content: None,
            meta: None,
        })
        .await?;
