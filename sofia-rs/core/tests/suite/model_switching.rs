...
use serde_json::json;
use sofia_config::types::Personality;
...
use sofia_extension_api::ThreadIdleInput;
...
use sofia_protocol::openai_models::default_input_modalities;
...
async fn first_turn_after_empty_prefix_fork_preserves_inherited_base_instructions(
    ...
    let source_rollout_path = test.sofia.rollout_path().expect("rollout path");
    let source_history =
        sofia_rollout::RolloutRecorder::get_rollout_history(&source_rollout_path).await?;
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-sofia-personality";
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
