...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.sofia, |event| {
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-sofia-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-sofia-turn-metadata/thread_id"),
        Some(&json!(test.session_configured.thread_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-sofia-turn-metadata/turn_id")
            .and_then(Value::as_str)
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-sofia-turn-metadata/model")
            .and_then(Value::as_str),
        Some("gpt-5.4")
    );
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-sofia-turn-metadata/reasoning_effort")
            .and_then(Value::as_str),
        Some(first_request_reasoning_effort)
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-sofia-turn-metadata/turn_started_at_unix_ms")
