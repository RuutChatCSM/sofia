...
use sofia_core::config::NetworkProxySpec;
use sofia_core::shell::ShellType;
use sofia_core::shell::get_shell;
...
use sofia_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "sofia-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://sofia-network-test.invalid:80";
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.sofia, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.sofia.next_event())
            .await
...
#[test_case("GET", "http://sofia-network-test.invalid/"; "plain_http")]
#[test_case("CONNECT", "sofia-network-test.invalid:443"; "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg_attr(
    not(target_os = "linux"),
    ignore = "requires the trusted Linux proxy bridge"
...
async fn background_network_approval_uses_active_turn_after_original_turn_completes() -> Result<()>
{
    ...
    .await?;
    let EventMsg::TurnComplete(first_turn) = wait_for_event(&test.sofia, |event| {
    ...
    assert_eq!(test.sofia.list_background_terminals().await.len(), 1);
    ...
    .await?;
    let EventMsg::TurnStarted(active_turn) = wait_for_event(&test.sofia, |event| {
    ...
    test.sofia
        ...
        .await?;
    let assessment = wait_for_event(&test.sofia, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    assert_eq!(
        test.sofia.list_background_terminals().await.len(),
        1,
        "approved network access must not terminate the background process"
    );
    test.sofia.submit(Op::CleanBackgroundTerminals).await?;
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#sofia-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("sofia-network-test.invalid is not in the allowed_domains")
    );
    test.sofia
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="sofia-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): sofia-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="sofia-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.sofia.list_background_terminals().await.is_empty() {
                break;
            }
