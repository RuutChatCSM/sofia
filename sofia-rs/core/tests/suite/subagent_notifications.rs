...
use sofia_core::config::AgentRoleConfig;
...
use sofia_protocol::openai_models::MultiAgentMessages;
...
fn write_home_skill(sofia_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = sofia_home.join("skills").join(dir);
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.sofia_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.sofia_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.sofia_home_path(), "stop_hook_log.jsonl")?;
...
async fn spawned_child_receives_forked_parent_context(
    ...
    let child_metadata: serde_json::Value = serde_json::from_str(
        child_body["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    if matches!(selection, FullHistoryV2ModelSelection::WorldStateIdentity) {
        ...
        let child_rollout = sofia_rollout::RolloutRecorder::get_rollout_history(
            &child_thread
                .rollout_path()
...
async fn plaintext_multi_agent_v2_completion_sends_agent_message(
    ...
    let (child_request, child_turn_metadata) = loop {
        let child_request = child_request.requests().into_iter().find_map(|request| {
            ...
            let turn_metadata: Value =
                serde_json::from_str(body["client_metadata"]["x-sofia-turn-metadata"].as_str()?)
                    .ok()?;
            turn_metadata
                .get("parent_turn_id")
                .and_then(Value::as_str)?;
    ...
    if let Some((parent_turn_id, child_turn_id, child_thread_id)) = expected_completed_activity {
        ...
        let rollout = sofia_rollout::RolloutRecorder::get_rollout_history(
            &test.sofia.rollout_path().expect("parent rollout path"),
        )
        .await?;
        assert!(
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.sofia_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.sofia_home.join("instructions-only-role.toml");
    ...
    let test = test_codex()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.sofia_home.join("model-only-role.toml");
