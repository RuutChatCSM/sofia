...
use sofia_protocol::protocol::ExecPolicyAmendment;
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Sofia helper binary
...
"#,
    ...
    let command = r#"printf '%s\n%s\n%s\n' "$GH_HOST" "$GH_ENTERPRISE_TOKEN" "$SOFIA_NETWORK_PROXY_CREDENTIAL_BROKER_ACTIVE""#;
    let event = shell_event(
        call_id,
        command,
        /*timeout_ms*/ 10_000,
...
/// same sandbox with `sofia sandbox -P`. The nested invocation must also pass
...
printenv SOFIA_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit SOFIA_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `sofia sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Sofia sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("SOFIA_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("SOFIA_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("SOFIA_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected sofia sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Sofia sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/sofia-fallback-rule-test.txt && echo hello > /tmp/sofia-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://sofia-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
