...
use sofia_core::RecoverTurnRequest;
...
async fn submit_user_message(
    ...
    text: &str,
) -> sofia_protocol::error::Result<TurnInputSubmission> {
    sofia.start_or_steer_turn(user_message_request(text)).await
}
...
async fn start_turn_if_idle_keeps_automatic_plan_rejections_atomic(
    ...
    let mut collaboration_mode = test.sofia.config_snapshot().await.collaboration_mode;
    ...
    let current_settings = test.sofia.thread_settings_snapshot().await;
    ...
    .await;
    assert_eq!(
        test.sofia.config_snapshot().await.collaboration_mode,
        collaboration_mode
    );
...
async fn recover_turn_if_idle_preserves_id_and_resumes_plan_mode() {
    ...
    assert_eq!(
        test.sofia.config_snapshot().await.collaboration_mode.mode,
        ModeKind::Plan
    );

    let started = wait_for_event(&test.sofia, |event| {
...
async fn turn_input_submission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    let first_submission = tokio::spawn({
        let sofia = Arc::clone(&sofia);
        let barrier = Arc::clone(&barrier);
        async move {
            barrier.wait().await;
            submit_user_message(sofia.as_ref(), "first message").await
        }
    });
    let second_submission = tokio::spawn({
        ...
        async move {
            barrier.wait().await;
            submit_user_message(sofia.as_ref(), "second message").await
        }
    });
    barrier.wait().await;
    ...
    wait_for_event(sofia.as_ref(), |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_input_submission_applies_thread_settings_only_after_accepted_input() {
    ...
    let started = submit_user_message(sofia, "start turn")
        .await
    ...
    assert_eq!(
        sofia.config_snapshot().await.approval_policy,
        AskForApproval::Never
    );
    assert_eq!(
        sofia.environment_selections().await,
