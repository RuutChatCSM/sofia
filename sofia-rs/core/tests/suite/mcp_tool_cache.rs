...
use sofia_protocol::mcp::McpServerConnectionStatus;
...
async fn mcp_calls_stay_bound_to_each_thread() -> anyhow::Result<()> {
    ...
    let calls = [
        (&fixture.sofia, "first-call", "first-runtime"),
        (&second_thread, "second-call", "second-runtime"),
        (&fixture.sofia, "first-again", "first-runtime"),
    ];
    ...
    fixture.sofia.shutdown_and_wait().await?;
...
async fn apps_cache_filled_during_binding_capture_reaches_the_model() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(30), async {
                ...
                config
                    ...
                    .set(std::collections::HashMap::from([(
                        ...
                        serde_json::from_value(json!({
                            "url": format!("{}/api/sofia/ps/mcp", waiting.chatgpt_base_url),
        ...
        let EventMsg::McpStartupComplete(startup) = wait_for_event(&test.sofia, |event| {
        ...
        let NewThread { thread: peer, .. } = test
            ...
            .await?;
        wait_for_mcp_server(&peer, SOFIA_APPS_MCP_SERVER_NAME).await?;
...
async fn cached_http_mcp_starts_lazily_for_subagents(
    ...
    let server_url = format!("{}/api/sofia/ps/mcp", http_server.chatgpt_base_url);
    let fixture = test_codex()
        .with_model_info_override("gpt-5.4", |model| model.supports_search_tool = false)
        .with_config(move |config| {
            config.permissions.approval_policy = Constrained::allow_any(AskForApproval::Never);
