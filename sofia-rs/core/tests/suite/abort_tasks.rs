...
use sofia_core::SuspendTurnOutcome;
...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    let fixture = test_codex()
        ...
        .await
        .unwrap();
    let sofia = Arc::clone(&fixture.sofia);
    ...
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    sofia.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
    sofia.submit(Op::CleanBackgroundTerminals).await.unwrap();
}
...
async fn root_turn_suspension_preserves_unfinished_turn_history() {
    ...
    let sofia_core::TurnInputSubmission::Started { turn_id } = submitted else {
    ...
    let items = rollout
        .lines()
        .map(|line| {
            sofia_rollout::parse_rollout_line(line)
    ...
    assert_eq!(
        resumed
            .sofia
            .recover_turn_if_idle(sofia_core::RecoverTurnRequest {
                turn_id: turn_id.clone(),
        ...
        sofia_core::StartIfIdleSubmission::Started {
            turn_id: turn_id.clone(),
        },
    );
    let completed = wait_for_event(&resumed.sofia, |event| {
