...
fn replacement_history_from_rollout(path: &Path) -> Result<Vec<Value>> {
    ...
    {
        let entry = sofia_rollout::parse_rollout_line(line)?;
...
async fn assert_compaction_uses_turn_lifecycle_id(sofia: &std::sync::Arc<sofia_core::CodexThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = sofia.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions(
    ...
    // Build config pointing to the mock server and spawn Sofia.
    ...
    let warning_event = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Sofia to flush rollout entries before inspecting the file.
    ...
    for line in text.lines() {
        ...
        let Ok(entry) = sofia_rollout::parse_rollout_line(trimmed) else {
            continue;
        };
        match entry.item {
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    sofia.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&sofia, |ev| match ev {
    ...
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.sofia_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.sofia_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_records_durable_and_local_token_usage() {
    ...
    let last = wait_for_event_match(&sofia, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    let sofia = builder.build(&server).await.unwrap().sofia;
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        sofia_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    wait_for_event(&resumed.sofia, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test sofia");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.sofia).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using Sofia with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test sofia");
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.sofia).await;
...
async fn pre_sampling_compact_falls_back_after_previous_model_invalid_request_on_downshift() {
    ...
    let [first_metadata, compact_metadata] = [&requests[0], &requests[1]].map(|request| {
        serde_json::from_str::<Value>(
            request.body_json()["client_metadata"]["x-sofia-turn-metadata"]
                .as_str()
...
async fn auto_compact_persists_rollout_entries() {
    ...
    let expected_settings = sofia.thread_settings_snapshot().await;
    sofia.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&sofia, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&sofia, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-sofia-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-sofia-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let sofia = builder.build(&server).await.unwrap();

    sofia.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    sofia.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    sofia.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.sofia.instruction_sources().await,
...
-> Result<()> {
    ...
    assert_eq!(
        resumed.sofia.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
