...
async fn settings_notifications_keep_their_commit_across_postcommit_work(
    ...
    let test = test_codex()
        ...
        .await?;
    let initial = test.sofia.restorable_thread_settings().await;
        ...
        async move {
            match operation {
                ...
                SettingsOperation::Standalone => {
                    sofia.submit(Op::ThreadSettings { thread_settings }).await
                }
            }
        }
    });
    ...
    let expected = test.sofia.thread_settings_snapshot().await;
    ...
    timeout(TIMEOUT, test.sofia.restore_thread_settings(initial)).await??;
    let restored = test.sofia.thread_settings_snapshot().await;
    ...
    assert_eq!(test.sofia.thread_settings_snapshot().await, restored);
    Ok(())
}
