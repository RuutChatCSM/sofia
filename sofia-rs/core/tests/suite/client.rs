...
use sofia_core::ModelClient;
use sofia_core::NewThread;
use sofia_core::Prompt;
use sofia_core::ResponseEvent;
use sofia_core::StartThreadOptions;
use sofia_core::ThreadManager;
use sofia_core::TurnInputRequest;
use sofia_core::X_CODEX_ROUTING_HINT_HEADER;
use sofia_core::resolve_installation_id;
use sofia_core::thread_store_from_config;
use sofia_extension_api::empty_extension_registry;
use sofia_features::Feature;
use sofia_history::RolloutItem;
use sofia_history::RolloutLine;
...
use sofia_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use sofia_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use sofia_protocol::models::LocalShellAction;
use sofia_protocol::models::LocalShellExecAction;
use sofia_protocol::models::LocalShellStatus;
use sofia_protocol::models::MessagePhase;
use sofia_protocol::models::ReasoningItemContent;
use sofia_protocol::models::ReasoningItemReasoningSummary;
use sofia_protocol::models::ResponseItem;
use sofia_protocol::models::WebSearchAction;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> sofia_core::CodexResponsesMetadata {
...
fn assert_codex_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-sofia-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-sofia-turn-metadata"]
        .as_str()
        .expect("missing x-sofia-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-sofia-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-sofia-window-id"].as_str(),
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.sofia.submit(Op::Shutdown).await?;
    wait_for_event(&initial.sofia, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let sofia_home = Arc::new(TempDir::new()?);
    let mut builder = test_codex();
    let first = builder
        .resume(&server, Arc::clone(&sofia_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.sofia.submit(Op::Shutdown).await?;
    wait_for_event(&first.sofia, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, sofia_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    test.sofia.submit(Op::Compact).await?;
...
/// Writes an `auth.json` into the provided `sofia_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        sofia_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: sofia_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = sofia_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = sofia_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![sofia_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = sofia_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Sofia to resume from our file
    let sofia_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(sofia_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, sofia_home, session_path.clone())
        .await
        .expect("resume conversation");
    let sofia = test.sofia.clone();
    ...
    sofia
        ...
        .await
        .unwrap();
    wait_for_event(&sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.sofia_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&sofia_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = sofia_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        sofia_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &sofia_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/sofia", server.uri()));
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/sofia/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = CodexAuth::from_auth_storage(
        ...
        &sofia_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load CodexAuth")
    .expect("No CodexAuth found in sofia_home");
    let auth_manager = sofia_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.sofia_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        sofia_core::build_models_manager(&config, auth_manager),
        sofia_core::CodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(sofia_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(sofia_core::test_support::EmptyUserInstructionsProvider),
        /*analytics_events_client*/ None,
        sofia_core::passthrough_image_store(),
    ...
    let NewThread { thread: sofia, .. } = thread_manager
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__sofia_apps"),
        "did not expect sofia_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect sofia_apps resource calls: {resource_methods:?}"
    );
}
...
async fn powershell_shell_version_is_model_visible_only_when_enabled() -> anyhow::Result<()> {
    ...
    let shell_path = sofia_shell_command::powershell::try_find_powershell_executable_blocking()
    ...
    for enabled in [false, true] {
        ...
        let user_shell = sofia_shell_command::shell_detect::DetectedShell {
            shell_type: sofia_shell_command::shell_detect::ShellType::PowerShell,
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { sofia, .. } = test_codex()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { sofia, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { sofia, config, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        sofia_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-sofia-primary-used-percent", "12.5")
        .insert_header("x-sofia-secondary-used-percent", "40.0")
        .insert_header("x-sofia-primary-window-minutes", "10")
        .insert_header("x-sofia-secondary-window-minutes", "60")
        .insert_header("x-sofia-primary-reset-at", "1704069000")
        .insert_header("x-sofia-secondary-reset-at", "1704074400")
    ...
    wait_for_event(&sofia, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-sofia-primary-used-percent", "100.0")
        .insert_header("x-sofia-secondary-used-percent", "87.5")
        .insert_header("x-sofia-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-sofia-primary-window-minutes", "15")
        .insert_header("x-sofia-secondary-window-minutes", "60")
        .insert_header("x-sofia-credits-has-credits", "true")
        .insert_header("x-sofia-credits-unlimited", "false")
        .insert_header("x-sofia-credits-balance", "")
        .insert_header(
            "x-sofia-rate-limit-reached-type",
    ...
    let mut builder = test_codex();
    let sofia_fixture = builder.build(&server).await?;
    let sofia = sofia_fixture.sofia.clone();
    ...
    let token_event = wait_for_event(&sofia, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&sofia, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&sofia, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::Error(_))).await;
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Sofia sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
