...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    sofia
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    assert_eq!(error.sofia_error_info, None);
...
async fn user_shell_command_honors_default_and_extended_deadlines() -> anyhow::Result<()> {
    for (timeout_ms, expected_ms) in [(None, 3_600_000), (Some(28_800_000), 28_800_000)] {
        ...
        let completed = async {
            loop {
                let event = fixture.sofia.next_event().await.expect("read shell event");
...
fn slow_user_shell_command() -> &'static str {
    match sofia_core::shell::default_user_shell().name() {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.sofia, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.sofia.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.sofia, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.sofia, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.sofia, |ev| match ev {
    ...
    let _ = wait_for_event(&test.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.sofia, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
