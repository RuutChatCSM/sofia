...
async fn compressed_shared_fork_resume_preserves_checkpoint_and_frozen_history() -> Result<()> {
    ...
    let parent_path = test.sofia.rollout_path().context("parent rollout")?;
    ...
    assert_eq!(
        sofia_rollout::read_session_meta_line(&child_path)
            .await?
            .meta
            .history_base
    ...
    let store =
        sofia_core::thread_store_from_config(&config, sofia_core::init_state_db(&config).await);
    ...
    let resumed = test
        ...
        .resume_thread_with_history(
            ...
            sofia_core::test_support::auth_manager_from_auth(sofia_login::CodexAuth::from_api_key(
                "dummy",
            )),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
