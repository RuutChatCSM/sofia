...
async fn sofia_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    sofia_returns_json_result("gpt-5.4".to_string()).await
}

async fn sofia_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("sofia_output_schema".into()))
    ...
    let TestCodex { sofia, config, .. } = test_codex().build(&server).await?;
    ...
    let message = wait_for_event(&sofia, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
