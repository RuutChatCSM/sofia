...
//! `just test -p sofia-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let sofia_home = home.path().join(".sofia");
    ...
    let mut cmd = Command::new(sofia_utils_cargo_bin::cargo_bin("sofia-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn sofia-rs");
