...
use sofia_core::config::TokenBudgetConfig;
...
use sofia_protocol::openai_models::ToolMessage;
use sofia_protocol::openai_models::ToolMessages;
use sofia_protocol::protocol::AskForApproval;
use sofia_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
use sofia_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_OPEN_TAG;
...
use sofia_protocol::protocol::TurnSettingsUpdate;
use sofia_protocol::protocol::TurnSettingsUpdateOutcome;
use sofia_protocol::request_user_input::RequestUserInputAnswer;
use sofia_protocol::request_user_input::RequestUserInputEvent;
...
async fn settings_updates_preserve_turn_identity_and_target(target: SettingsTarget) -> Result<()> {
    ...
    let original_settings = test.sofia.thread_settings_snapshot().await;
    let request = start_paused_turn(&test.sofia).await?;
    ...
    match target {
        ...
        SettingsTarget::Turn => {
            ...
            assert_eq!(
                submit_turn_settings(&test.sofia, "different-turn", update.clone()).await?,
                TurnSettingsUpdateOutcome::TargetUnavailable
            );
            assert_eq!(
                submit_turn_settings(&test.sofia, &request.turn_id, update).await?,
                TurnSettingsUpdateOutcome::Applied
            );
        }
    }
    answer_paused_turn(&test.sofia, &request.turn_id).await?;
    let mut settings_events = Vec::new();
    let mut new_turns = Vec::new();
    let completion = wait_for_event(&test.sofia, |event| match event {
...
async fn active_model_switch_resolves_token_budget_from_original_preferences(
    ...
    if reload_user_config {
        std::fs::write(
            test.sofia_home_path().join("config.toml"),
            "[features.token_budget]\nenabled = true\nreminder_message_template = \"Reloaded reminder\"\n",
        )?;
        test.sofia.submit(Op::ReloadUserConfig).await?;
    }
...
async fn mcp_confirmation_policy_follows_step_model_changes() -> Result<()> {
    ...
    .await;
    let mcp_url = format!("{}/api/sofia/ps/mcp", server.uri());
    let test = step_settings_test()
        .with_config(move |config| {
            config
                .features
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&test.sofia, "node_repl").await?;
...
async fn sparse_updates_preserve_divergent_active_and_future_models() -> Result<()> {
    ...
    answer_paused_turn(&test.sofia, &request.turn_id).await?;
    let second_request = wait_for_event_match(&test.sofia, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    let durable_settings = wait_for_event_match(&test.sofia, |event| match event {
    ...
    answer_paused_turn(&test.sofia, &second_request.turn_id).await?;
    wait_for_event(&test.sofia, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn turn_settings_do_not_target_idle_or_finished_turns() -> Result<()> {
    ...
    for (discarded_model, model) in [(MODEL_C, MODEL_B), (MODEL_A, MODEL_C)] {
        ...
        let before = test.sofia.thread_settings_snapshot().await;
        assert_eq!(
            submit_turn_settings(
                &test.sofia,
                &turn_id,
        ...
        assert_eq!(test.sofia.thread_settings_snapshot().await, before);
        core_test_support::submit_thread_settings(
            &test.sofia,
            ThreadSettingsOverrides {
                model: Some(model.to_string()),
...
async fn model_activation_uses_destination_metadata_defaults(
    ...
    answer_paused_turn(&test.sofia, &request.turn_id).await?;
    let paused = wait_for_event_match(&test.sofia, |event| match event {
        EventMsg::RequestUserInput(request) => Some(request.clone()),
        _ => None,
    })
    .await;
    ...
    answer_paused_turn(&test.sofia, &paused.turn_id).await?;
    wait_for_event(&test.sofia, |event| match event {
...
async fn request_user_input_async_description_follows_mid_turn_model_changes() -> Result<()> {
    ...
    let test = step_settings_test()
        ...
        .build_with_auto_env(&server)
        .await?;
    let paused = start_paused_turn(&test.sofia).await?;
    assert_eq!(
        submit_turn_settings(
            &test.sofia,
            &paused.turn_id,
...
async fn turn_settings_rejection_preserves_independent_future_settings() -> Result<()> {
    ...
    .await?;
    let future = test.sofia.thread_settings_snapshot().await;
    assert_eq!(
        submit_turn_settings(
            &test.sofia,
            &request.turn_id,
    ...
    assert_eq!(test.sofia.thread_settings_snapshot().await, future);
    answer_paused_turn(&test.sofia, &request.turn_id).await?;
    wait_for_event(&test.sofia, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
