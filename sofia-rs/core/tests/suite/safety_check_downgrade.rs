...
const REQUESTED_MODEL: &str = "gpt-5.3-sofia";
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let reroute = wait_for_event(&test.sofia, |event| {
    ...
    let warning = wait_for_event(&test.sofia, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.sofia, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.sofia, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.sofia_error_info, Some(CodexErrorInfo::CyberPolicy));
