...
use sofia_core::TimeProvider;
pub use sofia_core::TurnInputRequest;
use sofia_core::config::Config;
use sofia_core::resolve_installation_id;
use sofia_core::shell::Shell;
use sofia_core::shell::get_shell_by_model_provided_path;
...
use sofia_extension_api::LoadUserInstructionsFuture;
use sofia_extension_api::UserInstructionsProvider;
...
use sofia_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
use sofia_utils_path_uri::PathConvention;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-sofia";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: sofia_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let environment =
            sofia_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &sofia_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                sofia_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    ///
    /// With no remote test configuration, or with `SOFIA_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `SOFIA_TEST_ENVIRONMENT=docker` or
    /// `SOFIA_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `SOFIA_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `SOFIA_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.sofia.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let sofia_linux_sandbox_exe = Some(
            crate::find_codex_linux_sandbox_exe()
                .context("should find binary for sofia-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let sofia_linux_sandbox_exe = None;
        let local_runtime_paths = sofia_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            sofia_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            sofia_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            sofia_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<sofia_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        let auth = self.auth.clone();
        let state_db = sofia_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.sofia_home).await?;
        ...
        let auth_manager = sofia_core::test_support::auth_manager_from_auth_with_home(
            auth.clone(),
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| sofia_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            sofia_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    sofia_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    sofia_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = sofia_utils_cargo_bin::cargo_bin("sofia") {
            config.sofia_self_exe = Some(path);
        } else if let Ok(path) = sofia_utils_cargo_bin::cargo_bin("sofia-exec") {
            // `sofia-exec` also supports `--sofia-run-as-apply-patch`, so use it
        ...
        {
            let sofia = bin_dir.join("sofia");
            let sofia_exec = bin_dir.join("sofia-exec");
            if sofia.is_file() {
                config.sofia_self_exe = Some(sofia);
            } else if sofia_exec.is_file() {
                config.sofia_self_exe = Some(sofia_exec);
            }
        }
...
impl TestCodex {
    ...
    pub fn sofia_home_path(&self) -> &Path {
        self.config.sofia_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.sofia, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.sofia, |event| match event {
