...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let sofia_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-sofia-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-sofia-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-sofia-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-sofia-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-sofia-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_codex().build(&server).await.expect("build test sofia");
    ...
    let initial_header = initial_request
        ...
        .expect("x-sofia-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-sofia-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-sofia-turn-metadata"
    );
    ...
    let body_metadata: serde_json::Value = serde_json::from_str(
        initial_request.body_json()["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
            .expect("request body should include x-sofia-turn-metadata"),
    )
    .expect("body x-sofia-turn-metadata should be valid JSON");
    assert_eq!(
        body_metadata
            .get("sandbox_mode")
            .and_then(serde_json::Value::as_str),
...
async fn responses_stream_redacts_git_remote_credentials_from_turn_metadata() {
    ...
    let body_metadata = request_body["client_metadata"]["x-sofia-turn-metadata"]
        .as_str()
