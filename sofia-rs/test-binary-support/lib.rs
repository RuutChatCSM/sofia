    ...
    F: FnOnce(&str, Option<&str>) -> TestBinaryDispatchMode,
{
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let sofia_home = match tempfile::Builder::new().prefix(sofia_home_prefix).tempdir() {
                Ok(sofia_home) => sofia_home,
                Err(error) => panic!("failed to create test SOFIA_HOME: {error}"),
            };
            let previous_codex_home = std::env::var_os("SOFIA_HOME");
