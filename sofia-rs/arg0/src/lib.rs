...
use sofia_apply_patch::SOFIA_CORE_APPLY_PATCH_ARG1;
use sofia_async_utils::THREAD_STACK_SIZE_BYTES;
#[cfg(unix)]
use sofia_exec_server::SOFIA_ARG0_EXEC_HELPER_ARG1;
use sofia_exec_server::SOFIA_FS_HELPER_ARG1;
use sofia_install_context::InstallContext;
use sofia_sandboxing::landlock::SOFIA_LINUX_SANDBOX_ARG0;
use sofia_utils_home_dir::find_codex_home;
#[cfg(target_os = "windows")]
use sofia_windows_sandbox::SOFIA_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "sofia-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Sofia executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            sofia_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == SOFIA_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        sofia_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        sofia_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    #[cfg(unix)]
    if argv1 == SOFIA_ARG0_EXEC_HELPER_ARG1 {
        sofia_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == SOFIA_FS_HELPER_ARG1 {
        sofia_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == SOFIA_WINDOWS_SANDBOX_ARG1 {
        sofia_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == SOFIA_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match sofia_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                let update_file_mode = sofia_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(sofia_apply_patch::apply_patch_with_options(
                    &patch_arg,
                    sofia_apply_patch::ApplyPatchOptions {
                        update_file_mode,
                        ..Default::default()
                    },
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    sofia_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {SOFIA_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Sofia will proceed successfully even if
...
/// While we want to deploy the Sofia CLI as a single executable for simplicity,
...
/// `sofia-linux-sandbox` we *directly* execute
/// [`sofia_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.sofia/.env` before creating any threads.
...
///     `sofia-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        sofia_self_exe: current_exe.clone(),
        sofia_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `sofia-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().sofia_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "SOFIA_";

/// Load env vars from ~/.sofia/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `SOFIA_`.
fn load_dotenv() {
    if let Ok(sofia_home) = find_codex_home()
        && let Ok(iter) = dotenvy::from_path_iter(sofia_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `SOFIA_` keys.
...
///   with the hidden `--sofia-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Sofia
/// CLI.
...
fn prepare_path_entry_for_codex_aliases(
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if sofia_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (sofia_home: {sofia_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a SOFIA_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = sofia_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {SOFIA_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        sofia_self_exe: std::env::current_exe().ok(),
        sofia_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(SOFIA_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("sofia-path");
    ...
    fn windows_batch_alias_preserves_unicode_executable_paths() -> anyhow::Result<()> {
        ...
        let alias_directory = profile.join(".sofia").join("tmp").join("arg0");
    ...
    fn windows_batch_alias_preserves_cross_volume_executable_paths() {
        assert_eq!(
            super::windows_batch_executable_path(
                Path::new(r"D:\Tools\sofia.exe"),
                Path::new(r"C:\Users\person\.sofia\tmp\arg0"),
            ),
            r"D:\Tools\sofia.exe",
        );
    }
    ...
    fn linux_sandbox_exe_path_prefers_codex_linux_sandbox_alias() -> std::io::Result<()> {
        ...
        let alias_path = temp_dir.path().join("sofia-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                sofia_self_exe: Some(PathBuf::from("/usr/bin/sofia")),
                sofia_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/sofia"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("sofia-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/sofia")),
            |paths| async move {
