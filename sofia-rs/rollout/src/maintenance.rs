...
//! existing rollout path. They must not do that at the same time for one Sofia home, so they share
...
/// Try to exclude rollout compression and migration for one Sofia home.
pub fn try_acquire_rollout_maintenance_lock(
    sofia_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = sofia_home.join(".tmp");
