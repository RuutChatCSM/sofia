...
pub async fn append_thread_name(
    ...
    append_session_index_entry(sofia_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(sofia_home);
...
fn session_index_path(sofia_home: &Path) -> PathBuf {
    sofia_home.join(SESSION_INDEX_FILE)
}
