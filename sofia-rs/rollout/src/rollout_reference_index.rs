...
impl RolloutReferenceIndex {
    ...
    pub async fn scan(sofia_home: &Path) -> io::Result<Self> {
        Self::scan_paths(
            vec![
    ...
    pub async fn scan_unarchived(sofia_home: &Path) -> io::Result<Self> {
        Self::scan_paths(
            vec![sofia_home.join(SESSIONS_SUBDIR)],
            /*thread_ids*/ None,
        )
        .await
    }
    ...
    pub async fn scan_unarchived_threads(
        ...
        Self::scan_paths(vec![sofia_home.join(SESSIONS_SUBDIR)], Some(&thread_ids)).await
    }
