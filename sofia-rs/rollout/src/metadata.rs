...
use sofia_state::BackfillState;
use sofia_state::BackfillStats;
use sofia_state::BackfillStatus;
use sofia_state::DB_ERROR_METRIC;
use sofia_state::DB_METRIC_BACKFILL;
use sofia_state::DB_METRIC_BACKFILL_DURATION_MS;
use sofia_state::ExtractionOutcome;
use sofia_state::ThreadMetadataBuilder;
use sofia_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = sofia_otel::global();
    ...
    let sessions_root = sofia_home.join(SESSIONS_SUBDIR);
    let archived_root = sofia_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(sofia_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(sofia_home: &Path, path: &Path) -> String {
    path.strip_prefix(sofia_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
