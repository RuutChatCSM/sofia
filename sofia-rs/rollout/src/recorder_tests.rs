...
fn test_config(sofia_home: &Path) -> RolloutConfig {
    RolloutConfig {
        sofia_home: sofia_home.to_path_buf(),
        sqlite: sofia_state::SqliteConfig::new_for_testing(sofia_home.abs()),
        cwd: sofia_home.to_path_buf(),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        sofia_state::BackfillStatus::Complete
    );
...
-> std::io::Result<()> {
    ...
    let valid_created_at = chrono::Utc
        ...
        .single()
        .expect("valid datetime");
    let mut valid_builder = sofia_state::ThreadMetadataBuilder::new(
        valid_thread_id,
        valid_path.clone(),
        valid_created_at,
        SessionSource::Cli,
...
fn fill_missing_thread_item_metadata_preserves_identity_and_prefers_state_git_fields() {
    ...
    let state_item = ThreadItem {
        ...
        section: Some(sofia_state::ThreadSection {
