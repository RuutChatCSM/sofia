...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let sofia_home = dir.path().to_path_buf();
    ...
    let runtime = sofia_state::StateRuntime::init(
        sofia_state::SqliteConfig::new_for_testing(sofia_home.as_path().abs()),
        "test-provider".to_string(),
    )
    .await
    .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(sofia_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), sofia_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let sofia_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = sofia_home.join(".");
    let rollout_path = write_rollout_in_sessions_with_cwd(
        sofia_home.as_path(),
        "2026-01-27T12-34-56",
        "2026-01-27T12:34:56Z",
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = sofia_home.join("sessions");
