...
impl ConnectionDriver {
    ...
    pub(super) fn handle_host_message(&mut self, message: HostToClient) -> bool {
        if self.should_defer_host_message(&message) {
            if self.deferred_host_messages.len()
                >= sofia_code_mode_protocol::host::MAX_PENDING_DELEGATE_CALLS
            {
                self.fail(
        ...
        match message {
            HostToClient::DelegateRequest {
                ...
                let cell_id = match request {
                    sofia_code_mode_protocol::host::DelegateRequest::InvokeTool { invocation } => {
                        &invocation.cell_id
                    }
                    sofia_code_mode_protocol::host::DelegateRequest::Notify { cell_id, .. } => {
                        cell_id
                    }
                };
    ...
    fn complete_initial_response(
        &mut self,
        id: RequestId,
        result: Result<sofia_code_mode_protocol::host::WireRuntimeResponse, String>,
    ) -> bool {
