...
async fn removes_banned_allow_rules_once() {
    ...
    let policy_path = sofia_home.path().join("rules/default.rules");
...
"#,
    ...
    prefix_rule_migration(sofia_home.path(), &policy_path, BANNED_PREFIXES)
        .await
...
"#
    ...
    assert_eq!(
        std::fs::read_to_string(sofia_home.path().join(MIGRATION_MARKER_FILENAME))
