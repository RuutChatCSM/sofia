...
- From the Sofia CLI, run `sofia execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
sofia execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
sofia execpolicy check \
...
```bash
cargo run -p sofia-execpolicy -- check --rules path/to/policy.rules git status
```
