...
fn packaged_runtime_uses_manifest_version() {
    ...
    let executable = bin_dir.join("sofia");
    fs::write(&executable, b"").expect("create runtime binary");
    fs::write(
        package.path().join("sofia-package.json"),
    ...
    assert_eq!(
        ...
        BuildInfo {
            ...
            target: Some(env!("SOFIA_BUILD_TARGET").to_string()),
        },
    );
}
...
fn legacy_package_without_version_uses_build_commit() {
    ...
    let executable = bin_dir.join("sofia");
    fs::write(&executable, b"").expect("create runtime binary");
    fs::write(package.path().join("sofia-package.json"), "{}")
