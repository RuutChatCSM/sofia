...
use anyhow::Result;
use anyhow::ensure;
use sofia_realtime_webrtc::AudioState;
use sofia_realtime_webrtc::Message;
use sofia_realtime_webrtc::encode_frame;
use sofia_realtime_webrtc::read_message;
use std::fs;
use std::io::Write;
...
const APP: &str = if cfg!(windows) { "sofia.exe" } else { "sofia" };
const HELPER: &str = if cfg!(windows) {
    "sofia-voice-host.exe"
} else {
    "sofia-voice-host"
};
...
pub fn package(test: &str) -> Result<Option<PathBuf>> {
    ...
    if source.file_name().is_some_and(|name| name == APP) {
        sofia_build_info::BuildInfo::initialize(BUILD_COMMIT);
        return Ok(Some(
            source
                .parent()
                .context("bin")?
    ...
    fs::create_dir_all(root.join("sofia-resources/voice/bin"))?;
    fs::write(root.join("sofia-package.json"), "{}")?;
    fs::copy(&source, root.join("bin").join(APP))?;
    fs::copy(&source, root.join("sofia-resources/voice/bin").join(HELPER))?;
