...
fn managed_ca_dir() -> Result<PathBuf> {
    let sofia_home =
        find_codex_home().context("failed to resolve SOFIA_HOME for managed MITM CA")?;
    Ok(sofia_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Sofia-generated MITM CA bundle.
