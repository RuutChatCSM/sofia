...
mod tests {
        ...
        MitmHookConfig {
            ...
            actions: MitmHookActionsConfig {
                ...
                inject_request_headers: vec![InjectedHeaderConfig {
                    ...
                    secret_env_var: Some("SOFIA_GITHUB_TOKEN".to_string()),
                    secret_file: None,
    ...
    fn compile_resolves_env_backed_injected_headers() {
        ...
        let hooks = compile_mitm_hooks_with_resolvers(
            &config,
            |name| (name == "SOFIA_GITHUB_TOKEN").then(|| "ghp-secret".to_string()),
        ...
        assert_eq!(
            ...
            SecretSource::EnvVar("SOFIA_GITHUB_TOKEN".to_string())
        );
        assert_eq!(
    ...
    fn evaluate_matches_wildcard_path_query_and_header_constraints() {
        let mut config = base_config();
        let mut hook = github_hook();
        hook.matcher.path_prefixes = vec!["pattern:/repos/*/sofia/issues*".to_string()];
        hook.matcher.query =
    ...
    fn evaluate_path_wildcard_does_not_cross_segment_boundaries() {
        ...
        let nested_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/private/sofia/issues")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_rejects_paths_that_upstream_may_normalize() {
        ...
        let paths = [
            "/openai/openai/../sofia",
            "/openai/openai/%2e%2e/sofia",
            "/openai/openai/%2E%2E/sofia",
            "/openai/openai/.%2e/sofia",
            "/openai/openai/%2e./sofia",
            "/openai/openai/%252e%252e/sofia",
    ...
    fn evaluate_treats_glob_metacharacters_as_literal_without_glob_prefix() {
        ...
        let exact_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/[draft]/sofia/issues?state=op*")
            .header("x-github-api-version", "2022-11-28[preview]")
            .body(Body::empty())
            .unwrap();
        let non_literal_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/draft/sofia/issues?state=open")
            .header("x-github-api-version", "2022-11-28-preview")
            .body(Body::empty())
            .unwrap();
