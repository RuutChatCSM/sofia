...
#[command(name = "sofia-network-proxy", about = "Sofia network sandbox proxy")]
pub struct Args {}
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "SOFIA_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "SOFIA_NETWORK_ALLOW_LOCAL_BINDING";
...
pub fn is_managed_proxy_env_var(key: &str, value: &str) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        key == PROXY_GIT_SSH_COMMAND_ENV_KEY
            && value.starts_with(SOFIA_PROXY_GIT_SSH_COMMAND_MARKER)
    }
...
#[cfg(target_os = "macos")]
pub const SOFIA_PROXY_GIT_SSH_COMMAND_MARKER: &str = "SOFIA_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const SOFIA_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "SOFIA_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const SOFIA_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn sofia_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{SOFIA_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{SOFIA_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_codex_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(SOFIA_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(SOFIA_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected Sofia fallback so it cannot point
        ...
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            ...
            _ => {
                env.insert(
                    ...
                    sofia_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"SOFIA_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
        #[cfg(not(target_os = "macos"))]
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/sofia-proxy/ca-bundle.pem");
    ...
    fn apply_proxy_env_overrides_refreshes_previous_codex_proxy_git_ssh_command() {
        ...
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            sofia_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&sofia_proxy_git_ssh_command(SocketAddr::new(
