...
`sofia-network-proxy` is Sofia's local network policy enforcement proxy. It runs:
...
`sofia-network-proxy` reads from Sofia's merged `config.toml` (via `sofia-core` config loading).
...
# bundle env vars pointing at immutable public files under $SOFIA_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p sofia-network-proxy --
```
...
`sofia-network-proxy` can be embedded as a library with a thin API:

```rust
use sofia_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `sofia-network-proxy` is embedded in managed Sofia runtime, policy decisions emit structured
OTEL-compatible events with `target=sofia_otel.network_proxy`.
...
This section documents the protections implemented by `sofia-network-proxy`, and the boundaries of
