...
use sofia_protocol::auth::KnownPlan as InternalKnownPlan;
use sofia_protocol::auth::PlanType as InternalPlanType;
...
use sofia_protocol::shell_environment::OPENAI_FEDERATION_RULE_ID_ENV_VAR;
use sofia_protocol::shell_environment::OPENAI_IDENTITY_TOKEN_FILE_ENV_VAR;
use std::sync::Arc;
...
async fn agent_identity_jwt_uses_explicit_staging_endpoint_overrides() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/api/sofia/agent-identities/jwks"))
        .respond_with(ResponseTemplate::new(200).set_body_json(test_jwks_body()))
        .expect(1)
        .mount(&jwks_server)
        .await;
    ...
    let _authapi_guard =
        EnvVarGuard::set("SOFIA_AGENT_IDENTITY_AUTHAPI_BASE_URL", &authapi_base_url);
    let jwks_base_url = format!("{}/api/sofia/", jwks_server.uri());
    let _jwks_guard = EnvVarGuard::set("SOFIA_AGENT_IDENTITY_JWKS_BASE_URL", &jwks_base_url);
...
fn agent_identity_authapi_override_preserves_chatgpt_environment_validation() {
    let _authapi_guard = EnvVarGuard::set(
        "SOFIA_AGENT_IDENTITY_AUTHAPI_BASE_URL",
        "https://authapi.example/api/accounts",
    );
    let _jwks_guard = EnvVarGuard::set(
        "SOFIA_AGENT_IDENTITY_JWKS_BASE_URL",
        "https://jwks.example/api/sofia",
    );
...
fn agent_identity_custom_jwks_base_requires_explicit_authapi_override() {
    let _authapi_guard = EnvVarGuard::remove("SOFIA_AGENT_IDENTITY_AUTHAPI_BASE_URL");
    let jwks_base_url = "https://jwks.example/api/sofia";
    let _jwks_guard = EnvVarGuard::set("SOFIA_AGENT_IDENTITY_JWKS_BASE_URL", jwks_base_url);
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let storage = FileAuthStorage::new(sofia_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(sofia_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("SOFIA_AUTHAPI_BASE_URL", &server.uri());
    let allowed_workspaces = [WORKSPACE_ID_ALLOWED.to_string()];
    super::login_with_access_token(
        dir.path(),
        "at-login-test",
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(sofia_home.path().to_path_buf());
    let auth_path = get_auth_file(sofia_home.path());
...
async fn workload_identity_auth_is_immutable_and_process_local() {
    ...
    assert!(!get_auth_file(sofia_home.path()).exists());
...
fn write_auth_file(params: AuthFileParams, sofia_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(sofia_home);
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(SOFIA_ACCESS_TOKEN_ENV_VAR)
}
...
fn test_auth_manager_config(sofia_home: &Path) -> TestAuthManagerConfig {
...
fn auth_config_from_preserves_all_fields() {
    let sofia_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(sofia_home.path());
...
async fn shared_from_config_prefers_workload_identity_to_explicit_access_token() {
    let sofia_home = tempdir().expect("tempdir");
    let config = test_auth_manager_config(sofia_home.path());
    let _access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, "at-explicit");
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    assert!(
        !get_auth_file(sofia_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("SOFIA_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("SOFIA_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            sofia_home.path().to_path_buf(),
            /*enable_codex_api_key_env*/ false,
...
async fn load_auth_keeps_codex_api_key_env_precedence() {
    ...
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(SOFIA_API_KEY_ENV_VAR, "sk-env");
...
async fn api_only_policy_rejects_access_tokens_before_hydration() {
    ...
    let server = MockServer::start().await;
    let _authapi_guard = EnvVarGuard::set("SOFIA_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, "at-rejected");
    let mut config = build_config(
        sofia_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_rejects_agent_identity_before_hydration() {
    ...
    let access_token_guard = EnvVarGuard::set(SOFIA_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let mut config = build_config(
        sofia_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
...
async fn workspace_policy_checks_the_selected_request_account() {
    ...
    let auth_path = sofia_home.path().join("auth.json");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(SOFIA_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let sofia_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        sofia_agent_identity::generate_agent_key_material().expect("generate agent key material");
