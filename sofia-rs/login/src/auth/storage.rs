...
use sofia_agent_identity::AgentIdentityJwtClaims;
use sofia_agent_identity::decode_agent_identity_jwt;
use sofia_config::types::AuthCredentialsStoreMode;
pub use sofia_config::types::AuthKeyringBackendKind;
use sofia_keyring_store::DefaultKeyringStore;
use sofia_keyring_store::KeyringStore;
use sofia_protocol::account::PlanType as AccountPlanType;
use sofia_protocol::auth::AuthMode;
use sofia_secrets::LocalSecretsNamespace;
use sofia_secrets::SecretName;
use sofia_secrets::SecretScope;
use sofia_secrets::SecretsBackendKind;
use sofia_secrets::SecretsManager;

/// Expected structure for $SOFIA_HOME/auth.json.
...
pub(super) fn get_auth_file(sofia_home: &Path) -> PathBuf {
    sofia_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(sofia_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(sofia_home: PathBuf) -> Self {
        Self { sofia_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `SOFIA_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.sofia_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.sofia_home)
    }
}

static SOFIA_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("SOFIA_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("SOFIA_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Sofia Auth";

// turns sofia_home path into a stable, short key string
fn compute_store_key(sofia_home: &Path) -> std::io::Result<String> {
    let canonical = sofia_home
        .canonicalize()
        .unwrap_or_else(|_| sofia_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(sofia_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            sofia_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.sofia_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.sofia_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.sofia_home)?;
...
impl Debug for SecretsKeyringAuthStorage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SecretsKeyringAuthStorage")
            .field("sofia_home", &self.sofia_home)
...
impl SecretsKeyringAuthStorage {
    fn new(sofia_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(sofia_home.clone(), Arc::clone(&keyring_store));
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &SOFIA_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &SOFIA_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &SOFIA_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(sofia_home)),
        }
    }
}
...
// A global in-memory store for mapping sofia_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(sofia_home: PathBuf) -> Self {
        Self { sofia_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(sofia_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(sofia_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(sofia_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(sofia_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(sofia_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(sofia_home, keyring_store))
        }
    }
}
