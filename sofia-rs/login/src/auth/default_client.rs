//! Default Sofia HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`sofia_login::default_client`] from other crates in this
//! workspace.

use http::HeaderMap;
use http::HeaderValue;
use http::header::USER_AGENT;
use sofia_http_client::BuildRouteAwareHttpClientError;
...
pub use sofia_http_client::RequestBuilder as CodexRequestBuilder;
...
/// Parenthesis will be added by Sofia. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "sofia_cli_rs";
pub const SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "SOFIA_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-sofia-residency";

pub use sofia_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "sofia-tui"
        || originator_value == "sofia_vscode"
        || originator_value.starts_with("Sofia ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "sofia_atlas" || originator_value == "sofia_chatgpt_desktop"
}
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Sofia user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Sofia user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Sofia originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Sofia's route-aware system/PAC resolution.
...
/// Builds the default Sofia HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Sofia
...
/// Builds the default Sofia HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Sofia default headers.
...
/// Builds the default Sofia HTTP client wrapper for an auth endpoint.
