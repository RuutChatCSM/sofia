...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("sofia-tui"), true);
    assert_eq!(is_first_party_originator("sofia_vscode"), true);
    assert_eq!(is_first_party_originator("Sofia Something Else"), true);
    assert_eq!(is_first_party_originator("sofia_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("sofia_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("sofia_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("sofia_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Sofia UA for that originator
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "sofia_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "sofia_cli_rs/0.0.0 (bad_suffix)"
    );
}
