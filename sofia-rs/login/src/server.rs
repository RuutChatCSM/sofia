...
use sofia_utils_template::Template;
...
// Keep in sync with the Sofia CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub sofia_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Sofia failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "sofia_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("sofia_cli_simplified_flow".to_string(), "true".to_string()),
...
fn redact_sensitive_error_url(
    mut err: sofia_http_client::HttpError,
) -> sofia_http_client::HttpError {
...
/// Returns true when the OAuth callback represents a missing Sofia entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "Sofia is not enabled for your workspace. Contact your workspace administrator to request access to Sofia.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codex_entitlement_error(code, error_description) {
            (
                "You do not have access to Sofia".to_string(),
                "This account is not currently authorized to use Sofia in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Sofia.".to_string(),
                "Contact your workspace administrator to get access to Sofia, then return to Sofia and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Sofia to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(body.contains("You do not have access to Sofia"));
