...
use sofia_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = sofia_home.clone();

    let opts = ServerOptions {
        sofia_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: sofia_login::test_support::transport_default_auth_route_config(),
        client_id: sofia_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = sofia_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/sofia/open-app?source=old")?,
...
async fn creates_missing_codex_home_dir() -> Result<()> {
    ...
    let tmp = tempdir()?;
    let sofia_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Sofia"),
        "error body should clearly explain the Sofia access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Sofia to retry"),
    ...
    assert!(
        !body.contains("get access to Sofia"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not sofia"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        sofia_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
