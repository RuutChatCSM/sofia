...
/// The easiest way to iterate is to add unit tests and have Sofia fix the implementation.
...
/// Tests are at the top to encourage using TDD + Sofia to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd sofia-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/sofia && rg -n "sofia_api" sofia-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n sofia_api sofia-rs -S".to_string(),
                query: Some("sofia_api".to_string()),
                path: Some("sofia-rs".to_string()),
            }],
        );
    }
