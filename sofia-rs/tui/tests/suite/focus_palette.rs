...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(sofia_home.path(), &repo_root)?;

    let mut terminal = PtyCodex::start(&repo_root, sofia_home, &[])?;
...
async fn interactive_startup_honors_codex_home_symlink_opt_out() -> Result<()> {
    ...
    let visualizations = sofia_home.path().join("visualizations");
    std::os::unix::fs::symlink(target.path(), &visualizations)?;
    write_test_config(sofia_home.path(), &workspace_path)?;
...
impl PtyCodex {
    pub(super) fn start(
        ...
        let sofia = sofia_utils_cargo_bin::cargo_bin("sofia-tui")
            .or_else(|_| sofia_utils_cargo_bin::cargo_bin("sofia"))?;
        let child = Command::new(sofia)
            .args(extra_args)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            ...
            .stderr(slave)
            .spawn()
            .context("start Sofia in focus-test pseudo-terminal")?;
    ...
    pub(super) fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Sofia exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Sofia did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
    ...
    pub(super) fn wait_for_screen(&mut self, text: &str) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if let Some(status) = self.child.try_wait()? {
                bail!("Sofia exited while waiting for {text:?} ({status})");
            }
        }
    ...
    pub(super) fn ensure_running(&mut self) -> Result<()> {
            ...
            "Sofia exited unexpectedly; screen:\n{}",
            self.screen_contents()
        );
        Ok(())
    }
...
pub(super) fn write_test_config(sofia_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(sofia_home.join("config.toml"), config)
        .context("write focus-test Sofia configuration")?;
    std::fs::write(
