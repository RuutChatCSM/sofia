...
use sofia_utils_path as path_utils;
use tokio::sync::mpsc;
...
async fn run_resume_picker_with_launch_context(
    ...
    let options = SessionPickerRunOptions {
        ...
        local_filter_cwd,
        worktrees_enabled: config.features.enabled(sofia_features::Feature::Worktrees),
        ...
        view_persistence: Some(SessionPickerViewPersistence {
            sofia_home: local_settings.sofia_home.to_path_buf(),
        }),
...
impl PickerState {
    ...
    fn row_matches_local_cwd(&self, row: &Row) -> bool {
        ...
        paths_match(row_cwd, filter_cwd)
            || (self.worktrees_enabled
                && sofia_git_utils::repository_identity(row_cwd)
                    .zip(sofia_git_utils::repository_identity(filter_cwd))
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.sofia_home)
...
pub(crate) fn repository_cwd_filter(
    ...
    if worktrees_enabled
        && !uses_remote_filesystem
        && let Some(cwds) = sofia_git_utils::linked_worktree_cwds(cwd)
        && cwds.len() > 1
    {
...
mod tests {
    ...
    async fn worktrees_filtering_requires_feature() {
        ...
        let mut config = crate::legacy_core::config::ConfigBuilder::default()
            .sofia_home(root.path().to_path_buf())
            .build()
            .await
            .unwrap();
        config
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/sofia-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/sofia")),
            git_branch: Some(String::from("sofia/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      sofia/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/sofia", " main");
        assert_metadata_order(&created[0], "⌁ tmp/sofia", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/sofia"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/sofia", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/sofia.etraut-animations-false-improvements/sofia-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/sofia.owner-extremely-long-worktree-name-that-needs-truncating/sofia-rs";
        ...
        assert!(footer.contains("⌁ ~/code/sofia."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/sofia.owner-worktree/sofia-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let sofia_home_file = tmp.path().join("sofia-home-file");
        std::fs::write(&sofia_home_file, "not a directory").expect("write sofia home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/sofia.fcoury-session-picker/sofia-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/sofia")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
        ...
        state.transcript_previews.insert(
            ...
            TranscriptPreviewState::Loaded(vec![
                ...
                TranscriptPreviewLine {
                    ...
                    text: String::from(
                        r#"Here are the *last* lines: [docs](https://example.com) :sofia-file-citation{path="/tmp/sofia/report.xlsx"}."#,
                    ),
                },
            ]),
        );
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![sofia_app_server_protocol::Turn {
