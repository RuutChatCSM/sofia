        ...
        for source_kinds in source_kind_filters {
            ...
            loop {
                ...
                for thread in response.data {
                    ...
                    {
                        let expected_root = sofia_home.join(match collection {
                            SessionCollection::Active => sofia_rollout::SESSIONS_SUBDIR,
                            SessionCollection::Archived => sofia_rollout::ARCHIVED_SESSIONS_SUBDIR,
                        });
                        if !path.starts_with(expected_root)
                            || (thread.history_mode == ThreadHistoryMode::Legacy
                                && sofia_rollout::existing_rollout_path(path).await.is_none())
                        {
                            continue;
                        }
                    }
