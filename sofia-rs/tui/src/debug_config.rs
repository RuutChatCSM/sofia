...
use sofia_config::NetworkUnixSocketPermissionToml;
use sofia_config::RequirementSource;
use sofia_config::ResidencyRequirement;
use sofia_config::SandboxModeRequirement;
use sofia_config::WebSearchModeRequirement;
use sofia_config::format_config_layer_source;
...
mod tests {
    ...
    use sofia_config::ConstraintError;
    use sofia_config::FeatureRequirementsToml;
    use sofia_config::FilesystemConstraints;
    use sofia_config::HookEventsToml;
    use sofia_config::HookHandlerConfig;
    use sofia_config::LoaderOverrides;
    use sofia_config::ManagedHooksRequirementsToml;
    use sofia_config::MatcherGroup;
    use sofia_config::McpServerIdentity;
    use sofia_config::McpServerRequirement;
    use sofia_config::NetworkConstraints;
    use sofia_config::NetworkDomainPermissionToml;
    use sofia_config::NetworkDomainPermissionsToml;
    use sofia_config::NetworkHeaderInjectionToml;
    use sofia_config::NetworkUnixSocketPermissionToml;
    use sofia_config::NetworkUnixSocketPermissionsToml;
    use sofia_config::RequirementSource;
    use sofia_config::ResidencyRequirement;
    use sofia_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\sofia\\config.toml")
        } else {
            absolute_path("/etc/sofia/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.sofia")
        } else {
            absolute_path("/repo/.sofia")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Sofia\\requirements.toml")
        } else {
            absolute_path("/etc/sofia/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.sofia\\state")
        } else {
            absolute_path("/home/alice/.sofia/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.sofia\\logs")
        } else {
            absolute_path("/home/alice/.sofia/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.sofia\\models.json")
        } else {
            absolute_path("/home/alice/.sofia/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.sofia\\config.toml")
        } else {
            absolute_path("/home/alice/.sofia/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/sofia.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\sofia")
        } else {
            absolute_path("/var/lib/sofia")
        };
