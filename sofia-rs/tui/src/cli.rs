...
use clap::Parser;
use sofia_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `sofia resume` subcommand.
    // These are not exposed as user flags on the base `sofia` command.
    #[clap(skip)]
    ...
    /// top-level `sofia resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `sofia fork` subcommand.
    // These are not exposed as user flags on the base `sofia` command.
    #[clap(skip)]
    ...
    /// top-level `sofia fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
