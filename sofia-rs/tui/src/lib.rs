...
// The standalone `sofia-tui` binary prints a short help message before the
...
use sofia_app_server_client::RemoteAppServerClient;
use sofia_app_server_client::RemoteAppServerConnectArgs;
pub use sofia_app_server_client::RemoteAppServerEndpoint;
use sofia_app_server_protocol::Account as AppServerAccount;
...
use sofia_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
pub(crate) use sofia_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "sofia-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: sofia_feedback::CodexFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = sofia_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(sofia_home: &Path) {
    ...
    let _ = std::fs::remove_file(sofia_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let sofia_home = find_codex_home().wrap_err("failed to resolve SOFIA_HOME")?;
            sofia_app_server_client::app_server_control_socket_path(&sofia_home)
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        endpoint,
        client_name: "sofia-tui".to_string(),
...
async fn maybe_probe_default_daemon_socket(sofia_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = sofia_app_server_client::app_server_control_socket_path(sofia_home).ok()?;
    #[cfg(windows)]
    let (validated_path, _directory) =
        sofia_uds::validate_private_socket_path(socket_path.as_path()).ok()?;
    #[cfg(windows)]
    ...
    match tokio::time::timeout(
        ...
        sofia_uds::UnixStream::connect(probe_path),
    )
    .await
    {
        Ok(Ok(_stream)) => {
    ...
    ThreadListParams {
        ...
        cwd: cwd_filter.map(|cwd| {
            resume_picker::repository_cwd_filter(
                cwd,
                uses_remote_filesystem,
                config.features.enabled(sofia_features::Feature::Worktrees),
            )
        }),
...
async fn cloud_config_bundle_for_app_server_target(
    ...
    cloud_config_bundle_loader_for_storage(
        app_server_target
            .auth_config_for_cloud_loader(bootstrap_auth_config(sofia_home, bootstrap_config)?),
        /*enable_codex_api_key_env*/ false,
    )
    .await
}
...
async fn run_ratatui_app(
    ...
    let app_server_session = match startup_app_server {
        Ok(Ok(app_server)) => {
            AppServerSession::new(app_server, app_server_target.thread_params_mode())
                .with_local_codex_home(&initial_config.sofia_home)
        }
        Ok(Err(err)) => {
            terminal_restore_guard.restore_silently();
            session_log::log_session_end();
         ...
         terminal_restore_guard: &mut TerminalRestoreGuard| {
            ...
            Ok(AppExitInfo {
                ...
                exit_reason: ExitReason::Fatal(format!(
                    "No saved session found with ID {id_str}. Run `sofia {action}` without an ID to choose from existing sessions."
                )),
            })
        };
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.sofia_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        sofia_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
"#
    ...
    async fn bedrock_setup_wizard_requires_eligible_onboarding() -> color_eyre::Result<()> {
        ...
        for (label, config_toml, login_status, target, expected) in [
            ...
            std::fs::write(sofia_home.path().join("config.toml"), config_toml)?;
    ...
    pub(crate) fn write_session_rollout(
        ...
        let session_meta = sofia_protocol::protocol::SessionMeta {
            ...
            source: sofia_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(sofia_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_codex_home(sofia_home.path()).await?;
            ...
            let cloud_config_bundle =
                sofia_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: sofia_app_server_client::app_server_control_socket_path(&sofia_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://sofia.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("sofia.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("sofia.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let sofia_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(sofia_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let sofia_home = TempDir::new()?;
        let socket_path =
            sofia_app_server_client::app_server_control_socket_path(sofia_home.path())?;
        #[cfg(windows)]
        {
            let parent = socket_path.as_path().parent().expect("socket parent");
            std::fs::create_dir_all(parent)?;
            let listener = sofia_uds::UnixListener::bind(socket_path.as_path()).await?;
            assert!(
                maybe_probe_default_daemon_socket(sofia_home.path())
                    .await
                    .is_none()
        ...
        sofia_uds::prepare_private_socket_directory(
            socket_path.as_path().parent().expect("socket parent"),
        )
        .await?;
        let _listener = sofia_uds::UnixListener::bind(socket_path.as_path()).await?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(sofia_home.path()).await,
            expected
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("sofia.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            sofia_app_server_client::AppServerClient::InProcess(
        ...
        config
            .features
            .set_enabled(sofia_features::Feature::Worktrees, /*enabled*/ true)?;
        let filter_cwd = latest_session_cwd_filter(
            /*uses_remote_workspace*/ false, /*remote_cwd_override*/ None, &config,
            /*show_all*/ false,
        );
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            sofia_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = sofia_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            sofia_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let sofia_home = temp_dir.path().to_path_buf();
