    ...
    let service_tier_edit = service_tier.map_or_else(
        ...
        |service_tier| {
            let config_value = if service_tier == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                ...
                match sofia_protocol::config_types::ServiceTier::from_request_value(service_tier) {
                    Some(sofia_protocol::config_types::ServiceTier::Fast) => "fast",
                    Some(sofia_protocol::config_types::ServiceTier::Flex) => "flex",
                    None => service_tier,
                }
            };
    ...
    let trust_target = disabled_reason
        ...
        .or_else(|| {
            disabled_project
                ...
                .and_then(|path| {
                    path.strip_suffix("/.sofia")
                        .or_else(|| path.strip_suffix("\\.sofia"))
                })
        })
        .unwrap_or(&cwd);
