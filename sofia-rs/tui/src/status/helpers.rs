...
mod tests {
    ...
    async fn test_config(sofia_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let sofia_home = TempDir::new().expect("temp sofia home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = sofia_home.path().join("global.md");
        let config = test_config(&sofia_home, &cwd).await;
    ...
    async fn compose_agents_summary_collapses_home_and_preserves_project_relative_paths() {
        ...
        let sofia_home = TempDir::new().expect("temp sofia home");
        let cwd = TempDir::new().expect("temp cwd");
        let mut config = test_config(&sofia_home, &cwd).await;
        config.cwd = home.join("workspace").join("project").abs();

        let paths = [
            home.join(".sofia").join("AGENTS.md"),
        ...
        insta::assert_snapshot!(
            summary.replace('\\', "/"),
            @"~/.sofia/AGENTS.md, ../AGENTS.md, AGENTS.md, nested/AGENTS.md"
        );
    }
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let sofia_home = TempDir::new().expect("temp sofia home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = sofia_home.path().join("override.md");
        let config = test_config(&sofia_home, &cwd).await;
