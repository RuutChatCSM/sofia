...
impl StatusHistoryHandle {
    ...
    pub(crate) fn set_thread_usage(
        &self,
        estimate: Option<sofia_app_server_protocol::ThreadUsage>,
    ) {
...
impl StatusHistoryCell {
    fn content_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from(" ").dim(),
            Span::from(format!("(v{SOFIA_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run sofia login to use ChatGPT)".to_string()
            }
        });
