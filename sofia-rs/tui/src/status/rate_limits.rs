...
use chrono::Local;
use chrono::Utc;
use sofia_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use sofia_app_server_protocol::RateLimitSnapshot;
use sofia_app_server_protocol::RateLimitWindow;
use sofia_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use sofia_protocol::num_format::format_with_separators;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `sofia` or `sofia_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "sofia".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    ordered_snapshots.sort_by_key(|snapshot| !snapshot.limit_name.eq_ignore_ascii_case("sofia"));
    for snapshot in ordered_snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("sofia");
...
mod tests {
    ...
    fn non_codex_single_limit_renders_combined_row() {
        let now = Local::now();
        let sofia = RateLimitSnapshotDisplay {
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "sofia-other".to_string(),
            normal_model_slug: None,
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[sofia, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "sofia-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
