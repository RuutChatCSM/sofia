...
pub(super) async fn fetch_account_rate_limits(
    ...
    if matches!(
        &result,
        Err(sofia_app_server_client::TypedRequestError::Server { source, .. })
...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<sofia_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<sofia_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<sofia_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: sofia_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    } else if err.contains("sofia plugins are disabled")
        || err.contains("plugin sharing is disabled")
        || err.contains("plugin sharing is not enabled")
        || err.contains("feature disabled")
    {
        "Ask a workspace admin to enable Sofia plugins or plugin sharing."
    ...
    } else if err.contains("old build") || err.contains("update sofia") || err.contains("stale") {
        "Update Sofia, then try opening the shared plugin again."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this Sofia session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![sofia_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, sofia_protocol::mcp::Tool>,
    HashMap<String, Vec<sofia_protocol::mcp::Resource>>,
    HashMap<String, Vec<sofia_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                sofia_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
                sofia_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                sofia_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                sofia_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                sofia_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
    ...
    async fn fetch_thread_usage_uses_app_server_auth_after_persisted_account_changes() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("POST"))
            .and(wiremock::matchers::path(
                "/api/sofia/usage/thread_usage/query",
            ))
