            ...
            AppEvent::BrowseManagedWorktrees => {
                if let Some(request) = self.chat_widget.request_managed_worktrees() {
                    crate::worktree_browser::fetch(
                        request,
                        self.config.sofia_home.to_path_buf(),
...
&self.local_settings,
                    ...
                    if self.chat_widget.thread_id() == Some(thread_id)
                        ...
                        && let Err(error) = app_server
                            .thread_goal_set(
                                thread_id,
                                /*objective*/ None,
                                Some(sofia_app_server_protocol::ThreadGoalStatus::Paused),
                                /*token_budget*/ None,
                            )
                            .await
                    {
            ...
            AppEvent::ConnectProvider {
                ...
                let existing_key = sofia_model_provider_info::api_key_from_auth_file(
            ...
            AppEvent::FinalizeProviderSetup {
                ...
                let effort_level = match effort.as_str() {
                    "low" => sofia_protocol::openai_models::ReasoningEffort::Low,
                    "medium" => sofia_protocol::openai_models::ReasoningEffort::Medium,
                    "high" => sofia_protocol::openai_models::ReasoningEffort::High,
                    "max" => sofia_protocol::openai_models::ReasoningEffort::Max,
                    _ => sofia_protocol::openai_models::ReasoningEffort::Medium,
                };
                ...
                self.model_catalog.add_model(
                    sofia_protocol::openai_models::ModelPreset {
                        id: model_slug.clone(),
                        ...
                        input_modalities: vec![
                            sofia_protocol::openai_models::InputModality::Text,
                        ],
                    },
                );
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt {
                ...
                self.session_telemetry.counter(
                    "sofia.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "sofia.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let sofia_home = self.config.sofia_home.clone();
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "sofia.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                        ...
                        Ok(_) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Sofia can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.sofia_home.clone();
            ...
            AppEvent::AgentsDaemonStarted { result } => match result {
                Ok(()) => self.chat_widget.add_info_message(
                    "Background server started. Run `sofia agents` in another terminal; this session remains unchanged."
                        .to_string(),
                    /*hint*/ None,
                ),
                Err(error) => self
                ...
                match apply_result {
                    Ok(()) => {
                        ...
                        if let Some(theme) = crate::render::highlight::resolve_theme_by_name(
                            &name,
                            Some(&self.local_settings.sofia_home),
                        ) {
