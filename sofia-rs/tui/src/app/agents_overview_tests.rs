...
use sofia_app_server_protocol::CurrentTimeReadParams;
use sofia_app_server_protocol::ReasoningSummaryTextDeltaNotification;
...
use sofia_config::types::KeybindingSpec;
use sofia_config::types::KeybindingsSpec;
...
async fn overview_composer_preserves_editing_and_routes_focus() {
    ...
    view.handle_paste(format!(
        "x{}",
        " ".repeat(sofia_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS)
    ));
    view.handle_key_event(KeyCode::Enter.into());
...
async fn shared_overview_keeps_rows_and_replays_changes_over_stale_reads() -> Result<()> {
    ...
    for event in [
        AppServerEvent::ServerNotification(Box::new(ServerNotification::ThreadReverted(
            sofia_app_server_protocol::ThreadRevertedNotification {
                thread_id: retained.to_string(),
            },
        ))),
...
async fn shared_overview_seeds_once_and_retains_locally_resumed_history() -> Result<()> {
    ...
    for day in 1..=22 {
        let source = match day {
            3 => sofia_protocol::protocol::SessionSource::Custom("atlas".to_string()),
            4 => sofia_protocol::protocol::SessionSource::Custom("chatgpt".to_string()),
            5 => sofia_protocol::protocol::SessionSource::Exec,
            6 => sofia_protocol::protocol::SessionSource::Mcp,
            _ => sofia_protocol::protocol::SessionSource::Cli,
        };
    ...
    app_test_support::create_fake_rollout_with_source(
        ...
        /*git_info*/ None,
        sofia_protocol::protocol::SessionSource::Cli,
    )
...
async fn overview_selection_applies_user_permissions_only_to_unloaded_threads() -> Result<()> {
    ...
    server_config
        .permissions
        .approval_policy
        .set(sofia_protocol::protocol::AskForApproval::Never)?;
    let mut thread_ids = Vec::new();
    for day in 1..=5 {
        thread_ids.push(ThreadId::from_string(
            &app_test_support::create_fake_rollout(
                &server_config.sofia_home,
                &format!("2025-01-{day:02}T12-00-00"),
                &format!("2025-01-{day:02}T12:00:00Z"),
                "Historical session",
    ...
    let loaded = app_server
        ...
        .await?
        .session;
    app.harness_overrides.sandbox_mode = Some(sofia_protocol::config_types::SandboxMode::ReadOnly);
    app.harness_overrides.approval_policy =
        Some(sofia_protocol::protocol::AskForApproval::UnlessTrusted);
    app.config
        .permissions
    ...
    for (thread_id, expected_permissions, expected_approval) in [
        ...
        (
            ...
            sofia_app_server_protocol::AskForApproval::UnlessTrusted,
        ),
        (
            thread_ids[2],
            PermissionProfile::read_only(),
        ...
        if thread_id == thread_ids[2] {
            ...
            app.harness_overrides.sandbox_mode =
                Some(sofia_protocol::config_types::SandboxMode::WorkspaceWrite);
            app.harness_overrides.approval_policy =
                Some(sofia_protocol::protocol::AskForApproval::Never);
            app.runtime_permission_profile_override =
                Some(RuntimePermissionProfileOverride::from_config(&app.config));
            app.runtime_approval_policy_override = Some(RuntimeApprovalPolicyOverride::Explicit(
                AskForApproval::UnlessTrusted,
    ...
    let requirements = app.config.sofia_home.join("requirements.toml");
    std::fs::write(
        &requirements,
    ...
    assert_eq!(
        ...
        (
            Some(thread_ids[3]),
            sofia_app_server_protocol::ThreadStatus::NotLoaded
        ),
    );
    app_server.shutdown().await?;
    Ok(())
...
async fn overview_cold_resume_honors_working_directory_selection() -> Result<()> {
    for (mode, cli_cwd, runtime_cwd) in [
        ...
        let mut app = make_test_app().await;
        let chosen = app.config.sofia_home.join("chosen");
        let overridden = app.config.sofia_home.join("overridden");
...
async fn restored_server_permission_profile_survives_cd_without_turn_override() -> Result<()> {
    let mut app = make_test_app().await;
    let destination = app.config.sofia_home.join("destination");
        ...
        if expected_states.len() < targets.len() {
            ...
            app.chat_widget.handle_server_notification(
                ServerNotification::TurnStarted(
                    sofia_app_server_protocol::TurnStartedNotification {
                        ...
                        turn: sofia_app_server_protocol::Turn {
                            ...
                            items: Vec::new(),
                            status: sofia_app_server_protocol::TurnStatus::InProgress,
                            error: None,
                            started_at: None,
                            completed_at: None,
                            duration_ms: None,
...
async fn command_center_handles_resume_failure_and_success() -> Result<()> {
    ...
    let path = app_test_support::rollout_path(app.config.sofia_home.as_path(), timestamp, &id);
    let view = app.agents_overview_view(Vec::new(), /*selected_thread_id*/ None);
    app.chat_widget.show_bottom_pane_view(Box::new(view));
    let mut tui = crate::tui::test_support::make_test_tui()?;
    let mut server = Box::pin(crate::start_embedded_app_server_for_picker(&app.config)).await?;
