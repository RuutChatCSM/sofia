...
impl App {
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Sofia could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("sofia-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<sofia_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    async fn resume_model_settings_preserves_only_explicit_model_overrides() {
        ...
        let profile = "work"
            .parse::<sofia_config::ProfileV2Name>()
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let sofia_home = tempdir()?;
        let required_policy = sofia_protocol::protocol::AskForApproval::Never;
...
"#
    ...
    -> Result<()> {
        ...
        let sofia_home = tempdir()?;
        app.config.sofia_home = sofia_home.path().to_path_buf().abs();
        std::fs::write(sofia_home.path().join("config.toml"), "[broken")?;
