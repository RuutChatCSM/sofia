...
impl App {
            ...
            self.chat_widget.show_selection_view(SelectionViewParams {
                ...
                items: [
                    ...
                    (!workload_identity_selected).then(|| SelectionItem {
                        ...
                        description: Some(
                            "Open `sofia agents` in another terminal afterward.".to_string(),
                        ),
    ...
    pub(super) async fn select_agents_overview_thread(
        ...
        if self.primary_thread_id != Some(root_thread_id) {
            ...
            let resume_model_settings = match target_thread.status {
                ...
                sofia_app_server_protocol::ThreadStatus::Idle
                | sofia_app_server_protocol::ThreadStatus::Active { .. }
                | sofia_app_server_protocol::ThreadStatus::SystemError => {
    ...
    pub(super) async fn replay_agents_overview_requests(
        ...
        for request in requests {
            self.handle_app_server_event(
                app_server,
                sofia_app_server_client::AppServerEvent::ServerRequest(Box::new(request)),
            )
            .await;
        }
    }
    ...
    pub(super) async fn dispatch_agents_overview_task(
            ...
            && config.features.enabled(Feature::FastMode)
        {
            ...
            let models = self.model_catalog.models.lock().unwrap();
            config.model = models
                .iter()
        ...
        tokio::spawn(async move {
            let result = async {
                ...
                let executable = if current_executable
                    ...
                    .is_some_and(|name| {
                        if cfg!(windows) {
                            name.eq_ignore_ascii_case("sofia-tui")
                        } else {
                            name == "sofia-tui"
                        }
                    }) {
