...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `sofia` to start a saved session, or `sofia resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let sofia_home = app_server.sofia_home_path(&self.config.sofia_home);
        match goal_files::objective_text_for_edit(app_server, sofia_home.as_ref(), &goal.objective)
            .await
        {
