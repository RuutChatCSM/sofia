...
async fn lost_mutation_reply_preserves_work_without_resubmitting() -> Result<()> {
    ...
    let history = tokio::time::timeout(Duration::from_secs(/*secs*/ 5), async {
        loop {
            if let Ok(text) =
                tokio::fs::read_to_string(app.config.sofia_home.join("history.jsonl")).await
                && let Ok(entry) =
                    serde_json::from_str::<sofia_message_history::HistoryEntry>(&text)
            {
                break entry;
            }
    ...
    let mut keymap = sofia_config::types::TuiKeymap::default();
    keymap.chat.edit_queued_message = Some(sofia_config::types::KeybindingsSpec::One(
        sofia_config::types::KeybindingSpec("ctrl-x up".into()),
    ));
    let runtime =
