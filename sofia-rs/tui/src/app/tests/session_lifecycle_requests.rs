...
use sofia_app_server_protocol::ClientNotification;
...
use sofia_protocol::items::AgentMessageContent;
use sofia_protocol::items::AgentMessageItem;
use sofia_protocol::items::EnteredReviewModeItem;
use sofia_protocol::items::ExitedReviewModeItem;
...
use sofia_protocol::user_input::UserInput as CoreUserInput;
use sofia_state::SqliteConfig;
use std::sync::Mutex;
...
async fn same_thread_retry_keeps_subscription_and_restores_draft() -> Result<()> {
    let (mut app, sofia_home) = make_history_test_app().await?;
...
pub(super) async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        sofia_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        loader_overrides,
        /*strict_config*/ false,
        sofia_config::CloudConfigBundleLoader::default(),
        sofia_feedback::CodexFeedback::new(),
        /*log_db*/ None,
        state_db,
        Arc::new(sofia_exec_server::EnvironmentManager::default_for_tests()),
    )
    .await?;
    let sofia_home = config.sofia_home.display().to_string();
                        ...
                        .send(Message::Text(
                            serde_json::to_string(&JSONRPCMessage::Response(JSONRPCResponse {
                                ...
                                result: serde_json::json!({
                                    "userAgent": "sofia-tui-test",
                                    "codexHome": sofia_home,
                                }),
                            }))?
                            .into(),
                        ))
    ...
    Ok((
        AppServerSession::new(app_server, thread_params_mode)
            .with_local_codex_home(&config.sofia_home),
        requests,
        proxy,
    ))
}
...
async fn make_history_test_app() -> Result<(App, tempfile::TempDir)> {
    ...
    app.config.sqlite = SqliteConfig::new_for_testing(sofia_home.path().abs());
    Ok((app, sofia_home))
}
...
async fn external_transport_registers_dynamic_tools_and_finds_task_mentions() -> Result<()> {
    ...
    for params in starts {
        ...
        assert_eq!(params["dynamicTools"][0]["name"], "sofia_tui");
        assert_eq!(
...
async fn local_daemon_registers_approval_gated_mcp_tools_for_both_start_paths() -> Result<()> {
    ...
    app.config
        .web_search_mode
        .set(sofia_protocol::config_types::WebSearchMode::Live)?;
    std::fs::write(
    ...
    let inventory: sofia_app_server_protocol::ListMcpServerStatusResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::McpServerStatusList {
            request_id: AppServerRequestId::String("tui-tool-inventory".to_string()),
            params: sofia_app_server_protocol::ListMcpServerStatusParams {
                cursor: None,
                limit: None,
                detail: Some(sofia_app_server_protocol::McpServerStatusDetail::ToolsAndAuthOnly),
    ...
    let tools = &inventory
        .data
        .iter()
        .find(|server| server.name == "sofia_tui")
    ...
    for params in &starts {
        ...
        let server = &params["config"]["mcp_servers.sofia_tui"];
        assert!(
            server["url"]
                .as_str()
    ...
    let mcp_url = starts[0]["config"]["mcp_servers.sofia_tui"]["url"]
        .as_str()
        .expect("MCP server URL");
    let unauthorized = sofia_http_client::HttpClientBuilder::new()
        .build_direct()?
        .post(mcp_url)
        .send()
        .await?;
    ...
    app.config
        .web_search_mode
        .set(sofia_protocol::config_types::WebSearchMode::Disabled)?;
    ...
    assert_eq!(
        resumed["config"]["mcp_servers.sofia_tui"],
        starts[0]["config"]["mcp_servers.sofia_tui"]
    );
    app_server
        .resume_thread(
            &app.local_settings,
    ...
    assert_eq!(
        reattached["config"]["mcp_servers.sofia_tui"],
        starts[0]["config"]["mcp_servers.sofia_tui"]
    );
    app_server
    ...
    assert_eq!(
        forked["config"]["mcp_servers.sofia_tui"],
        starts[0]["config"]["mcp_servers.sofia_tui"]
    );
    let authorization =
        starts[0]["config"]["mcp_servers.sofia_tui"]["http_headers"]["Authorization"]
            .as_str()
            .expect("MCP bearer token");
    let client = sofia_http_client::HttpClientBuilder::new().build_direct()?;
    ...
    assert_eq!(
        child["config"]["mcp_servers.sofia_tui"],
        starts[0]["config"]["mcp_servers.sofia_tui"]
    );
...
async fn local_mcp_respects_configured_servers_and_managed_requirements() -> Result<()> {
    for scenario in ["conflicting", "blocked", "mismatched", "allowed"] {
        ...
        if scenario == "conflicting" {
            let raw = serde_json::from_value::<sofia_config::RawMcpServerConfig>(
            ...
            servers.insert(
                ...
                sofia_config::McpServerConfig::try_from(raw)
            ...
            if matches!(scenario, "mismatched" | "allowed") {
                let requirement = if scenario == "allowed" {
                    sofia_config::McpServerRequirement::Url(
                        sofia_protocol::mcp_policy::McpServerValueMatcher::Prefix {
                            value: "http://127.0.0.1:".to_string(),
                        },
                    )
                } else {
                    sofia_config::McpServerRequirement::Identity {
                        identity: sofia_config::McpServerIdentity::Url {
            ...
            let requirements = sofia_config::ConfigRequirements {
                mcp_servers: Some(sofia_config::Sourced::new(
                    allowed_servers,
                    sofia_config::RequirementSource::Unknown,
                )),
                ..Default::default()
            };
            app.config.config_layer_stack = sofia_config::ConfigLayerStack::new(
                Vec::new(),
                requirements,
        ...
        if scenario == "allowed" {
            ...
            assert!(start["config"]["mcp_servers.sofia_tui"].is_object());
        } else {
            assert_eq!(
                start["dynamicTools"][0]["tools"].as_array().map(Vec::len),
                Some(6)
            );
            assert!(start["config"]["mcp_servers.sofia_tui"].is_null());
        }
        app_server.shutdown().await?;
        proxy.await??;
    }
...
async fn embedded_server_rejects_unowned_dynamic_tool_calls() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        sofia_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: sofia_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("sofia_app".to_string()),
...
async fn dynamic_tool_requests_ignore_other_namespaces_and_dispatch_tui_namespace() -> Result<()> {
    ...
    for namespace in [Some("sofia_app"), None] {
        app.handle_app_server_event(
            &app_server,
            sofia_app_server_client::AppServerEvent::ServerRequest(Box::new(
                ServerRequest::DynamicToolCall {
        ...
        sofia_app_server_client::AppServerEvent::ServerRequest(Box::new(
            ServerRequest::DynamicToolCall {
                ...
                params: sofia_app_server_protocol::DynamicToolCallParams {
                    ...
                    namespace: Some("sofia_tui".to_string()),
                    tool: "list_threads".to_string(),
                    arguments: serde_json::json!({}),
                },
            },
    ...
    app_server
        ...
        .await?;
    let project: sofia_app_server_protocol::ProjectCreateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ProjectCreate {
            request_id: AppServerRequestId::String("create-source-project".to_string()),
            params: sofia_app_server_protocol::ProjectCreateParams {
                name: "Source project".to_string(),
                roots: vec![sofia_app_server_protocol::ProjectRoot {
        ...
        .await?;
    let _: sofia_app_server_protocol::ThreadMetadataUpdateResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadMetadataUpdate {
            request_id: AppServerRequestId::String("assign-source-project".to_string()),
            params: sofia_app_server_protocol::ThreadMetadataUpdateParams {
        ...
        .await?;
    let source_settings: sofia_app_server_protocol::ThreadResumeResponse = app_server
        .request_handle()
        .request_typed(ClientRequest::ThreadResume {
            request_id: AppServerRequestId::String("read-source-sandbox".to_string()),
            params: sofia_app_server_protocol::ThreadResumeParams {
                thread_id: creation_source.to_string(),
                ..sofia_app_server_protocol::ThreadResumeParams::default()
            },
        })
        .await?;
    ...
    spawn_approved_task_tool_call(
        ...
        sofia_app_server_protocol::DynamicToolCallParams {
            thread_id: creation_source.to_string(),
            turn_id: "turn-1".to_string(),
...
async fn transcript_home_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = sofia_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    ...
    let response = crate::dynamic_tools::execute(
        ...
        sofia_app_server_protocol::ThreadStartParams::default(),
        status_updates,
        /*app_event_tx*/ None,
    )
    .await;
                ...
                app.handle_app_server_event(
                    ...
                    AppServerEvent::ServerNotification(Box::new(
                        ServerNotification::ThreadStatusChanged(
                            sofia_app_server_protocol::ThreadStatusChangedNotification {
                                thread_id: started.session.thread_id.to_string(),
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, sofia_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
...
async fn managed_worktree_transitions_bind_owner_and_preserve_only_fork_history() -> Result<()> {
    ...
    fs::create_dir_all(source.join(".sofia"))?;
    fs::write(home.join("config.toml"), "[features]\nworktrees = true\n")?;
    crate::legacy_core::config::set_project_trust_level(
        &home,
        &source,
        sofia_protocol::config_types::TrustLevel::Trusted,
    )
...
"#,
    ...
    fs::write(source.join(".sofia/config.toml"), destination)?;
...
"#,
    ...
    app.config.sofia_home = home.clone().abs();
    ...
    let manager = sofia_worktree::WorktreeManager::new(
        sofia_worktree::WorktreeSettings::for_cli(&home, /*desktop*/ None)
            .map_err(|error| color_eyre::eyre::eyre!(error.to_string()))?,
    );
...
"#,
    ...
    for mode in [ManagedWorktreeMode::New, ManagedWorktreeMode::Fork] {
        ...
        let metadata = sofia_rollout::read_session_meta_line(&rollout).await?;
    ...
    let unowned = manager
        .create(&sofia_worktree::CreateWorktree {
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use serde_json::json;
    use sofia_protocol::config_types::TrustLevel as T;
    use std::fs;
    ...
    let [current, trusted, unknown, untrusted, mismatch, failed] =
        names.map(|name| sofia_home.path().join(name));
    fs::create_dir_all(&current)?;
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".sofia"))?;
        fs::write(directory.join(".sofia/config.toml"), "")?;
    }
    let contents = "developer_instructions = \"destination policy\"\nmodel_reasoning_effort = \"high\"\napproval_policy = \"on-request\"\n[tui]\ntheme = \"dracula\"\n[tui.keymap.global]\nopen_transcript = \"f12\"";
    fs::write(trusted.join(".sofia/config.toml"), contents)?;
    ...
    fs::write(trusted.join(".sofia/hooks.json"), hooks)?;
    let contents = "default_permissions = \"dev\"\n[permissions.dev.filesystem]\n\":root\" = \"write\"\n[tui.keymap.global]\nopen_transcript = \"ctrl-l\"";
    fs::write(mismatch.join(".sofia/config.toml"), contents)?;
    let requirements = sofia_home.path().join("requirements.toml");
    ...
    for dir in [&trusted, &untrusted, &mismatch, &failed] {
        ...
        crate::legacy_core::config::set_project_trust_level(sofia_home.path(), dir, trust)
            .map_err(|error| color_eyre::eyre::eyre!(error.to_string()))?;
    }
    ...
    let meta = sofia_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = sofia_utils_path_uri::PathUri::from_abs_path(&agents.abs());
