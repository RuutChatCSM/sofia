...
async fn luna_reserve_periodic_refresh_adapts_without_an_experiment_banner() -> Result<()> {
    ...
    app.config.sqlite = sofia_state::SqliteConfig::new_for_testing(home.path().abs());
    ...
    for (used, seconds) in [(10, 60), (75, 30), (90, 15), (99, 5), (20, 60)] {
        ...
        let usage = requests
            .iter()
            .find(|request| request.url.path() == "/api/sofia/usage")
            .unwrap();
        assert!(usage.headers.contains_key("x-openai-sofia-luna-reserve"));
...
-> Result<()> {
    ...
    app.chat_widget.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                sofia_error_info: Some(CodexErrorInfo::UsageLimitExceeded),
    ...
    rolling.rate_limit_reached_type =
        Some(sofia_app_server_protocol::RateLimitReachedType::WorkspaceMemberCreditsDepleted);
    app.handle_app_server_event(
        &session,
        sofia_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountRateLimitsUpdated(
                sofia_app_server_protocol::AccountRateLimitsUpdatedNotification {
    ...
    app.handle_app_server_event(
        ...
        sofia_app_server_client::AppServerEvent::ServerNotification(Box::new(
            ServerNotification::AccountUpdated(
                sofia_app_server_protocol::AccountUpdatedNotification {
                    auth_mode: None,
                    plan_type: None,
                },
            ),
...
async fn backend_banner_reset_redemption_rejects_pre_reset_content() -> Result<()> {
    ...
    app.handle_event(
        ...
        AppEvent::RateLimitResetCreditConsumed {
            ...
            credit_id: None,
            result: Ok(
                sofia_app_server_protocol::ConsumeAccountRateLimitResetCreditResponse {
                    outcome:
                        sofia_app_server_protocol::ConsumeAccountRateLimitResetCreditOutcome::Reset,
                },
            ),
        },
    )
    ...
    recovered.rate_limit_reset_credits =
        Some(sofia_app_server_protocol::RateLimitResetCreditsSummary {
            available_count: 0,
            credits: None,
        });
    app.handle_event(
