...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        sofia_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        sofia_app_server_protocol::McpServerElicitationRequest::UserVerification { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                thread_id, params.server_name.clone(), request_id.clone(),
                                sofia_app_server_protocol::McpServerElicitationAction::Cancel,
                                /*content*/ None, /*meta*/ None,
                            );
                            None
                        }
                        sofia_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | sofia_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm { .. }
                        | sofia_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                thread_id,
                                params.server_name.clone(),
                                request_id.clone(),
                                sofia_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = sofia_message_history::HistoryConfig::new(
            self.local_settings.sofia_home.clone(),
            &self.local_settings.history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                sofia_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                sofia_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
    ...
    pub(super) async fn lookup_message_history_batch(
        &mut self,
        thread_id: ThreadId,
        cursor: sofia_message_history::HistoryBatchCursor,
        log_id: u64,
    ) -> Result<()> {
        let history_config = sofia_message_history::HistoryConfig::new(
            self.local_settings.sofia_home.clone(),
        ...
        tokio::spawn(async move {
            let event = match tokio::task::spawn_blocking(move || {
                sofia_message_history::lookup_batch(log_id, cursor, &history_config)
            })
            .await
            {
                Ok(Ok(batch)) => {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(sofia_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
        ...
        let misalignment_policy_violation =
            match &notification {
                ServerNotification::Error(notification) if !notification.will_retry => {
                    notification.error.sofia_error_info.as_ref()
                }
                ServerNotification::TurnCompleted(notification) => notification
                    .turn
                    .error
                    .as_ref()
                    .and_then(|error| error.sofia_error_info.as_ref()),
                _ => None,
    ...
    fn startup_request_may_open_protected_view(&self, request: &ServerRequest) -> bool {
        ...
        match &params.request {
            ...
            sofia_app_server_protocol::McpServerElicitationRequest::Form { .. } => true,
            sofia_app_server_protocol::McpServerElicitationRequest::OpenAiForm { .. }
            | sofia_app_server_protocol::McpServerElicitationRequest::OpenAiElicitationForm {
                ..
            } => false,
            ...
            Some(
                content
                    ...
                    .filter_map(|item| match item {
                        sofia_app_server_protocol::UserInput::Text { text, .. } => {
