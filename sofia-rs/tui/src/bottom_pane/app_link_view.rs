...
const MCP_CODEX_APPS_SERVER_NAME: &str = "sofia_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &sofia_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let sofia_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> sofia_app_server_protocol::McpServerElicitationRequest {
        sofia_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            url: url.to_string(),
            elicitation_id: "sofia_apps_auth_call_123".to_string(),
        }
    }

    #[test]
    fn sofia_apps_auth_url_elicitation_builds_auth_app_link_params() {
    ...
    fn non_codex_apps_url_elicitation_builds_generic_app_link_params() {
        ...
        let request = sofia_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    #[test]
    fn sofia_apps_auth_url_elicitation_rejects_untrusted_urls() {
        let target = suggestion_target();
        for url in [
