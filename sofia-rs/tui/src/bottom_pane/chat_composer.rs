...
//!
//! For the detailed burst state machine, see `sofia-rs/tui/src/bottom_pane/paste_burst.rs`.
//!
...
use sofia_message_history::HistoryBatchCursor;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name = sofia_connectors::metadata::connector_display_label(connector);
...
mod tests {
    ...
    use crate::bottom_pane::textarea::TextArea;
    use sofia_protocol::models::local_image_label_text;
    ...
    pub(super) fn new_test_composer() -> (ChatComposer, UnboundedReceiver<AppEvent>) {
        ...
        (
            ChatComposer::new(
                ...
                "Ask Sofia to do anything".to_string(),
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_status_line(Some(Line::from(
                    "gpt-5.4 high fast · ~/code/sofia-1 · Context 0% used",
                )));
    ...
    fn set_skill_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_skill_mentions(Some(vec![SkillMetadata {
            ...
            description: "Primary personal Sofia repo skill.".to_string(),
            short_description: None,
            interface: None,
            dependencies: None,
        ...
        assert_eq!(mention.insert_text, "$sofia".to_string());
    ...
    fn default_unified_mention_popup_snapshot() {
        snapshot_composer_state(
            ...
            /*enhanced_keys_supported*/ false,
            |composer| {
                let features = sofia_features::Features::with_defaults();
                composer
                    .set_mentions_v2_enabled(features.enabled(sofia_features::Feature::MentionsV2));
                composer.set_skill_mentions(Some(vec![
                    SkillMetadata {
...
程序员 与 Unicode 同行";
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: sofia_file_search::MatchType::File,
    ...
    fn pasting_filepath_attaches_image() {
        let tmp = tempdir().expect("create TempDir");
        let tmp_path: PathBuf = tmp.path().join("sofia_tui_test_paste_image.png");
