...
use sofia_message_history::HistoryConfig;
...
async fn lifecycle_metadata_uses_local_prompt_history() -> Result<()> {
    ...
    let local_config = ConfigBuilder::default()
        .sofia_home(local_home.path().to_path_buf())
        .build()
        .await?;
    let local_settings = LocalSettings::from(&local_config);
    let history_config =
        HistoryConfig::new(local_settings.sofia_home.clone(), &local_settings.history);
    sofia_message_history::append_entry("local prompt", ThreadId::new(), &history_config).await?;
    ...
    for mode in [ThreadParamsMode::Embedded, ThreadParamsMode::Remote] {
        ...
        let mut config = ConfigBuilder::default()
            .sofia_home(server_home.path().to_path_buf())
            .build()
            .await?;
