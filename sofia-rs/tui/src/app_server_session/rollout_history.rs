...
impl AppServerSession {
    ...
    pub(crate) fn with_local_codex_home(mut self, sofia_home: &AbsolutePathBuf) -> Self {
        self.task_tool_capabilities_dir = (!self.uses_embedded_app_server())
            .then(|| sofia_home.join("tui-thread-reference-capabilities"));
        self
    }
                ...
                && (!self.uses_embedded_app_server()
                    || ({
                        ...
                        rollout_maintenance_guard =
                            sofia_rollout::try_acquire_rollout_maintenance_lock(
