...
async fn remote_resume_restores_saved_server_profile_without_permission_overrides() -> Result<()> {
    ...
    server
        .thread_settings_update(sofia_app_server_protocol::ThreadSettingsUpdateParams {
            thread_id: thread_id.to_string(),
            permissions: Some("server-only".into()),
            approval_policy: Some(sofia_app_server_protocol::AskForApproval::Never),
            approvals_reviewer: Some(sofia_app_server_protocol::ApprovalsReviewer::AutoReview),
            ..Default::default()
        })
        .await?;
    tokio::time::timeout(std::time::Duration::from_secs(5), async {
        loop {
            if let Some(sofia_app_server_client::AppServerEvent::ServerNotification(notification)) =
                server.next_event().await
                && let sofia_app_server_protocol::ServerNotification::ThreadSettingsUpdated(
                    settings,
                ) = *notification
    ...
    assert_eq!(
        ...
        sofia_protocol::config_types::ApprovalsReviewer::AutoReview
    );
    ...
    let explicit_config = ConfigBuilder::default()
        .sofia_home(home.path().to_path_buf())
        .harness_overrides(crate::legacy_core::config::ConfigOverrides {
            sandbox_mode: Some(sofia_protocol::config_types::SandboxMode::ReadOnly),
            approval_policy: Some(sofia_protocol::protocol::AskForApproval::OnRequest),
            ..Default::default()
        })
        .build()
        .await?;
    ...
    assert_eq!(
        ...
        sofia_app_server_protocol::AskForApproval::OnRequest
    );
    assert_eq!(
        explicit_fork.session.permission_profile,
        sofia_protocol::models::PermissionProfile::read_only()
    );
    server.shutdown().await?;
    Ok(())
}
...
async fn cached_legacy_resume_revalidates_history_across_migration_settings() -> Result<()> {
        ...
        [(false, false), (false, true), (true, false), (true, true)]
    {
        ...
        let maintenance_guard =
            sofia_rollout::try_acquire_rollout_maintenance_lock(sofia_home.path())?
        ...
        let legacy = {
            ...
            assert!(
                sofia_rollout::try_acquire_rollout_maintenance_lock(sofia_home.path())?.is_none()
            );
            resume.await?
        };
        assert_eq!(app_server.next_request_id, next_request_id + 2);
