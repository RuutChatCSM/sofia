...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<sofia_protocol::mcp::CallToolResult, String>,
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    ...
    docs_line.push_span(
        "MCP docs".underlined(),
        Some("https://developers.openai.com/sofia/mcp"),
    );
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, sofia_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        let (label, style) = match status.runtime_status {
            ...
            None if matches!(
                status.auth_status,
                sofia_app_server_protocol::McpAuthStatus::NotLoggedIn
            ) =>
            {
                (
                    "authentication required",
