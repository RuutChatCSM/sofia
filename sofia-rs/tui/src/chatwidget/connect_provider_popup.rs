...
/// The credential variable name sofia derives for a provider: `<ID>_API_KEY`
...
fn providers_config_path() -> String {
    format!(
        "{}/providers.json",
        sofia_utils_home_dir::sofia_home_string()
    )
}

/// Providers config path under an explicit home (used by callers that must not
/// depend on the process-global `SOFIA_HOME`, such as the model catalog).
pub(crate) fn providers_config_path_in(sofia_home: &std::path::Path) -> std::path::PathBuf {
    sofia_home.join("providers.json")
}
...
/// Load `providers.json` from an explicit sofia home.
pub(crate) fn load_providers_config_in(sofia_home: &std::path::Path) -> ProvidersConfig {
    std::fs::read_to_string(providers_config_path_in(sofia_home))
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default()
}
...
pub(crate) fn cached_catalog_models(
    ...
    let cache_path = sofia_home.join("models_dev_cache.json");
...
impl ChatWidget {
    ...
    pub(crate) fn fetch_models_for_provider(&mut self, provider_id: String, provider_name: String) {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let api_key =
                sofia_model_provider_info::api_key_from_auth_file(&provider_env_key(&provider_id));
