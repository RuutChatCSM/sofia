...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        sofia_error_info: &AppServerCodexErrorInfo,
    ) -> bool {
        matches!(
            sofia_error_info,
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Sofia is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.sofia_rate_limit_reached_type.map(|kind| {
        ...
        if self.sofia_rate_limit_reached_type != rate_limit_reached_type {
            self.clear_backend_banner();
        }
        self.sofia_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        sofia_error_info: Option<AppServerCodexErrorInfo>,
    ) {
        if sofia_error_info == Some(AppServerCodexErrorInfo::MisalignmentPolicyViolation) {
            self.on_misalignment_policy_violation();
        } else if sofia_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = sofia_error_info
            .as_ref()
