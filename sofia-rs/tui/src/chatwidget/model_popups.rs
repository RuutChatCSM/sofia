...
impl ChatWidget {
    ...
    pub(super) fn is_auto_model(model: &str) -> bool {
        model.starts_with("sofia-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "sofia-auto-fast" => 0,
            "sofia-auto-balanced" => 1,
            "sofia-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    fn open_all_models_popup_with_view_id(
        ...
        let header = self.model_menu_header(
            ...
            "Browse by provider. Type to search.",
        );
        self.show_model_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-sofia")
            || preset.model.starts_with("gpt-5.1-sofia-max")
