...
impl ChatWidget {
    ...
    pub(crate) fn rate_limit_refresh_interval(&self) -> Option<std::time::Duration> {
        ...
        let used_percent = self
            ...
            .filter(|(limit_id, snapshot)| {
                limit_id.as_str() == "sofia" || snapshot.limit_name == self.current_model()
            })
    ...
    pub(crate) fn on_rolling_rate_limit_snapshot(&mut self, snapshot: RateLimitSnapshot) {
        if let (Some(previous), Some(current)) = (
            self.sofia_rate_limit_reached_type,
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_codex_limit = limit_id.eq_ignore_ascii_case("sofia");
            if is_codex_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.sofia_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_codex_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.sofia_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.sofia_spend_control_reached == Some(true)
                || matches!(
                    self.sofia_rate_limit_reached_type,
                    Some(
            ...
            self.sofia_rate_limit_reached_type = None;
            self.sofia_spend_control_reached = None;
        }
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using sofia. Request increase?",
            ),
        };
