...
impl ChatWidget {
            ...
            && self.automatic_model_switch_state.reserve_return.is_none()
        {
            self.automatic_model_switch_state.reserve_return =
                self.thread_id().and_then(|thread_id| {
                    ReserveReturnModel::load(self.config.sofia_home.as_path(), thread_id).or_else(
                        || {
                            ...
                            previous
                                .save(self.config.sofia_home.as_path(), thread_id)
                                .ok()?;
                            Some(previous)
                        },
                    )
    ...
    pub(super) fn clear_reserve_return(&mut self) {
        if let Some(thread_id) = self.thread_id() {
            ReserveReturnModel::clear(self.config.sofia_home.as_path(), thread_id);
        }
    ...
    pub(crate) fn inherit_backend_banner_state(&mut self, previous: &mut ChatWidget) {
        ...
        self.sofia_rate_limit_reached_type = previous.sofia_rate_limit_reached_type;
        self.sofia_spend_control_reached = previous.sofia_spend_control_reached;
        self.backend_banner_state.presented = None;
        self.refresh_backend_banner_visibility();
    }
