...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![sofia_utils_path_uri::PathUri::from_abs_path(
...
async fn goal_slash_command_emits_attached_images() {
    ...
    let local_image = chat.config.sofia_home.join("goal-local.png");
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(sofia_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(sofia_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask Sofia to do anything")
    );
}
...
async fn slash_copy_picker_copies_status_fields_and_preserves_source_after_copying() {
    ...
    assert_chatwidget_snapshot!(
        ...
        render_bottom_popup(&chat, /*width*/ 100)
            ...
            .replace(crate::version::SOFIA_CLI_VERSION, "VERSION"),
    );
    chat.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(sofia_config::types::KeybindingsSpec::One(
        sofia_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            sofia_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: sofia_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: sofia_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/sofia-test-rollout.jsonl");
