...
pub(super) async fn test_config() -> Config {
    ...
    let sofia_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_codex_home(sofia_home.clone(), Vec::new())
            .await
            .expect("config");
    // Keep generic UI snapshots stable when the bundled catalog default changes.
    config.model = Some("gpt-5.6-sol".to_string());
    config.sofia_home = sofia_home.abs();
    config.sqlite = sofia_state::SqliteConfig::new_for_testing(sofia_home.as_path().abs());
    config.log_dir = sofia_home.join("log");
...
pub(super) fn test_model_catalog(_config: &Config) -> Arc<ModelCatalog> {
    Arc::new(
        ModelCatalog::new(crate::test_support::TEST_MODEL_PRESETS.clone())
            ...
            sofia_models_manager::collaboration_mode_presets::builtin_collaboration_mode_presets(),
        ),
    )
}
...
fn token_usage_breakdown(usage: TokenUsage) -> sofia_app_server_protocol::TokenUsageBreakdown {
    sofia_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    sofia_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: sofia_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    sofia_error_info: Option<CodexErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = sofia_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            sofia_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(sofia_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    let mut entries = vec![sofia_app_server_protocol::HookOutputEntry {
        kind: sofia_app_server_protocol::HookOutputEntryKind::Warning,
        text: "Heads up from the hook".to_string(),
    }];
    if event_name != sofia_app_server_protocol::HookEventName::Interrupt {
        entries.push(sofia_app_server_protocol::HookOutputEntry {
