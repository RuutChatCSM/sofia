...
async fn permission_discovery_uses_server_catalog_for_remote_custom_selection() {
    ...
    discovery
        .profiles
        .push(sofia_app_server_protocol::PermissionProfileSummary {
...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = sofia_config::ConfigRequirementsToml {
        windows: Some(sofia_config::WindowsRequirementsToml {
...
pub(super) fn requirements_stack(
    requirements_toml: sofia_config::ConfigRequirementsToml,
) -> ConfigLayerStack {
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(sofia_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Sofia agent sandbox"),
