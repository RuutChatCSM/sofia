...
impl ReserveReturnModel {
    fn path(sofia_home: &Path, thread_id: ThreadId) -> PathBuf {
        sofia_home
            .join("tui-luna-reserve")
            .join(format!("{thread_id}.json"))
    }

    pub(super) fn load(sofia_home: &Path, thread_id: ThreadId) -> Option<Self> {
        let file = std::fs::File::open(Self::path(sofia_home, thread_id)).ok()?;
        // A corrupt or unrelated cache file must not cause an unbounded read.
        serde_json::from_reader(file.take(/*limit*/ 4096)).ok()
    }

    pub(super) fn save(&self, sofia_home: &Path, thread_id: ThreadId) -> io::Result<()> {
        let path = Self::path(sofia_home, thread_id);
    ...
    pub(super) fn clear(sofia_home: &Path, thread_id: ThreadId) {
        // A missing cache needs no cleanup; the live model is authoritative after leaving Reserve.
        let _ = std::fs::remove_file(Self::path(sofia_home, thread_id));
    }
}
