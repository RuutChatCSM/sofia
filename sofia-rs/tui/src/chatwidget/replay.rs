    ...
    pub(crate) fn replay_thread_turns(&mut self, turns: Vec<Turn>, replay_kind: ReplayKind) {
        ...
        for (turn, hidden_nested_review_turn) in turns.into_iter().zip(hidden_nested_review_turns) {
            ...
            if turn.status == TurnStatus::Completed
                && turn.items_view == sofia_app_server_protocol::TurnItemsView::NotLoaded
                && turn.items.is_empty()
            {
                continue;
            }
            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: sofia_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            sofia_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        sofia_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: sofia_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            } => self.on_command_execution_started(item),
            item @ ThreadItem::CommandExecution {
                source: ExecCommandSource::Agent | ExecCommandSource::UnifiedExecStartup,
                status:
                    sofia_app_server_protocol::CommandExecutionStatus::Completed
                    | sofia_app_server_protocol::CommandExecutionStatus::Failed,
                ..
            ...
            ThreadItem::WebSearch(item) => {
                ...
                self.on_web_search_end(
                    ...
                    item.query,
                    item.action
                        .unwrap_or(sofia_app_server_protocol::WebSearchAction::Other),
                );
            }
            ...
            ThreadItem::FunctionCallOutput {
                    ...
                    )
                {
                    self.add_to_history(history_cell::PrefixedWrappedHistoryCell::new(
                        format!("Sent by Sofia from task {source_thread_id}\n{prompt}"),
                        "• ".dim(),
                        "  ",
                    ));
                }
