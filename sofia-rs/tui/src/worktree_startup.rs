...
pub(crate) struct ManagedTuiWorktree {
    manager: sofia_worktree::WorktreeManager,
    checkout: sofia_worktree::ManagedWorktree,
...
async fn latest_thread_cwd(path: Option<PathBuf>, fallback: PathBuf) -> PathBuf {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let sofia_rollout::ScanOutcome::Parsed(sofia_rollout::RolloutLine {
                item: sofia_rollout::RolloutItem::TurnContext(item),
                ..
            }) = outcome
            {
...
pub(super) async fn prepare(
    ...
    if let Some(id_or_name) = cli.fork_session_id.as_deref() {
        let prepared = if should_load_configured_environments(&loader_overrides, target) {
            EnvironmentManager::prepare_from_codex_home(&source.sofia_home).await
        } else {
        ...
        if cli.cwd.is_none() {
            ...
            .await;
            let source_bundle =
                cloud_config_bundle_for_app_server_target(target, &bootstrap, &source.sofia_home)
                    .await?;
    ...
    if !source.features.enabled(sofia_features::Feature::Worktrees) {
    ...
    let manager = sofia_worktree::WorktreeManager::new(
        sofia_worktree::WorktreeSettings::for_cli(
            &source.sofia_home,
    ...
    .await?;
    let bundle =
        cloud_config_bundle_for_app_server_target(target, &bootstrap, &source.sofia_home).await?;
    managed
        .check_source_policy(
            &cli_overrides,
            overrides,
