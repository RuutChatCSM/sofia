...
const GOAL_FILE_PREFIX: &str = "Read the Sofia goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    sofia_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, sofia_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, sofia_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let sofia_home = sofia_home
        .context("App server did not report $SOFIA_HOME; cannot materialize goal files")?;
    let path = sofia_home
        .join(GOAL_ATTACHMENT_DIR)
