...
const DISABLE_KEYBOARD_ENHANCEMENT_ENV_VAR: &str = "SOFIA_TUI_DISABLE_KEYBOARD_ENHANCEMENT";
...
fn read_windows_term_program() -> Option<String> {
    ...
    let executable = sofia_utils_path::system_executable("cmd.exe")?;
...
fn read_tmux_extended_keys_format() -> Option<String> {
    let executable = sofia_utils_path::system_executable("tmux")?;
    let path = sofia_utils_path::system_path().ok()?;
    for args in [
