...
async fn explicit_remote_worktree_rejection_is_snapshotted() -> anyhow::Result<()> {
    let cli = Cli::parse_from(["sofia", "--worktree"]);
...
async fn refreshed_bundle_rechecks_source_during_config_reload() -> anyhow::Result<()> {
    ...
    let manager = sofia_worktree::WorktreeManager::new(sofia_worktree::WorktreeSettings::for_cli(
        dir.path(),
        /*desktop*/ None,
    )?);
    let checkout = sofia_worktree::ManagedWorktree {
    ...
    .await?;
    let refreshed =
        sofia_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_config(
            format!(
                "[projects.{}]\ntrust_level = \"untrusted\"\n",
                serde_json::to_string(&sofia_config::loader::project_trust_key(&nested))?
            ),
        );
