...
/// has a sensible starting point without implying that Sofia is already the
...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(sofia_home);
...
fn available_pet_entries(sofia_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(sofia_home));
    entries
}

fn custom_pet_entries(sofia_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(sofia_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) =
                Pet::load_with_codex_home(&selector, /*sofia_home*/ Some(sofia_home))
            else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(sofia_home.path(), "chefito", "Chefito");
    ...
    fn picker_preselects_codex_without_marking_it_current_when_no_pet_is_configured() {
        ...
        assert_eq!(params.initial_selected_idx, Some(2));
        assert_eq!(params.items[2].name, "Sofia");
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let sofia_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(sofia_home.path(), "legacy", "Legacy");
