//! Ambient terminal rendering for the Sofia companion.
//!
...
impl AmbientPet {
    ...
    /// SOFIA_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_codex_home(
            ...
            /*sofia_home*/ Some(sofia_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = sofia_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
