...
impl EditorPaths {
    fn new() -> Self {
        ...
        let sofia_home = canonical_root.join("sofia-home");
        let cwd = canonical_root.join("workspace");
        fs::create_dir(&sofia_home).expect("create Sofia home");
...
fn editor_directory_is_inside_isolated_codex_home() {
    ...
    let directory = editor_directory(&[&paths.sofia_home], &policy, &paths.cwd)
        .expect("create isolated editor directory");

    assert_eq!(directory, paths.sofia_home.join("editor"));
...
fn editor_directory_rejects_writable_home_editor_and_parent() {
    let paths = EditorPaths::new();
    let editor = paths.sofia_home.join("editor");
    fs::create_dir(&editor).expect("create editor directory");
    let parent = paths.sofia_home.parent().expect("Sofia home parent");

    for writable_root in [paths.sofia_home.as_path(), editor.as_path(), parent] {
        let policy = workspace_write_policy(&[writable_root]);

        assert!(
            editor_directory(&[&paths.sofia_home], &policy, &paths.cwd).is_err(),
...
fn editor_directory_rejects_writable_descendant() {
    let paths = EditorPaths::new();
    let writable_descendant = paths.sofia_home.join("editor").join("nested");
    ...
    assert!(editor_directory(&[&paths.sofia_home], &policy, &paths.cwd).is_err());
}
...
fn editor_directory_rejects_read_only_carveout_with_writable_parent() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry::new(
            AbsolutePathBuf::from_absolute_path(&paths.sofia_home)
                .expect("absolute Sofia home")
                .into(),
...
fn editor_directory_rejects_preexisting_symlink() {
    ...
    symlink(&outside, paths.sofia_home.join("editor")).expect("create editor directory symlink");
...
fn editor_directory_rejects_writable_codex_home_alias() {
    ...
    let paths = EditorPaths::new();
    let aliased_home = paths.cwd.join("sofia-home-link");
    symlink(&paths.sofia_home, &aliased_home).expect("create Sofia home symlink");
    let policy = workspace_write_policy(&[]);

    assert!(policy.can_write_local_path_with_cwd(&aliased_home.join("editor"), &paths.cwd));
    assert!(!policy.can_write_local_path_with_cwd(&paths.sofia_home.join("editor"), &paths.cwd));
...
fn editor_directory_rejects_writable_codex_home_alias_target() {
    ...
    let alias_parent = paths
        .sofia_home
        .parent()
        .expect("Sofia home parent")
        .join("aliases");
    fs::create_dir(&alias_parent).expect("create protected alias parent");
    let aliased_home = alias_parent.join("sofia-home-link");
    symlink(&paths.sofia_home, &aliased_home).expect("create Sofia home symlink");
    let policy = workspace_write_policy(&[&paths.sofia_home]);
...
fn editor_directory_uses_protected_workspace_fallback_with_default_temporary_grants() {
    let root = tempfile::tempdir().expect("create editor test root");
    let sofia_home = root.path().join("sofia-home");
    let cwd = root.path().join("workspace");
    fs::create_dir(&sofia_home).expect("create Sofia home");
    fs::create_dir(&cwd).expect("create workspace");
    let workspace_codex_home = cwd.join(".sofia");
    ...
    assert!(policy.can_write_local_path_with_cwd(&sofia_home, &cwd));
    ...
    let directory = editor_directory(&[&sofia_home, &workspace_codex_home], &policy, &cwd)
...
fn editor_directory_rejects_explicitly_writable_workspace_fallback() {
    ...
    assert!(policy.can_write_local_path_with_cwd(&workspace_codex_home, &cwd));
    assert!(
        editor_directory(&[&sofia_home, &workspace_codex_home], &policy, &cwd).is_err(),
...
fn editor_directory_rejects_workspace_fallback_symlink_to_writable_target() {
    ...
    let paths = EditorPaths::new();
    let workspace_codex_home = paths.cwd.join(".sofia");
    symlink(&paths.sofia_home, &workspace_codex_home).expect("create workspace metadata symlink");
    let policy = workspace_write_policy(&[&paths.sofia_home]);

    assert!(!policy.can_write_local_path_with_cwd(&workspace_codex_home, &paths.cwd));
    assert!(policy.can_write_local_path_with_cwd(&paths.sofia_home, &paths.cwd));
    assert!(
        editor_directory(
            &[&paths.sofia_home, &workspace_codex_home],
            &policy,
            &paths.cwd,
        )
        .is_err()
...
fn editor_directory_uses_next_protected_candidate_after_creation_error() {
    ...
    fs::write(&unavailable_home, "not a directory").expect("create unavailable Sofia home");
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&[&unavailable_home, &paths.sofia_home], &policy, &paths.cwd)
...
fn editor_directory_rejects_windows_temporary_directory_outside_tmpdir_policy_root() {
    ...
    assert!(!paths.sofia_home.join("editor").exists());
}
...
async fn editor_process_receives_buffer_in_isolated_codex_home() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[]);
    let editor_directory = paths.sofia_home.join("editor");
...
async fn editor_process_uses_protected_workspace_fallback_with_default_temporary_grants() {
    ...
    let writable_default_codex_home = AbsolutePathBuf::from_absolute_path(&default_codex_home)
        .expect("absolute default Sofia home");
    let policy = FileSystemSandboxPolicy::workspace_write(
    ...
    let content = run_editor("seed", &editor_command, &sofia_home, &policy, &cwd)
        .await
