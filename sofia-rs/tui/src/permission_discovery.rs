...
pub(crate) fn fetch(
    ...
    let local_discovery = (!app_server.uses_embedded_app_server()
        ...
        && config.config_layer_stack.layers_low_to_high().any(|layer| {
            matches!(layer.name, sofia_config::ConfigLayerSource::SessionFlags)
...
fn discovery_error(error: TypedRequestError) -> String {
    ...
    {
        return "This server does not support permission discovery. Upgrade the Sofia server to use this menu.".to_string();
    }
