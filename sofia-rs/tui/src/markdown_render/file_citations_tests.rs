...
fn file_citation_paths_preserve_markdown_significant_characters() {
    for path in [
        ...
        let markdown = format!(":sofia-file-citation{{path=\"{path}\"}}");
...
fn file_url_citation_preserves_literal_query_delimiters_snapshot() {
    ...
    for (cwd, directory) in [
        ...
        let markdown = format!(":sofia-file-citation{{path=\"{directory}report?final.xlsx\"}}");
...
fn file_citations_inside_code_existing_links_and_html_remain_literal() {
    let citation = r#":sofia-file-citation{path="/tmp/report.xlsx" purpose="output"}"#;
...
fn file_citations_accept_unquoted_paths_and_trailing_windows_separators() {
    for (citation, expected) in [
        (
            r#":sofia-file-citation{path="C:\Users\me\.sofia\report.xlsx"}"#,
            "C:/Users/me/.sofia/report.xlsx",
        ),
        (
            ":sofia-file-citation{path=/tmp/a*b*.txt purpose=output}",
            "/tmp/a*b*.txt",
        ),
        (
            r#":sofia-file-citation{path="C:\repo\" purpose="output" sheet="Q1=Actual"}"#,
            "C:/repo/",
        ),
        (
            r#":sofia-file-citation{path="/tmp/a\"b" label="team's \"report\""}"#,
            "/tmp/a\"b",
        ),
    ] {
...
fn file_citations_preserve_escaped_nested_and_reference_directives() {
    let citation = ":sofia-file-citation{path=/tmp/report.xlsx}";
    ...
    for literal in [
        ...
        format!(":::{citation}"),
        format!("sofia-file-citation {}", ":x{v=".repeat(16_000)),
        format!("sofia-file-citation {}}}", ":x{v=a v=".repeat(16_000)),
        format!(
            "sofia-file-citation {} x bad}}",
            ":a{k=".repeat(/*n*/ 16_000)
        ),
    ] {
    ...
    insta::assert_snapshot!(rendered, @r#":unsupported{path="C:\repo" value=":sofia-file-citation{path=/tmp/report.xlsx}"}"#);
...
fn multiple_file_citations_render_without_interpreting_encoded_source() {
    ...
    let markdown = format!(
        r#"&#58;sofia-file-citation{{path="ignored.xlsx"}} :sofia-file-citation{{artifact_kind="workbook" path="reports/final%20report.xlsx" purpose="output" sheet="Dashboard" range="A1:D8"}} and ::sofia-file-citation{{path="{}"}}"#,
        second.display(),
    );

    assert_eq!(
        rendered_text(&markdown, Some(&cwd)),
        r#":sofia-file-citation{path="ignored.xlsx"} reports/final%20report.xlsx and second.xlsx"#,
    );

    // A work budget must not become a fixed count limit on legitimate citations.
    let many = [":sofia-file-citation{path=/tmp/report.xlsx}"; 512].join(" ");
    assert_eq!(
...
fn file_citations_preserve_adjacent_entities_and_escaped_punctuation() {
    ...
    for (markdown, expected) in [
        (
            r#"Préface &amp;:sofia-file-citation{path="first&amp;.txt"}&#x1F642;:sofia-file-citation{path="second.txt"}&lt;fin&gt;"#,
            "Préface &first&amp;.txt🙂second.txt<fin>",
        ),
        (
            r#"\[:sofia-file-citation{path="first.txt"}\]\*:sofia-file-citation{path="second.txt"}\!"#,
...
fn file_citation_after_local_link_soft_break_starts_a_new_line_snapshot() {
    let cwd = std::env::temp_dir();
    let markdown = format!(
        "[first](<{}>)\n:sofia-file-citation{{path=\"second.txt\"}}",
...
fn generic_markdown_keeps_assistant_directives_literal() {
    let citation = r#":sofia-file-citation{path="/tmp/report.xlsx"}"#;
