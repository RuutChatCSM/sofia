...
pub(super) async fn run_main_inner(
    ...
    if cli.shared.worktree {
        ...
        if cli.fork_picker || cli.fork_last {
            return Err(std::io::Error::other(
                "`sofia fork --worktree` requires an explicit session ID",
            ));
        }
    }
    ...
    // `oss` model provider.
    let overrides_cli = sofia_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let sofia_home = match find_codex_home() {
        Ok(sofia_home) => sofia_home.to_path_buf(),
        Err(err) => {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&sofia_home, profile_v2);
    ...
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        let validation_target = app_server_target_for_launch(
            ...
            std::env::var_os(sofia_exec_server::SOFIA_EXEC_SERVER_URL_ENV_VAR).as_deref(),
        )?;
        let validation_environment_manager =
            if should_load_configured_environments(&loader_overrides, &validation_target) {
                EnvironmentManager::prepare_from_codex_home(&sofia_home).await
            } else {
    ...
    let initial_screen = if cli.resume_picker || cli.fork_picker || cli.agents_overview {
        ...
        && startup_preflight::should_delay_startup_composer_for_first_login(
            &sofia_home,
            sofia_config::loader::system_config_toml_file(),
            || sofia_config::loader::has_local_managed_configuration(&sofia_home),
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        startup_draft
            .run_until(maybe_probe_default_daemon_socket(&sofia_home))
            .await?
    } else {
        None
    };
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            startup_draft
                .run_until(EnvironmentManager::prepare_from_codex_home(&sofia_home))
                .await?
        } else {
            startup_draft
    ...
    remove_legacy_tui_log_file(config.sofia_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        sofia_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(sofia_otel::OtelProvider::metrics) {
        let _ = sofia_otel::record_process_start_once(metrics, otel_originator.as_str());
        ...
        let telemetry =
            sofia_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = sofia_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("sofia_core=info,sofia_tui=info,sofia_rmcp_client=info")
        });
    ...
    let feedback = sofia_feedback::CodexFeedback::new();
