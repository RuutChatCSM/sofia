...
async fn run_git_command(
    ...
    argv.push(sofia_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string());
...
mod tests {
    ...
    async fn open_pull_request_falls_back_to_parent_repo_commit_lookup() {
        let runner = FakeRunner::new(vec![
            ...
            response(
                ...
                /*exit_code*/ 0,
                r#"{"nameWithOwner":"fcoury/sofia","parent":{"nameWithOwner":"openai/codex"}}"#,
            ),
            response(
                &[
                    "gh",
    ...
    fn status_line_pr_fallback_searches_parent_repo_first() {
        assert_eq!(
            ...
            Some(vec!["openai/codex".to_string(), "fcoury/sofia".to_string()])
        );
    }
    ...
    fn response(argv: &[&str], exit_code: i32, stdout: &str) -> FakeResponse {
        ...
        if argv.first().map(String::as_str) == Some("git") {
            argv.splice(
                ...
                [
                    "-c".to_string(),
                    sofia_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string(),
                ],
            );
        }
        FakeResponse {
