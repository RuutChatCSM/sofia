...
//! under `{SOFIA_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(sofia_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = sofia_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(sofia_home);
    let sofia_home_owned = sofia_home.map(Path::to_path_buf);
    ...
    let preview_home = sofia_home_owned.clone();
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*sofia_home*/ None, /*terminal_width*/ None,
        );
    ...
    fn subtitle_uses_tilde_path_when_codex_home_under_home_directory() {
        ...
        let sofia_home = home.join(".sofia");

        let subtitle = theme_picker_subtitle(Some(&sofia_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let sofia_home = home.join(long_segment).join(".sofia");

        let subtitle = theme_picker_subtitle(Some(&sofia_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle =
            theme_picker_subtitle(/*sofia_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&sofia_home), Some(94));
