...
use sofia_app_server_client::RemoteAppServerEndpoint;
...
async fn state_runtime(config: &Config) -> std::io::Result<Arc<sofia_state::StateRuntime>> {
    let runtime = sofia_state::StateRuntime::init(
        sofia_state::SqliteConfig::new_for_testing(config.sofia_home.as_path().abs()),
...
) -> color_eyre::Result<Option<sofia_app_server_protocol::Thread>> {
...
async fn upsert_thread(
    ...
    metadata: sofia_state::ThreadMetadata,
) -> std::io::Result<()> {
    runtime
...
fn thread_metadata(
    ...
    builder.cwd = config.sofia_home.join("project").to_path_buf();
...
fn write_rollout(
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            cwd: config.sofia_home.join("project").to_path_buf(),
...
async fn uses_listed_thread_when_older_server_cannot_read_it() -> color_eyre::Result<()> {
    ...
    for mode in [ThreadParamsMode::Remote, ThreadParamsMode::Embedded] {
        ...
        let mut remote = AppServerSession::new(
            sofia_app_server_client::AppServerClient::Remote(client),
            mode,
        );
        let found = lookup(
            &mut remote,
