...
use sofia_ansi_escape::ansi_escape_line;
...
#[cfg(test)]
use sofia_app_server_protocol::McpAuthStatus;
...
use sofia_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use sofia_app_server_protocol::SkillErrorInfo;
...
use sofia_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use sofia_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use sofia_protocol::permissions::FileSystemSandboxKind;
use sofia_rollout::StateDbHandle;
use sofia_terminal_detection::user_agent;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        sofia_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&sofia_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&sofia_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[sofia_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&sofia_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<sofia_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    sofia_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
pub(crate) struct App {
    ...
    dynamic_tool_status_updates:
        tokio::sync::broadcast::Sender<sofia_app_server_protocol::ThreadStatusChangedNotification>,
    dynamic_tool_tasks: HashMap<sofia_app_server_protocol::RequestId, (String, JoinHandle<()>)>,
