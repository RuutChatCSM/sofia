...
fn context_with_fragment(fragment: &str) -> (TempDir, InlineVisualizationContext) {
    let sofia_home = tempfile::tempdir().expect("temp sofia home");
    let thread_id = ThreadId::new();
    let context = InlineVisualizationContext::new(sofia_home.path(), thread_id)
...
fn granted_visualization_root_overrides_thread_id_derived_root() {
    let sofia_home = tempfile::tempdir().expect("temp sofia home");
    let granted_context = InlineVisualizationContext::new(sofia_home.path(), ThreadId::new())
...
fn rewrites_complete_directive_to_trusted_static_file_placeholder() {
    ...
    let rewritten = rewrite_inline_visualizations(
        "Before\n::sofia-inline-vis{file=\"chart.html\"}\nAfter",
        Some(&context),
    );
...
fn unavailable_artifact_has_explicit_fallback() {
    let sofia_home = tempfile::tempdir().expect("temp sofia home");
    let context = InlineVisualizationContext::new(sofia_home.path(), ThreadId::new())
        .expect("UUIDv7 thread id should provide a timestamp");

    assert_eq!(
        rewrite_inline_visualizations("::sofia-inline-vis{file=\"missing.html\"}", Some(&context),)
            .markdown,
        "_Visualization unavailable on this device._"
    );
}
    ...
    for file in ["../chart.html", "chart.svg"] {
        assert_eq!(
            rewrite_inline_visualizations(
                &format!("::sofia-inline-vis{{file=\"{file}\"}}"),
                Some(&context),
            )
            .markdown,
            "_Visualization unavailable on this device._"
...
fn rejects_oversized_fragment() {
    ...
    assert_eq!(
        rewrite_inline_visualizations("::sofia-inline-vis{file=\"chart.html\"}", Some(&context),)
            .markdown,
        "_Visualization unavailable on this device._"
    );
}
...
fn viewer_materializes_sandboxed_static_document() {
    ...
    assert_eq!(
        ...
        Some(std::ffi::OsStr::new(".sofia-viewers"))
    );
...
fn transcript_overlay_remeasures_visualization_when_artifact_becomes_available() {
    ...
    let cell = AgentMarkdownCell::new_with_inline_visualizations(
        "::sofia-inline-vis{file=\"chart.html\"}".to_string(),
        Path::new("/workspace"),
        Some(context.clone()),
    );
...
fn visualization_link_uses_the_artifact_name() {
    ...
    let rewritten = rewrite_inline_visualizations(
        "::sofia-inline-vis{file=\"compound-interest-explorer.html\"}",
        Some(&context),
    );
...
fn user_markdown_keeps_directive_literal() {
    let rendered =
        crate::markdown_render::render_markdown_text("::sofia-inline-vis{file=\"chart.html\"}");
    let text = rendered
        .lines
        .iter()
    ...
    assert_eq!(text, "::sofia-inline-vis{file=\"chart.html\"}");
}
