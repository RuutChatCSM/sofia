...
use sofia_windows_sandbox::SandboxProvisioningRequest;
use sofia_windows_sandbox::SandboxProvisioningResponse;
use sofia_windows_sandbox::WindowsSandboxProvisioningSettings;
use sofia_windows_sandbox::WindowsSandboxProxyListeners;
use sofia_windows_sandbox::read_provisioning_frame;
...
fn config_parse_errors_are_distinguished_from_policy_and_io_failures() {
    let schema_error = sofia_core::config::deserialize_config_toml_with_base(
    ...
    let syntax_error = sofia_config::io_error_from_config_error(
        std::io::ErrorKind::InvalidData,
        sofia_config::config_error_from_toml("config.toml", contents, parse_error.clone()),
        Some(parse_error),
    );
...
fn provisioning_request_preserves_home_spaces_and_unicode() {
    let request = framed_request(SandboxProvisioningRequest {
        sofia_home: "D:\\Sofia Homes\\Jos\u{00e9}\\.sofia".to_string(),
    ...
    assert_eq!(
        ...
        ProvisioningRequest {
            sofia_home: PathBuf::from("D:\\Sofia Homes\\Jos\u{00e9}\\.sofia"),
            listeners: WindowsSandboxProxyListeners::default(),
            settings: WindowsSandboxProvisioningSettings::default(),
        }
    );
...
fn structured_provisioning_request_accepts_independent_and_additional_proxy_ports() {
    for (http_ports, socks_ports, proxy_ports) in [
        ...
        let request = framed_request(SandboxProvisioningRequest {
            sofia_home: r"C:\Users\alice\.sofia".to_string(),
        ...
        assert_eq!(
            validate_request(&request).unwrap(),
            ProvisioningRequest {
                sofia_home: PathBuf::from(r"C:\Users\alice\.sofia"),
                settings,
                listeners,
            }
        );
...
fn unpackaged_pipe_clients_are_rejected_before_sending_a_request() {
    ...
    assert!(
        error
            .to_string()
            .contains("installed Sofia package identity"),
