//! Binds provisioning requests to the packaged Sofia client and its Windows user.
...
pub(crate) fn authorize_client_process(pipe: HANDLE) -> Result<AuthorizedClientProcess> {
    ...
    match client_family {
        Some(client_family) => match service_family.as_deref() {
            ...
            #[cfg(debug_assertions)]
            None if FOREGROUND_MODE.load(Ordering::Acquire) => {
                bail!("provisioning client does not belong to a trusted Sofia package family")
            }
        ...
        #[cfg(debug_assertions)]
        None if FOREGROUND_MODE.load(Ordering::Acquire) => {}
        None => bail!("provisioning clients must run with an installed Sofia package identity"),
    }
