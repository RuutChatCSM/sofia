...
use sofia_windows_sandbox::FramedProvisioningMessage;
use sofia_windows_sandbox::PROVISIONING_PROTOCOL_VERSION;
use sofia_windows_sandbox::ProvisioningMessage;
use sofia_windows_sandbox::SandboxProvisioningResponse;
use sofia_windows_sandbox::ensure_sandbox_users_group;
use sofia_windows_sandbox::run_elevated_provisioning_setup_with_retained_handles;
use sofia_windows_sandbox::sandbox_setup_is_complete_with_settings;
use sofia_windows_sandbox::string_from_sid_bytes;
use sofia_windows_sandbox::to_wide;
use sofia_windows_sandbox::write_provisioning_frame;
use std::mem::size_of;
...
pub(crate) const PIPE_NAME: &str = sofia_windows_sandbox::SANDBOX_PROVISIONING_PIPE_NAME;
...
fn handle_request(
    ...
    if let Err(error) = policy_result {
        ...
        crate::service::log_error(
            ...
            &format!("Sofia sandbox provisioning was rejected by administrator policy: {error}"),
        );
        return Err(error)
    ...
    let previous = crate::installation_record::load()?.filter(|record| {
        record.user_sid == identity.user_sid && record.sofia_home == identity.sofia_home
    });
    let installation = InstallationRecord {
        sofia_home: identity.sofia_home.clone(),
    ...
    if sandbox_setup_is_complete_with_settings(&identity.sofia_home, &request.settings) {
    ...
    let helper = std::env::current_exe()
        ...
        .with_file_name("sofia-windows-sandbox-setup.exe");
    ...
    match run_elevated_provisioning_setup_with_retained_handles(
        ...
        Ok(()) => {
            ...
            crate::service::log_information(
                ...
                "Sofia sandbox provisioning completed successfully.",
            );
        ...
        Err(error) => {
            crate::service::log_error(
                ...
                &format!("Sofia sandbox provisioning failed: {error}"),
            );
