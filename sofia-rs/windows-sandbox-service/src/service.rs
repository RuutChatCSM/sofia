...
unsafe extern "system" fn service_main(_argument_count: u32, _arguments: *mut *mut u16) {
    ...
    if let Err(error) = service_main_inner(state) {
        log_error(
            ...
            &format!("The Sofia sandbox service encountered a fatal error: {error:#}"),
        );
...
fn service_main_inner(state: &ServiceState) -> Result<()> {
    ...
    crate::ipc::run(
        ...
        || {
            ...
            log_information(
                ...
                "The Sofia sandbox service is running.",
            );
            Ok(())
        },
    ...
    log_information(
        ...
        "The Sofia sandbox service has stopped.",
    );
    ...
    match control {
        SERVICE_CONTROL_STOP | SERVICE_CONTROL_SHUTDOWN => {
            ...
            if !state.shutdown.swap(true, Ordering::AcqRel) {
                ...
                log_information(
                    ...
                    "The Sofia sandbox service was asked to stop.",
                );
        ...
        SERVICE_CONTROL_INTERROGATE => {
            ...
            if let Err(error) = state.report_status(current_status, NO_ERROR) {
                log_error(
                    EVENT_SERVICE_FAILED,
                    "The Sofia sandbox service could not report its current status.",
                );
