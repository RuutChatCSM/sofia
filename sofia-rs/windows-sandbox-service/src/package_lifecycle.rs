...
use anyhow::Result;
use anyhow::ensure;
use sofia_windows_sandbox::clean_up_packaged_windows_sandbox;
...
impl PackageLifecycle {
    ...
    pub(crate) fn watch_authenticated_user(
        ...
        with_owner_impersonation(user_token.0, || {
            ...
            let sofia_home = match crate::ipc::pin_existing_ancestors(
                &installation.sofia_home,
                &mut directory_handles,
            ) {
                Ok(()) => Some(installation.sofia_home.clone()),
                Err(error) => {
    ...
    pub(crate) fn clean_up(&self) -> Result<()> {
        ...
        let sofia_home = installation.sofia_home.clone();
        clean_up_packaged_windows_sandbox(sofia_home.as_deref(), || {
                ...
                if desktop.cache_home.is_dir() {
                    match crate::ipc::pin_existing_ancestors(
                        ...
                        Ok(()) => record_error(std::fs::remove_dir_all(
                            desktop.cache_home.join("sofia-runtimes"),
                        )),
