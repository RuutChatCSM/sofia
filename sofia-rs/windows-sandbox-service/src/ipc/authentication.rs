...
fn authenticate_impersonated_client(
    ...
    crate::package_identity::authorize_client(authorized_process, token.0)
        .context("authorize the packaged Sofia provisioning client")?;
    ...
    let (sofia_home, handles) = prepare_codex_home(requested_home)?;
    let desktop_installation =
        crate::installation_record::read_desktop_installation(&sofia_home, token.0)
            .inspect_err(|_| {
