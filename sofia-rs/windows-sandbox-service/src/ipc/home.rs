//! Validates and pins local Sofia home and sandbox directories, keeping sandbox
...
use sofia_windows_sandbox::create_directory_guard;
use sofia_windows_sandbox::open_directory_no_reparse;
use sofia_windows_sandbox::to_wide;
use sofia_windows_sandbox::validate_local_directory_path;
...
impl std::fmt::Display for UnsupportedHomeDrive {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("Sofia home must be located on a fixed local drive")
    }
}
...
pub(super) fn prepare_codex_home(requested: &Path) -> Result<(PathBuf, Vec<OwnedHandle>)> {
    ...
    let requested_root = requested
        .ancestors()
        .last()
        .context("find the root of the requested Sofia home")?;
    ...
    let parent = requested
        .parent()
        .context("Sofia home must have an existing parent directory")?;
    ...
    let home = requested
        .canonicalize()
        .with_context(|| format!("canonicalize Sofia home {}", requested.display()))?;
    ...
    .enumerate()
    {
        ...
        let handle = pin_directory(directory, access, DirectoryOpenDisposition::OpenOrCreate)
            ...
            if index == 0 {
                format!(
                    "requesting user must be permitted to write Sofia home {}",
                    directory.display()
                )
            } else {
                format!(
