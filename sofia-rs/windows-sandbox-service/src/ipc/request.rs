...
pub(super) fn validate_request(request: &[u8]) -> Result<ProvisioningRequest> {
    ...
    if request.sofia_home.is_empty() || request.sofia_home.contains(['\0', '\r', '\n']) {
        bail!("Sofia home is empty or contains an invalid control character");
    }
    ...
    Ok(ProvisioningRequest {
        sofia_home: PathBuf::from(request.sofia_home),
        listeners,
        settings,
    })
}
