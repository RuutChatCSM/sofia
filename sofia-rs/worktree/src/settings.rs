...
impl WorktreeSettings {
    ...
    pub fn for_cli(sofia_home: &Path, desktop: Option<&HashMap<String, Value>>) -> Result<Self> {
        let desktop = desktop
            .and_then(|settings| settings.get(WORKTREE_ROOT))
            .map(|root| HashMap::from([(WORKTREE_ROOT.to_owned(), root.clone())]));
        Ok(Self {
            auto_cleanup_enabled: false,
            ..Self::from_desktop_config(sofia_home, desktop.as_ref())?
        })
    }
    ...
    pub fn from_desktop_config(
        ...
        let root = match desktop.and_then(|settings| settings.get(WORKTREE_ROOT)) {
            None | Some(Value::Null) => sofia_home.join("worktrees"),
            Some(value) => {
