...
use sofia_worktree::ManagedWorktree;
...
impl RepositoryFixture {
    fn new() -> Self {
        ...
        let sofia_home = temp_root.join("sofia-home");
    ...
    fn manager(&self) -> WorktreeManager {
        let settings =
            WorktreeSettings::from_desktop_config(&self.sofia_home, /*desktop*/ None)
...
fn git_output(repository: &Path, args: &[&str]) -> std::process::Output {
    Command::new("git")
        ...
        .args([
            "-c",
            "user.name=Sofia Worktree Test",
            "-c",
            "user.email=sofia-worktree-test@example.invalid",
            "-c",
...
fn cli_creation_shares_default_and_custom_desktop_pools_without_enabling_cleanup() {
    ...
    for root in [None, Some(fixture.sofia_home.join("custom-desktop"))] {
        let desktop =
            root.map(|root| HashMap::from([("git-worktree-root".to_owned(), json!(root))]));
        let desktop_settings =
            WorktreeSettings::from_desktop_config(&fixture.sofia_home, desktop.as_ref())
        ...
        let manager = WorktreeManager::new(
            WorktreeSettings::for_cli(&fixture.sofia_home, Some(&cli_desktop))
...
fn thread_binding_rejects_primary_and_unmanaged_worktrees() {
    ...
    assert!(
        !fixture.repository.join(".git/sofia-thread.json").exists(),
...
fn thread_binding_writes_the_exact_desktop_owner_schema_into_git_metadata() {
    ...
    let metadata_path = checkout.join(run_git(
        &checkout,
        &["rev-parse", "--git-path", "sofia-thread.json"],
    ));
...
fn creation_uses_desktop_hash_bucket_layout_and_detached_head() {
    ...
    assert_eq!(
        ...
        Some(fixture.sofia_home.join("worktrees").as_path())
    );
    assert_eq!(
...
fn creation_rejects_a_working_directory_that_escapes_through_a_base_symlink() {
    ...
    let outside = fixture.sofia_home.join("outside-worktree");
...
fn creation_ignores_inherited_git_environment() {
    const CHILD_ROOT: &str = "SOFIA_WORKTREE_ENV_TEST_ROOT";
    if let Some(root) = std::env::var_os(CHILD_ROOT) {
        let root = PathBuf::from(root);
        let source = root.join("project");
        let source_cwd = source.join("nested/component");
        let settings =
            WorktreeSettings::from_desktop_config(&root.join("sofia-home"), /*desktop*/ None)
        ...
        assert_eq!(
            worktree.head_sha,
            std::env::var("SOFIA_WORKTREE_ENV_TEST_HEAD").expect("fixture operation succeeds")
        );
        return;
    }

    let fixture = RepositoryFixture::new();
    let other = fixture.sofia_home.join("other-repository");
    ...
    child
        ...
        .env("SOFIA_WORKTREE_ENV_TEST_HEAD", run_git(&fixture.repository, &["rev-parse", "HEAD"]))
        ...
        .env("GIT_CONFIG_PARAMETERS", "'filter.review.process=sofia-worktree-nonexistent-filter' 'filter.review.required=true'");
...
fn creation_disables_filters_from_destination_conditional_config() {
    ...
    let config = fixture.sofia_home.join("destination.gitconfig");
    fs::write(
        &config,
        "[filter \"late\"]\nsmudge = sofia-worktree-nonexistent-filter\nrequired = true\n",
    )
...
fn creation_fails_for_a_directory_outside_a_git_repository() {
    let fixture = RepositoryFixture::new();
    let manager = fixture.manager();
    let non_repository = fixture.sofia_home.join("not-a-repository");
...
fn listing_rejects_a_primary_checkout_even_when_its_path_matches_the_layout() {
    ...
    let git_dir = fixture.sofia_home.join("separate-git-dir");
    run_git(
        &primary,
        &[
            "init",
...
fn listing_rejects_checkout_and_bucket_aliases_but_allows_a_root_alias() {
    ...
    let aliased_root = fixture.sofia_home.join("aliased-worktrees");
...
fn listing_only_includes_managed_worktrees_for_the_requested_repository() {
    ...
    let other_repository = fixture.sofia_home.join("other-project");
...
fn creation_listing_and_thread_binding_preserve_native_repository_paths() {
    ...
    for name in [
        ...
        let repository = fixture.sofia_home.join(&name);
        initialize_repository(&repository);
