...
use anyhow::bail;
use clap::Parser;
use sofia_core_api::AbsolutePathBuf;
use sofia_core_api::AltScreenMode;
use sofia_core_api::ApprovalsReviewer;
use sofia_core_api::Arg0DispatchPaths;
use sofia_core_api::AskForApproval;
use sofia_core_api::AuthCredentialsStoreMode;
use sofia_core_api::AuthManager;
use sofia_core_api::AutoCompactTokenLimitScope;
use sofia_core_api::CodexAppsToolsCache;
use sofia_core_api::CodexHomeUserInstructionsProvider;
use sofia_core_api::CodexThread;
use sofia_core_api::Config;
use sofia_core_api::ConfigLayerStack;
use sofia_core_api::Constrained;
use sofia_core_api::EnvironmentManager;
use sofia_core_api::EventMsg;
use sofia_core_api::ExecServerRuntimePaths;
use sofia_core_api::ExtensionRegistryBuilder;
use sofia_core_api::Features;
use sofia_core_api::GhostSnapshotConfig;
use sofia_core_api::History;
use sofia_core_api::MemoriesConfig;
use sofia_core_api::ModelAvailabilityNuxConfig;
use sofia_core_api::MultiAgentV2Config;
use sofia_core_api::NewThread;
use sofia_core_api::Notice;
use sofia_core_api::OAuthCredentialsStoreMode;
use sofia_core_api::OPENAI_PROVIDER_ID;
use sofia_core_api::OtelConfig;
use sofia_core_api::PermissionProfile;
use sofia_core_api::Permissions;
use sofia_core_api::ProjectConfig;
use sofia_core_api::RealtimeAudioConfig;
use sofia_core_api::RealtimeConfig;
use sofia_core_api::SessionPickerViewMode;
use sofia_core_api::SessionSource;
use sofia_core_api::SqliteConfig;
use sofia_core_api::StartIfIdleSubmission;
use sofia_core_api::StartThreadOptions;
use sofia_core_api::TerminalResizeReflowConfig;
use sofia_core_api::ThreadManager;
use sofia_core_api::ThreadStoreConfig;
use sofia_core_api::ToolSuggestConfig;
use sofia_core_api::TuiKeymap;
use sofia_core_api::TuiNotificationSettings;
use sofia_core_api::TuiPetAnchor;
use sofia_core_api::TurnInputRequest;
use sofia_core_api::UriBasedFileOpener;
use sofia_core_api::UserInput;
use sofia_core_api::WebSearchMode;
use sofia_core_api::arg0_dispatch_or_else;
use sofia_core_api::build_models_manager;
use sofia_core_api::built_in_model_providers;
use sofia_core_api::find_codex_home;
use sofia_core_api::init_state_db;
use sofia_core_api::install_image_generation_extension;
use sofia_core_api::item_event_to_server_notification;
use sofia_core_api::local_agent_graph_store_from_state_db;
use sofia_core_api::passthrough_image_store;
use sofia_core_api::resolve_installation_id;
use sofia_core_api::set_default_originator;
use sofia_core_api::thread_store_from_config;

#[derive(Debug, Parser)]
#[command(
    name = "sofia-thread-manager-sample",
    about = "Run one Sofia turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("sofia_thread_manager_sample".to_string()) {
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        config.sofia_self_exe.clone(),
        config.sofia_linux_sandbox_exe.clone(),
    )?;
    ...
    turn_output?;
    shutdown_result.context("shut down Sofia thread")?;

    Ok(())
}

fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let sofia_home = find_codex_home().context("find Sofia home")?;
    ...
    let mut config = Config {
        ...
        sqlite: SqliteConfig::from_sqlite_home(sofia_home.clone()),
        log_dir: sofia_home.join("log").to_path_buf(),
        sofia_home,
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read Sofia event")?;
