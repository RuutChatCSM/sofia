...
use sofia_app_server_protocol::JSONRPCNotification;
use sofia_app_server_protocol::JSONRPCRequest;
...
use sofia_app_server_protocol::SandboxPolicy;
...
use sofia_otel::OtelProvider;
use sofia_otel::current_span_w3c_trace_context;
use sofia_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "sofia-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $SOFIA_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Sofia app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Sofia", version, about = "Bootstrap Sofia app-server", long_about = None)]
struct Cli {
    /// Path to the `sofia` CLI binary. When set, requests use stdio by
    /// spawning `sofia app-server` as a child process.
    #[arg(long, env = "SOFIA_BIN", global = true)]
    sofia_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--sofia-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "SOFIA_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `sofia` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `sofia app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/sofia-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `sofia app-server --listen`.
    ...
    /// Send a user message through the Sofia app-server.
    SendMessage {
        /// User message to send to Sofia.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Sofia-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Sofia app-server.
    GetAccountRateLimits,
    /// List the available models from the Sofia app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Sofia app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "SOFIA_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let sofia_bin = sofia_bin.unwrap_or_else(|| PathBuf::from("sofia"));
            serve(&sofia_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(sofia_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(sofia_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(sofia_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --sofia-bin and does not support --url");
            }
            let sofia_bin = sofia_bin.context("plugin-analytics-smoke requires --sofia-bin")?;
            plugin_analytics_smoke::run(&sofia_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --sofia-bin and does not support --url"
                );
            }
            let sofia_bin =
                sofia_bin.context("plugin-analytics-mutation-smoke requires --sofia-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --sofia-bin and does not support --url");
            }
            let sofia_bin = sofia_bin.context("plugin-remote-uninstall requires --sofia-bin")?;
...
fn resolve_endpoint(sofia_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if sofia_bin.is_some() && url.is_some() {
        bail!("--sofia-bin and --url are mutually exclusive");
    }
    if let Some(sofia_bin) = sofia_bin {
        return Ok(Endpoint::SpawnCodex(sofia_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if sofia_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --sofia-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(sofia_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(sofia_bin);
        if let Some(sofia_bin_parent) = sofia_bin.parent() {
            let mut path = OsString::from(sofia_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", sofia_bin.display()))?;
...
fn serve(sofia_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/sofia-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,sofia_=trace {}",
        shell_quote(&sofia_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started sofia app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnCodex(sofia_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: sofia_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (sofia_bin, url) {
        (Some(_), Some(_)) => bail!("--sofia-bin and --url are mutually exclusive"),
        (Some(sofia_bin), None) => {
            let server = BackgroundAppServer::spawn(&sofia_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve sofia-app-server-test-client binary path")?;
...
impl CodexClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnCodex(sofia_bin) => Self::spawn_stdio(sofia_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(sofia_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(sofia_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let sofia_bin_display = sofia_bin.display();
        let mut cmd = Command::new(sofia_bin);
        if let Some(sofia_bin_parent) = sofia_bin.parent() {
            let mut path = OsString::from(sofia_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut sofia_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{sofia_bin_display}` app-server"))?;

        let stdin = sofia_app_server
            .stdin
            .take()
            .context("sofia app-server stdin unavailable")?;
        let stdout = sofia_app_server
            .stdout
            .take()
            .context("sofia app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: sofia_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `sofia-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "sofia-toy-app-server".to_string(),
                    title: Some("Sofia Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: sofia_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                sofia_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: sofia_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to sofia app-server")?;
                    return Ok(());
                }
                bail!("sofia app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from sofia app-server")?;
                if bytes == 0 {
                    bail!("sofia app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = sofia_core::otel_init::build_provider(
            &config,
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[sofia app-server exited: {status}]");
            return;
        }
