...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "SOFIA_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "SOFIA_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("sofia-plugin-analytics-{}.jsonl", process::id()))
    });
    ...
    let mut client = CodexClient::spawn_stdio_with_env(sofia_bin, &overrides, &child_environment)?;
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "sofia_plugin_used"
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug Sofia binary",
                path.display()
            );
        }
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "sofia_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "sofia_plugin_disabled",
        "sofia_plugin_enabled",
        "sofia_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("sofia_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "sofia-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")
