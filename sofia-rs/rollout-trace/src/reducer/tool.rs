...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, sofia_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, sofia_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_codex_turn_id: sofia_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        sofia_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(sofia_turn_id) = sofia_turn_id else {
            bail!("tool call start did not include thread or Sofia turn context");
        };
        self.rollout
            .sofia_turns
            .get(sofia_turn_id)
            ...
            .with_context(|| {
                format!("tool call start referenced unknown Sofia turn {sofia_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, sofia_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(sofia_turn_id) = sofia_turn_id {
            let Some(turn) = self.rollout.sofia_turns.get(sofia_turn_id) else {
                bail!("tool call start referenced unknown Sofia turn {sofia_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Sofia turn {sofia_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
