...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.sofia_turns.get(&started.sofia_turn_id) else {
            bail!(
                "compaction request {} referenced unknown sofia turn {}",
                started.compaction_request_id,
                started.sofia_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but sofia turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.sofia_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                sofia_turn_id: started.sofia_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.sofia_turns.get(&sofia_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown sofia turn {sofia_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but sofia turn {sofia_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) sofia_turn_id: String,
