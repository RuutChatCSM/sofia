...
impl TraceReducer {
    ...
    /// Starts a Sofia turn inside an existing thread.
    pub(super) fn start_codex_turn(
        ...
        if self.rollout.sofia_turns.contains_key(&sofia_turn_id) {
            bail!("duplicate sofia turn start for {sofia_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.sofia_turns.insert(
            sofia_turn_id.clone(),
            CodexTurn {
                sofia_turn_id,
                thread_id,
    ...
    /// Marks a Sofia turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_codex_turn(
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.sofia_turns.get(&sofia_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "sofia turn end for {sofia_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.sofia_turns.get_mut(&sofia_turn_id) else {
            bail!("sofia turn end referenced unknown turn {sofia_turn_id}");
        };
