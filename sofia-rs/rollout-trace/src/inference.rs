...
const INFERENCE_CALL_ID_HEADER: &str = "x-sofia-inference-call-id";
...
///
/// A Sofia turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Sofia turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        sofia_turn_id: CodexTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                sofia_turn_id: attempt.context.sofia_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps sofia-core focused on
    ...
    /// Records a provider stream that Sofia intentionally stopped consuming.
    ///
...
/// Sofia received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        sofia_turn_id: Some(context.sofia_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.sofia_turn_id, "turn-1");
