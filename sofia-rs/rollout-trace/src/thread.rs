...
use crate::protocol_event::sofia_turn_trace_event;
...
pub const SOFIA_ROLLOUT_TRACE_ROOT_ENV: &str = "SOFIA_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `SOFIA_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Sofia
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(SOFIA_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Sofia turn lifecycle events from protocol lifecycle events.
    pub fn record_codex_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            sofia_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Sofia did while executing that boundary.
    pub fn record_tool_call_event(&self, sofia_turn_id: impl Into<CodexTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(sofia_turn_id.into(), payload);
    }
    ...
    pub fn record_codex_turn_started(&self, sofia_turn_id: impl Into<CodexTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let sofia_turn_id = sofia_turn_id.into();
        context.append_with_context_best_effort(
            sofia_turn_id.clone(),
    ...
    /// Builds reusable inference trace context for one Sofia turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        sofia_turn_id: CodexTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            sofia_turn_id: Some(sofia_turn_id),
        };
