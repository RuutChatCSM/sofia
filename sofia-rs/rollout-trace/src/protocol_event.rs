//! Mapping from Sofia protocol events into raw rollout-trace events.
//!
...
//! `sofia-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `sofia-rollout-trace`.
//!
...
use sofia_protocol::protocol::PatchApplyStatus;
use sofia_protocol::protocol::SubAgentActivityEvent;
...
pub(crate) fn sofia_turn_trace_event(
    ...
    match event {
        EventMsg::TurnStarted(event) => {
            let sofia_turn_id = event.turn_id.clone();
            Some(CodexTurnTraceEvent {
                context_turn_id: sofia_turn_id.clone(),
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a sofia_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a sofia_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a sofia_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a sofia_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a sofia_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a sofia_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a sofia_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a sofia_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [sofia_protocol::parse_command::ParsedCommand],
