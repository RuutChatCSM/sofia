...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("sofia_other"),
    );
    headers.insert(
        "x-sofia-other-limit-name",
        http::HeaderValue::from_static("sofia_other"),
    );
    let body = serde_json::json!({
        "error": {
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [(None, "sofia"), (Some("sofia_other"), "sofia_other")]
    {
        let mut headers = HeaderMap::new();
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-sofia-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
