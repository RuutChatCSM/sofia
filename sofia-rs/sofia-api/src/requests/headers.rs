...
pub(crate) fn subagent_header(source: &Option<SessionSource>) -> Option<String> {
    ...
    match sub {
        sofia_protocol::protocol::SubAgentSource::Review => Some("review".to_string()),
        sofia_protocol::protocol::SubAgentSource::Compact => Some("compact".to_string()),
        sofia_protocol::protocol::SubAgentSource::MemoryConsolidation => {
            Some("memory_consolidation".to_string())
        }
        sofia_protocol::protocol::SubAgentSource::ThreadSpawn { .. } => {
            Some("collab_spawn".to_string())
        }
        sofia_protocol::protocol::SubAgentSource::Other(label) => Some(label.clone()),
    }
}
