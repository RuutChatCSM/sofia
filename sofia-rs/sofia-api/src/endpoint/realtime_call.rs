...
use sofia_client::Request;
use sofia_client::RequestBody;
use sofia_client::RequestTelemetry;
use std::sync::Arc;
use tracing::instrument;
use tracing::trace;

const MULTIPART_BOUNDARY: &str = "sofia-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=sofia-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "sofia_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/sofia"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/sofia/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
    ...
    async fn sends_backend_session_call_as_json_body() {
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/sofia/realtime/calls?intent=quicksilver&architecture=avas"
        );
