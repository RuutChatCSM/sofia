...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "sofia_api::responses_websocket_timing";
...
const WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str = "x-sofia-ws-stream-request-start-ms";
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "sofia.response.metadata"
                    && let Some(etag) =
                        event
                            .headers
                            .as_ref()
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='sofia_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn direct_serialization_preserves_websocket_request_payload() {
        let api_request = ResponsesApiRequest {
            ...
            access_programs: Some(
                sofia_protocol::turn_input::CyberAccessProgram::DaybreakBlue.into(),
            ),
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-sofia-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-sofia-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            ...
            "headers": {
                "x-sofia-safety-buffering-enabled": "true",
                "x-sofia-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "sofia.response.metadata",
            "headers": {
                "x-sofia-safety-buffering-enabled": "false",
                "x-sofia-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
