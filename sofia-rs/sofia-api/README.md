...
Typed clients for Sofia/OpenAI APIs built on top of the generic transport in `sofia-client`.
...
- Serves as the wire-level layer consumed by `sofia-core`; higher layers handle auth refresh and business logic.
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `sofia_api::CompactionInput`):
    - `model: &str`.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `sofia_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `sofia-api` and `sofia-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
