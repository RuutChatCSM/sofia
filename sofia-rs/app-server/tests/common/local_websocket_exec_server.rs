...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(sofia_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(sofia_home);
        command.env("SOFIA_HOME", sofia_home);
        #[cfg(target_os = "linux")]
        command.env(
            SOFIA_LINUX_SANDBOX_EXE_ENV_VAR,
