...
use sofia_app_server_protocol::FsCopyParams;
use sofia_app_server_protocol::FsCreateDirectoryParams;
use sofia_app_server_protocol::FsGetMetadataParams;
use sofia_app_server_protocol::FsReadDirectoryParams;
use sofia_app_server_protocol::FsReadFileParams;
use sofia_app_server_protocol::FsRemoveParams;
use sofia_app_server_protocol::FsUnwatchParams;
use sofia_app_server_protocol::FsWatchParams;
use sofia_app_server_protocol::FsWriteFileParams;
...
use sofia_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use sofia_app_server_protocol::PluginSearchParams;
use sofia_app_server_protocol::PluginSkillReadParams;
use sofia_app_server_protocol::PluginUninstallParams;
use sofia_app_server_protocol::ProcessKillParams;
use sofia_app_server_protocol::ProcessSpawnParams;
use sofia_app_server_protocol::ProjectImportParams;
use sofia_app_server_protocol::ProjectListParams;
use sofia_app_server_protocol::ProjectReadParams;
use sofia_app_server_protocol::RemoteControlClientsListParams;
use sofia_app_server_protocol::RemoteControlClientsRevokeParams;
use sofia_app_server_protocol::RemoteControlPairingStartParams;
use sofia_app_server_protocol::RemoteControlPairingStatusParams;
...
use sofia_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use sofia_exec_server::SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use sofia_exec_server::SOFIA_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use sofia_exec_server::SOFIA_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use sofia_exec_server::SOFIA_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use sofia_exec_server::SOFIA_EXEC_SERVER_URL_ENV_VAR;
use sofia_login::default_client::SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned SOFIA_HOME that may still be its cwd on Windows.
...
pub const DEFAULT_CLIENT_NAME: &str = "sofia-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "SOFIA_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary SOFIA_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            sofia_home.join("managed_config.toml"),
        );
        cmd.env_remove(SOFIA_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("sofia app-server proc should start")?;
            }
            retries += 1;
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: sofia_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing SOFIA_HOME instead of a temporary one.
    pub fn with_codex_home(mut self, sofia_home: &Path) -> Self {
        self.sofia_home = Some(sofia_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary SOFIA_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (sofia_home, owned_codex_home) = match sofia_home {
            Some(sofia_home) => (sofia_home, None),
            None => {
        ...
        let attribution_settings_server = if sofia_home.join("auth.json").is_file() {
            let config_path = sofia_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = sofia_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            sofia_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&sofia_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        SOFIA_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        SOFIA_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        SOFIA_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        SOFIA_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => sofia_utils_cargo_bin::cargo_bin("sofia-app-server")
                .context("should find binary for sofia-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && sofia_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                sofia_utils_cargo_bin::cargo_bin("sofia-code-mode-host")
        {
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `sofia-app-server` child process.
        //
