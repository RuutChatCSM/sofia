...
"#
    ...
    let state = StateRuntime::init(
        SqliteConfig::new_for_testing(sofia_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
fn standalone_app_server_emits_json_info_events() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let event = app_server_json_shutdown_event("sofia-app-server", &[], sofia_home.path())?;
...
async fn sqlite_log_metrics_exports_do_not_create_log_cycles() -> Result<()> {
    let quiet_period = sofia_state::log_db::LogSinkQueueConfig::default().flush_interval
    ...
    for status in [200, 500] {
        ...
        let _app_server = TestAppServer::builder()
            ...
            .with_env_overrides(&[
                ...
                (
                    sofia_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR,
                    Some("1"),
                ),
            ])
        ...
        let exports = timeout(export_timeout, async {
            loop {
                ...
                if requests.iter().any(|request| {
                    String::from_utf8_lossy(&request.body).contains(sofia_state::LOG_WRITE_METRIC)
                }) {
                    break requests.len();
                }
        ...
        let expected_metrics = [
            (sofia_state::LOG_WRITE_METRIC, "sum"),
            (sofia_state::LOG_WRITE_DURATION_METRIC, "histogram"),
            (sofia_state::LOG_WRITE_BYTES_METRIC, "histogram"),
            (sofia_state::LOG_WRITE_ENTRIES_METRIC, "histogram"),
            (sofia_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC, "histogram"),
        ];
        ...
        for (name, kind) in expected_metrics {
            ...
            for point in points {
                ...
                if matches!(
                    name,
                    sofia_state::LOG_WRITE_BYTES_METRIC
                        | sofia_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC
                ) {
        ...
        let write_count = observed_metrics[sofia_state::LOG_WRITE_METRIC].0;
        ...
        .await?;
        let persisted_logs = state.query_logs(&sofia_state::LogQuery::default()).await?;
        assert_eq!(
            observed_metrics[sofia_state::LOG_WRITE_ENTRIES_METRIC].1,
            persisted_logs.len() as f64,
            "exported batch sizes must equal the number of persisted SQLite log rows"
        );
        assert!(
            observed_metrics[sofia_state::LOG_WRITE_BYTES_METRIC].1
                >= observed_metrics[sofia_state::LOG_WRITE_MAX_ENTRY_BYTES_METRIC].1,
...
async fn sqlite_log_metrics_grpc_exports_do_not_create_log_cycles(grpc_status: &str) -> Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |request: &wiremock::Request| {
            let metric_name = sofia_state::LOG_WRITE_METRIC.as_bytes();
            if request
                .body
...
async fn app_server_emits_structured_tool_call_timing_event() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .with_json_logging("warn,sofia_core::tools::parallel=info")
        .build_initialized()
        .await?;
    ...
    let mut tool_call = app_server
        .wait_for_json_log_event("sofia.tool_call")
        .await?;
    ...
    assert_eq!(
        ...
        json!({
            ...
            "target": "sofia_core::tools::parallel",
        })
    );
