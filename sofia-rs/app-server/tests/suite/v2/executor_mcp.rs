...
async fn selected_executor_discovers_browser_mcp_with_executor_only_bearer_token() -> Result<()> {
    ...
    let mut executor = Command::new(sofia_utils_cargo_bin::cargo_bin("sofia")?)
        .args(["exec-server", "--listen", "ws://127.0.0.1:0"])
        .stdout(Stdio::piped())
        .kill_on_drop(true)
        .env("SOFIA_HOME", executor_home.path())
...
async fn guardian_review_does_not_discover_executor_mcp() -> Result<()> {
    ...
    let sofia_bin = toml::Value::String(
        sofia_utils_cargo_bin::cargo_bin("sofia")?
            .to_string_lossy()
            .into_owned(),
    );
...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        ...
        .write(sofia_home.path())?;
    let executor_config: sofia_config::types::McpServerConfig = serde_json::from_value(json!({
    ...
    let oauth_credentials_path = sofia_home.path().join(".credentials.json");
    std::fs::write(
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".sofia-plugin"))?;
    std::fs::write(
        plugin.path().join(".sofia-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(registration_request["client_name"], json!("Sofia"));
    assert_eq!(
