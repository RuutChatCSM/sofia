...
async fn get_account_rate_limits_returns_snapshot(
    ...
    write_chatgpt_base_url(sofia_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "sofia_other",
                "metered_feature": "sofia_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/sofia/rate-limit-reset-credits"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
async fn get_account_rate_limits_filters_banner_by_identity(
    ...
    let server = MockServer::start().await;
    write_chatgpt_base_url(sofia_home.path(), &server.uri())?;
    let banner = json!({
    ...
    let expected: GetAccountRateLimitsResponse = serde_json::from_value(json!({
        ...
        "rateLimits": snapshot, "rateLimitsByLimitId": {"sofia": snapshot},
...
async fn send_add_credits_nudge_email_posts_expected_body() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/sofia/accounts/send_add_credits_nudge_email"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
fn write_chatgpt_base_url(sofia_home: &Path, base_url: &str) -> std::io::Result<()> {
