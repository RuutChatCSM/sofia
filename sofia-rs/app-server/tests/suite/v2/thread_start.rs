...
use sofia_app_server_protocol::TextPosition;
use sofia_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = sofia_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml_without_approval_policy(sofia_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .build_initialized()
        .await?;
...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    assert_eq!(thread.originator.as_deref(), Some("sofia_work_desktop"));
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml_without_approval_policy(sofia_home.path(), &server.uri())?;
    let global_agents_path = sofia_home.path().join("AGENTS.md");
...
async fn thread_start_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(sofia_home.path(), &server.uri(), &server.uri())?;
...
async fn thread_start_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml_with_required_broken_mcp(sofia_home.path(), &server.uri())?;
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(sofia_home.path(), &server.uri())?;
...
"#,
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized()
        .await?;
    let config_before = std::fs::read_to_string(sofia_home.path().join("config.toml"))?;
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            sofia_app_server_protocol::SandboxPolicy::ReadOnly {
...
"#,
    ...
    let config_after = std::fs::read_to_string(sofia_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml(sofia_home, server_uri, "sandbox_mode = \"read-only\"", "")
}
