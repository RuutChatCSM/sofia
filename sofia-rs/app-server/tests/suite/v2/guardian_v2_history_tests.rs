...
use sofia_app_server_protocol::GuardianRiskLevel;
use sofia_app_server_protocol::GuardianUserAuthorization;
...
async fn guardians_retain_evidence_after_compaction_and_discard_it_after_rollback(
    ...
    let models = [
        ...
        ("sofia-auto-review", reviewer_hash),
        ("resumed-parent", Some("matching")),
    ]
    .into_iter()
    .map(|(model, hash)| {
        let mut info = sofia_core::test_support::construct_model_info_offline(model, &config);
        info.comp_hash = hash.map(str::to_owned);
        if model == MODEL || model == "resumed-parent" {
            info.auto_review_model_override = Some("sofia-auto-review".to_owned());
        }
        info
    })
    .collect();
    write_models_cache_with_models(sofia_home.path(), models)?;
    write_chatgpt_auth(
        sofia_home.path(),
    ...
    .enumerate()
    {
        ...
        if reject_sync_checkpoint && (1..=2).contains(&index) {
            ...
            assert_eq!(reviews[0]["model"], "sofia-auto-review");
