...
use sofia_app_server_protocol::AppConfig;
use sofia_app_server_protocol::AppLinkConfig;
use sofia_app_server_protocol::AppLinksConfig;
use sofia_app_server_protocol::AppToolApproval;
use sofia_app_server_protocol::ApprovalsReviewer;
use sofia_app_server_protocol::AppsConfig;
use sofia_app_server_protocol::AppsDefaultConfig;
use sofia_app_server_protocol::AskForApproval;
use sofia_app_server_protocol::BrowserUseAccessApprovalLifetime;
use sofia_app_server_protocol::BrowserUseConfig;
use sofia_app_server_protocol::BrowserUseOriginPolicy;
use sofia_app_server_protocol::BrowserUseOriginPolicyConfig;
use sofia_app_server_protocol::BrowserUseRequirements;
use sofia_app_server_protocol::CliAuthCredentialsStoreMode;
use sofia_app_server_protocol::ComputerUseConfig;
use sofia_app_server_protocol::ComputerUseMacosConfig;
use sofia_app_server_protocol::ComputerUseMacosRequirements;
use sofia_app_server_protocol::ComputerUseRequirements;
use sofia_app_server_protocol::ComputerUseWindowsConfig;
use sofia_app_server_protocol::ComputerUseWindowsExeConfig;
...
use sofia_app_server_protocol::ConfigLayerSource;
...
use sofia_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use sofia_app_server_protocol::ToolsV2;
...
use sofia_protocol::config_types::WebSearchContextSize;
use sofia_protocol::config_types::WebSearchLocation;
use sofia_protocol::config_types::WebSearchToolConfig;
...
fn write_config(sofia_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
async fn config_read_disables_guardian_v2_when_managed_config_requires_guardian_v1() -> Result<()> {
    let sofia_home = TempDir::new()?;
    write_config(&sofia_home, "[features]\nguardianv2 = true\n")?;
    std::fs::write(
        sofia_home.path().join("requirements.toml"),
...
"#,
    ...
    let sofia_home_path = sofia_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(sofia_home_path.join("config.toml"))?;
...
"#,
    ...
    set_project_trust_level(sofia_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let sofia_home_path = sofia_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(sofia_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("sofia")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let sofia_home = TempDir::new()?;
    write_config(&sofia_home, r#"model = "gpt-user""#)?;
...
async fn config_read_respects_managed_project_root_markers() -> Result<()> {
    let sofia_home = TempDir::new()?;
    write_config(&sofia_home, "model_context_window = 16384\n")?;
    let workspace = TempDir::new()?;
    let ancestor_config = workspace.path().join(".sofia");
    let child = workspace.path().join("child");
    let child_config = child.join(".sofia");
    for dir in [
    ...
    let mut app_server = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .with_env_overrides(&[("SOFIA_APP_SERVER_MANAGED_CONFIG_PATH", Some(&managed_path))])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let sofia_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", sofia_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(sofia_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let sofia_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(sofia_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    assert_eq!(
        ...
        AbsolutePathBuf::resolve_path_against_base("config.toml", &sofia_home)
    );
    assert_eq!(batch_write.overridden_metadata, None);

    let persisted: toml::Value =
        toml::from_str(&std::fs::read_to_string(sofia_home.join("config.toml"))?)?;
...
fn assert_layers_user_then_optional_system(
    layers: &[sofia_app_server_protocol::ConfigLayer],
