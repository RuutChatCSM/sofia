...
-> Result<()> {
    ...
    .await;
    let sofia_home = TempDir::new()?;
    mock_config(sofia_home.path(), &server.uri())?.write(sofia_home.path())?;
    let mut app = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn mock_config(sofia_home: &Path, server_uri: &str) -> Result<MockResponsesConfig> {
    ...
    let models = [MODEL_A, MODEL_B]
        ...
        .collect();
    let catalog_path = sofia_home.join("step-models.json");
    std::fs::write(
        &catalog_path,
