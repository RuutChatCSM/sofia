...
fn write_mock_config(sofia_home: &Path, server_uri: &str) -> std::io::Result<()> {
...
async fn thread_start_reports_selected_environment_metadata() -> Result<()> {
    ...
    write_mock_config(sofia_home.path(), &server.uri())?;
    ...
    let ThreadStartResponse {
        ...
        .await?;
    let host_cwd = sofia_home.path().to_path_buf().abs().canonicalize()?;
