...
const COMMIT_ATTRIBUTION: &str = "Co-authored-by: Sofia <noreply@openai.com>";
const PR_ATTRIBUTION: &str = "Generated with [Sofia](https://openai.com/sofia/).";
...
async fn git_attribution_follows_authenticated_workspace_policy() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_env_overrides(&[("OPENAI_API_KEY", None), ("SOFIA_ACCESS_TOKEN", None)])
        .build()
        .await?;
...
fn replace_attribution_fragment_with_legacy(
    ...
    let lines = rollout
        ...
        .map(|line| {
            let mut line = sofia_rollout::parse_rollout_line(line)?;
                        ...
                        && text.contains("<git_attribution>")
                    {
                        *text = match legacy_attribution {
                            ...
                            LegacyAttribution::UnlinkedPullRequest => {
                                text.replace(PR_ATTRIBUTION, "Generated with Sofia.")
                            }
                        };
                        replaced = true;
                    }
