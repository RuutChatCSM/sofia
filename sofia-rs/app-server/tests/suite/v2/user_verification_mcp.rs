...
struct HostedVerificationServer(sofia_config::McpServerConfig);
...
async fn user_verification_mcp_round_trip_requires_proof_in_full_access() -> Result<()> {
    ...
    let router = Router::new().nest_service("/api/sofia/ps/mcp", service);
    ...
    let plugin_service_config = serde_json::from_value(json!({
        "url": format!("{base_url}/api/sofia/ps/mcp"),
    }))?;
    ...
    assert_eq!(
        ...
        sofia_protocol::mcp::CallToolResult {
