...
use sofia_protocol::mcp_approval_meta as approval_meta;
...
const TOOL_NAMESPACE: &str = "mcp__sofia_apps__calendar";
...
async fn openai_form_capability_follows_the_turn_starting_connection() -> Result<()> {
    ...
    write_config_toml(sofia_home.path(), &responses_server.uri(), &apps_server_url)?;
    write_chatgpt_auth(
        sofia_home.path(),
