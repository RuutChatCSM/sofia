...
use sofia_app_server_protocol::ResponseUsageMetadata;
...
async fn auto_compaction_local_emits_started_and_completed_items() -> Result<()> {
    ...
    compaction_config(&server.uri(), AUTO_COMPACT_LIMIT).write(sofia_home.path())?;
...
async fn auto_compaction_remote_emits_started_and_completed_items() -> Result<()> {
    ...
    assert_eq!(
        ...
        compact_requests[0].header("x-sofia-window-id").as_deref()
    );
...
async fn thread_compact_start_triggers_compaction_and_returns_empty_response() -> Result<()> {
    ...
    compaction_config(&server.uri(), /*auto_compact_limit*/ 1_000_000).write(sofia_home.path())?;
