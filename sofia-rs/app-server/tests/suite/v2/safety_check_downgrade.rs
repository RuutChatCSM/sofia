...
const SERVER_MODEL: &str = "gpt-5.3-sofia";
...
async fn openai_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    create_config_toml(sofia_home.path(), &server.uri())?;
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: sofia_app_server_protocol::TurnError {
                misalignment: None,
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.sofia_error_info == Some(CodexErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
...
fn create_config_toml(sofia_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
