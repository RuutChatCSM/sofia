...
use serde_json::json;
use sofia_app_server_protocol::BedrockDiscoverParams;
use sofia_app_server_protocol::BedrockDiscoverResponse;
use sofia_app_server_protocol::BedrockSetupParams;
use sofia_app_server_protocol::BedrockSetupResponse;
...
async fn bedrock_app_server(
    ...
    let aws_config = sofia_home.join("aws-config");
    let aws_credentials = sofia_home.join("aws-credentials");
...
async fn setup_bedrock_profile_and_environment() -> Result<()> {
    ...
    assert_eq!(
        error.error.message,
        "Sofia-managed Bedrock credentials are already configured and take priority over AWS environment credentials. Run `sofia logout` and try again."
    );
...
async fn setup_bedrock_rejects_invalid_or_conflicting_credentials() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut app_server = bedrock_app_server(sofia_home.path(), &[]).await?;
    ...
    assert!(!sofia_home.path().join(".env").exists());
    assert!(!sofia_home.path().join("config.toml").exists());
