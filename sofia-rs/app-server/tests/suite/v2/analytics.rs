...
use sofia_config::types::OtelHttpProtocol;
use sofia_core::config::ConfigBuilder;
use sofia_core_plugins::loader::curated_plugin_cache_version;
...
async fn guardian_review_turns_and_tools_reach_analytics() -> Result<()> {
    ...
    let turns = children
        .iter()
        .filter(|event| event["event_type"] == "sofia_turn_event")
        .map(|event| {
            let params = &event["event_params"];
    ...
    let tools = children
        .iter()
        .filter(|event| event["event_type"] == "sofia_web_search_event")
        .collect::<Vec<_>>();
...
fn set_metrics_exporter(config: &mut sofia_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = sofia_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("sofia-app-server"),
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, sofia_home: &Path) -> Result<()> {
    Mock::given(method("POST"))
    ...
    let body = timeout(read_timeout, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/sofia/analytics-events/events"
            }) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "sofia_goal_event"
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/sofia/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("sofia_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(sofia_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(sofia_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    std::fs::write(
        &script_path,
        r#"test -n "$SOFIA_PLUGIN_METRICS_OUTPUT"
sleep "${1:-0.3}"
printf '%s' '{"version":1,"measurements":[{"name":"findings","value":3,"dimensions":{"severity":"high"}},{"name":"files_scanned","value":17}]}' > "$SOFIA_PLUGIN_METRICS_OUTPUT"
"#,
    )?;

    let curated_repo = sofia_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(remote: bool, background: bool) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local Sofia home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let sofia_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(sofia_home.path())?.canonicalize()?;
    let mut command = vec![
        "/bin/sh".to_string(),
...
"#,
    ...
    mount_analytics_capture(&analytics_server, sofia_home.path()).await?;
    ...
    if remote {
        assert_eq!(
            ...
            sofia_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "sofia_plugin_measurement_event"
    ...
    let command_event =
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "sofia_command_execution_event"
                && event["event_params"]["item_id"] == call_id
        })
        .await?;
    assert_eq!(
    ...
    for request in analytics_server
        ...
        .unwrap_or_default()
    {
        if request.method != "POST" || request.url.path() != "/sofia/analytics-events/events" {
            continue;
        }
        let payload: Value = serde_json::from_slice(&request.body)?;
        let Some(events) = payload["events"].as_array() else {
    ...
    for event in &measurement_events {
        ...
        assert_eq!(event_params["originator"], "sofia_work_desktop");
    }
