...
const RATE_LIMIT_RESET_REQUEST_TIMEOUT_ENV_VAR: &str =
    "SOFIA_TEST_RATE_LIMIT_RESET_REQUEST_TIMEOUT_MS";
...
async fn consume_rate_limit_reset_credit_requires_chatgpt_auth() -> Result<()> {
    ...
    let mut mcp = initialized_app_server(sofia_home.path()).await?;
    ...
    assert_eq!(
        ...
        "sofia account authentication required for rate limit reset credits"
    );
...
async fn consume_account_rate_limit_reset_credit_maps_backend_outcomes() -> Result<()> {
    let (sofia_home, server) = chatgpt_test_context().await?;
    let cases = [
        (
            "request-reset",
            "reset",
    ...
    for (idempotency_key, backend_code, _, windows_reset) in cases {
        Mock::given(method("POST"))
            .and(path("/api/sofia/rate-limit-reset-credits/consume"))
...
async fn consume_account_rate_limit_reset_credit_rejects_empty_idempotency_key() -> Result<()> {
    let (sofia_home, _server) = chatgpt_test_context().await?;
    let mut mcp = initialized_app_server(sofia_home.path()).await?;
...
async fn chatgpt_test_context() -> Result<(TempDir, MockServer)> {
    ...
    Ok((sofia_home, server))
}

async fn initialized_app_server(sofia_home: &Path) -> Result<TestAppServer> {
