...
use serde_json::json;
use sofia_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(sofia_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
    ...
    let image_request = server
        ...
        .into_iter()
        .find(|request| request.url.path() == "/api/sofia/images/generations")
...
async fn standalone_image_generation_failure_emits_terminal_item() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/sofia/images/generations"))
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(429)
                .insert_header("x-sofia-active-limit", "image_gen")
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let (edit_request, _) = run_image_edit_test(|sofia_home| {
        let image_path = sofia_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let sofia_home = TempDir::new()?;
    let (arguments, input) = input(sofia_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    let image_request = requests
        .iter()
        .find(|request| request.url.path() == "/api/sofia/images/edits")
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .insert_header("x-sofia-imagegen-request-id", "req-imagegen-123")
                .set_body_json(json!({
                    "created": 1,
...
async fn mount_image_edit_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/sofia/images/edits"))
...
fn create_config_toml(
    ...
    let mut config = MockResponsesConfig::new(server_uri)
        ...
        .with_provider_base_url(&format!("{server_uri}/api/sofia"))
