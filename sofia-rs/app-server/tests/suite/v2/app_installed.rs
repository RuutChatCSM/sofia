...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let sofia_home = configured_codex_home(fixture.base_url())?;
    let mut app_server = start_app_server(sofia_home.path()).await?;
...
async fn installed_apps_thread_refresh_updates_live_tools_and_retains_them_on_failure() -> Result<()>
{
    ...
    assert!(
        initial_model_request
            .single_request()
            .tool_by_name("mcp__sofia_apps__alpha", "connector_alpha")
            .is_some()
    );
    ...
    for (call_id, refresh_fails) in [("after-refresh", false), ("after-failed-refresh", true)] {
        ...
        assert!(
            requests[0]
                .tool_by_name("mcp__sofia_apps__beta", "connector_beta")
                .is_some()
        );
        assert!(
            requests[0]
