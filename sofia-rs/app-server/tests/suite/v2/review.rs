...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": sofia_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    create_config_toml(sofia_home.path(), &server.uri())?;
    let colliding_skill_dir = sofia_home.path().join("skills/review-agent-collision");
    ...
    let canonical_codex_home = std::fs::canonicalize(sofia_home.path())?.try_into()?;
