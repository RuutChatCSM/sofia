...
use sofia_app_server_protocol::QueuedSubmission;
...
use sofia_app_server_protocol::ThreadQueueAddParams;
use sofia_app_server_protocol::ThreadQueueAddResponse;
use sofia_app_server_protocol::ThreadQueueChangedNotification;
use sofia_app_server_protocol::ThreadQueueDeleteParams;
use sofia_app_server_protocol::ThreadQueueDeleteResponse;
use sofia_app_server_protocol::ThreadQueueListParams;
use sofia_app_server_protocol::ThreadQueueListResponse;
use sofia_app_server_protocol::ThreadQueueReorderParams;
use sofia_app_server_protocol::ThreadQueueReorderResponse;
use sofia_app_server_protocol::ThreadQueueStartParams;
use sofia_app_server_protocol::ThreadQueueStartResponse;
use sofia_app_server_protocol::ThreadQueueUpdateParams;
use sofia_app_server_protocol::ThreadQueueUpdateResponse;
...
async fn queue_requires_experimental_handshake() -> Result<()> {
    let (mut app, sofia_home, _server) = queue_app(Vec::new()).await?;
...
async fn idle_queue_dispatch_preserves_client_id() -> Result<()> {
    ...
    let metadata_header = request
        ...
        .context("queued model request is missing its x-sofia-turn-metadata header")?
        .to_str()
...
async fn cold_thread_resume_dispatches_a_persisted_queued_submission() -> Result<()> {
    ...
    let (mut first, sofia_home, _server) = queue_app(responses).await?;
    let thread_id = first
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn interrupt_preserves_queue_and_queue_start_can_resume_a_non_head_item() -> Result<()> {
    ...
    let (mut app, sofia_home, _server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
