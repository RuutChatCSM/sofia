...
use serde_json::json;
use sofia_app_server::in_process;
...
use sofia_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use sofia_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_can_page_backward_and_forward() -> Result<()> {
    ...
    let rollout_path = rollout_path(sofia_home.path(), filename_ts, &conversation_id);
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(sofia_home.path())?;
    let thread_id = sofia_protocol::ThreadId::default();
    let sqlite = sofia_state::SqliteConfig::new_for_testing(sofia_home.path().abs());
    let state_db =
        sofia_state::StateRuntime::init(sqlite.clone(), "mock_provider".to_string()).await?;
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: sofia_protocol::protocol::ThreadHistoryMode::Paginated,
            history_base: None,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            sofia_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let thread_id = sofia_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(sofia_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "sofia-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = sofia_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let thread_id = sofia_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    MockResponsesConfig::new("http://127.0.0.1:1")
        .with_root_config(&format!(
            r#"experimental_thread_store = {{ type = "in_memory", id = "{store_id}" }}"#
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(sofia_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<sofia_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
async fn read_turns_page(
    mcp: &mut TestAppServer,
    thread_id: sofia_protocol::ThreadId,
...
fn turn_user_texts(turns: &[sofia_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[sofia_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
