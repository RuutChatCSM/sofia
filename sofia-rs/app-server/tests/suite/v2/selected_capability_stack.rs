...
async fn selected_plugin_mcp_startup_respects_explicit_mentions(
    ...
    let config_path = fixture.sofia_home.path().join("config.toml");
    ...
    let mut app_server = TestAppServer::builder()
        .with_codex_home(fixture.sofia_home.path())
    ...
    .await?;
    let mut exec_server =
        spawn_exec_server(fixture.sofia_home.path(), &fixture.exec_server_url).await?;
...
async fn managed_plugins_requirement_disables_selected_executor_plugin_capabilities(
    ...
    std::fs::write(config_path, config)?;
    std::fs::write(
        fixture.sofia_home.path().join("requirements.toml"),
        "[features]\nplugins = false\n",
    )?;
    let response_mock = responses::mount_sse_once(
        &responses_server,
...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    exec_server = spawn_exec_server(fixture.sofia_home.path(), &fixture.exec_server_url).await?;
    add_environment(&mut app_server, &fixture.exec_server_url).await?;
    wait_for_selected_mcp_server(&mut app_server, &thread_id).await?;
...
async fn selected_capabilities_become_available_between_samples_in_one_turn(
    ...
    let stop_hook_barrier = if matches!(mention_timing, MentionTiming::InitialBeforeRestart) {
        let barrier = fixture.sofia_home.path().join("allow-stop-hook");
        let hook_path = fixture.sofia_home.path().join("wait-for-stop.py");
        std::fs::write(
            &hook_path,
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = sofia_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".sofia-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__sofia_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(sofia_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(sofia_utils_cargo_bin::cargo_bin("sofia")?)
