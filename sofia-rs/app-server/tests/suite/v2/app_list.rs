...
use serde_json::json;
use sofia_app_server_protocol::AppBranding;
...
use sofia_app_server_protocol::AppReview;
use sofia_app_server_protocol::AppScreenshot;
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    write_connectors_and_plugins_config(sofia_home.path(), &server_url)?;
    write_plugin_app_fixture(sofia_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        sofia_home.path(),
...
fn write_connectors_and_plugins_config(sofia_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = sofia_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(sofia_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {
