...
use sofia_app_server_protocol::ThreadSectionListResponse;
...
use sofia_protocol::protocol::GitInfo as RolloutGitInfo;
...
async fn thread_section_move_pins_and_unpins_with_filtered_recency_pagination() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(sofia_home.path())?;
    let state_db = init_state_db(sofia_home.path()).await?;
    ...
    for (filename_timestamp, timestamp, preview) in [
        ...
        reconcile_rollout(
            Some(&state_db),
            rollout_path(sofia_home.path(), filename_timestamp, &thread_id).as_path(),
            "mock_provider",
            /*builder*/ None,
            &[],
            /*archived_only*/ None,
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(sofia_home.path())?;
    let _state_db = init_state_db(sofia_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let rollout_path = rollout_path(sofia_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(sofia_home.path(), "2025-01-06T08-30-00", &thread_id);
...
async fn init_state_db(sofia_home: &Path) -> Result<Arc<StateRuntime>> {
