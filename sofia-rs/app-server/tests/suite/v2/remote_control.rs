...
use sofia_app_server::AppServerRuntimeOptions;
use sofia_app_server::AppServerTransport;
use sofia_app_server::AppServerWebsocketAuthSettings;
use sofia_app_server::PluginStartupTasks;
use sofia_app_server::RemoteControlStartupMode;
use sofia_app_server::run_main_with_transport_options;
...
use sofia_app_server_protocol::RemoteControlClient;
use sofia_app_server_protocol::RemoteControlClientsListOrder;
use sofia_app_server_protocol::RemoteControlClientsListParams;
use sofia_app_server_protocol::RemoteControlClientsListResponse;
use sofia_app_server_protocol::RemoteControlClientsRevokeParams;
use sofia_app_server_protocol::RemoteControlClientsRevokeResponse;
use sofia_app_server_protocol::RemoteControlConnectionStatus;
use sofia_app_server_protocol::RemoteControlDisableResponse;
use sofia_app_server_protocol::RemoteControlEnableResponse;
use sofia_app_server_protocol::RemoteControlPairingStartParams;
use sofia_app_server_protocol::RemoteControlPairingStartResponse;
use sofia_app_server_protocol::RemoteControlPairingStatusParams;
use sofia_app_server_protocol::RemoteControlPairingStatusResponse;
use sofia_app_server_protocol::RemoteControlStatusChangedNotification;
use sofia_app_server_protocol::RemoteControlStatusReadResponse;
...
use sofia_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = sofia_home.path().join("managed_config.toml");
    let socket_path = sofia_home
        .path()
        .join("app-server-control")
        .join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _codex_home_guard = EnvVarGuard::set("SOFIA_HOME", sofia_home.path().as_os_str());

    let result = timeout(
        STARTUP_TIMEOUT,
        run_main_with_transport_options(
            Arg0DispatchPaths {
                sofia_self_exe: Some(std::env::current_exe()?),
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let listener = configured_remote_control_listener(sofia_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(sofia_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(sofia_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(sofia_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(sofia_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(sofia_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_codex_home(sofia_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(sofia_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(sofia_home).await?;
...
async fn configured_remote_control_listener(sofia_home: &std::path::Path) -> Result<TcpListener> {
