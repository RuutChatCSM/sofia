...
async fn assert_standalone_web_search_round_trips_output(
    ...
    let search_path = match provider {
        WebSearchProvider::ChatGpt => "/api/sofia/alpha/search",
    ...
    let config = match provider {
        WebSearchProvider::ChatGpt => config
            ...
            .with_provider_base_url(&format!("{}/api/sofia", server.uri()))
    ...
    let turn_metadata_header = search_request
        ...
        .context("standalone search should include x-sofia-turn-metadata")?
        .to_str()
        .context("x-sofia-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-sofia-turn-metadata should be valid JSON")?;
