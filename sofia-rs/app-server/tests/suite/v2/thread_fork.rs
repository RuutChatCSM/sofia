...
use sofia_rollout::RolloutLine;
use sofia_rollout::append_rollout_item_to_path;
use sofia_rollout::append_thread_name;
...
async fn thread_fork_defers_inherited_active_goal_until_next_turn() -> Result<()> {
    ...
    assert_eq!(
        forked_goal.status,
        sofia_state::ThreadGoalStatus::BudgetLimited
    );
    assert_eq!(
        state_db
            .thread_goals()
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(sofia_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(sofia_home.path(), filename_ts, &conversation_id);
...
async fn thread_fork_creates_reference_backed_paginated_thread() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        request_body["client_metadata"]["x-sofia-turn-metadata"]
            .as_str()
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    let source_path = rollout_path(sofia_home.path(), "2025-01-05T12-00-00", &source_thread_id);
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            sofia_protocol::models::ResponseItem::Message { role, .. }
...
async fn pathless_ephemeral_thread_rejects_codex_home_path_after_reload() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .build_initialized()
        .await?;
    let sofia_home_path = sofia_home.path().to_path_buf();

    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            thread_id: side_thread_id.clone(),
            path: Some(sofia_home_path.clone()),
            ..Default::default()
        })
        .await?;
