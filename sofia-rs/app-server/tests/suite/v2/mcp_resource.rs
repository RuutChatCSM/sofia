...
use sofia_app_server::in_process::InProcessServerEvent;
...
use sofia_app_server_protocol::ProjectCreateParams;
use sofia_app_server_protocol::ProjectCreateResponse;
use sofia_app_server_protocol::ProjectDeleteParams;
...
use sofia_config::ThreadConfigContext;
use sofia_config::ThreadConfigLoader;
use sofia_config::ThreadConfigLoaderFuture;
...
use sofia_state::PINNED_THREAD_SECTION_ID;
use sofia_state::PINNED_THREAD_SECTION_NAME;
...
const TEST_RESOURCE_URI: &str = "test://sofia/resource";
pub(super) const TEST_WIDGET_RESOURCE_URI: &str = "ui://widget/checkout-session.html";
const TEST_BLOB_RESOURCE_URI: &str = "test://sofia/resource.bin";
const TEST_RESOURCE_BLOB: &str = "YmluYXJ5LXJlc291cmNl";
const TEST_RESOURCE_TEXT: &str = "Resource body from the MCP server.";
const TEST_ERROR_RESOURCE_URI: &str = "test://sofia/error";
const TEST_ELICITATION_RESOURCE_URI: &str = "test://sofia/elicitation";
...
async fn mcp_resource_read_preserves_protocol_errors(protocol: ProtocolVersion) -> Result<()> {
    ...
    let config = MockResponsesConfig::new(&responses_server.uri()).with_extra_config(&format!(
        "[mcp_servers.resource_server]\nurl = \"{apps_server_url}/api/sofia/ps/mcp\""
    ));
...
async fn orchestrator_skill_can_read_referenced_resource_without_an_executor() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "warnings": ["Orchestrator skill discovery stopped after 2 resource pages: failed to list orchestrator skill resources: resources/list failed for `sofia_apps`: Mcp error: -32603: simulated later-page failure"],
            "next_cursor": null,
        })
    );
...
async fn mcp_resource_read_returns_error_for_unknown_thread() -> Result<()> {
    ...
    let client = start_resource_in_process_client(
        sofia_home.path(),
        Arc::new(sofia_config::NoopThreadConfigLoader),
    )
    .await?;
...
async fn metadata_and_mcp_requests_complete_while_unrelated_resume_loads_config() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        .with_extra_config(&format!(
            "[mcp_servers.resource_server]\nurl = \"{mcp_server_url}/api/sofia/ps/mcp\""
        ))
    ...
    let client =
        start_resource_in_process_client(sofia_home.path(), blocked_resume.clone()).await?;
        ...
        timeout(
            ...
            sender.request(ClientRequest::ThreadResume {
                ...
                params: ThreadResumeParams {
                    thread_id,
                    cwd: Some(sofia_home.path().to_string_lossy().into_owned()),
                    ..Default::default()
                },
            }),
        )
...
async fn resume_revalidates_persisted_thread_after_config_load(
    ...
    let sofia_home = TempDir::new()?;
    MockResponsesConfig::new(&responses_server.uri()).write(sofia_home.path())?;
    ...
    let thread_id = create_fake_rollout(
        ...
        matches!(mutation, ResumeMutation::GitMetadata).then(|| {
            sofia_protocol::protocol::GitInfo {
                commit_hash: None,
    ...
    let path = rollout_path(sofia_home.path(), filename_timestamp, &thread_id);
    let resume_cwd = TempDir::new()?;
    let blocked_resume = Arc::new(BlockedResumeConfig {
        cwd: AbsolutePathBuf::from_absolute_path(resume_cwd.path())?,
        matching_loads: AtomicUsize::new(0),
...
async fn start_resource_in_process_client(
    ...
    let config = ConfigBuilder::default()
        ...
        .fallback_cwd(Some(sofia_home.to_path_buf()))
        .cli_overrides(cli_overrides.clone())
        .loader_overrides(loader_overrides.clone())
        .build()
        .await?;
    let state_db = sofia_rollout::state_db::try_init(&config).await?;
...
async fn start_resource_test_app_server_with_extra_config(
    ...
    let builder = TestAppServer::builder().with_codex_home(sofia_home.path());
...
-> Result<(String, Arc<ResourceAppsMcpCalls>, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/sofia/ps/mcp", mcp_service);
