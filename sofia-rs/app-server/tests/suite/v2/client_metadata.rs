...
use sofia_protocol::ThreadId as CoreThreadId;
...
async fn turn_start_sends_nested_subagent_lineage_after_cold_thread_resume_v2() -> Result<()> {
    ...
    let turn_event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "sofia_turn_event").await?;
    ...
    assert_eq!(
        (count("sofia_turn_event"), count("sofia_web_search_event")),
        (1, 1)
    );
...
async fn turn_steer_updates_client_metadata_on_follow_up_responses_request_v2() -> Result<()> {
    ...
    let first_metadata = requests[0]
        ...
        .as_deref()
        .map(parse_json_header)
        .expect("first x-sofia-turn-metadata header should be present");
    assert_eq!(
    ...
    let second_metadata = requests[1]
        ...
        .as_deref()
        .map(parse_json_header)
        .expect("second x-sofia-turn-metadata header should be present");
    assert_eq!(
    ...
    assert_eq!(
        (
            ...
            event["event_params"]["sofia_turn_source"].as_str(),
        ),
...
async fn turn_start_forwards_client_metadata_to_responses_websocket_request_body_v2() -> Result<()>
{
    ...
    let metadata = request["client_metadata"]["x-sofia-turn-metadata"]
        .as_str()
        .map(parse_json_header)
        .expect("websocket x-sofia-turn-metadata client metadata should be present");
    assert_eq!(
        (
            metadata["parent_response_id"].as_str(),
    ...
    assert_eq!(
        metadata["window_id"].as_str(),
        request["client_metadata"]["x-sofia-window-id"].as_str()
    );
    assert!(
        metadata["context_window_id"]
            .as_str()
