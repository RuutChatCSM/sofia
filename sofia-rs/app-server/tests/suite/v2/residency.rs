...
const PROVIDER_RESIDENCY_ENV_VAR: &str = "SOFIA_TEST_RESIDENCY_HEADER";
...
"X-OpenAI-Internal-Sofia-Residency" = "eu-static"
"x-provider-header" = "preserved"

[model_providers.custom-openai.env_http_headers]
"x-openai-internal-sofia-residency" = "{PROVIDER_RESIDENCY_ENV_VAR}"
"#
    );
...
async fn managed_residency_overrides_provider_headers(transport: ModelTransport) -> Result<()> {
    ...
    write_provider_config(sofia_home.path(), &model_base_url, transport)?;
    std::fs::write(
