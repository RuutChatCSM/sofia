...
use sofia_code_mode::GrpcCodeModeSessionProvider;
...
use sofia_analytics::AppServerRpcTransport;
...
use sofia_app_server_protocol::TextPosition as AppTextPosition;
use sofia_app_server_protocol::TextRange as AppTextRange;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLoadError;
use sofia_config::TextRange as CoreTextRange;
use sofia_core::ExecPolicyError;
...
use sofia_rollout::state_db as rollout_state_db;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Sofia rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "sofia-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "SOFIA_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub async fn run_main_with_transport_options(
    ...
    let otel = sofia_core::otel_init::build_provider(
        &config,
    ...
    sofia_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    sofia_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&sofia_home)?;
    ...
    match &transport {
        ...
        AppServerTransport::UnixSocket { socket_path } => {
            let accept_handle = start_control_socket_acceptor(
                ...
                if cfg!(windows)
                    && std::env::var_os(sofia_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV)
                        .is_some()
                {
    ...
    if let Some(metrics) = otel.as_ref().and_then(sofia_otel::OtelProvider::metrics) {
        sofia_home_metrics::spawn(&config, metrics.clone(), transport_shutdown_token.clone());
    }
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !sofia_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Sofia local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = sofia_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Sofia local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("sofia-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
