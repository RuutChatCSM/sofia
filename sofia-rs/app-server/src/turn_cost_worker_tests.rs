...
use sofia_backend_client::ApiKeyResponseCost;
...
const TURN_COST_PATH: &str = "/v1/analytics/sofia/turn-costs";
...
async fn worker_starts_with_otlp_metrics_exporter_without_log_exporter() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let sofia_home = TempDir::new().expect("temporary Sofia home");
...
async fn custom_provider_auth_failure_retries_without_auth_changes() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/analytics/sofia/turn-costs"))
...
async fn auth_manager_at(sofia_home: &std::path::Path) -> Arc<AuthManager> {
    Arc::new(
        AuthManager::new(
