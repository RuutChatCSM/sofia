...
use sofia_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# Sofia user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# Sofia user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn psp_feature_configures_first_party_routing() -> Result<()> {
    ...
    assert!(config.features.enabled(sofia_features::Feature::Psp));
    assert_eq!(
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                sofia_app_server_protocol::ConfigEdit {
...
async fn load_default_config_preserves_managed_requirements_and_selected_user_config_path() {
    ...
    assert_eq!(
        config
            ...
            .requirements()
            .managed_auth_policy(),
        sofia_config::ManagedAuthPolicy {
            allowed_login_methods: Some(vec![sofia_protocol::config_types::ForcedLoginMethod::Api]),
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    let auth_manager = sofia_login::AuthManager::shared_from_config(
    ...
    assert!(
        auth_manager.is_login_method_allowed(sofia_protocol::config_types::ForcedLoginMethod::Api)
    );
    assert!(
        !auth_manager
            .is_login_method_allowed(sofia_protocol::config_types::ForcedLoginMethod::Chatgpt)
    );
    assert_eq!(
