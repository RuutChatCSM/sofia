...
use sofia_cloud_config::cloud_config_bundle_loader;
...
use sofia_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective Sofia configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn sofia_home(&self) -> &Path {
        self.sofia_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.sofia_home())
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            .sofia_home(self.sofia_home.clone())
            .cli_overrides(self.current_cli_overrides())
            .loader_overrides(loader_overrides)
            .fallback_cwd(Some(self.sofia_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = sofia_core::config::ConfigBuilder::default()
            .sofia_home(self.sofia_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.sofia_self_exe = self.arg0_paths.sofia_self_exe.clone();
        config.sofia_linux_sandbox_exe = self.arg0_paths.sofia_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(sofia_home: PathBuf) -> Self {
        Self::new_for_tests(
            sofia_home,
            Vec::new(),
