...
use sofia_app_server_protocol::ThreadQueueChangedNotification;
...
use sofia_extension_api::AgentSpawnFuture;
use sofia_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn sofia_skills_extension::SkillProvider>,
    ...
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CodexErr> + 'static,
{
    ...
    if let Some(queue_service) = queue_service {
        sofia_queue_extension::install(&mut builder, queue_service);
    }
    sofia_history_notes_extension::install(&mut builder, auth_manager.clone());
    if let Some(state_db) = state_db {
        sofia_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            sofia_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(sofia_features::Feature::Goals),
    ...
    sofia_guardian_v2::install(
        &mut builder,
        guardian_agent_spawner,
        auth_manager.clone(),
        thread_manager,
    );
    sofia_memories_extension::install(&mut builder, sofia_otel::global());
    sofia_mcp_extension::install(&mut builder);
    sofia_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    sofia_web_search_extension::install(&mut builder, auth_manager.clone());
    sofia_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
        Some(config.sofia_home.clone())
    });
    let skill_providers = sofia_skills_extension::SkillProviders::new()
        .with_executor_provider(executor_skill_provider)
        .with_orchestrator_provider(Arc::new(
            sofia_skills_extension::OrchestratorSkillProvider::new(),
        ))
        .with_host_provider(Arc::new(sofia_skills_extension::HostSkillProvider::new()));
    sofia_skills_extension::install_with_providers_and_metrics(
        &mut builder,
        skill_providers,
        sofia_otel::global(),
        |config: &Config| sofia_skills_extension::SkillsExtensionConfig {
...
mod tests {
    ...
    use sofia_protocol::protocol::ThreadGoal as CoreThreadGoal;
    use sofia_protocol::protocol::ThreadGoalStatus;
    use sofia_protocol::protocol::ThreadGoalUpdatedEvent;
    use tokio::sync::mpsc;
