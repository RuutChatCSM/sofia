//! Measures local SOFIA_HOME storage once at standalone app-server startup.
//!
...
const SIZE_BYTES_METRIC: &str = "sofia.app_server.sofia_home.size_bytes";
...
pub(crate) fn spawn(
    ...
    tokio::task::spawn_blocking(move || {
        let sizes = match directory_sizes(&sofia_home, &shutdown) {
            Ok(sizes) => sizes,
            Err(error) => {
                tracing::debug!(error_kind = ?error.kind(), "Skipping SOFIA_HOME size metrics");
                return;
            }
        };
        for (directory, bytes) in [
            ("sofia_home", sizes.sofia_home),
...
fn directory_sizes(sofia_home: &Path, shutdown: &CancellationToken) -> io::Result<DirectorySizes> {
    let sessions = sofia_home.join(SESSIONS_SUBDIR);
    let archived_sessions = sofia_home.join(ARCHIVED_SESSIONS_SUBDIR);
    let mut sizes = DirectorySizes::default();
    let mut pending = vec![sofia_home.to_path_buf()];
    while let Some(directory) = pending.pop() {
        ...
        for entry in std::fs::read_dir(directory)? {
            ...
            if metadata.is_dir() {
                ...
                sizes.sofia_home = sizes.sofia_home.saturating_add(bytes);
