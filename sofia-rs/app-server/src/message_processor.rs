...
use sofia_app_server_protocol::ExperimentalApi;
...
use sofia_app_server_protocol::experimental_required_message;
...
use sofia_queue_extension::QueuedItemService;
use sofia_rollout::StateDbHandle;
use sofia_state::log_db::LogDbLayer;
use sofia_thread_store::LocalQueueStore;
use sofia_thread_store::QueueStore;
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = sofia_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let executor_skill_provider: Arc<dyn sofia_skills_extension::SkillProvider> = Arc::new(
            sofia_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                sofia_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let sofia_request = deserialize_client_request(request);
                let result = match sofia_request {
                    Ok(sofia_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = sofia_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = sofia_request.experimental_reason()
        ...
        let event_stream_ready = match &sofia_request {
        ...
        let serialization_scope = sofia_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: sofia_request.id().clone(),
        };
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match sofia_request {
            ...
            ClientRequest::McpServerEventStreamStart { params, .. } => {
                ...
                session
                    ...
                    .await?;
                Ok(Some(
                    sofia_app_server_protocol::McpServerEventStreamStartResponse {}.into(),
                ))
            }
            ClientRequest::McpServerEventStreamStop { params, .. } => {
                session
                    ...
                    .await;
                Ok(Some(
                    sofia_app_server_protocol::McpServerEventStreamStopResponse {}.into(),
                ))
            }
