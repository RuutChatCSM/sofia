use sofia_app_server_protocol::ConfigLayer as ApiConfigLayer;
use sofia_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use sofia_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use sofia_config::ConfigLayer;
use sofia_config::ConfigLayerMetadata;
use sofia_config::ConfigLayerSource;

/// Converts a config-layer source owned by `sofia-config` into the app-server wire type owned by
/// `sofia-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `sofia-config` into the app-server wire type owned by
/// `sofia-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `sofia-config` into the app-server wire type owned by
/// `sofia-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
