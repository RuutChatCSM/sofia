...
use sofia_connectors::ConnectorRuntimeTool;
use sofia_connectors::connector_runtime_context_key;
use sofia_connectors::connector_tool_is_synthetic;
use sofia_connectors::installed_connector_runtime;
...
use sofia_mcp::host_owned_codex_apps_enabled;
use sofia_mcp::tool_is_model_visible;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "sofia.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "sofia.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "sofia.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "sofia.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.sofia_home.to_path_buf(), cache_key.clone());
            ...
            let tools = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    ...
                    anyhow::ensure!(
                        ...
                        "host-owned MCP server '{SOFIA_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let runtime = McpRuntime::new(McpRuntimeInput {
                        ...
                        sofia_apps_tools_cache_key: cache_key.clone(),
                        ...
                        .await
                    {
                        ...
                        Err(anyhow::anyhow!(
                            "failed to refresh tools for MCP server '{SOFIA_APPS_MCP_SERVER_NAME}'"
                        ))
                    };
