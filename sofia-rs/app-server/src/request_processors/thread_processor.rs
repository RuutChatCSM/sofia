...
use sofia_app_server_protocol::SelectedCapabilityRoot;
use sofia_app_server_protocol::ThreadHistoryMode as ApiThreadHistoryMode;
use sofia_app_server_protocol::ThreadRevertParams;
use sofia_app_server_protocol::ThreadRevertResponse;
use sofia_app_server_protocol::ThreadRevertedNotification;
use sofia_app_server_protocol::ThreadSection;
use sofia_app_server_protocol::ThreadSectionAppearance;
use sofia_app_server_protocol::ThreadSectionMoveParams;
use sofia_app_server_protocol::ThreadSectionMoveResponse;
use sofia_extension_api::ExtensionDataInit;
use sofia_extension_api::ThreadIdleCause;
use sofia_protocol::SanitizedGitUrl;
...
const SOFIA_TUI_CLIENT_NAME: &str = "sofia-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: sofia_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                sofia_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = sofia_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(SOFIA_TUI_CLIENT_NAME) {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            sofia_home: self.config.sofia_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(sofia_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<sofia_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<sofia_app_server_protocol::ThreadStartSource>,
        ...
        let effective_permissions_trust_project = match &effective_permission_profile {
            sofia_protocol::models::PermissionProfile::Disabled
            | sofia_protocol::models::PermissionProfile::External { .. } => true,
            sofia_protocol::models::PermissionProfile::Managed { .. } => {
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                sofia_core::config::set_project_trust_level(
                    &listener_task_context.sofia_home,
        ...
        if let Ok(Some(err)) =
            sofia_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let new_thread = listener_task_context
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(sofia_app_server_protocol::ThreadStartSource::Startup)
                {
                    sofia_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    sofia_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<sofia_app_server_protocol::AskForApproval>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(sofia_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(sofia_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            sofia_linux_sandbox_exe: self.arg0_paths.sofia_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = sofia_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.sofia_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.sofia_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(sofia_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
    ...
    async fn reload_paginated_thread(
        ...
        let thread_state = self.thread_state_manager.thread_state(thread_id).await;
        self.ensure_listener_task_running(thread_id, Arc::clone(&sofia_thread), thread_state)
            .await?;
        let mut thread = self
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                sofia_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<sofia_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = sofia_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = sofia_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = sofia_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
    ...
    async fn read_stored_thread_for_resume(
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    sofia_rollout::plain_rollout_path(requested_path).as_path(),
                    sofia_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `sofia unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(sofia_core::util::normalize_thread_name);
        let mut prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    sofia_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    sofia_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => sofia_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(sofia_thread_store::PrepareForkParams {
...
fn deserialize_stored_thread_item(
    item: sofia_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        sofia_error_info: error.sofia_error_info,
...
) -> (Thread, Option<sofia_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<sofia_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(sofia_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match sofia_core::parse_turn_item(&item.item) {
                Some(sofia_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<sofia_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
