...
use sofia_core::config::permission_profile_catalog;
use sofia_hooks::HookListEntryHandler;
...
) -> Vec<sofia_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            sofia_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    sofia_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| sofia_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[sofia_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[sofia_skills::SkillError],
) -> Vec<sofia_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| sofia_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
                ...
                async move {
                    let config = match self.load_latest_config(Some(cwd.clone())).await {
                        ...
                        Err(error) => {
                            ...
                            return (
                                index,
                                sofia_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![sofia_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                    ...
                    let skills_input = sofia_skills_extension::HostSkillsLoadInput::new(
                        config.cwd.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(sofia_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![sofia_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let plugin_hooks = if hooks_enabled && config.features.enabled(Feature::Plugins) {
                ...
                sofia_core_plugins::PluginHookLoadOutcome {
                ...
                sofia_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = sofia_hooks::list_hooks(sofia_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.sofia_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
