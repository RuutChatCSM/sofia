...
use sofia_app_server_protocol::AllowDenyRequirement;
use sofia_app_server_protocol::AutoReviewRequirements;
use sofia_app_server_protocol::BrowserUseAccessApprovalLifetime;
use sofia_app_server_protocol::BrowserUseOriginPolicy;
use sofia_app_server_protocol::BrowserUseRequirements;
use sofia_app_server_protocol::CliAuthCredentialsStoreMode;
use sofia_app_server_protocol::ClientResponsePayload;
use sofia_app_server_protocol::ComputerUseMacosRequirements;
use sofia_app_server_protocol::ComputerUseRequirements;
use sofia_app_server_protocol::ComputerUseWindowsExeRequirement;
use sofia_app_server_protocol::ComputerUseWindowsRequirements;
use sofia_app_server_protocol::ConfigBatchWriteParams;
use sofia_app_server_protocol::ConfigReadParams;
use sofia_app_server_protocol::ConfigReadResponse;
use sofia_app_server_protocol::ConfigRequirements;
use sofia_app_server_protocol::ConfigRequirementsReadResponse;
use sofia_app_server_protocol::ConfigValueWriteParams;
use sofia_app_server_protocol::ConfigWriteErrorCode;
use sofia_app_server_protocol::ConfigWriteResponse;
use sofia_app_server_protocol::ConfiguredHookHandler;
use sofia_app_server_protocol::ConfiguredHookMatcherGroup;
use sofia_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use sofia_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use sofia_app_server_protocol::FeedbackRequirements;
use sofia_app_server_protocol::InAppBrowserRequirements;
use sofia_app_server_protocol::JSONRPCErrorError;
use sofia_app_server_protocol::ManagedHooksRequirements;
use sofia_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use sofia_app_server_protocol::ModelsRequirements;
use sofia_app_server_protocol::NetworkDomainPermission;
use sofia_app_server_protocol::NetworkRequirements;
use sofia_app_server_protocol::NetworkUnixSocketPermission;
...
use sofia_config::HookEventsToml;
use sofia_config::HookHandlerConfig as CoreHookHandlerConfig;
use sofia_config::ManagedHooksRequirementsToml;
use sofia_config::MatcherGroup as CoreMatcherGroup;
use sofia_config::ResidencyRequirement as CoreResidencyRequirement;
use sofia_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use sofia_core::ThreadManager;
use sofia_features::Feature;
use sofia_features::canonical_feature_for_key;
use sofia_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<sofia_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = sofia_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        application: requirements.application.map(|application| {
            sofia_app_server_protocol::ApplicationRequirements {
                network: application.network.map(|network| {
                    sofia_app_server_protocol::ApplicationNetworkRequirements {
        ...
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                sofia_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                sofia_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                sofia_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                sofia_config::types::AuthCredentialsStoreMode::Ephemeral => {
        ...
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(sofia_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(sofia_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            sofia_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            sofia_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: sofia_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: sofia_config::BrowserUseRequirementsToml,
...
fn map_browser_use_origin_policy_to_api(
    policy: sofia_config::BrowserUseOriginPolicyToml,
...
fn map_allow_deny_requirement_to_api(
    requirement: sofia_config::AllowDenyRequirementToml,
) -> AllowDenyRequirement {
    match requirement {
        sofia_config::AllowDenyRequirementToml::Allow => AllowDenyRequirement::Allow,
        sofia_config::AllowDenyRequirementToml::Deny => AllowDenyRequirement::Deny,
    }
}

fn map_browser_use_access_approval_lifetime_to_api(
    lifetime: sofia_config::BrowserUseAccessApprovalLifetimeToml,
) -> BrowserUseAccessApprovalLifetime {
    match lifetime {
        sofia_config::BrowserUseAccessApprovalLifetimeToml::Turn => {
            BrowserUseAccessApprovalLifetime::Turn
        }
        sofia_config::BrowserUseAccessApprovalLifetimeToml::Thread => {
...
) -> sofia_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => sofia_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: sofia_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(sofia_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(sofia_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(sofia_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: sofia_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        sofia_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        sofia_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: sofia_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        sofia_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        sofia_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use sofia_app_server_protocol::FeedbackRequirements;
    use sofia_app_server_protocol::WindowsSandboxSetupMode;
    use sofia_config::AllowDenyRequirementToml;
    use sofia_config::AutoReviewRequirementsToml;
    use sofia_config::BrowserUseAccessApprovalLifetimeToml;
    use sofia_config::BrowserUseOriginPolicyToml;
    use sofia_config::BrowserUseRequirementsToml;
    use sofia_config::ComputerUseMacosRequirementsToml;
    use sofia_config::ComputerUseRequirementsToml;
    use sofia_config::ComputerUseWindowsExeRequirementToml;
    use sofia_config::ComputerUseWindowsRequirementsToml;
    use sofia_config::ConfigRequirementsToml;
    use sofia_config::ModelsRequirementsToml;
    use sofia_config::NewThreadModelDefaultsToml;
    use sofia_config::WindowsRequirementsToml;
