...
//! the configured Sofia executable as a subprocess, accepts only valid JSON from
//! `sofia doctor --json --feedback`, derives a small set of Sentry tags, and otherwise
...
use sofia_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `sofia-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `sofia --cd <workspace> doctor --json --feedback` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn Sofia, finish before the timeout, or parse JSON means the
...
pub(crate) async fn doctor_feedback_report(
    ...
    let mut command = doctor_command(&executable, workspace, config.sofia_home.as_path());
...
fn doctor_command(executable: &Path, cwd: &Path, sofia_home: &Path) -> Command {
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        let sofia_home = tempdir().expect("sofia home");
        let workspace = sofia_home.path().join("deleted-workspace");

        let command = doctor_command(Path::new("sofia"), &workspace, sofia_home.path());
        let command = command.as_std();

        assert_eq!(command.get_current_dir(), Some(sofia_home.path()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            [("SOFIA_HOME".as_ref(), Some(sofia_home.path().as_os_str()))]
        );
    }
