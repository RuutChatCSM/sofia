...
use sofia_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn thread_goal_set_inner(
        ...
        let persist_result = match self.thread_manager.get_thread(thread_id).await {
            Ok(thread) => match thread.rollout_path() {
                Some(path) if sofia_rollout::existing_rollout_path(&path).await.is_none() => {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            let rollout_path = sofia_rollout::find_thread_path_by_id_str(
            ...
            if matches!(access, GoalAccess::Mutate) {
                let session_meta = sofia_rollout::read_session_meta_line(&rollout_path)
                    .await
                    .map_err(|err| {
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => sofia_rollout::find_thread_path_by_id_str(
                &self.config.sofia_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
        ...
        if let Ok(Some(metadata)) = state_db.get_thread(thread_id).await
            && sofia_rollout::plain_rollout_path(metadata.rollout_path.as_path())
                == sofia_rollout::plain_rollout_path(rollout_path.as_path())
            && let Some(existing_path) =
                sofia_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
            && sofia_rollout::read_session_meta_line(existing_path.as_path())
                .await
...
pub(super) fn api_thread_goal_from_state(goal: sofia_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: sofia_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
