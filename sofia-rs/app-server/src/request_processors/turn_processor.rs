...
use super::*;
use sofia_agent_extension::AgentInvocation;
use sofia_agent_extension::AgentRun;
use sofia_agent_extension::AgentRunner;
...
use sofia_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use sofia_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
use sofia_protocol::protocol::TurnSettingsUpdate;
use sofia_protocol::protocol::TurnSettingsUpdateOutcome;
use sofia_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<sofia_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = sofia_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            if config_snapshot.is_primary_environment_configured() {
                sofia_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<sofia_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(sofia_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(sofia_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(sofia_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != sofia_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            sofia_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            sofia_protocol::protocol::NonSteerableTurnKind::Compact => (
        ...
        self.submit_core_op(
            ...
            Op::RealtimeConversationStart(ConversationStartParams {
                ...
                flush_transcript_tail_on_session_end: params
                    ...
                    .unwrap_or(false),
                sofia_responses_as_items: params.sofia_responses_as_items.unwrap_or(false),
                sofia_response_item_prefix: params.sofia_response_item_prefix,
                sofia_response_handoff_mode: params.sofia_response_handoff_mode.unwrap_or_default(),
                sofia_response_handoff_channel_prefixes: params
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            sofia_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.sofia_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(
