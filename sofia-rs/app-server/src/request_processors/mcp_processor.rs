...
use sofia_mcp::ReadResourceRequestParams;
...
impl McpRequestProcessor {
    ...
    async fn mcp_server_oauth_login_response(
        ...
        let effective_servers = sofia_mcp::effective_mcp_servers(&mcp_config, auth.as_ref());
        ...
        let discovered_scopes = if scopes.is_none() && server.scopes.is_none() {
            discover_supported_scopes(
                ...
                sofia_rmcp_client::OAuthDiscoveryTimeout::Requested,
                redirect_mode,
            )
            .await
        } else {
    ...
    async fn list_mcp_server_status_response(
        request_id: String,
        params: ListMcpServerStatusParams,
        mcp_config: sofia_mcp::McpConfig,
        auth: Option<CodexAuth>,
        runtime_context: McpRuntimeContext,
        mcp_manager: Arc<McpManager>,
        thread: Option<Arc<sofia_core::CodexThread>>,
        ...
        let snapshot = collect_mcp_server_status_snapshot_with_detail(
            ...
            request_id,
            runtime_context,
            mcp_manager.sofia_apps_tools_cache(),
    ...
    async fn read_mcp_resource(
        ...
        if let Some(thread_id) = thread_id {
            ...
            tokio::spawn(async move {
                let origin_call_id =
                    origin_call_id.filter(|_| server == sofia_mcp::SOFIA_APPS_MCP_SERVER_NAME);
        ...
        let mcp_manager = self.thread_manager.mcp_manager();
        let mcp_config = mcp_manager.runtime_config(&config).await;
        let sofia_apps_tools_cache = mcp_manager.sofia_apps_tools_cache();
...
fn mcp_operation_error(error: anyhow::Error) -> JSONRPCErrorError {
    match sofia_rmcp_client::mcp_error(&error) {
