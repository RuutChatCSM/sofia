...
use sofia_app_server_protocol::GetAccountRateLimitsParams;
use sofia_login::LoginOnboardingEntrypoint;
use sofia_login::login_with_bedrock_access_keys;
use sofia_model_provider::is_supported_amazon_bedrock_region;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "SOFIA_APP_SERVER_LOGIN_ISSUER";
// The development success-page redirect remains debug-only.
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "SOFIA_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Sofia => LoginSuccessPageBrand::Sofia,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: SOFIA_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Sofia open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
                ...
                self.login_chatgpt_v2(request_id, sofia_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_common(
            ...
            && !open_app_url.trim().is_empty()
        {
            *url = open_app_url
                .parse()
                .map_err(|err| internal_error(format!("invalid Sofia open app URL: {err}")))?;
        }
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(sofia_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(sofia_streamlined_login, login_success_page)
            .await?;
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "sofia account authentication required to read rate limits",
            ));
        };
        ...
        let usage_request = async {
            if params.supports_luna_reserve
                && auth.auth_mode() == sofia_protocol::auth::AuthMode::Chatgpt
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch sofia rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch sofia rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "sofia".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "sofia account authentication required to read token usage",
            ));
        };
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(sofia_app_server_protocol::ThreadUsage {
                    ...
                    groups: usage
                        ...
                        .into_iter()
                        .map(
                            |group| sofia_app_server_protocol::ThreadUsageBreakdownGroup {
                                model: group.model,
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "sofia account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "sofia account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    ...
    use sofia_backend_client::TokenUsageProfileDailyBucket;
    use sofia_backend_client::TokenUsageProfileStats;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/sofia/workspace-messages".to_string(),
                status,
