        ...
        tokio::spawn(async move {
            ...
            let setup_result = async {
                ...
                if setup_mode == CoreWindowsSandboxSetupMode::Elevated
                    ...
                    && !sofia_login::is_workload_identity_selected()
                {
                    let provisioning = match config
                        ...
                        .map_or_else(
                            || {
                                Ok((
                                    sofia_windows_sandbox::WindowsSandboxProvisioningSettings::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                    sofia_windows_sandbox::WindowsSandboxProxyListeners::from_environment(
                                        &setup_request.permission_profile,
                                        &setup_request.env_map,
                                    ),
                                ))
                            },
                            sofia_core::config::NetworkProxySpec::windows_sandbox_proxy_listeners,
                        )
                    {
                    ...
                    if let Some((settings, listeners)) = provisioning {
                        ...
                        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
                            if sofia_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                            ...
                            sofia_windows_sandbox::provision_windows_sandbox_via_service(
                                &service_setup_request.sofia_home,
                                settings,
                                listeners,
                            )?;
                            Ok(())
                        ...
                        .inspect_err(|error| {
                            sofia_core::windows_sandbox::emit_windows_sandbox_setup_failure_metrics(
                                setup_mode,
                ...
                sofia_core::windows_sandbox::run_windows_sandbox_setup(setup_request).await
            }
            .await;
...
fn resolve_allowed_windows_sandbox_setup_mode(
    requirements: &sofia_config::ConfigRequirements,
...
fn determine_windows_sandbox_readiness(config: &Config) -> WindowsSandboxReadinessResponse {
    ...
    determine_windows_sandbox_readiness_from_state(
        ...
        sandbox_setup_is_complete(config.sofia_home.as_path()),
    )
}
