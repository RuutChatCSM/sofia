...
use sofia_app_server_protocol::Config as ApiConfig;
...
use sofia_app_server_protocol::OverriddenMetadata;
use sofia_app_server_protocol::WriteStatus;
use sofia_config::CONFIG_TOML_FILE;
use sofia_config::ConfigLayerEntry;
use sofia_config::ConfigLayerMetadata;
use sofia_config::ConfigLayerSource;
use sofia_config::ConfigLayerStack;
use sofia_config::ConfigRequirementsToml;
use sofia_config::ShellEnvironmentPolicyFilterRepresentation;
use sofia_config::config_toml::ConfigToml;
use sofia_config::is_structured_feature_path;
use sofia_config::merge_toml_values;
use sofia_config::shell_environment_filter_entry;
use sofia_config::validate_shell_environment_policy_filter_config;
use sofia_core::config::deserialize_config_toml_with_base;
use sofia_core::config::edit::ConfigEdit;
use sofia_core::config::edit::ConfigEditsBuilder;
use sofia_core::config::validate_feature_requirements_for_config_toml;
use sofia_core::path_utils;
use sofia_core::path_utils::SymlinkWritePaths;
use sofia_core::path_utils::resolve_symlink_write_paths;
use sofia_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.sofia_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .sofia/ folders because there is no cwd/project root
