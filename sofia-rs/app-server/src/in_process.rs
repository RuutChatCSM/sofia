...
//!
//! # Relationship to `sofia-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `sofia-app-server-client`,
...
use sofia_app_server_protocol::AgentMessageDelivery;
...
pub use sofia_rollout::StateDbHandle;
pub use sofia_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `sofia-app-server-client`, which adds worker-task buffering,
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.sofia_home).await?;
    let auth_manager =
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.sofia_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(sofia_home.path()).await);
        let state_db = sofia_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "sofia-in-process-test".to_string(),
                    title: None,
        ...
        client._test_codex_home = Some(sofia_home);
        client
    }
