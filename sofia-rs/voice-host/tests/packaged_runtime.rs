...
#[tokio::test]
#[ignore = "requires SOFIA_TEST_VOICE_RUNTIME containing real prepared native libraries"]
async fn relocated_runtime_initializes_closes_and_rejects_duplicate_initialization() -> Result<()> {
    let source = std::env::var_os("SOFIA_TEST_VOICE_RUNTIME")
        .context("set SOFIA_TEST_VOICE_RUNTIME to a matching prepared runtime")?;
    ...
    let runtime = root.join("sofia-resources/voice");
    ...
    let helper_source = cargo_bin("sofia-voice-host")?;
    ...
    let app_name = if cfg!(windows) { "sofia.exe" } else { "sofia" };
