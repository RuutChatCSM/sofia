...
use sofia_realtime_webrtc::SessionDescription;
use sofia_realtime_webrtc::VoiceHost;
...
async fn build_commit() -> Result<String> {
    let output = timeout(
        DEADLINE,
        Command::new(cargo_bin("sofia-voice-host")?)
            .arg("--build-commit")
            .kill_on_drop(true)
            .output(),
    )
...
fn install_startup_libraries(runtime: &Path) -> Result<()> {
    ...
    let source = if sofia_utils_cargo_bin::runfiles_available() {
        let resource = format!(
            "../../third_party/voice/native_link_{}_{}/runtime.json",
            std::env::consts::OS,
            std::env::consts::ARCH
        );
        sofia_utils_cargo_bin::find_resource!(resource)?
            .parent()
            .context("runtime parent")?
            .to_owned()
    } else {
        std::env::var_os("SOFIA_TEST_VOICE_RUNTIME")
            .map(PathBuf::from)
...
fn install_helper(root: &Path) -> Result<CodexPackageLayout> {
    ...
    let helper_dir = root.join("sofia-resources/voice/bin");
    fs::create_dir_all(&bin)?;
    fs::create_dir_all(&helper_dir)?;
    fs::write(root.join("sofia-package.json"), "{}")?;
    let app = bin.join(if cfg!(windows) { "sofia.exe" } else { "sofia" });
    fs::write(&app, [])?;
    let source = cargo_bin("sofia-voice-host")?;
...
async fn installed_client_rejects_mixed_builds_and_missing_helper() -> Result<()> {
    ...
    let helper = directory
        .path()
        .join("sofia-resources/voice/bin")
