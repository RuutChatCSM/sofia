...
`sofia-voice-host` establishes the inherited-pipe lifecycle for the proposed
...
`sofia-resources/voice/bin/sofia-voice-host[.exe]`, filters the child environment,
and owns process cleanup through `sofia-utils-pty`. Its runtime must remain alive
...
into ordinary Sofia. The existing `libloading` dependency supplies OS loading.
...
the host platform. From `sofia-rs`, run
`SOFIA_TEST_VOICE_RUNTIME=/absolute/prepared/runtime just test -p sofia-voice-host --test packaged_runtime --run-ignored all`.
