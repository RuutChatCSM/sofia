...
use sofia_extension_items::image_generation::ImageGenerationItem;
use sofia_protocol::items::parse_hook_prompt_message;
use sofia_protocol::protocol::AgentMessageEvent;
use sofia_protocol::protocol::AgentReasoningEvent;
use sofia_protocol::protocol::AgentReasoningRawContentEvent;
use sofia_protocol::protocol::AgentStatus;
use sofia_protocol::protocol::ApplyPatchApprovalRequestEvent;
use sofia_protocol::protocol::ContextCompactedEvent;
use sofia_protocol::protocol::DynamicToolCallResponseEvent;
use sofia_protocol::protocol::ErrorEvent;
use sofia_protocol::protocol::EventMsg;
use sofia_protocol::protocol::ExecCommandBeginEvent;
use sofia_protocol::protocol::ExecCommandEndEvent;
use sofia_protocol::protocol::GuardianAssessmentEvent;
use sofia_protocol::protocol::GuardianAssessmentStatus;
use sofia_protocol::protocol::ImageGenerationBeginEvent;
use sofia_protocol::protocol::ImageGenerationEndEvent;
use sofia_protocol::protocol::ItemCompletedEvent;
use sofia_protocol::protocol::ItemStartedEvent;
use sofia_protocol::protocol::McpToolCallBeginEvent;
use sofia_protocol::protocol::McpToolCallEndEvent;
use sofia_protocol::protocol::PatchApplyBeginEvent;
use sofia_protocol::protocol::PatchApplyEndEvent;
...
use sofia_protocol::protocol::ViewImageToolCallEvent;
use sofia_protocol::protocol::WebSearchBeginEvent;
use sofia_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use sofia_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
use sofia_rollout::CompactedItem;
...
#[cfg(test)]
use sofia_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use sofia_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `sofia-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &sofia_protocol::models::ResponseItem) {
        let sofia_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &sofia_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            sofia_protocol::items::TurnItem::EnteredReviewMode(_)
                | sofia_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            sofia_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            sofia_protocol::items::TurnItem::HookPrompt(_)
            | sofia_protocol::items::TurnItem::FunctionCallOutput(_)
            | sofia_protocol::items::TurnItem::CommandExecution(_)
            | sofia_protocol::items::TurnItem::DynamicToolCall(_)
            | sofia_protocol::items::TurnItem::CollabAgentToolCall(_)
            | sofia_protocol::items::TurnItem::SubAgentActivity(_)
            | sofia_protocol::items::TurnItem::Extension(_)
            | sofia_protocol::items::TurnItem::EnteredReviewMode(_)
            | sofia_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            sofia_protocol::items::TurnItem::UserMessage(_)
            | sofia_protocol::items::TurnItem::AgentMessage(_)
            | sofia_protocol::items::TurnItem::Reasoning(_)
            | sofia_protocol::items::TurnItem::WebSearch(_)
            | sofia_protocol::items::TurnItem::ImageView(_)
            | sofia_protocol::items::TurnItem::ImageGeneration(_)
            | sofia_protocol::items::TurnItem::FileChange(_)
            | sofia_protocol::items::TurnItem::McpToolCall(_)
            | sofia_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &sofia_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &sofia_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &sofia_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &sofia_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &sofia_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &sofia_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &sofia_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &sofia_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &sofia_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &sofia_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &sofia_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &sofia_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &sofia_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &sofia_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                sofia_error_info: payload.sofia_error_info.clone().map(Into::into),
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            sofia_error_info: error.sofia_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use sofia_extension_items::ExtensionItem as CoreExtensionItem;
    use sofia_extension_items::sleep::SleepItem as CoreSleepItem;
    use sofia_protocol::ThreadId;
    use sofia_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use sofia_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use sofia_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use sofia_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use sofia_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use sofia_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use sofia_protocol::items::SubAgentActivityItem as CoreSubAgentActivityItem;
    use sofia_protocol::items::TurnItem as CoreTurnItem;
    use sofia_protocol::items::UserMessageItem as CoreUserMessageItem;
    use sofia_protocol::items::build_hook_prompt_message;
    use sofia_protocol::mcp::CallToolResult;
    use sofia_protocol::models::ImageDetail;
    use sofia_protocol::models::MessagePhase as CoreMessagePhase;
    use sofia_protocol::models::WebSearchAction as CoreWebSearchAction;
    use sofia_protocol::parse_command::ParsedCommand;
    use sofia_protocol::protocol::AgentReasoningEvent;
    use sofia_protocol::protocol::AgentReasoningRawContentEvent;
    use sofia_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use sofia_protocol::protocol::CodexErrorInfo;
    use sofia_protocol::protocol::DynamicToolCallResponseEvent;
    use sofia_protocol::protocol::EnteredReviewModeEvent;
    use sofia_protocol::protocol::ExecCommandBeginEvent;
    use sofia_protocol::protocol::ExecCommandEndEvent;
    use sofia_protocol::protocol::ExecCommandSource;
    use sofia_protocol::protocol::ExitedReviewModeEvent;
    use sofia_protocol::protocol::ItemStartedEvent;
    use sofia_protocol::protocol::McpInvocation;
    use sofia_protocol::protocol::McpToolCallEndEvent;
    use sofia_protocol::protocol::PatchApplyBeginEvent;
    use sofia_protocol::protocol::ReviewTarget;
    use sofia_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
    use sofia_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![sofia_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    sofia_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("sofia".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/sofia",
                })]),
            }),
            EventMsg::ExecCommandEnd(ExecCommandEndEvent {
                call_id: "exec-1".into(),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                sofia_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("sofia_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    sofia_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    sofia_error_info: Some(CodexErrorInfo::ServerOverloaded),
                }),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(sofia_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(sofia_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: sofia_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(sofia_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                sofia_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                sofia_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                sofia_error_info: Some(CodexErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn out_of_turn_error_does_not_create_or_fail_a_turn() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                sofia_error_info: Some(CodexErrorInfo::BadRequest),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                sofia_error_info: Some(CodexErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            vec![Turn {
                ...
                error: Some(TurnError {
                    misalignment: None,
                    message: "Selected model is at capacity. Please try a different model.".into(),
                    sofia_error_info: Some(crate::protocol::v2::CodexErrorInfo::ServerOverloaded),
                    additional_details: None,
                }),
                started_at: Some(10),
                completed_at: Some(20),
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(sofia_protocol::items::HookPromptItem {
    ...
    fn completed_sub_agent_activity_updates_completed_parent_turn() {
        ...
        let child_path = sofia_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path");
        let items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(TurnStartedEvent {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                sofia_protocol::models::ResponseItem::Message {
                    id: Some(sofia_protocol::ResponseItemId::with_suffix("msg", "1")),
                    role: "user".into(),
                    content: vec![sofia_protocol::models::ContentItem::InputText {
