...
use sofia_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use sofia_protocol::protocol::RateLimitReachedType as CoreRateLimitReachedType;
use sofia_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use sofia_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use sofia_protocol::protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Sofia-managed ChatGPT auth tokens have.
    ...
    ChatgptAuthTokens {
        ...
        ///
        /// When `null`, Sofia attempts to derive the plan type from access-token
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Sofia attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Sofia was previously using.
    ///
...
pub struct GetAccountRateLimitsResponse {
    ...
    /// Multi-bucket view keyed by metered `limit_id` (for example, `sofia`).
