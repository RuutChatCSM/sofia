...
use sofia_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
use sofia_protocol::realtime::BemItemPresentation as CoreBemItemPresentation;
use sofia_protocol::realtime::RealtimeItem as CoreRealtimeItem;
use sofia_protocol::realtime::RealtimeItemContent as CoreRealtimeItemContent;
use sofia_protocol::realtime::RealtimeSessionOutcome as CoreRealtimeSessionOutcome;
use sofia_protocol::realtime::RealtimeTranscriptRole as CoreRealtimeTranscriptRole;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Sofia response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Sofia. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub sofia_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `sofia_responses_as_items`.
    /// Optional prefix added to automatic Sofia response items when `codexResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub sofia_response_item_prefix: Option<String>,
    /// Selects how automatic Sofia responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub sofia_response_handoff_mode: Option<CodexResponseHandoffMode>,
    ...
    /// Set to false to start without Sofia's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Sofia model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Sofia model when this realtime session ends.
    #[ts(optional = nullable)]
