...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("sofia_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("sofia_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
mod tests {
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("sofia_schema_{}", Uuid::now_v7()));
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("sofia_ts_filter_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("sofia_app_server_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("sofia_app_server_protocol.v2.schemas.json"))?;
        ...
        let flat_v2_bundle =
            read_json_value(&output_dir.join("sofia_app_server_protocol.v2.schemas.json"))?;
