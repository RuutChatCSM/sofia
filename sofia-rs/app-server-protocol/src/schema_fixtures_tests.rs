...
fn write_schema_fixtures_from_env() -> Result<()> {
    let schema_root = std::env::var_os("SOFIA_APP_SERVER_SCHEMA_ROOT")
        .map(PathBuf::from)
        .context("SOFIA_APP_SERVER_SCHEMA_ROOT must be set")?;
    let prettier = std::env::var_os("SOFIA_APP_SERVER_SCHEMA_PRETTIER").map(PathBuf::from);
    let experimental = std::env::var("SOFIA_APP_SERVER_SCHEMA_EXPERIMENTAL")
        .context("SOFIA_APP_SERVER_SCHEMA_EXPERIMENTAL must be set")?;
    let experimental_api = match experimental.as_str() {
        "0" => false,
        "1" => true,
        value => {
            anyhow::bail!("SOFIA_APP_SERVER_SCHEMA_EXPERIMENTAL must be 0 or 1, got {value:?}")
        }
    };
...
fn schema_root() -> Result<PathBuf> {
    let typescript_index = sofia_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle =
        sofia_utils_cargo_bin::find_resource!("schema/json/sofia_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/sofia_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
