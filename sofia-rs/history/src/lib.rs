...
/// Metadata owned by the Sofia harness and persisted with a response item.
///
...
pub struct CodexHarnessMetadata {
    ...
    /// Whether a response configuration update was created by the Sofia harness itself.
...
/// sofia_rollout's canonical parser so nested decimal values survive the flattened envelope.
