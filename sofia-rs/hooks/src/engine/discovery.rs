...
use sofia_config::version_for_toml;
use sofia_plugin::PluginHookSource;
use sofia_plugin::is_allowlisted_bundled_cleanup_hook;
...
fn append_matcher_groups(
    ...
    event_name: sofia_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        sofia_protocol::protocol::HookEventName::PreToolUse
                            | sofia_protocol::protocol::HookEventName::PostToolUse
                            | sofia_protocol::protocol::HookEventName::SessionStart
                            | sofia_protocol::protocol::HookEventName::UserPromptSubmit
                            | sofia_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/sofia/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.sofia/config.toml").abs();
        let dot_codex_folder = test_path_buf("/tmp/worktree/.sofia").abs();
