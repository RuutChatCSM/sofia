...
use sofia_mcp::hosted_plugin_runtime_mcp_server_config;
...
impl McpServerContributor<Config> for HostedPluginRuntimeExtension {
    ...
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            let name = SOFIA_APPS_MCP_SERVER_NAME.to_string();
            if !config.features.enabled(sofia_features::Feature::Apps) {
...
pub fn install_executor_plugins(
    ...
    environment_manager: std::sync::Arc<sofia_exec_server::EnvironmentManager>,
) {
