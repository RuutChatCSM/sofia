...
use sofia_history_notes_extension::install;
...
async fn installed_extension_exposes_and_invokes_history_notes_tools() -> TestResult {
    ...
    Mock::given(method("POST"))
        .and(path("/backend-api/sofia/alpha/notes/v2/read_file"))
        ...
        .await;
    Mock::given(method("POST"))
        .and(path("/backend-api/sofia/alpha/notes/v2/thread_hint"))
        .and(header("x-openai-actor-authorization", "actor-biscuit"))
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    config.model_provider.base_url = Some(format!("{}/backend-api/sofia", server.uri()));
    ...
    for (namespace, name, mut arguments) in [
        ...
        Mock::given(method("POST"))
            .and(path(format!(
                "/backend-api/sofia/alpha/{namespace}/v2/{name}"
            )))
            .and(header("x-openai-actor-authorization", "actor-biscuit"))
    ...
    Mock::given(method("POST"))
        .and(path("/backend-api/sofia/alpha/history/v2/read_item"))
        .and(header("x-openai-actor-authorization", "actor-biscuit"))
        .respond_with(ResponseTemplate::new(200).set_body_json(response))
        .mount(&server)
        .await;
