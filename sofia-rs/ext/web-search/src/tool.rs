...
use sofia_api::SearchClient;
use sofia_api::SearchCommands;
use sofia_api::SearchQuery;
use sofia_api::SearchRequest;
use sofia_api::SearchSettings;
use sofia_core::X_CODEX_TURN_METADATA_HEADER;
use sofia_core::web_search_action_detail;
...
use sofia_extension_api::parse_tool_input_schema_without_compaction;
use sofia_extension_items::ExtensionItem;
use sofia_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "sofia.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(
        ...
        let extra_headers = search_request_headers(
            ...
            call.sofia_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = sofia_otel::global()
