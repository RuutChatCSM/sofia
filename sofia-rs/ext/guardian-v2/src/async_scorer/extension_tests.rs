...
use sofia_core::context::ContextualUserFragment;
use sofia_core::context::InternalContextSource;
use sofia_core::context::InternalModelContextFragment;
...
use sofia_login::ExternalAuth;
use sofia_login::ExternalAuthFuture;
use sofia_login::ExternalAuthRefreshContext;
...
use sofia_protocol::openai_models::GuardianScope;
use sofia_protocol::openai_models::GuardianV2ModelConfig;
use sofia_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
use sofia_protocol::security_risk::SecurityRiskScore;
...
use sofia_features::GuardianV2ReviewScopeConfigToml;
...
async fn installed_extension_warms_connections_without_blocking_thread_start() -> Result<()> {
    ...
    let thread_store = test.sofia.thread_extension_data();
...
impl ExtensionMetrics for RecordingMetrics {
    ...
    fn histogram(&self, name: &str, value: i64, tags: &[(&str, &str)]) {
        if name == "sofia.guardian_v2.connection.duration_ms" {
            return;
        }
...
fn user_instruction(text: &str) -> ResponseItem {
    ResponseItem::Message {
        ...
        internal_chat_message_metadata_passthrough: Some(InternalChatMessageMetadataPassthrough {
            content_item_kinds: Some(vec![sofia_protocol::models::ContentItemKind(
                "user.text".to_owned(),
            )]),
            ..Default::default()
        }),
...
async fn sandboxed_shell_classification_respects_review_scope() -> Result<()> {
    ...
    let thread_store = fixture.test.sofia.thread_extension_data();
...
async fn computer_use_only_scores_cannot_approve_other_actions() -> Result<()> {
    ...
    let authorization =
        super::super::authorization::ScoreAuthorization::current(&fixture.test.sofia).await;
    *progress.authorization.lock().unwrap() = Some(authorization);
    ...
    let mut changed_model = fixture
        ...
        .await;
    changed_model.guardian = Some(sofia_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(sofia_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
...
async fn sample_configured_conversation_history_with_source(
    ...
    let builder = test_codex()
        ...
        .with_model_info_override("sofia-auto-review", |model_info| {
            model_info
                .model_messages
                .as_mut()
    ...
    let conversation_history = test.sofia.conversation_history_snapshot().await;
...
async fn disabled_code_mode_invalidates_cached_scores() -> Result<()> {
    ...
    let mut model = thread_store
        .get::<sofia_protocol::openai_models::ModelInfo>()
        .expect("resolved model")
        .as_ref()
        .clone();
    model.guardian = Some(sofia_protocol::openai_models::GuardianModelPolicy {
        computer_use: Some(sofia_protocol::openai_models::GuardianReviewMode::Adaptive),
        ..Default::default()
    });
    thread_store.insert(model);
...
impl GuardianFailureFixture {
    ...
    async fn score_tool(&self, tool_name: ToolName) {
        let thread_store = self.test.sofia.thread_extension_data();
        thread_store.insert(SecurityRiskScore {
            scores: BTreeMap::from([("action_risk".to_owned(), 0.25)]),
            call_id: None,
            action: None,
...
async fn contributor_fails_closed_when_luna_classification_fails() -> Result<()> {
    ...
    fixture.registry.thread_lifecycle_contributors()[0]
        .on_thread_start(ThreadStartInput {
            ...
            extension_metrics: None,
            session_store: &fixture.session_store,
            thread_store: fixture.test.sofia.thread_extension_data(),
        })
        .await;
    fixture
        .test
...
async fn assert_luna_pool_context(thread_context_enabled: bool) -> Result<()> {
    ...
    assert_eq!(
        request["client_metadata"]["x-sofia-window-id"],
    ...
    let mut expected_content = json!([
        ...
        {
            "type": "input_text",
            "text": "The Sofia agent has requested the following action:\n"
        },
...
async fn contributor_skips_required_models_in_standard_scope() -> Result<()> {
    ...
    let config_layer_stack = ConfigBuilder::default()
        .sofia_home(initial.home.path().to_path_buf())
    ...
    let authorization = super::ScoreAuthorization::current(&test.sofia).await;
    let progress = thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("Guardian v2 should track score progress per thread");
    *progress.authorization.lock().unwrap() = Some(authorization);
...
async fn assert_compaction_approval_policy(thread_context_enabled: bool) -> Result<()> {
    ...
    let authorization = fixture.test.sofia.guardian_authorization_version().await;
    fixture
        .test
        .sofia
    ...
    thread_store.insert(model);
    assert_eq!(
        fixture.test.sofia.guardian_authorization_version().await,
        authorization
    );
    let score_authorization = super::ScoreAuthorization::current(&fixture.test.sofia).await;
    *thread_store
        .get::<GuardianV2ScoreProgress>()
        .expect("score progress")
        .authorization
...
struct CacheMiss;
impl sofia_extension_api::SynchronousApprovalReviewer for CacheMiss {
    fn review(
        &self,
        _reason: sofia_protocol::approvals::GuardianReviewReason,
    ) -> sofia_extension_api::ExtensionFuture<'_, ReviewDecision> {
...
async fn cached_approval_discounts_only_its_own_unscored_wrapper() -> Result<()> {
    skip_if_no_network!(Ok(()));
    let fixture = GuardianFailureFixture::new().await?;
    let store = fixture.test.sofia.thread_extension_data();
...
async fn cached_approval(
    registry: &sofia_extension_api::ExtensionRegistry<Config>,
    store: &ExtensionData,
    action: &str,
    ...
    let category = match review_scope(&action) {
        ...
        None if store.get::<super::GuardianV2Config>()?.policy.other_tools
            == sofia_protocol::openai_models::GuardianReviewMode::Adaptive =>
        {
            sofia_protocol::openai_models::GuardianScope::Shell
        }
        None => {
    ...
    let input = sofia_extension_api::ApprovalDecisionInput {
        approval_id: "cache-probe",
        tool_call_id: action.get("id").and_then(serde_json::Value::as_str),
        action: &action,
        thread_id: sofia_protocol::ThreadId::from_string(store.level_id()).unwrap(),
        thread_store: store,
        category,
        approval_policy: sofia_protocol::protocol::AskForApproval::OnRequest,
    ...
    match registry.decide_approval(&input).await {
        Some(sofia_extension_api::ApprovalDecision::Allow) => Some(ReviewDecision::Approved),
        _ => None,
    }
}
