...
fn assert_connection_metadata(
    ...
    assert_eq!(
        [
            ...
            handshake.header("x-sofia-window-id"),
        ],
        [
    ...
    for (request, (parent_turn_id, root_turn_id)) in requests.iter().zip(expected_lineage) {
        ...
        metadata["x-sofia-turn-metadata"] = serde_json::from_str(
            metadata["x-sofia-turn-metadata"]
                .as_str()
        ...
        let mut expected = json!({
            ...
            "x-sofia-window-id": format!("{thread_id}:0"),
            "ws_request_header_x_openai_internal_codex_responses_lite": "true",
            "x-sofia-turn-metadata": {
                "session_id": "session-1",
                "thread_id": thread_id,
        ...
        if let Some(root_turn_id) = root_turn_id {
            ...
            expected["x-sofia-turn-metadata"]["root_turn_id"] = json!(root_turn_id);
        }
...
async fn classifier_uses_free_endpoint_only_with_codex_backend_auth() -> Result<()> {
    ...
    for (auth, base_path, free_guardian, expected_path, expected_service_tier) in [
        ...
        (
            ...
            "/backend-api/sofia",
            true,
            "/backend-api/sofia/guardian-classifier",
            None,
        ),
        (
            CodexAuth::create_dummy_chatgpt_auth_for_testing(),
...
async fn preconnected_sampler_reuses_authenticated_websocket_for_classifications() -> Result<()> {
    ...
    assert_eq!(
        handshake.header("x-openai-internal-sofia-responses-lite"),
        Some("true".to_owned())
    );
...
async fn sampler_retries_expired_websockets_on_another_warm_connection() -> Result<()> {
    ...
    request.trusted_skills = Some(sofia_guardian_context::TrustedSkills {
