...
use sofia_features::GuardianV2ConfigToml;
use sofia_features::GuardianV2TranscriptConfigToml;
...
impl GuardianV2Config {
    pub(crate) fn resolve(config: &Config) -> Result<Self, String> {
        ...
        resolved.force_synchronous_review = config
            ...
            .is_err();
        resolved.scoring_disabled = !config.features.enabled(sofia_features::Feature::GuardianV2);
        Ok(resolved)
    }

    pub(super) fn policy_for_model(
        &self,
        model: Option<&sofia_protocol::openai_models::ModelInfo>,
    ) -> GuardianPolicy {
