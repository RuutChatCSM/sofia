...
async fn trusts_only_user_owned_skill_roots() -> Result<()> {
    ...
    let sofia_skill = test.home.path().join("skills/trusted/SKILL.md");
    ...
    for path in [&sofia_skill, &agents_skill, &repo_skill] {
    ...
    for trusted_path in [&sofia_skill, &agents_skill] {
        assert_eq!(
...
fn invoked_skill_paths_are_deduplicated_and_bounded() {
    ...
    for index in 0..MAX_TRUSTED_SKILLS.saturating_mul(2) {
        let path = format!("/home/user/.sofia/skills/{index:03}/SKILL.md");
    ...
    assert_eq!(
        ...
        (0..MAX_TRUSTED_SKILLS)
            .map(|index| format!("/home/user/.sofia/skills/{index:03}/SKILL.md"))
            .collect::<Vec<_>>()
    );
