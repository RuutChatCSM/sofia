...
fn expected_context(tool: &McpToolInfo, source: &Path) -> sofia_guardian_context::TrustedTool {
    sofia_guardian_context::TrustedTool {
...
async fn trusts_only_tools_configured_in_codex_home() -> Result<()> {
    ...
    for (tool, unrelated, source) in [
        ...
        (
            mcp_tool("sofia_apps", Some("connector_home"))?,
            mcp_tool("sofia_apps", Some("connector_other"))?,
...
async fn trusts_connector_declared_by_home_owned_plugin() -> Result<()> {
    ...
    let plugin_root = test
        ...
        .join("trusted")
        .join("local");
    let tool = mcp_tool("sofia_apps", Some("connector_calendar"))?;
