...
use sofia_extension_api::McpToolInfo;
...
pub(crate) async fn trusted_tool_context(
    ...
    let sofia_home = config.sofia_home.as_path().canonicalize().ok()?;
    ...
    let source = match source {
        McpToolSource::Connector => {
            ...
            if let Some(plugin) = plugins.as_ref()?.plugins().iter().find(|plugin| {
                ...
                trusted_user_config_source(config, "apps", connector_id, &sofia_home)?
            }
        }
        ...
        McpToolSource::Config => {
            trusted_user_config_source(config, "mcp_servers", &tool.server_name, &sofia_home)?
        }
        McpToolSource::SelectedPlugin | McpToolSource::Other => return None,
    };
...
fn is_home_owned_plugin_capability(
    ...
    if !is_home_owned_path(plugin_root, sofia_home) {
        return false;
    }
    ...
    if !is_home_owned_path(&manifest_path, sofia_home) {
        return false;
    }
    ...
    is_home_owned_path(&declaration_path, sofia_home)
}
...
fn trusted_user_config_source(
    ...
    if !is_home_owned_path(user_config_file.as_path(), sofia_home) {
        return None;
    }
...
fn is_home_owned_path(path: &Path, sofia_home: &Path) -> bool {
    path.canonicalize()
        .is_ok_and(|canonical_path| canonical_path.starts_with(sofia_home))
}
