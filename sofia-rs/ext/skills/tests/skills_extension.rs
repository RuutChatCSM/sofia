...
use sofia_exec_server::CapabilityRootDiscovery;
use sofia_exec_server::ExecutorCapabilityDiscoverySnapshot;
use sofia_exec_server::LOCAL_FS;
use sofia_extension_api::ConversationHistory;
use sofia_extension_api::ExtensionData;
use sofia_extension_api::ExtensionEventSink;
use sofia_extension_api::ExtensionMetrics;
use sofia_extension_api::ExtensionRegistryBuilder;
use sofia_extension_api::ExtensionWarning;
use sofia_extension_api::FunctionCallError;
use sofia_extension_api::NoopTurnItemEmitter;
use sofia_extension_api::PreviousWorldStateSection;
use sofia_extension_api::RenderedWorldStateFragment;
use sofia_extension_api::SkillInvocationInput;
use sofia_extension_api::SkillInvocationKind;
use sofia_extension_api::ThreadStartInput;
use sofia_extension_api::ToolCall;
use sofia_extension_api::ToolCallSource;
use sofia_extension_api::ToolPayload;
...
use sofia_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use sofia_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use sofia_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use sofia_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use sofia_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use sofia_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use sofia_protocol::protocol::SessionSource;
use sofia_protocol::protocol::SkillScope;
use sofia_protocol::protocol::TruncationPolicy;
use sofia_protocol::protocol::TurnEnvironmentSelection;
use sofia_protocol::user_input::UserInput;
use sofia_skills::SkillMetadata;
...
use sofia_skills_extension::HostSkillsSnapshot;
use sofia_skills_extension::InjectedHostSkillPrompts;
use sofia_skills_extension::SkillLoadOutcome;
...
use sofia_skills_extension::install_with_providers_and_metrics;
use sofia_skills_extension::provider::SkillListQuery;
use sofia_skills_extension::provider::SkillProvider;
...
async fn skill_world_state_fragments(
    registry: &sofia_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: sofia_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let sofia_home = test_codex_home();
    let skill_path = sofia_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(sofia_home)?;
    Ok(())
}
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "sofia.skills.shadow_selection.catalog_entries")
...
async fn shadow_lru_selector_recovers_a_skill_invoked_on_an_earlier_turn() -> TestResult {
    ...
    let metric = snapshot
        .scope_metrics()
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
        .find(|metric| metric.name() == "sofia.skills.shadow_selection.invocation")
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    ...
    let sofia_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(sofia_home, Vec::new(), config_layer_stack),
...
fn test_codex_home() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "sofia-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
