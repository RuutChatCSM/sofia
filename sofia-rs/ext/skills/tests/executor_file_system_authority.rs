...
use sofia_exec_server::CapabilityTextFile;
use sofia_exec_server::CopyOptions;
use sofia_exec_server::CreateDirectoryOptions;
use sofia_exec_server::DiscoveredSkillFiles;
...
use sofia_exec_server::ExecutorFileSystemFuture;
use sofia_exec_server::FileMetadata;
use sofia_exec_server::FileSystemReadStream;
use sofia_exec_server::FileSystemSandboxContext;
use sofia_exec_server::GetMetadataOptions;
use sofia_exec_server::ReadDirectoryEntry;
use sofia_exec_server::ReadFileOptions;
use sofia_exec_server::RemoveOptions;
use sofia_exec_server::WalkEntry;
use sofia_exec_server::WalkEntryKind;
use sofia_exec_server::WalkOptions;
use sofia_exec_server::WalkOutcome;
use sofia_exec_server::WriteFileOptions;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("sofia-executor-skill-fs-{}", std::process::id()));
    let project_folder = AbsolutePathBuf::from_absolute_path_checked(test_root.join(".sofia"))
...
async fn executor_discovery_routes_produce_equivalent_catalog_metadata() {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "sofia-executor-skill-parity-{}-{id}",
        std::process::id()
    ));
    let plugin_manifest = test_root.join(".sofia-plugin/plugin.json");
    ...
    let comparable_entries = |catalog: &sofia_skills_extension::catalog::SkillCatalog| {
        catalog
            .entries
            .iter()
            .map(|entry| {
...
async fn pre_discovered_executor_catalog_snapshot() {
    ...
    let manifest_dir = test_root.join(".sofia-plugin");
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "sofia-executor-skill-discovery-{}-{id}",
        std::process::id()
    ));
    let outer_manifest = test_root.join(".sofia-plugin/plugin.json");
    ...
    let inner_manifest = test_root.join("nested/.sofia-plugin/plugin.json");
