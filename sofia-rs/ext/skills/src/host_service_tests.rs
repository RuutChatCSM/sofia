...
use sofia_skills::ImplicitSkillLookup;
use sofia_skills::LoadedSkillRoot;
use sofia_skills::SkillRootSnapshotCache;
use sofia_skills::SkillRootSnapshots;
...
use sofia_utils_plugins::PluginIdentity;
use sofia_utils_plugins::PluginSkillRoot;
...
fn write_user_skill(sofia_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = sofia_home.path().join("skills").join(dir);
...
fn user_config_layer(sofia_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(sofia_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(sofia_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(sofia_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(sofia_home, user_config_toml),
            ConfigLayerEntry::new(
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&sofia_home, "");
    let skills_service = HostSkillsService::new(
        sofia_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&sofia_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&sofia_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_config_bounds_plugin_generations_and_preserves_live_snapshots() {
    ...
    let base_input = HostSkillsLoadInput::new(
        cwd.path().abs(),
        vec![plugin_root],
        config_stack(&sofia_home, "[skills.bundled]\nenabled = false\n"),
    );
    let skills_service = HostSkillsService::new(
        sofia_home.path().abs(),
...
async fn watchable_skill_root_paths_exclude_plugin_and_system_roots() {
    ...
    assert!(watchable_paths.contains(&sofia_home.path().join("skills").abs()));
    assert!(!watchable_paths.contains(&plugin_skill_root.path));
    assert!(!watchable_paths.contains(&sofia_home.path().join("skills/.system").abs()));
}
...
async fn snapshot_for_config_merges_extension_host_and_legacy_plugin_roots() {
    let sofia_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    write_user_skill(&sofia_home, "user", "user-skill", "from the host loader");
    ...
    let config_layer_stack = config_stack(&sofia_home, "[skills.bundled]\nenabled = false\n");
    let input = HostSkillsLoadInput::new(
        cwd.path().abs(),
...
async fn snapshot_for_config_preserves_host_precedence_for_symlinked_plugin_root() {
    ...
    assert_eq!(
        outcome.skills,
        vec![sofia_skills::SkillMetadata {
...
async fn skills_list_snapshots_share_host_roots_only_within_one_request() {
    ...
    write_user_skill(&sofia_home, "first", "first-skill", "first skill");
    let first = request
        .snapshot_for_cwd(
    ...
    write_user_skill(&sofia_home, "second", "second-skill", "second skill");
    let second = request
        .snapshot_for_cwd(
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let sofia_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_codex = cwd.path().join(".sofia");
    fs::create_dir_all(&repo_dot_codex).expect("create repo config dir");

    write_user_skill(&sofia_home, "user", "user-skill", "from local user root");
    ...
    let other_file_system: Arc<dyn sofia_exec_server::ExecutorFileSystem> =
        Arc::new(sofia_exec_server::LocalFileSystem::unsandboxed());
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let sofia_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = sofia_home.path().join("skills/.system/bundled-skill");
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    write_user_skill(&sofia_home, "late", "late-skill", "added after cache");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let sofia_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = sofia_home.path().join("skills").join("demo");
    fs::create_dir_all(&skill_dir).expect("create skill dir");
    let skill_path = skill_dir.join("SKILL.md");
    fs::write(
        &skill_path,
    ...
    let parent_stack = config_stack(&sofia_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&sofia_home, &disabled_skill_config, &enabled_skill_config);
    let skills_service = HostSkillsService::new(
        sofia_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );
