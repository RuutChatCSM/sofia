...
use sofia_protocol::mcp::ResourceContent;
...
const ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC: &str =
    "sofia.skills.orchestrator.discovery.duration_ms";
const ORCHESTRATOR_SKILL_DISCOVERY_RESOURCES_METRIC: &str =
    "sofia.skills.orchestrator.discovery.resources_total";
...
impl SkillProvider for OrchestratorSkillProvider {
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        Box::pin(async move {
            ...
            if !client.has_server(SOFIA_APPS_MCP_SERVER_NAME).await {
                return Ok(SkillCatalog::default());
            }

            let _discovery_timer =
                sofia_otel::start_global_timer(ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC, &[])
                    .ok();
            ...
            for _ in 0..MAX_RESOURCE_PAGES {
                let page = match tokio::time::timeout_at(
                    discovery_deadline,
                    client.list_resources(SOFIA_APPS_MCP_SERVER_NAME, cursor.clone()),
                )
                .await
                {
    ...
    fn read<'a>(
        ...
        Box::pin(async move {
            ...
            let result = tokio::time::timeout(
                ...
                client.read_resource(SOFIA_APPS_MCP_SERVER_NAME, request.resource.as_str()),
            )
            .await
            .map_err(|_| {
                SkillProviderError::new(format!(
