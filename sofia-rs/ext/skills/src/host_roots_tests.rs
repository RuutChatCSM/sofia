...
fn user_layer(sofia_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: sofia_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/sofia"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("sofia");
    let project_folder = absolute(temp_dir.path().join("repo/.sofia"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.sofia"));
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_codex = workspace.join(".sofia");
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_codex = repository.join(".sofia");
    let skill_root = dot_codex.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let sofia_home = home_folder.join("sofia");
    let system_folder = absolute(temp_dir.path().join("etc/sofia"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_codex = repository.join(".sofia");
    let nested_project_dot_codex = repository.join("nested/.sofia");
    let user_skills = sofia_home.join("skills");
    ...
    let system_skill = write_skill(&sofia_home.join("skills/.system"), "system", "system-skill");
