...
use sofia_exec_server::CapabilityRootDiscoverRequest;
use sofia_exec_server::CapabilityRootsDiscoverParams;
use sofia_exec_server::LOCAL_FS;
use sofia_exec_server::discover_capability_roots;
use sofia_protocol::protocol::Product;
use sofia_skills::EnvironmentSkillMetadata;
use sofia_skills::SkillDependencies;
use sofia_skills::SkillPolicy;
...
async fn direct_environment_loader_preserves_plugin_dependencies_and_product_policy() {
    ...
    fs::create_dir_all(root.path().join(".sofia-plugin")).expect("manifest dir");
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(LOCAL_FS.as_ref(), &root_uri, Some(Product::Sofia)).await;
    ...
    assert_eq!(
        ...
        vec![EnvironmentSkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::Sofia, Product::Atlas],
            }),
        }]
    );
    let atlas =
...
async fn executor_bundle_parser_matches_direct_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".sofia-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".sofia-plugin/plugin.json", "sofia-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.sofia-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "sofia-name:search");
}
