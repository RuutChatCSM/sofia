    ...
    B: MemoriesBackend,
{
    ...
    fn spec(&self) -> ToolSpec {
        memory_function_tool::<SearchArgs, SearchMemoriesResponse>(
            SEARCH_TOOL_NAME,
            "Search Sofia memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        )
    }
    ...
    B: MemoriesBackend,
{
    async fn handle_call(
        &self,
        call: ToolCall<'_>,
    ) -> Result<Box<dyn sofia_extension_api::ToolOutput>, sofia_extension_api::FunctionCallError>
    {
