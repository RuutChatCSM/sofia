...
use sofia_extension_api::ThreadResumeInput;
use sofia_extension_api::ThreadStartInput;
use sofia_extension_api::ThreadStopInput;
use sofia_extension_api::ToolCall;
use sofia_extension_api::ToolCallOutcome;
use sofia_extension_api::ToolCallSource;
use sofia_extension_api::ToolExecutor;
use sofia_extension_api::ToolFinishInput;
use sofia_extension_api::ToolPayload;
use sofia_extension_api::TurnErrorInput;
use sofia_extension_api::TurnStartInput;
use sofia_extension_api::TurnStopInput;
use sofia_goal_extension::GoalExtensionConfig;
use sofia_goal_extension::GoalObjectiveUpdate;
use sofia_goal_extension::GoalRuntimeHandle;
use sofia_goal_extension::GoalService;
use sofia_goal_extension::GoalSetRequest;
use sofia_goal_extension::GoalTokenBudgetUpdate;
use sofia_goal_extension::install_with_backend;
...
use sofia_protocol::protocol::ThreadGoalStatus;
...
async fn spawned_descendant_usage_exhausts_root_goal_budget_once() -> anyhow::Result<()> {
    ...
    assert_eq!(sofia_state::ThreadGoalStatus::BudgetLimited, goal.status);
    Ok(())
}
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(sofia_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(sofia_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
        ...
        [(None, None, 3), (Some(2), None, 5), (None, Some(2), 4)]
    {
        ...
        for turn in 1..=blocking_turn {
            ...
            let expected = if turn == blocking_turn {
                sofia_state::ThreadGoalStatus::Blocked
            } else {
                sofia_state::ThreadGoalStatus::Active
            };
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(sofia_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<sofia_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: Arc<sofia_extension_api::ExtensionRegistry<()>>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish_with_outcome(
        ...
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = sofia_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall<'static> {
    ToolCall {
        ...
        tool_name: sofia_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        sofia_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        source: ToolCallSource::Direct,
        conversation_history: sofia_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<sofia_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    sofia_state::StateRuntime::init(
        sofia_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &sofia_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = sofia_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: sofia_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        sofia_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        sofia_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        sofia_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        sofia_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        sofia_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        sofia_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
