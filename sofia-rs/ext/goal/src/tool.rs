...
use sofia_extension_api::JsonToolOutput;
use sofia_extension_api::ToolCall;
use sofia_extension_api::ToolExecutor;
use sofia_extension_api::ToolName;
use sofia_extension_api::ToolOutput;
use sofia_extension_api::ToolSpec;
use sofia_protocol::ThreadId;
use sofia_protocol::protocol::ThreadGoal;
use sofia_protocol::protocol::ThreadGoalStatus;
use sofia_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<sofia_state::StateRuntime>,
...
impl<'call> ToolExecutor<ToolCall<'call>> for GoalToolExecutor {
    ...
    ) -> sofia_extension_api::ToolExecutorFuture<'a>
    where
        'call: 'a,
    {
        Box::pin(async move {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => sofia_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => sofia_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: sofia_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            sofia_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            sofia_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<sofia_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &sofia_state::StateRuntime,
    thread_id: ThreadId,
    goal: &sofia_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: sofia_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: sofia_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> sofia_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => sofia_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => sofia_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => sofia_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => sofia_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => sofia_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => sofia_state::ThreadGoalStatus::Complete,
    }
}
