...
use sofia_mcp::ResolvedMcpServer;
use sofia_protocol::mcp::CallToolResult;
use sofia_tools::ToolCallSource;
...
pub enum McpToolSource {
    /// A connector routed through the host-owned Sofia Apps MCP server.
    Connector,
    /// An MCP server whose frozen registration matches the active Sofia configuration.
    Config,
...
pub struct ToolStartInput<'a> {
    ...
    pub originating_item_id: Option<&'a sofia_protocol::ResponseItemId>,
