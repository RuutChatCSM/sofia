...
use sofia_extension_api::ApprovalReviewContributor;
...
use sofia_extension_api::ContextualUserFragment;
...
use sofia_extension_api::TokenUsageContributor;
...
use sofia_extension_api::TurnInputContributor;
use sofia_extension_api::TurnItemContributor;
use sofia_extension_api::TurnLifecycleContributor;
use sofia_protocol::items::HookPromptItem;
...
impl ApprovalReviewContributor for AllContributors {
    ...
    ) -> ExtensionFuture<'a, Option<sofia_extension_api::ApprovalDecision>> {
        Box::pin(async { Some(sofia_extension_api::ApprovalDecision::AskUser) })
    }
}

impl sofia_extension_api::SynchronousApprovalReviewer for AllContributors {
    fn review(
        &self,
        _reason: sofia_protocol::approvals::GuardianReviewReason,
    ) -> ExtensionFuture<'_, sofia_protocol::protocol::ReviewDecision> {
        Box::pin(std::future::ready(
            sofia_protocol::protocol::ReviewDecision::Approved,
        ))
    }
}
...
async fn build_round_trips_every_contributor_category() {
    ...
    let input = sofia_extension_api::ApprovalDecisionInput {
        ...
        category: sofia_protocol::openai_models::GuardianScope::Shell,
        approval_policy: sofia_protocol::protocol::AskForApproval::OnRequest,
        approvals_reviewer: sofia_protocol::config_types::ApprovalsReviewer::AutoReview,
    ...
    assert_eq!(
        ...
        Some(sofia_extension_api::ApprovalDecision::AskUser)
    );
}
        ...
        fragments.extend(
            contributor
                .contribute_turn_context(TurnContextContributionInput {
                    thread_id: sofia_protocol::ThreadId::default(),
