...
use sofia_network_proxy::ManagedNetworkSandboxContext;
...
use sofia_network_proxy::build_config_state;
...
use sofia_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn process_platform_defaults_allow_scratch_without_granting_it_to_filesystem_helpers() {
    let workspace = tempfile::Builder::new()
        .prefix("sofia-seatbelt-approved-project-")
    ...
    for scratch_root in ["/private/tmp", "/private/var/tmp"] {
        let scratch = tempfile::Builder::new()
            .prefix("sofia-seatbelt-process-scratch-")
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/sofia-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.sofia".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/sofia-readable");
    let unreadable = absolute_path("/tmp/sofia-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
            path: root.into(),
...
fn preferences_access_requires_unrestricted_reads() {
    ...
    denied_path.entries.push(FileSystemSandboxEntry::new(
        absolute_path("/tmp/sofia-private").into(),
        FileSystemAccessMode::Deny,
    ));
...
fn restricted_reads_cannot_read_preferences_outside_allowed_roots() {
    ...
    let marker = "sofia-preferences-read-canary";
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/sofia-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec!["/usr/bin/true".to_string()],
        file_system_sandbox_policy: &file_system_policy,
        network_sandbox_policy: NetworkSandboxPolicy::Restricted,
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/sofia-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/sofia-proxy-use";
    let explicit_socket = "/tmp/sofia-browser-use";
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // write to .sofia/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.sofia carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    // Verify that .sofia/config.toml cannot be modified under the generated
    // Seatbelt policy.
    ...
    // Verify that writing a file to the folder containing .git and .sofia is allowed.
...
fn seatbelt_protects_resolved_target_of_symlinked_metadata_directory() {
    ...
    let dot_codex = writable_root.join(".sofia");
    ...
    symlink(&actual_config, &dot_codex).expect("create .sofia symlink");
    let policy = restricted_write_policy(&[writable_root.as_path()]);
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
        command: vec![
            "/bin/sh".to_string(),
    ...
    assert!(
        !output.status.success(),
        "resolved .sofia target should remain read-only"
    );
    assert_eq!(
...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_metadata_name_regex() {
    ...
    let dot_codex = repo_root.join(".sofia");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .sofia/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".sofia")).expect("create .sofia");
    fs::write(
        vulnerable_root.join(".sofia").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .sofia/config.toml");
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".sofia");
