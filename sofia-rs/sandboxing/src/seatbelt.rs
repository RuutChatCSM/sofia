...
use sofia_network_proxy::PROXY_URL_ENV_KEYS;
use sofia_network_proxy::has_proxy_url_env_vars;
use sofia_network_proxy::proxy_url_env_value;
...
use sofia_protocol::protocol::WritableRoot;
...
fn normalize_writable_root_for_sandbox(
    ...
    if !allow_symlinks && let Some(symlink) = nested_symlink_component(root.as_path()) {
        return Err(SeatbeltPreparationError::FileSystem(format!(
            "writable root {} contains symlink component {}; symlinked writable roots are not supported.\n\
             If this writable root is at or beneath SOFIA_HOME and you trust its symlink targets, \
             set `allow_symlinked_codex_home = true` at the top level of `$SOFIA_HOME/config.toml` \
             (normally `~/.sofia/config.toml`) on the execution host, then restart Sofia or its executor. \
             This opt-out trusts targets outside SOFIA_HOME and targets changed between commands. \
    ...
    let normalized = if allow_symlinks {
        ...
        // outside SOFIA_HOME and links changed by an earlier command.
    ...
    for (index, access_root) in roots.into_iter().enumerate() {
            ...
            access_root.excluded_subpaths.into_iter().enumerate()
        {
            ...
            for (excluded_param, excluded_subpath) in excluded_subpaths {
                ...
                // protected directory itself, such as `mkdir .sofia`.
