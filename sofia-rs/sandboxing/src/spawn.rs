...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    ...
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let sofia_home = sofia_utils_home_dir::find_codex_home()
                .context("windows sandbox: failed to resolve sofia_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return sofia_windows_sandbox::spawn_windows_sandbox_session_for_level(
                sofia_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    sofia_home: sofia_home.as_path(),
    ...
    let spawned = if tty {
        sofia_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        sofia_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        sofia_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,
