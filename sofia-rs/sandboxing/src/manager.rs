...
use crate::landlock::SOFIA_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub sofia_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: sofia_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing sofia-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = sofia_linux_sandbox_exe
            ...
            SandboxType::WindowsRestrictedToken => {
                ...
                if let Some(metrics) = sofia_otel::global() {
                    let _ = metrics.counter(
                        "sofia.windows_sandbox.private_desktop",
                        /*inc*/ 1,
                        &[(
                            "enabled",
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let sofia_home = sofia_utils_home_dir::find_codex_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_codex_home(request, sofia_home.as_path())
        }
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    sofia_home: &Path,
    proxy_settings_mode: sofia_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = sofia_windows_sandbox::resolve_exe_for_launch(source.as_path(), sofia_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        sofia_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(SOFIA_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        SOFIA_LINUX_SANDBOX_ARG0.to_string()
    }
}
