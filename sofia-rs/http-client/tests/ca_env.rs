...
//! detailed explanation of what "hermetic" means here lives in `sofia_http_client::custom_ca`; these
...
const SOFIA_CA_CERT_ENV: &str = "SOFIA_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "SOFIA_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "SOFIA_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "SOFIA_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(SOFIA_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    sofia_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls13_material() -> Tls13Material {
    ...
    ca_distinguished_name.push(DnType::CommonName, "sofia test CA");
...
fn uses_codex_ca_cert_env() {
    ...
    let output = run_probe(&[(SOFIA_CA_CERT_ENV, cert_path.as_path())]);
...
fn prefers_codex_ca_cert_over_ssl_cert_file() {
    ...
    let output = run_probe(&[
        (SOFIA_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(SOFIA_CA_CERT_ENV, cert_path.as_path())], &server.url);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("SOFIA_CA_CERTIFICATE"));
