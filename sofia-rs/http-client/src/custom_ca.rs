//! Custom CA handling shared by Sofia outbound HTTP and websocket clients.
//!
//! Sofia constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `SOFIA_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Sofia itself commonly runs spawned test processes under seatbelt, so this is not just
...
use sofia_utils_rustls_provider::ensure_rustls_crypto_provider;
use thiserror::Error;
use tracing::info;
use tracing::warn;

pub const SOFIA_CA_CERT_ENV: &str = "SOFIA_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set SOFIA_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Sofia custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `SOFIA_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Sofia HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Sofia custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `SOFIA_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
/// Builds a rustls client config using native roots and any configured Sofia custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Sofia custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        sofia_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `SOFIA_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Sofia-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(SOFIA_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: SOFIA_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Sofia CA overrides.
    ///
    /// This accepts a few common real-world variants so Sofia behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Sofia only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(SOFIA_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
