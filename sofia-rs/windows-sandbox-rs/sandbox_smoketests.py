...
# Run a suite of smoke tests against the Windows sandbox via the Sofia CLI
...
def _resolve_codex_cmd() -> List[str]:
    """Resolve the Sofia CLI to invoke `sofia sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Sofia.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "sofia.exe",
        ws_root / "target" / "release" / "sofia.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend(
            [
                cargo_base / "debug" / "sofia.exe",
                cargo_base / "release" / "sofia.exe",
            ]
        )
    ...
    if shutil.which("sofia"):
        return ["sofia"]

    raise FileNotFoundError(
        "Sofia CLI not found. Build it first, e.g.\n"
        "  cargo build -p sofia-cli --release\n"
        "or for debug:\n"
        "  cargo build -p sofia-cli\n"
    )


SOFIA_CMD = _resolve_codex_cmd()
print(SOFIA_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to sofia CLI overrides.
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"SOFIA_HOME": str(proxy_home)},
            )
            add(
    ...
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", "echo hi > \\\\.\\pipe\\sofia_testpipe"],
        WS_ROOT,
    )
    ...
    # 34. WS: policy tamper (.sofia artifacts) denied
    sofia_home = Path(os.environ["USERPROFILE"]) / ".sofia"
    cap_sid_target = sofia_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
    ...
    rc2, out2, err2 = run_sbx(
        "workspace-write", ["cmd", "/c", "echo tamper > .sofia\\policy.json"], WS_ROOT
    )
    add("WS: .sofia cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .sofia policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx(
            "workspace-write", ["cmd", "/c", "echo owned > sofia_escape.txt"], fake_root
        )
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://sofia-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
