...
pub fn current_log_file_path_for_codex_home(sofia_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(sofia_home))
}
...
mod tests {
    ...
    fn current_log_file_path_for_codex_home_uses_sandbox_dir() {
        let sofia_home = Path::new("sofia-home");

        assert_eq!(
            current_log_file_path_for_codex_home(sofia_home),
            current_log_file_path(&sofia_home.join(".sandbox"))
        );
    }
}
