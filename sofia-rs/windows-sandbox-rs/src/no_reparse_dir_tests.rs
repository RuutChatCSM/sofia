...
fn local_directory_path_accepts_drive_and_verbatim_drive_paths() {
    assert!(validate_local_directory_path(Path::new(r"C:\Users\alice\.sofia")).is_ok());
    assert!(validate_local_directory_path(Path::new(r"\\?\D:\Sofia Data\home")).is_ok());
}
...
fn local_directory_path_rejects_relative_network_parent_and_stream_paths() {
    for path in [
        ...
        r"\\?\UNC\server\share\.sofia",
        r"\\.\C:\Users\alice",
        r"C:\Users\alice\..\other",
        r"C:\Users\alice\.sofia:stream",
    ] {
        assert!(
