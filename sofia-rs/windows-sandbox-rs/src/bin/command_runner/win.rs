...
use anyhow::Context;
use anyhow::Result;
use sofia_utils_pty::JobObject;
use sofia_windows_sandbox::ConsoleMode;
use sofia_windows_sandbox::ErrorPayload;
use sofia_windows_sandbox::ErrorStage;
use sofia_windows_sandbox::ExitPayload;
use sofia_windows_sandbox::FramedMessage;
use sofia_windows_sandbox::IPC_PROTOCOL_VERSION;
use sofia_windows_sandbox::LaunchDesktop;
use sofia_windows_sandbox::LocalSid;
use sofia_windows_sandbox::Message;
use sofia_windows_sandbox::OutputPayload;
use sofia_windows_sandbox::OutputStream;
use sofia_windows_sandbox::PipeSpawnHandles;
use sofia_windows_sandbox::ResizePayload;
use sofia_windows_sandbox::SpawnReady;
use sofia_windows_sandbox::SpawnRequest;
use sofia_windows_sandbox::StderrMode;
use sofia_windows_sandbox::StdinMode;
use sofia_windows_sandbox::WindowsSandboxTokenMode;
use sofia_windows_sandbox::allow_null_device;
use sofia_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use sofia_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use sofia_windows_sandbox::decode_bytes;
use sofia_windows_sandbox::encode_bytes;
use sofia_windows_sandbox::get_current_token_for_restriction;
use sofia_windows_sandbox::hide_current_user_profile_dir;
use sofia_windows_sandbox::log_note;
use sofia_windows_sandbox::read_frame;
use sofia_windows_sandbox::read_handle_loop;
use sofia_windows_sandbox::spawn_process_with_pipes;
use sofia_windows_sandbox::to_wide;
use sofia_windows_sandbox::token_mode_for_permission_profile;
use sofia_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<sofia_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.sofia_home.clone();
    hide_current_user_profile_dir(req.sofia_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = sofia_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
