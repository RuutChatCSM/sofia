...
use serde::Serialize;
use sofia_otel::StatsigMetricsSettings;
use sofia_windows_sandbox::DirectoryOpenDisposition;
use sofia_windows_sandbox::SETUP_VERSION;
use sofia_windows_sandbox::SetupErrorCode;
use sofia_windows_sandbox::SetupErrorReport;
use sofia_windows_sandbox::SetupFailure;
use sofia_windows_sandbox::acquire_sandbox_setup_lock;
use sofia_windows_sandbox::add_deny_write_ace;
use sofia_windows_sandbox::convert_string_sid_to_sid;
use sofia_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use sofia_windows_sandbox::ensure_allow_write_aces;
use sofia_windows_sandbox::extract_setup_failure;
use sofia_windows_sandbox::hide_newly_created_users;
use sofia_windows_sandbox::install_wfp_filters;
use sofia_windows_sandbox::local_user_flags;
use sofia_windows_sandbox::log_note;
use sofia_windows_sandbox::log_writer;
use sofia_windows_sandbox::open_directory_no_reparse;
use sofia_windows_sandbox::path_mask_allows;
use sofia_windows_sandbox::path_write_aces_need_refresh;
use sofia_windows_sandbox::resolve_sid;
use sofia_windows_sandbox::sandbox_bin_dir;
use sofia_windows_sandbox::sandbox_dir;
use sofia_windows_sandbox::sandbox_secrets_dir;
use sofia_windows_sandbox::set_local_user_flags;
use sofia_windows_sandbox::setup_error_path;
use sofia_windows_sandbox::setup_log_writer;
use sofia_windows_sandbox::string_from_sid_bytes;
use sofia_windows_sandbox::sync_persistent_deny_read_acls;
use sofia_windows_sandbox::to_wide;
use sofia_windows_sandbox::workspace_write_cap_sid_for_root;
use sofia_windows_sandbox::workspace_write_root_overlaps_path;
use sofia_windows_sandbox::write_file_atomically;
use sofia_windows_sandbox::write_setup_error_report;
...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's SOFIA_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(sofia_home) = std::env::var("SOFIA_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&sofia_home));
...
fn real_main(setup_mode: &mut Option<SetupMode>) -> Result<()> {
    ...
    *setup_mode = Some(payload.mode);
    ...
    let sbx_dir = sandbox_dir(&payload.sofia_home);
    ...
    if let Err(err) = &result {
        ...
        let write_report = match payload.mode {
            SetupMode::ProvisionOnly => serde_json::to_vec_pretty(&report)
                ...
                .and_then(|json| {
                    write_file_atomically(&setup_error_path(&payload.sofia_home), &json)
                }),
            SetupMode::Full | SetupMode::InteractiveProvision | SetupMode::ReadAclsOnly => {
                write_setup_error_report(&payload.sofia_home, &report)
            }
        };
...
fn lock_persistent_sandbox_dirs(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_dir(&payload.sofia_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.sofia_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.sofia_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.sofia_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.sofia_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.sofia_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.sofia_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.sofia_home, &payload.command_cwd, root)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.sofia`, and `.agents`.
        //
...
mod tests {
    ...
    use serde_json::json;
    use sofia_otel::StatsigMetricsSettings;
    use sofia_windows_sandbox::ensure_allow_mask_aces;
    use sofia_windows_sandbox::ensure_allow_write_aces;
    use sofia_windows_sandbox::load_or_create_cap_sids;
    use sofia_windows_sandbox::path_mask_allows;
    use sofia_windows_sandbox::path_write_aces_need_refresh;
    use sofia_windows_sandbox::workspace_write_cap_sid_for_root;
    use std::fs;
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let sofia_home = temp.path().join("sofia-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&sofia_home).expect("create sofia home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &workspace)
    ...
    fn write_root_refresh_checks_each_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &workspace)
            .expect("workspace sid");
        let other_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &other_root)
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &active_root)
            .expect("active sid");
        let workspace_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &workspace)
            .expect("workspace sid");
        let caps = load_or_create_cap_sids(&sofia_home).expect("load caps");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let sofia_home = temp.path().join("sofia-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".sofia");
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &workspace)
            .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &nested_root)
            .expect("nested sid");
