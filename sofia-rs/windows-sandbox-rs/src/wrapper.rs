//! Internal `sofia.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const SOFIA_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const SOFIA_HOME_FLAG: &str = "--sofia-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        SOFIA_WINDOWS_SANDBOX_ARG1.to_string(),
        SOFIA_HOME_FLAG.to_string(),
        sofia_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            sofia_home: request.sofia_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            SOFIA_HOME_FLAG => sofia_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let sofia_home = sofia_home.ok_or_else(|| anyhow!("missing required {SOFIA_HOME_FLAG}"))?;
    if !sofia_home.is_absolute() {
        bail!(
            "{SOFIA_HOME_FLAG} must be absolute: {}",
            sofia_home.display()
        );
    }
