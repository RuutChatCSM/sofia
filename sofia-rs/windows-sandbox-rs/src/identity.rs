...
pub fn sandbox_setup_is_complete(sofia_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(sofia_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(sofia_home), Ok(Some(users)) if users.version_matches())
}
...
pub fn sandbox_setup_is_complete_with_settings(
    ...
    let Ok(Some(mut marker)) = load_marker(sofia_home) else {
        return false;
    };
...
fn load_marker(sofia_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(sofia_home);
...
fn load_users(sofia_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(sofia_home);
...
fn remove_sandbox_users_file(sofia_home: &Path, reason: &str) -> Result<()> {
    let path = sandbox_users_path(sofia_home);
    debug_log(
...
fn select_identity(
    ...
    let _marker = match load_marker(sofia_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(sofia_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(sofia_home);
    let needed_read = read_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| gather_read_roots(command_cwd, permissions, env_map, sofia_home));
    ...
    let marker = load_marker(sofia_home)?;
    ...
    // NOTE: Do not add SOFIA_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match marker {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, sofia_home)?;
                if selected.is_none() {
                    setup_reason = Some(
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, sofia_home)?;
    }
...
pub(crate) fn refresh_logon_sandbox_creds(
    ...
    remove_sandbox_users_file(sofia_home, "sandbox user login failed")?;
...
mod tests {
    ...
    fn remove_sandbox_users_file_deletes_existing_file() {
        ...
        let users_path = sandbox_users_path(sofia_home.path());
        ...
        remove_sandbox_users_file(sofia_home.path(), "stale creds").expect("remove users");
