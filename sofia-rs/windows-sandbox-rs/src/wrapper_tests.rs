...
use super::SOFIA_HOME_FLAG;
use super::SOFIA_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "sofia.exe".to_string(),
            "--sofia-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.sofia"),
    );

    assert_eq!(args[0], SOFIA_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&SOFIA_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["sofia.exe", "--sofia-run-as-fs-helper"]
    );
