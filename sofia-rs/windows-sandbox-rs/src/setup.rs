...
const SETUP_EXE_FILENAME: &str = "sofia-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(sofia_home: &Path) -> PathBuf {
    sofia_home.join(".sandbox")
}

pub fn sandbox_bin_dir(sofia_home: &Path) -> PathBuf {
    sofia_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(sofia_home: &Path) -> PathBuf {
    sofia_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(sofia_home: &Path) -> PathBuf {
    sandbox_dir(sofia_home).join("setup_marker.json")
}

pub fn sandbox_users_path(sofia_home: &Path) -> PathBuf {
    sandbox_secrets_dir(sofia_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let deny_read_paths =
        setup_refresh_deny_read_paths(permission_profile, workspace_roots, command_cwd)?;
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, sofia_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        sofia_home: request.sofia_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.sofia_home)
    })
}

fn run_setup_refresh_payload(b64: &str, sofia_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(sofia_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(sofia_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| sofia_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(sofia_home) {
        log_note(
...
fn gather_helper_read_roots(sofia_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(sofia_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(sofia_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, sofia_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<sofia_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "SOFIA_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "SOFIA_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(sofia_home) {
...
fn verify_setup_completed(sofia_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(sofia_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    if !retained_handles.is_empty() {
        ...
        return run_setup_exe_payload(&payload_b64, needs_elevation, sofia_home, retained_handles);
    }
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, sofia_home, retained_handles)
    })
}
...
fn run_setup_exe_payload(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(sofia_home)?;
        if let Err(err) = clear_setup_error_report(sofia_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.sofia_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.sofia_home, &[])
}
...
fn elevated_provisioning_payload(
    ...
    ElevationPayload {
        ...
        online_username: ONLINE_USERNAME.to_string(),
        sofia_home: request.sofia_home.to_path_buf(),
        command_cwd: request.sofia_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: sofia_otel::global_statsig_metrics_settings(),
...
pub fn run_elevated_provisioning_setup(
    ...
    run_elevated_provisioning_setup_with_retained_handles(sofia_home, real_user, settings, &[])
}
...
pub fn run_elevated_provisioning_setup_with_retained_handles(
    ...
    if !sofia_home.is_absolute()
        || !matches!(
            sofia_home.components().next(),
        ...
        )
    {
        return Err(failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!(
                "sandbox provisioning SOFIA_HOME must be an absolute local disk path: {}",
                sofia_home.display()
            ),
        ));
    }
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: sofia_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
        deny_read_paths: Vec::new(),
        deny_write_paths: Vec::new(),
...
pub(crate) fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.sofia_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sofia_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to SOFIA_HOME or anything under SOFIA_HOME/.sandbox,
    // SOFIA_HOME/.sandbox-bin, or SOFIA_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let sofia_home_key = canonical_path_key(sofia_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(sofia_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(sofia_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(sofia_home));
...
mod tests {
    ...
    use sofia_protocol::permissions::project_roots_glob_pattern;
    use sofia_utils_absolute_path::AbsolutePathBuf;
    use std::collections::HashMap;
    use std::collections::HashSet;
    use std::fs;
    ...
    fn setup_completion_requires_ready_artifacts() {
        let sofia_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(sofia_home.path())
    ...
    fn elevated_setup_payload_contains_no_caller_acl_roots() {
        ...
        assert_eq!(payload.command_cwd, sofia_home);
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("sofia.exe");
        let setup_exe = resources_dir.join("sofia-windows-sandbox-setup.exe");
        fs::write(&exe, b"sofia").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "SOFIA_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    fn expanded_write_roots_still_drop_protected_codex_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let sofia_home = user_profile.join("CodexHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&sofia_home).expect("create sofia home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &sofia_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &sofia_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&sofia_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&sofia_home);
        fs::create_dir_all(&sofia_home).expect("create sofia home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_codex_home = dunce::canonicalize(&sofia_home).expect("canonical sofia home");
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_codex).expect("canonical extra .sofia"),
                explicit_deny,
            ]
            .into_iter()
