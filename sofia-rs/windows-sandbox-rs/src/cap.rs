...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.sofia`)
...
pub fn cap_sid_file(sofia_home: &Path) -> PathBuf {
    sofia_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(sofia_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(sofia_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(sofia_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(sofia_home);
    let mut caps = load_or_create_cap_sids(sofia_home)?;
...
pub fn writable_root_cap_sid_for_path(sofia_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(sofia_home);
    let mut caps = load_or_create_cap_sids(sofia_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(sofia_home, cwd)
    } else {
        writable_root_cap_sid_for_path(sofia_home, root)
    }
}
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&sofia_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&sofia_home, alt_spelling.as_path()).expect("second sid");
    ...
    fn write_roots_get_path_scoped_sids() {
        ...
        let extra_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &extra_root)
            .expect("extra root sid");

        assert_ne!(workspace_sid, extra_sid);
        assert_eq!(
            extra_sid,
            writable_root_cap_sid_for_path(&sofia_home, &extra_root).expect("extra root sid again")
        );
