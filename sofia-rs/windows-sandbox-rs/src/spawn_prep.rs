...
fn prepare_spawn_context_common(
    ...
    ensure_codex_home_exists(sofia_home)?;
    let sandbox_base = sofia_home.join(".sandbox");
...
pub(crate) fn prepare_legacy_session_security(
    ...
    let caps = load_or_create_cap_sids(sofia_home)?;
    let (h_token, readonly_sid, readonly_sid_str, write_root_sids) = unsafe {
        if uses_write_capabilities {
            let write_root_sids = root_capability_sids(sofia_home, cwd, capability_roots)?;
...
pub(crate) fn root_capability_sids(
    ...
    for root in roots {
        let sid_str = workspace_write_cap_sid_for_root(sofia_home, cwd, &root)?;
...
pub(crate) fn prepare_elevated_spawn_context_for_permissions(
    ...
    let (psid_to_use, cap_sids) = if uses_write_capabilities {
        let cap_sids = root_capability_sids(sofia_home, cwd, effective_write_roots)?
            .into_iter()
...
mod tests {
    ...
    fn legacy_deny_path_includes_nested_active_root_sid() {
        ...
        let nested_sid = workspace_write_cap_sid_for_root(&sofia_home, &workspace, &nested_root)
            .expect("nested sid");
        let unrelated_sid =
            workspace_write_cap_sid_for_root(&sofia_home, &workspace, &unrelated_root)
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let workspace = temp.path().join("workspace");
        let active_root = temp.path().join("active-root");
        let sandbox_root = sofia_home.join(".sandbox");
        std::fs::create_dir_all(&sofia_home).expect("create sofia home");
        std::fs::create_dir_all(&workspace).expect("create workspace");
        std::fs::create_dir_all(&active_root).expect("create active root");
        ...
        let writable_roots = vec![
            ...
            AbsolutePathBuf::try_from(sofia_home.as_path()).expect("sofia home"),
        ...
        let roots =
            legacy_session_capability_roots(&permissions, &workspace, &HashMap::new(), &sofia_home);
        ...
        assert!(!roots.contains(&dunce::canonicalize(&sofia_home).expect("sofia home")));
