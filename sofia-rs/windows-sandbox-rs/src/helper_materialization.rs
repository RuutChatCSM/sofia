...
pub(crate) const RESOURCES_DIRNAME: &str = "sofia-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "sofia-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(sofia_home: &Path) -> PathBuf {
    sandbox_bin_dir(sofia_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, sofia_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(sofia_home: &Path, fallback_executable: &str) -> PathBuf {
    let source = match std::env::current_exe() {
        Ok(path) => path,
        Err(_) => return PathBuf::from(fallback_executable),
    };
    resolve_exe_for_launch(&source, sofia_home)
}

pub fn resolve_exe_for_launch(source: &Path, sofia_home: &Path) -> PathBuf {
    let Some(file_name) = source.file_name() else {
        return source.to_path_buf();
    };
    let destination = helper_bin_dir(sofia_home).join(file_name);
    match copy_from_source_if_needed(source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(sofia_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), sofia_home.display());
    ...
    let destination = helper_destination_for_source(kind, sofia_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(sofia_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let sofia_home = Path::new(r"C:\Users\example\.sofia");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.sofia\.sandbox-bin"),
            helper_bin_dir(sofia_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("sofia-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&sofia_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("sofia.exe");
        let helper = resources_dir.join("sofia-command-runner.exe");
        fs::write(&exe, b"sofia").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "sofia-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_resolves_bin_junctions() {
        ...
        fs::write(bin_dir.join("sofia.exe"), b"sofia").expect("write exe");
        let helper = resources_dir.join("sofia-windows-sandbox-setup.exe");
        ...
        assert_eq!(
            bundled_executable_path_for_exe(
                &junction.join("sofia.exe"),
                /*file_name*/ "sofia-windows-sandbox-setup.exe"
            ),
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("sofia.exe");
        let package_helper = package_resources_dir.join("sofia-command-runner.exe");
        let bin_helper = bin_resources_dir.join("sofia-command-runner.exe");
        fs::write(&exe, b"sofia").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("sofia.exe");
        let sibling_helper = release_dir.join("sofia-command-runner.exe");
        let resource_helper = resources_dir.join("sofia-command-runner.exe");
        fs::write(&exe, b"sofia").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "sofia-command-runner-test-suffix.exe");
    }
}
