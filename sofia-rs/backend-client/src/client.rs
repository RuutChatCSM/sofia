...
use sofia_http_client::RouteAwareRequestBuilder;
...
use sofia_protocol::protocol::RateLimitReachedType;
use sofia_protocol::protocol::RateLimitSnapshot;
use sofia_protocol::protocol::RateLimitWindow;
use sofia_protocol::protocol::SpendControlLimitSnapshot;
use std::fmt;
...
impl Client {
    ...
    fn with_http(mut base_url: String, http: RouteAwareClientPool) -> Self {
        ...
        Self {
            base_url,
            http,
            auth_provider: sofia_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            ...
            .with_auth_provider(sofia_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("sofia-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("sofia"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!(
                "{}/api/sofia/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from sofia-backend.
    ///
    /// `GET /api/sofia/config/bundle` (Sofia API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Sofia user settings from the active backend route.
    ///
    /// Uses `GET /api/sofia/settings/user` for Sofia API hosts and
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/tasks", self.base_url),
    ...
    fn make_additional_rate_limit_snapshot(
        details: sofia_backend_openapi_models::models::AdditionalRateLimitDetails,
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<sofia_backend_openapi_models::models::SpendControlStatusDetails>,
        plan_type: Option<AccountPlanType>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!(
                "{}/api/sofia/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/settings/user", self.base_url),
...
mod tests {
    ...
    use sofia_backend_openapi_models::models::AdditionalRateLimitDetails;
    use sofia_backend_openapi_models::models::RateLimitReachedKind;
    use sofia_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    use wiremock::Mock;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "sofia_other".to_string(),
                metered_feature: "sofia_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                sofia_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("sofia"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("sofia_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("sofia_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("sofia"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let sofia_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            sofia_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/sofia/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let sofia_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            sofia_client.token_usage_profile_url(),
            "https://example.test/api/sofia/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let sofia_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            sofia_client.workspace_messages_url(),
            "https://example.test/api/sofia/workspace-messages"
        );
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        let sofia_response = Client::new(
            server.uri(),
            HttpClientFactory::new(sofia_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [sofia_response, chatgpt_response],
            [
