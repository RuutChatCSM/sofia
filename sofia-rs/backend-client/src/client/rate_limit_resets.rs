...
impl Client {
    ...
    async fn get_rate_limit_status(
        ...
        if supports_luna_reserve {
            req = req.header("x-openai-sofia-luna-reserve", HeaderValue::from_static("1"));
        }
    ...
    fn rate_limit_status_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/sofia/usage", self.base_url),
    ...
    fn rate_limit_reset_credits_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => {
                format!("{}/api/sofia/rate-limit-reset-credits", self.base_url)
            }
    ...
    fn consume_rate_limit_reset_credit_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => {
                format!(
                    "{}/api/sofia/rate-limit-reset-credits/consume",
                    self.base_url
                )
            }
            PathStyle::ChatGptApi => {
