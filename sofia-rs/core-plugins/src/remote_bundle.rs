...
use sofia_utils_plugins::AgentPluginSchemaStatus;
use sofia_utils_plugins::agent_plugin_schema_status;
...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: sofia_http_client::HttpError,
    },
...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(sofia_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = sofia_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(sofia_home)?;
...
mod tests {
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".sofia-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".sofia-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create manifest dir");
        std::fs::write(
