...
fn pretrust_startup_sync_uses_installed_git_with_hostile_path() {
    const CHILD_HOME: &str = "SOFIA_TEST_CURATED_SYNC_HOME";
        ...
        for plugin in plugins {
            assert!(
                curated_plugins_repo_path(&home)
                    .join(format!("plugins/{plugin}/.sofia-plugin/plugin.json"))
                    .is_file()
            );
        }
    }
...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(sofia_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
fn pretrust_git_sync_ignores_repository_local_transport_config() {
    ...
    let sofia_home = fixture.path().join("sofia-home");
    ...
    let err = sync_openai_plugins_repo_via_git(&sofia_home, &git_wrapper)
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    run_setup_git(
        ...
        &[
            "-c",
            "user.name=Sofia Tests",
            "-c",
            "user.email=sofia-tests@example.com",
            "commit",
            "--quiet",
            "-m",
    ...
    let apply_error = sofia_git_utils::apply_git_patch(&sofia_git_utils::ApplyGitRequest {
        cwd: nested.clone(),
        diff: String::new(),
        revert: false,
        preflight: true,
    ...
    assert!(sofia_git_utils::collect_git_info(&nested).await.is_none());
    assert!(sofia_git_utils::git_diff_to_remote(&nested).await.is_none());
    assert!(
        !marker.exists(),
    ...
    let explicit_environment = Command::new("git")
        .args(["-c", sofia_git_utils::SAFE_BARE_REPOSITORY_CONFIG])
        .args(["rev-parse", "--git-dir"])
        .env("GIT_DIR", &nested)
        .current_dir(&clone)
        .output()
...
fn write_curated_plugin_sha(sofia_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(sofia_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(sofia_home.join(".tmp")) else {
        return false;
    };
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.sofia-plugin/plugin.json")
            .is_file()
    );
}
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.sofia-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.sofia-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.sofia-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.sofia-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.sofia-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.sofia-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.sofia-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.sofia-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");
