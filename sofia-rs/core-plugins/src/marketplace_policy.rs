...
use sofia_config::MarketplaceAllowedSourceKind;
use sofia_config::MarketplaceAllowedSourceToml;
use sofia_config::RequirementSource;
use sofia_config::types::MarketplaceConfig;
use sofia_config::types::MarketplaceSourceType;
use sofia_config::types::PluginConfig;
use sofia_plugin::PluginId;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_path::paths_match_after_normalization;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(sofia_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(sofia_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn policy_filtered_plugin_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&effective_config, &policy, sofia_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(sofia_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(effective_config) = policy_filtered_plugin_config(config_layer_stack, sofia_home)
    else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(sofia_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if is_expected_managed_path(
        ...
        &curated_plugins_api_marketplace_path(sofia_home),
    ) && is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(sofia_home))
    {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(sofia_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(sofia_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(sofia_home, path)
}

fn managed_local_marketplace_name(sofia_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = sofia_home
...
fn is_expected_managed_path(path: &Path, expected: &Path) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        let path = sofia_utils_absolute_path::normalize_windows_device_path(&path)
            .unwrap_or_else(|| path.into_owned());
        let expected = sofia_utils_absolute_path::normalize_windows_device_path(&expected)
...
pub(crate) fn primary_runtime_marketplace_root() -> Option<PathBuf> {
    Some(
        primary_runtime_cache_dir()?
            .join("sofia-runtimes/sofia-primary-runtime/plugins")
