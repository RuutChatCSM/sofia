...
fn legacy_codex_overlay_keeps_portable_components_fixed() {
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create overlay dir");
    fs::write(
  ...
  "skills": [],
  "mcpServers": null,
  "interface": {"displayName": "Sofia Demo"}
}"#,
    )
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline Sofia"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".sofia-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy Sofia"}}"#,
    )
    .expect("write overlay");
