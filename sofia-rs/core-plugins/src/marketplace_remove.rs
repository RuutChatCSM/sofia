...
use sofia_config::RemoveMarketplaceConfigOutcome;
use sofia_config::format_config_layer_source;
use sofia_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        remove_marketplace_sync(sofia_home.as_path(), &config_layer_stack, request)
    })
    .await
    .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let user_config_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sofia_home);
    ...
    let destination = marketplace_install_root(sofia_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(sofia_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        let config =
            fs::read_to_string(sofia_home.path().join(sofia_config::CONFIG_TOML_FILE)).unwrap();
    ...
    fn remove_marketplace_sync_preserves_shadowed_system_marketplace() {
        ...
        let user_file =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sofia_home.path());
        let system_file =
            AbsolutePathBuf::resolve_path_against_base("system.toml", sofia_home.path());
