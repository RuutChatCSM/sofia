...
use sofia_http_client::RouteAwareRequestError;
...
const SOFIA_PRODUCT_SKU: &str = "sofia";
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(
        sofia_home, config, auth, scope, /*collection*/ None, &plugins,
    );
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(sofia_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(
            sofia_home, config, auth, *scope, /*collection*/ None,
        );
...
pub async fn fetch_recommended_plugins(
    ...
    let mut url = Url::parse(&format!("{base_url}/ps/plugins/suggested/sofia"))
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(sofia_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(sofia_home.clone())
    ...
    let legacy_remote_plugin_cache_root = sofia_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode != RemotePluginCatalogCacheMode::ForceRefetch
        && let Some(sofia_home) = sofia_home
        && let Some(cached) = catalog_cache::load_cached_directory_plugins(
            sofia_home, config, auth, scope, collection,
        )
    ...
    if let Some(sofia_home) = sofia_home {
        catalog_cache::write_cached_directory_plugins(
            sofia_home, config, auth, scope, collection, &plugins,
        );
    }
...
fn authenticated_request(
    ...
    request
        ...
        .headers(sofia_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, SOFIA_PRODUCT_SKU)
}
