...
const PERSONAL_MARKETPLACE_NAME: &str = "sofia-curated";
...
pub async fn checkout_remote_plugin_share(
    ...
    let local_paths = load_share_local_paths_for_checkout(sofia_home)?;
...
fn load_share_local_paths_for_checkout(
    ...
    match local_paths::load_plugin_share_local_paths(sofia_home) {
        Ok(paths) => Ok(paths),
