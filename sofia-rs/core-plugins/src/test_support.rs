...
pub(crate) fn test_plugins_manager(sofia_home: PathBuf) -> PluginsManager {
    PluginsManager::new(
        sofia_home,
...
pub(crate) fn write_curated_plugin_sha_with(sofia_home: &Path, sha: &str) {
    write_file(&sofia_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(sofia_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let sofia_home = AbsolutePathBuf::try_from(sofia_home).expect("sofia home should be absolute");
