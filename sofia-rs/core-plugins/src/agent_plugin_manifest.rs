...
use sofia_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use sofia_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use sofia_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;

const SOFIA_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let sofia_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(SOFIA_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = SOFIA_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    if let Some(extension_contents) = sofia_extension.as_deref() {
