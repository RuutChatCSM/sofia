...
use sofia_config::clear_user_plugin;
use sofia_config::set_user_plugin_enabled;
use sofia_config::skill_config_rules_from_stack;
use sofia_config::types::PluginConfig;
use sofia_config::types::ToolSuggestDisabledTool;
use sofia_config::types::ToolSuggestDiscoverableType;
use sofia_hooks::plugin_hook_declarations;
...
use sofia_plugin::PluginIdError;
use sofia_plugin::PluginTelemetryMetadata;
use sofia_plugin::app_connector_ids_from_declarations;
use sofia_plugin::prompt_safe_plugin_description;
...
use sofia_skills::SkillRootLoader;
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given SOFIA_HOME:
        ...
        // contents of that SOFIA_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single SOFIA_HOME is only used by one product.
        let remote_installed_plugin_bundle_sync_gate =
            crate::remote::remote_installed_plugin_bundle_sync_gate(&sofia_home);
        Self {
            sofia_home: sofia_home.clone(),
            store: PluginStore::new(sofia_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.sofia_home.as_path())
                    .ok_or_else(|| {
        ...
        let sofia_home = self.sofia_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(sofia_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.sofia_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(sofia_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if sofia_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
    ...
    fn start_curated_repo_sync(
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(sofia_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_codex_home(sofia_home.as_path());
    ...
    fn configured_plugin_states(&self, config: &PluginsConfigInput) -> ConfiguredPluginStates {
        let configured_plugins =
            configured_plugins_from_stack(&config.config_layer_stack, self.sofia_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            match target_curated_marketplace(self.auth_mode()) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root = curated_plugins_repo_path(self.sofia_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.sofia_home.as_path());
