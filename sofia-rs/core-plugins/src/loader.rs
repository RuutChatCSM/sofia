...
use sofia_config::types::PluginMcpServerConfig;
use sofia_connectors::parse_plugin_app_config;
use sofia_connectors::parse_plugin_app_config_value;
use sofia_mcp::parse_agent_plugin_mcp_config;
use sofia_mcp::parse_plugin_mcp_config;
use sofia_plugin::AppDeclaration;
use sofia_plugin::LoadedPlugin;
...
use sofia_skills::SkillRootLoadRequest;
...
use sofia_utils_plugins::migrated_command_skills_root;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let mut configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.sofia_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(sofia_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(sofia_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = sofia_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized =
                materialize_marketplace_plugin_source_with_mode(sofia_home, &source, git_mode)
                    .map_err(|err| {
...
pub fn configured_curated_plugin_ids_from_codex_home(sofia_home: &Path) -> Vec<PluginId> {
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".sofia-plugin/plugin.json"));
...
pub async fn load_configured_plugin_mcp_servers(
    ...
    let configured_plugins = configured_plugins_from_stack(config_layer_stack, sofia_home);
...
pub fn materialize_marketplace_plugin_source(
    ...
    materialize_marketplace_plugin_source_with_mode(sofia_home, source, PluginGitMode::Manual)
}
...
pub(crate) fn materialize_marketplace_plugin_source_with_mode(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = sofia_home.join("plugins/.marketplace-plugin-source-staging");
...
fn clone_git_plugin_source(
    ...
    let clone_cwd = match mode {
        PluginGitMode::Automatic => Some(sofia_home),
