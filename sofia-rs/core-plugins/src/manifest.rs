...
use sofia_config::HooksFile;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_path_uri::PathConvention;
use sofia_utils_path_uri::PathUri;
use sofia_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use sofia_utils_plugins::find_plugin_manifest_path;
use std::fs;
use std::io;
use std::path::Path;
...
pub type PluginManifest = sofia_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = sofia_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = sofia_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    sofia_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = sofia_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = sofia_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".sofia-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = sofia_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(sofia_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: sofia_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<sofia_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| sofia_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(sofia_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(sofia_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(sofia_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<sofia_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(sofia_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(sofia_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use sofia_plugin::PluginProvider;
    use sofia_plugin::ResolvedPlugin;
    use sofia_plugin::manifest::PluginManifest as GenericPluginManifest;
    use sofia_plugin::manifest::PluginManifestHooks;
    use sofia_plugin::manifest::PluginManifestInterface;
    use sofia_plugin::manifest::PluginManifestMcpServers;
    use sofia_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".sofia-plugin/plugin.json")).expect("manifest");
