...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        add_marketplace_sync(sofia_home.as_path(), &requirements, request)
    })
    .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(sofia_home, requirements, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let managed_marketplace_name =
        validate_marketplace_source_for_add(sofia_home, requirements, &source)
    ...
    let install_root = marketplace_install_root(sofia_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(sofia_home, &install_root, &install_metadata)?
    {
        ...
        validate_marketplace_name_for_add(managed_marketplace_name, &marketplace_name)
            .map_err(MarketplaceAddError::InvalidRequest)?;
        record_added_marketplace_entry(sofia_home, &marketplace_name, &install_metadata)?;
    ...
    if let MarketplaceSource::Local { path } = &source {
        ...
        validate_marketplace_name_for_add(managed_marketplace_name, &marketplace_name)
            .map_err(MarketplaceAddError::InvalidRequest)?;
        if find_marketplace_root_by_name(sofia_home, &install_root, &marketplace_name)?.is_some() {
...
mod tests {
    ...
    fn add_marketplace_sync_installs_marketplace_and_updates_config() -> Result<()> {
        ...
        let config = fs::read_to_string(sofia_home.path().join(sofia_config::CONFIG_TOML_FILE))?;
...
"#,
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.sofia-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.sofia-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;
