...
use sofia_exec_server::DiscoveredPluginFiles;
...
fn expected_source(index: usize) -> ExecutorPluginHookSource {
    ExecutorPluginHookSource {
        ...
        manifest_path: PathUri::parse("file:///plugins/computer-use/.sofia-plugin/plugin.json")
            .expect("manifest path"),
        source_relative_path: format!(".sofia-plugin/plugin.json#hooks[{index}]"),
...
fn discovers_allowlisted_executor_plugin_hook_sources() {
    ...
    let snapshot = snapshot_for_manifest(
        ...
        "executor-a",
        "file:///plugins/computer-use/.sofia-plugin/plugin.json",
        manifest,
    );
...
fn resolves_apps_hook_metadata_from_the_registered_connector() {
    ...
    let handler = &mut manifest["hooks"]["hooks"]["Stop"][0]["hooks"][0];
    handler["server"] = json!("sofia_apps");
