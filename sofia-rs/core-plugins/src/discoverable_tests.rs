...
async fn returns_fallback_plugins_when_remote_disabled_for_codex_auth() {
    let sofia_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(sofia_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);

    let plugins = load_plugins_config(sofia_home.path(), sofia_home.path()).await;
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(sofia_home.path(), curated_root.as_path(), "teams").await;
...
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_codex_auth() {
    ...
    let bundled_marketplace_root = sofia_home.path().join(format!(
...
async fn includes_openai_api_curated_when_remote_enabled_without_auth() {
    ...
    let plugins = load_plugins_config(sofia_home.path(), sofia_home.path()).await;
    let plugins_manager = test_plugins_manager(sofia_home.path().to_path_buf());
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        /*auth*/ None,
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(sofia_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.sofia-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(sofia_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.sofia-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(sofia_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(sofia_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.sofia-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.sofia-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.sofia-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = sofia_home
        .path()
...
"#
    ...
    install_marketplace_plugin(sofia_home.path(), sales_marketplace_root.as_path(), "sales").await;
...
"#,
    ...
    let auth = CodexAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(sofia_home.path(), sofia_home.path()).await;
...
async fn install_marketplace_plugin(sofia_home: &Path, marketplace_root: &Path, plugin_name: &str) {
    write_curated_plugin_sha_with(sofia_home, TEST_CURATED_PLUGIN_SHA);
    let config = load_plugins_config(sofia_home, marketplace_root).await;
    test_plugins_manager(sofia_home.to_path_buf())
        .install_plugin(
            &config,
