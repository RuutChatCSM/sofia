...
use sofia_config::McpServerOAuthConfig;
use sofia_config::McpServerToolConfig;
use sofia_config::RequirementSource;
use sofia_config::RequirementsLayerEntry;
use sofia_config::SkillConfigRules;
use sofia_config::compose_requirements;
use sofia_config::types::McpServerTransportConfig;
use sofia_login::CodexAuth;
use sofia_login::test_support::auth_manager_with_agent_identity;
use sofia_model_provider::AMAZON_BEDROCK_PROVIDER_ID;
use sofia_plugin::AppDeclaration;
use sofia_plugin::PluginId;
use sofia_protocol::auth::AuthMode;
use sofia_protocol::protocol::HookEventName;
use sofia_protocol::protocol::Product;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_absolute_path::test_support::PathBufExt;
use sofia_utils_plugins::SkillDiscoveryMode;
use std::fs;
use std::path::Path;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file =
        AbsolutePathBuf::try_from(sofia_home.join(CONFIG_TOML_FILE)).expect("absolute config path");
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(sofia_home, user_config, requirements),
        String::new(),
        /*plugins_enabled*/ true,
        /*remote_plugin_enabled*/ false,
        String::new(),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let sofia_home = TempDir::new().expect("create Sofia home");
    write_plugin(
        &sofia_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = test_plugins_manager(sofia_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let sofia_home = TempDir::new().expect("create Sofia home");
    let marketplace_root = sofia_home.path().join("marketplace");
...
"#,
    ...
    let err = test_plugins_manager(sofia_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let sofia_home = TempDir::new().expect("create Sofia home");
    let repo_root = sofia_home.path().join("repo");
...
fn write_auth_projection_plugin(sofia_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(sofia_home, name, name);
    }
}

fn write_auth_projection_app(sofia_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = sofia_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(sofia_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&sofia_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(sofia_home, sofia_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    let sofia_home = TempDir::new().unwrap();
    write_auth_projection_plugin(sofia_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(sofia_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(sofia_home.path()).await;
...
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_chatgpt_apps_route() {
    let sofia_home = TempDir::new().unwrap();
    write_auth_projection_plugin(sofia_home.path(), "sample", /*include_app*/ false);
    write_auth_projection_app(sofia_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(sofia_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(sofia_home.path()).await;
    let manager = test_plugins_manager_with_options(
        sofia_home.path().to_path_buf(),
...
"#,
    ...
    let config = load_config(sofia_home.path(), sofia_home.path()).await;
    let manager = test_plugins_manager_with_options(
        sofia_home.path().to_path_buf(),
        Some(Product::Sofia),
        Some(AuthMode::Chatgpt),
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".sofia-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "sofia-test@example.com"]);
    run_git(repo, &["config", "user.name", "Sofia Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&sofia_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(sofia_home, sofia_home).await;
    test_plugins_manager_with_options(sofia_home.to_path_buf(), Some(Product::Sofia), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(sofia_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(sofia_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        installed_at: None,
        enabled: true,
        install_policy: sofia_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: sofia_app_server_protocol::PluginAuthPolicy::OnUse,
...
fn write_cached_plugin(sofia_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &sofia_home
            .join("plugins/cache")
...
"#;
    let outcome =
        load_plugins_from_config(config_toml, sofia_home.path(), /*auth_mode*/ None).await;
...
async fn remote_installed_plugin_preserves_configured_mcp_server_policy() {
    ...
    write_cached_plugin(sofia_home.path(), "openai-curated-remote", "linear");
    write_file(
        &plugin_root.join(".mcp.json"),
        r#"{
  "mcpServers": {
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let sofia_home = TempDir::new().unwrap();
    write_cached_plugin(sofia_home.path(), "test", "sample");
    let manager = test_plugins_manager(sofia_home.path().to_path_buf());
...
async fn installed_plugin_telemetry_metadata_resolves_persisted_remote_identity() {
    ...
    PluginStore::new(sofia_home.path().to_path_buf())
...
"#,
    ...
    write_cached_plugin(sofia_home.path(), "openai-curated", "linear");
    write_cached_plugin(sofia_home.path(), "openai-curated", "calendar");
    write_cached_plugin(sofia_home.path(), "openai-api-curated", "linear");
    write_cached_plugin(sofia_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(sofia_home.path(), "openai-curated-remote", "remote-only");
...
"#,
    ...
    let mut config = load_config(sofia_home.path(), sofia_home.path()).await;
    let auth_manager = test_auth_manager(/*auth_mode*/ None);
    set_test_auth_mode(&auth_manager, Some(AuthMode::Chatgpt)).await;
    let manager = test_plugins_manager_with_auth_manager(
        sofia_home.path().to_path_buf(),
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = sofia_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = sofia_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(sofia_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        ...
        sofia_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        sofia_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    let sofia_home = TempDir::new().unwrap();
    let source_root = sofia_home.path().join("source/sample");

    write_file(
        &source_root.join(".sofia-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".sofia-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(sofia_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".sofia-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".sofia-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".sofia-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".sofia-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".sofia-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let sofia_home = TempDir::new().unwrap();
    let plugin_a_root = sofia_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = sofia_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".sofia-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".sofia-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, sofia_home.path(), Some(AuthMode::Chatgpt)).await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(sofia_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        skill_discovery_mode: SkillDiscoveryMode::Recursive,
        disabled_skill_paths: HashSet::new(),
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![sofia_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![sofia_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(sofia_home.path(), sofia_home.path()).await;
    let outcome = test_plugins_manager(sofia_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_reuses_effective_configurations() {
    ...
    let plugin_root = sofia_home
        ...
        .join("test/sample/local");
    write_plugin(
        sofia_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = sofia_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let sofia_home = TempDir::new().expect("create Sofia home");
    let marketplace_root = sofia_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = plugins_config_input_with_requirements(sofia_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = test_plugins_manager(sofia_home.path().to_path_buf());
    ...
    assert!(!sofia_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    write_file(&sofia_home.path().join(CONFIG_TOML_FILE), &user_config);
    let config =
        plugins_config_input_with_requirements(sofia_home.path(), &user_config, &requirements);
    let outcome = manager
        .install_plugin(
            &config,
            PluginInstallRequest {
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".sofia-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".sofia-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".sofia-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".sofia-plugin/plugin.json").is_file());
}
...
async fn read_plugin_for_config_filters_mcp_servers_for_codex_backend_auth() {
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.sofia-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".sofia-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".sofia/config.toml"),
        r#"[[skills.config]]
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.sofia-plugin/plugin.json"),
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["sofia-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["sofia-plugin".to_string()]);
}
...
"#,
    ...
    let config = load_plugins_config_input(sofia_home.path(), &marketplace_root).await;
    ...
    let manager = Arc::new(test_plugins_manager(sofia_home.path().to_path_buf()));
    manager
...
async fn load_plugins_uses_project_config_files() {
    let sofia_home = TempDir::new().unwrap();
    let project_root = sofia_home.path().join("project");
    let plugin_root = sofia_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".sofia/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::try_from(project_root.join(".sofia")).unwrap(),
            },
...
"#,
    ...
    for marketplace_name in [
        ...
        write_cached_plugin(sofia_home.path(), marketplace_name, "linear");
        write_file(
            &sofia_home
                .path()
                .join("plugins/cache")
...
"#,
    ...
    write_cached_plugin(sofia_home.path(), REMOTE_GLOBAL_MARKETPLACE_NAME, "linear");
    write_cached_plugin(
        sofia_home.path(),
        REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME,
        "malformed",
    ...
    let plugin_cache_root = sofia_home.path().join("plugins/cache");
    ...
    let linear_metadata = plugin_cache_root
        ...
        .join("linear")
        .join(".sofia-remote-plugin-install.json");
