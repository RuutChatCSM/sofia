...
impl RemoteAppServerClient {
    ...
    pub fn sofia_home(&self) -> Option<&str> {
        self.metadata.sofia_home.as_deref()
    }
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            metadata.sofia_home = response
                                .result
                                .get("codexHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|sofia_home| !sofia_home.is_empty())
                                .map(str::to_string);
