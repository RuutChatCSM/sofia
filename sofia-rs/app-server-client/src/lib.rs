...
//!
//! This crate wraps [`sofia_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](sofia_app_server::in_process::InProcessClientHandle),
...
pub use sofia_app_server::app_server_control_socket_path;
pub use sofia_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use sofia_app_server::in_process::InProcessServerEvent;
use sofia_app_server::in_process::InProcessStartArgs;
use sofia_app_server::in_process::LogDbLayer;
pub use sofia_app_server::in_process::StateDbHandle;
...
use sofia_app_server_protocol::Result as JsonRpcResult;
...
use sofia_config::NoopThreadConfigLoader;
use sofia_core::config::Config;
pub use sofia_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `sofia-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use sofia_core::config::*;

        pub mod edit {
            pub use sofia_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `SOFIA_API_KEY` environment variable.
...
/// - [`sofia_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            sofia_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    ...
    pub fn sofia_home(&self, local_codex_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_codex_home.display().to_string(),
            )),
            Self::Remote(client) => client.sofia_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use sofia_app_server_protocol::ServerNotification;
    use sofia_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use sofia_core::init_state_db;
    use sofia_protocol::config_types::Personality;
    use sofia_uds::UnixListener;
    use sofia_utils_absolute_path::AbsolutePathBuf;
    use std::ops::Deref;
    use std::path::Path;
    ...
    async fn build_test_config_for_codex_home(sofia_home: &Path) -> Config {
    ...
    async fn start_test_client_with_capacity(
        ...
        let sofia_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_codex_home(sofia_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "sofia-app-server-client-test".to_string(),
        ...
        S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        expect_remote_initialize_with_metadata(
            websocket,
            serde_json::json!({
                "userAgent": "sofia_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                "codexHome": "/server/.sofia",
            }),
        )
        .await;
    }
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            sofia_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            sofia_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(sofia_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: sofia_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(sofia_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: sofia_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: sofia_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: sofia_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: sofia_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<sofia_app_server_protocol::ThreadReadResponse>(
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.sofia_home(), Some("/server/.sofia"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: sofia_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("sofia.sock"))
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(notification) => match notification.as_ref() {
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            sofia_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        ) =>
                    {
