...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    sofia_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("sofia-test", "0.0.0-test").with_title("Sofia rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<sofia_http_client::HttpClient> {
    Ok(sofia_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `sofia exec-server --listen`.
