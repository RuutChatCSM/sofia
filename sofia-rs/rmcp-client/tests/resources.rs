...
use sofia_rmcp_client::mcp_error;
use sofia_utils_cargo_bin::CargoBinError;

const RESOURCE_URI: &str = "memo://sofia/example-note";

fn stdio_server_bin() -> Result<PathBuf, CargoBinError> {
    sofia_utils_cargo_bin::cargo_bin("test_stdio_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("sofia-test", "0.0.0-test").with_title("Sofia rmcp resource test"),
    )
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    let mut expected_templates = ListResourceTemplatesResult::with_all_items(vec![
        rmcp::model::ResourceTemplate::new("memo://sofia/{slug}", "sofia-memo")
...
async fn rmcp_client_preserves_each_resource_error() -> anyhow::Result<()> {
    let client = resource_client().await?;
    for uri in ["memo://sofia/missing-first", "memo://sofia/missing-second"] {
        let error = client
            .read_resource(
