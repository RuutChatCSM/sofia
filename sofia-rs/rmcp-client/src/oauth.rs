...
//!
//! If the keyring is not available or fails, we fall back to SOFIA_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Sofia MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring_with_lock_held<K: KeyringStore + Clone + 'static>(
    ...
    let sofia_home = find_codex_home().map_err(anyhow::Error::from)?;
...
fn compute_store_key(server_name: &str, server_url: &str) -> Result<String> {
    ...
    let payload = if enterprise_owned {
        ...
        // isolated by Sofia profile as well as authenticated user and workspace.
        let sofia_home = find_codex_home()?;
        fs::create_dir_all(&sofia_home)?;
        payload.insert(
            "sofia_home".to_string(),
            serde_json::to_value(sofia_home.as_path().canonicalize()?)?,
        );
...
mod tests {
    ...
    fn fallback_file_is_private_at_creation() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;
        const CHILD: &str = "SOFIA_TEST_OAUTH_PERMISSIVE_UMASK";
