...
const MEMO_URI: &str = "memo://sofia/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "sofia/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Sofia TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Sofia TUI):
    /// - Build this binary: `cargo build -p sofia-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `sofia mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Sofia TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://sofia/{slug}", "sofia-memo")
            .with_title("Sofia Memo")
            .with_description("Template for memo://sofia/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise Sofia TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("sofia-rmcp-test-server", env!("CARGO_PKG_VERSION"))
        ...
        match request.name.as_ref() {
            "js" => {
                ...
                if args.code == "nodeRepl.fail()" {
                    ...
                    let mut meta = MetaObject::new();
                    meta.insert("sofia/imageDetail".to_string(), json!("low"));
                    Ok(CallToolResult::success(vec![
            ...
            "encrypted_output" => {
                let mut meta = MetaObject::new();
                meta.insert("sofia/encryptedContent".to_string(), json!(true));
