//! Adapts Sofia's pinned credential storage to RMCP-owned OAuth refreshes.
//!
...
//! the pinned store before RMCP exchanges the token and saves the result. Sofia
        ...
        Self: 'async_trait,
    {
        Box::pin(async move {
            let mut token_response = credentials
                .token_response
                .ok_or(AuthError::AuthorizationRequired)?;
            // Sofia stores granted scopes in the token response rather than a separate field.
