...
const LOCK_CONTENTION_EVENT_TARGET: &str = "sofia_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are SOFIA_HOME-backed today: if SOFIA_HOME is unset they use
        // the default home (`~/.sofia`), and if an embedder has no local home/filesystem authority
        ...
        Self::acquire_in_with_mode(&sofia_home, store, acquire_timeout, mode)
    }
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(sofia_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve SOFIA_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(sofia_home: &Path, store: OAuthStore) -> PathBuf {
    sofia_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}
