...
//!
//! The history is stored at `~/.sofia/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.sofia`.
...
impl HistoryConfig {
    pub fn new(sofia_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            sofia_home: sofia_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.sofia_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.sofia/history.jsonl` and ensure the parent directory exists.
