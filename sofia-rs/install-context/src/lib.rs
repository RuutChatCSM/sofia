...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "sofia-code-mode-host.exe"
} else {
    "sofia-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "sofia-package.json";
const PATH_DIRNAME: &str = "sofia-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "sofia-resources";
...
pub struct CodexPackageLayout {
    ...
    /// Directory containing the Sofia entrypoint executable.
...
/// Version metadata recorded in a bundled Sofia runtime package.
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.sofia/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `sofia-resources/`, and `sofia-path/`.
    ...
    /// A Sofia binary launched through the npm-managed `sofia.js` shim.
    Npm,
    /// A Sofia binary launched through the bun-managed `sofia.js` shim.
    Bun,
    /// A Sofia binary launched through the pnpm-managed `sofia.js` shim.
    Pnpm,
    /// A Sofia binary launched through the Vite+-managed `sofia.js` shim.
    VitePlus,
    /// A Sofia binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Sofia binaries, custom
    /// internal launchers, and tests that execute Sofia from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let sofia_home = sofia_utils_home_dir::find_codex_home().ok();
    ...
    fn from_exe_with_codex_home(
        ...
        let method = if let Some(method) = method_override {
            ...
            install_method_from_exe(exe_path, sofia_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("SOFIA_MANAGED_BY_VITE_PLUS").is_some() {
                Some(InstallMethod::VitePlus)
            } else if std::env::var_os("SOFIA_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("SOFIA_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("SOFIA_MANAGED_BY_BUN").is_some() {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under sofia-resources
    ...
    fn from_exe(exe_path: &Path) -> Option<Self> {
        ...
        match exe_dir.file_name() {
            ...
            Some(name) if name == OsStr::new("MacOS") => {
                ...
                // package. current_exe points inside the bundle, not at bin/sofia.
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, sofia_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_codex_home = canonical_absolute_path(sofia_home?)?;
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "sofia-test-helper";
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "sofia.exe" } else { "sofia" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = sofia_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "sofia.exe" } else { "sofia" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_codex_home(
            ...
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*sofia_home*/ Some(sofia_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
...
"#,
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("sofia");
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "sofia.exe" } else { "sofia" });
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-sofia");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new("/opt/sofia/bin/sofia"))),
            PathBuf::from("/opt/sofia/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let vite_plus_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/sofia")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_codex_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/sofia")),
            /*method_override*/ None,
            /*sofia_home*/ None,
        );
        assert_eq!(
