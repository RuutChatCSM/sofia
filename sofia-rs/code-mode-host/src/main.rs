...
use sofia_otel_trace_websocket::TraceWebSocket;
...
struct Cli {
    ...
    #[arg(
        long,
        value_name = "URL",
        default_value = sofia_code_mode_host::DEFAULT_LISTEN_URL
    )]
    listen: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(trace_transport) = trace_transport.as_ref() {
        ...
        tracing::info!("sofia-code-mode-host OTEL trace websocket listening on ws://{listen_addr}");
    }
    tracing::info_span!(
    ...
    let main_transport = sofia_code_mode_host::run_main(&cli.listen);
...
fn build_trace_provider(endpoint: &str) -> anyhow::Result<OtelProvider> {
    OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "sofia-code-mode-host".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        sofia_home: PathBuf::from("/tmp"),
