...
use sofia_utils_string::truncate_middle_chars;
...
pub enum CodexErrorDetails {
    ...
    #[error(
        "Sofia ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    QuotaExceeded,
    #[error(
        "To use Sofia with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("sofia-linux-sandbox was required but not provided")]
...
macro_rules! sofia_err_unit_constructors {
    ($($variant:ident),* $(,)?) => {
        $(
            #[doc(hidden)]
...
// TODO(anp): Remove this compatibility macro once callers construct
// `CodexErrorDetails` directly.
macro_rules! sofia_err_tuple_constructors {
...
impl CodexErr {
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            sofia_error_info: Some(self.to_codex_protocol_error()),
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("sofia")
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Sofia (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
