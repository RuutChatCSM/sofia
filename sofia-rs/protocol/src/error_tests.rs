...
#[test]
fn sofia_err_debug_preserves_legacy_shape() {
    let actual = [
        CodexErr::Timeout,
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Sofia (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_hides_upsell_for_non_codex_limit_name() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit for sofia_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn unexpected_status_prefers_error_message_when_present() {
    let err = UnexpectedResponseError {
        ...
        body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
            .to_string(),
        user_message: None,
        url: Some("https://chatgpt.com/backend-api/sofia/responses".to_string()),
        cf_ray: None,
        request_id: Some("req-123".to_string()),
        identity_authorization_error: None,
        identity_error_code: None,
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: Workspace is not authorized in this region., url: https://chatgpt.com/backend-api/sofia/responses, request id: req-123"
        )
    );
}
...
fn unexpected_status_includes_cf_ray_and_request_id() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/sofia/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
        )
    );
}
...
fn unexpected_status_includes_identity_auth_details() {
    let err = UnexpectedResponseError {
        ...
        body: "plain text error".to_string(),
        user_message: None,
        url: Some("https://chatgpt.com/backend-api/sofia/models".to_string()),
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/sofia/models, cf-ray: cf-ray-auth-401-test, request id: req-auth, auth error: missing_authorization_header, auth error code: token_expired"
        )
    );
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Sofia, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Sofia, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
