...
//! `schemars`) so they can be embedded in Sofia's own protocol structures.
...
/// MCP extensions supplied by the client that created a Sofia session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `sofia-protocol` so other crates can convert
