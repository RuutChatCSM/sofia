//! Defines the protocol for a Sofia session between a client and an agent.
//!
...
use sofia_extension_items::image_generation::ImageGenerationFailure;
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Sofia:";
...
pub struct ConversationStartParams {
    /// Whether Sofia response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Sofia when the session ends.
    ...
    /// Sends automatic Sofia responses as realtime conversation items instead of handoff appends.
    pub sofia_responses_as_items: bool,
    /// Optional prefix added to automatic Sofia response items when `sofia_responses_as_items` is set.
    pub sofia_response_item_prefix: Option<String>,
    /// Selects how automatic Sofia handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub sofia_response_handoff_mode: CodexResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub sofia_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Sofia's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Sofia when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Sofia when this realtime session ends.
...
pub enum Op {
    ...
    /// Request Sofia to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down sofia instance.
    Shutdown,
...
/// running the command proposed by Sofia.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.sofia`, `.git`, notably `.git/hooks`) under
...
/// Sofia errors that we expose to clients.
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub sofia_error_info: Option<CodexErrorInfo>,
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub sofia_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Sofia),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Sofia
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(sofia_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                sofia_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("sofia".to_string()).restriction_product(),
            Some(Product::Sofia)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Sofia])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Sofia]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = sofia_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            sofia_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".sofia")).expect("create .sofia");
        let canonical_cwd = sofia_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".sofia"))
            .expect("canonical .sofia");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".sofia"))
            .expect("canonical .sofia");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
    ...
    fn misalignment_explanation_and_steer_are_never_serialized_into_error_events() {
        let event = ErrorEvent {
            ...
            sofia_error_info: Some(CodexErrorInfo::MisalignmentPolicyViolation),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "sofia_error_info": "misalignment_policy_violation"
            })
        );
