...
const PROTECTED_METADATA_CODEX_PATH_NAME: &str = ".sofia";
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Sofia degrades to warn-and-ignore
    /// instead of failing to load. Sofia 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "sofia-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".sofia");
    ...
    fn get_writable_roots_with_cwd_impl(
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.sofia -> <root>/decoy`, bwrap must still see
            // `<root>/.sofia`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.sofia` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .sofia read-only to the agent by
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "SOFIA_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn uri_deny_and_metadata_matcher_use_executor_paths() {
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry::new(
                path("file:///C:/workspace/.sofia").into(),
                FileSystemAccessMode::Read,
            ),
        ...
        assert!(!policy.can_write_path(&path("file:///C:/workspace/.sofia/config"), &context));
        assert_eq!(
            policy.metadata_write_denial(&path("file:///C:/workspace/.sofia/config"), &context),
            Some(".sofia"),
        );
    }
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".sofia");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".sofia".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    fn writable_roots_skip_default_dot_codex_when_explicit_user_rule_exists() {
        ...
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".sofia");
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".sofia".to_string()),
            "explicit .sofia rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_codex),
            "explicit .sofia rule should win over the default protected carveout"
        );
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_codex_config = cwd.path().join(".sofia").join("config.toml");
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_local_path_with_cwd(Path::new(".sofia/config.toml"), relative_cwd,)
        );
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let sofia_dir = real_root.join(".sofia");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&sofia_dir).expect("create .sofia");
        ...
        let expected_codex = link_root.join(".sofia");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".sofia");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-sofia");
        let dot_codex = root.join(".sofia");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_codex).expect("create .sofia symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let sofia_dir = real_tmpdir.join(".sofia");
