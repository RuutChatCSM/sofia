...
async fn authenticated_plugin_migration_uses_chatgpt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".sofia-plugin")).expect("create curated plugin directory");
    fs::write(
  ...
  "plugins": [{
    ...
    let mut service = service_for_paths(external_agent_home.clone(), sofia_home);
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, sofia_home)
...
async fn detect_repo_skips_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".sofia")).expect("create repo sofia dir");
    fs::create_dir_all(&sofia_home).expect("create sofia home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project sofia config");
...
async fn detect_repo_skips_plugins_even_with_installed_marketplace() {
    ...
    let sofia_home = root.path().join(".sofia");
    let repo_root = root.path().join("repo");
    let marketplace_root = sofia_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
