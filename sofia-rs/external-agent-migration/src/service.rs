...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "sofia.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) sofia_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    pub fn sofia_home(&self) -> &Path {
        &self.sofia_home
    }

    #[cfg(test)]
    fn new_for_test(sofia_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            sofia_home,
            ...
            auth_manager: sofia_login::test_support::auth_manager_from_optional_auth(
                /*auth*/ None,
            ),
            source,
            session_import_limits: ExternalAgentSessionImportLimits::default(),
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.sofia_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.sofia_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".sofia").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.sofia_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".sofia").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.sofia_home.join("AGENTS.md"))
            }
        };
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Sofia.matches_product_restriction(products))
            })
