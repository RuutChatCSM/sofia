...
use sofia_utils_path_uri::PathUriParseError;
...
/// Special argv[1] flag used when the Sofia executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `sofia-apply-patch` (to avoid forcing
/// `sofia-arg0` to depend on `sofia-core`), it remains part of the "sofia core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const SOFIA_CORE_APPLY_PATCH_ARG1: &str = "--sofia-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(SOFIA_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
