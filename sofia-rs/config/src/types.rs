//! Types used to define loaded and effective Sofia configuration values.
...
pub use sofia_protocol::config_types::ModeKind;
pub use sofia_protocol::config_types::Personality;
pub use sofia_protocol::config_types::ServiceTier;
...
pub enum ResumeCwdMode {
    /// Use the directory where Sofia was launched.
    Current,
...
/// Determine where Sofia should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in SOFIA_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in SOFIA_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Sofia should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Sofia product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Sofia product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Sofia rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Sofia does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct OtelConfigToml {
    ...
    #[serde(default)]
    pub tool_result: sofia_protocol::config_types::ToolResultLogConfig,
...
/// Fallback resize-reflow row cap when Sofia cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$SOFIA_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against SOFIA_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Sofia's terminal-specific default. Set to `0` to
...
/// (primarily the Sofia IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Sofia-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-sofia-max migration prompt
    #[serde(rename = "hide_gpt-5.1-sofia-max_migration_prompt")]
...
pub struct PluginMcpServerConfig {
    /// When `false`, Sofia skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Sofia successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Sofia last successfully activated for this marketplace.
    #[serde(default)]
