...
use sofia_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == sofia_features::Feature::Artifact {
            continue;
        }
        if feature.id == sofia_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::GuardianThreadContext {
            // This setting is already part of the guardianv2 feature table.
            continue;
        }
        if feature.id == sofia_features::Feature::GuardianV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::GuardianV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::ContextManagement {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::ContextManagementConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::SleepTool {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::SleepToolConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == sofia_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == sofia_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<sofia_features::FeatureToml<
                    sofia_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<sofia_features::ToolRegistryConfigToml>(),
    );
