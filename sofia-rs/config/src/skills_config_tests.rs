...
fn user_layer(sofia_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(sofia_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(sofia_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let sofia_home = TempDir::new().expect("temp dir");
    let skill_path = sofia_home.path().join("skills/demo/SKILL.md");
...
fn path_rule_disables_selected_path() {
    let sofia_home = TempDir::new().expect("temp dir");
    let path = sofia_home.path().join("disable-by-path/SKILL.md").abs();
...
fn later_name_rule_reenables_path_disabled_skill() {
    let sofia_home = TempDir::new().expect("temp dir");
    let path = sofia_home.path().join("reenable-by-name/SKILL.md").abs();
    let rules = SkillConfigRules {
        entries: vec![
            SkillConfigRule {
                selector: SkillConfigRuleSelector::Path(path.clone()),
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let sofia_home = TempDir::new().expect("temp dir");
    let root = sofia_home.path().join("reenable-by-path");
