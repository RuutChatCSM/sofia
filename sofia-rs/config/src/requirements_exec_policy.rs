...
use sofia_execpolicy::RuleRef;
use sofia_execpolicy::rule::PatternToken;
use sofia_execpolicy::rule::PrefixPattern;
use sofia_execpolicy::rule::PrefixRule;
use std::sync::Arc;
use thiserror::Error;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Sofia merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `sofia-execpolicy`.
