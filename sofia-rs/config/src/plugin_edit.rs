...
use sofia_utils_path::resolve_symlink_write_paths;
use sofia_utils_path::write_atomically;
use tokio::task;
...
pub async fn clear_user_plugin(sofia_home: &Path, plugin_key: String) -> std::io::Result<()> {
    apply_user_plugin_config_edits(sofia_home, vec![PluginConfigEdit::Clear { plugin_key }]).await
}
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&sofia_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(sofia_home.path());
...
"#,
        ...
        clear_user_plugin(sofia_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(sofia_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let sofia_home = TempDir::new().unwrap();
        let target_path = sofia_home.path().join("target_config.toml");
        symlink(&target_path, sofia_home.path().join(CONFIG_TOML_FILE)).unwrap();
...
enabled = true
    ...
    fn read_config(sofia_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(sofia_home.join(CONFIG_TOML_FILE)).unwrap()).unwrap()
    }
}
