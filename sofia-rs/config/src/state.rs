...
pub struct LoaderOverrides {
    /// Optional configuration file supplied with the installed Sofia package.
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("sofia-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, sofia_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.sofia/` folder associated with this config layer, if any.
    ...
    /// Returns the `.sofia/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$SOFIA_HOME/config.toml` layer because the active layer is the
