...
#[cfg(unix)]
const SOFIA_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/sofia/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/sofia/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(sofia_home)),
    )?;
...
pub(super) fn managed_config_default_path(sofia_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = sofia_home;
        PathBuf::from(SOFIA_MANAGED_CONFIG_SYSTEM_PATH)
    }
