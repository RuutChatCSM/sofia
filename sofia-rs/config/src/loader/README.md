...
This module is the canonical place to **load and describe Sofia configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `sofia_config::loader`:

- `load_config_layers_state(fs, sofia_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
