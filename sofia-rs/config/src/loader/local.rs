...
pub async fn load_local_config_layers(
    ...
    load_local_config_layers_with_overrides(fs, sofia_home, cwd, &LoaderOverrides::default()).await
}
...
pub(super) async fn load_local_config_layers_with_overrides(
    ...
    let requirements =
        local_requirements_layers(fs, sofia_home.as_path(), overrides, loaded_managed.clone())
            .await?;
    ...
    append_legacy_config_layers(&mut config_layers, loaded_managed, &sofia_home)?;
...
async fn local_requirements_layers(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        entries.extend(
            ...
            .await?
            .map(|layer| layer.with_base_dir(sofia_home)),
        );
    }
