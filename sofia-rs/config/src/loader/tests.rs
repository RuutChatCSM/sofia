...
use sofia_file_system::CopyOptions;
use sofia_file_system::CreateDirectoryOptions;
use sofia_file_system::ExecutorFileSystemFuture;
use sofia_file_system::FileMetadata;
use sofia_file_system::FileSystemReadStream;
use sofia_file_system::FileSystemSandboxContext;
use sofia_file_system::GetMetadataOptions;
use sofia_file_system::ReadDirectoryEntry;
use sofia_file_system::ReadFileOptions;
use sofia_file_system::RemoveOptions;
use sofia_file_system::WalkOptions;
use sofia_file_system::WalkOutcome;
use sofia_file_system::WriteFileOptions;
...
fn project_environment_filters_preserve_only_trusted_credential_host_bindings() {
    ...
    for project_policy in [
        ...
        let actual = sofia_protocol::shell_environment::populate_env(
            [
                ("GH_HOST", host),
...
async fn default_windows_managed_config_is_ignored_with_warning() {
    ...
    let managed_config_path = sofia_home.join("managed_config.toml");
    std::fs::write(
        &managed_config_path,
        r#"
model = "legacy-model"
...
"#,
    ...
    std::fs::write(sofia_home.join(CONFIG_TOML_FILE), r#"model = "user-model""#)
    ...
    let expected_warnings = vec![format!(
        "Ignoring deprecated managed config file at {}; SOFIA_HOME/managed_config.toml is no longer supported on Windows. Use %ProgramData%\\OpenAI\\Sofia\\requirements.toml for enforced settings or config.toml for defaults.",
...
fn windows_local_managed_configuration_ignores_legacy_file_but_detects_requirements() {
    ...
    let sofia_home = tmp.path().join("sofia-home");
    std::fs::create_dir_all(&sofia_home).expect("create sofia home");
    std::fs::write(sofia_home.join("managed_config.toml"), "")
        .expect("write default legacy managed config");
...
"#,
    ...
    assert!(
        message.contains("https://developers.openai.com/sofia/config-advanced#profiles"),
        "unexpected error message: {message}"
    );
}
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    let dot_codex = project.join(".sofia");
    let system_dir = tmp.path().join("system");
    let managed_dir = tmp.path().join("managed");
    for dir in [&sofia_home, &dot_codex, &system_dir, &managed_dir] {
    ...
    let user_file = sofia_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &user_file,
        format!(
