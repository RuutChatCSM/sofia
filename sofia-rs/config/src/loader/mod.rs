...
use sofia_file_system::ExecutorFileSystem;
use sofia_git_utils::resolve_root_git_project_for_trust;
use sofia_network_proxy::credential_broker_provider_context_env_keys;
use sofia_network_proxy::is_credential_broker_provider_env_key;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/sofia/config.toml";
...
///
/// - system    `/etc/sofia/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Sofia\requirements.toml` (Windows)
/// - cloud:    enterprise-managed cloud config bundle requirements
/// - legacy:   `/etc/sofia/managed_config.toml` (Unix) reinterpreted as
///   requirements.toml
/// - admin:    managed preferences (*)
///
/// For backwards compatibility, Unix continues to load
/// `/etc/sofia/managed_config.toml` and map it to `requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - package:  optional default configuration supplied with the Sofia package
/// - admin:    managed preferences (*)
/// - system    `/etc/sofia/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Sofia\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${SOFIA_HOME}/config.toml`
/// - profile   `${SOFIA_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.sofia/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.sofia/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/sofia/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    let packaged_defaults_layer = if let Some(file) = &overrides.packaged_defaults_path {
        ...
        let config = toml::from_str(raw_toml).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid embedded packaged defaults; this is a Sofia build error: {error}"),
            )
        })?;
        ConfigLayerEntry::new_with_raw_toml(
            ...
            config,
            raw_toml.to_owned(),
            AbsolutePathBuf::from_absolute_path(sofia_home)?,
        )
    };
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(sofia_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(sofia_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, sofia_home, overrides.clone(), strict_config)
            .await?;
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(sofia_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sofia_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/sofia/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(sofia_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/sofia/requirements.toml"))
}
...
pub fn has_local_managed_configuration(sofia_home: &Path) -> io::Result<bool> {
...
fn has_local_managed_configuration_with_system_requirements_path(
    ...
    #[cfg(not(windows))]
    if layer_io::managed_config_default_path(sofia_home).try_exists()? {
        return Ok(true);
    }
...
fn windows_codex_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Sofia")
}
...
fn requirements_layers_from_legacy_scheme(
    ...
    let sofia_home = AbsolutePathBuf::from_absolute_path(sofia_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Sofia to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".sofia"))
    }
}
...
async fn discover_project_layers(
    ...
    let sofia_home_abs = AbsolutePathBuf::from_absolute_path(sofia_home)?;
    let sofia_home_normalized =
        normalize_path(sofia_home_abs.as_path()).unwrap_or_else(|_| sofia_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".sofia");
        ...
        if dot_codex_abs == sofia_home_abs || dot_codex_normalized == sofia_home_normalized {
            continue;
        }
...
/// from a file like `/etc/sofia/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Sofia").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Sofia").join("config.toml"))
        );
    }
}
