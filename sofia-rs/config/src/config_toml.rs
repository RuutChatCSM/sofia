//! Schema-heavy configuration TOML types used by Sofia.
...
use sofia_model_provider_info::AMAZON_BEDROCK_RUNTIME_PROVIDER_ID;
...
/// Base config deserialized from ~/.sofia/config.toml.
...
pub struct ConfigToml {
    ...
    /// Allow macOS sandbox writable roots at or beneath SOFIA_HOME to traverse
    ...
    /// they change between commands or lie outside SOFIA_HOME.
    ...
    /// sanctioned by Sofia will likely degrade model performance.
    ...
    /// file: Use a file in the Sofia home directory.
    ...
    /// Directory where Sofia stores the SQLite state DB.
    /// Defaults to `$SOFIA_SQLITE_HOME` when set. Otherwise uses `$SOFIA_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Sofia writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$SOFIA_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Sofia Apps MCP requests.
    ...
    /// directories for `.sofia` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use sofia_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use sofia_protocol::protocol::RealtimeVoice;
