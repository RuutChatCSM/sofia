...
use sofia_git_utils::diff_since_latest_init;
use sofia_git_utils::reset_git_repository;
...
use sofia_state::Phase2JobClaimOutcome;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Sofia before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        sofia_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(sofia_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.sofia_home);
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<sofia_state::StateRuntime>> {
    let db = sofia_state::StateRuntime::init(
...
async fn trigger_memories_startup(test: &TestCodex) {
    let config_snapshot = test.sofia.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<sofia_core::config::Config>) {
    let config_snapshot = test.sofia.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> sofia_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(sofia_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &sofia_state::StateRuntime,
    sofia_home: &Path,
    ...
    let mut metadata_builder = sofia_state::ThreadMetadataBuilder::new(
        thread_id,
        sofia_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = sofia_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = sofia_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<sofia_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.sofia.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        sofia_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_codex(test: &TestCodex) -> anyhow::Result<()> {
    ...
    wait_for_event(&test.sofia, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
