...
use sofia_rollout::INTERACTIVE_SESSION_SOURCES;
use sofia_rollout::RolloutItem;
use sofia_rollout::should_persist_response_item_for_memories;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<sofia_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            sofia_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<sofia_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: sofia_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> sofia_protocol::error::Result<String> {
        let filtered = items
            .iter()
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: sofia_protocol::models::FunctionCallOutputPayload {
                        body: sofia_protocol::models::FunctionCallOutputBody::Text(
