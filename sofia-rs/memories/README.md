...
Runtime orchestration for Phase 1 and Phase 2 still lives in `sofia-core` under
`sofia-rs/core/src/memories/`.

## Crates

- `sofia-rs/memories/read` (`sofia-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `sofia-rs/memories/write` (`sofia-memories-write`) owns the write path:
...
- In `sofia`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.sofia/memories/.git` by `sofia-git-utils`
