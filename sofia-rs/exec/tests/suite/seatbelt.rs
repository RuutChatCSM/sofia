...
use sofia_core::spawn::StdioPolicy;
...
async fn sofia_home_symlink_opt_out_uses_loaded_user_config() -> anyhow::Result<()> {
    ...
    for (home_env, root_alias, enabled, ignore_user_config) in [
        ...
        ("home-link/../.sofia", Some(".sofia"), false, false),
        ("home-link/../.sofia", None, true, false),
        ("user-home", Some("user-home"), true, true),
    ] {
        let test = test_codex_exec();
        let home = test.home_path().join(".sofia");
        let project_home = test.cwd_path().join(".sofia");
