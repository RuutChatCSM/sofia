...
pub(super) async fn spawn_command_under_sandbox(
    ...
    use sofia_core::exec::build_exec_request;
...
async fn sandbox_blocks_first_time_dot_codex_creation() {
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .sofia && echo 'sandbox_mode = \"danger-full-access\"' > .sofia/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .sofia");

    let status = child.wait().await.expect("should wait for .sofia command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .sofia creation: {status:?}"
    );
