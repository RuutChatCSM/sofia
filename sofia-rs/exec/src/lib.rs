...
use sofia_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use sofia_app_server_client::EnvironmentManager;
use sofia_app_server_client::ExecServerRuntimePaths;
use sofia_app_server_client::InProcessAppServerClient;
use sofia_app_server_client::InProcessClientStartArgs;
use sofia_app_server_client::InProcessServerEvent;
...
use sofia_app_server_protocol::Thread as AppServerThread;
...
use sofia_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use sofia_config::format_config_error_with_source;
use sofia_core::StateDbHandle;
use sofia_core::check_execpolicy_for_warnings;
use sofia_core::config::Config;
use sofia_core::config::ConfigBuilder;
use sofia_core::config::ConfigOverrides;
use sofia_core::config::ConfigTomlLoadResult;
...
use sofia_core::config::resolve_oss_provider;
use sofia_core::config::resolve_profile_v2_config_path;
use sofia_core::find_thread_meta_by_name_str;
use sofia_core::format_exec_policy_error_with_source;
use sofia_core::path_utils;
use sofia_core::read_session_meta_line;
...
use sofia_login::default_client::set_default_client_residency_requirement;
use sofia_login::default_client::set_default_originator;
use sofia_login::enforce_login_restrictions;
...
use sofia_otel::set_parent_from_context;
use sofia_otel::traceparent_context_from_env;
...
use sofia_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use sofia_utils_cli::SharedCliOptions;
use sofia_utils_oss::ensure_oss_provider_ready;
use sofia_utils_oss::get_default_model_for_oss_provider;
use sofia_worktree::CreateWorktree;
use sofia_worktree::WorktreeManager;
use sofia_worktree::WorktreeSettings;
...
enum StdinPromptBehavior {
    ...
    /// `sofia exec` behavior for `sofia exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `sofia exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<sofia_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("sofia_exec".to_string()) {
        tracing::warn!(?err, "Failed to set sofia exec originator override {err:?}");
    }
    ...
    if worktree {
        ...
        match command.as_ref() {
            Some(ExecCommand::Resume(_)) => {
                anyhow::bail!("--worktree is not supported with `sofia exec resume`");
            }
            Some(ExecCommand::Review(_)) => {
                anyhow::bail!("--worktree is not supported with `sofia exec review`");
            }
    ...
    #[allow(clippy::print_stderr)]
    let sofia_home = match find_codex_home() {
        Ok(sofia_home) => sofia_home,
        Err(err) => {
            eprintln!("Error finding sofia home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&sofia_home, profile_v2));
    ...
    if worktree
        && EnvironmentManager::prepare_from_codex_home(&sofia_home)
            .await?
    ...
    let managed_worktree = if worktree {
        ...
        let gate_cloud_config = cloud_config_bundle_loader_for_storage(
            bootstrap_auth_config(&sofia_home, &gate_bootstrap)?,
        ...
        .await;
        let settings =
            WorktreeSettings::for_cli(&sofia_home, host_config.config_toml.desktop.as_ref())?;
    ...
    let overrides = ConfigOverrides {
        ...
        service_tier: None,
        sofia_self_exe: arg0_paths.sofia_self_exe.clone(),
        sofia_linux_sandbox_exe: arg0_paths.sofia_linux_sandbox_exe.clone(),
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        sofia_core::otel_init::build_provider(
            &config,
    ...
    sofia_core::otel_init::record_process_start(otel.as_ref(), "sofia_exec");
    sofia_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "sofia_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.sofia_self_exe.clone(),
        arg0_paths.sofia_linux_sandbox_exe.clone(),
    )?;
    #[cfg(target_os = "macos")]
    let local_runtime_paths = local_runtime_paths.with_allowed_symlinked_codex_home(
        sofia_config::allowed_symlinked_codex_home(&config.config_layer_stack, &config.sofia_home),
    );
    let state_db = sofia_core::init_state_db(&config).await;
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "sofia_exec".to_string(),
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, sofia_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = sofia_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Sofia
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            sofia_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Sofia initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        sofia_app_server_protocol::TurnStatus::Failed
                            | sofia_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<sofia_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<sofia_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<sofia_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != sofia_app_server_protocol::TurnItemsView::Full
}
...
async fn parse_latest_turn_context_cwd(path: &Path) -> Option<PathBuf> {
    ...
    tokio::task::spawn_blocking(move || {
        let reader = sofia_rollout::open_rollout_seekable_reader(&path).ok()?;
        let mut scanner = sofia_rollout::ReverseJsonlScanner::new(reader).ok()?;
        while let Some(outcome) = scanner.scan_next_rollout_line().ok()? {
            if let sofia_rollout::ScanOutcome::Parsed(RolloutLine {
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.sofia_home, session_id, Some(state_db.as_ref()))
                .await?
