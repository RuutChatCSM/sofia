...
#[command(
    version,
    override_usage = "sofia exec [OPTIONS] [PROMPT]\n       sofia exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Sofia outside a Git repository.
    ...
    /// Do not load `$SOFIA_HOME/config.toml`; auth still uses `SOFIA_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "sofia resume --last <prompt>" treats the positional as a prompt, not a session id.
