...
`sofia-exec-server` is the library backing `sofia exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`sofia-utils-pty`.

It provides:

- a CLI entrypoint: `sofia exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `sofia` binary owns hidden helper dispatch for sandboxed
filesystem operations and `sofia-linux-sandbox`.

## Transport

The server speaks the exec-specific `sofia-exec-server-protocol` message
...
It uses the standard Sofia ChatGPT sign-in state; run `sofia login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `SOFIA_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Sofia then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `SOFIA_API_KEY`;
Sofia sends it as a bearer token on the registration request. For example:

```sh
SOFIA_API_KEY="$OPENAI_API_KEY" \
sofia exec-server \
  --remote ... \
...
exec-server JSON-RPC over the authenticated WebSocket. The existing Sofia Noise
...
`sofia.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/sofia.exec_server.relay.v1.proto`. The relay frame carries stream
...
`sofia_build_info::build_id(commit, target)`, which CI can also call for an
...
- `arg0`: optional argv0 override forwarded to `sofia-utils-pty`.
...
hidden helper process launched from the top-level `sofia` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `sofia exec-server` command
builds these paths from the `sofia` arg0 dispatch state and resolves its HTTP
client factory from the effective Sofia configuration.
...
provider from Sofia auth state before starting remote mode.
