...
use sofia_exec_server::ExecServerTelemetry;
use sofia_exec_server::RequestDispatchMode;
...
use sofia_test_binary_support::TestBinaryDispatchGuard;
use sofia_test_binary_support::TestBinaryDispatchMode;
use sofia_test_binary_support::configure_test_binary_dispatch;
...
pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--sofia-test-delayed-output-after-exit-parent";
pub(crate) const SYSTEM_PROXY_REQUEST_URL_ENV: &str =
    "SOFIA_EXEC_SERVER_TEST_SYSTEM_PROXY_REQUEST_URL";
pub(crate) const SYSTEM_PROXY_URL_ENV: &str = "SOFIA_EXEC_SERVER_TEST_SYSTEM_PROXY_URL";

const SOFIA_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";
const DELAYED_OUTPUT_AFTER_EXIT_CHILD_ARG: &str = "--sofia-test-delayed-output-after-exit-child";

#[ctor]
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("sofia-exec-server-tests", |exe_name, argv1| {
        if argv1 == Some(SOFIA_ARG0_EXEC_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(SOFIA_FS_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(SOFIA_WINDOWS_SANDBOX_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let sofia_linux_sandbox_exe = if cfg!(target_os = "linux") {
        TEST_BINARY_DISPATCH_GUARD
            .as_ref()
            .and_then(|guard| guard.paths().sofia_linux_sandbox_exe.clone())
            .or_else(|| Some(current_exe.clone()))
    } else {
        None
    };
    Ok((current_exe, sofia_linux_sandbox_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    sofia_build_info::BuildInfo::initialize(TEST_BUILD_COMMIT);
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            sofia_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(async {
        #[cfg(target_os = "macos")]
        let runtime_paths = {
            let home = sofia_utils_home_dir::find_codex_home()?;
            let config = sofia_config::loader::load_config_layers_state(
                &sofia_exec_server::LocalFileSystem::unsandboxed(),
                home.as_path(),
                /*cwd*/ None,
                &[],
            ...
            runtime_paths.with_allowed_symlinked_codex_home(
                sofia_config::allowed_symlinked_codex_home(&config, &home),
            )
        };
