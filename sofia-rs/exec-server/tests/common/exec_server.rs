...
pub(crate) struct TestCodexHelperPaths {
    pub(crate) sofia_exe: PathBuf,
    pub(crate) sofia_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_codex_helper_paths() -> anyhow::Result<TestCodexHelperPaths> {
    let (helper_binary, sofia_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestCodexHelperPaths {
        sofia_exe: helper_binary,
    ...
    V: AsRef<std::ffi::OsStr>,
{
    ...
    let mut child = Command::new(&helper_paths.sofia_exe);
...
impl ExecServerHarness {
    pub(crate) async fn start(mut command: Command) -> anyhow::Result<Self> {
        ...
        if !command
            .as_std()
            .get_envs()
            .any(|(key, value)| key == "SOFIA_HOME" && value.is_some())
        {
            command.env("SOFIA_HOME", sofia_home.path());
        }
    ...
    pub(crate) fn sofia_home(&self) -> &std::path::Path {
