...
use sofia_exec_server::EnvironmentConfigLayer;
use sofia_exec_server::EnvironmentConfigLayerStack;
use sofia_exec_server::EnvironmentConfigReadParams;
use sofia_exec_server::EnvironmentConfigReadResponse;
...
async fn remote_environment_reads_projected_executor_config() -> anyhow::Result<()> {
    ...
    let sofia_home =
        AbsolutePathBuf::from_absolute_path(std::fs::canonicalize(server.sofia_home())?)?;
    let config_file = sofia_home.join(CONFIG_TOML_FILE);
    let project = sofia_home.join("project");
...
"#,
    ...
    assert_eq!(
        ...
        EnvironmentConfigReadResponse {
            ...
            hostname: sofia_config::host_name(),
...
async fn environment_config_read_rejects_empty_selectors() -> anyhow::Result<()> {
    ...
    for (config_paths, expected_message) in [
        ...
        let error = environment
            .read_environment_config(EnvironmentConfigReadParams {
                cwd: PathUri::from_abs_path(&sofia_home),
                config_paths,
                requirements_paths: Vec::new(),
            })
            .await
