...
use sofia_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use sofia_exec_server::CapabilityRootDiscovery;
use sofia_exec_server::CapabilityRootsDiscoverParams;
use sofia_exec_server::CapabilityRootsDiscoverResponse;
...
use sofia_exec_server_protocol::CapabilityRootDiscoverRequest;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
    ...
    write_file(
        &root.path().join(".sofia-plugin/plugin.json"),
        r#"{
  "name": "demo",
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".sofia-plugin/plugin.json")?
    );
