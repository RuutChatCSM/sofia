...
use sofia_exec_server::FsCloseParams;
use sofia_exec_server::FsOpenParams;
use sofia_exec_server::FsReadBlockParams;
use sofia_exec_server::FsReadBlockResponse;
use sofia_exec_server::ReadFileOptions;
use sofia_exec_server::RemoteExecServerConnectArgs;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\sofia-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\sofia-fs-stream-{}", Uuid::new_v4());
...
#[cfg(unix)]
fn read_only_sandbox(path: std::path::PathBuf) -> sofia_exec_server::FileSystemSandboxContext {
