...
pub(crate) fn environment_provider_from_codex_home(
    ...
    let path = sofia_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "sofia exec-server --listen stdio".to_string(),
                    ]),
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = sofia_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "sofia exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
SOFIA_LOG = "debug"
"#,
...
"#,
        ...
        for (index, (contents, expected)) in cases.into_iter().enumerate() {
            let path = sofia_home.path().join(format!("environments-{index}.toml"));
    ...
    async fn environment_provider_from_codex_home_uses_present_environments_file() {
        let sofia_home = tempdir().expect("tempdir");
        std::fs::write(
            sofia_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider =
            environment_provider_from_codex_home(sofia_home.path()).expect("environment provider");
