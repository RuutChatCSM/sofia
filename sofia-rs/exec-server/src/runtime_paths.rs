...
pub struct ExecServerRuntimePaths {
    /// Stable path to the Sofia executable used to launch hidden helper modes.
    pub sofia_self_exe: AbsolutePathBuf,
    /// Path to the Linux sandbox helper alias used when the platform sandbox
    /// needs to re-enter Sofia by argv0.
    pub sofia_linux_sandbox_exe: Option<AbsolutePathBuf>,
...
impl ExecServerRuntimePaths {
    pub fn from_optional_paths(
        sofia_self_exe: Option<PathBuf>,
        sofia_linux_sandbox_exe: Option<PathBuf>,
    ) -> std::io::Result<Self> {
        let sofia_self_exe = sofia_self_exe.ok_or_else(|| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "Sofia executable path is not configured",
            )
        })?;
        Self::new(sofia_self_exe, sofia_linux_sandbox_exe)
    }

    pub fn new(
        sofia_self_exe: PathBuf,
        sofia_linux_sandbox_exe: Option<PathBuf>,
    ) -> std::io::Result<Self> {
        Ok(Self {
            sofia_self_exe: absolute_path(sofia_self_exe)?,
            sofia_linux_sandbox_exe: sofia_linux_sandbox_exe.map(absolute_path).transpose()?,
            #[cfg(target_os = "macos")]
