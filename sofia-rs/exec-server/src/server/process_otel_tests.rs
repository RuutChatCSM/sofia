...
fn exported_process_and_network_logs_keep_the_validated_launch_reference() {
    ...
    for (export_traces, registered) in [(false, true), (true, true), (false, false)] {
        ...
        for record in &records {
            ...
            if export_traces
                && name.starts_with("sofia.exec_server.process_")
                && let Some(span) = span
            {
        ...
        {
            for name in [
                "sofia.exec_server.process_start",
                "sofia.exec_server.process_exit",
                "sofia.network_proxy.policy_decision",
            ] {
                expected.push((
                    name.to_string(),
...
fn launch_rpc_span_finishes_before_its_process_exits() {
    ...
    let exit_record = records
        .iter()
        .find(|record| attribute(record, "event.name") == Some("sofia.exec_server.process_exit"))
    ...
    assert_eq!(process_span["name"], "sofia.exec_server.process");
    assert_eq!(
        (
    ...
    let mut expected_exit =
        expected_process_attributes("sofia.exec_server.process_exit", /*index*/ 0, "None");
    ...
    for record in &records {
        ...
        let span = if attribute(record, "event.name") == Some("sofia.exec_server.process_start") {
            launch_span
        } else {
            process_span
        };
...
fn exported_spawn_failure_keeps_launch_identity_without_outcome_or_error_text() {
    ...
    assert_exported_attributes(
        ...
        vec![expected_process_attributes(
            "sofia.exec_server.process_spawn_failed",
            /*index*/ 0,
            "None",
        )],
    );
...
fn exported_sandbox_denial_keeps_launch_identity_and_separate_exit_outcome() {
    let records = exported_logs(/*export_traces*/ false, |runtime, _, _| {
        runtime.block_on(async {
            ...
            let sandbox = crate::FileSystemSandboxContext::from_legacy_sandbox_policy(
                sofia_protocol::protocol::SandboxPolicy::new_read_only_policy(), cwd.clone(),
    ...
    for index in 0..2 {
        ...
        let mut exited =
            expected_process_attributes("sofia.exec_server.process_exit", index, "MacosSeatbelt");
    ...
    let mut denied = expected_process_attributes(
        "sofia.exec_server.sandbox_denied",
        /*index*/ 1,
        "MacosSeatbelt",
    );
...
fn exported_logs(
    ...
    let otel = OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "sofia-exec-server".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        sofia_home: std::env::current_dir().expect("cwd"),
        exporter,
