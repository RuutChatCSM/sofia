...
use sofia_websocket_client::WebSocketConnection;
use sofia_websocket_client::WebSocketConnector;
use sofia_websocket_client::WebSocketTlsMode;
...
impl EnvironmentRegistryClient {
    ...
    #[tracing::instrument(
        name = "sofia.exec_server.remote.register",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "sofia.exec_server.remote.register",
    ...
    async fn register_environment_inner(
        ...
        .unwrap_or_else(|_| {
            Err(ExecServerError::EnvironmentRegistryRequest(
                sofia_http_client::RouteAwareRequestError::Timeout,
            ))
        })?;
    ...
    #[tracing::instrument(
        name = "sofia.exec_server.remote.environment_registry.connect",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "sofia.exec_server.remote.environment_registry.connect",
...
impl HarnessKeyValidator for RegistryHarnessKeyValidator {
    ...
    #[tracing::instrument(
        name = "sofia.exec_server.remote.environment_registry.validate_harness_key",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "sofia.exec_server.remote.environment_registry.validate_harness_key",
...
/// Noise connection configuration for a Sofia harness.
///
...
impl RemoteEnvironmentConfig {
    ...
    pub fn new_with_transport(
        ...
        Ok(Self {
            base_url,
            environment_id,
            name: "sofia-exec-server".to_string(),
...
#[tracing::instrument(
    name = "sofia.exec_server.remote.rendezvous.connect",
    skip_all,
    fields(
        otel.kind = "client",
        otel.name = "sofia.exec_server.remote.rendezvous.connect",
        result = tracing::field::Empty,
    )
)]
...
mod tests {
    ...
    impl AuthProvider for StaticRegistryAuthProvider {
        ...
        fn resolve_auth_headers(&self) -> sofia_api::AuthHeadersFuture<'_> {
            Box::pin(async {
