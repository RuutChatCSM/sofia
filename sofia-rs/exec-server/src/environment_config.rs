...
use sofia_config::McpServerDisabledReason;
use sofia_config::RequirementSource;
use sofia_config::RequirementsLayerEntry;
use sofia_config::compose_requirements_for_hostname;
use sofia_config::format_config_layer_source;
use sofia_config::host_name;
use sofia_config::loader::LocalTomlLayerStack;
use sofia_config::loader::load_local_config_layers;
use sofia_exec_server_protocol::EnvironmentConfigLayer;
use sofia_exec_server_protocol::EnvironmentConfigLayerStack;
use sofia_exec_server_protocol::EnvironmentConfigReadParams;
use sofia_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let sofia_home = find_codex_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find Sofia home: {error}"))
    })?;
    let layers = load_local_config_layers(file_system, sofia_home.as_path(), &cwd)
        .await
        .map_err(|error| {
    ...
    Ok(EnvironmentConfigReadResponse {
        ...
        sofia_home_dir: PathUri::from_abs_path(&sofia_home),
        hostname: host_name(),
...
impl Environment {
    ...
    pub async fn discover_http_mcp_servers(
        ...
        for layer in response.config.layers {
            ...
            sofia_config::merge_toml_values(&mut merged, &config);
        }
