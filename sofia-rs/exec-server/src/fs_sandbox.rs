...
use sofia_exec_server_protocol::JSONRPCErrorError;
...
use sofia_sandboxing::SandboxCommand;
use sofia_sandboxing::SandboxDirectSpawnTransformRequest;
use sofia_sandboxing::SandboxExecRequest;
use sofia_sandboxing::SandboxManager;
use sofia_sandboxing::SandboxTransformRequest;
use sofia_sandboxing::SandboxType;
use sofia_sandboxing::SandboxablePreference;
use sofia_utils_absolute_path::AbsolutePathBuf;
#[cfg(not(target_os = "linux"))]
use sofia_utils_absolute_path::canonicalize_preserving_symlinks;
...
use crate::fs_helper::SOFIA_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.sofia_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![SOFIA_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    sofia_linux_sandbox_exe: self.runtime_paths.sofia_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.sofia_self_exe.clone()];
    if let Some(path) = &runtime_paths.sofia_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !sofia_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            sofia_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let sofia_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(sofia_self_exe, /*sofia_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.sofia_self_exe.clone();
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/sofia".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let sofia_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(sofia_self_exe.clone(), Some(sofia_self_exe))
                .expect("runtime paths");
        ...
        let sandbox_context = {
            ...
            crate::FileSystemSandboxContext {
                windows_sandbox_level:
                    sofia_protocol::config_types::WindowsSandboxLevel::RestrictedToken,
                ..sandbox_context
            }
        };
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = sofia_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_the_helper_executable() {
        ...
        assert!(
            policy.can_read_local_path_with_cwd(
                runtime_paths.sofia_self_exe.as_path(),
                cwd.as_path(),
            )
        );
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let sofia_self_exe = root.path().join("bin").join("sofia");
        let sofia_linux_sandbox_exe = root.path().join("aliases").join("sofia-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(sofia_self_exe, Some(sofia_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let sofia_parent = runtime_paths.sofia_self_exe.parent().expect("sofia parent");
        ...
        assert!(!policy.can_read_local_path_with_cwd(sofia_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        sofia_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
