...
use sofia_network_proxy::NetworkPolicyAuditObserver;
use sofia_network_proxy::NetworkPolicyDecider;
use sofia_network_proxy::NetworkProxy;
use sofia_network_proxy::NetworkProxyHandle;
use sofia_network_proxy::NetworkProxyState;
use sofia_network_proxy::RemoteNetworkProxyLaunchConfig;
use sofia_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use sofia_network_proxy::strip_managed_proxy_env;
...
use sofia_sandboxing::WindowsSandboxProxySettingsMode;
use sofia_sandboxing::WindowsSandboxSpawnRequest;
...
#[cfg(unix)]
use crate::SOFIA_ARG0_EXEC_HELPER_ARG1;
...
pub(crate) async fn prepare_exec_request_with_telemetry(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.sofia_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.sofia_self_exe) {
            sandbox_helper_paths.push(runtime_paths.sofia_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.sofia_linux_sandbox_exe.iter().cloned());
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(SOFIA_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            sofia_linux_sandbox_exe: runtime_paths.sofia_linux_sandbox_exe.as_deref(),
...
> {
    ...
    state.set_process_log_metadata(sofia_network_proxy::NetworkProxyProcessLogMetadata {
        thread_id: telemetry.thread_id.clone(),
        tool_call_id: telemetry.tool_call_id.clone(),
        executor_identity: telemetry
            .executor_registration
            .as_ref()
            .map(|registration| sofia_network_proxy::ExecutorLogIdentity {
