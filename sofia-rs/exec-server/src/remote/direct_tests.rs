...
impl AuthProvider for StaticAuthProvider {
    ...
    fn apply_auth(
        &self,
        mut request: sofia_http_client::Request,
    ) -> sofia_api::AuthProviderFuture<'_> {
        Box::pin(async move {
    ...
    fn apply_auth(
        ...
        Box::pin(async move {
            let mut url = url::Url::parse(&request.url)
                .map_err(|error| sofia_api::AuthError::Build(error.to_string()))?;
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                tracing_subscriber::filter::Targets::new()
                    .with_target("sofia_http_client", tracing::Level::TRACE)
                    .with_target("sofia_exec_server", tracing::Level::TRACE),
            ),
    );
    let _guard = tracing::subscriber::set_default(subscriber);
    tracing::debug!(target: "sofia_exec_server", "direct registry log capture sentinel");
