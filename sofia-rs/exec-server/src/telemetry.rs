...
impl ExecServerTelemetry {
    ...
    pub(crate) fn shell_snapshot_captured(
        ...
        let _ = metrics.record_duration("sofia.shell_snapshot.duration_ms", duration, &tags);
        let _ = metrics.counter("sofia.shell_snapshot", /*inc*/ 1, &tags);
    }
    ...
    pub(crate) fn process_started(&self, process_id: &str) -> ProcessMetricGuard {
        ...
        let parent = sofia_otel::current_span_w3c_trace_context();
        ...
        let span = tracing::info_span!(
            parent: None,
            "sofia.exec_server.process",
            otel.kind = "internal",
            process.id = process_id,
            result = tracing::field::Empty,
        );
        if let Some(parent) = parent {
            sofia_otel::set_parent_from_w3c_trace_context(&span, &parent);
        }
        ProcessMetricGuard {
            telemetry: self.clone(),
            span,
