...
impl OtelProvider {
    ...
    fn prepare_shutdown_worker(&mut self) -> io::Result<()> {
        self.prepare_shutdown_worker_with_spawner(|startup| {
            std::thread::Builder::new()
                .name("sofia-otel-shutdown".to_string())
                .spawn(move || {
    ...
    pub fn sofia_export_filter(meta: &tracing::Metadata<'_>) -> bool {
...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("sofia_otel.log_only"));
        assert!(is_log_export_target("sofia_otel.network_proxy"));
        assert!(!is_log_export_target("sofia_otel.trace_safe"));
        assert!(!is_log_export_target("sofia_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("sofia_otel.trace_safe"));
        assert!(is_trace_safe_target("sofia_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("sofia_otel.log_only"));
        assert!(!is_trace_safe_target("sofia_otel.network_proxy"));
    }
    ...
    fn cached_global_metrics_follow_reinstalled_provider() -> Result<(), Box<dyn Error>> {
        ...
        cached.counter("sofia.after_transition", /*inc*/ 1, &[])?;
        initial.shutdown()?;
        ...
        names.sort_unstable();
        names.dedup();
        assert_eq!(names, vec!["sofia.after_transition"]);
    ...
    fn statsig_disabled_metrics_are_not_exported() -> Result<(), Box<dyn Error>> {
        ...
        names.sort_unstable();
        names.dedup();
        assert_eq!(names, vec!["sofia.turns"]);
    ...
    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            ...
            service_name: "sofia-test".to_string(),
