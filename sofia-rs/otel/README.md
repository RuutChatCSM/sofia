...
`sofia-otel` is the OpenTelemetry integration crate for Sofia. It provides:

- Provider wiring for log/trace/metric exporters (`sofia_otel::OtelProvider`
  and `sofia_otel::provider`).
- Session-scoped business event emission via `sofia_otel::SessionTelemetry`.
- Low-level metrics APIs via `sofia_otel::metrics`.
- Trace-context helpers via `sofia_otel::trace_context` and crate-root re-exports.
...
```rust
use sofia_otel::config::OtelExporter;
use sofia_otel::config::OtelHttpProtocol;
use sofia_otel::config::OtelSettings;
...
let settings = OtelSettings {
    ...
    sofia_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Sofia
...
Sofia-specific session events. Rich session/business events should go through
...
`sofia-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Sofia-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use sofia_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("sofia.session_started", 1, &[("source", "tui")])?;
metrics.histogram("sofia.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("sofia.turns", 1, &[("model", "gpt-5.1")])?;
