...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "sofia-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "sofia.turns",
        "Total number of Sofia turns.",
        /*inc*/ 1,
    ...
    let counter = find_metric(&resource_metrics, "sofia.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Sofia turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "sofia-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "sofia.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "sofia.tool_latency")
            ...
            .expect("sofia.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "sofia.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Sofia operations.");
...
fn histogram_uses_explicit_bucket_boundaries() -> Result<()> {
    ...
    assert_eq!(
        histogram_data(&latest_metrics(&exporter), "sofia.payload_bytes"),
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "sofia.alpha").expect("sofia.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "sofia.beta").expect("sofia.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("sofia.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("sofia.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
