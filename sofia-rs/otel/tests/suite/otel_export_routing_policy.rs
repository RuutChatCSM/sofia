...
use sofia_otel::AuthEnvTelemetryMetadata;
...
use sofia_protocol::config_types::ToolResultLogConfig;
...
fn auth_env_metadata() -> AuthEnvTelemetryMetadata {
    AuthEnvTelemetryMetadata {
        ...
        sofia_api_key_env_present: false,
        sofia_api_key_env_enabled: true,
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    assert!(
        logs.iter()
            .all(|log| { log.record.target().map(Cow::as_ref) == Some("sofia_otel.log_only") })
    );

    let prompt_log = find_log_by_event_name(&logs, "sofia.user_prompt");
    ...
    let prompt_trace_event = find_span_event_by_name_attr(span_events, "sofia.user_prompt");
...
fn otel_export_routing_policy_routes_tool_result_log_and_trace_events() {
    ...
    let tool_log = find_log_by_event_name(&logs, "sofia.tool_result");
    ...
    let tool_logs: Vec<_> = logs
        ...
        .filter(|attrs| attrs.get("event.name").map(String::as_str) == Some("sofia.tool_result"))
        .collect();
    assert_eq!(
        tool_logs
            .iter()
    ...
    let tool_trace_event = find_span_event_by_name_attr(span_events, "sofia.tool_result");
...
fn otel_export_routing_policy_routes_auth_recovery_log_and_trace_events() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let recovery_log = find_log_by_event_name(&logs, "sofia.auth_recovery");
    ...
    let recovery_trace_event = find_span_event_by_name_attr(span_events, "sofia.auth_recovery");
...
fn otel_export_routing_policy_routes_api_request_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let conversation_log = find_log_by_event_name(&logs, "sofia.conversation_starts");
    ...
    let request_log = find_log_by_event_name(&logs, "sofia.api_request");
    ...
    let conversation_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "sofia.conversation_starts");
    ...
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "sofia.api_request");
...
fn otel_export_routing_policy_routes_websocket_connect_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let connect_log = find_log_by_event_name(&logs, "sofia.websocket_connect");
    ...
    let connect_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "sofia.websocket_connect");
...
fn otel_export_routing_policy_routes_websocket_request_transport_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let request_log = find_log_by_event_name(&logs, "sofia.websocket_request");
    let request_log_attrs = log_attributes(&request_log.record);
    assert_eq!(
        request_log_attrs
            .get("auth.connection_reused")
    ...
    let spans = span_exporter.get_finished_spans().expect("span export");
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "sofia.websocket_request");
    let request_trace_attrs = span_event_attributes(request_trace_event);
    assert_eq!(
        request_trace_attrs
            .get("auth.connection_reused")
