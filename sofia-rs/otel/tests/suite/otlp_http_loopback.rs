use sofia_otel::EXEC_SERVER_CLIENT_REQUEST_COUNT_METRIC;
use sofia_otel::MetricsClient;
use sofia_otel::MetricsConfig;
use sofia_otel::OtelExporter;
use sofia_otel::OtelHttpProtocol;
use sofia_otel::OtelProvider;
use sofia_otel::OtelSettings;
use sofia_otel::Result;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("sofia.turns", /*inc*/ 1, &[("source", "test")])?;
    metrics.counter("sofia.api_request", /*inc*/ 1, &[("status", "200")])?;
    ...
    metrics.record_duration(
        "sofia.api_request.duration_ms",
        Duration::from_millis(100),
        &[("status", "200")],
    )?;
    metrics.counter("sofia.conversation.turn.count", /*inc*/ 1, &[])?;
    metrics.record_duration(
    ...
    metrics.counter("sofia.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "sofia.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.histogram(
        "sofia.turn.token_usage",
        /*value*/ 100,
        &[("token_type", "total")],
    )?;
    metrics.gauge_with_description(
        "sofia.active",
        "Number of active Sofia operations.",
        /*value*/ 1,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    assert!(
        body.contains("\"sofia.api_request.duration_ms\""),
        "expected API-request duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.conversation.turn.count\""),
        "expected conversation turn count not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.responses_api_engine_iapi_ttft.duration_ms\""),
        "expected engine IAPI TTFT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.responses_api_engine_service_tbt.duration_ms\""),
        "expected engine service TBT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.responses_api_engine_service_ttft.duration_ms\""),
        "expected engine service TTFT duration not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.tool.call\""),
        "expected tool-call counter not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.turn.token_usage\""),
        "expected turn-token histogram not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("\"sofia.tool.call.duration_ms\""),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let otel = OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "sofia-cli".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        sofia_home: PathBuf::from("."),
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "sofia.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("sofia.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "sofia.test.trace_event",
    ...
    assert!(
        body.contains("sofia.test.trace_event"),
