...
async fn private_directory_rejects_junctions() {
    ...
    let output = std::process::Command::new("powershell.exe")
        .args(["-NoProfile", "-NonInteractive", "-Command", "$ErrorActionPreference = 'Stop'; New-Item -ItemType Junction -Path $env:SOFIA_TEST_LINK -Target $env:SOFIA_TEST_TARGET | Out-Null"])
        .env("SOFIA_TEST_LINK", &junction)
        .env("SOFIA_TEST_TARGET", &target)
...
async fn socket_validation_rejects_broad_acl_without_repairing_it() {
    ...
    let inspect = |script: &str| {
        let output = std::process::Command::new("powershell.exe")
            ...
            .env("SOFIA_TEST_DIRECTORY", &directory)
            .output()
            .unwrap();
        assert!(
            output.status.success(),
...
$acl = Get-Acl -LiteralPath $env:SOFIA_TEST_DIRECTORY
...
Set-Acl -LiteralPath $env:SOFIA_TEST_DIRECTORY -AclObject $acl
(Get-Acl -LiteralPath $env:SOFIA_TEST_DIRECTORY).Sddl
"#,
    );
    assert_eq!(
    ...
    assert_eq!(
        before,
        inspect(
            "$ErrorActionPreference = 'Stop'; (Get-Acl -LiteralPath $env:SOFIA_TEST_DIRECTORY).Sddl"
        )
    );
}
...
$directory = Get-Acl -LiteralPath $env:SOFIA_TEST_PRIVATE_DIRECTORY
if (-not $directory.AreAccessRulesProtected) { throw 'directory inherits broad access' }
foreach ($acl in @($directory, (Get-Acl -LiteralPath (Join-Path $env:SOFIA_TEST_PRIVATE_DIRECTORY 'child')), (Get-Acl -LiteralPath (Join-Path $env:SOFIA_TEST_PRIVATE_DIRECTORY 'socket')))) {
    $rules = @($acl.Access)
...
"#])
        .env("SOFIA_TEST_PRIVATE_DIRECTORY", &directory)
