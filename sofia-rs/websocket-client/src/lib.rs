//! Proxy-aware WebSocket connection setup shared by Sofia API clients.
...
use sofia_http_client::BuildCustomCaTransportError;
use sofia_http_client::HttpClientFactory;
use sofia_http_client::OutboundProxyRoute;
use sofia_http_client::build_rustls_client_config_with_custom_ca;
...
/// Selects whether WebSocket TLS follows Sofia custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Sofia custom CAs.
    ExplicitCodexTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Sofia custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ...
    /// Creates a connector with explicit Sofia TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Sofia TLS configuration when they establish their
