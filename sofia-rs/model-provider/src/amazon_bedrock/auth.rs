...
use sofia_aws_auth::AwsAccessKeys;
    ...
    match source {
        ...
        BedrockAuthSource::ManagedBearerToken => {
            let Some(CodexAuth::BedrockApiKey(auth)) = managed_auth else {
                return Err(CodexErr::Fatal(
                    "selected Sofia-managed Amazon Bedrock API key is no longer available"
                        .to_string(),
                ));
            };
        ...
        BedrockAuthSource::ManagedAccessKeys => {
            let Some(CodexAuth::BedrockAccessKeys(auth)) = managed_auth else {
                return Err(CodexErr::Fatal(
                    "selected Sofia-managed Amazon Bedrock access keys are no longer available"
                        .to_string(),
                ));
            };
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Sofia agent requests fail even though raw Responses requests work.
