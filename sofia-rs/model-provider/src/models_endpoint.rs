...
use sofia_login::collect_auth_env_telemetry;
use sofia_login::default_client::create_client_for_route_async;
use sofia_model_provider_info::ModelProviderInfo;
use sofia_models_manager::manager::ModelsEndpointClient;
use sofia_models_manager::manager::ModelsEndpointFuture;
use sofia_otel::TelemetryAuthMode;
use sofia_protocol::error::CodexErr;
use sofia_protocol::error::Result as CoreResult;
...
impl OpenAiModelsEndpoint {
    ...
    async fn list_models(
        ...
        let _timer =
            sofia_otel::start_global_timer("sofia.remote_models.fetch_update.duration_ms", &[]);
    ...
    fn auth_env(&self) -> AuthEnvTelemetry {
        let sofia_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.sofia_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, sofia_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn provider_key(&self) -> Option<String> {
        self.provider_info
            ...
            .or_else(|| Some(self.provider_info.name.clone()))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "sofia_otel.log_only",
            tracing::Level::INFO,
            event.name = "sofia.api_request",
            ...
            auth.env_codex_api_key_present = self.auth_env.sofia_api_key_env_present,
            auth.env_codex_api_key_enabled = self.auth_env.sofia_api_key_env_enabled,
