...
use sofia_login::default_client::ResidencyRequirement;
use sofia_login::default_client::read_default_client_residency_requirement;
use sofia_model_provider_info::ModelProviderInfo;
use sofia_models_manager::cache::ModelsCache;
use sofia_models_manager::manager::OpenAiModelsManager;
use sofia_models_manager::manager::SharedModelsManager;
use sofia_models_manager::manager::StaticModelsManager;
use sofia_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Sofia may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "sofia-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Sofia should have a unified provider-specific auth
    ...
    fn map_api_error(&self, error: ApiError) -> CodexErr {
        sofia_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, sofia_protocol::error::Result<Provider>> {
        Box::pin(async move {
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, sofia_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, sofia_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Sofia session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, sofia_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| sofia_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
...
mod tests {
    ...
    use sofia_model_provider_info::AwsAuthRefreshConfig;
    use sofia_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    use sofia_utils_redacted_string::RedactedString;
    use wiremock::Mock;
    ...
    fn test_codex_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("sofia-model-provider-test-{}", std::process::id()))
    }
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("sofia-bedrock".to_string()),
                region: None,
                auth_refresh: None,
            })),
            Some(AuthManager::from_auth_for_testing(CodexAuth::from_api_key(
    ...
    async fn shared_bedrock_auth_refresh_is_reused_only_for_matching_configuration() {
        ...
        const HELPER_ARG: &str = "SOFIA_BEDROCK_SHARED_AUTH_REFRESH_COMMAND";
        const SUBPROCESS_ARG: &str = "SOFIA_BEDROCK_SHARED_AUTH_REFRESH_SUBPROCESS";
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = sofia_models_manager::bundled_models_response()
