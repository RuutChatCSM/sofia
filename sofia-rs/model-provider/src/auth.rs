...
use sofia_agent_identity::authorization_header_for_agent_task;
use sofia_api::AgentIdentityTelemetry;
use sofia_api::AuthError;
use sofia_api::AuthHeadersFuture;
...
use sofia_login::auth::AgentIdentityAuthError;
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> sofia_protocol::error::Result<SharedAuthProvider> {
...
) -> sofia_protocol::error::Result<ResolvedProviderAuth> {
...
fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> sofia_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Sofia auth snapshot.
...
mod tests {
    ...
    use sofia_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_codex_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "sofia-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp sofia home");
        path
    }

    fn write_chatgpt_auth_json(sofia_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
        ...
        write_chatgpt_auth_json(&sofia_home);
