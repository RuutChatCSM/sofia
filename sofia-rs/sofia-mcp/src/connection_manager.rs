//! Aggregates MCP server connections for Sofia.
//!
...
use sofia_protocol::protocol::McpStartupCompleteEvent;
use sofia_protocol::protocol::McpStartupFailure;
...
use sofia_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            ...
            .map(sofia_model_provider::auth_provider_from_auth);
        let sofia_apps_auth_provider = auth_manager.as_ref().and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_codex_backend()).map(|auth| {
                sofia_model_provider::auth_provider_from_auth_manager(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in Sofia Apps, `SOFIA_CONNECTORS_TOKEN` is a debug
            ...
            let sofia_apps_tools_cache_context = shares_codex_apps_tools_cache.then(|| {
                sofia_apps_tools_cache
                    .context(sofia_home.clone(), sofia_apps_tools_cache_key.clone())
            });
            // The reserved Sofia Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == SOFIA_APPS_MCP_SERVER_NAME {
                sofia_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Sofia Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient CodexAuth provider.
            let runtime_auth_provider =
                if server_name == SOFIA_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                shares_codex_apps_tools_cache
                    .then(|| (sofia_home.clone(), sofia_apps_tools_cache_key.clone())),
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("SOFIA_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == SOFIA_APPS_MCP_SERVER_NAME {
                None
    ...
    /// Sofia Apps metadata may come from its existing cache; regular MCP server information is
