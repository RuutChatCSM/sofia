...
use sofia_exec_server::RouteAwareHttpClient;
...
use sofia_rmcp_client::with_http_headers_helper;
...
pub struct McpRuntimeInput {
    ...
    pub sofia_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub sofia_apps_tools_cache_key: ConnectorRuntimeContextKey,
...
/// Owns all mutable MCP state for one Sofia thread.
///
...
impl McpRuntime {
    ...
    pub async fn read_resource_for_call(
        ...
        let origin = self
            ...
            .find(call_id)?;
        let binding = self
            .current_binding_for_call(crate::SOFIA_APPS_MCP_SERVER_NAME)
            .await
            .ok_or_else(|| anyhow::anyhow!("sofia_apps MCP server is unavailable"))?;
    ...
    async fn publish(&self, input: McpRuntimeInput, previous: Option<&McpConnectionSet>) {
        ...
        let hosted_event_server_retained = connections.contains_server(SOFIA_APPS_MCP_SERVER_NAME)
            && config
                .mcp_server_catalog
                ...
                .is_some_and(|registration| {
                    registration
                        .source()
                        .is_host_owned_apps(SOFIA_APPS_MCP_SERVER_NAME, registration.config())
                });
...
pub fn apply_http_headers_helper(
    ...
    config: &sofia_config::McpServerConfig,
    local_process_cwd: PathBuf,
    ...
    let sofia_config::McpServerTransportConfig::StreamableHttp {
        url,
    ...
    pub(crate) fn resolve_server_environment(
        ...
        if config.is_local_environment() {
            return match config.transport {
                sofia_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                sofia_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
...
mod tests {
    ...
    async fn cached_bindings_follow_the_clients_catalog_revision() -> anyhow::Result<()> {
        ...
        let call = refreshed
            .prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "search")
