...
use crate::rmcp_client::SOFIA_APPS_RECONNECT_INITIAL_BACKOFF;
...
use sofia_config::AppToolApproval;
use sofia_config::Constrained;
use sofia_config::McpServerAuth;
use sofia_config::McpServerConfig;
use sofia_config::McpServerEnvVar;
use sofia_config::McpServerToolConfig;
use sofia_config::types::AuthKeyringBackendKind;
use sofia_config::types::OAuthCredentialsStoreMode;
use sofia_connectors::ConnectorRuntimeContext;
use sofia_connectors::ConnectorRuntimeContextKey;
use sofia_connectors::ConnectorRuntimeFetchSource;
use sofia_connectors::ConnectorRuntimeManager;
use sofia_exec_server::ExecServerError;
use sofia_exec_server::HttpClient;
use sofia_exec_server::HttpRequestParams;
use sofia_exec_server::HttpRequestResponse;
use sofia_exec_server::HttpResponseBodyStream;
use sofia_exec_server_test_support::environment_manager_without_environments;
use sofia_login::AuthHeaders;
use sofia_login::AuthManager;
use sofia_login::CodexAuth;
use sofia_protocol::ToolName;
use sofia_protocol::approvals::ElicitationRequest;
use sofia_protocol::mcp::ClientMcpExtensions;
use sofia_protocol::mcp::McpServerInfo;
...
use sofia_protocol::protocol::McpStartupFailureReason;
use sofia_rmcp_client::ElicitationResponse;
use sofia_rmcp_client::InProcessTransportFactory;
use sofia_rmcp_client::McpAuthState;
use sofia_rmcp_client::McpLoginRequirement;
use sofia_rmcp_client::McpOAuthRefreshMode;
use sofia_rmcp_client::RmcpClient;
...
fn store_current_tools(cache_context: &ConnectorRuntimeContext<ToolInfo>, tools: Vec<ToolInfo>) {
    let _ = cache_context.publish_if_newest_accepted(
        ...
        &create_test_server_info("Sofia Apps"),
        tools,
    );
}
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("sofia-test", "0.0.0-test"),
            )
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        sofia_apps_tools_cache_context: None,
    }
}
...
async fn connection_statuses_observe_clients_without_starting_them() {
    use sofia_protocol::mcp::McpServerConnectionStatus as Status;
...
async fn connection_statuses_follow_latest_reconnect_outcome() {
    ...
    let client = manager.test_client(SOFIA_APPS_MCP_SERVER_NAME);
    assert!(client.client().await.is_err());
    let expected = |status| HashMap::from([(SOFIA_APPS_MCP_SERVER_NAME.to_string(), status)]);
    assert_eq!(
    ...
    for status in [
        ...
        tokio::time::advance(SOFIA_APPS_RECONNECT_INITIAL_BACKOFF * 2).await;
    }
...
pub(crate) async fn create_test_manager_with_ready_apps_client(
    ...
    let tool = create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, tool_name);
    let client = Arc::new(
        RmcpClient::new_in_process_client(Arc::new(RefreshTestTransportFactory {
    ...
    let managed_client = ManagedClient {
        _auth_change_notifications: None,
        client,
        server_info: create_test_server_info("Sofia Apps"),
        tool_catalog: Arc::new(ClientToolCatalog::new(vec![tool])),
        tool_timeout: Some(Duration::from_secs(5)),
        server_instructions: None,
        server_supports_sandbox_state_meta_capability: false,
        sofia_apps_tools_cache_context: Some(cache_context.clone()),
    };
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
    ...
    manager.insert_test_client(
        SOFIA_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: futures::future::ready::<Result<ManagedClient, StartupOutcomeError>>(Ok(
                managed_client,
            ))
            ...
            cached_server_info: Some(create_test_server_info("Sofia Apps")),
            sofia_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
            cancel_token: CancellationToken::new(),
...
fn create_test_manager_with_failed_apps_startup(
    ...
    .boxed()
    .shared();
    let sofia_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        sofia_home.path().to_path_buf(),
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        sofia_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn assert_elicitation_declined_with_reviewer_calls(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            sofia_rmcp_client::Elicitation::Mcp(elicitation),
...
async fn assert_requested_user_input_is_declined(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            sofia_rmcp_client::Elicitation::Mcp(
    ...
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "sofia_approval_kind": "tool_suggestion",
            })),
...
async fn assert_disabled_permissions_surface_requested_user_input(
    ...
    let sofia_protocol::mcp::RequestId::String(request_id) = request.id else {
        panic!("expected Sofia-owned string request ID");
    };
...
async fn disabled_permissions_decline_requested_user_input_with_approval_metadata() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "sofia_approval_kind": "mcp_tool_call",
...
async fn on_request_approval_forms_remain_with_the_reviewer() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "sofia_request_type": "approval_request",
                "sofia_approval_kind": "mcp_tool_call",
                "tool_name": "test_tool",
            })),
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        sofia_protocol::mcp::RequestId::String(request_a_id),
        sofia_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected Sofia-owned string request IDs");
    };
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        sofia_protocol::mcp::RequestId::String(cancelled_id),
        sofia_protocol::mcp::RequestId::String(pending_id),
...
fn test_normalize_tools_selects_raw_server_name() {
    let mut tool = create_test_tool("sofia_apps", "search");
    tool.callable_namespace = "sofia_apps__calendar".to_string();
    ...
    assert_eq!(
        model_tool_names(&model_tools),
        HashSet::from([ToolName::namespaced("sofia_apps__calendar", "search")])
    );
}
...
#[test]
fn sofia_apps_env_bearer_token_bypasses_shared_tools_cache() {
...
#[tokio::test]
async fn sofia_apps_extension_does_not_share_host_owned_tools_cache() -> anyhow::Result<()> {
    let sofia_home = tempdir()?;
    let cache_key = ConnectorRuntimeContextKey::personal(
        /*account_id*/ None, /*chatgpt_user_id*/ None,
    );
    let sofia_apps_tools_cache = ConnectorRuntimeManager::<ToolInfo>::default();
    let cache_context =
        sofia_apps_tools_cache.context(sofia_home.path().to_path_buf(), cache_key.clone());
    store_current_tools(
        &cache_context,
    ...
    let mut config = crate::mcp::tests::test_mcp_config(sofia_home.path().to_path_buf());
    let mut catalog = crate::ResolvedMcpCatalog::builder();
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            sofia_apps_tools_cache,
            tool_catalog_cache: McpToolCatalogCache::default(),
            sofia_apps_tools_cache_key: cache_key,
    ...
    let client = manager.test_client(SOFIA_APPS_MCP_SERVER_NAME);
    assert!(
        client.sofia_apps_tools_cache_context.is_none(),
...
async fn list_all_tools_uses_shared_codex_apps_cache_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__sofia_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, SOFIA_APPS_MCP_SERVER_NAME);
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    let mut ready_client = create_test_managed_client(vec![
        create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "client_local_tool"),
        create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "client_local_blocked"),
    ])
    .await;
    ...
    ready_client.sofia_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
        &approval_policy,
    ...
    manager
        .servers
        .get_mut(SOFIA_APPS_MCP_SERVER_NAME)
        .expect("test server exists")
    ...
    assert!(
        step.prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        step.prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    assert!(
        step.prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "client_local_blocked")
            .is_none()
    );
}
...
async fn hard_refresh_keeps_client_catalog_local_when_shared_cache_loses_race() -> anyhow::Result<()>
{
    ...
    let cache_context_a = shared_cache.context(sofia_home.path().to_path_buf(), cache_key.clone());
    let cache_context_b = shared_cache.context(sofia_home.path().to_path_buf(), cache_key);
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<sofia_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    let server_info = create_test_server_info("Sofia Apps");
    manager.insert_test_client(
        SOFIA_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: pending_client,
    ...
    assert_eq!(
        server_infos.get(SOFIA_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn capture_binding_exposes_cached_tools_before_startup() {
    ...
    let mut cached_tool = create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "shared_cached_tool");
    ...
    assert!(
        cached_binding
            .prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    ...
    let startup = tokio::spawn(async move {
        manager_for_startup
            .wait_for_server_startup(SOFIA_APPS_MCP_SERVER_NAME)
            .await
    });
...
async fn capture_binding_resolves_concurrently_and_rechecks_cached_clients() {
    ...
    apps_client.sofia_apps_tools_cache_context = Some(cache_context);
    let first_client =
    ...
    manager.insert_test_client(SOFIA_APPS_MCP_SERVER_NAME, apps_client);
    ...
    tokio::time::timeout(Duration::from_secs(1), apps_started)
        .await
        .expect("Sofia Apps startup should begin")
        .expect("signal Sofia Apps startup");
    ...
    release_apps.send(()).expect("release Sofia Apps startup");
    assert!(startup.await.expect("Sofia Apps startup task"));
    ...
    assert!(
        binding
            .prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        binding
...
async fn list_all_tools_reconnects_failed_codex_apps_startup_and_reuses_client() {
    ...
    let step = capture_binding(&manager).await;
    let prepared = step
        .prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "drive_search")
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(SOFIA_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::sofia_apps_mcp_server_config(
        "https://docs.example",
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    ...
    sofia_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let provider = sofia_model_provider::auth_provider_from_auth(&hosted_auth);
    let mut local_config = crate::sofia_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    ...
    let local_provider =
        chatgpt_auth_provider_for_server(&local_server, Some(Arc::clone(&provider)))
            .expect("host-owned Sofia Apps must retain hosted authentication");
    assert_eq!(
        local_provider
            .to_auth_headers()
...
async fn executor_owned_chatgpt_mcp_accepts_only_safe_explicit_authorization() -> anyhow::Result<()>
{
    ...
    let hosted_auth = CodexAuth::Headers(AuthHeaders::new(actor_headers));
    let runtime_config = crate::mcp::tests::test_mcp_config(sofia_home.path().to_path_buf());
    let cases = [
        ...
        (
            ...
            None,
            Some(("aUtHoRiZaTiOn", "SOFIA_TEST_HOSTED_SECRET")),
            false,
        ),
        (
            "environment bearer",
            Some(("Authorization", "Bearer executor-secret")),
            Some("SOFIA_TEST_HOSTED_SECRET"),
            None,
            false,
        ),
        (
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        ...
        assert!(
            chatgpt_auth_provider_for_server(
                remote_server,
                Some(sofia_model_provider::auth_provider_from_auth(&hosted_auth)),
            )
            .is_none(),
        ...
        let connection_identity = |keyring_backend_kind| {
            McpServerConnectionIdentity::new(
                ...
                Some(&hosted_auth),
                /*sofia_apps_cache_identity*/ None,
                ElicitationCapability::default(),
                ClientMcpExtensions::default(),
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                sofia_apps_tools_cache: ConnectorRuntimeManager::default(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                sofia_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                    /*account_id*/ None, /*chatgpt_user_id*/ None,
                ),
                client_mcp_extensions: ClientMcpExtensions::default(),
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            sofia_apps_tools_cache: ConnectorRuntimeManager::<ToolInfo>::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            sofia_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                /*account_id*/ None, /*chatgpt_user_id*/ None,
            ),
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = SOFIA_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `sofia mcp login {server_name}`."
    );
...
fn mcp_init_error_display_identifies_oauth_reauthentication() {
    ...
    for (config, recovery_hint) in [
        (None, "Run `sofia mcp login example`."),
        (
            Some(&executor_config),
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("SOFIA_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
            http_headers_helper: None,
        },
...
async fn reconcile_reusable_server(
    ...
    reconcile_reusable_server_with_mcp_config(
        ...
        config,
        runtime_context,
        crate::mcp::tests::test_mcp_config(sofia_home.path().to_path_buf()),
    )
    .await
}
...
-> anyhow::Result<()> {
    ...
    let startup_tools = vec![create_test_tool(SOFIA_APPS_MCP_SERVER_NAME, "startup")];
    client
        .tool_catalog
    ...
    let old_call = old_binding
        .prepare_call(SOFIA_APPS_MCP_SERVER_NAME, "startup")
        .expect("startup call");
    ...
    assert!(previous.shares_test_connection_with(&republished, SOFIA_APPS_MCP_SERVER_NAME));
    assert_eq!(
        model_tool_names(capture_binding(&republished).await.tools()),
        HashSet::from([ToolName::namespaced("mcp__sofia_apps", "refreshed")]),
    );
...
async fn reconciliation_reuses_connection_without_relisting_regular_tools() -> anyhow::Result<()> {
    ...
    let initial_tools = list_tools_for_client_uncached(
        "docs",
        /*is_codex_apps_mcp_server*/ false,
        /*sofia_apps_refresh_trigger*/ "test",
        &client,
        /*timeout*/ None,
        crate::pagination::MAX_MCP_CATALOG_ITEMS,
...
fn connection_identity_uses_effective_authorization_headers() {
    let runtime_context = reusable_server_runtime_context();
    let missing_env_var = format!("SOFIA_TEST_UNSET_MCP_AUTHORIZATION_{}", std::process::id());
...
async fn reconciliation_reuses_legacy_stdio_server_with_existing_protocol_marker() {
    ...
    config.transport = McpServerTransportConfig::Stdio {
        ...
        env: Some(HashMap::from([(
            "SOFIA_MCP_PROTOCOL_VERSION".to_string(),
...
async fn reconciliation_replaces_connection_when_auth_mode_changes() -> anyhow::Result<()> {
    ...
    let sofia_home = tempdir()?;
    let mcp_config = crate::mcp::tests::test_mcp_config(sofia_home.path().to_path_buf());
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    .await;
    let sofia_home = tempdir().expect("tempdir");
    let mut mcp_config = crate::mcp::tests::test_mcp_config(sofia_home.path().to_path_buf());
    mcp_config.protocol_mode = sofia_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let connection_identity = |auth: &CodexAuth| {
        let provider = sofia_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            /*host_plugin_root*/ None,
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = sofia_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: sofia_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = sofia_login::test_support::transport_default_auth_route_config();
    let previous_auth = CodexAuth::AgentIdentity(
        sofia_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        sofia_login::auth::AgentIdentityAuthRecord {
