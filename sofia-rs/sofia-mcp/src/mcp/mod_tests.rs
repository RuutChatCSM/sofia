...
pub(crate) fn test_mcp_config(sofia_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: sofia_config::ConfigLayerStack::default(),
        approvals_reviewer: sofia_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: sofia_connectors::ConnectorSnapshot::default(),
    }
}
...
fn mcp_server_permissions_handle_unattached_and_threadless_servers() {
    ...
    let mut missing_server = sofia_apps_mcp_server_config(
        "https://example.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
...
#[test]
fn sofia_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        sofia_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        sofia_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        sofia_apps_mcp_url_for_base_url("http://localhost:8080/api/sofia"),
        "http://localhost:8080/api/sofia/ps/mcp"
    );
    assert_eq!(
        sofia_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/sofia/ps/mcp"
    );
}

#[test]
fn sofia_apps_server_config_uses_plugin_service_path() {
    let config = sofia_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for sofia apps"),
    };
...
#[test]
fn sofia_apps_server_config_forwards_thread_originator_header() {
    let config = sofia_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "sofia".to_string()),
                ]))
            );
...
#[test]
fn sofia_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "sofia"), (Some("tpp"), "tpp")] {
        let config = sofia_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn sofia_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = sofia_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = sofia_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(sofia_home.path().to_path_buf());
    ...
    let sofia_apps = effective
        .get(SOFIA_APPS_MCP_SERVER_NAME)
        .expect("sofia apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let sofia_apps = sofia_apps.config();
