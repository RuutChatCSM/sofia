...
pub const SOFIA_APPS_MCP_SERVER_NAME: &str = "sofia_apps";
const DEFAULT_CODEX_APPS_MCP_PRODUCT_SKU: &str = "sofia";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const SOFIA_CONNECTORS_TOKEN_ENV_VAR: &str = "SOFIA_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `sofia_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Sofia home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `sofia-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn effective_mcp_servers_from_configured(
    ...
    if !host_owned_codex_apps_enabled(config, auth) {
        servers.remove(SOFIA_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    sofia_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            sofia_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn sofia_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(SOFIA_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(SOFIA_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(SOFIA_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}
...
fn sofia_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_codex_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/sofia") {
        base_url
    } else {
        format!("{base_url}/api/sofia")
    };
    format!("{base_url}/ps/mcp")
}

pub fn sofia_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        sofia_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    sofia_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: sofia_apps_mcp_bearer_token_env_var(),
