...
//! and exposing cached Sofia Apps tools while a client is still connecting.
...
use crate::mcp::SOFIA_APPS_MCP_SERVER_NAME;
...
use crate::sofia_apps::normalize_codex_apps_callable_name;
use crate::sofia_apps::normalize_codex_apps_callable_namespace;
use crate::sofia_apps::normalize_codex_apps_tool_title;
use crate::sofia_apps::prepare_openai_file_params_for_model;
...
use sofia_async_utils::CancelErr;
...
use sofia_login::AuthChangeState;
...
use sofia_protocol::protocol::McpStartupUpdateEvent;
use sofia_rmcp_client::ExecutorStdioServerLauncher;
use sofia_rmcp_client::LocalStdioServerLauncher;
use sofia_rmcp_client::McpOAuthRefreshMode;
use sofia_rmcp_client::McpProtocolMode;
use sofia_rmcp_client::RmcpClient;
use sofia_rmcp_client::StdioServerLauncher;
use sofia_rmcp_client::StreamableHttpBearerToken;
use sofia_rmcp_client::StreamableHttpRedirectMode;
use sofia_rmcp_client::ToolWithConnectorId;
use sofia_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that Sofia should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "sofia/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "sofia/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "sofia.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "sofia.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const SOFIA_APPS_REFRESH_DURATION_METRIC: &str = "sofia.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const SOFIA_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const SOFIA_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) sofia_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
        ...
        self.tool_catalog
            .read(|catalog| {
                ...
                if catalog.revision == 0
                    && let Some(cache_context) = &self.sofia_apps_tools_cache_context
                {
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = sofia_apps_reconnect_backoff(state.consecutive_failures);
...
fn sofia_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    SOFIA_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(SOFIA_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    sofia_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        let Self {
            ...
            sofia_apps_tools_cache_context,
        ...
        let is_codex_apps_mcp_server = server_name == SOFIA_APPS_MCP_SERVER_NAME;
        ...
        async move {
            ...
            {
                emit_duration(
                    SOFIA_APPS_REFRESH_DURATION_METRIC,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            sofia_apps_tools_cache_context: sofia_apps_tools_cache_context.clone(),
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.sofia_apps_tools_cache_context
            .as_ref()
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    sofia_apps_refresh_trigger: &'static str,
    ...
    if is_codex_apps_mcp_server {
        emit_duration(
            ...
            fetch_start.elapsed(),
            &[("trigger", sofia_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Sofia Apps file parameters to the model as local-path inputs and adds plugin
...
fn tool_info_from_listed_tool(
    ...
    if is_codex_apps_mcp_server {
        sofia_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Sofia Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn sofia_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
...
async fn start_server_task(
    ...
    let fetch_ticket = sofia_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_codex_apps_mcp_server,
        /*sofia_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (sofia_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("Sofia Apps fetch ticket requires cache context"),
    };
...
fn record_protocol_discovery_metrics(
    ...
    let Some(metrics) = sofia_otel::global() else {
        return;
    };
    ...
    let _ = metrics.counter("sofia.mcp.protocol_discovery", /*inc*/ 1, &tags);
    let _ = metrics.record_duration(
        "sofia.mcp.protocol_discovery.duration_ms",
        started_at.elapsed(),
        &tags,
    );
}
...
pub(crate) fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("sofia-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Sofia"),
    )
...
pub(crate) async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(sofia_utils_path_uri::LegacyAppPathString::into_string);
                ...
                && let Some(env_var) = bearer_token_env_var.as_ref()
            {
                (
                    Arc::new(ExecutorEnvironmentHttpClient {
                        ...
                        http_client,
                    }) as Arc<dyn sofia_exec_server::HttpClient>,
...
mod tests {
    ...
    #[test]
    fn sofia_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: SOFIA_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "sofia_apps__gmail".to_string(),
