...
async fn reused_elicitation_senders_follow_each_servers_latest_permission_authority() {
    ...
    config.approval_policy = sofia_config::Constrained::allow_any(AskForApproval::Never);
    ...
    let hosted_server = crate::sofia_apps_mcp_server_config(
        "https://example.com",
    ...
    let hosted = manager.make_sender(
        crate::SOFIA_APPS_MCP_SERVER_NAME.to_string(),
        /*tx_event*/ None,
        &ClientMcpExtensions::default(),
    );
...
fn verification_fixture(
    ...
    let mut config = test_elicitation_config(
        crate::SOFIA_APPS_MCP_SERVER_NAME,
        approval_policy,
        PermissionProfile::Disabled,
    );
    ...
    catalog.register(crate::catalog::McpServerRegistration::from_hosted_apps(
        "verification-test",
        /*contribution_order*/ 0,
        crate::mcp::sofia_apps_mcp_server_config(
            "https://example.com",
            /*apps_mcp_product_sku*/ None,
            /*originator*/ None,
        ),
    ...
    let sender = manager.make_sender(
        crate::SOFIA_APPS_MCP_SERVER_NAME.into(),
        Some(tx),
...
async fn user_verification_rejects_attached_servers_even_if_they_use_the_plugin_service_name() {
    for name in ["attached", crate::SOFIA_APPS_MCP_SERVER_NAME] {
        let (manager, _, _) =
            verification_fixture(AskForApproval::OnRequest, /*reviewer*/ None);
        {
