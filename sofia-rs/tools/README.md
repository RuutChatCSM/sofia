...
`sofia-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `sofia-core`.
...
compatibility-sensitive orchestration in `sofia-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `sofia-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `sofia-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `sofia-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `sofia-core`.
3. Leave compatibility-sensitive adapters in `sofia-core` while downstream
...
`sofia-core`, that is a sign to revisit the extraction boundary before adding
more here.
