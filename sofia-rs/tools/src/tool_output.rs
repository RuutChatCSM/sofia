...
impl ToolOutput for sofia_protocol::mcp::CallToolResult {
    fn log_output(&self) -> String {
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    sofia_protocol::models::ContentItem::InputText { text }
                    | sofia_protocol::models::ContentItem::OutputText { text } => {
                    ...
                    sofia_protocol::models::ContentItem::InputAudio { audio_url } => {
