...
Higher-level request policy layered on `sofia-http-client` without any Sofia/OpenAI API awareness.
...
- Re-exports the low-level HTTP types temporarily so consumers can migrate to `sofia-http-client` incrementally.
