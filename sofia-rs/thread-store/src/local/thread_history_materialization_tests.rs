...
use sofia_protocol::realtime::BemItemPresentation;
use sofia_protocol::realtime::RealtimeItem;
use sofia_protocol::realtime::RealtimeItemContent;
use sofia_protocol::realtime::RealtimeSessionOutcome;
use sofia_protocol::realtime::RealtimeTranscriptRole;
use sofia_rollout::RolloutConfig;
use sofia_rollout::RolloutItem;
use sofia_rollout::RolloutLine;
use sofia_rollout::RolloutRecorder;
use sofia_rollout::RolloutRecorderParams;
...
/// Separate Sofia and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let sofia_home = root.path().join("sofia");
    ...
    let runtime = sofia_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Sofia home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !sofia_home.join(filename).exists(),
            "SQLite database should not be created under Sofia home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = sofia_state::open_thread_history_db(&sofia_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn paginated_fork_reads_compressed_shared_lineage_without_materializing() {
    ...
    assert!(matches!(
        error,
        crate::ThreadStoreError::InvalidRequest { message } if message.contains("must be in Sofia home")
    ));
    store
        .shutdown_thread(source_thread_id)
        .await
...
async fn catch_up_skips_invalid_complete_suffixes_and_projects_later_history() {
    ...
    for (name, suffix, expected_next_ordinal, expected_turn_ids) in cases {
        ...
        let pool = sofia_state::open_thread_history_db(
            &sofia_state::SqliteConfig::new_for_testing(home.path().abs()),
        )
        .await
        .expect("open thread history db");
        let before = projection_state(&pool, thread_id).await;
...
async fn projection_store(sofia_home: &Path) -> LocalThreadStore {
    let config = test_config(sofia_home);
...
fn rollout_line_byte_offsets(path: &std::path::Path, ordinal: u64) -> (i64, i64) {
    ...
    for line in bytes.split_inclusive(|byte| *byte == b'\n') {
        ...
        if sofia_rollout::parse_rollout_line_bytes(line)
            .ok()
