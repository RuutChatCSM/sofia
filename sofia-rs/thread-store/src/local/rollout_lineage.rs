...
impl LocalThreadStore {
    ...
    async fn resolve_rollout_lineage_with_representation(
        ...
        loop {
            ...
            let meta = sofia_rollout::read_session_meta_line(rollout_path.as_path())
                .await
            ...
            let rollout_path = match representation {
                    ...
                    if next_rollout_id.is_none() && meta.meta.history_base.is_none() =>
                {
                    ...
                    sofia_rollout::materialize_rollout_for_reference(rollout_path.as_path())
                        .await
                        .map_err(|err| ThreadStoreError::Internal {
                            message: format!(
...
async fn resolve_rollout_path_by_id(
    ...
    sofia_rollout::find_rollout_path_by_rollout_id(store.config.sofia_home.as_path(), rollout_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
async fn validate_cutoff_bounds(
    ...
    let contains_prefix = tokio::task::spawn_blocking(move || {
        sofia_rollout::rollout_contains_prefix(&path, end_byte_offset)
    })
    .await
    .map_err(|err| ThreadStoreError::Internal {
