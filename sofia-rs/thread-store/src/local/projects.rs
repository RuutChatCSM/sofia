...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&sofia_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> sofia_state::ProjectRoot {
    sofia_state::ProjectRoot { path: root.path }
}

fn stored_project(project: sofia_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project
