...
use serde_json::json;
use sofia_app_server_protocol::build_turns_from_rollout_items;
...
use sofia_protocol::mcp::McpResourceOrigin;
use sofia_protocol::mcp::McpResourceOriginCheckpoint;
...
fn move_to_archived(home: &Path, path: PathBuf) -> PathBuf {
    let directory = home.join(sofia_rollout::ARCHIVED_SESSIONS_SUBDIR);
...
fn read_rollout(path: &Path) -> Vec<RolloutLine> {
    fs::read_to_string(path)
        ...
        .lines()
        .map(|line| sofia_rollout::parse_rollout_line(line).expect("parse migrated rollout"))
        .collect()
}
...
async fn indexed_store(home: &Path) -> LocalThreadStore {
    ...
    let state_db = sofia_rollout::state_db::try_init(&rollout_config)
        .await
...
async fn migration_publishes_canonical_projected_history_and_is_idempotent() {
    let home = TempDir::new().expect("create Sofia home");
...
async fn migration_preserves_answers_before_a_rolled_back_steer() {
    ...
    for call_id in ["before-steer", "after-steer"] {
        ...
        let answer: sofia_rollout::RetainedContextEvent = serde_json::from_value(json!({
    ...
    assert_eq!(
        checkpoint
            ...
            .cloned()
            .map(
                |answer| sofia_rollout::RetainedContextEvent::VerifiedAnswer {
                    answer,
...
async fn assert_migrated_evidence_order(steer_order: Option<u64>) {
    ...
    let answers = checkpoint
        ...
        .map(|answer| {
            ...
            sofia_rollout::RetainedContextEvent::VerifiedAnswer {
                answer,
                acceptance_order,
            }
        })
    ...
    let path = write_rollout(
        ...
        vec![
            ...
            RolloutItem::ResponseItem(sofia_rollout::ResponseItemEnvelope {
                item: steer,
    ...
    let replacement_histories = read_rollout(&path)
        ...
        .filter_map(|line| match line.item {
            RolloutItem::Compacted(item) => item.replacement_history.map(|items| {
                items
                    .into_iter()
                    .map(sofia_rollout::ResponseItemEnvelope::into_item)
                    .collect::<Vec<_>>()
            }),
            _ => None,
        })
...
async fn migration_preserves_compressed_rollouts_during_publish_and_recovery() {
    ...
    assert_eq!(
        sofia_rollout::read_session_meta_line(&compressed_path)
            .await
...
async fn migration_preserves_legacy_displayed_thread_names() {
    ...
    sofia_rollout::append_thread_name(home.path(), title_thread_id, "stale index title")
        .await
        .expect("write stale legacy index name");
    sofia_rollout::append_thread_name(home.path(), index_thread_id, "indexed title")
        .await
...
async fn migration_apply_conflicts_with_rollout_maintenance() {
    ...
    let _maintenance_guard = sofia_rollout::try_acquire_rollout_maintenance_lock(home.path())
