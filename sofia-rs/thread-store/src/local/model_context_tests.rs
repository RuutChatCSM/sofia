...
async fn loads_latest_checkpoint_with_required_turn_metadata() {
    ...
    let thread_id = sofia_protocol::ThreadId::from_string(&uuid.to_string()).expect("thread id");
...
async fn loads_recent_context_after_many_empty_wake_turns() {
    ...
    let session_meta = sofia_rollout::read_session_meta_line(&path)
        .await
...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let session_meta = sofia_rollout::read_session_meta_line(path.as_path())
        .await
...
async fn fork_version_stops_before_older_segments_once_resolved() {
    for stored_version in [None, Some(MultiAgentVersion::Disabled)] {
        ...
        let mut source_meta = sofia_rollout::read_session_meta_line(&child_path)
            .await
...
async fn replays_nested_archived_lineage_from_frozen_prefix() {
    ...
    let child_meta = sofia_rollout::read_session_meta_line(child_path.as_path())
        .await
        .expect("read child metadata");
    let expected = vec![
...
fn rollout_end_byte_offset(path: &Path, end_ordinal_exclusive: u64) -> u64 {
    ...
    for line in contents.split_inclusive(|byte| *byte == b'\n') {
        let parsed = sofia_rollout::parse_rollout_line_bytes(line)
...
fn completed_user_message(turn_id: &str, message: &str) -> RolloutItem {
    RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
        thread_id: sofia_protocol::ThreadId::from_string("00000000-0000-0000-0000-000000000000")
