...
use sofia_rollout::ARCHIVED_SESSIONS_SUBDIR;
use sofia_rollout::RolloutReferenceIndex;
use sofia_rollout::SESSIONS_SUBDIR;
use sofia_rollout::find_archived_thread_path_by_id_str;
use sofia_rollout::find_thread_path_by_id_str;
use sofia_rollout::remove_thread_name_entries;
...
struct ThreadRollouts {
    ...
    rollout_ids: HashSet<sofia_protocol::ThreadId>,
    paths: Vec<PathBuf>,
}
...
impl ThreadRollouts {
    ...
    fn add_path(&mut self, path: PathBuf) {
        if let Some(rollout_id) = sofia_rollout::rollout_id_from_path(path.as_path()) {
...
async fn scan_reference_index(
    ...
    RolloutReferenceIndex::scan(store.config.sofia_home.as_path())
        .await
...
fn referenced_thread_error(thread_id: sofia_protocol::ThreadId) -> ThreadStoreError {
...
async fn delete_thread_after_reference_check(
    ...
    remove_thread_name_entries(store.config.sofia_home.as_path(), thread_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn delete_rollout_file(store: &LocalThreadStore, rollout_path: &Path) -> ThreadStoreResult<bool> {
    let plain_path = sofia_rollout::plain_rollout_path(rollout_path);
...
fn delete_rollout_path(store: &LocalThreadStore, rollout_path: &Path) -> ThreadStoreResult<bool> {
    let canonical_rollout_path = scoped_rollout_path(
        store.config.sofia_home.join(SESSIONS_SUBDIR),
        rollout_path,
        "sessions",
    )
    .or_else(|_| {
        scoped_rollout_path(
            store.config.sofia_home.join(ARCHIVED_SESSIONS_SUBDIR),
            rollout_path,
            "archived sessions",
        )
    })
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = sofia_state::open_thread_history_db(&config.sqlite)
            .await
