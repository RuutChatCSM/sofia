...
pub(super) fn migration_journal_path(sofia_home: &Path, thread_id: ThreadId) -> PathBuf {
    sofia_home
...
pub(super) async fn pending_migration_thread_ids(
    ...
    let mut entries = match tokio::fs::read_dir(sofia_home.join(MIGRATION_JOURNAL_DIRECTORY)).await
    {
        Ok(entries) => entries,
...
pub(super) async fn rewrite_subagent_history_boundary(
    ...
    source
        ...
        .await
        .map_err(migration_error)?;
    let mut head = sofia_rollout::parse_rollout_line_bytes(&head_bytes).map_err(migration_error)?;
