...
use sofia_rollout::parse_cursor;
use sofia_state::ThreadFilterOptions;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => sofia_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => sofia_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => sofia_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => sofia_rollout::SortDirection::Asc,
        SortDirection::Desc => sofia_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        sofia_home: store.config.sofia_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.sofia_home.clone(),
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<sofia_state::Anchor> {
            ...
            let thread_id = sofia_protocol::ThreadId::from_string(thread_id).map_err(|_| {
                ThreadStoreError::InvalidRequest {
                    message: format!("invalid cursor: {cursor}"),
                }
            })?;
            Ok(sofia_state::Anchor {
                ts: timestamp,
                id: Some(thread_id),
            })
        })
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| sofia_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: sofia_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => sofia_state::SortDirection::Asc,
            SortDirection::Desc => sofia_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    sofia_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    sofia_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
...
pub(super) async fn list_rollout_threads(
    state_db: Option<sofia_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&sofia_rollout::Cursor>,
    sort_key: sofia_rollout::ThreadSortKey,
    sort_direction: sofia_rollout::SortDirection,
) -> ThreadStoreResult<sofia_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    sofia_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    sofia_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = sofia_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    async fn list_paginated_threads_uses_sqlite_name_over_legacy_compatibility() {
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        sofia_rollout::append_thread_name(home.path(), thread_id, "stale index name")
            .await
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = sofia_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            sofia_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),
