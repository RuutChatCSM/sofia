...
async fn revert_keeps_thread_id_and_hides_suffix_across_repeated_reverts() {
    ...
    assert_ne!(
        sofia_rollout::rollout_id_from_path(first_replacement_path.as_path()),
        Some(thread_id)
    );
    let replacement_meta = sofia_rollout::read_session_meta_line(first_replacement_path.as_path())
        .await
    ...
    for version in [
        ...
        for (before_turn_id, multi_agent_version) in [("turn-2", Some(version)), ("turn-1", None)] {
            ...
            let path = state_db
                ...
                .expect("thread metadata")
                .rollout_path;
            let meta = sofia_rollout::read_session_meta_line(&path)
                .await
                .expect("read replacement metadata");
...
async fn rollout_paths_for_thread(
    ...
    sofia_rollout::RolloutReferenceIndex::scan(home)
        .await
