...
) -> ThreadStoreResult<Vec<sofia_protocol::ThreadId>> {
...
async fn archive_thread_with_paths(
    ...
    let archive_folder = store
        .config
        .sofia_home
        .join(sofia_rollout::ARCHIVED_SESSIONS_SUBDIR);
    ...
    for rollout_path in rollout_paths {
        if rollout_path_is_archived(store.config.sofia_home.as_path(), rollout_path.as_path()) {
            continue;
        }
        let canonical_rollout_path = scoped_rollout_path(
            store.config.sofia_home.join(sofia_rollout::SESSIONS_SUBDIR),
