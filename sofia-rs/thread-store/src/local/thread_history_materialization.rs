...
use chrono::DateTime;
use sofia_app_server_protocol::ThreadHistoryChangeSet;
...
const SQLITE_PROJECTION_METRIC: &str = "sofia.thread_history.sqlite_projection";
const SQLITE_PROJECTION_ANOMALY_METRIC: &str = "sofia.thread_history.sqlite_projection.anomaly";
...
async fn materialize_to_sqlite_with_state_db(
    ...
    if projection_state.is_none()
        && sofia_rollout::existing_rollout_path(rollout_path)
            .await
            .is_none()
    {
        return Ok(());
...
async fn read_projection_steps(
    ...
    let file =
        tokio::task::spawn_blocking(move || sofia_rollout::open_rollout_seekable_reader(&path))
            .await
    ...
    for line_bytes in bytes[..complete_byte_count].split_inclusive(|byte| *byte == b'\n') {
        ...
        let line = match sofia_rollout::decode_rollout_line(value) {
            Ok(line) => line,
            Err(err) => {
                warn!(
                    thread_id = %thread_id,
