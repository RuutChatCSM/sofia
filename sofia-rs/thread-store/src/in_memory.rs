...
use sofia_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| sofia_state::ThreadSection {
                name: if id == sofia_state::PINNED_THREAD_SECTION_ID {
                    sofia_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<sofia_protocol::protocol::GitInfo> {
    ...
    Some(sofia_protocol::protocol::GitInfo {
