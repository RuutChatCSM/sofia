...
This repository uses Bazel to build the Rust workspace under `sofia-rs`.
...
- `rules_rs` imports third-party crates from `sofia-rs/Cargo.toml` and
  `sofia-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `sofia_rust_crate`, which wraps `rust_library`,
...
- Each crate in `sofia-rs/*/BUILD.bazel` typically uses `sofia_rust_crate` and
...
Sofia uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/sofia \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //sofia-rs/cli:sofia
```
...
2. Create a `BUILD.bazel` that calls `sofia_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `sofia_rust_crate` to configure it.
