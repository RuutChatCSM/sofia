...
impl PidBackend {
    ...
    pub(crate) fn new(sofia_bin: PathBuf, pid_file: PathBuf, remote_control_enabled: bool) -> Self {
    ...
    pub(crate) fn new_update_loop(sofia_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            sofia_bin,
            pid_file,
                ...
                match self.command_kind {
                    PidCommandKind::AppServer { .. } => {
                        let sofia_home = self
                            ...
                            .context("daemon pid path has no Sofia home")?;
                        let socket_path =
                            sofia_app_server_transport::app_server_control_socket_path(sofia_home)?;
                        if let Err(err) =
