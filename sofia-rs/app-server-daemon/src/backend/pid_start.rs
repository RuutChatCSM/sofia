...
impl PidBackend {
    pub(super) async fn start_inner(&self, replacement: Option<PidRecord>) -> Result<Option<u32>> {
        ...
        let sofia_bin = fs::canonicalize(&self.sofia_bin)
            .await
            .unwrap_or_else(|_| self.sofia_bin.clone());
        let launched_identity =
            if matches!(self.command_kind, super::PidCommandKind::AppServer { .. }) {
                executable_identity(&sofia_bin).await.ok()
            } else {
                None
            };
        ...
        #[cfg(windows)]
        {
            ...
            for name in [
                ...
                if matches!(name, "SOFIA_SQLITE_HOME" | "npm_config_cafile") {
                ...
                let expands_home = if name == "npm_config_cafile" {
                    ...
                    matches!(
                        name,
                        "SOFIA_SQLITE_HOME" | "AWS_CONFIG_FILE" | "AWS_SHARED_CREDENTIALS_FILE"
            ...
            match self.command_kind {
                PidCommandKind::AppServer { .. } => {
                    command.env(sofia_app_server_transport::DAEMON_SHUTDOWN_SOCKET_ENV, "1");
                }
                PidCommandKind::UpdateLoop => {
                    ...
                    command.env(
                        sofia_app_server_transport::DAEMON_SHUTDOWN_FILE_ENV,
                        shutdown_file,
                    );
                }
            }
