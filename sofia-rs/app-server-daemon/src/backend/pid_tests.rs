...
use sofia_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
...
async fn start_retries_stale_empty_pid_file_under_its_own_lock() {
    ...
    sofia_uds::prepare_private_socket_directory(&state_dir)
        .await
    ...
    tokio::fs::write(&pid_file, "")
        ...
        .expect("write pid file");
    let backend = PidBackend::new(
        temp_dir.path().join("missing-sofia"),
        pid_file,
        /*remote_control_enabled*/ false,
    );
...
async fn pid_record_captures_the_resolved_launch_binary() {
    ...
    let original = temp.path().join("original-sofia");
    let replacement = temp.path().join("replacement-sofia");
    for (path, bytes) in [
        (&original, b"#!/bin/sh\nexec sleep 30\n".as_slice()),
        (
            &replacement,
    ...
    let selected = temp.path().join("current-sofia");
...
async fn shutdown_grace_child() {
    let Some(ready) = std::env::var_os("SOFIA_TEST_SHUTDOWN_GRACE_READY") else {
        return;
    };
    ...
    let _ = tokio::time::timeout(Duration::from_secs(8), async {
        ...
        if std::env::var_os("SOFIA_TEST_SHUTDOWN_GRACE_EXIT").is_some() {
...
async fn shutdown_grace_handles_process_exit() {
    ...
    for (name, grace_seconds, exits) in [
        ...
        let mut child = std::process::Command::new(std::env::current_exe().expect("test binary"))
            ...
            .env("SOFIA_TEST_SHUTDOWN_GRACE_READY", &ready)
            .envs(exits.then_some(("SOFIA_TEST_SHUTDOWN_GRACE_EXIT", "1")))
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
        ...
        #[cfg(unix)]
        let backend = PidBackend::new(
            temp.path().join("sofia"),
            pid_file,
            /*remote_control_enabled*/ false,
        );
        #[cfg(windows)]
        let backend = PidBackend::new_update_loop(temp.path().join("sofia"), pid_file);
...
async fn exited_unreaped_updater_is_reaped() {
    ...
    let backend =
        PidBackend::new_update_loop(temp.path().join("sofia"), temp.path().join("updater.pid"));
...
async fn failed_updater_handoff_preserves_predecessor_record() {
    ...
    let backend = PidBackend::new_update_loop(
        temp.path().join("missing-sofia.exe"),
