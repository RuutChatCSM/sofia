...
fn discovers_package_and_legacy_installs() {
    ...
    let expected = if cfg!(windows) {
        current.join("bin").join("sofia.exe")
    } else {
        legacy.clone()
    };
...
fn updater_only_runs_for_stable_installer_owned_releases() {
    ...
    let managed = release.join("bin/sofia");
    ...
    let alpha_managed = alpha.join("bin/sofia");
    ...
    let local_managed = local.join("bin/sofia");
...
async fn older_managed_binary_does_not_claim_updater_support() {
    ...
    let binary = temp.path().join("sofia");
