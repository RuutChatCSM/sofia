...
fn manual_update_daemon(home: &TempDir) -> (Daemon, String) {
    ...
    let bin = standalone.join("releases").join(&release).join("sofia");
    std::fs::create_dir_all(bin.parent().expect("binary parent")).expect("release directory");
    std::fs::write(
        &bin,
        b"#!/bin/sh\nif [ \"$1\" = '--version' ]; then echo sofia 1.0.0; else exec sleep 30; fi\n",
    )
    ...
    (
        Daemon {
            ...
            managed_codex_bin: standalone.join("current/sofia"),
        },
        release,
    )
}
...
async fn manual_request_retries_after_updater_replacement() {
    ...
    sofia_uds::prepare_private_socket_directory(socket_path.parent().expect("socket parent"))
        .await
        .expect("socket directory");
    let mut listener = sofia_uds::UnixListener::bind(&socket_path)
        .await
    ...
    let server = tokio::spawn(async move {
        ...
        let mut successor = sofia_uds::UnixListener::bind(&socket_path)
            .await
...
async fn unsupported_request_preserves_updater_schedule() {
    ...
    let mut malformed = sofia_uds::UnixStream::connect(&socket_path)
        .await
...
async fn manual_update_restarts_managed_daemon_with_automatic_updates_disabled() {
    ...
    std::fs::create_dir_all(daemon.socket_path.parent().expect("socket parent"))
        .expect("socket directory");
    let mut listener = sofia_uds::UnixListener::bind(&daemon.socket_path)
        .await
    ...
    let server = tokio::spawn(async move {
        loop {
            ...
            let version = if std::fs::read_to_string(
                sofia_home.join("packages/standalone/auto-update-version"),
            )
            ...
            websocket.send(tokio_tungstenite::tungstenite::Message::Text(
                serde_json::json!({"id": 1, "result": {
                    "userAgent": format!("sofia_app_server_daemon/{version}"),
                    "codexHome": sofia_home, "platformFamily": "unix", "platformOs": std::env::consts::OS,
                }}).to_string().into(),
    ...
    let script = format!(
        "#!/bin/sh\n# SOFIA_INSTALL_IF_LATEST\ntest \"$SOFIA_INSTALL_IF_LATEST\" = 1 || exit 4\nif [ \"$SOFIA_UPDATE_FROM_RELEASE\" = '{next}' ]; then exit 0; fi\ntest \"$SOFIA_UPDATE_FROM_RELEASE\" = '{release}' || exit 5\ntouch '{ready}'\nwhile [ ! -e '{proceed}' ]; do sleep .05; done\nmkdir -p '{root}/releases/{next}/bin'\nprintf '#!/bin/sh\\nif [ \"$1\" = --version ]; then echo sofia 1.1.0; else exec sleep 30; fi\\n' > '{root}/releases/{next}/bin/sofia'\nchmod +x '{root}/releases/{next}/bin/sofia'\nln -sfn 'releases/{next}' '{root}/current'\nprintf '{next}' > '{root}/auto-update-version'\n",
    ...
    let mut queued = sofia_uds::UnixStream::connect(&daemon.manual_update_socket_path())
        .await
    ...
    assert_eq!(
        ...
        standalone.join("current/bin/sofia")
    );
