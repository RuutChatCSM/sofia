...
pub(crate) fn managed_codex_bin(sofia_home: &Path) -> PathBuf {
    let current = sofia_home
        .join("packages")
        .join("standalone")
        .join("current");
...
pub(crate) fn is_stable_standalone_release(sofia_home: &Path, sofia_bin: &Path) -> bool {
    let standalone = sofia_home.join("packages/standalone");
    ...
    components.len() == 3
        ...
        && std::fs::canonicalize(sofia_bin).is_ok_and(|bin| bin.starts_with(&release))
}

/// Older managed binaries can serve app-server requests without owning an updater.
pub(crate) async fn supports_daemon_update_loop(sofia_bin: &Path) -> bool {
    let mut command = Command::new(sofia_bin);
    #[cfg(windows)]
...
pub(crate) async fn resolved_managed_codex_bin(sofia_bin: &Path) -> Result<PathBuf> {
    fs::canonicalize(sofia_bin).await.with_context(|| {
        format!(
            "failed to resolve managed Sofia binary {}",
            sofia_bin.display()
        )
    })
}

pub(crate) async fn managed_codex_version(sofia_bin: &Path) -> Result<String> {
    let mut command = Command::new(sofia_bin);
    #[cfg(windows)]
    command.creation_flags(windows_sys::Win32::System::Threading::CREATE_NO_WINDOW);
    let output = command.arg("--version").output().await.with_context(|| {
        format!(
            "failed to invoke managed Sofia binary {}",
            sofia_bin.display()
        )
    })?;
    if !output.status.success() {
        return Err(anyhow!(
            "managed Sofia binary {} exited with status {}",
            sofia_bin.display(),
            output.status
        ));
    }
    ...
    let stdout = String::from_utf8(output.stdout).with_context(|| {
        format!(
            "managed Sofia version was not utf-8: {}",
            sofia_bin.display()
        )
    })?;
...
fn managed_codex_file_name() -> &'static str {
    if cfg!(windows) { "sofia.exe" } else { "sofia" }
}
...
fn parse_codex_version(output: &str) -> Result<String> {
    let version = output
        ...
        .ok_or_else(|| anyhow!("managed Sofia version output was malformed"))?;
