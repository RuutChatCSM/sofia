...
async fn run_with_http(
    ...
    let mut listener = Some(sofia_uds::UnixListener::bind(&socket_path).await?);
    #[cfg(windows)]
...
async fn adopt_managed_updater(
    ...
    listener: &mut Option<sofia_uds::UnixListener>,
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(err) = replacement.replace_current_updater().await {
            ...
            *listener = Some(sofia_uds::UnixListener::bind(&socket_path).await?);
            return Err(err);
        }
...
async fn update_once(
    ...
    anyhow::ensure!(
        script
            .windows(b"SOFIA_INSTALL_IF_LATEST".len())
            .any(|window| window == b"SOFIA_INSTALL_IF_LATEST"),
...
fn selected_release(daemon: &Daemon) -> Result<(&Path, std::path::PathBuf, String)> {
    let home = daemon
        ...
        .context("daemon settings path has no Sofia home")?;
...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Sofia binary {}",
...
async fn run_installer_script(
    ...
    let mut child = command
        .env("SOFIA_RELEASE", "latest")
        .env("CODEX_NON_INTERACTIVE", "1")
        .env("SOFIA_INSTALL_IF_LATEST", "1")
        .env("SOFIA_UPDATE_FROM_RELEASE", previous_release)
        .kill_on_drop(true)
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Sofia updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Sofia updater stdin was unavailable")?;
    #[cfg(unix)]
    ...
    if write_result.is_none() {
        cancel_installer(&mut child, sofia_home).await;
        return Ok(UpdateLoopControl::Stop);
    }
    write_result
        .context("installer write was cancelled")?
        .context("failed to pass standalone Sofia updater to shell")?;
    #[cfg(unix)]
    ...
    let status = status.context("failed to wait for standalone Sofia updater")?;

    if status.success() {
        Ok(UpdateLoopControl::Continue)
    } else {
        anyhow::bail!("standalone Sofia updater exited with status {status}")
    }
}

#[cfg(unix)]
async fn cancel_installer(child: &mut tokio::process::Child, sofia_home: &Path) {
    ...
    let lock = sofia_home.join("packages/standalone/install.lock.d");
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Sofia updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Sofia updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Sofia updater")?
            .to_vec();
...
impl Signal {
    async fn recv(&mut self) -> Option<()> {
        ...
        let _ = sofia_app_server_transport::daemon_shutdown_signal().await;
        Some(())
    }
}
