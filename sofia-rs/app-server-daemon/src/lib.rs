...
use sofia_app_server_transport::app_server_control_socket_path;
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "sofia app-server daemon lifecycle is only supported on Unix and Windows platforms"
    ))
}
...
impl Daemon {
    fn from_environment() -> Result<Self> {
        ...
        let socket_path = app_server_control_socket_path(sofia_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = sofia_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_codex_bin: managed_codex_bin(sofia_home.as_path()),
        })
    }
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by sofia app-server daemon"
            ));
        }
    ...
    async fn ensure_managed_updater(&self, settings: &DaemonSettings) -> Result<bool> {
        ...
        if !managed_install::supports_daemon_update_loop(&sofia_bin).await
            || !self.is_stable_standalone_release()?
            || !managed_install::resolved_managed_codex_bin(&self.managed_codex_bin)
                .await
                .is_ok_and(|selected| selected == sofia_bin)
        {
            if !self.has_latest_selection_marker() {
                updater.stop().await?;
            }
        ...
        backend::pid_update_loop_backend(self.backend_paths_with_bin(settings, &sofia_bin))
            .start()
            .await?;
        Ok(true)
    }
    ...
    fn ensure_managed_codex_bin(&self) -> Result<()> {
        ...
        Err(anyhow!(
            "managed standalone Sofia install not found at {managed_codex_path}\n\n\
             This command requires the standalone install managed by the Sofia installer, because \
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            sofia_bin: managed_codex_bin.to_path_buf(),
    ...
    async fn open_operation_lock_file(&self) -> Result<tokio::fs::File> {
        if let Some(parent) = self.operation_lock_file.parent() {
            ...
            sofia_uds::prepare_private_socket_directory(parent)
                .await
                .with_context(|| {
                    format!(
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_codex_path: "sofia".into(),
            managed_codex_version: Some("1.2.3".to_string()),
            socket_path: "sofia.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedCodexPath": "sofia",
                "managedCodexVersion": "1.2.3",
                "socketPath": "sofia.sock",
                "cliVersion": "1.2.3",
    ...
    async fn stop_creates_missing_home_parent() {
        ...
        let daemon = Daemon {
            ...
            managed_codex_bin: state.join("missing-sofia"),
        };
        assert_eq!(
            daemon
    ...
    async fn managed_local_backend_counts_as_bootstrapped_without_updater() {
        ...
        let local_bin = standalone.join("local-main/bin/sofia");
        ...
        let daemon = Daemon {
            ...
            operation_lock_file: state.join("daemon.lock"),
            settings_file: state.join("settings.json"),
            managed_codex_bin: standalone.join("current/bin/sofia"),
        };
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_codex_bin: temp_dir.path().join("missing-sofia"),
        };
