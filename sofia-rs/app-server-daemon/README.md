# sofia-app-server-daemon

> `sofia-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`sofia-app-server-daemon` backs the machine-readable `sofia app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Sofia instances launched over SSH, including fresh developer
...
shorter `SOFIA_HOME` to share the daemon; discovery does not trust a mutable alias.
...
An invocation that sets `SOFIA_EXEC_SERVER_URL` skips implicit daemon attachment
...
```sh
sofia app-server daemon start
sofia app-server daemon restart
sofia app-server daemon update
sofia app-server daemon enable-remote-control
sofia app-server daemon disable-remote-control
sofia app-server daemon stop
sofia app-server daemon version
sofia app-server daemon bootstrap --remote-control
```
...
default. Edit `SOFIA_HOME/app-server-daemon/settings.json` to change this:
...
not affect an explicit `sofia update` command or `daemon update`.

`daemon update` checks the latest stable release once, even with automatic
updates disabled. It requires a Sofia installer-owned latest-channel standalone
...
settings under `SOFIA_HOME/app-server-daemon/`, starts app-server as a
...
on Windows) and its managed binary under `SOFIA_HOME/packages/standalone/current`:
`bin/sofia` or `bin/sofia.exe`, falling back to the legacy flat layout when present.
...
- if the updater was absent during a same-version binary replacement, a later
  ...
  executable identity; use `sofia app-server daemon restart` to refresh the server
...
Top-level `sofia remote-control start` enables and persists remote control for
the managed daemon, overriding a saved disabled value. It starts or bootstraps
the daemon as needed. Plain `sofia remote-control` runs a separate foreground
server and does not change daemon settings; `sofia remote-control stop` stops
...
All mutating lifecycle commands are serialized per `SOFIA_HOME`, so a concurrent
...
The daemon stores its local state under `SOFIA_HOME/app-server-daemon/`:
