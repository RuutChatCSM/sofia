...
use sofia_protocol::protocol::GuardianAssessmentOutcome;
use sofia_protocol::protocol::GuardianCommandSource;
use sofia_protocol::protocol::GuardianRiskLevel;
use sofia_protocol::protocol::GuardianUserAuthorization;
...
pub(crate) fn sofia_artifact_operation_event_request(
...
pub(crate) struct CodexRuntimeMetadata {
    pub(crate) sofia_rs_version: String,
...
pub enum GuardianApprovalRequestSource {
    /// Approval requested directly by the main Sofia turn.
    MainTurn,
...
pub(crate) struct CodexCompactionEventParams {
    ...
    pub(crate) sofia_error_kind: Option<CodexErrKind>,
    pub(crate) sofia_error_http_status_code: Option<u16>,
...
pub(crate) struct CodexGoalEventParams {
    ...
    pub(crate) goal_status: sofia_state::ThreadGoalStatus,
...
pub(crate) struct CodexTurnEventParams {
    ...
    pub(crate) sofia_turn_source: Option<String>,
...
pub(crate) fn plugin_state_event_type(state: PluginState) -> &'static str {
    match state {
        PluginState::Installed => "sofia_plugin_installed",
        PluginState::Uninstalled => "sofia_plugin_uninstalled",
        PluginState::Enabled => "sofia_plugin_enabled",
        PluginState::Disabled => "sofia_plugin_disabled",
    }
}

pub(crate) fn sofia_app_metadata(
    tracking: &TrackEventsContext,
    app: AppInvocation,
) -> CodexAppMetadata {
    CodexAppMetadata {
...
pub(crate) fn sofia_plugin_metadata(plugin: PluginTelemetryMetadata) -> CodexPluginMetadata {
    sofia_plugin_metadata_with_product_client_id(plugin, originator().value)
}

fn sofia_plugin_metadata_with_product_client_id(
...
pub(crate) fn sofia_plugin_install_requested_metadata(
    tracking: &TrackEventsContext,
...
pub(crate) fn sofia_compaction_event_params(
    ...
    CodexCompactionEventParams {
        ...
        status: input.status,
        sofia_error_kind: input.sofia_error_kind,
        sofia_error_http_status_code: input.sofia_error_http_status_code,
...
pub(crate) fn sofia_goal_event_params(
...
pub(crate) fn sofia_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
    ...
    CodexPluginUsedMetadata {
        plugin: sofia_plugin_metadata_with_product_client_id(
            plugin,
...
pub(crate) fn sofia_hook_run_metadata(
    tracking: &TrackEventsContext,
    hook: HookRunFact,
...
pub(crate) fn current_runtime_metadata() -> CodexRuntimeMetadata {
    ...
    CodexRuntimeMetadata {
        sofia_rs_version: env!("CARGO_PKG_VERSION").to_string(),
