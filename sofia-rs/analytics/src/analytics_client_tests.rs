...
use crate::events::sofia_app_metadata;
use crate::events::sofia_hook_run_metadata;
use crate::events::sofia_plugin_metadata;
use crate::events::sofia_plugin_used_metadata;
...
use serde_json::json;
use sofia_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use sofia_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use sofia_app_server_protocol::CollabAgentTool;
use sofia_app_server_protocol::CollabAgentToolCallStatus;
use sofia_app_server_protocol::CommandAction;
...
use sofia_app_server_protocol::DynamicToolCallStatus;
use sofia_app_server_protocol::GuardianApprovalReview;
use sofia_app_server_protocol::GuardianApprovalReviewAction;
use sofia_app_server_protocol::GuardianApprovalReviewStatus;
use sofia_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use sofia_app_server_protocol::ImageGenerationItem;
use sofia_app_server_protocol::InitializeCapabilities;
use sofia_app_server_protocol::InitializeParams;
use sofia_app_server_protocol::ItemCompletedNotification;
use sofia_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use sofia_app_server_protocol::ItemStartedNotification;
use sofia_app_server_protocol::JSONRPCErrorError;
use sofia_app_server_protocol::McpToolCallAppContext;
use sofia_app_server_protocol::McpToolCallStatus;
use sofia_app_server_protocol::NonSteerableTurnKind;
use sofia_app_server_protocol::PatchApplyStatus;
use sofia_app_server_protocol::PermissionsRequestApprovalParams;
use sofia_app_server_protocol::RequestId;
use sofia_app_server_protocol::RequestPermissionProfile;
use sofia_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use sofia_app_server_protocol::ServerNotification;
use sofia_app_server_protocol::ServerRequest;
use sofia_app_server_protocol::ServerResponse;
use sofia_app_server_protocol::SessionSource as AppServerSessionSource;
use sofia_app_server_protocol::SubAgentActivityKind;
...
use sofia_app_server_protocol::ThreadSource as AppServerThreadSource;
use sofia_app_server_protocol::ThreadStartResponse;
use sofia_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
use sofia_app_server_protocol::TurnError as AppServerTurnError;
...
use sofia_app_server_protocol::TurnStatus as AppServerTurnStatus;
use sofia_app_server_protocol::TurnSteerParams;
use sofia_app_server_protocol::TurnSteerResponse;
use sofia_app_server_protocol::UserInput;
use sofia_app_server_protocol::WebSearchItem;
use sofia_login::default_client::DEFAULT_ORIGINATOR;
use sofia_login::default_client::originator;
use sofia_plugin::AppConnectorId;
use sofia_plugin::PluginCapabilitySummary;
use sofia_plugin::PluginId;
use sofia_plugin::PluginTelemetryMetadata;
use sofia_protocol::approvals::NetworkApprovalProtocol;
...
use sofia_protocol::models::PermissionProfile as CorePermissionProfile;
use sofia_protocol::protocol::AskForApproval;
use sofia_protocol::protocol::HookEventName;
use sofia_protocol::protocol::HookExecutionMode;
use sofia_protocol::protocol::HookHandlerType;
use sofia_protocol::protocol::HookRunStatus;
use sofia_protocol::protocol::HookSource;
use sofia_protocol::protocol::SessionSource;
use sofia_protocol::protocol::SubAgentSource;
use sofia_protocol::protocol::ThreadSource;
...
const TEST_PRODUCT_CLIENT_ID: &str = "sofia_work_desktop";
...
impl TurnAnalyticsMetadata for TestTurnMetadata {
    ...
    fn sofia_turn_source(&self) -> Option<String> {
        None
    }
}
...
fn sample_runtime_metadata() -> CodexRuntimeMetadata {
    CodexRuntimeMetadata {
        sofia_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(sofia_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: sofia_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    sofia_error_info: Option<sofia_app_server_protocol::CodexErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: sofia_error_info.map(|sofia_error_info| AppServerTurnError {
                misalignment: None,
                message: "turn failed".to_string(),
                sofia_error_info: Some(sofia_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                misalignment: None,
                message: "cannot steer a review turn".to_string(),
                sofia_error_info: Some(CodexErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "sofia-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "sofia-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "sofia-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "sofia-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn plugin_measurements(rows: Vec<PluginMeasurementRow>) -> PluginMeasurementsInput {
    PluginMeasurementsInput {
        ...
        originator: "sofia_cli_rs".to_string(),
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        runtime: CodexRuntimeMetadata {
            sofia_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(sofia_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: sofia_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.sofia/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".sofia/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.sofia/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/sofia/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.sofia/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(CodexAppMentionedEventRequest {
        event_type: "sofia_app_mentioned",
        event_params: sofia_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        CodexAcceptedLineFingerprintsEventRequest {
            event_type: "sofia_accepted_line_fingerprints",
            event_params: CodexAcceptedLineFingerprintsEventParams {
                event_type: "sofia.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("sofia".to_string()),
                model_slug: Some("gpt-5.1-sofia".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "sofia",
                "model_slug": "gpt-5.1-sofia",
                "completed_at": 1710000000,
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "sofia-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
    let auth_manager = sofia_login::AuthManager::from_auth_for_testing(
        sofia_login::CodexAuth::create_dummy_chatgpt_auth_for_testing(),
    );
...
"
    ...
    let event_types = contents
        ...
        .collect::<Vec<_>>();
    assert!(event_types.iter().any(|event| event == "sofia_turn_event"));
    assert!(
        event_types
            .iter()
            .any(|event| event == "sofia_accepted_line_fingerprints")
    );
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(CodexCompactionEventRequest {
        event_type: "sofia_compaction_event",
        event_params: crate::events::sofia_compaction_event_params(
            CodexCompactionEvent {
                ...
                sofia_error_kind: None,
                sofia_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "sofia-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "sofia_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "sofia_error_kind": null,
                "sofia_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "sofia_thread_initialized",
...
async fn thread_initialized_classifies_validated_linked_worktrees() {
    let root = std::env::temp_dir().join(format!(
        "sofia-analytics-worktree-{}",
        sofia_protocol::ThreadId::new()
    ));
    ...
    for (cwd, expected) in [
        ...
        start.thread.cwd = sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(cwd)
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(CodexCommandExecutionEventRequest {
        event_type: "sofia_command_execution_event",
        event_params: CodexCommandExecutionEventParams {
            base: CodexToolItemEventBase {
                ...
                app_server_client: CodexAppServerClientMetadata {
                    product_client_id: "sofia_tui".to_string(),
                    client_name: Some("sofia-tui".to_string()),
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "sofia_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["sofia_rs_version"],
        "0.99.0"
    );
...
async fn thread_originator_overrides_shared_connection_across_thread_events() {
    ...
    assert_eq!(
        ...
        json!([
            {
                "event_type": "sofia_thread_archive_event",
                "event_params": {
                    "thread_id": "thread-work",
                    "action": "archived",
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        sofia_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                CodexCompactionEvent {
                    ...
                    sofia_error_kind: Some(CodexErrKind::ContextWindowExceeded),
                    sofia_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sofia_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["sofia_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["sofia_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("sofia-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sofia_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sofia_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn plugin_measurement_batch_emits_directly_and_filters_invalid_rows() {
    ...
    assert_eq!(
        ...
        json!([
            {
                "event_type": "sofia_plugin_measurement_event",
                "event_params": {
                    "thread_id": "thread-1",
                    "turn_id": "turn-1",
                    "item_id": "item-1",
...
async fn command_execution_approval_response_publishes_user_review_event() {
    for (kind, approval_id, subject, trigger) in [
        ...
        let payload = serde_json::to_value(&events).expect("serialize events");
        assert_eq!(payload.as_array().expect("events array").len(), 1);
        assert_eq!(payload[0]["event_type"], "sofia_review_event");
        assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
        assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
        assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn collaborator_tool_events_keep_response_ids_when_completion_races_sampling() {
    for response_first in [false, true] {
        ...
        assert_eq!(
            ...
            json!({
                "type": "sofia_collab_agent_tool_call_event",
                "item": "call-1",
                "origin": "response-1",
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "sofia_review_event");
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        sofia_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        sofia_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        sofia_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn guardian_events_keep_thread_source_and_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        sofia_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.sofia_rs_version, "0.1.0");
...
async fn subagent_tool_items_inherit_parent_connection_metadata() {
    ...
    assert_eq!(payload[1]["event_type"], "sofia_dynamic_tool_call_event");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
        event_type: "sofia_plugin_used",
        event_params: sofia_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(CodexPluginEventRequest {
        event_type: "sofia_plugin_installed",
        event_params: sofia_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(CodexPluginInstallFailedEventRequest {
        event_type: "sofia_plugin_install_failed",
        event_params: CodexPluginInstallFailedMetadata {
            plugin: sofia_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(CodexPluginEventRequest {
        event_type: "sofia_plugin_installed",
        event_params: sofia_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(CodexHookRunEventRequest {
        event_type: "sofia_hook_run",
        event_params: sofia_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(sofia_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(sofia_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(sofia_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(sofia_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(sofia_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
...
async fn reducer_ingests_artifact_operation_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "sofia_artifact_operation",
            "event_params": {
                "thread_id": "thread-1",
                "turn_id": "turn-1",
                "item_id": "call-1",
            ...
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    location: SkillInvocationLocation::Host {
                        path: skill_path,
                        scope: sofia_protocol::protocol::SkillScope::User,
                    },
                    plugin_id: None,
...
async fn reducer_includes_plugin_ids_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.sofia/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "sofia_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "sofia_app_mentioned");
    assert_eq!(payload[1]["event_type"], "sofia_app_used");
    assert_eq!(payload[2]["event_type"], "sofia_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "sofia_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        serde_json::to_value(&events).expect("serialize events"),
        json!([{
            "event_type": "sofia_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "sofia_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        CodexOnboardingExternalAgentImportFailureEventRequest {
            event_type: "sofia_onboarding_external_agent_import_failure",
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(CodexTurnEventRequest {
        ...
        event_params: crate::events::CodexTurnEventParams {
            ...
            sofia_turn_source: Some("composer".to_string()),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "root_turn_id": "turn-2",
                "turn_trigger": "user",
                "sofia_turn_source": "composer",
                "submission_type": null,
                "app_server_client": {
                    "product_client_id": "sofia_cli_rs",
                    "client_name": "sofia-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("sofia_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["sofia_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("sofia_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
    ...
    assert_eq!(
        (
            ...
            payload["event_params"].get("sofia_turn_source"),
        ),
        (
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        [
            "sofia_command_execution_event",
            "sofia_file_change_event",
            "sofia_mcp_tool_call_event",
            "sofia_dynamic_tool_call_event",
            "sofia_web_search_event",
            "sofia_image_generation_event",
            "sofia_collab_agent_tool_call_event",
            "sofia_control_tool_call_event",
        ]
    ...
    *turn_metadata.root_turn_id.lock().expect("root turn ID") = None;
    ...
    assert_eq!(
        payload
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "sofia_command_execution_event", "turn_id": "turn-1", "item_id": "before-conflict", "root_turn_id": "root-ancestor"}),
            json!({"event_type": "sofia_command_execution_event", "turn_id": "turn-1", "item_id": "after-conflict", "root_turn_id": null}),
            json!({"event_type": "sofia_control_tool_call_event", "turn_id": "turn-1", "item_id": "queued-control", "root_turn_id": null}),
            json!({"event_type": "sofia_dynamic_tool_call_event", "turn_id": "turn-1", "item_id": "queued-code-mode", "root_turn_id": null}),
            json!({"event_type": "sofia_turn_event", "turn_id": "turn-1", "item_id": null, "root_turn_id": null}),
        ]
    );
}
...
async fn completed_background_tool_item_emits_after_turn_event() {
    ...
    assert_eq!(
        out.iter()
            ...
            .collect::<Vec<_>>(),
        vec![
            json!({"event_type": "sofia_turn_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "sofia_command_execution_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "sofia_control_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
            json!({"event_type": "sofia_dynamic_tool_call_event", "turn_id": "turn-2", "root_turn_id": "root-background"}),
        ]
    );
}
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(sofia_app_server_protocol::CodexErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["sofia_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["sofia_error_http_status_code"],
        json!(null)
    );
}
...
async fn rejected_turn_interrupt_does_not_tag_interrupted_turn_event() {
    ...
    assert_eq!(payload["event_params"]["sofia_error_kind"], json!(null));
}
