...
use crate::events::sofia_artifact_operation_event_request;
use crate::events::sofia_compaction_event_params;
use crate::events::sofia_goal_event_params;
use crate::events::sofia_hook_run_metadata;
use crate::events::sofia_plugin_install_requested_metadata;
...
use sofia_app_server_protocol::ClientResponse;
...
use sofia_app_server_protocol::CommandExecutionApprovalKind;
use sofia_app_server_protocol::CommandExecutionSource;
use sofia_app_server_protocol::CommandExecutionStatus;
use sofia_app_server_protocol::DynamicToolCallOutputContentItem;
...
use sofia_app_server_protocol::NetworkPolicyRuleAction;
...
use sofia_app_server_protocol::WebSearchAction;
use sofia_git_utils::SanitizedGitUrl;
use sofia_git_utils::collect_git_info;
...
use sofia_protocol::items::is_safe_plugin_relative_path;
use sofia_protocol::models::PermissionProfile;
use sofia_protocol::protocol::SessionSource;
use sofia_protocol::protocol::SkillScope;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn tool_item(
        notification: &'a sofia_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct TurnState {
    ...
    sofia_error: Option<TurnCodexError>,
...
impl AnalyticsReducer {
                ...
                CustomAnalyticsFact::ThreadHintStatus(input) => {
                    if let Some((connection, thread, metadata)) =
                        self.thread_context_or_warn(AnalyticsDropSite {
                            event_name: "sofia_thread_hint_status",
                            thread_id: &input.thread_id,
                            turn_id: None,
                            review_id: None,
                            item_id: None,
                        })
                    {
                        out.push(TrackEventRequest::ThreadHintStatus(Box::new(
                            crate::thread_hint::ThreadHintStatusEventRequest {
                                event_type: "sofia_thread_hint_status",
                ...
                CustomAnalyticsFact::GuardianV2(input) => {
                    let event_type = match &input.kind {
                        GuardianV2EventKind::Classification { .. } => {
                            "sofia_guardian_v2_classification"
                        }
                        GuardianV2EventKind::FastDecision { .. } => {
                            "sofia_guardian_v2_fast_decision"
                        }
                    };
                    if let Some((connection, thread, metadata)) =
                        self.thread_context_or_warn(AnalyticsDropSite {
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            sofia_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(CodexDynamicToolCallEventRequest {
                    event_type: "sofia_dynamic_tool_call_event",
    ...
    fn ingest_control_tool_call(
        ...
        let event = TrackEventRequest::ControlToolCall(CodexControlToolCallEventRequest {
            event_type: "sofia_control_tool_call_event",
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "sofia_guardian_review",
    ...
    fn ingest_turn_codex_error(&mut self, input: TurnCodexErrorFact) {
        ...
        turn_state.sofia_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = sofia_app_metadata(&tracking, mention);
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = sofia_app_metadata(&tracking, app);
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(CodexHookRunEventRequest {
            ...
            event_params: sofia_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
            ...
            event_params: sofia_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            CodexPluginInstallRequestedEventRequest {
                ...
                event_params: sofia_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
        ...
        out.push(TrackEventRequest::PluginInstallFailed(
            CodexPluginInstallFailedEventRequest {
                ...
                event_params: CodexPluginInstallFailedMetadata {
                    plugin: sofia_plugin_metadata(plugin),
                    source,
                    error_type,
                    sub_error_type,
                },
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.sofia_error_info),
    ...
    fn emit_thread_initialized(
        ...
        let is_worktree =
            sofia_git_utils::repository_identity(thread.cwd.as_path()).and_then(|_| {
                sofia_git_utils::get_git_repo_root(thread.cwd.canonicalize().ok()?.as_path())
    ...
    fn ingest_compaction(&mut self, input: CodexCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            CodexCompactionEventRequest {
                ...
                event_params: sofia_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: CodexGoalEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Goal(Box::new(CodexGoalEventRequest {
            event_type: "sofia_goal_event",
            event_params: sofia_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: sofia_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(CodexTurnSteerEventRequest {
            event_type: "sofia_turn_steer_event",
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(CodexTurnEventRequest {
            ...
            event_params: sofia_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
    ...
    match item {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(CodexFileChangeEventRequest {
                event_type: "sofia_file_change_event",
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                CodexMcpToolCallEventRequest {
                    event_type: "sofia_mcp_tool_call_event",
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                CodexCollabAgentToolCallEventRequest {
                    event_type: "sofia_collab_agent_tool_call_event",
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(CodexWebSearchEventRequest {
                event_type: "sofia_web_search_event",
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                CodexImageGenerationEventRequest {
                    event_type: "sofia_image_generation_event",
...
fn file_change_counts(changes: &[sofia_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn sofia_turn_event_params(
    ...
    let sofia_error = turn_state.sofia_error.as_ref();
    CodexTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        sofia_turn_source: turn_metadata.sofia_turn_source(),
        app_server_client,
        runtime,
        submission_type,
        ephemeral,
        ...
        sofia_error_kind: sofia_error.map(|error| error.kind),
        sofia_error_http_status_code: sofia_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: sofia_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        sofia_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        sofia_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        sofia_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        sofia_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.sofia/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
