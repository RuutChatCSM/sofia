...
use sofia_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(sofia_artifact_operation_event_request(
        TrackEventsContext {
            model_slug: "gpt-5.1-sofia".to_string(),
            thread_id: thread_id.to_string(),
            turn_id: "turn-1".to_string(),
            product_client_id: "sofia_desktop".to_string(),
        },
        ArtifactOperation {
        ...
        event_params: CodexMcpToolCallEventParams {
            base: CodexToolItemEventBase {
                ...
                runtime: CodexRuntimeMetadata {
                    sofia_rs_version: "0.0.0".to_string(),
    ...
    TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
        ...
        event_params: CodexPluginUsedMetadata {
            plugin: CodexPluginMetadata {
                ...
                product_client_id: Some("sofia_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "sofia-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/sofia/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = sofia_login::CodexAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "sofia_accepted_line_fingerprints"
    );
...
async fn api_key_auth_sends_only_plugin_events_to_codex_backend() {
    ...
    let auth_manager = sofia_login::AuthManager::from_auth_for_testing(
        sofia_login::CodexAuth::from_api_key("sk-test"),
    );
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn flush_is_noop_when_analytics_is_disabled() {
    let client = AnalyticsEventsClient::new(
        sofia_login::AuthManager::from_auth_for_testing(
