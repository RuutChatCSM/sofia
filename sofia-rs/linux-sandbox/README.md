...
- a `sofia-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Sofia CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `sofia-exec` CLI can check if its arg0 is `sofia-linux-sandbox` and, if so, execute as if it were `sofia-linux-sandbox`
  - this should also be true of the `sofia` multitool CLI

On Linux, Sofia prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `sofia-resources/bwrap` binary shipped
with Sofia.
Sofia also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Sofia surfaces the same startup warning
...
it cannot create the required user namespaces, so Sofia rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `sofia-resources/bwrap` path.
- If `bwrap` is missing, Sofia also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Sofia surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Sofia rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.sofia`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `sofia sandbox`; the host OS selects the sandbox backend.
