...
//! - sensitive subpaths such as `.git`, `.agents`, and `.sofia` remain
...
use sofia_protocol::permissions::is_protected_metadata_name;
...
use sofia_protocol::protocol::FileSystemSpecialPath;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.sofia`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".sofia")
                {
                    return None;
                }
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_codex = temp_dir.path().join("real-sofia");
        let logical_codex = logical_home.join(".sofia");
        ...
        std::os::unix::fs::symlink(&real_codex, &logical_codex)
            .expect("create symlinked sofia home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".sofia"));
    ...
    fn missing_child_git_under_parent_repo_is_mounted_read_only() {
        ...
        assert_eq!(
            synthetic_mount_target_paths(&args),
            vec![workspace.join(".sofia"), dot_git, workspace.join(".agents")],
        );
        assert!(
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_codex = path_to_string(&temp_dir.path().join(".sofia"));
