...
fn sofia_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_sofia-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> sofia_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<sofia_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let sofia_linux_sandbox_exe = Some(sofia_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &sofia_protocol::exec_output::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<sofia_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> sofia_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sofia_linux_sandbox_exe: Option<PathBuf> = Some(sofia_linux_sandbox_exe());
...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    let dot_codex = tmpdir.path().join(".sofia");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codex).expect("create .sofia");

    let git_target = dot_git.join("config");
    let sofia_target = dot_codex.join("config.toml");
    ...
    let sofia_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", sofia_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".sofia write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(sofia_output.exit_code, 0);
}
...
async fn sandbox_blocks_codex_symlink_replacement_attack() {
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-sofia");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_codex = tmpdir.path().join(".sofia");
    symlink(&decoy, &dot_codex).expect("create .sofia symlink");
    ...
    let sofia_output = expect_denied(
        ...
        .await,
        ".sofia symlink replacement should be denied",
    );
    assert_ne!(sofia_output.exit_code, 0);
}
...
async fn sandbox_reports_codex_symlink_build_failure_without_panicking() {
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".sofia symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".sofia symlink build failure should deny: {output:?}"),
    };
...
async fn sandbox_rejects_symlinked_synthetic_mount_registry() {
    ...
    let registry = temp.path().join(format!(
        "sofia-bwrap-synthetic-mount-targets-{effective_uid}"
    ));
...
registry="${{TMPDIR:-/tmp}}/sofia-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/sofia-bwrap-synthetic-mount-targets-$(id -u)
...
if grep -E '(^|[[:space:]])\.(git|sofia|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_codex_output = expect_denied(
        ...
        .await,
        "child .sofia directory creation should be denied",
    );
    assert_ne!(mkdir_codex_output.exit_code, 0);
    assert!(!subdir.join(".sofia").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = sofia_linux_sandbox_exe()
        .parent()
...
async fn sandbox_starts_with_denied_tmp_without_exposing_registry() {
    ...
    let sandbox_helper = sofia_linux_sandbox_exe();
...
registry="$TMPDIR/sofia-bwrap-synthetic-mount-targets-$(id -u)"
test ! -e "$registry"
"#,
                ],
                cwd.clone(),
