...
Set a stable path to the skill CLI (default `SOFIA_HOME` is `~/.sofia`):

```
export SOFIA_HOME="${SOFIA_HOME:-$HOME/.sofia}"
export IMAGE_GEN="$SOFIA_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/sofia-network.md`
