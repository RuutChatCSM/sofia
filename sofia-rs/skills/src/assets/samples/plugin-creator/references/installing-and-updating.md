...
- the user wants Sofia to see the updated plugin without manually editing marketplace files
...
2. Update the plugin manifest to a single Sofia cachebuster suffix:
...
outside Sofia depends on a specific token:
...
```bash
sofia plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `sofia plugin marketplace add` for that
path, and `sofia plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
sofia plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Sofia picks up new skills
   and tools.
...
```text
<base-version>+sofia.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+sofia.local-20260519-184516`
- `0.1.0+sofia.old-token` → `0.1.0+sofia.local-20260519-184516`
- `1.2.3-beta.1+sofia.prev` → `1.2.3-beta.1+sofia.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+sofia.local-20260519-184516`

Replace the existing Sofia cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `sofia plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `sofia plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Sofia.
...
- Use `sofia plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `sofia plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `sofia plugin add <plugin-name>@<marketplace-name>`.
