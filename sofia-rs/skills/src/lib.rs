...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".sofia-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills from an absolute SOFIA_HOME.
pub fn system_cache_root_dir(sofia_home: &AbsolutePathBuf) -> AbsolutePathBuf {
    sofia_home
        .join(SKILLS_DIR_NAME)
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `SOFIA_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(sofia_home: &AbsolutePathBuf) -> Result<(), SystemSkillsError> {
    let skills_root_dir = sofia_home.join(SKILLS_DIR_NAME);
    fs::create_dir_all(skills_root_dir.as_path())
        .map_err(|source| SystemSkillsError::io("create skills root dir", source))?;

    let dest_system = system_cache_root_dir(sofia_home);
