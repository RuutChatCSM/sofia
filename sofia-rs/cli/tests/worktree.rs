...
async fn rejected_start(
    ...
    let spawned = sofia_utils_pty::spawn_pty_process(
        &program.to_string_lossy(),
        args,
        cwd,
        env,
        /*arg0*/ &None,
        sofia_utils_pty::TerminalSize {
            rows: 40,
            cols: 120,
        },
        &[],
...
async fn interactive_worktree_start_and_fork_bind_owner_before_turn() -> anyhow::Result<()> {
    ...
    let root = sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(root.path())?
        .canonicalize()?
        .into_path_buf();
    ...
    fs::create_dir(source.join(".sofia"))?;
    fs::write(
...
"#,
    ...
    let program = sofia_utils_cargo_bin::cargo_bin("sofia")?;
    let mut env: HashMap<String, String> = std::env::vars().collect();
    env.insert("SOFIA_HOME".into(), home.display().to_string());
    env.insert("SOFIA_SQLITE_HOME".into(), home.display().to_string());
    env.insert("NO_PROXY".into(), "127.0.0.1,localhost".into());
    env.insert("no_proxy".into(), "127.0.0.1,localhost".into());
    env.insert("TERM".into(), "xterm-256color".into());
    env.insert("OTEL_METRIC_EXPORT_INTERVAL".into(), "100".into());
    for key in [
        "SOFIA_EXEC_SERVER_URL",
        "SOFIA_ACCESS_TOKEN",
        "OPENAI_API_KEY",
        "SOFIA_API_KEY",
    ] {
    ...
    for (fork, explicit_cd, analytics, auth_failure) in [
        ...
        if auth_failure {
            ...
            fs::write(source.join(".sofia/config.toml"), "")?;
        }
        ...
        if explicit_cd {
            args.extend(["--cd".into(), source.join(".sofia").display().to_string()]);
        }
        if analytics {
        ...
        let renamed = if !fork && matches!(observed, Ok(Ok(_))) {
            tokio::time::timeout(Duration::from_secs(/*secs*/ 10), async {
                ...
                let thread_id = sofia_protocol::ThreadId::from_string(
                    metadata["ownerThreadId"].as_str().context("owner id")?,
                )?;
                loop {
                    if sofia_rollout::find_thread_name_by_id(&home, &thread_id)
                        .await?
                        .as_deref()
                        == Some("managed-source")
                    {
        ...
        assert!(
            if analytics {
                String::from_utf8_lossy(&metrics).contains("sofia.tui.start")
            } else {
                metrics.is_empty()
            },
        ...
        let context = body["input"]
            ...
            .join("\n");
        if explicit_cd {
            let expected = format!("{checkout}/.sofia").replace('\\', "/");
        ...
        if previous.is_empty() {
            ...
            let sqlite = sofia_state::SqliteConfig::from_sqlite_home(
                sofia_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&home)?,
            );
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust", "name":"distrust",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&sofia_config::loader::project_trust_key(Path::new(&checkout)))?)}]}
            ...
            Mock::given(wiremock::matchers::path("/source/backend-api/wham/config/bundle"))
                .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                    "config_toml": {"enterprise_managed": [{"id":"distrust-source", "name":"distrust-source",
                        "contents": format!("[projects.{}]\ntrust_level = \"untrusted\"\n", serde_json::to_string(&sofia_config::loader::project_trust_key(&source.join(".sofia")))?)}]}
                }))).mount(&server).await;
            let output = rejected_start(
                ...
                &[
                    ...
                    "--no-alt-screen".into(),
                    "--cd".into(),
                    source.join(".sofia").display().to_string(),
                ],
                &launcher,
                &env,
                "cannot create a checkout from an explicitly untrusted source",
    ...
    fs::write(source.join(".sofia/config.toml"), "not = [valid toml")?;
    git(
        &source,
        &[
            "commit",
