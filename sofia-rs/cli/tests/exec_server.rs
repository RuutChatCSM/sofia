...
use sofia_exec_server::EnvironmentInfo;
use sofia_exec_server::ExecParams;
use sofia_exec_server::ExecServerClient;
use sofia_exec_server::NoiseChannelIdentity;
use sofia_exec_server::NoiseChannelPublicKey;
use sofia_exec_server::NoiseRendezvousConnectArgs;
...
"#,
    ...
    let mut cmd = sofia_command(sofia_home.path())?;
    cmd.args([
        "exec-server",
        "--strict-config",
        "--listen",
...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let sofia_home = TempDir::new()?;
    std::fs::write(sofia_home.path().join("config.toml"), "not valid toml = [")?;
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = sofia_command(sofia_home.path())?;
    cmd.env(
        sofia_exec_server::SOFIA_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    let executable = bin_dir.join(format!("sofia{}", std::env::consts::EXE_SUFFIX));
    std::fs::copy(sofia_utils_cargo_bin::cargo_bin("sofia")?, &executable)?;
    let manifest = package.path().join("sofia-package.json");
    std::fs::write(&manifest, r#"{"version":"1.2.3-alpha.4"}"#)?;

    let mut command = tokio::process::Command::new(executable);
    command
        .env("SOFIA_HOME", sofia_home.path())
        .env("SOFIA_API_KEY", "test-api-key")
        .env(
            sofia_exec_server::SOFIA_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined SOFIA_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${SOFIA_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            shell_snapshot: None,
            env_policy: Some(sofia_exec_server::ExecEnvPolicy {
                inherit: sofia_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let sofia_bin = sofia_utils_cargo_bin::cargo_bin("sofia")?;
    let sofia_home = sofia_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(sofia_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let mut child = std::process::Command::new(sofia_utils_cargo_bin::cargo_bin("sofia")?)
        .env("SOFIA_HOME", sofia_home.path())
