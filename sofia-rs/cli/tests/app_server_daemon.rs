...
impl TestDaemon {
    fn new() -> Result<Self> {
        ...
        let sofia_source = std::fs::canonicalize(&sofia)?;
        ...
        std::fs::hard_link(&sofia_source, &managed)
            .or_else(|_| std::fs::copy(&sofia_source, managed).map(|_| ()))?;
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(&self.sofia);
        command.env("SOFIA_HOME", self.home.path());
        command
    }
