...
impl Fixture {
    fn new() -> Result<Self> {
        ...
        let program = root
            .path()
            .join(format!("sofia{}", std::env::consts::EXE_SUFFIX));
        let source = sofia_utils_cargo_bin::cargo_bin("sofia")?;
...
"#,
        ...
        for name in [
            ...
            std::fs::write(
                ...
                "@echo helper ran>>\"%SOFIA_TEST_HELPER_MARKER%\"\r\n@exit /b 0\r\n",
            )?;
        }
    ...
    fn command(&self) -> Result<assert_cmd::Command> {
        ...
        command
            ...
            .env("SOFIA_HOME", &self.home)
            .env("HOME", self.root.path())
            .env("PATH", &self.path)
            .env("SOFIA_TEST_HELPER_MARKER", &self.marker)
            .env("SOFIA_MANAGED_BY_NPM", "1")
            .env(
...
async fn interactive_tmux_startup_does_not_execute_workspace_helpers() -> Result<()> {
    ...
    env.insert(
        "SOFIA_TUI_DISABLE_KEYBOARD_ENHANCEMENT".to_string(),
        "0".to_string(),
    );
    env.insert(
...
async fn feedback_with_logs_does_not_execute_path_helpers() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("SOFIA_TEST_HELPER_MARKER", Some(marker.as_ref())),
            ("SOFIA_MANAGED_BY_NPM", Some("1")),
            ("SOFIA_MANAGED_PACKAGE_ROOT", Some(package_root.as_ref())),
            ("ZELLIJ", Some("0")),
            ("ZELLIJ_VERSION", None),
            ("TMUX", None),
            ("TMUX_PANE", None),
