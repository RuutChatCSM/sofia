...
"#,
        ...
        let output = Command::new(sofia_utils_cargo_bin::cargo_bin("sofia")?)
...
async fn invalid_custom_ca_falls_back_to_system_roots() -> Result<()> {
    ...
    let sofia_home = TempDir::new()?;
    let certificate = sofia_home.path().join("invalid-ca.pem");
    ...
    for sandbox in [None, Some("seatbelt")] {
        ...
        command
            .args(["doctor", "--json"])
            .env("SOFIA_HOME", sofia_home.path())
            .env("SOFIA_CA_CERTIFICATE", &certificate)
            .stdin(Stdio::null());
...
fn doctor_reports_macos_system_proxy_configuration_and_policy() -> Result<()> {
    let sofia_home = TempDir::new()?;
    let report = doctor_report(sofia_home.path())?;
...
#[cfg(target_os = "macos")]
fn doctor_report(sofia_home: &Path) -> Result<Value> {
    let output = Command::new(sofia_utils_cargo_bin::cargo_bin("sofia")?)
        .args(["doctor", "--json"])
