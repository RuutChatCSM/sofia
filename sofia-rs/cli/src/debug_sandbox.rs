...
#[cfg(target_os = "macos")]
use sofia_core::spawn::CODEX_SANDBOX_ENV_VAR;
use sofia_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
use sofia_sandboxing::landlock::allow_network_for_proxy;
use sofia_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use sofia_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use sofia_sandboxing::seatbelt::create_seatbelt_command_args;
use sofia_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<sofia_mcp::SandboxState>)
        .transpose()
    ...
    let sofia_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .sofia_linux_sandbox_exe
                .clone()
                .or(sofia_linux_sandbox_exe)
        }
        None => sofia_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(sofia_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            // for the unsandboxed shell that resumes when Sofia exits.
        ...
        SandboxType::Landlock => {
            ...
            let sofia_linux_sandbox_exe = config
                .sofia_linux_sandbox_exe
                .expect("sofia-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use sofia_windows_sandbox::WindowsSandboxProxySettingsMode;
    use sofia_windows_sandbox::WindowsSandboxSessionRequest;
    use sofia_windows_sandbox::resolve_windows_deny_read_paths;
    use sofia_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        ...
        sofia_home: config.sofia_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = sofia_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn load_debug_sandbox_config_with_codex_home(
    ...
    // For legacy configs, `sofia sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            sofia_linux_sandbox_exe: sofia_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        sofia_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(sofia_home) = sofia_home {
        builder = builder
            .sofia_home(sofia_home.clone())
            .fallback_cwd(Some(sofia_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = sofia_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_codex_home(
            ...
            Some(sofia_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = sofia_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
