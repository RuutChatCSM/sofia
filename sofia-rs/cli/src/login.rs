...
//! interactive-session layers. Direct `sofia login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `sofia-login.log` artifact that
...
use sofia_login::CLIENT_ID;
use sofia_login::ServerOptions;
use sofia_login::is_workload_identity_selected;
use sofia_login::login_with_access_token;
use sofia_login::login_with_api_key;
use sofia_login::logout_with_revoke;
use sofia_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `sofia login` flows.
///
...
/// command produce a durable `sofia-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match sofia_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("sofia-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("sofia_cli=info,sofia_core=info,sofia_login=info"));
    ...
    // Direct `sofia login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `sofia login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | sofia login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv SOFIA_ACCESS_TOKEN | sofia login --with-access-token`.",
...
/// `sofia login` working in environments where device-code may be disabled/feature-gated.
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let sofia_home = tempdir().expect("create temporary Sofia home");
        login_with_api_key(
            sofia_home.path(),
            "sk-existing",
