...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed Sofia app; sofia.exe; sofia-windows-sandbox-setup.exe; sofia-command-runner.exe; sofia-code-mode-host.exe"
    } else if cfg!(target_os = "macos") {
        "signing team 2DC432GLL2 plus the installed Sofia app identity, signed sofia agent, and required helpers"
    } else {
        "verified Sofia app, sofia agent, and required helpers"
    };
    let mut check = DoctorCheck::new(
        "security.endpoint",
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; Sofia exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("Sofia exclusions: not verified")
    .remediation(
        "ask your security administrator to verify Sofia exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            "CrowdStrike Falcon" => {
                "CrowdStrike Falcon: Add a certificate or IOA exclusion for Sofia. If sensor overhead continues, exclude the Sofia agent from sensor visibility. Keep monitoring descendant processes."
            }
            "BeyondTrust Privilege Management" => {
                "BeyondTrust: Remove Sofia from application blocking rules. Add allow rules for Sofia helper executables. Do not grant administrator privileges."
            }
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for Sofia and its helpers. If Attack Surface Reduction blocks Sofia, add a rule exclusion. If Controlled Folder Access blocks Sofia, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for Sofia and its helpers."
            }
            "Jamf Protect" => {
                "Jamf Protect: Add an Override Threat Prevention exception for Sofia app and helper signing identities. If analytics cause delays, add an Ignore System Events for Analytics exception."
            }
            _ => {
                "Add an exclusion for Sofia and its helpers. Use the endpoint product instructions."
            }
        };
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with Sofia. Verify Sofia exclusions."),
            )
            .measured("not verified")
            .expected("Sofia application and helper exclusions")
            .remedy(remedy)
            .field("Sofia exclusions"),
        );
    }
