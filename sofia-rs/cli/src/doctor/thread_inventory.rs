...
use sofia_state::ThreadStateAuditRow;
use sofia_utils_path::normalize_for_path_comparison;
...
async fn thread_inventory_check_for_roots(
    ...
    let scan = scan_rollout_files(sofia_home).await;
    ...
    let rows = match sofia_state::read_thread_state_audit_rows(sqlite).await {
        Ok(rows) => rows,
        Err(err) => {
    ...
    parity_check_from_scan_and_rows(sofia_home, scan, rows, details)
}
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Sofia with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Sofia with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under SOFIA_HOME sessions."),
        );
    }
    check
}
                ...
                let expected_archived = rollout_by_key
                    ...
                    .or_else(|| {
                        (scan
                            ...
                            .contains(&rollout_path_key(&row.rollout_path))
                            || row.rollout_path.is_file())
                        .then(|| archived_from_rollout_path(sofia_home, &row.rollout_path))
                        .flatten()
                    })?;
...
async fn scan_rollout_files(sofia_home: &Path) -> RolloutScan {
    let mut scan = RolloutScan::default();
    scan_rollout_root(
        &sofia_home.join("sessions"),
        /*archived*/ false,
        &mut scan,
    )
    .await;
    scan_rollout_root(
        &sofia_home.join("archived_sessions"),
        /*archived*/ true,
        &mut scan,
    )
    .await;
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = sofia_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match sofia_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    for _ in 0..MAX_ROLLOUT_HEADER_LINES {
        ...
        if item_type == "session_meta" {
            return match sofia_rollout::parse_rollout_line(line.trim()) {
        ...
        if !has_legacy_item {
            has_legacy_item = sofia_rollout::parse_rollout_line(line.trim()).is_ok();
        }
    }
    ...
    let logical_path = sofia_rollout::plain_rollout_path(path);
    sofia_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&sofia_rollout::plain_rollout_path(path))
}

fn archived_from_rollout_path(sofia_home: &Path, path: &Path) -> Option<bool> {
    let key = path_key(path);
    if key.starts_with(path_key(&sofia_home.join("archived_sessions"))) {
        return Some(true);
    }
    if key.starts_with(path_key(&sofia_home.join("sessions"))) {
        return Some(false);
    }
    None
}
...
mod tests {
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart Sofia"))
        }));
    ...
    async fn thread_inventory_check_uses_metadata_id_when_filename_and_db_disagree() {
        ...
        let mut rollout_line =
            sofia_rollout::parse_rollout_line(contents.trim()).expect("rollout line");
    ...
    async fn thread_inventory_check_uses_compressed_metadata_id_and_legacy_filename_fallback() {
        ...
        let legacy_path = fixture.sofia_home.path().join(format!(
    ...
    async fn thread_inventory_check_ignores_compression_temp_files() {
        let fixture = Fixture::new().await;
        let temp_path = fixture.sofia_home.path().join(
    ...
    impl Fixture {
        async fn new() -> Self {
            let sofia_home = TempDir::new().expect("sofia home");
            let sqlite_home = TempDir::new().expect("sqlite home");
            let _runtime = sofia_state::StateRuntime::init(
        ...
        fn sqlite(&self) -> sofia_state::SqliteConfig {
            sofia_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
        }

        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            let root = if archived {
                self.sofia_home.path().join("archived_sessions")
            } else {
                self.sofia_home.path().join("sessions/2025/01/02")
            };
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(sofia_protocol::protocol::SessionMetaLine {
                    meta: sofia_protocol::protocol::SessionMeta {
                        session_id: parsed_thread_id.into(),
                        id: parsed_thread_id,
                        timestamp: timestamp.to_string(),
                        cwd: self.sofia_home.path().to_path_buf(),
...
INSERT INTO threads (
    ...
    archived_at
            ...
            .bind("cli")
            .bind("test-provider")
            .bind(self.sofia_home.path().display().to_string())
            .bind("test title")
            .bind("read-only")
            .bind("on-request")
