...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    let name = *parts.last()?;
    let package = parts.iter().any(|part| part.starts_with("ruut.sofia_"));
    let trusted = package || parts.windows(2).any(|pair| pair == ["ruut", "sofia"]);
    match name {
        "sofia-windows-sandbox-setup.exe" => Some("sandbox_setup"),
        "sofia-command-runner.exe" => Some("command_runner"),
        "sofia.exe" => Some("sofia"),
        "sofia-desktop.exe" => Some("sofia_desktop"),
        "chatgpt.exe" | "electron.exe" if trusted => Some("sofia_desktop"),
        "rg.exe" if trusted => Some("ripgrep"),
        _ if package => Some("sofia_desktop_package"),
        _ => None,
    }
}
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    let summary = match (status, visible) {
        ...
        (CheckStatus::Ok, _) => "no locally visible recent Sofia security enforcement was found",
        (CheckStatus::Warning, false) => "security event channels could not be inspected",
        (CheckStatus::Warning, true) => "recent Sofia security audit or detection requires review",
        (CheckStatus::Fail, _) => "endpoint security blocked or quarantined a Sofia executable",
    };
    let mut check = desktop_check("desktop.security.enforcement", status, summary).details(details);
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved Sofia application policy",
        );
    }
    check
}
