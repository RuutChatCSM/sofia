...
#[cfg(target_os = "macos")]
use sofia_http_client::MacosSystemProxyConfiguration;
use sofia_http_client::RouteAwareClientPool;
use sofia_http_client::RouteAwareRequestError;
use sofia_http_client::RouteFailureClass;
#[cfg(target_os = "macos")]
use sofia_http_client::macos_system_proxy_configuration;
...
pub(super) fn check(config: Option<&Config>) -> DoctorCheck {
    ...
    for name in ["SOFIA_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
pub(super) fn with_system_proxy_remediation(
        ...
        })
    {
        check.remediation = Some(
            "A macOS system proxy is configured but unused. If your organization requires it, ask your administrator whether to enable the under-development feature with `sofia features enable respect_system_proxy`."
                .to_string(),
        );
    }
    check
