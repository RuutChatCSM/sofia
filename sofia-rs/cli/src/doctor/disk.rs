...
pub(super) fn check(config: Option<&Config>, cwd: &Path) -> DoctorCheck {
    let home = config
        .map(|config| config.sofia_home.as_path().to_path_buf())
        .or_else(|| find_codex_home().ok().map(Into::into))
        .or_else(|| std::env::var_os("SOFIA_HOME").map(Into::into));
...
fn check_with_paths(
    ...
    for (label, path) in [("SOFIA_HOME", home.as_deref()), ("worktree", Some(cwd))] {
