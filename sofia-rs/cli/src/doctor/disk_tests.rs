...
fn disk_capacity_is_reported_without_loaded_configuration() {
    ...
    for label in ["SOFIA_HOME available: ", "worktree available: "] {
        assert!(
            check
                .details
                .iter()
...
fn disk_capacity_thresholds_produce_complete_diagnostics() {
    for (available, status, measured, summary) in [
        ...
        let issues = ["SOFIA_HOME", "worktree"]
            .into_iter()
            .filter(|_| status != "ok")
            .map(|label| {
                json!({
        ...
        assert_eq!(
            ...
            json!({
                ...
                "details": [
                    ...
                    format!("SOFIA_HOME available: {measured}"),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "details": [
                "warning threshold: 5.0 GiB",
                "failure threshold: 1.0 GiB",
                "SOFIA_HOME available: unavailable (PermissionDenied)",
