...
use sofia_core::config::load_global_mcp_servers;
...
use sofia_mcp::ResolvedMcpOAuthScopes;
use sofia_mcp::apply_http_headers_helper;
use sofia_mcp::compute_auth_statuses;
...
use sofia_rmcp_client::delete_oauth_tokens;
use sofia_rmcp_client::perform_oauth_login;
use sofia_rmcp_client::resolve_mcp_oauth_callback_url;
use sofia_utils_cli::CliConfigOverrides;
use sofia_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.sofia/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "sofia mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: sofia_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: sofia_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&sofia_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", sofia_home.display()))?;
    ...
    ConfigEditsBuilder::new(&sofia_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", sofia_home.display()))?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `sofia mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `sofia mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: sofia mcp remove {}", get_args.name);
