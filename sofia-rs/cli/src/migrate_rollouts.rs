...
use sofia_thread_store::RolloutMigrationMode;
use sofia_thread_store::RolloutMigrationOptions;
use sofia_thread_store::RolloutMigrationProgress;
use sofia_thread_store::RolloutMigrationReport;
use sofia_thread_store::RolloutMigrationStatus;
...
pub(crate) async fn run(
    ...
    sofia_core::otel_init::record_process_start(otel.as_ref(), "sofia_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            sofia_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `sofia migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &sofia_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(sofia_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    let mut bytes = 0_u64;
    let mut directories = vec![
        sofia_home.join(sofia_rollout::SESSIONS_SUBDIR),
        sofia_home.join(sofia_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];
