//! Implements the `sofia doctor` diagnostic report.
//!
...
use sofia_api::ResponsesWebsocketClient;
use sofia_api::is_azure_responses_provider;
...
use sofia_install_context::CodexPackageLayout;
use sofia_install_context::InstallContext;
use sofia_install_context::InstallMethod;
use sofia_install_context::StandalonePlatform;
use sofia_login::AuthDotJson;
use sofia_login::AuthManager;
use sofia_login::CodexAuth;
use sofia_login::OPENAI_API_KEY_ENV_VAR;
use sofia_login::SOFIA_ACCESS_TOKEN_ENV_VAR;
use sofia_login::SOFIA_API_KEY_ENV_VAR;
use sofia_login::default_client::create_client_without_request_logging;
use sofia_login::default_client::default_headers;
use sofia_login::load_auth_dot_json;
use sofia_model_provider::create_model_provider;
use sofia_protocol::auth::AuthMode;
use sofia_protocol::protocol::AskForApproval;
use sofia_terminal_detection::Multiplexer;
use sofia_terminal_detection::TerminalInfo;
use sofia_terminal_detection::TerminalName;
use sofia_terminal_detection::terminal_info;
...
/// Options for building a local Sofia diagnostic report.
///
...
///
/// This is the CLI entry point for sofia doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("auth", progress.clone(), || match &auth_manager_result {
                        ...
                        .remediation(
                            "Fix the reported authentication error, then rerun sofia doctor.",
                        ),
                    })
                },
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun sofia doctor.")
                    })
                },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        sofia_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    .build()
    .await
    .context("failed to load Sofia config")
}
...
/// JSON support report emitted by `sofia doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        sofia_version: report.sofia_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("SOFIA_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by Vite+: {}",
        env::var_os("SOFIA_MANAGED_BY_VITE_PLUS").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("SOFIA_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "SOFIA_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = sofia_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH sofia entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH sofia #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        ...
        if env::var_os("SOFIA_MANAGED_PACKAGE_ROOT").is_none() {
            status = status.max(CheckStatus::Warning);
            summary = "npm-managed launch is missing package-root provenance".to_string();
            remediation = Some(
                "Reinstall or update Sofia so the JS shim provides SOFIA_MANAGED_PACKAGE_ROOT."
                    .to_string(),
            );
        }
    }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("SOFIA_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("SOFIA_MANAGED_BY_NPM").is_none()
        && env::var_os("SOFIA_MANAGED_BY_BUN").is_none()
        && env::var_os("SOFIA_MANAGED_BY_VITE_PLUS").is_none()
        && env::var_os("SOFIA_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn sofia_path_entries() -> Vec<String> {
    let Ok(candidates) = which::which_all("sofia") else {
        return Vec::new();
    };
...
fn config_check(config: &Config) -> DoctorCheck {
    ...
    details.push(format!("SOFIA_HOME: {}", config.sofia_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.sofia_home.join(sofia_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.sofia_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        SOFIA_API_KEY_ENV_VAR,
        SOFIA_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run sofia login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Sofia credentials were found",
        )
        .details(details)
        .remediation("Run sofia login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run sofia login again."),
    }
}
...
fn stored_auth_mode(auth: &sofia_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(SOFIA_API_KEY_ENV_VAR);
...
async fn state_check(config: &Config, command: &DoctorCommand) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "SOFIA_HOME", &config.sofia_home);
    ...
    rollout_stats_details(&mut details, &config.sofia_home);
...
async fn sqlite_integrity_detail(
    sqlite: &sofia_state::SqliteConfig,
    details: &mut Vec<String>,
    label: &str,
    path: &Path,
    ...
    let (rows, timed_out) = match sofia_state::sqlite_integrity_check(sqlite, path, deadline).await
    {
        Ok(sofia_state::SqliteIntegrityCheck::Complete(rows)) => (rows, false),
        Ok(sofia_state::SqliteIntegrityCheck::TimedOut(rows)) => (rows, true),
        Err(err) => {
...
fn rollout_stats_details(details: &mut Vec<String>, sofia_home: &Path) {
    let active = collect_rollout_stats(&sofia_home.join("sessions"));
    let archived = collect_rollout_stats(&sofia_home.join("archived_sessions"));
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match sofia_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "SOFIA_HOME was resolved without config",
        )
        .detail(format!("SOFIA_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "SOFIA_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if provider_base_url.is_some_and(|url| !url.trim().is_empty())
        ...
        || env_var_present(SOFIA_API_KEY_ENV_VAR)
    {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(SOFIA_ACCESS_TOKEN_ENV_VAR) {
...
fn provider_reachability_plan_from_parts(
    ...
    let endpoints = match (mode, provider_base_url) {
        ...
        (ProviderAuthReachabilityMode::Chatgpt, None) => vec![ReachabilityEndpoint {
            ...
            url: provider_url_for_path(chatgpt_base_url, "sofia/responses", provider_query_params),
            required: true,
...
fn credential_store_has(name: &str) -> bool {
    // Use the canonical Sofia home from the shared utility crate.
    let sofia_home = sofia_utils_home_dir::sofia_home_string();
    let mut candidates = vec![std::path::PathBuf::from(&sofia_home)];
...
mod tests {
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].sofia_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            sofia_self_exe: Some(PathBuf::from("/bin/sofia")),
            sofia_linux_sandbox_exe: Some(PathBuf::from("/bin/sofia-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/sofia-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.sofia_self_exe, arg0_paths.sofia_self_exe);
        assert_eq!(
            overrides.sofia_linux_sandbox_exe,
            arg0_paths.sofia_linux_sandbox_exe
        );
        assert_eq!(
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "SOFIA_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("SOFIA_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Sofia."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Sofia.".to_string())
        );
    }
    ...
    fn provider_reachability_mode_uses_api_key_auth() {
        ...
        assert_eq!(
            provider_auth_reachability_mode_from_auth(
                ...
                /*provider_env_key*/ None,
                /*provider_base_url*/ None,
                |name| name == SOFIA_API_KEY_ENV_VAR,
                /*stored_auth*/ None,
            ),
            ProviderAuthReachabilityMode::ApiKey
        );
    ...
    fn provider_reachability_chatgpt_uses_inference_endpoint() {
        assert_eq!(
            ...
            vec![ReachabilityEndpoint {
                label: "ChatGPT".to_string(),
                url: "https://chatgpt.com/backend-api/sofia/responses".to_string(),
                required: true,
                route_probe_url: None,
            }]
        );
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-sofia-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-sofia-doctor-mcp\" is not resolvable",
            )
        }));
    }
