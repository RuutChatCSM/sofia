...
use sofia_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use sofia_core_plugins::marketplace::marketplace_root_dir;
use sofia_core_plugins::marketplace_add::MarketplaceAddOutcome;
use sofia_core_plugins::marketplace_add::MarketplaceAddRequest;
use sofia_core_plugins::marketplace_add::add_marketplace;
use sofia_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use sofia_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "sofia plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Sofia is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin marketplace add",
    after_help = "Examples:\n  sofia plugin marketplace add ./path/to/marketplace\n  sofia plugin marketplace add owner/repo --ref main\n  sofia plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "sofia plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin marketplace upgrade",
    after_help = "Examples:\n  sofia plugin marketplace upgrade\n  sofia plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin marketplace remove",
    after_help = "Example:\n  sofia plugin marketplace remove debug"
)]
...
async fn run_list(config: Config, args: ListMarketplaceArgs) -> Result<()> {
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.sofia_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<sofia_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, sofia_home);
...
async fn run_upgrade(
    ...
    builder: sofia_core::config::ConfigBuilder,
) -> Result<()> {
    ...
    let reload_config: sofia_core_plugins::ConfigLayerReload = std::sync::Arc::new(move || {
        runtime
