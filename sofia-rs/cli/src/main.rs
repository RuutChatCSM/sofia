...
use sofia_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use sofia_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use sofia_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use sofia_arg0::Arg0DispatchPaths;
use sofia_arg0::arg0_dispatch_or_else;
use sofia_chatgpt::apply_command::ApplyCommand;
use sofia_chatgpt::apply_command::run_apply_command;
use sofia_cli::read_access_token_from_stdin;
use sofia_cli::read_api_key_from_stdin;
use sofia_cli::run_login_status;
use sofia_cli::run_login_with_access_token;
use sofia_cli::run_login_with_api_key;
use sofia_cli::run_login_with_chatgpt;
use sofia_cli::run_login_with_device_code;
use sofia_cli::run_logout;
use sofia_cloud_config::cloud_config_bundle_loader_for_storage;
use sofia_cloud_tasks::Cli as CloudTasksCli;
use sofia_exec::Cli as ExecCli;
use sofia_exec::Command as ExecCommand;
use sofia_exec::ReviewArgs;
use sofia_exec_server::ExecServerRuntimePaths;
use sofia_execpolicy::ExecPolicyCheckCommand;
use sofia_http_client::HttpClientFactory;
use sofia_http_client::OutboundProxyPolicy;
use sofia_responses_api_proxy::Args as ResponsesApiProxyArgs;
use sofia_rollout_trace::REDUCED_STATE_FILE_NAME;
use sofia_rollout_trace::replay_bundle;
use sofia_state::StateRuntime;
use sofia_tui::AppExitInfo;
use sofia_tui::Cli as TuiCli;
use sofia_tui::ExitReason;
use sofia_tui::UpdateAction;
use sofia_utils_absolute_path::AbsolutePathBuf;
use sofia_utils_cli::CliConfigOverrides;
use sofia_utils_cli::ProfileV2Name;
use sofia_utils_cli::SharedCliOptions;
...
use sofia_core::build_models_manager;
use sofia_core::config::Config;
use sofia_core::config::ConfigBuilder;
use sofia_core::config::ConfigLoadOptions;
...
use sofia_core::config::resolve_profile_v2_config_path;
use sofia_features::FEATURES;
use sofia_features::Stage;
use sofia_features::is_known_feature_key;
use sofia_home::CodexHomeUserInstructionsProvider;
use sofia_login::AuthManager;
use sofia_login::CodexAuth;
use sofia_login::is_workload_identity_selected;
use sofia_login::read_codex_access_token_from_env;
use sofia_memories_write::clear_memory_roots_contents;
use sofia_models_manager::bundled_models_response;
use sofia_models_manager::manager::RefreshStrategy;
use sofia_protocol::protocol::AskForApproval;
use sofia_protocol::user_input::UserInput;
use sofia_terminal_detection::TerminalName;

/// Sofia CLI — a local multi-model agent based on Sofia.
///
...
#[clap(
    ...
    // `sofia-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `sofia` command name that users run.
    bin_name = "sofia",
    display_name = "sofia",
    override_usage = "sofia [OPTIONS] [PROMPT]\n       sofia [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Sofia.
    Mcp(McpCli),

    /// Manage Sofia plugins.
    Plugin(PluginCli),
    ...
    /// Update Sofia to the latest version.
    Update,

    /// Diagnose local Sofia installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Sofia-provided sandbox.
    ...
    /// Apply the latest diff produced by Sofia agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Sofia Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Sofia.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = sofia_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = sofia_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = sofia_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $SOFIA_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | sofia login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv SOFIA_ACCESS_TOKEN | sofia login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: sofia_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = sofia_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: sofia_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/sofia/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: sofia_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: sofia_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from SOFIA_ACCESS_TOKEN for remote registration.
    #[arg(
    ...
    #[arg(
        ...
        env = sofia_exec_server::SOFIA_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Sofia tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Sofia via `{cmd_str}`...");
    let status = {
        #[cfg(windows)]
        {
    ...
    println!("\n🎉 Update ran successfully! Please restart Sofia.");
    Ok(())
}
...
fn resolve_windows_update_command_from_path(
    ...
    if path_env.is_empty() {
        anyhow::bail!(
            "Could not find an absolute update command `{command}` on PATH. Please update manually: https://developers.openai.com/sofia/cli/"
        );
    }
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`sofia update` is not available in debug builds. Install a release build of Sofia to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = sofia_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Sofia installation method. Please update manually: https://developers.openai.com/sofia/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: sofia_tui::SessionArchiveAction,
    ...
    sofia_tui::run_session_archive_command(
        action,
        target,
        sofia_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<sofia_tui::SessionArchiveAction> {
    if force && sofia_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => sofia_tui::DeleteConfirmation::Skip,
        false => sofia_tui::DeleteConfirmation::Prompt,
    };
    Ok(sofia_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let sofia_bin = std::env::current_exe()?;
            sofia_app_server_test_client::send_message_v2(&sofia_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    sofia_build_info::initialize!();
    let remote_control_disabled = sofia_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`sofia agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!("`sofia agents` does not accept an initial prompt or images");
                }
                ...
                {
                    anyhow::bail!(
                        "`sofia agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`sofia agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    anyhow::bail!("`sofia agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            sofia_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["sofia", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        sofia_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = sofia_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                sofia_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                sofia_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                sofia_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    sofia_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        ...
                        analytics_default_enabled,
                        transport,
                        sofia_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            sofia_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::Update => {
                        let output = sofia_app_server_daemon::update().await?;
                        println!("{}", serde_json::to_string(&output)?);
                    }
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        sofia_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let sofia_home = find_codex_home()?;
                            sofia_app_server::app_server_control_socket_path(&sofia_home)?
                        }
                    };
                    sofia_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = sofia_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    sofia_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    sofia_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    sofia_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                sofia_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                sofia_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | sofia login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            sofia_cloud_tasks::run_main(cloud_cli, arg0_paths.sofia_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            sofia_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.sofia_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            sofia_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.sofia_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            sofia_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.sofia_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`sofia sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || sofia_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `sofia mcp`: `sofia`, `sofia exec`, `sofia review`, `sofia resume`, `sofia queue`, `sofia archive`, `sofia delete`, `sofia unarchive`, `sofia fork`, `sofia mcp`, `sofia sandbox`, and `sofia debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let sofia_self_exe = arg0_paths
        .sofia_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Sofia executable path is not configured"))?;
    let runtime_paths =
        ExecServerRuntimePaths::new(sofia_self_exe, arg0_paths.sofia_linux_sandbox_exe.clone())?;
    if let Some(base_url) = cmd.remote.take() {
        ...
        let auth_provider = if cmd.aws_sigv4 {
            exec_server_auth::aws_sigv4_auth_provider(sofia_aws_auth::AwsAuthConfig {
        ...
        let mut remote_config = sofia_exec_server::RemoteEnvironmentConfig::new_with_transport(
            base_url,
            environment_id,
            if direct_transport {
                sofia_exec_server::RemoteEnvironmentTransport::Direct
            } else {
                sofia_exec_server::RemoteEnvironmentTransport::Noise
            },
            auth_provider,
        ...
        let runtime_paths = runtime_paths.with_allowed_symlinked_codex_home(
            sofia_config::allowed_symlinked_codex_home(
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        sofia_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
                    ...
                    None => {
                        sofia_exec_server::run_remote_environment_until_shutdown(
                            remote_config,
                            runtime_paths,
                            shutdown,
                        )
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| sofia_exec_server::DEFAULT_LISTEN_URL.to_string());
        let run = exec_server_telemetry::run_until_shutdown(
            sofia_exec_server::run_main_with_telemetry(
                &listen_url,
                runtime_paths,
                telemetry,
                http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &sofia_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<sofia_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        read_codex_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("SOFIA_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        ...
        if !matches!(auth, CodexAuth::AgentIdentity(_)) {
            anyhow::bail!(
                "SOFIA_ACCESS_TOKEN did not provide permitted Agent Identity authentication"
            );
        }
        return Ok(sofia_model_provider::auth_provider_from_auth(&auth));
    }

    let (auth_manager, auth) = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `sofia login` or set SOFIA_API_KEY",
    )
    .await?;
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(sofia_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(sofia_model_provider::auth_provider_from_auth(&auth))
    }
}
...
async fn load_exec_server_config(
...
) -> anyhow::Result<sofia_core::config::Config> {
    ...
    if enable_workload_identity && is_workload_identity_selected() {
        ...
        .await?;
        let bootstrap_auth_config = bootstrap_auth_config(&sofia_home, &bootstrap_config)?;
...
) -> anyhow::Result<(Arc<AuthManager>, sofia_login::CodexAuth)> {
    let auth_manager =
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let sofia_home = find_codex_home()?;
    ConfigEditsBuilder::new(&sofia_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&sofia_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile_at_codex_home(
    profile_v2: Option<&ProfileV2Name>,
    sofia_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(sofia_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(sofia_home: &std::path::Path, feature: &str) {
    ...
    let config_path = sofia_home.join(sofia_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(sofia_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        sofia_self_exe: arg0_paths.sofia_self_exe,
        sofia_linux_sandbox_exe: arg0_paths.sofia_linux_sandbox_exe,
    ...
    let user_instructions_provider = Arc::new(CodexHomeUserInstructionsProvider::new(
        config.sofia_home.clone(),
    ));
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_codex_api_key_env*/ false).await?;
    let mut extensions = sofia_extension_api::ExtensionRegistryBuilder::new();
    sofia_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    sofia_skills_extension::install(&mut extensions, |config: &Config| {
        sofia_skills_extension::SkillsExtensionConfig {
            ...
            shadow_selection_enabled: config
                .features
                .enabled(sofia_features::Feature::SkillSearch),
        }
    });
    let prompt_input = sofia_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.sofia_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `sofia {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `sofia {subcommand}`"
        );
    }
    Ok(())
}
...
fn reject_unsupported_worktree_for_subcommand(
    ...
    match subcommand {
        ...
        Some(Subcommand::Fork(_)) => {
            anyhow::bail!("`sofia fork --worktree` requires an explicit session ID")
        }
        Some(Subcommand::Exec(command)) => match &command.command {
            None | Some(ExecCommand::Fork(_)) => Ok(()),
            Some(ExecCommand::Resume(_)) => anyhow::bail!(
                "`--worktree` cannot resume an existing session; use `sofia exec fork --worktree`"
            ),
        ...
        _ => {
            anyhow::bail!(
                "`--worktree` supports new interactive sessions, `sofia fork`, `sofia exec`, and `sofia exec fork`"
            )
        }
    }
}
...
/// `sofia --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `sofia {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = sofia_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<sofia_core::config::Config>,
) -> sofia_http_client::HttpClientFactory {
    match config {
        Ok(config) => config.http_client_factory(),
        Err(error) => {
            eprintln!("warning: failed to load updater network configuration: {error}");
            sofia_http_client::HttpClientFactory::new(
                sofia_http_client::OutboundProxyPolicy::ReqwestDefault,
            )
        }
    }
}
...
async fn print_app_server_remote_control_output(
    ...
    let output = sofia_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = sofia_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Sofia's interactive TUI may not work in this terminal."
        );
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        sofia_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(std::io::Error::other)?;
    }
    ...
    let start_tui = || {
        sofia_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            sofia_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Sofia local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<sofia_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(sofia_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !sofia_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let sofia_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `sofia resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `sofia` without additional flags.
...
/// Build the final `TuiCli` for a `sofia fork` invocation.
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let sofia_home = tempfile::tempdir().expect("temporary Sofia home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            sofia_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn profile_loader_overrides_use_explicit_codex_home() -> anyhow::Result<()> {
        ...
        let overrides =
            loader_overrides_for_profile_at_codex_home(Some(&profile), sofia_home.path());

        assert_eq!(
            overrides.user_config_path,
            Some(resolve_profile_v2_config_path(sofia_home.path(), &profile))
        );
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["sofia", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["sofia", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["sofia", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["sofia", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["sofia", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["sofia", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["sofia", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn worktree_flag_supports_interactive_exec_and_explicit_fork_positions() {
        let arguments = [
            vec!["sofia", "--worktree"],
            vec!["sofia", "--worktree", "hello"],
            vec!["sofia", "--worktree", "exec", "hello"],
            vec!["sofia", "exec", "--worktree", "hello"],
            vec![
                "sofia",
                "fork",
                "--worktree",
    ...
    fn worktree_flag_rejects_unsupported_session_and_management_commands() {
        let arguments = [
            vec!["sofia", "--worktree", "login"],
            vec!["sofia", "fork", "--worktree"],
            vec!["sofia", "fork", "--worktree", "--last"],
            vec!["sofia", "exec", "resume", "--worktree", "session"],
            vec!["sofia", "exec", "review", "--worktree"],
            vec!["sofia", "resume", "--worktree", "session"],
            vec!["sofia", "archive", "session", "--worktree"],
            vec![
                "sofia",
                "queue",
                "--thread",
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(sofia_exec::Command::Resume(args)) = exec.command else {
    ...
    fn approve_for_me_configures_interactive_mode() {
        for flag in ["--approve-for-me", "--not-so-yolo"] {
            let mut cli = MultitoolCli::try_parse_from(["sofia", flag]).expect("parse flag");
    ...
    fn not_so_yolo_alias_is_hidden_from_help() {
        for args in [&["sofia", "--help"][..], &["sofia", "exec", "--help"][..]] {
    ...
    fn approve_for_me_defaults_propagate_from_root_to_exec() {
        let exec = finalize_exec_from_args(&["sofia", "--approve-for-me", "exec", "summarize"]);
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(sofia_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(sofia_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn approve_for_me_conflicts_with_explicit_interactive_permissions() {
        for conflicting_args in [
            ...
            let mut args = vec!["sofia", "--approve-for-me"];
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let sofia_home = find_codex_home().expect("sofia home");
        sofia_app_server::app_server_control_socket_path(&sofia_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["sofia", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: sofia plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: sofia plugin marketplace add"),
            ("list", "Usage: sofia plugin marketplace list"),
            ("upgrade", "Usage: sofia plugin marketplace upgrade"),
            ("remove", "Usage: sofia plugin marketplace remove"),
        ] {
            let help = help_from_args(&["sofia", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["sofia", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["sofia", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["sofia", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["sofia", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["sofia", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["sofia", "marketplace", "remove", "debug"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: thread_id.map(|thread_id| sofia_tui::ResumableThread {
                thread_id,
    ...
    async fn format_exit_messages_preserves_auth_env_through_tui_runner() {
        let exit_info = run_tui_with_recovery(
            || async {
                ...
                exit_info.disconnect_info = Some(sofia_tui::DisconnectInfo {
                    command: vec![
                        "sofia".to_string(),
                        "--remote".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Reconnect: sofia --remote wss://example.com:443/ --remote-auth-token-env SOFIA_REMOTE_TOKEN resume 123e4567-e89b-12d3-a456-426614174000",
                "Stop the current turn: run sofia --remote wss://example.com:443/ --remote-auth-token-env SOFIA_REMOTE_TOKEN agents, select this task, and press ctrl + x.",
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "  sofia resume 123e4567-e89b-12d3-a456-426614174000".to_string(),
            ]
        );
    }
        ...
        insta::allow_duplicates! {
            for thread_name in [None, Some("")] {
                ...
                To continue this session, run:
                  sofia resume 123e4567-e89b-12d3-a456-426614174000
                ");
            }
        }
    }
    ...
    fn format_exit_messages_applies_color_when_enabled() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2",
                "To continue this session, run:",
                "  \u{1b}[36msofia resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        To continue this session, run:
          sofia resume 123e4567-e89b-12d3-a456-426614174000
        Or run sofia resume and select my-thread.
        ");
    }
    ...
    fn format_exit_messages_colors_commands_and_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run:",
                "  \u{1b}[36msofia resume 123e4567-e89b-12d3-a456-426614174000\u{1b}[39m",
                "Or run \u{1b}[36msofia resume\u{1b}[39m and select \u{1b}[36mmy-thread\u{1b}[39m.",
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["sofia", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_and_fork_preserve_no_alt_screen() {
        for (command, finalize) in [
            ...
            assert!(finalize(&["sofia", command, "--no-alt-screen"]).no_alt_screen);
            assert!(finalize(&["sofia", "--no-alt-screen", command]).no_alt_screen);
            assert!(!finalize(&["sofia", command]).no_alt_screen);
        }
    }

    #[test]
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["sofia", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["sofia", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["sofia", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["sofia", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["sofia", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["sofia", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["sofia", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["sofia", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(sofia_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(sofia_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["sofia", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["sofia", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["sofia", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["sofia", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["sofia", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["sofia", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["sofia", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["sofia", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["sofia", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["sofia", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["sofia", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["sofia", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli =
            MultitoolCli::try_parse_from(["sofia", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["sofia", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn exec_server_forward_requires_registration_and_destination() {
        for args in [
            ...
            assert!(
                MultitoolCli::try_parse_from(["sofia", "exec-server"].into_iter().chain(args))
                    .is_err()
            );
        }
    }
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["sofia", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["sofia", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `sofia mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["sofia", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `sofia remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["sofia", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `sofia app-server proxy`"
        );
    }

    #[test]
    fn reject_remote_flag_for_remote_control() {
        let cli = MultitoolCli::try_parse_from(["sofia", "--remote", "unix://", "remote-control"])
            .expect("parse");
    ...
    fn remote_control_pair_parses() {
        let cli = MultitoolCli::try_parse_from(["sofia", "remote-control", "pair"]).expect("parse");
        let Some(Subcommand::RemoteControl(remote_control)) = &cli.subcommand else {
            panic!("expected remote-control subcommand");
        };
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["sofia", "--remote", "unix://sofia.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://sofia.sock"));
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["sofia", "resume", "--remote", "unix://sofia.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://sofia.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("SOFIA_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("SOFIA_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_rejects_invalid_code_mode_host_urls() {
        for endpoint in [
            ...
            let error =
                MultitoolCli::try_parse_from(["sofia", "app-server", "--code-mode-host", endpoint])
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["sofia", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            sofia_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["sofia", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            sofia_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["sofia", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["sofia", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            sofia_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["sofia", "app-server", "--listen", "unix:///tmp/sofia.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            sofia_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/sofia.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["sofia", "app-server", "--listen", "off"].as_ref());
        assert_eq!(app_server.listen, sofia_app_server::AppServerTransport::Off);
    }

    #[test]
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["sofia", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["sofia", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["sofia", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["sofia", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["sofia", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["sofia", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["sofia", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["sofia", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["sofia", "app-server", "proxy", "--sock", "sofia.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("sofia.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(sofia_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/sofia-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(sofia_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/sofia-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["sofia", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["sofia", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("sofia")
