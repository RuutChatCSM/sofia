...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Sofia after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Sofia user.
    #[arg(
        long = "current-user",
    ...
    /// SOFIA_HOME for the Sofia user. Required with --user.
    #[arg(long = "sofia-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`sofia sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .fallback_cwd(Some(identity.sofia_home.clone()))
        ...
        .build()
        .await
        .context("failed to load target user's Sofia config for sandbox provisioning")?;

    sofia_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.sofia_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.sofia_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let sofia_home = match cmd.sofia_home.clone() {
            Some(sofia_home) => sofia_home,
    ...
    let sofia_home = cmd
        .sofia_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--sofia-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.sofia_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.sofia"))
        );
    }
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--sofia-home".to_string(),
            r"C:\Users\alice\.sofia".to_string(),
        ])
        .expect("parse")
