...
use sofia_state::LogRow;
use sofia_state::SqliteConfig;
use sofia_state::StateRuntime;
use sofia_utils_absolute_path::AbsolutePathBuf;

#[derive(Debug, Parser)]
#[command(name = "sofia-state-logs")]
#[command(about = "Tail Sofia logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to SOFIA_HOME. Defaults to $SOFIA_HOME or ~/.sofia.
    #[arg(long, env = "SOFIA_HOME")]
    sofia_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --sofia-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(sofia_home) = args.sofia_home.as_ref() {
        config_builder = config_builder.sofia_home(sofia_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["sofia-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["sofia-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["sofia-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["sofia-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Sofia home.
    #[tokio::test]
    async fn direct_db_skips_codex_home_config() {
        let sofia_home = tempfile::tempdir().expect("create Sofia home");
        std::fs::write(sofia_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("sofia-state-logs"),
            OsString::from("--sofia-home"),
            sofia_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
