...
use sofia_core_plugins::ConfiguredMarketplace;
use sofia_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use sofia_core_plugins::PluginInstallOutcome;
...
use sofia_core_plugins::allowed_configured_marketplace_names;
use sofia_core_plugins::installed_marketplaces::marketplace_install_root;
use sofia_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use sofia_core_plugins::marketplace::MarketplaceListError;
use sofia_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use sofia_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use sofia_core_plugins::marketplace::MarketplacePluginSource;
use sofia_core_plugins::marketplace::find_marketplace_manifest_path;
use sofia_core_plugins::remote;
...
use sofia_core_plugins::remote::RemotePluginSummary;
use sofia_login::AuthManager;
use sofia_login::CodexAuth;
use sofia_plugin::PluginId;
use sofia_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin add",
    after_help = "Examples:\n  sofia plugin add sample@debug\n  sofia plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin list",
    after_help = "Examples:\n  sofia plugin list\n  sofia plugin list --marketplace debug\n  sofia plugin list --json\n  sofia plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "sofia plugin remove",
    after_help = "Examples:\n  sofia plugin remove sample@debug\n  sofia plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources = configured_marketplace_sources(&plugins_input, sofia_home.as_path());
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, sofia_home);
...
async fn fetch_remote_marketplaces(
    ...
    let result = if context.plugins_input.remote_plugin_enabled {
        remote::fetch_remote_marketplaces(
            ...
            /*catalog_cache_root*/ Some(context.sofia_home.as_path()),
            cache_mode,
        )
        .await
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(sofia_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, sofia_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
