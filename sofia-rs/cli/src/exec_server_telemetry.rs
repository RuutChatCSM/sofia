...
const OTEL_SERVICE_NAME: &str = "sofia-exec-server";
...
pub(crate) fn init(
    config: Option<&sofia_core::config::Config>,
) -> (impl Send + Sync, sofia_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => sofia_core::otel_init::build_provider(
            config,
    ...
    sofia_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(sofia_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();
