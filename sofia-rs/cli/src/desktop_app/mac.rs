...
const SOFIA_BUNDLE_IDENTIFIER: &str = "com.openai.sofia";
const OPENAI_APPLE_TEAM_IDENTIFIER: &str = "2DC432GLL2";
const SOFIA_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/sofia-app-prod/Sofia.dmg";
const SOFIA_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/sofia-app-prod/Sofia-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_codex_app_path(&sofia_app_search_dirs()) {
        eprintln!(
...
fn find_existing_codex_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Sofia.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_codex_app_bundle(candidate))
}

fn sofia_app_search_dirs() -> Vec<PathBuf> {
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == SOFIA_BUNDLE_IDENTIFIER
        })
}
...
async fn open_codex_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    ...
    let url = sofia_new_thread_url(workspace);
...
async fn verify_codex_app_bundle(app_path: &Path) -> anyhow::Result<()> {
    let requirement = format!(
        "identifier \"{SOFIA_BUNDLE_IDENTIFIER}\" and anchor apple generic and certificate leaf[subject.OU] = \"{OPENAI_APPLE_TEAM_IDENTIFIER}\""
    );
...
async fn download_and_install_codex_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    ...
    let result = async {
        let app_in_volume = find_codex_app_in_mount(&mount_point)
            .context("failed to locate Sofia.app in mounted dmg")?;
...
async fn install_codex_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("Sofia.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Sofia.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Sofia.app to any applications directory");
}
...
fn find_codex_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Sofia.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    use super::sofia_new_thread_url;
    ...
    fn finds_chatgpt_app_with_codex_bundle_identifier() {
        ...
        write_app_bundle(&app_path, "com.openai.sofia");
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let sofia_app_path = temp_dir.path().join("Sofia.app");
        write_app_bundle(&sofia_app_path, "com.openai.sofia");
    ...
    async fn rejects_unsigned_app_with_codex_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("Sofia.app");
        write_app_bundle(&app_path, "com.openai.sofia");
    ...
    async fn rejects_valid_signature_without_openai_signing_identity() {
        ...
        let app_path = temp_dir.path().join("Sofia.app");
        let contents_path = app_path.join("Contents");
        let executable_path = contents_path.join("MacOS/Sofia");
        ...
        fs::write(
            contents_path.join("Info.plist"),
            r#"<?xml version="1.0"?><plist version="1.0"><dict><key>CFBundleIdentifier</key><string>com.openai.sofia</string><key>CFBundleExecutable</key><string>Sofia</string></dict></plist>"#,
        )
        .expect("write Info.plist");
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex\t/Volumes/Sofia\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex Installer\t/Volumes/Sofia Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Sofia Installer")
        );
    }

    #[test]
    fn sofia_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&sofia_new_thread_url(Path::new("/tmp/sofia workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/sofia workspace/#1".to_string())],
            )
        );
    }
}
