...
pub fn app_server_control_socket_path(sofia_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub fn app_server_startup_lock_path(sofia_home: &Path) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        sofia_home
            .join(APP_SERVER_CONTROL_SOCKET_DIR_NAME)
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let sofia_home = find_codex_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve SOFIA_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&sofia_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),
