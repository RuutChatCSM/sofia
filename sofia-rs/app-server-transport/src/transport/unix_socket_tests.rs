...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/sofia.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/sofia.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://sofia.sock"),
        Ok(AppServerTransport::UnixSocket {
...
fn default_control_socket_path() -> AbsolutePathBuf {
    ...
    app_server_control_socket_path(&sofia_home).expect("default control socket path")
}
