...
pub async fn start_control_socket_acceptor(
    ...
    let (socket_path, directory_guard) = {
        if let Some(parent) = socket_path.as_path().parent() {
            sofia_uds::prepare_private_socket_directory(parent).await?;
        }
        let (path, guard) = sofia_uds::validate_private_socket_path(socket_path.as_path())?;
...
pub async fn prepare_control_socket_path(socket_path: &Path) -> IoResult<()> {
    ...
    #[cfg(windows)]
    let (socket_path, _directory_guard) = sofia_uds::validate_private_socket_path(socket_path)?;
    #[cfg(windows)]
    ...
    if !sofia_uds::is_stale_socket_path(socket_path).await? {
        return Err(std::io::Error::new(
