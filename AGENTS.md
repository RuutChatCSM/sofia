...
In the sofia-rs folder where the rust code lives:

- Crate names are prefixed with `sofia-`. For example, the `core` folder's crate is named `sofia-core`
...
- Do not add general product or user-facing documentation to the `docs/` folder. The official Sofia documentation lives elsewhere. The exception is app-server API documentation, which is covered by the app-server guidance below.
- Prefer private modules and explicitly exported public crate API.
- If you change `ConfigToml` or nested config types, run `just write-config-schema` to update `sofia-rs/core/config.schema.json`.
- When working with MCP tool calls, prefer using `sofia-rs/sofia-mcp/src/mcp_connection_manager.rs` to handle mutation of tools and tool calls. Aim to minimize the footprint of changes and leverage existing abstractions rather than plumbing code through multiple levels of function calls.
...
- Avoid large modules:
  ...
  - This rule applies especially to high-touch files that already attract unrelated changes, such
    as `sofia-rs/tui/src/app.rs`, `sofia-rs/tui/src/bottom_pane/chat_composer.rs`,
    `sofia-rs/tui/src/bottom_pane/footer.rs`, `sofia-rs/tui/src/chatwidget.rs`,
    `sofia-rs/tui/src/bottom_pane/mod.rs`, and similarly central orchestration modules.
  - When extracting code from a large module, move the related tests and module/type docs toward
    the new implementation so the invariants stay close to the code that owns them.
  - Avoid adding new standalone methods to `sofia-rs/tui/src/chatwidget.rs` unless the change is
    trivial; prefer new modules/files and keep `chatwidget.rs` focused on orchestration.
- When running Rust commands (e.g. `just fix` or `just test`) be patient with the command and never try to kill them using the PID. Rust lock can make the execution slow, this is expected.

Run `just fmt` (in the `sofia-rs` directory) automatically after you have finished making code changes anywhere in this repository; do not ask for approval to run it. Additionally, run the tests:
...
Over time, the `sofia-core` crate (defined in `sofia-rs/core/`) has become bloated because it is the largest crate, so it is often easier to add something new to `sofia-core` rather than refactor out the library code you need so your new code neither takes a dependency on, nor contributes to the size of, `sofia-core`.

To that end: **resist adding code to sofia-core**!

Particularly when introducing a new concept/feature/API, before adding to `sofia-core`, consider whether:

- There is an existing crate other than `sofia-core` that is an appropriate place for your new code to live.
- It is time to introduce a new crate to the Cargo workspace for your new functionality. Refactor existing code as necessary to make this happen.

Likewise, when reviewing code, do not hesitate to push back on PRs that would unnecessarily add code to `sofia-core`.
...
Sofia maintains a context (history of messages) that is sent to the model in inference requests.
...
For agent changes prefer integration tests over unit tests. Integration tests are under `core/suite` and use `test_codex` to set up a test instance of sofia.
...
This repo uses snapshot tests (via `insta`), especially in `sofia-rs/tui`, to validate rendered output.
...
- Run tests to generate any updated snapshots:
  - `just test -p sofia-tui`
- Check what’s pending:
  - `cargo insta pending-snapshots -p sofia-tui`
- Review changes by reading the generated `*.snap.new` files directly in the repo, or preview a specific file:
  - `cargo insta show -p sofia-tui path/to/file.snap.new`
- Only if you intend to accept all new snapshots in this crate, run:
  - `cargo insta accept -p sofia-tui`
...
- Prefer `sofia_utils_cargo_bin::cargo_bin("...")` over `assert_cmd::Command::cargo_bin(...)` or `escargot` when tests need to spawn first-party binaries.
  - Under Bazel, binaries and resources may live under runfiles; use `sofia_utils_cargo_bin::cargo_bin` to resolve absolute paths that remain stable after `chdir`.
- When locating fixture files or test resources under Bazel, avoid `env!("CARGO_MANIFEST_DIR")`. Prefer `sofia_utils_cargo_bin::find_resource!` so paths resolve correctly under both Cargo and Bazel runfiles.

### Integration tests

#### sofia_core integration testing

- Prefer the utilities in `core_test_support::responses` when writing end-to-end Sofia tests.
...
- Typical pattern:
  ...
  sofia.submit(Op::UserTurn { ... }).await?;
...
These guidelines apply to app-server protocol work in `sofia-rs`, especially:
...
- Validate with `just test -p sofia-app-server-protocol`.
...
Sofia supports running connected app-server and exec-server on different operating systems. See the
