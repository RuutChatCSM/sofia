...
describe("CodexExec", () => {
  it("rejects when exit happens before stdout closes", async () => {
    ...
    const exec = new CodexExec("sofia");
    ...
    if (result.status === "rejected") {
      ...
      expect(result.error.message).toMatch(/Sofia Exec exited/);
    }
  });
  ...
  it.each(configOverrideCases)(
    ...
    async ({ config, configOverrides, expectedOverrides }) => {
      ...
      const exec = new CodexExec("sofia", undefined, config, configOverrides);
      for await (const _ of exec.run({ input: "hi" })) {
        // no-op
      }
  ...
  it("lets SDK-managed and thread settings override raw configuration when resuming", async () => {
    ...
    const exec = new CodexExec("sofia", undefined, { approval_policy: "never" }, [
      'approval_policy="on-failure"',
  ...
  it("allows overriding the env passed to the Sofia CLI", async () => {
    const { CodexExec } = await import("../src/exec");
    spawnMock.mockClear();
    const child = new FakeChildProcess();
    spawnMock.mockReturnValue(child as unknown as child_process.ChildProcess);
    ...
    process.env.SOFIA_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new CodexExec("sofia", {
        SOFIA_HOME: "/tmp/sofia-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.SOFIA_HOME).toBe("/tmp/sofia-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.SOFIA_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.SOFIA_API_KEY).toBe("test");
      expect(spawnEnv.SOFIA_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.SOFIA_ENV_SHOULD_NOT_LEAK;
    }
  });

  it("resolves the package-layout binary and PATH directory", async () => {
    const { resolveNativePackage } = await import("../src/exec");
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "sofia-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "bin");
    const pathDir = path.join(packageRoot, "sofia-path");
    mkdirSync(binDir, { recursive: true });
    mkdirSync(pathDir, { recursive: true });
    writeFileSync(path.join(packageRoot, "sofia-package.json"), "{}");
    writeFileSync(path.join(binDir, "sofia"), "");

    expect(resolveNativePackage(vendorRoot, "x86_64-unknown-linux-musl", "sofia")).toEqual({
      executablePath: path.join(binDir, "sofia"),
      pathDirs: [pathDir],
    });
  });
  ...
  it("falls back to the legacy binary layout", async () => {
    ...
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "sofia-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "sofia");
  ...
  it("prepends package PATH entries without duplicating them", async () => {
    ...
    const pathDir = path.join(tmpdir(), "sofia-path");
