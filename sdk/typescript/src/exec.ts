...
const INTERNAL_ORIGINATOR_ENV = "SOFIA_INTERNAL_ORIGINATOR_OVERRIDE";
const TYPESCRIPT_SDK_ORIGINATOR = "sofia_sdk_ts";
const SOFIA_NPM_NAME = "@openai/codex";
  ...
  async *run(args: CodexExecArgs): AsyncGenerator<string> {
    ...
    try {
      ...
      if (code !== 0 || signal) {
        ...
        throw new Error(`Sofia Exec exited with ${detail}: ${stderrBuffer.toString("utf8")}`);
      }
    } finally {
      rl.close();
...
function flattenConfigOverrides(
  ...
  if (!isPlainObject(value)) {
    if (prefix) {
      ...
      throw new Error("Sofia config overrides must be a plain object");
    }
  }
  ...
  for (const [key, child] of entries) {
    if (!key) {
      throw new Error("Sofia config override keys must be non-empty strings");
    }
    if (child === undefined) {
      continue;
    }
...
function toTomlValue(value: CodexConfigValue, path: string): string {
  if (typeof value === "string") {
    ...
    if (!Number.isFinite(value)) {
      throw new Error(`Sofia config override at ${path} must be a finite number`);
    }
    return `${value}`;
    ...
    throw new Error(`Sofia config override at ${path} cannot be null`);
  } else {
    const typeName = typeof value;
    throw new Error(`Unsupported Sofia config override value at ${path}: ${typeName}`);
  }
}
...
function findCodexPath(): CodexPathResolution {
  ...
  try {
    const codexPackageJsonPath = moduleRequire.resolve(`${SOFIA_NPM_NAME}/package.json`);
    const codexRequire = createRequire(codexPackageJsonPath);
    const platformPackageJsonPath = codexRequire.resolve(`${platformPackage}/package.json`);
    vendorRoot = path.join(path.dirname(platformPackageJsonPath), "vendor");
  } catch {
    throw new Error(
      `Unable to locate Sofia CLI binaries. Ensure ${SOFIA_NPM_NAME} is installed with optional dependencies.`,
    );
  }

  const codexBinaryName = process.platform === "win32" ? "sofia.exe" : "sofia";
  const nativePackage = resolveNativePackage(vendorRoot, targetTriple, codexBinaryName);
  if (!nativePackage) {
    throw new Error(
      `Unable to locate Sofia CLI binaries for ${targetTriple}. Ensure ${SOFIA_NPM_NAME} is installed with optional dependencies.`,
    );
  }
...
export function resolveNativePackage(
  ...
  if (isFile(packageBinaryPath) && isFile(path.join(packageRoot, "sofia-package.json"))) {
    return {
      executablePath: packageBinaryPath,
      pathDirs: existingDirs(path.join(packageRoot, "sofia-path")),
    };
  }

  const legacyBinaryPath = path.join(packageRoot, "sofia", codexBinaryName);
