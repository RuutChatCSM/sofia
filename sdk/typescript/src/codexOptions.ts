...
export type CodexOptions = {
  ...
  apiKey?: string;
  /**
   * Additional `--config key=value` overrides to pass to the Sofia CLI.
   *
  ...
  /**
   * Raw `--config key=value` overrides to pass unchanged to the Sofia CLI after
   * structured configuration and before SDK-managed or thread-specific overrides.
   */
  configOverrides?: string[];
  /**
   * Environment variables passed to the Sofia CLI process. When provided, the SDK
