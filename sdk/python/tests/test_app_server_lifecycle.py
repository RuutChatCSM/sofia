...
from openai_codex import AsyncCodex, Sofia
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncCodex(config=harness.app_server_config()) as sofia:
            thread = await sofia.thread_start()
            server = sofia.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": sofia.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            active_thread = sofia.thread_start()
            archived_thread = sofia.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            sofia.thread_archive(archived_thread.id)
            active_list = sofia.thread_list(archived=False)
            archived_list = sofia.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as sofia:
                ...
                resumed = await sofia.thread_resume(thread.id)
                forked = await sofia.thread_fork(thread.id)
                archive_response = await sofia.thread_archive(thread.id)
                unarchived = await sofia.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            ...
            forked = sofia.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            ...
            archived = sofia.thread_archive(thread.id)
            unarchived = sofia.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with Sofia(config=harness.app_server_config()) as sofia:
            models = sofia.models(include_hidden=True)
