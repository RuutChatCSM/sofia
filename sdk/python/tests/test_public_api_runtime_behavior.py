...
def test_async_codex_init_failure_closes_client() -> None:
    async def scenario() -> None:
        ...
        sofia._client.start = fake_start  # type: ignore[method-assign]
        sofia._client.initialize = fake_initialize  # type: ignore[method-assign]
        sofia._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await sofia.models()

        assert close_calls == 1
        assert sofia._initialized is False
        assert sofia._init is None
...
def test_async_codex_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "sofia-cli/1.2.3",
                    "serverInfo": {"name": "sofia-cli", "version": "1.2.3"},
                }
            )
        ...
        sofia._client.start = fake_start  # type: ignore[method-assign]
        sofia._client.initialize = fake_initialize  # type: ignore[method-assign]
        sofia._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(sofia.models(), sofia.models())
