...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            thread = sofia.thread_start()
            chunks = list(sofia._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as sofia:
                thread = await sofia.thread_start()
                stream = sofia._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(sofia.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            first_thread = sofia.thread_start()
            second_thread = sofia.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as sofia:
                first_thread = await sofia.thread_start()
                second_thread = await sofia.thread_start()
