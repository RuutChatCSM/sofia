...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Sofia(config=harness.app_server_config()) as sofia:
            ...
            state, turn_id = sofia._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: sofia._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: sofia._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = sofia._client._router._goal_operations.copy()  # noqa: SLF001
