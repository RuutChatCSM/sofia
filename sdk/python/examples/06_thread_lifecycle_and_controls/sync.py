...
with Sofia(config=runtime_config()) as sofia:
    ...
    reopened = sofia.thread_resume(thread.id)
    listing_active = sofia.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = sofia.thread_archive(reopened.id)
    listing_archived = sofia.thread_list(limit=20, archived=True)
    unarchived = sofia.thread_unarchive(reopened.id)

    resumed = sofia.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = sofia.thread_fork(unarchived.id, model="gpt-5.4")
