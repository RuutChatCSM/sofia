...
class Sofia:
    """Synchronous client for creating threads and running Sofia turns.
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate Sofia with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current Sofia account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current Sofia account session."""
        self._client.account_logout()

    # BEGIN GENERATED: Sofia.flat_methods
    def thread_start(
        self,
        *,
