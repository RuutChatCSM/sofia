...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Resolve a Python runtime package version to its Sofia release tag."
    )
...
def resolve_python_runtime_release(python_version: str) -> tuple[str, str]:
    ...
    return python_version, sofia_release_tag(python_version)


def sofia_release_tag(version: str) -> str:
    return f"rust-v{sofia_release_version(version)}"


def sofia_release_version(version: str) -> str:
...
def normalize_codex_version(version: str) -> str:
    ...
    if _NORMALIZED_CODEX_VERSION_PATTERN.fullmatch(normalized) is None:
        raise RuntimeError(f"Could not normalize Sofia version {version!r} to a PEP 440 version")
    return normalized
