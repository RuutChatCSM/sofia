...
Public surface of `openai_codex` for Sofia workflows.
...
- Public Sofia protocol value and event types live in `openai_codex.types`

## Sofia (sync)

```python
Sofia(config: CodexConfig | None = None)
```
...
When `sandbox=` is omitted, Sofia uses its configured default. A sandbox
...
- one `Sofia` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public Sofia protocol models wherever possible:
