...
`openai-sofia` publishes stable releases. Install the latest one with
`pip install openai-sofia`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding Sofia CLI release. Each SDK release
...
If your app is not already async, stay with `Sofia`.
...
When `sandbox=` is omitted, Sofia uses its configured default. A turn
...
`Sofia()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-sofia-cli-bin` dependency is missing
- local `sofia_bin` override points to a missing file
- a custom local Sofia executable does not support the SDK operation being used
