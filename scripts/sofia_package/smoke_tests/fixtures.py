...
class SmokePackage:
    ...
    def from_archives(
        ...
        config_dir = directory / "sofia-config"
        config_dir.mkdir()
        ...
        # Isolate package configuration and state from the user's Sofia setup.
        environment["SOFIA_HOME"] = str(config_dir)
        ...
        for index, archive_path in enumerate((cli_archive, app_server_archive)):
            ...
            manifest = json.loads((extracted / "sofia-package.json").read_text())
...
def package(
    ...
    directory = tmp_path_factory.mktemp(f"sofia-package-smoke-{compression}")
...
def responses_server(package: SmokePackage) -> Iterator[MockResponsesServer]:
    config_path = Path(package.environment["SOFIA_HOME"]) / "config.toml"
...
def code_mode_host_debug_symbols(
    ...
    binaries = {"sofia", "sofia-app-server", "sofia-code-mode-host"}
    if "windows" in target:
        binaries.update({"sofia-command-runner", "sofia-windows-sandbox-setup"})
    ...
    destination = tmp_path_factory.mktemp("sofia-debug-symbols")
    found: set[str] = set()
    symbol_path = None
        ...
        for member in archive:
            ...
            for binary, marker in markers.items():
                ...
                if not matches:
                    continue
                found.add(binary)
                if binary == "sofia-code-mode-host":
