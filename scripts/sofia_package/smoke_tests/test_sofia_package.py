...
├── bin
│   ├── sofia[.exe]                       # CLI package only
│   ├── sofia-app-server[.exe]            # app-server package only
...
Each package contains one entrypoint, not both sofia and sofia-app-server.
"""
...
from openai_codex import ApprovalMode, Sofia, CodexConfig, Sandbox
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(("completion", "bash"), "sofia", id="completion"),
    ],
)
...
@pytest.mark.parametrize("entrypoint", ["sofia", "sofia-app-server"])
def test_app_server_runs_code_mode_through_python_sdk(
    ...
    with Sofia(config=config) as client:
...
def _code_mode_host_symbol_address(
    ...
    *arguments: str,
    ...
    return next(
        ...
        and parts[1].lower() == "t"
        and "sofia_code_mode_host" in parts[2]
    )
...
def test_linux_debug_symbols_resolve_packaged_code(
    ...
    assert "sofia_code_mode_host" in resolved, resolved
...
def test_windows_debug_symbols_resolve_packaged_code(
    ...
    symbols = code_mode_host_debug_symbols.rename(
        code_mode_host_debug_symbols.with_name("sofia_code_mode_host.pdb")
    )
    host = symbols.with_suffix(".exe")
    shutil.copy2(package.cli.with_name("sofia-code-mode-host.exe"), host)
