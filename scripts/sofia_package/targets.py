...
_repo_root = os.environ.get("SOFIA_REPO_ROOT")
if _repo_root is None:
    raise RuntimeError(
        "SOFIA_REPO_ROOT must point to the repository root; "
        "run `just assemble-sofia-package` to set it automatically"
    )
...
PACKAGE_VARIANTS: dict[str, PackageVariant] = {
    "sofia": PackageVariant(
        name="sofia",
        cargo_bin="sofia",
        executable_stem="sofia",
    ),
    "sofia-app-server": PackageVariant(
        name="sofia-app-server",
        cargo_bin="sofia-app-server",
        executable_stem="sofia-app-server",
    ),
}
