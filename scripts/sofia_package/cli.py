"""Command-line interface for building Sofia package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Sofia package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        help="Semantic version to record in sofia-package.json.",
    )
    parser.add_argument(
        "--package-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt sofia-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/sofia_package/sofia-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        "--sofia-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows sofia-command-runner.exe executable. "
            "If omitted for Windows targets, sofia-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--sofia-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows sofia-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "sofia-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="sofia-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        sofia_command_runner_bin=resolve_optional_input_path(
            args.sofia_command_runner_bin,
            "prebuilt Windows sofia-command-runner.exe executable",
            "--sofia-command-runner-bin",
        ),
        sofia_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.sofia_windows_sandbox_setup_bin,
            "prebuilt Windows sofia-windows-sandbox-setup.exe executable",
            "--sofia-windows-sandbox-setup-bin",
        ),
    )
    inputs = PackageInputs(
        ...
        sofia_command_runner_bin=source_outputs.sofia_command_runner_bin,
        sofia_windows_sandbox_setup_bin=source_outputs.sofia_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Sofia package archive at {archive_path}")

    print(f"Built Sofia package directory at {package_dir}")
    return 0
