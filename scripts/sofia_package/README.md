# Sofia package builder

This package contains the implementation behind `scripts/build_sofia_package.py`.
...
```bash
just assemble-sofia-package --help
just assemble-sofia-package --variant sofia-app-server
just assemble-sofia-package --target x86_64-unknown-linux-gnu
```

The builder creates a canonical Sofia package directory:
...
│   ├── sofia-command-runner.exe          # Windows only
│   └── sofia-windows-sandbox-setup.exe   # Windows only
└── sofia-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Sofia release
...
`sofia` and `sofia-app-server`. The `--package-version` flag sets the version in
`sofia-package.json`; it defaults to `[workspace.package].version` in
`sofia-rs/Cargo.toml`.
...
- all targets: `sofia-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `sofia-command-runner` and `sofia-windows-sandbox-setup`,
...
`--sofia-command-runner-bin` plus `--sofia-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Sofia-built V8 release pair before invoking
...
DotSlash manifest at `scripts/sofia_package/rg`. Downloaded archives are cached
under `$TMPDIR/sofia-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/sofia_package/sofia-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/sofia-package/<target>-zsh` and installed at
`sofia-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed
