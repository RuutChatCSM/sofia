"""Cargo builds for source-built Sofia package artifacts."""
...
SOFIA_RS_ROOT = REPO_ROOT / "sofia-rs"
...
class SourceBuildOutputs:
    ...
    bwrap_bin: Path | None
    sofia_command_runner_bin: Path | None
    sofia_windows_sandbox_setup_bin: Path | None
...
def build_source_binaries(
    ...
    *,
    ...
    validate_prebuilt_resource_inputs(
        spec,
        bwrap_bin=bwrap_bin,
        sofia_command_runner_bin=sofia_command_runner_bin,
        sofia_windows_sandbox_setup_bin=sofia_windows_sandbox_setup_bin,
    )
    binaries = source_binaries_for_target(
        ...
        build_codex_command_runner=spec.is_windows and sofia_command_runner_bin is None,
        build_codex_windows_sandbox_setup=spec.is_windows
        and sofia_windows_sandbox_setup_bin is None,
    )
    if binaries:
        cmd = [
            cargo,
        ...
        if entrypoint_bin is None or code_mode_host_bin is None:
            sofia_v8_env = resolve_codex_v8_cargo_env(spec)
            if sofia_v8_env:
                cargo_env = {**os.environ, **sofia_v8_env}
    ...
    outputs = SourceBuildOutputs(
        ...
        code_mode_host_bin=(
            ...
            else output_dir / f"sofia-code-mode-host{spec.exe_suffix}"
        ),
        bwrap_bin=resolve_output_path(
            bwrap_bin,
            output_dir / "bwrap" if spec.is_linux else None,
        ),
        sofia_command_runner_bin=resolve_output_path(
            sofia_command_runner_bin,
            output_dir / "sofia-command-runner.exe" if spec.is_windows else None,
        ),
        sofia_windows_sandbox_setup_bin=resolve_output_path(
            sofia_windows_sandbox_setup_bin,
            output_dir / "sofia-windows-sandbox-setup.exe" if spec.is_windows else None,
        ),
    )
...
def source_binaries_for_target(
    ...
    *,
    ...
    if build_code_mode_host:
        binaries.append("sofia-code-mode-host")
    if build_bwrap:
        binaries.append("bwrap")
    if build_codex_command_runner:
        binaries.append("sofia-command-runner")
    if build_codex_windows_sandbox_setup:
        binaries.append("sofia-windows-sandbox-setup")
    return binaries
...
def validate_prebuilt_resource_inputs(
    ...
    *,
    ...
    if sofia_command_runner_bin is not None and not spec.is_windows:
        raise RuntimeError(
            "--sofia-command-runner-bin is only supported for Windows targets."
        )
    if sofia_windows_sandbox_setup_bin is not None and not spec.is_windows:
        raise RuntimeError(
            "--sofia-windows-sandbox-setup-bin is only supported for Windows targets."
        )
...
def cargo_target_dir() -> Path:
    ...
    if target_dir is None:
        return SOFIA_RS_ROOT / "target"

    path = Path(target_dir)
    if path.is_absolute():
        return path

    return SOFIA_RS_ROOT / path
...
def validate_source_outputs(outputs: SourceBuildOutputs) -> None:
    for path in [
        ...
        outputs.bwrap_bin,
        outputs.sofia_command_runner_bin,
        outputs.sofia_windows_sandbox_setup_bin,
    ]:
