...
from sofia_package.cargo import build_source_binaries
from sofia_package.cargo import source_binaries_for_target
from sofia_package.targets import PACKAGE_VARIANTS
...
class SourceBinariesForTargetTest(unittest.TestCase):
    ...
    def test_missing_windows_helpers_are_built(self) -> None:
        self.assertEqual(
            ...
            ["sofia-command-runner", "sofia-windows-sandbox-setup"],
        )
    ...
    def test_missing_code_mode_host_is_built_for_app_server(self) -> None:
        self.assertEqual(
            source_binaries_for_target(
                TARGET_SPECS["aarch64-apple-darwin"],
                PACKAGE_VARIANTS["sofia-app-server"],
                build_entrypoint=False,
    ...
    def test_build_uses_prebuilt_windows_helpers_without_running_cargo(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            entrypoint = touch_file(root / "sofia.exe")
            code_mode_host = touch_file(root / "sofia-code-mode-host.exe")
            command_runner = touch_file(root / "sofia-command-runner.exe")
            sandbox_setup = touch_file(root / "sofia-windows-sandbox-setup.exe")
            ...
            outputs = build_source_binaries(
                ...
                bwrap_bin=None,
                sofia_command_runner_bin=command_runner,
                sofia_windows_sandbox_setup_bin=sandbox_setup,
            )

        self.assertEqual(outputs.entrypoint_bin, entrypoint)
        self.assertEqual(outputs.code_mode_host_bin, code_mode_host)
        self.assertEqual(outputs.sofia_command_runner_bin, command_runner)
        self.assertEqual(outputs.sofia_windows_sandbox_setup_bin, sandbox_setup)
