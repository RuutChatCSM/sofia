...
from sofia_package.layout import build_package_dir
from sofia_package.layout import validate_package_dir
from sofia_package.targets import PACKAGE_VARIANTS
from sofia_package.targets import PackageInputs
...
class PackageLayoutTest(unittest.TestCase):
    def test_macos_package_preserves_prebuilt_resource_binaries(self) -> None:
        for variant_name in ("sofia", "sofia-app-server"):
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        inputs = PackageInputs(
                            ...
                            code_mode_host_bin=touch_executable(
                                root / "sofia-code-mode-host"
                            ),
                            rg_bin=rg_bin,
                            zsh_bin=zsh_bin,
                            bwrap_bin=None,
                            sofia_command_runner_bin=None,
                            sofia_windows_sandbox_setup_bin=None,
                        )
                        ...
                        self.assertEqual(
                            {
                                "rg": (package_dir / "sofia-path" / "rg").read_bytes(),
                                "zsh": (
                                    package_dir
                                    / "sofia-resources"
                                    / "zsh"
    ...
    def test_app_server_package_places_code_mode_host_beside_entrypoint(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            package_dir.mkdir()
            inputs = PackageInputs(
                entrypoint_bin=touch_executable(root / "sofia-app-server"),
                code_mode_host_bin=touch_executable(root / "sofia-code-mode-host"),
            ...
            self.assertTrue((package_dir / "bin" / "sofia-code-mode-host").is_file())
