...
class ResolveZshBinTest(unittest.TestCase):
    def test_uses_prebuilt_executable_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("sofia_package.zsh.fetch_dotslash_executable") as fetch:
    ...
    def test_uses_manifest_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive = root / "sofia-zsh.tar.gz"
            source = root / "zsh"
            source.write_bytes(b"standalone zsh")
            with tarfile.open(archive, "w:gz") as tar:
                tar.add(source, arcname="sofia-zsh/bin/zsh")

            manifest = root / "sofia-zsh"
            manifest.write_text(
                json.dumps(
                    {
                        "platforms": {
            ...
            with patch(
                "sofia_package.dotslash.default_cache_root",
