"""Sofia-built V8 artifact overrides for package Cargo builds."""
...
def resolved_v8_crate_version() -> str:
    ...
    cargo_lock = tomllib.loads((REPO_ROOT / "sofia-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def default_cache_root() -> Path:
    return Path(tempfile.gettempdir()) / "sofia-package"
...
def ensure_valid_artifact(artifact: Path, checksum: str, url: str) -> None:
    ...
    artifact.unlink(missing_ok=True)
    raise RuntimeError(
        f"Sofia-built V8 artifact {artifact} failed checksum validation."
    )
