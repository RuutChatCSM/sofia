"""Canonical Sofia package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "sofia-resources"
    path_dir = package_dir / "sofia-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"sofia-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.sofia_command_runner_bin is not None:
        copy_executable(
            inputs.sofia_command_runner_bin,
            resources_dir / "sofia-command-runner.exe",
            is_windows=True,
        )

    if inputs.sofia_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.sofia_windows_sandbox_setup_bin,
            resources_dir / "sofia-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    write_json(package_dir / "sofia-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "sofia-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: sofia-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"sofia-code-mode-host{spec.exe_suffix}",
        Path("sofia-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("sofia-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("sofia-resources") / "bwrap")
        executable_files.append(Path("sofia-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("sofia-resources") / "sofia-command-runner.exe",
                Path("sofia-resources") / "sofia-windows-sandbox-setup.exe",
            ]
        )
