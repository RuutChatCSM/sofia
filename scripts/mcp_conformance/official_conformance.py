...
def _make_adapter_launcher(
    ...
    *,
    ...
    if sys.platform == "win32":
        ...
        launcher.write_text(
            "@echo off\n"
            f'set "SOFIA_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH={automatic_auth}"\n'
            f"{adapter_command} %*\n",
            encoding="utf-8",
        )
    else:
        ...
        launcher.write_text(
            "#!/bin/sh\n"
            f"export SOFIA_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH={automatic_auth}\n"
...
def _load_adapter_report(report_path: Path) -> tuple[bool, str]:
    try:
        ...
        return False, f"Sofia adapter did not write a valid report: {exc}"
    if not isinstance(decoded, dict):
        return False, "Sofia adapter report was not an object"
    ...
    if success:
        return True, "Sofia adapter completed the scenario"
    if failed_steps:
        return False, redact_sensitive_text("; ".join(failed_steps))[-4_000:]
    return False, redact_sensitive_text(
        str(decoded.get("error") or "Sofia adapter failed")
    )[-4_000:]
...
def _run_scenario(
    *,
    ...
    adapter_report = scenario_dir / "sofia-adapter.json"
    adapter_home = scenario_dir / "sofia-home"
    adapter_home.mkdir()

    env = dict(base_env)
    env.update(
        {
            "SOFIA_CONFORMANCE_BINARY": str(sofia_binary),
            "SOFIA_CONFORMANCE_HOME": str(adapter_home),
            "SOFIA_CONFORMANCE_ADAPTER_REPORT": str(adapter_report),
        }
    )
    command = [
        *conformance_command,
...
def run_official_mode(
    *,
    ...
    launcher = _make_adapter_launcher(
        adapter_script,
        require_automatic_auth=base_env.get("SOFIA_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH")
        == "1",
    )
    try:
        return [
