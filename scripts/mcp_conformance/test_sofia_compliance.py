...
def test_registration_commands_select_stdio_then_http_shapes() -> None:
    ...
    assert stdio == [
        ...
        TEST_SERVER_NAME,
        "--env",
        f"SOFIA_MCP_PROTOCOL_VERSION={MODERN_VERSION}",
        "--",
        sys.executable,
        "/src/server.py",
        "--mode",
    ...
    valid, _ = _validate_registration(
        {
            ...
            "transport": {
                ...
                "env": {"SOFIA_MCP_PROTOCOL_VERSION": MODERN_VERSION},
            },
        },
        transport="stdio",
        mode=MODERN_VERSION,
...
def test_isolated_environment_drops_model_credentials(
    ...
    monkeypatch.setenv("SOFIA_API_KEY", "secret")
    monkeypatch.setenv("SOFIA_ACCESS_TOKEN", "secret")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")

    env = _isolated_environment(tmp_path)

    assert env["SOFIA_HOME"] == str(tmp_path)
    assert "SOFIA_API_KEY" not in env
    assert "SOFIA_ACCESS_TOKEN" not in env
...
def test_scenario_ratio_includes_adapter_failures() -> None:
    checks = [
        ...
        CheckResult(
            "harness/auth/example/sofia-adapter",
            False,
            "adapter failed",
            source="harness",
            scenario="auth/example",
...
def test_official_case_preserves_adapter_check_identity_on_failure_and_success(
    ...
    case = _run_official_case(
        Path("/opt/sofia"),
        Path("/src/sofia_conformance_adapter.py"),
    ...
    assert case.checks == [
        CheckResult(
            name=f"harness/{scenario}/sofia-adapter",
            success=adapter_success,
...
def test_official_case_keeps_cimd_registration_checks_in_regression_gated_http_case(
    ...
    assert auto_env.get("SOFIA_CONFORMANCE_CLIENT_REGISTRATION") is None
    assert forced_env["SOFIA_CONFORMANCE_CLIENT_REGISTRATION"] == "cimd"
    ...
    assert gate["fixedChecks"] == [
        {
            ...
            "source": "harness",
            "scenario": scenario,
            "check_id": f"harness/{scenario}/sofia-adapter",
        },
        {
            "mode": MODERN_VERSION,
            "transport": "official-http",
...
def test_regression_cli_accepts_a_baseline_report() -> None:
    args = _parse_args(
        ["/opt/sofia", "--baseline-report", "/tmp/sofia-mcp-baseline.json"]
    )

    assert args.baseline_report == Path("/tmp/sofia-mcp-baseline.json")
...
def test_regression_cli_extracts_compact_baseline_without_starting_conformance(
    ...
    sofia = tmp_path / "sofia"
    sofia.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
    sofia.chmod(0o755)
...
def test_regression_cli_rejects_extraction_without_a_source_report(
    ...
    assert main([str(sofia), "--extract-baseline", str(tmp_path / "compact.json")]) == 2
...
def test_regression_cli_rejects_malformed_baselines(
    ...
    assert main([str(sofia), "--baseline-report", str(baseline)]) == 2
...
def test_regression_cli_rejects_missing_baseline_before_starting_conformance(
    ...
    assert main([str(sofia), "--baseline-report", str(tmp_path / "missing.json")]) == 2
