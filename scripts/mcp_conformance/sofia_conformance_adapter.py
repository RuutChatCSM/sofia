#!/usr/bin/env python3
"""Adapt one official MCP client scenario to Sofia app-server requests."""
...
CLIENT_REGISTRATION_OVERRIDE_ENV_VAR = "SOFIA_CONFORMANCE_CLIENT_REGISTRATION"
...
def _elicitation_content(
    ...
    if scenario == "elicitation-sep1034-client-defaults":
        # The official scenario deliberately supplies no values. Sofia, as the
...
def _validated_callback_url(authorization_url: str, location: str) -> str:
    ...
    if (
        ...
        raise AdapterFailure(
            "authorization server redirect did not target Sofia's exact loopback callback"
        )
...
def _drive_headless_authorization(
    ...
    *,
    ...
    if not 200 <= callback_status < 500:
        raise AdapterFailure("Sofia OAuth callback endpoint returned an invalid status")
...
def _validate_oauth_secret_not_persisted(sofia_home: Path, client_secret: str) -> None:
    if not client_secret:
        return

    candidates = [sofia_home / "config.toml", *sofia_home.rglob(".credentials.json")]
    ...
    for candidate in dict.fromkeys(candidates):
        ...
        if secret_bytes in candidate.read_bytes():
            raise AdapterFailure(
                ...
                f"{candidate.relative_to(sofia_home)}"
            )
    ...
    *,
    ...
    if scenario == "auth/scope-step-up":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return "Sofia automatically recovered from the challenged OAuth scope"
        try:
            _auth_tool_call(client, workspace)
    ...
    if scenario == "auth/scope-retry-limit":
        if require_automatic_auth:
            ...
            try:
                ...
                return "observed Sofia's production OAuth retry-limit behavior"
            raise AdapterFailure(
    ...
    if scenario == "auth/authorization-server-migration":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return (
                "Sofia automatically registered with the migrated authorization server"
            )
        try:
            _auth_tool_call(client, workspace)
        except AdapterFailure:
...
def run_adapter(server_url: str) -> dict[str, object]:
    sofia_binary = _required_path("SOFIA_CONFORMANCE_BINARY")
    sofia_home = _required_path("SOFIA_CONFORMANCE_HOME")
    scenario = _required_env("MCP_CONFORMANCE_SCENARIO")
    protocol_version = _required_env("MCP_CONFORMANCE_PROTOCOL_VERSION")
    timeout_seconds = float(os.environ.get("SOFIA_CONFORMANCE_TIMEOUT", "30"))
    enable_modern_feature = (
        os.environ.get("SOFIA_CONFORMANCE_ENABLE_MODERN_FEATURE", "1") != "0"
    )
    require_automatic_auth = (
        os.environ.get("SOFIA_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH", "0") == "1"
    )
    context = _conformance_context()

    sofia_home.mkdir(parents=True, exist_ok=True)
    workspace = sofia_home / "workspace"
    workspace.mkdir()
    env = _isolated_environment(sofia_home)
    # Scenario context can contain ephemeral OAuth client secrets. The adapter
    # consumes it directly and does not expose the full blob to Sofia.
    ...
    try:
        if scenario.startswith("auth/"):
            config_path = sofia_home / "config.toml"
        ...
        if scenario.startswith("auth/"):
            ...
            if not registered:
                raise AdapterFailure("sofia mcp add failed")
        ...
        if not registration_ok:
            raise AdapterFailure("Sofia registration did not preserve the scenario URL")
            ...
            if scenario.startswith("auth/"):
                ...
                if scenario == "auth/pre-registration" and isinstance(
                    ...
                    _validate_oauth_secret_not_persisted(
                        sofia_home, oauth_client_secret
                    )
                    steps.append(
                        Step(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report_path_value = os.environ.get("SOFIA_CONFORMANCE_ADAPTER_REPORT")
