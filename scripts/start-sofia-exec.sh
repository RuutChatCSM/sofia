...
remote_path='~/code/sofia-sync'
local_exec_server_port="${SOFIA_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${SOFIA_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/sofia-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/sofia-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/sofia-sync"
remote_codex_rs="$remote_repo_root/sofia-rs"

cd "${remote_codex_rs}"
cargo build -p sofia-cli --bin sofia

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/sofia exec-server --listen ws://127.0.0.1:0 \
...
echo "Start sofia via: "
printf '  SOFIA_EXEC_SERVER_URL=ws://127.0.0.1:%s sofia -C %q\n' \
