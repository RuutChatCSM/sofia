...
work_root="$(mktemp -d "${temp_root%/}/sofia-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/sofia-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${SOFIA_WRAPPER_LOG:?missing SOFIA_WRAPPER_LOG}"
printf '%s\n' "$@" > "$SOFIA_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
SOFIA_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" sofia-zsh)
