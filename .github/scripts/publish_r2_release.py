...
objects are verified, the same metadata advances ``sofia/channels/latest`` when
the release is marked latest and ``sofia/channels/prerelease`` for prereleases.
Stable releases also update the mutable ``sofia/install.sh`` and
``sofia/install.ps1`` bootstrap aliases from their verified versioned assets.
"""
