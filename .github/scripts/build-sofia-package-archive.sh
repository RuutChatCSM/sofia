...
usage() {
  cat <<'EOF'
Usage: build-sofia-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--sofia-command-runner-bin <path>] \
  [--sofia-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --sofia-command-runner-bin)
      resource_args+=(
        --sofia-command-runner-bin
        "${2:?--sofia-command-runner-bin requires a value}"
      )
    ...
    --sofia-windows-sandbox-setup-bin)
      resource_args+=(
        --sofia-windows-sandbox-setup-bin
        "${2:?--sofia-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="sofia"
    entrypoint="sofia"
    archive_stem="sofia-package"
    ;;
  app-server)
    variant="sofia-app-server"
    entrypoint="sofia-app-server"
    archive_stem="sofia-app-server-package"
    ;;
  *)
    echo "No Sofia package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/sofia-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/sofia-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/sofia-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--sofia-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--sofia-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_sofia_package.py"
  --target "$target"
  --variant "$variant"
