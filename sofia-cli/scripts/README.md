...
./scripts/stage_npm_packages.py \
  ...
  --package sofia \
  --package sofia-responses-api-proxy \
  --package sofia-sdk
```
...
When `--package sofia` is provided, the staging helper builds the lightweight
`@ruut/sofia` meta package plus all platform-native `@ruut/sofia` variants
