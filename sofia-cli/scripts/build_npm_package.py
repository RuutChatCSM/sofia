#!/usr/bin/env python3
"""Stage and optionally package the @ruut/sofia npm module."""
...
SOFIA_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = SOFIA_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "sofia-rs" / "responses-api-proxy" / "npm"
SOFIA_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"
SOFIA_NPM_NAME = "@ruut/sofia"
SOFIA_PACKAGE_COMPONENT = "sofia-package"

# `npm_name` is the local optional-dependency alias consumed by `bin/sofia.js`.
# The underlying package published to npm is always `@ruut/sofia`.
SOFIA_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "sofia-linux-x64": {
        "npm_name": "@ruut/sofia-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "sofia-linux-arm64": {
        "npm_name": "@ruut/sofia-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "sofia-darwin-x64": {
        "npm_name": "@ruut/sofia-darwin-x64",
        "npm_tag": "darwin-x64",
    ...
    "sofia-darwin-arm64": {
        "npm_name": "@ruut/sofia-darwin-arm64",
    ...
    "sofia-win32-arm64": {
        "npm_name": "@ruut/sofia-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "sofia": ["sofia", *SOFIA_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "sofia": [],
    "sofia-linux-x64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-linux-arm64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-darwin-x64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-darwin-arm64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-win32-x64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-win32-arm64": [SOFIA_PACKAGE_COMPONENT],
    "sofia-responses-api-proxy": ["sofia-responses-api-proxy"],
    "sofia-sdk": [],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in SOFIA_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build or stage the Sofia CLI npm package."
    )
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="sofia",
        help="Which npm package to stage (default: sofia).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "sofia":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/sofia.js --version\n"
                    f"    node {staging_dir_str}/bin/sofia.js --help\n\n"
                )
            elif package == "sofia-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/sofia-responses-api-proxy.js --help\n\n"
                )
            elif package in SOFIA_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="sofia-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "sofia":
        ...
        shutil.copy2(SOFIA_CLI_ROOT / "bin" / "sofia.js", bin_dir / "sofia.js")

        readme_src = REPO_ROOT / "README.md"
        if readme_src.exists():
            shutil.copy2(readme_src, staging_dir / "README.md")

        package_json_path = SOFIA_CLI_ROOT / "package.json"
    elif package in SOFIA_PLATFORM_PACKAGES:
        platform_package = SOFIA_PLATFORM_PACKAGES[package]
        ...
        with open(SOFIA_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            sofia_package_json = json.load(fh)

        package_json = {
            "name": SOFIA_NPM_NAME,
            "version": platform_version,
            "license": sofia_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": sofia_package_json.get("repository"),
        }

        engines = sofia_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = sofia_package_json.get("packageManager")
        ...
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = (
            RESPONSES_API_PROXY_NPM_ROOT / "bin" / "sofia-responses-api-proxy.js"
        )
        shutil.copy2(launcher_src, bin_dir / "sofia-responses-api-proxy.js")
    ...
    elif package == "sofia-sdk":
        package_json_path = SOFIA_SDK_ROOT / "package.json"
    ...
    if package == "sofia":
        package_json["files"] = ["bin/sofia.js"]
        package_json["optionalDependencies"] = {
            SOFIA_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{SOFIA_NPM_NAME}@"
                f"{compute_platform_package_version(version, SOFIA_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["sofia"]
            if platform_package != "sofia"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[SOFIA_NPM_NAME] = version
...
def stage_codex_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("sofia-sdk build did not produce a dist directory.")
...
def copy_native_binaries(
    ...
    for target_dir in vendor_src.iterdir():
        ...
        if SOFIA_PACKAGE_COMPONENT in components_set:
        ...
        for component in sorted(components_set - {SOFIA_PACKAGE_COMPONENT}):
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="sofia-npm-pack-") as pack_dir_str:
