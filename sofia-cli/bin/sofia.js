#!/usr/bin/env node
// Unified entry point for the Sofia CLI.
...
const PLATFORM_PACKAGE_BY_TARGET = {
  "x86_64-unknown-linux-musl": "@ruut/sofia-linux-x64",
  "aarch64-unknown-linux-musl": "@ruut/sofia-linux-arm64",
  "x86_64-apple-darwin": "@ruut/sofia-darwin-x64",
  "aarch64-apple-darwin": "@ruut/sofia-darwin-arm64",
  "x86_64-pc-windows-msvc": "@ruut/sofia-win32-x64",
  "aarch64-pc-windows-msvc": "@ruut/sofia-win32-arm64",
};
...
function findCodexExecutable() {
  ...
  const updateCommand =
    packageManager === "bun"
      ? "bun install -g @ruut/sofia@latest"
      : packageManager === "pnpm"
        ? "pnpm add -g @ruut/sofia@latest"
        : packageManager === "vite-plus"
          ? "vp install -g @ruut/sofia@latest"
          : "npm install -g @ruut/sofia@latest";
  throw new Error(
    `Missing optional dependency ${platformPackage}. Reinstall Sofia: ${updateCommand}`,
  );
}
...
function isPnpmOwnedCodexInstall(nodeModulesDir) {
  ...
  try {
    return (
      realpathSync(path.join(nodeModulesDir, "@ruut", "sofia")) ===
      codexPackageRoot
    );
  } catch {
    return false;
...
function isVitePlusOwnedCodexInstall(packagesDir) {
  ...
  try {
    const metadata = JSON.parse(
      readFileSync(path.join(packagesDir, "@ruut", "sofia.json"), "utf8"),
    );
    if (metadata.name !== "@ruut/sofia") {
      return false;
    }

    // Vite+ records the active global installation in packages/@ruut/sofia.json.
    ...
    // Missing or unreadable ownership metadata must not prevent Sofia starting.
  }
  return false;
}

/**
 * Use heuristics to detect the package manager that was used to install Sofia
...
const packageManagerEnvVar =
  packageManager === "bun"
    ? "SOFIA_MANAGED_BY_BUN"
    : packageManager === "pnpm"
      ? "SOFIA_MANAGED_BY_PNPM"
      : packageManager === "vite-plus"
        ? "SOFIA_MANAGED_BY_VITE_PLUS"
        : "SOFIA_MANAGED_BY_NPM";
const env = {
  ...process.env,
  SOFIA_MANAGED_PACKAGE_ROOT: codexPackageRoot,
};
delete env.SOFIA_MANAGED_BY_NPM;
delete env.SOFIA_MANAGED_BY_BUN;
delete env.SOFIA_MANAGED_BY_PNPM;
delete env.SOFIA_MANAGED_BY_VITE_PLUS;
