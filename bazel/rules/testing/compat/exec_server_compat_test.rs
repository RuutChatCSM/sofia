...
use sofia_exec_server_test_support::relay::TEST_TIMEOUT as VERSION_SKEW_TIMEOUT;
use sofia_exec_server_test_support::relay::accept_websocket;
use sofia_exec_server_test_support::relay::assert_relay_data_is_encrypted;
use sofia_exec_server_test_support::relay::proxy_relay_frames;
use sofia_exec_server_test_support::relay::registered_executor_public_key;
...
const RELEASED_CODEX_ENV_VAR: &str = "SOFIA_TEST_RELEASED_CODEX";
const CURRENT_CODEX_ENV_VAR: &str = "SOFIA_TEST_CURRENT_CODEX";
const EXECUTOR_MARKER_ENV_VAR: &str = "SOFIA_EXECUTOR_VERSION_SKEW_MARKER";
...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    ...
    let current = std::env::var_os(CURRENT_CODEX_ENV_VAR)
        .with_context(|| format!("{CURRENT_CODEX_ENV_VAR} must name the current Sofia binary"))?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current Sofia does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released Sofia does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(sofia_home.path()).await?;
...
"#
    ...
    let mut executor = Command::new(executor_binary)
        ...
        .current_dir(sofia_home.path())
        .env("SOFIA_HOME", sofia_home.path())
        .env("SOFIA_API_KEY", REGISTRY_TOKEN)
    ...
    let mut app_server = Command::new(app_binary)
        ...
        .env(SOFIA_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR, &registry_url)
        .env(
            SOFIA_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR,
            ENVIRONMENT_ID,
        )
        .env(SOFIA_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR, REGISTRY_TOKEN)
        .env_remove(SOFIA_EXEC_SERVER_URL_ENV_VAR)
    ...
    assert_eq!(
        std::fs::read_to_string(sofia_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );
...
async fn mock_model(sofia_home: &Path) -> Result<MockServer> {
