"""Bazel module extension for pinned Sofia release archives."""
...
def _codex_release_repository_impl(repository_ctx):
    asset = "sofia-package-x86_64-unknown-linux-musl.tar.gz"
    version = repository_ctx.attr.version
    repository_ctx.download_and_extract(
        url = [
            "https://releases.openai.com/sofia/releases/{}/{}".format(version, asset),
            "https://github.com/openai/codex/releases/download/rust-v{}/{}".format(version, asset),
        ],
        sha256 = repository_ctx.attr.sha256,
    )
    manifest = json.decode(repository_ctx.read("sofia-package.json"))
    ...
    for module in module_ctx.modules:
        for release in module.tags.release:
            _codex_release_repository(
                name = "sofia_release_{}_linux_x86_64".format(release.version),
...
sofia_release_archive = module_extension(
